...
Trace işlemi API Proxy bazında yapılır. Log satırına tıkladığınızda sağdan trace detay çekmesi açılır; zaman dağılımı, Client–Gateway–API haritası ve politika öncesi/sonrası paneller burada yer alır. Tüm adımlar için [Adım Adım İzleme](/tr/develop/izleme-analiz/adim-adim-izleme) sayfasına bakabilirsiniz.
...
:::note[Davranış düzeltmesi]

Koşul kutusu **OR** olan bir Custom Query filtresi artık yalnızca gerçekten eşleşen istekleri trace kaydına alır; koşulunda **birden fazla** koşulu **NOT** olarak birleştiren bir filtre de artık koşulların **tamamını** birlikte terslendirir. Bu sürümden önce OR'lu bir filtre çocuklarından bağımsız her isteği yakalıyor, birden çok koşullu NOT filtresinde ise yalnızca son koşul terslenip diğerleri yok sayılıyordu — sonuç, Analitik > Advanced Search'te aynı sorguyla görülen sonuçla artık birebir örtüşüyor. **AND** ve tek koşullu **NOT** filtrelerinin davranışı değişmedi. Bu filtreleri kullanan kurulumlarda, düzeltmeden sonra trace'e düşen kayıt sayısı azalabilir — bu, filtrenin artık doğru çalıştığının göstergesidir.
...
<Tabs>
  <TabItem value="detayli-goruntuleme" label="Detaylı Görüntüleme">
  ...
  <TabItem value="json-formatinda-goruntuleme" label="JSON Formatında Görüntüleme">
...
:::
  ...
  <TabItem value="i-ndirme" label="İndirme">
...
:::
  ...
  <TabItem value="hizli-test" label="Hızlı Test">
...
Tabloda bir satıra tıkladığınızda veya **Seç** ile kaydı seçtiğinizde **sağdan trace detay çekmecesi** açılır. Üstte **zaman dağılımı** ve **Client → Gateway → API** işlem haritası yer alır; haritada istek hattı (üst sıra) ve yanıt hattı (alt sıra) üzerindeki politika düğümleri, varsa [API Proxy Grup](/tr/concepts/temel-kavramlar/api-proxy-group) politikaları da dahil olmak üzere işletilme sırasına göre gösterilir.

Düğümlerde başarı için yeşil onay, hata veya blok için **!**, atlanan politika için soluk görünüm ve **S** rozeti kullanılır. **‹ Back** / **Next ›** veya doğrudan düğüme tıklayarak adımlar arasında gezersiniz. Ayrıntılı anlatım için [Adım Adım İzleme](/tr/develop/izleme-analiz/adim-adim-izleme) sayfasına bakın.

<Tabs>
  <TabItem value="basarili-akis" label="Başarılı Akış">
    Tüm adımların başarılı işlendiği örnek: çekmede zaman çubuğu, harita ve **Client** adımında istek/yanıt panelleri.
      ...
      <img 
        ...
        alt="Trace çekmecesi: başarılı akışta zaman dağılımı, harita ve Client adımı"
        width="1000"
      />

  </TabItem>
  
  <TabItem value="basarisiz-akis" label="Başarısız Akış">
...
:::
      ...
      <img 
        ...
        alt="Trace çekmecesi: başarısız istek, haritada uyarı ve 404 fault yanıtı"
        width="1000"
      />
  </TabItem>
</Tabs>
...
<img src="/images/develop/izleme-analiz/details-2.png"
     alt="Trace — Cache policy selected, Cache Execution Details button"
     width="1000"/>

<img src="/images/develop/izleme-analiz/details-modal.png"
     alt="Cache LOOKUP Execution Details modal"
     width="1000"/>
...
Bir log satırı seçildiğinde açılan çekmece, mesaj akışının başarılı olup olmadığını harita ve üst bilgilerle özetler; seçilen düğüme göre alt panelde tablolar ve gövdeler gösterilir. Genel görünüm örneği — harita, özet satırı ve istek başlıkları:
  ...
  <img
    ...
    alt="Trace çekmecesi: harita ve gateway giriş özeti"
    width="1000"
  />

Aşağıda aynı çekmece içinde sık kullanılan adımlar ve önerilen görseller özetlenmiştir (tam akış [Adım Adım İzleme](/tr/develop/izleme-analiz/adim-adim-izleme) sayfasındadır).



  <Accordion title="İstemci (Client) adımı">
    Haritada **Client** düğümü seçildiğinde sol sütunda istemciden gelen istek (**HTTP Info**, başlıklar, parametreler, gövde), sağ sütunda istemciye giden yanıt görüntülenir.
      ...
      <img 
        src="/images/develop/izleme-analiz/trace-drawer-step-client.png" 
        alt="Trace çekmecesi: Client adımı istek ve yanıt"
        width="1000"
      />

  </Accordion>

  <Accordion title="Politika adımı (Before / After)">
    Haritada bir politika düğümüne tıkladığınızda yürütme bilgisi ve **Before** / **After** akordeonları ile mesajın politika öncesi ve sonrası karşılaştırılır.
      ...
      <img 
        ...
        alt="Trace çekmecesi: politika Before ve After"
        width="1000"
      />

  </Accordion>

  <Accordion title="Backend API ve yönlendirme">
    **Hedef (API)** özeti, **Routing** tablosu (genişletilmiş satır; yönlendirme için bkz. [HTTP Yönlendirme](/tr/develop/yonlendirme/http-yonlendirme)) ve **hedeften gelen yanıt** sırayla aşağıdaki görsellerle örneklenir.

      <img 
        src="/images/develop/izleme-analiz/trace-drawer-step-target-backend.png" 
        alt="Trace çekmecesi: backend hedef özeti"
        width="1000"
      />

      <img 
        src="/images/develop/izleme-analiz/trace-drawer-step-routing-table.png" 
        alt="Trace çekmecesi: routing tablosu"
        width="1000"
      />

      <img 
        src="/images/develop/izleme-analiz/trace-drawer-step-response-from-target.png" 
        alt="Trace çekmecesi: hedeften yanıt"
        width="1000"
      />
