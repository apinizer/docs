...
<img
  src="/images/2026-09-2/dashboard-overview.png"
  alt="Pano — özet kartlar, Traffic/Response Time grafikleri, Most Active Endpoints/Clients ve Routing Diagnostics paneli"
  width="1000"
  height="949"
/>
...
Apinizer Yönetim Konsolunda API Trafiğinin sorgulanması ve analitik olarak görselleştirilmesi ancak ilgili ortama [Elasticsearch Konnektörü](/tr/integrations/baglanti-yonetimi/elasticsearch-monitor) eklenmesi ile mümkün olmaktadır.

:::

:::info

Bir API Proxy birden fazla [Ortam](/tr/concepts/temel-kavramlar/ortam-nedir)'a yüklenebildiği için metrikler, **ortam bazlı sorgulanmaktadır**. Sayfanın üst kısmından ortam seçimi yapılmalıdır. **Administration > Analytic > Dashboard** yolunda ek **Project** filtresi ile tüm projeler veya seçili projeler kapsanır; proje menüsündeki pano yalnızca aktif projeyi kapsar.
...
Sayfanın alt bölümünde yönlendirme teşhis özeti yer alır. Bölümün ilk satırında, panelin ne gösterdiğini özetleyen kalıcı bir açıklama bulunur; her kartın başlığındaki bilgi simgesinin üzerine gelindiğinde o kartın nasıl okunacağı da görüntülenir.

- **Failure Reason Distribution** — hata nedenlerinin dağılımı (ör. Upstream HTTP Error); grafiğin dikey ekseni **istek sayısını** gösterir
- **Phase Latency (p50/p95/p99)** — Upstream Selection, DNS, TTFB, Body Read, Pool Wait faz gecikmeleri; grafiğin dikey ekseni **milisaniye (ms)** gösterir
- **Top Failing Proxies** — seçilen pencerede en çok hata veren API Proxy'ler; tablo **API Proxy** ve **Hata Sayısı** kolonlarıyla listelenir

Ortalama süreler ve en sık kullanılan upstream adresleri de birimleriyle (ms, istek) birlikte gösterilir. Seçilen aralıkta kayıt yoksa ilgili kart, panelin ne zaman boş kaldığını anlatan "Nasıl okunur?" yönlendirmesini gösterir.

Tekil istek düzeyinde aynı sinyaller için [Ağ Geçidi Trafiği](/tr/analytic/api-trafigi) detay penceresindeki **Yönlendirme Teşhisi** bölümüne bakabilirsiniz.
...
2. [Ağ Geçidi Trafiği](/tr/analytic/api-trafigi) sayfasından hatalı istekleri filtreleyin
...
3. [Ağ Geçidi Trafiği](/tr/analytic/api-trafigi) ile şüpheli istekleri inceleyin
...
<CardGroup cols={2}>
  ...
  <Card title="Detaylı Analiz Araçlarını Kullanın" icon="magnifying-glass-chart">
    ...
    - [Ağ Geçidi Trafiği](/tr/analytic/api-trafigi) ile detaylı inceleyin
...
<CardGroup cols={2}>
  ...
  <Card title="Analytics Engine" icon="chart-pie" href="/tr/concepts/temel-bilesenler/analytics-engine">
