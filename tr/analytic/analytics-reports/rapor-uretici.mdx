...
| **Ağ Geçidi Trafiği (Zamana Göre)** | Ağ geçidi trafiği belirli zaman dilimlerine (saatlik, günlük vb.) göre gruplanarak analiz edilir. |
| **Ağ Geçidi Trafiği Metrikleri** | API Proxy bazında trafik hacmi, başarı/hata oranları gibi metrikler raporlanır. |
...
| **Aralık** | Ağ Geçidi Trafiği (Zamana Göre) | Trafik verilerinin gruplanacağı zaman dilimi: saniyelik, dakikalık, saatlik, günlük, haftalık, aylık, üç aylık veya yıllık. |
...
| **Değer birimi** | Ağ Geçidi Trafiği (Zamana Göre) | Değerlerin *İstek Sayısı* (her kovadaki ham sayı) olarak mı yoksa *TPS (istek/saniye)* olarak mı raporlanacağı. TPS her kovayı gerçek süresine böler; farklı uzunluktaki kovalar ancak böyle karşılaştırılabilir. TPS seçildiğinde çıktıya **TPS** sütunu eklenir. |
...
| **API Proxy** | Ağ Geçidi Trafiği Metrikleri | Raporun yalnızca belirli bir proxy üzerinde çalışmasını sağlar. |
