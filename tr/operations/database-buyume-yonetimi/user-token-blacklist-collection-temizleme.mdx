---
title: "user_token_blackList Koleksiyonunun Temizlenmesi"
description: "Kullanıcıların çoklu giriş yapmaları durumunda tutulan user_token_blackList koleksiyonunda depolanan kayıtlardan belirtilen zaman dilimine göre eski kayıtların temizlenmesini sağlar."
---
...
Bu koleksiyondaki kayıt, çıkış yapılmış bir oturumun aynı token ile sürdürülmesini engelleyen kayıttır. Bir kaydın, token'ının kendi süresi dolmadan önce silinmesi o token'ı yeniden geçerli kılar.

Yalnızca kurulumun üretebileceği en uzun ömürlü token'dan eski kayıtları silin: 24 saat, kullanıcılar **Beni Hatırla** ile giriş yapıyorsa 30 gün. **Uygulama Logu Temizleme Görevleri** ekranındaki yerleşik temizleme bu alt sınırı zaten uygular; elle çalıştırılan script uygulamaz.

:::

## Kayıt Temizleme Scripti

Aşağıdaki script, MongoDB'deki `user_token_blackList` koleksiyonunda depolanan kayıtlardan belirtilen saat öncesine kadar olan kayıtları silmek için kullanılır. Bu işlem ile veritabanı temizlenerek depolama alanı optimize edilir.

```bash
#!/bin/bash  değerini analiz edip milisaniye cinsinden dönüştürme # MongoDB sunucusuna bağlanmak için gerekli bilgiler
...
2. Dosyaya çalıştırma yetkisi verilir: `chmod +x purge_user_token_blacklist_collection.sh`
3. Dosya çalıştırılır: `./purge_user_token_blacklist_collection.sh`
...
```bash
chmod +x /path/to/purge_user_token_blacklist_collection.sh
./path/to/purge_user_token_blacklist_collection.sh &
```
...
<Steps>
  ...
  <Step title="Cron Job Ekleme">
    ...
    ```bash
    59 23 * * * /path/to/purge_user_token_blacklist_collection.sh > /path/to/logfile.log 2>&1
    ```
  </Step>
  <Step title="Kaydetme">
