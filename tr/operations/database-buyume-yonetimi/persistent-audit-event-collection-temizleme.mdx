---
title: "persistent_audit_event Koleksiyonunun Temizlenmesi"
description: "API Manager giriş denemesi kayıtlarının depolandığı persistent_audit_event koleksiyonunda depolanan kayıtlardan belirtilen zaman dilimine göre eski kayıtların temizlenmesini sağlar."
---

<Warning>
  Giriş logları her zaman veritabanına kaydedilir ve Temizleme Görevleri aracılığıyla otomatik olarak silinemez. Bu manuel scripti sadece kesinlikle gerekli olduğunda kullanın.
</Warning>
...
Aşağıdaki script, MongoDB'deki `persistent_audit_event` koleksiyonunda depolanan kayıtlardan belirtilen saat öncesine kadar olan kayıtları silmek için kullanılır. Bu işlem ile veritabanı temizlenerek depolama alanı optimize edilir.
...
PASSWORD="<PASSWORD>"
AUTH_DB="admin"
TARGET_COLLECTION="persistent_audit_event"
TARGET_LOG_LOCATION=$(pwd)/purge_persistent_audit_event_collection.log
...
2. Dosyaya çalıştırma yetkisi verilir: `chmod +x purge_persistent_audit_event_collection.sh`
3. Dosya çalıştırılır: `./purge_persistent_audit_event_collection.sh`
...
```bash
chmod +x /path/to/purge_persistent_audit_event_collection.sh
./path/to/purge_persistent_audit_event_collection.sh &
```
...
<Steps>
  ...
  <Step title="Cron Job Ekleme">
    ...
    ```bash
    59 23 * * * /path/to/purge_persistent_audit_event_collection.sh > /path/to/logfile.log 2>&1
    ```
  </Step>
  <Step title="Kaydetme">
