...
### Senaryo 5: Yönetim ekranı "undeployed" gösteriyor ama API hâlâ cevap veriyor

:::warning

**Belirtiler:**
- API Proxy ekranında environment "undeployed" görünüyor, ancak Gateway üzerinden aynı adrese istek atıldığında `200` dönüyor
- Manager'da yapılan ayar değişiklikleri (backend adresi, politika, cache süresi) çağrılara hiç yansımıyor
- "Redeploy gerekli" uyarısı hiç çıkmıyor

:::

Bu durumda Gateway Worker, proxy'yi deploy edildiği andaki yapılandırma anlık görüntüsüyle sunmaya devam eder; Manager'da sonradan yapılan değişiklikler ona ulaşmaz.

**Tespit:**
- En doğrudan yol: Management API'nin `route-health` ucunu ilgili environment için çağırın. Yanıttaki `inconsistent` alanı `true` ise Manager kaydı ile Worker gerçekliği ayrışmış demektir; `deployedInManager=false` + `routeActive=true` birleşimi tam olarak bu durumdur
- Elle doğrulamak isterseniz: Manager'da "undeployed" görünen proxy'nin Gateway adresine istek atın; `404` yerine `200` dönüyorsa aynı sonuç. Adresi tahmin etmeyin, `invoke-info` ucundan okuyun — bu uç, deploy edilmemiş görünen kayıtlar için de gerçek çağrı adresini döndürür

**Çözüm:**
- API Proxy hâlâ duruyorsa: ilgili environment'a **yeniden deploy** edin (yapılandırma tazelenir) veya açıkça **undeploy** edin (route anında kalkar)
- API Proxy silinmişse: route'u yalnızca ilgili Gateway Worker pod'unun yeniden başlatılması temizler; Worker açılışta deploy kayıtlarını veritabanından yeniden yükler

:::tip

Bu tutarsızlığı üreten bilinen nedenler (geçersiz environment adıyla güncelleme, üzerine yazan içe aktarma, promotion güncellemesi) giderilmiştir; ayrıca API Proxy silme işlemi artık deploy kaydına bakmaksızın tüm çalışan environment'lara undeploy gönderir. Eski sürümlerden kalan tutarsızlıklar yukarıdaki adımlarla temizlenir.
