...
<Steps>
  <Step title="Write Numbers as Strings Seçeneğini Etkinleştirin">
  ...
  <Step title="Örnek Yanıtı Doğrulayın">
...
- `;trustServerCertificate=true` (önerilen geçici çözüm; dikkatle eklenmeli, güvenlik açısından olası sonuçları göz önünde bulundurulmalıdır)
- Bazı ortamlarda ek olarak `;encrypt=true` de gerekebilir
...
<Steps>
  <Step title="Connection String Parametresini Ekleyin">
...
<Steps>
  <Step title="Cache Replicasını 0'a Ayarlayıp Tekrar Yükseltme">
  ...
  <Step title="Eski ReplicaSet Tanımını Silme">
  ...
  <Step title="DeploymentStrategy'i Recreate'e Çevirme">
...
## Application Logs Aşırı Üretimi ve Eşik Bildirimleri

#### Problem

Application Logs kayıtları beklenenin üzerinde artmakta ve aşağıdaki bildirim alınmaktadır:
...
Bu durum birden fazla nedenden kaynaklanabilir.
  
  ### Debug ihtiyacı için API Proxy'nin Log Level'ının değiştirilmesi
    Application Logs sekmesinde debug seviyesi açık bırakıldığında log hacmi artar. Apinizer bu seviyeyi otomatik olarak kapatmaz.

  ### ERROR seviyesinde yoğun log üretimi
    Loglar `ERROR` seviyesinde olmasına rağmen kayıt sayısı yüksekse sistemde bir sorun olabilir ve bu loglar incelenmelidir. Gerçek hata bildirimleri göz ardı edilmemelidir.


:::info

Application Logs ayar ekranında anlık durum değil, sayfadan kaydedilen son ayar görüntülenmektedir.
...
Log seviyesinin yönetici tarafından doğrudan `ERROR`'e çekilmesi, ilgili yazılım ekiplerinin çalışma sırasında beklediği logların kaçırılmasına neden olabilir. Bu nedenle öncelikle ilgili ekiplerin bilgilendirilmesi gerekmektedir.

:::

Loglar `ERROR` dışında bir seviyedeyse Application Logs sekmesinden seviye `ERROR`'e çekilebilir. Her pod başlangıcında varsayılan olarak sabitlemek için worker deployment dosyasına aşağıdaki parametre eklenebilir:

```yaml
logLevel: ERROR
```

Detaylı bilgi için [Gateway Ayarları — Ek Değişkenler](/tr/setup/gateway-ayarlari#ek-değişkenler-additional-variables-1) sayfasına bakılabilir.

<Steps>
  <Step title="Eski Logları Düzenli Temizleyin">
    Bu logların 3 günde bir temizlenmesi önerilmektedir. Bunun için Admin menüsündeki [Uygulama Logu Temizleme Görevleri](/tr/admin/audit-compliance/uygulama-logu-temizleme-gorevleri) sayfasındaki ilgili ayarlar etkin değilse etkinleştirilmeli ve düzenlenmelidir.
  </Step>
  <Step title="Alarm Eşiğini Ortama Göre Ayarlayın">
    Application Logs fazlalığı için varsayılan olarak alert açıktır. Kayıt sayısı 10.000 limitine ulaştığında Apinizer sağ üst köşesinden erişilebilen bildirimler oluşmaktadır. Test ortamında yoğun loglama kullanılıyorsa günlük log alarm eşiği worker için uygun bir seviyeye çekilebilir.
  </Step>
</Steps>

:::note

İleriki versiyonlarda debug seviyesinin varsayılan olarak kısa süreli açılması ve süre sonunda otomatik kapanması planlanmaktadır.
