...
## Trafik Logları Elasticsearch'e Yazılamıyor

### Belirti

API trafik ekranlarında son isteklere ait kayıtlar görünmez ve gateway (worker) uygulama logunda şu satırlar tekrarlanır:

```
ERROR ConnectorElasticSearchIndex - Elasticsearch traffic log could not be indexed for connection [<BAGLANTI_ID>]: ...
WARN  ConnectorElasticSearchIndex - Elasticsearch I/O reactor stopped (...) for connection [<BAGLANTI_ID>]. Client evicted ...
```

### Sebep

Elasticsearch bağlantısı geçici olarak yanıt veremez duruma geldiğinde (yukarıdaki "I/O Reactor Status: STOPPED" durumu, cluster'ın yeniden başlatılması, ağ kesintisi) Apinizer bağlantıyı kapatıp yeniden kurar. Yeniden kurulum sürerken gelen istekler bağlantıyı hazır bulamaz.

### Davranış

- **API çağrıları etkilenmez.** Trafik logunun yazılması istemciye dönen yanıtı geciktirmez veya engellemez.
- **Kayıt kaybolmaz.** İlgili bağlantı için bir Failover Connector tanımlıysa yazılamayan trafik kaydı oraya yazılır. Failover Connector tanımlı değilse kayıt düşer ve log'a bu konuda ayrı bir uyarı satırı basılır.
- **Yeniden kurulum bir soğuma süresine bağlıdır.** Hatadan sonra bağlantı en fazla 10 saniyede bir yeniden kurulur. Bu sayede yoğun trafikte her istek için ayrı bir yeniden kurulum denenmesinden kaynaklanan hata fırtınası oluşmaz; soğuma süresi içindeki denemeler sessizce atlanır ve yalnız DEBUG seviyesinde görünür. Bu nedenle yukarıdaki uyarı satırının 10 saniyede birden sık tekrarlanmaması normaldir.

### Kontrol Adımları

1. Elasticsearch cluster'ının durumunu doğrulayın; bu sayfadaki "I/O Reactor Status: STOPPED" ve "Disk Doluluğu Nedeniyle Log Yazımının Durdurulması" bölümlerini uygulayın.
2. İlgili Elasticsearch bağlantı tanımı için bir Failover Connector seçili olduğundan emin olun; seçili değilse kesinti süresindeki trafik kayıtları saklanmaz.
3. Uyarı satırları Elasticsearch düzeldikten sonra da sürüyorsa bağlantı tanımındaki adres, kimlik bilgileri ve index adını kontrol edin.

### Log Satırında Yığın İzi (Stack Trace) Görünmüyorsa

Java sanal makinesi aynı hatayı çok kez ürettiğinde bir süre sonra yığın izini kısaltır ve log satırında yalnızca hatanın kendisi kalır. Ayrıntılı yığın izine ihtiyaç duyulan teşhis çalışmalarında gateway (worker) uygulaması `-XX:-OmitStackTraceInFastThrow` seçeneği ile başlatılabilir. Bu seçenek yalnızca teşhis amaçlıdır; sorun çözüldükten sonra kaldırılması önerilir.
