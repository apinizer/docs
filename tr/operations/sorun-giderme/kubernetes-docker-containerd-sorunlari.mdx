...
```
  ...
  <Accordion title="Apinizer'ın oluşturduğu pod'larda ndots:1 (musl resolver ve search domain etkisi)">
    ...
    <p><strong>Mevcut kurulumlar:</strong> <code>ndots:1</code> yalnızca deployment'ın yeniden uygulandığı
    işlemlerde (bir ortamı <strong>Yayınla / Yeniden Yayınla (Publish / Republish)</strong> etmek, ya da
    Cache/Integration/Portal/Manager için ilgili "kaydet ve uygula" akışı) eklenir. Salt <strong>Restart</strong>
    işlemi veya kaynak/ortam değişkeni düzenleyip kaydetmek (Republish yapmadan) mevcut deployment'a
    <code>ndots</code> eklemez. Bu sürüme yükseltilen kurulumlarda eski ortamların <code>ndots:1</code> alması
    için ortamın <strong>Yeniden Yayınlanması (Republish)</strong> önerilir.</p>
  </Accordion>
