...
```
  ...
  <Accordion title="Nexus proxy kullanılıyorsa">
    <p><strong>Sebep/Neden:</strong> Kurum Nexus docker-proxy kullanıyorsa node'larda registry yapılandırması gerekir.</p>
    
    <p><strong>Çözüm:</strong> Containerd mirror tanımı, imaj çekme testi ve deployment imaj path güncellemesi için <a href="/tr/operations/container-registry/nexus-kullanimi">Nexus Kullanımı</a> sayfasına bakın. Docker runtime kullanıyorsanız aynı sayfadaki Docker sekmesini uygulayın.</p>
  </Accordion>
...
```
  ...
  <Accordion title="Apinizer'ın oluşturduğu pod'larda ndots:1 (musl resolver ve search domain etkisi)">
    <p><strong>Sebep/Neden:</strong> Yukarıdaki örnekte görülen <code>options ndots:5</code>, Kubernetes'in
    varsayılan davranışıdır: içinde 5'ten az nokta bulunan her isim önce <code>/etc/resolv.conf</code>'taki
    search domain listesi üzerinden denenir. Kısa <em>cluster-içi</em> adresler (örn.
    <code>cache-service.apinizer.svc.cluster.local</code>) bu listeye göre birkaç ekstra sorgu gerektirir; ek
    olarak Alpine tabanlı imajlarda kullanılan <strong>musl</strong> resolver, search listesinde ilk "NODATA"
    yanıtında aramayı bırakır. Sunucunun <code>/etc/resolv.conf</code>'una fazladan bir search domain eklendiğinde
    (yukarıdaki senaryodaki gibi) bu davranış cluster-içi isim çözümlemesini bozabilir.</p>

    <p>Bu sürümden itibaren Apinizer'ın oluşturduğu tüm pod spec'leri (Worker, Cache, Integration, Portal,
    Manager) <code>dnsConfig.options</code> altında <code>ndots:1</code> taşır; <code>dnsPolicy</code>
    değiştirilmez (boş bırakılır, yani Kubernetes varsayılanı <strong>ClusterFirst</strong> geçerli kalır) ve
    nameserver/search listesi override edilmez. <code>ndots:1</code> ile içinde nokta bulunan isimler önce
    doğrudan sorgulanır, search listesine geri dönülmez.</p>

    <p><strong>Etki:</strong> Manager, Cache, Integration ve Portal gibi Alpine/musl tabanlı bileşenlerde
    <code>servis.namespace</code> veya <code>servis.namespace.svc</code> gibi noktalı ama tam olmayan adresler
    artık çözülmeyebilir; yalnızca tek etiketli kısa adlar (örn. <code>servis</code>) search listesi üzerinden
    çözülmeye devam eder. Worker glibc tabanlı bir imaj kullandığı için bu davranıştan etkilenmez. Bu nedenle
    cluster-içi bağlantılarda her zaman <strong>tam FQDN</strong> (örn.
    <code>servis.namespace.svc.cluster.local</code>) kullanılması önerilir; Apinizer kendi oluşturduğu
    cluster-içi adresleri zaten FQDN olarak üretir, elle girilmiş kısa adresler varsa gözden geçirilmelidir.</p>

    <p><strong>Mevcut kurulumlar:</strong> <code>ndots:1</code>, bir ortamı
    <strong>Yayınla / Yeniden Yayınla (Publish / Republish)</strong> ederken her zaman eklenir. Mevcut bir
    deployment'ta ayar eksikse, kaynak (CPU/bellek), güvenlik bağlamı, sağlık probu veya ortam değişkeni
    düzenlemesi de ayarı geri doldurur. Operatörün elle verdiği <code>ndots</code> değeri bu sırada korunur;
    <code>dnsPolicy</code>, nameserver ve search listesi değişmez. Salt <strong>Restart</strong> işlemi veya
    <strong>Yayınla/Yayından Kaldır</strong> (replica ölçekleme) mevcut deployment'a <code>ndots</code> eklemez.
    Bu sürüme yükseltilen kurulumlarda salt Restart almış bir ortam kaldıysa, yukarıdaki düzenlemelerden birini
    uygulayın ya da ortamı <strong>Yeniden Yayınlayın (Republish)</strong>.</p>
  </Accordion>
