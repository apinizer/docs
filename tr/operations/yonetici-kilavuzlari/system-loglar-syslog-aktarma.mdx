...
Uygulama, token, denetim ve giriş loglarının alıcıları **[SIEM ve Log Yönlendirme](/tr/admin/system-settings/siem-log-yonlendirme)** sayfasından yönetilir; her bir alıcı orada bir **hedef** olarak tanımlanır ve ilgili hat (`APPLICATION`, `TOKEN`, `AUDIT`, `SESSION`) hedefin kurallarında açılır. Olay tipi, sonuç, seviye ve kapsam filtreleri de aynı sayfadadır.

Bu sayfadaki alan listeleri ve syslog örnekleri `LEGACY_RAW` yük profilinin çıktısıdır: biçim **dondurulmuştur**, sonradan eklenen alanlar bu çıktıya girmez ve mevcut ayrıştırıcılarınız değişmeden çalışır. Kanonik zarf, ortak aktör/hedef/kaynak blokları, gizlilik profili ve boyut politikası isteniyorsa hedef `APINIZER_JSON_V2` profiline geçirilir; örnek zarf ve alan listeleri yeni sayfadadır.
...
Denetim olaylarından yalnızca bir bölümünü SIEM'e göndermek (örneğin yalnızca başarısız ve reddedilen işlemler ya da yönetim isteği kayıtları hariç), gönderilen olay boyutunu sınırlamak veya kimlik alanlarını maskelemek için [SIEM ve Log Yönlendirme](/tr/admin/system-settings/siem-log-yonlendirme) sayfasındaki hat kurallarını kullanabilirsiniz.
