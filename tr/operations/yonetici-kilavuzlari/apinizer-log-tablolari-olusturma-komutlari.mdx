...
Aşağıdaki `CREATE TABLE` komutu **yeni kurulumlar** içindir ve AI Gateway, MCP Gateway, A2A Gateway, yönlendirme teşhisi, outbound MCP/A2A çağrı denetimi ile politika çalışma süreleri kolonları dahil tüm kolonları içerir — ayrıca bir `ALTER` çalıştırmanız gerekmez.

**Mevcut bir kurulumu yükseltiyorsanız** bu komutu çalıştırmayın; tablonuz zaten var. Geçtiğiniz her sürümün `ALTER TABLE` scriptini sırasıyla çalıştırın:

- **2026.04.x sürümünden geliyorsanız:** önce [2026.09.0 — Log Tablosu ALTER Scriptleri](/tr/release-notes/2026-09-0-log-tablosu-alter), ardından [2026.09.1 — Log Tablosu ALTER Scriptleri](/tr/release-notes/2026-09-1-log-tablosu-alter). Kümülatif 2026.09.0 scripti dört outbound MCP/A2A denetim kolonunu zaten oluşturduğu için 2026.09.1 sayfasında yalnızca `policy_exec_times` kolonunu ekleyin.
- **2026.09.0 sürümünden geliyorsanız:** yalnızca [2026.09.1 — Log Tablosu ALTER Scriptleri](/tr/release-notes/2026-09-1-log-tablosu-alter). Tablonuzda dört outbound MCP/A2A denetim kolonu zaten varsa yalnızca `policy_exec_times` kolonunu ekleyin — var olan bir kolonu yeniden eklemek hata verir.
- **Hangi sürümden geliyor olursanız olun:** ayrıca [2026.09.2 — Log Tablosu ALTER Scriptleri](/tr/release-notes/2026-09-2-log-tablosu-alter) scriptini bir kez çalıştırın — hiçbir önceki scriptin içermediği istemci atıf kolonlarını ekler.
...
:::note SQL Server: satır boyutu uyarısı

SQL Server bu tabloyu oluştururken şu uyarıyı verir: *"the table has been created, but its maximum row size exceeds the allowed maximum of 8060 bytes"*. **Bu beklenen bir durumdur ve bir hata değildir.**

Tablodaki değişken uzunluklu `NVARCHAR` kolonların toplam bildirilen genişliği satır başına 8060 byte'lık in-row sınırını aşar; SQL Server 2005'ten beri bu kolonlar gerektiğinde otomatik olarak ROW_OVERFLOW_DATA sayfalarına taşınır ve satırda 24 byte'lık bir işaretçi bırakılır. Sabit uzunluklu kolonların toplamı (~287 byte) sınırın çok altında olduğu için `INSERT` işlemleri başarısız olmaz. Tek etki, aynı anda çok sayıda uzun metin alanı dolu olan satırlarda ek sayfa okuma/yazma maliyetidir.

Kolon genişliklerini bu uyarıyı susturacak kadar (tüm metin kolonları 64 karaktere) daraltmak, `req_request_uri`, `req_query_string` ve `application_name` gibi alanlarda veri kaybına yol açacağı için tercih edilmemiştir.
...
| mcp_tool_names_out | Outbound isteklerde çağrılan farklı MCP tool adları (CSV, 256 karakterle sınırlı) |
| mcp_tool_call_count_out | Outbound MCP tool çağrılarının toplam sayısı (ham, sınırsız) |
| a2a_skill_names_out | Outbound isteklerde çağrılan farklı A2A skill/agent adları (CSV, 256 karakterle sınırlı) |
| a2a_agent_call_count_out | Outbound A2A agent çağrılarının toplam sayısı (ham, sınırsız) |
| policy_exec_times | Politika çalışma süreleri — `sıra:tip:süre` CSV; yalnızca log hedefinde ayar açıkken yazılır |
| api_client_key | İsteği doğrulayan API Client'ın anahtarı (kimlik bir API Client olmadan doğrulandığında boş) |
| client_owner_type | API Client'ın ait olduğu özne türü — `CONSUMER` veya `APPLICATION` |
| client_owner_id | Sahip Consumer'ın (Credential) veya Portal Application'ın kimliği |
...
-- Bir partition scheme kullanılıyorsa (örn. ps_logs_daily), idx_apitraffic_timestamp gereksizdir —
-- log_timestamp üzerindeki clustered index zaten bunu karşılar. Diğer indeksleri hizalayın:
-- CREATE INDEX idx_apitraffic_correlation ON log_ApiProxyTraffic(correlation_id)
--   ON ps_logs_daily(log_timestamp);
-- CREATE INDEX idx_apitraffic_proxy ON log_ApiProxyTraffic(api_proxy_id)
--   ON ps_logs_daily(log_timestamp);
-- ... (project, status, result için aynı desen)
```
  </TabItem>
</Tabs>
...
| api_client_key | Token'ın verildiği API Client'ın anahtarı |
| client_organization_id | Token'ın verildiği kimliğin organizasyon kimliği |
...
Elasticsearch konnektörü kullanıyorsanız log politikası verileri için ayrı bir index template oluşturulmalıdır. Bu template standart API trafik log template'inden farklıdır ve yalnızca log politikasının gönderdiği alanları içerir.
...
Elasticsearch konnektörünün **Index Adı** alanına `apinizer-log-policy-capture` girin. Bu isim, template'teki `index_patterns` ile eşleşmelidir. Farklı bir index adı kullanmak istiyorsanız template'teki `index_patterns` alanını buna göre güncelleyin.
...
Aşağıdaki `CREATE TABLE` komutu **yeni kurulumlar** içindir ve 2026.09.2 ile eklenen olay tipi, sonuç, istemci IP, User Agent, Correlation ID ve kaynak kolonlarını da içerir — ayrıca bir `ALTER` çalıştırmanız gerekmez.

**Mevcut bir kurulumu 2026.09.2'ye yükseltiyorsanız** bu komutu çalıştırmayın; tablonuz zaten var. Bunun yerine [2026.09.2 — Denetim ve Login Log Tablosu ALTER Scriptleri](/tr/release-notes/2026-09-2-denetim-login-tablosu-alter) sayfasındaki scripti **yükseltmeden önce** bir kez çalıştırın — bu kolonlar eklenmeden yazım başarısız olur.
...
| event_type | Denetim Kayıtları olay tiplerinden hangisi olduğu. Bkz. [Denetim Kayıtları — Olay Tipleri](/tr/admin/audit-compliance/denetim-kayitlari#olay-tipleri) |
| outcome | İşlemin sonucu — `SUCCESS`, `FAILURE` veya `DENIED` |
| client_ip | İstemcinin gerçek IP adresi (mevcutsa `X-Forwarded-For`'dan okunur) |
| user_agent | İstemcinin gönderdiği `User-Agent` başlığı |
| correlation_id | Aynı istekten üretilen diğer log kayıtlarını birbirine bağlayan kimlik |
| source | İsteğin geldiği yüzey — Yönetim Konsolu, APIOps, API Portal veya System |
...
Aşağıdaki `CREATE TABLE` komutu **yeni kurulumlar** içindir ve 2026.09.2 ile eklenen User Agent, neden kodu ve hedef proje kolonlarını da içerir — ayrıca bir `ALTER` çalıştırmanız gerekmez.
...
| email | Kullanıcı e-postası (API Portal girişi) |
...
| user_agent | İstemcinin gönderdiği `User-Agent` başlığı |
| reason_code | Bir başarısızlık, kilitlenme veya parola olayının ardındaki makine-okunur [neden](/tr/admin/audit-compliance/giris-kayitlari#user-agent-ve-sebep) |
| target_project_id | Proje değişimi (token yenileme) olayında oturumun geçtiği proje |
...
Login log hacmi trafik loglarına göre tipik olarak düşüktür. Güvenlik denetimi için sıkça gerekli olan uzun saklama süreleri için **`log_timestamp` üzerinde aylık bölümleme** yeterlidir. Örnekler için [Bölümleme Söz Dizimi](#partition-söz-dizimi) bölümüne bakın ve aralık için `DAY` yerine `MONTH` kullanın.
...
Bu tablo kısa bir zaman penceresi içinde tüketilen geçici kayıtları barındırır. **Partition gerekli değildir.** Tablo beklenmedik şekilde büyürse (uzun süreli kesintiler), partition yerine operasyonel temizlik daha uygun bir çözümdür. Partitioning mümkündür ve Apinizer INSERT davranışını etkilemez, ancak bu geçici kuyruk tablosu için genellikle operasyonel temizlik yeterlidir.
...
```
  ...
  <TabItem value="sql-server" label="SQL Server">
```sql
-- 0) Ön koşul: N sınır değeri N+1 partition üretir; scheme, her partition için tam olarak
-- bir filegroup listelemelidir. Enterprise dağıtımlar yıllık filegroup kullanabilir; minimal
-- örnekte tüm partition'lar [PRIMARY]'ye eşlenir.
...
-- 3) Tabloyu inline PK OLMADAN oluşturun (yukarıdaki kolon listesiyle aynı):
CREATE TABLE log_ApiProxyTraffic (
...
-- 4) Clustered bir PK partition sütununun dışında olamaz — NC PK + partition sütunu üzerinde CI:
ALTER TABLE log_ApiProxyTraffic
...
-- 5) Önerilen nonclustered indeksler (partition scheme ile hizalı):
CREATE INDEX idx_apitraffic_correlation ON log_ApiProxyTraffic(correlation_id)
  ON ps_logs_daily(log_timestamp);
CREATE INDEX idx_apitraffic_proxy ON log_ApiProxyTraffic(api_proxy_id)
  ON ps_logs_daily(log_timestamp);
...
-- 6) Sınırları zamanlanmış bir iş ile genişletme:
...
SQL Server'da, tablo `log_timestamp` veya `audit_event_date` ile anahtarlanmış bir partition scheme kullanıyorsa `id` üzerinde **clustered** PRIMARY KEY tanımlayamazsınız (Msg 1908). Clustered index partition sütunu üzerinde olmalıdır; `id` için **NONCLUSTERED** bir PK kullanın. Yıllık filegroup'lar veya üç aylık partition'lar geçerli operasyonel tercihlerdir ve Apinizer INSERT davranışını değiştirmez.
