...
Aşağıdaki `CREATE TABLE` komutu **yeni kurulumlar** içindir ve AI Gateway, MCP Gateway, A2A Gateway ile yönlendirme teşhisi kolonları dahil tüm kolonları içerir — ayrıca bir `ALTER` çalıştırmanız gerekmez.

**Mevcut bir kurulumu 2026.07.0'a yükseltiyorsanız** bu komutu çalıştırmayın; tablonuz zaten var. Bunun yerine [2026.07.0 — Log Tablosu ALTER Scriptleri](/tr/release-notes/2026-07-0-log-tablosu-alter) sayfasındaki `ALTER TABLE` komutunu kullanın.
...
### Sürüm Yükseltme: Yönlendirme Teşhisi Kolonları (Upgrade ALTER)

Yönlendirme Teşhisi özelliği ile bu tabloya, backend'e giden isteklerdeki hata sınıflandırması, bağlantı kurma/DNS/TLS/okuma faz süreleri, bağlantı havuzu anlık görüntüsü ve yapılandırılmış zaman aşımlarını tutan **24 yeni kolon** eklenmiştir. **Yeni kurulumlarda** bu kolonlar yukarıdaki `CREATE TABLE` komutunda zaten yer alır; aşağıdaki `ALTER TABLE` komutu **yalnızca mevcut bir kurulumu yükseltirken** gereklidir.

:::danger

Apinizer servis hesabına DDL (tablo yapısını değiştirme) yetkisi kasıtlı olarak verilmez. Bu nedenle aşağıdaki komut, ilgili şema üzerinde **bir veritabanı yöneticisi (DBA) tarafından** çalıştırılmalıdır.
...
- **Ne zaman çalıştırılır**: Bu komut, Api Manager'ın Yönlendirme Teşhisi özelliğini içeren sürüme yükseltilmesinden **önce çalıştırılması önerilir**; böylece yeni sürüm devreye girer girmez teşhis verileri kayıt altına alınmaya başlar. Komut yükseltmeden sonra çalıştırılırsa da veri kaybı yaşanmaz — yalnızca komutun çalıştırıldığı ana kadar geçen süredeki isteklerin teşhis verileri boş kalır.
- **Hangi ortamlar**: Yalnızca API trafik loglarını **veritabanı konnektörü** (Oracle/MySQL/PostgreSQL/SQL Server) ile bir ilişkisel veritabanına yazan kurulumlar için gereklidir.
- **Çalıştırılmazsa ne olur**: Apinizer, kolonların eksik olduğunu algılar ve yalnızca Yönlendirme Teşhisi alanlarını atlayarak kaydı yazmaya devam eder. **Trafik loglaması kesintiye uğramaz**; sadece bu istekler için Yönlendirme Teşhisi verisi kaydedilmez. Komut daha sonra çalıştırıldığında, çalıştırma anından itibaren gelen istekler için veriler toplanmaya başlar.
- **Geri alma**: Komut yalnızca yeni (nullable) kolon ekler, mevcut veriyi değiştirmez; bu nedenle bir geri alma riski taşımaz. Kolonları kaldırmak isterseniz standart `ALTER TABLE ... DROP COLUMN` ile geri alınabilir, ancak bu durumda o ana kadar toplanan teşhis verileri kalıcı olarak silinir.
- **Geriye dönük veri**: Bu kolonlar yalnızca ileriye dönüktür (forward-only); ALTER işleminden önce oluşmuş kayıtlarda bu alanlar boş (NULL) kalır.

:::

:::info

**Elasticsearch konnektörleri bu komuta ihtiyaç duymaz.** Elasticsearch üzerinde çalışan kurulumlarda ilgili şablon (mapping) güncellemesi ve index rollover işlemi, Apinizer sürüm yükseltmesiyle **otomatik** uygulanır; ek bir manuel adım gerekmez.
