---
title: Apinizer Cache için Prometheus Kurulumu ve Scraping Yapılandırması
description: Apinizer Cache'in metriklerini Prometheus'a entegre etmek için scraping yapılandırması yapılabilir. Prometheus, Cache'in 9091 portu üzerinden sağladığı metrikleri sabit (static) veya dinamik (Kubernetes service discovery) yöntemlerle toplayabilir. Bu sayede Cache performansını izleme, analiz etme ve görselleştirme imkanı sağlar.
---
...
<Info>
  Cache metriklerinin Prometheus tarafından toplanabilmesi için Ağ Geçidi Ortamları (Gateway Environments) sayfasında Deployments & Services bölümünde Cache edit deployment seçeneği seçilerek `METRICS_ENABLED=TRUE` değişkeni eklenmelidir.
</Info>
...
#### Kubernetes Yönetimi Apinizer ile Yapılmıyor İse
