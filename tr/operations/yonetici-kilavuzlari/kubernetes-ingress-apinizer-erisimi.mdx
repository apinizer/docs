...
description: "NGINX Ingress Controller kurarak Apinizer servislerine Ingress ile erişim sağlayabilir, MetalLB ile LoadBalancer hizmeti yapılandırabilir, X-Forwarded-For (XFF) ile gerçek istemci IP bilgisini iletebilir, Ingress nesneleri oluşturarak DNS tabanlı yönlendirme yapabilir ve TLS sertifikaları ile güvenli erişim sağlayabilir."
---
...
## X-Forwarded-For (XFF) Yapılandırması

Apinizer 2024.05.4 ve sonrasında Yönetim Konsolu, istemci IP bilgisini zorunlu olarak kontrol eder. Ingress üzerinden erişimde gerçek istemci IP'sinin Apinizer'a iletilmesi için `X-Forwarded-For` (XFF) header'ının doğru şekilde geçirilmesi gerekir.

Config Maps tanımındaki data bölümüne `use-forwarded-headers` anahtarını `true` değeri ile ekleyiniz:
...
Helm kurulumu sırasında kullandığınız namespace farklıysa (`ingress` gibi), ConfigMap'in `metadata.namespace` ve `metadata.name` alanlarını kendi ortamınıza göre düzenleyiniz. Mevcut Controller ConfigMap'ini `kubectl get configmap -n <namespace>` komutu ile bulabilirsiniz.

:::

Yaml dosyasını uygulayınız:

```bash
kubectl apply -f nginx-configuration.yaml
```

Bir sonraki adımda oluşturulacak Ingress nesnesine de aşağıdaki anotasyon eklenmelidir:
...
Load balancer tarafında aşağıdaki header'lardan birinin mutlaka gelmesi gerekir:
