...
<CardGroup cols={2}>
  <Card title="4. Harici Sistem Bağlantıları" icon="plug" href="#4-harici-sistem-bağlantıları">
    Kimlik doğrulama, log yönetimi, mesajlaşma ve [API Integrator](/tr/apinizer-anlama/temel-bilesenler/api-integrator) [konnektörleri](/tr/apinizer-anlama/temel-kavramlar/konnektor).
  </Card>
...
<Info>
  API Manager'ın detaylı özellikleri ve kullanımı için [API Manager](/tr/apinizer-anlama/temel-bilesenler/api-manager) sayfasına bakabilirsiniz.
</Info>
...
<Info>
  API Gateway'in detaylı özellikleri, alt bileşenleri ve kullanımı için [API Gateway](/tr/apinizer-anlama/temel-bilesenler/api-gateway) sayfasına bakabilirsiniz.
</Info>

#### Ortam (Environment)

Apinizer Platformu'nda bir **[Ortam (Environment)](/tr/apinizer-anlama/temel-kavramlar/ortam-nedir)**, API Gateway'lerin (Worker'ların) çalıştığı, kendisine ait bir erişim adresi ve ayarları olan, diğer Ortam'lardan yalıtılmış olarak ve kendisine ayrılmış CPU ve RAM gibi kaynakları kullanarak çalışan sanal sunucu alanıdır.
...
* **Worker Pod'ları**: [API Proxy](/tr/apinizer-anlama/temel-kavramlar/api-proxy-nedir)'lerin çalıştığı pod'lar. Her Worker pod'u bağımsız bir API Gateway instance'ıdır ve kendi Local Cache'ine sahiptir.

* **Cache Pod'ları**: Dağıtılmış önbellek ([Hazelcast](/tr/apinizer-anlama/temel-bilesenler/onbellek-bileseni)) pod'ları. Ortam içindeki Worker'lar arasında paylaşılan cache verilerini tutar.
...
* **Konfigürasyon İzolasyonu**: Her Ortam'ın kendi konfigürasyon ayarları vardır. [API Proxy](/tr/apinizer-anlama/temel-kavramlar/api-proxy-nedir)'ler bir veya birden fazla Ortam'a yüklenebilir ve her Ortam'da farklı konfigürasyonlarla çalışabilir.
...
<Info>
  **Not:** [API Proxy](/tr/apinizer-anlama/temel-kavramlar/api-proxy-nedir)'ler, bir veya birden fazla Ortam üzerine yüklenebilir. Kurulumu yapılan Ortam ya da Ortam'ların hepsi bir Kubernetes kümesi (cluster) içinde yer alır ve Apinizer Platformu'nun kurulu olduğu sunucular üzerinde çalışır.
</Info>
...
<img
  src="/images/apinizer-anlama/mimari-duzlem.png"
...
1. **Route Matching**: Gelen istek Route Matching bileşeni tarafından karşılanır. Path, Host, Method ve Header bilgilerine göre uygun [API Proxy](/tr/apinizer-anlama/temel-kavramlar/api-proxy-nedir) bulunur ve MessageContext oluşturulur.

2. **Policy Enforcement (Request Pipeline)**: MessageContext, Request Pipeline'dan geçer ve 4 kategori [politikalar](/tr/apinizer-anlama/temel-kavramlar/politika-nedir) sırayla uygulanır:
   - Güvenlik Politikaları
   - Trafik Yönetimi Politikaları
   - Mesaj Doğrulama Politikaları
   - Mesaj Dönüşümü Politikaları

   Mesaj işleme ve politika uygulama hakkında detaylı bilgi için [Mesaj İşleme ve Politika Uygulama](/tr/apinizer-anlama/temel-kavramlar/mesaj-isleme-ve-politika-uygulama) sayfasına bakabilirsiniz.

3. **Routing Decision**: Politikalar başarıyla uygulandıktan sonra, RoutingHandlerFactory tarafından uygun handler seçilir. [Routing](/tr/apinizer-anlama/temel-kavramlar/routing-ve-upstream) hakkında daha fazla bilgi için ilgili sayfaya bakabilirsiniz.
...
Proxy Handler içinde uygulanan [politikalar](/tr/apinizer-anlama/temel-kavramlar/politika-nedir) dört ana kategoride gruplandırılır: Güvenlik, Trafik Yönetimi, Mesaj Doğrulama ve Mesaj Dönüşümü. Bu politikalar Request Pipeline ve Response Pipeline üzerinden sırayla uygulanır. Detaylı bilgi için [Mesaj İşleme ve Politika Uygulama](/tr/apinizer-anlama/temel-kavramlar/mesaj-isleme-ve-politika-uygulama) sayfasına bakabilirsiniz.
...
<Info>
  Dağıtılmış önbellek mimarisi ve kullanımı hakkında detaylı bilgi için [Önbellek Bileşeni](/tr/apinizer-anlama/temel-bilesenler/onbellek-bileseni) sayfasına bakabilirsiniz.
</Info>
...
* **[API Portal](/tr/apinizer-anlama/temel-bilesenler/api-portal)**: Dış geliştiriciler ve partner kurumlar için API keşif ve dokümantasyon portalı. Portal, DMZ'de konumlandırılarak dış erişime açılır.
...
* **[MongoDB](/tr/apinizer-anlama/temel-kavramlar/baglantilar) Cluster**: Konfigürasyon verilerini almak için (başlangıçta ve konfigürasyon değişikliklerinde)
* **Logging Servisleri**: API trafik log'larını göndermek için ([Elasticsearch](/tr/apinizer-anlama/temel-bilesenler/analytics-engine), Kafka vb.)
...
* **[MongoDB](/tr/apinizer-anlama/temel-kavramlar/baglantilar) Cluster**: Tüm konfigürasyon ve metadata verilerinin saklandığı merkezi veritabanı. Gateway'ler başlangıçta ve konfigürasyon değişikliklerinde bu veritabanından veri çeker.

* **[Elasticsearch Cluster](/tr/apinizer-anlama/temel-bilesenler/analytics-engine)**: API trafik log'larının ve analitik verilerin saklandığı log yönetim sistemi. API Manager'ın Analytics özellikleri bu verileri kullanır.

* **Kafka Cluster**: Yüksek hacimli log akışı için kullanılan mesajlaşma platformu (opsiyonel).

* **[Dağıtılmış Cache (Hazelcast)](/tr/apinizer-anlama/temel-bilesenler/onbellek-bileseni)**: Quota, throttling, OAuth2 token gibi paylaşılan verilerin tutulduğu cache sistemi.
...
* **[MongoDB](/tr/apinizer-anlama/temel-kavramlar/baglantilar) Replica Set**: MongoDB replica set ile okuma performansı artırılabilir. Okuma işlemleri secondary node'lara dağıtılabilir.
* **MongoDB Sharding**: Sharding ile yatay ölçeklendirme yapılabilir. Veriler farklı shard'lara dağıtılır.
* **[Elasticsearch](/tr/apinizer-anlama/temel-bilesenler/analytics-engine) Cluster Genişletme**: Elasticsearch cluster'ına yeni node'lar eklenerek kapasite artırılabilir. Index'ler farklı node'lara dağıtılır.
...
* **[Cache Replication](/tr/apinizer-anlama/temel-bilesenler/onbellek-bileseni)**: Hazelcast'in built-in replication özellikleri ile cache verileri birden fazla node'da tutulur. Tek bir cache node'unun çökmesi durumunda veriler diğer node'lardan erişilebilir.
...
* **[LDAP/Active Directory](/tr/apinizer-anlama/temel-bilesenler/identity-manager)**: Kurumsal LDAP ve Active Directory sistemleri ile kullanıcı kimlik doğrulama. Kullanıcı bilgileri ve grup üyelikleri LDAP/AD'den alınır ve API erişim kontrolü için kullanılır.
...
[API Integrator](/tr/apinizer-anlama/temel-bilesenler/api-integrator) modülü, çeşitli harici sistemlerle entegrasyon yapmak için geniş bir konnektör yelpazesi sunar. Bu konnektörler sayesinde API'ler oluşturulabilir, veritabanlarına erişilebilir ve çeşitli aksiyonlar gerçekleştirilebilir.

**Veritabanı Konnektörleri:**

[API Creator](/tr/apinizer-anlama/temel-bilesenler/api-creator) ve API Integrator modülleri, aşağıdaki veritabanları ile entegrasyon yapabilir. Veritabanı [bağlantıları](/tr/apinizer-anlama/temel-kavramlar/baglantilar) ve [konnektörler](/tr/apinizer-anlama/temel-kavramlar/konnektor) hakkında detaylı bilgi için ilgili sayfalara bakabilirsiniz.
...
Apinizer platformunun tüm konfigürasyon ve meta verilerinin tutulduğu merkezi bileşendir. Platform üzerinde yapılan her türlü yapılandırma, ayar ve tanımlama bu veritabanında saklanır. Apinizer, MongoDB ve PostgreSQL gibi popüler veritabanlarını destekler. Veritabanı bağlantıları hakkında detaylı bilgi için [Bağlantılar](/tr/apinizer-anlama/temel-kavramlar/baglantilar) sayfasına bakabilirsiniz.
...
<CardGroup cols={3}>
  <Card title="API ve Politika Konfigürasyonları" icon="network-wired">
    * [API Proxy](/tr/apinizer-anlama/temel-kavramlar/api-proxy-nedir) tanımları ve yapılandırmaları
    * [Politika](/tr/apinizer-anlama/temel-kavramlar/politika-nedir) konfigürasyonları (rate limiting, authentication, transformation vb.)
    * [Ortam (Environment)](/tr/apinizer-anlama/temel-kavramlar/ortam-nedir) ayarları ve konfigürasyonları
    * [Bağlantı (Connection)](/tr/apinizer-anlama/temel-kavramlar/baglantilar) bilgileri ve ayarları
    * [API Integrator](/tr/apinizer-anlama/temel-bilesenler/api-integrator) konfigürasyonları
    * [Cache](/tr/apinizer-anlama/temel-bilesenler/onbellek-bileseni) konfigürasyonları
  </Card>
  <Card title="Yönetim ve İzleme" icon="chart-line">
    ...
    * [Monitoring](/tr/apinizer-anlama/temel-bilesenler/izleme-bileseni) ve Anomaly Detection ayarları
