...
description: 'Apinizer platformunun performans test sonuçları ve yük testi detayları'
---

Bu sayfa, Apinizer platformunun performans test sonuçlarını ve yük testi detaylarını içermektedir. Test sonuçları, platformun farklı senaryolardaki performansını değerlendirmek için yapılmıştır.
...
Yük testi topolojisi aşağıdaki bileşenlerden oluşmaktadır:
...
* **İstek**: Belirli bir HTTP metodu ile belirli bir hedef için yapılan HTTP isteğidir
* **Session**: Session başına sıfır veya daha fazla istek olabilir
* Gateway'lerde session tutmak çok nadir görülür, genellikle servislere erişim stateless'dir
* Bu yüzden eş zamanlı istek sayısını ve gecikmeyi (latency) ölçmek daha anlamlı hale gelir
...
## Policy Performans Etkisi

Gateway'e eklediğimiz her bir policy, gateway'de karmaşıklığına ve bağımlılıklarına göre performansı etkiler.

### Basic Authentication Policy Testi

"Basic Authentication" policy'si eklenerek test edilmiştir (Durum D):
...
<Info>
  Gördüğümüz gibi performansa etkisi hissedilmeyecek derecede de olsa olmuş. Fakat örneğin "content filtering" gibi işlem gücü yüksek maliyetli bir policy eklenseydi ya da "LDAP Authentication" gibi dış bağlantı gerektiren ve araya bir de network latency ekleyen policy eklenseydi performans daha da hızlı bir şekilde düşecekti.
</Info>

### Policy Seçimi Best Practices

<CardGroup cols={2}>
  <Card title="Policy Karmaşıklığı" icon="slider">
    Her bir policy'nin ne kadar yük getireceğini bilmek ve tasarımı ona göre seçmek önemlidir.
  </Card>
  <Card title="Dış Bağlantılar" icon="network-wired">
    LDAP Authentication gibi dış bağlantı gerektiren policy'ler network latency ekler ve performansı etkiler.
  </Card>
  <Card title="İşlem Gücü" icon="microchip">
    Content filtering gibi işlem gücü yüksek maliyetli policy'ler CPU kullanımını artırır.
  </Card>
  <Card title="Policy Sıralaması" icon="list-ol">
    Policy'lerin sıralaması ve koşullu çalıştırılması performansı etkiler.
  </Card>
</CardGroup>
...
* **GET İstekleri**: 15,000+ reqs/sec (2,000 thread)
* **POST 5KB İstekleri**: 13,000+ reqs/sec (2,000 thread)
* **POST 50KB İstekleri**: 2,700+ reqs/sec (2,000 thread)

### Response Time (Yanıt Süresi)

Ortalama yanıt süreleri:

* **GET İstekleri**: 3-655 ms (thread sayısına göre)
* **POST 5KB İstekleri**: 3-701 ms
* **POST 50KB İstekleri**: 10-710 ms
...
<CardGroup cols={2}>
  ...
  <Card title="Policy Etkisi" icon="shield">
    Basic Authentication gibi basit policy'lerin performans etkisi minimaldir.
  </Card>
</CardGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Policy Yönetimi">
    * Policy'lerin performans etkisini değerlendirin
    * Gereksiz policy'leri kaldırın
    * Policy sıralamasını optimize edin
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Sistem Gereksinimleri" icon="server" href="/tr/apinizer-anlama/sistem/donanim">
