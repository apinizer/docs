...
description: "RESTful API'lerinizi Apinizer platformu üzerinden yönetebilir ve güvenli bir şekilde expose edebilir. OpenAPI/Swagger spesifikasyon dosyasından REST API Proxy oluşturabilir, URL veya dosya yükleyebilir, routing yapılandırması yapabilir ve JSON formatında mesaj işleme gerçekleştirebilir"
---
...
<Steps>
  ...
  <Step title="OpenAPI Dosyasını Yükleyin">
    ...
    <AccordionGroup>
      <Accordion title="Yöntem 1: URL'den Yükleme">
        **URL Gir (Enter URL)** bağlantısına tıklanır. OpenAPI/Swagger dosyasının URL'si girilir (örneğin: `https://petstore.swagger.io/v2/swagger.json`) ve **Parse** butonuna tıklanır. Dosya parse edilir ve API bilgileri görüntülenir.
  ...
  <Step title="API Proxy'yi Kaydedin">
    Tüm bilgiler kontrol edilir. **Relative Path** alanı doldurulur (zorunlu). **Upstream Target** bilgisi kontrol edilir veya düzenlenir. Sağ üstteki **Kaydet (Save)** butonuna tıklanır.
  </Step>
</Steps>
