...
| **Adım 4: Token Yaşam Döngüsü ve Refresh Ayarları** | - **Delete Previous**: Yeni token üretildiğinde eski tokenları iptal edin.<br>- **Token Never Expires**: Süresiz token gerekiyorsa açık bırakın; kapatırsanız süre ve birim tanımlayın.<br>- **Refresh Token Allowed**: Yenileme hakkını aktif edin, sayıyı ve süresini belirleyin.<br>- **Allow URL Parameters / Clear Auth**: Token endpoint'inde URL parametresi kabulü ve eski kimlik bilgilerinin temizlenmesini yapılandırın. |
| **Adım 5: Header ve Yetkilendirme Entegrasyonu** | - **Add User To Header / User Header Name**: Doğrulanan kullanıcı bilgisini özel header'a ekleyin.<br>- **Authorization Configuration**: Method bazlı rol atamaları, yetkilendirme kaynağı (Secret Manager, API, LDAP, DB) ve rol header yönetimini ayarlayın.<br>- **Add Roles To Header / Roles Header Name**: Roller için iletilecek header isimlerini belirleyin. |
...
| **Adım 7: Hata Mesajı Özelleştirme (İsteğe Bağlı)** | - **Error Message Customization** sekmesine gidin.<br>- Erişim reddedildiğinde dönecek mesajı özelleştirin.<br><br>**Varsayılan:**<br>```json<br>{<br>  "statusCode": 401,<br>  "message": "Authentication policy failed! OAuth2 Token is not valid!"<br>}<br>```<br><br>**Özel:**<br>```json<br>{<br>  "statusCode": 401,<br>  "errorCode": "OAUTH2_INVALID_TOKEN",<br>  "message": "Geçersiz veya süresi dolmuş token."<br>}<br>``` |
