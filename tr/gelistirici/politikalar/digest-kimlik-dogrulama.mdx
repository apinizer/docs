...
- **Politika Kontrolü**: IP Digest Kimlik Doğrulama politikası aktif ise, sistem aşağıdaki sırayla kontrol yapar:
...
| **Adım 4: İstemci Kontrollerini Ayarlama** | Clear Auth ve checkClientIpAddress seçeneklerini gereksinime göre işaretleyin. Clear Auth, upstream servise gönderilmeden önce Authorization başlığını temizler; IP kontrolü ise kimlik bilgisi paylaşımını engeller. |
| **Adım 5: Başlık ve Yetkilendirme Yapılandırması** | Add User to Header seçeneği ile kimliği özel header'a taşıyın, gerekli ise header adını güncelleyin. AuthorizationConfiguration bileşeninde yetkilendirme Kaynağı, rol başlığı ve metot kısıtlarını tanımlayın. |
...
| :-------- | :-------------------------------------------------------------------------------- | :-------------- |
| ERR-051  | Authentication policy failed! Error message is: {0}                              | 500             |
