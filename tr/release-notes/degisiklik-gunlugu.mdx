...
Politika veya API yönlendirme seviyesinde özelleştirilmiş hata kaydında yalnızca `customizedHttpStatusCode` veya `customizedErrorCode` set edilip mesaj alanı boş bırakıldığında istemciye boş yanıt gövdesi dönmesi sorunu giderildi. Özel mesaj boş ise artık varsayılan (default) hata mesajına geri dönülür. Düzeltme `PolicyException`, `ApiProxyException` ve `ApinizerException` sınıflarının tümünde tutarlı şekilde uygulanır; etkilenen politika veya yönlendirmelerin yeniden oluşturulmasına gerek kalmaz. Script politikasının `errorMessageTT` üzerinden çalışma zamanında mesaj dolduran akışı korunur. APNZ-6209

- **OIDC Politikasında Yanıt Çerezi Tutarsızlığı Düzeltildi**

OIDC oturum çerezi yazımındaki SameSite/Secure davranışı kanıtlanmış Groovy script referansıyla uyumlu hale getirildi. Aynı yanıtta birden fazla `Set-Cookie` yazıldığında çerezlerin birbirini ezmesine yol açan map okuma hatası giderildi. State doğrulaması açıkken `state` parametresi olmadan gelen callback istekleri CSRF koruması olarak reddedilir; süresi dolmuş geçici oturumlarda nonce doğrulama NPE'si önlendi; cache silme anahtarı yazıcı anahtar formatıyla hizalandı. `Add Token To Cookie` etkinken access/id/refresh token'ları için ayrı `HttpOnly` çerezler yazılması (Groovy `setAllTokenCookies` paritesi) sağlandı. APNZ-6230

- **x-www-form-urlencoded Body ile Liste İki Yönlü Senkronize Tutuluyor**

`application/x-www-form-urlencoded` istekleri için body alanı ile parametre listesi (`formUrlEncodedDataParameterList`) artık trafik logu ve script politikası akışlarında çift yönlü senkron tutulur. İstek girişinde form parametreleri parse edilirken body alanı RFC 3986 formatında doldurulur (önceden boş kalıyordu). Script politikalarında yalnızca `bodyToTarget`'a yazan eski script'ler ve yalnızca `formUrlEncodedTT` listesi üzerinden manipüle eden yeni script'ler aynı sonucu üretir; her iki taraf değiştirilmişse list öncelikli kabul edilir ve body otomatik yeniden serileştirilir. APNZ-6229
...
API Traffic kaydı download/export işleminde 5 MB üstü request/response body içeriklerinin ZIP içine "Body size exceeds the display limit..." placeholder'ı ile yazılması sorunu giderildi. UI tarafında büyük body'leri ekrana render etmemek için uygulanan kısaltma davranışı korunurken, download yolu (`exportApiTraffic`) artık Elasticsearch'ten gelen tam body'yi ZIP'e yazar. APNZ-6235

- **Upgrade Sonrası Project Owner ve API Developer Rollerine Eksik EXECUTE Yetkileri Eklendi**

Eski sürümlerden yükseltilen ortamlarda Project Owner rolünde `TESTING:EXECUTE` ve diğer yeni asset action'larının; API Developer rolünde ise `TESTING:VIEW`/`TESTING:EXECUTE` yetkilerinin eksik kalması nedeniyle Quick Test ve REST API Test çağrılarının 401 alması sorunu giderildi. Yeni `202605-02` migration'ı Project Owner için tüm asset kategorisi × action matrisindeki eksik action'ları, API Developer için ise REST API Test ve Quick Test yetkilerini idempotent şekilde geri doldurur. APNZ-6234

- **LDAP Kullanıcıları Proje Listesini Görebiliyor**

`ProjectService.findByUser()` sorgusunda yalnızca standart `projectMember` üzerinden filtreleme yapıldığı için LDAP üzerinden gelen kullanıcıların proje listesini boş görmesi sorunu giderildi. Sorgu artık `AuthorityControlService.hasPermissionInProject` ile aynı pattern'i kullanır; standart kullanıcı kriteri ile LDAP `distinguishedName` ve grup üyelikleri (`projectLdapMember.teamMemberList.fullName`, `projectLdapMember.teamList.name`) `OR` ile birleştirilir. APNZ-6233
