...
Tüm platform bileşenleri (API Manager, Gateway Worker, Cache Server, Integration, API Portal) Java 25'e yükseltildi.

- **Elasticsearch 7, 8 ve 9 Tam Desteği**

Elasticsearch 7.x, 8.x ve 9.x sürümleri tam desteklenir hale getirildi.

- **Message Builder Politikası**

İstek ve yanıt mesajlarını dinamik olarak oluşturmayı sağlayan yeni bir politika eklendi.

- **Hata Mesajı Özelleştirme**

Politikalardaki hata mesajlarının dönüş formatları (JSON/XML) özelleştirilebilir hale getirildi.

- **API Promotion Modülü**

Farklı ortamlar arasında API tanımlarının taşınması ve yönetilmesini sağlayan modül eklendi.

- **Gelişmiş Loglama Özellikleri**

API Proxy bazlı log ayarları, log politikası ve metadata alanları yeniden tasarlandı.

- **Report Generator Zamanlı Rapor Gönderimi**

Mevcut raporların seçilerek connector'lere zamanlı olarak gönderilebilmesi sağlandı.

- **TCP Connectivity Test Ekranı**

Gateway pod'larından hedef sunuculara TCP ve TLS bağlantı testi yapılabilmesi sağlandı.

- **API Tanım Dosyası Erişim Kontrolü**

API Proxy bazlı WSDL, OpenAPI gibi tanım dosyalarına üç seviyeli erişim kontrolü eklendi.
...
- **Trace Ekranı Yeniden Tasarımı:** API izleme ekranı yeniden tasarlandı.
- **Güvenlik Sıkılaştırmaları:** XSS, IDOR ve Stack Trace Leak güvenlik açıkları giderildi.
- **Kütüphane Güncellemeleri:** Spring Boot 3.5.x, Angular 19, PrimeNG 19 ve çok sayıda bağımlılık güncellendi.
- **Context ve Environment Variable Genişletmeleri:** Tüm yapılandırma ekranlarında variable kullanımı genişletildi.
- **Test Konsol ve API Call'da mTLS Desteği** eklendi.
- **Import İşlemi İyileştirmesi:** Diff-based import seçeneği eklendi.
- **System Properties Tablosu** eklendi.
- **Deployment History'de fark gösterimi** eklendi.
...
Platformun genelinde kapsamlı kararlılık, performans ve kullanılabilirlik iyileştirmeleri yapıldı. Detaylar için [Sürüm Notları 2026](/tr/release-notes/surum-notlari-2026#sürüm-2026040) sayfasına bakınız.
