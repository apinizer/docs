...
OIDC oturum çerezi yazımındaki `SameSite` ve `Secure` davranışı, OIDC akışının referans uygulaması ile birebir uyumlu hale getirildi. Aynı yanıtta birden fazla `Set-Cookie` üretildiğinde çerezlerin birbirini ezmesine yol açan hata giderildi. `state` doğrulaması etkinken `state` parametresi olmadan gelen callback istekleri CSRF koruması gereği reddedilir; süresi dolmuş geçici oturumlarda nonce doğrulaması sırasında oluşan boş referans hatası önlendi; oturum cache silme akışı yazma akışıyla aynı anahtar formatını kullanacak şekilde hizalandı. **Add Token To Cookie** seçeneği etkinken erişim, kimlik ve yenileme token'ları için ayrı `HttpOnly` çerezler yazılması sağlandı. APNZ-6230

- **x-www-form-urlencoded Body ile Liste İki Yönlü Senkronize Tutuluyor**

`application/x-www-form-urlencoded` istekleri için body içeriği ile form parametre listesi artık trafik logu ve script politikası akışlarında çift yönlü senkron tutulur. İstek girişinde form parametreleri ayrıştırılırken body alanı da RFC 3986 formatında doldurulur (önceden bu durumda boş kalıyordu). Script politikalarında yalnızca body alanına yazan eski script'ler ve yalnızca form parametre listesi üzerinden değişiklik yapan yeni script'ler aynı sonucu üretir; her iki taraf da değiştirilmişse parametre listesi öncelikli kabul edilir ve body buna göre otomatik yeniden serileştirilir. APNZ-6229

- **Elasticsearch Index Lifecycle Policy Save & Deploy Sonrası Güncellenmesi**

Elasticsearch bağlantısında Edit Index Lifecycle Policy ekranında phase değerleri değiştirildikten sonra Save & Deploy ile değişikliklerin Elasticsearch'e gönderilmemesi sorunu giderildi. ILM policy adı değişmemiş olsa bile her Save & Deploy'da tam payload Elasticsearch'e push edilir; daha önce policy adı eşit kaldığında push atlanıyordu. APNZ-6185
...
Eski sürümlerden yükseltilen ortamlarda Project Owner rolünde Testing kategorisinin Execute yetkisinin ve sonradan eklenen diğer varlık (asset) eylem yetkilerinin; API Developer rolünde ise Testing kategorisinin View ve Execute yetkilerinin eksik kalması nedeniyle Quick Test ve REST API Test çağrılarının HTTP 401 hatası alması sorunu giderildi. Yükseltme sırasında çalışan veritabanı geçişi Project Owner için tüm varlık kategorisi × eylem matrisindeki eksik yetkileri, API Developer için ise REST API Test ve Quick Test yetkilerini mevcut yetkilere dokunmadan geri doldurur. APNZ-6234
...
Global Politikalar altındaki bir politikanın View sayfasında Delete butonuna basıldığında onay diyaloğu gösterilmeden silme işleminin gerçekleşmesi sorunu giderildi. Silme işlemi artık standart onay diyaloğu üzerinden yürütülür; silinmek istenen politika bir veya daha fazla API Proxy tarafından kullanılıyorsa kullanıcı, silmeden önce bağımlılıklar hakkında uyarılır. Düzeltme önce `BasePolicy.deletePolicy` üzerinde temel iskelet olarak hazırlanmış, ardından Authentication ve SAML dahil tüm politika view template'lerine yaygınlaştırılmıştır. APNZ-6149

- **Politika Sürükle-Bırak Önizlemesinin Yarım Görünmesi Düzeltildi**

API Proxy Akışı ekranında istek, yanıt veya hata politikası kartları sürüklenirken önizleme kutusunun yalnızca üst yarısının görünmesi sorunu giderildi. Politikalar artık sürükleme sırasında tam boyutlu kart önizlemesi olarak imlecin ucunda görünür; politikaların listede bırakılacağı konumda yarı saydam bir yer tutucu gösterilerek hedef sıra görsel olarak belirginleşir. Düzeltme istek, yanıt ve hata politika listelerinin tamamında aynı şekilde uygulanır. APNZ-6056
...
Oracle Türkçe karakter seti (WE8ISO8859P9) ile veritabanı bağlantısı kurulamaması sorunu giderildi (orai18n bileşeni eklendi). APNZ-6168
