...
- ### Mesaj Oluşturucu (Message Builder) Politikası
Template ve form tabanlı mesaj oluşturma politikası eklendi. İki oluşturma modu desteklenir: **Form** modu (basit değişken kopyalama) ve **Template** modu (JEXL expression desteği ile dinamik içerik oluşturma). Her satır için bağımsız koşul, varsayılan değer, zorunluluk ve hata yönetimi yapılandırılabilir. Satırlar sürükle-bırak ile sıralanabilir.
...
- **Sertifika dışa aktarma seçenekleri:** Sertifika görüntüleme ekranında iki farklı dışa aktarma seçeneği eklendi: "Sertifika Olarak Dışa Aktar" (sertifika dosya formatında) ve "Dışa Aktar" (JSON/ZIP formatında, ortamlar arası taşıma için).
- **IP Group ve Credential'da Context Variable desteği:** IP Group ve Credential düzenleme ekranlarına Context Variable (`#{key}`) ve Environment Variable (`${key}`) seçim diyaloğu eklendi. Artık her iki değişken türü de bu alanlarda kullanılabilir.
- **Message Builder Policy politikası:** Yeni [Mesaj Oluşturucu](/tr/develop/politikalar/mesaj-olusturucu) politikası eklendi.
...
- **Şablon tabanlı mesajlar:** Serbest metin yazın; istek/hata bilgilerini placeholder'larla otomatik dahil edin (örn: `#{error.effectiveErrorCode}`, `#{request.uri}`, `#{dateTime.formattedText}`, `#{message.correlationId}`).
...
**Örnek şablon:** `API çağrınız başarısız oldu. Hata Kodu: #{error.effectiveErrorCode} | Zaman: #{dateTime.formattedText} | İstek ID: #{message.correlationId}`
...
- **Error template placeholder'ları:** `#{error.effectiveErrorCode}`, `#{error.effectiveMessage}`, `#{error.effectiveHttpCode}`, `#{error.customizedMessage}` vb.
- **Variable türleri:**
...
- ### Dinamik Hata Mesajı Özelleştirme
Politikalarda ve İş Kuralı STOP aksiyonunda hata durumlarında döndürülen mesajları şablon tabanlı olarak özelleştirebilirsiniz. Serbest metin şablonunda `#{error.effectiveErrorCode}`, `#{request.uri}`, `#{dateTime.formattedText}`, `#{message.correlationId}` gibi yer tutucılarla hata, istek, zaman, API ve mesaj bilgilerini otomatik dahil edebilirsiniz. HTTP status kodu ve Content-Type (JSON, XML, plain text) ayarlanabilir. Kullanım alanları: Global politikalar, API Proxy, Yönlendirme (Routing), İş Kuralı STOP. [Bkz.](/tr/concepts/temel-kavramlar/politika-nedir#şablon-tabanlı-dinamik-hata-mesajları)
