...
**Yayınlanma tarihi:** Belirlenecek
...
- **API Proxy Önbelleğinde Yalnızca Başarılı Yanıtların Saklanması**

API Proxy önbelleği başarısız yanıtları (ör. 400 ve diğer 4xx/5xx) da saklayabiliyor, sonraki isteklere önbellekten hatalı yanıt dönebiliyordu. Artık yalnızca başarılı (`HTTP < 400`) yanıtlar önbelleğe alınır; hata yanıtları (4xx/5xx) hiçbir zaman önbelleğe alınmaz. [Bkz.](/tr/develop/api-proxy-konfigurasyonu/settings) APNZ-6362

- **Önbellek Ekranlarında Form Görünüm Tutarlılığı**

Önbellek ayarları ekranlarındaki aç/kapa anahtarları, onay kutuları ve seçim alanlarının kullanımı ile boyut/görünümleri platform ölçütlerine göre tutarlı hale getirildi. APNZ-6362

- **Kimlik Doğrulama Politikalarında Kimlik Yansıtma Kontrolü**

Tüm kimlik doğrulama politikalarında (Basic, Plain Text, Digest, JWT, OAuth2, mTLS, OIDC ve JOSE Doğrulama), bu politikaların çözümlediği kimliğin (kullanıcı/anahtar) isteğin kimliği olarak kaydedilip kaydedilmeyeceği artık yapılandırılabiliyor. Etkin olduğunda bu kimlik trafik loglarında ve sonraki politikalar ile scriptlerde görünür; kapatıldığında kimlik yalnızca gerekli iç doğrulamalar için kullanılır, isteğin kalıcı kimliği olarak tutulmaz. Varsayılan olarak etkindir ve mevcut davranış değişmez. APNZ-6363
...
**APNZ-6362** : API Method/Endpoint için önbellek ayarları ilk kez açıldığında varsayılan kapasite ve geçerlilik süresi (TTL) değerleri boş geliyordu (API Proxy önbelleğinde sorun yoktu). Bu varsayılanlar artık Method/Endpoint önbellek ekranında da geliyor; daha önce girilmiş değerler korunur.
...
**APNZ-6366** : Önceki bir sürümden yükseltme sonrasında, API Trafik Log ayarlarındaki istek ve yanıt akışı içerik seçimleri (parametre, başlık, gövde) sıfırlanmış olarak görünebiliyordu. Bu yapılandırma artık yükseltme sırasında korunuyor; daha önce etkilenmiş kurulumlarda ilgili içerik seçimleri etkin duruma geri getiriliyor.
