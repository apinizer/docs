...
- **Script Politikasında Ortam Değişkenlerine Erişim**

Aktif ortamda tanımlı ortam değişkenlerinin değerleri artık script gövdesinden de okunabiliyor. Diğer politikalarda kullanılan dinamik değişken yapısı script içinde de mümkün hale geldi. [Detaylı bilgi.](/tr/develop/politikalar/script#ortam-değişkenlerine-erişim) APNZ-6267

- **Script Politikasında Diğer Credential'lara Erişim**

Mevcut credential değişkenleri yalnızca isteğin doğrulanmış credential'ını veriyordu; script içinde başka bir credential'a kullanıcı adı veya istemci kimliği ile erişip kimlik bilgileri ve anahtar materyali (parola, sertifika, anahtar çiftleri, key/trust store, JWK) okunabiliyor. [Detaylı bilgi.](/tr/develop/politikalar/script#credential-bilgilerine-erişim) APNZ-6267 APNZ-6279

- **XSLT Politikasında Dinamik Parametre Desteği**

XML dönüşüm politikasında XSLT'ye geçirilen parametreler artık sabit değerin yanı sıra ortam değişkeni veya özel değişken (custom variable) üzerinden dinamik olarak çözümlenebiliyor. [Detaylı bilgi.](/tr/develop/politikalar/xml-donusturme#xslt-parametre-kullanımı) APNZ-6280

- **Credential ve Credential Organization Metadata**

Credential ve credential organization üzerinde arbitrary anahtar/değer metadata girişleri tanımlanabiliyor. Her giriş, bağımsız olarak JWT payload'a claim olarak veya OAuth token endpoint response body'sine ek alan olarak eklenecek şekilde işaretlenebiliyor. Hassas (secret) girişler şifrelenmiş saklanır, API response'larında maskelenir. RFC 7519 / Apinizer dahili rezerve JWT claim isimleri ve OAuth response alanları (Token Yönetim Ayarları'ndaki dinamik isim override'ları dahil) save sırasında reddediliyor. Kurum (organization) seviyesi metadata token oluşturma sırasında credential metadata ile merge edilir (aynı anahtar üzerinde credential öncelik kazanır — child wins). Aynı merged görünüm Groovy/JavaScript script politikalarına `credentialMap.get(username).getMetadata()` ile sunulur; katman bazlı erişim için `getCredentialMetadata()` / `getOrganizationMetadata()` mevcut. `/apiops/projects/{p}/credentials/{u}` ve yeni `/apiops/projects/{p}/credential-organizations/{code}` endpoint'leri programatik yönetim için metadata'yı expose eder. [Detaylı bilgi.](/tr/admin/identity-management/kimlik-bilgileri#metadata) APNZ-6285 APNZ-6286
...
**APNZ-6281** : Script politikası içinden önbellek işlemleri sırasında oluşan hata giderildi.
...
Politika zinciri içinde bağımsız cache okuma, yazma ve silme işlemlerini gerçekleştiren yeni politika tipi eklendi. LOOKUP (cache'ten oku), POPULATE (cache'e yaz) ve INVALIDATE (cache'ten sil) olmak üzere üç operasyon desteklenir. Distributed veya yerel cache storage seçimi, dinamik TTL, hash key, variable interpolation ve hata işleme (CONTINUE/FAULT) özelliklerine sahiptir. [Detaylı bilgi.](/tr/develop/politikalar/onbellekleme) APNZ-6246
...
API Call cache, OIDC politikası, JOSE Key Fetch, Backend API Authentication, Throttling, Quota, Client Ban ve Token Service cache dahil 10+ ekran için ortak `Cache Connection Timeout (saniye)` ve `Error Action (CONTINUE/FAULT)` alanları eklendi. Cache erişim hatalarında davranış artık platform genelinde tutarlı. APNZ-6250

- **Trace Ekranında Politika-Spesifik Yürütme Detayları**

8 politika tipi (Cache, API Call, Backend Auth, JOSE Key Fetch, Client Ban, Throttling, Quota, Business Rule) için trace ekranında "Execution Details" butonu ile politika-tipine özel modal eklendi. Modal, her politika türü için domain-spesifik ayrıntıları gösterir (örn: Cache için operation/hit-miss/TTL; Business Rule için per-action listesi). Request ve response hatlarının her ikisinde de kullanılabilir. [Bkz.](/tr/analytic/tracing) APNZ-6259

<span style={{ display: 'inline-block', backgroundColor: '#e65a2a', color: '#ffffff', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>HATA DÜZELTMELERİ</span>

- **Business Rule Modify Operatörlerinde Specify Value İçinde Variable Kullanımı**

Business Rule politikasında bir aksiyonun Target tarafında **Specify Value** seçildiğinde, **Modify** aksiyonunun **Format**, **Mask**, **Transform** ve **Insert** operatörlerinde girilen değer içinde yer alan `${...}` Environment Variable ve `#{...}` Context Variable yer tutucuları artık runtime'da çözümlenir. Önceden bu operatörlerde girilen değer ham metin olarak ele alınıyor, yer tutucular olduğu gibi kullanılıyordu. APNZ-6248

- **Encoded URL Path İçeren İsteklerin Backend'e İletilmesi**

API Gateway üzerinden gelen ve URL yolunda encode edilmiş özel karakterler (örneğin path içine gömülmüş JSON parametreleri) taşıyan isteklerin backend servise iletilirken "Illegal character in path" hatasıyla reddedilmesi sorunu giderildi. Path içinde yer alan tüm özel karakterler artık tutarlı biçimde yeniden encode edilerek backend'e iletilir. APNZ-6249

- **Koşul Kurallarında Use Custom Value ve Aksiyon Kurallarında Specify Value Alanlarında Variable Kullanımı**

Politika koşul kurallarında (Query Builder) ikinci değer için **Use Custom Value** seçildiğinde ve aksiyon kurallarında **Specify Value** dalında girilen değer içinde yer alan `${...}` Environment Variable ve `#{...}` Context Variable yer tutucuları artık runtime'da çözümlenir. Önceden bu alanlardaki değer literal string olarak karşılaştırılıyor, yer tutucular çözümlenmiyordu. Bu fix tüm politikaların koşul ve aksiyon custom value alanlarını kapsar. APNZ-6253

- **Yönlendirme Adresinde ve Şablon Alanlarında Noktalı Custom Variable Anahtarı (`#{custom.environment}`)**

Script ile `customVariableMap.put("custom.environment", value)` şeklinde set edilen noktalı flat custom variable anahtarlarının `#{custom.environment}` üzerinden okunduğunda boş string dönmesi sorunu giderildi. Yönlendirme adresi (örn. `https://petstore.#{custom.environment}.io/v2`), header/parameter şablonları, koşul ve aksiyon custom value alanları gibi `#{...}` ifadesinin kullanıldığı tüm yerlerde noktalı flat anahtarlar artık doğru biçimde çözümlenir. APNZ-6254

- **API Traffic Ekranında Boşluklu İfadelerle Gövde Arama**

API Traffic ekranındaki gövde arama alanlarında (From Client Body, To Backend API Body, From Backend API Body, To Client Body) ve Gelişmiş Filtreleme sekmesinde CONTAINS / NOT_CONTAINS ile yapılan aramalarda birden fazla kelimeden oluşan ifadeler (örn. `doğum tarihi`) artık doğru sonuçları döndürüyor. Tek kelime aramaları aynı davranışla devam eder. [Bkz.](/tr/analytic/api-trafigi#gövde-alanlarında-arama-body-search) APNZ-6256

- **Action Definition Condition'larında ve Add Aksiyonu Injection Target'larında Variable Kullanımı**

Bir politikanın **Action Definition > Conditions** bölümünde **Specify the Value** alanına girilen `${...}` Environment Variable ve `#{...}` Context Variable yer tutucuları artık runtime'da çözümlenir. Ayrıca Business Rule **Add** aksiyonunun **Injection Target** alanı, hedefin türüne göre doğru bölgeye yazılacak biçimde düzeltildi: body tipi bir Variable Definition hedef olarak seçildiğinde **Body Element Name** verilmişse değer mesaj gövdesinde belirtilen yol altına alt-element olarak eklenir, **Body Element Name** boş bırakıldığında mesaj gövdesi bütünüyle değiştirilir; header, parameter, custom variable veya context value tipi hedefler ise değişkenin tipine uygun bölgeye yazılır. Daha önce yalnızca body altına alt-element ekleme akışı çalışıyor, all-body ve diğer hedef türleri eksik veya hatalı sonuç üretebiliyordu; ayrıca body için içerik tipinin belirsiz olduğu durumlarda işlem beklenmedik şekilde başarısız olabiliyordu. APNZ-6257 APNZ-6258
...
Elasticsearch Monitor ekranına, ham tanılama çıktılarını Gateway Diagnostics tarzında tek noktadan sunan **Küme Tanılama** sekmesi eklendi. Sekme; Küme Sağlığı (`_cluster/health`), Küme Ayarları (`_cluster/settings`), Shard Listesi (`_cat/shards`), bağlantıya tanımlı ILM Politikası (`_ilm/policy/{ad}`), Bekleyen Görevler, Yerleşim Açıklaması, İndeks Şablonları ve Aktif Recovery panellerini içerir. Her panelin içeriği biçimlendirilmiş JSON olarak tek tıkla panoya kopyalanabilir. [Bkz.](/tr/integrations/baglanti-yonetimi/elasticsearch-monitor#küme-tanılama) APNZ-6177

<span style={{ display: 'inline-block', backgroundColor: '#bfdbfe', color: '#1e40af', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>DEĞİŞİKLİKLER VE İYİLEŞTİRMELER</span>

- **OAuth/JWT Token Endpoint'lerinde Authorization Basic Header Desteği (RFC 6749 §2.3.1)**

Hem proxy hem credential token endpoint'lerinde istemci kimlik bilgileri (`client_id` ve `client_secret`) artık HTTP Basic Authorization header'ı ile gönderilebilir. RFC 6749 §2.3.1 `client_secret_basic` standardı tam desteklenir; Spring Security, MSAL ve `oauth4webapi` gibi varsayılan olarak Basic header kullanan istemciler artık Apinizer token endpoint'lerine entegre olabilir. Aynı istekte hem header hem body parametresi gönderilmesi yasaktır (HTTP 400 `invalid_request`); bozuk Basic header HTTP 401 (`invalid_client`) ile reddedilir; Basic dışı şemalar (Bearer vb.) görmezden gelinir ve body parametrelerine fallback yapılır. [Bkz.](/tr/develop/politikalar/oauth2) APNZ-6231

<span style={{ display: 'inline-block', backgroundColor: '#e65a2a', color: '#ffffff', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>HATA DÜZELTMELERİ</span>

- **Özelleştirilmiş Hata Mesajları Boş Dönmüyor**

Politika veya API yönlendirme seviyesinde özelleştirilmiş hata kaydında yalnızca özel HTTP durum kodu veya özel hata kodu doldurulup mesaj alanı boş bırakıldığında istemciye boş yanıt gövdesi dönmesi sorunu giderildi. Özel mesaj boş bırakıldığında artık sistem tarafından tanımlanan varsayılan hata mesajına geri dönülür. Düzeltme politika hatası, API yönlendirme hatası ve genel sistem hatası akışlarının tümünde tutarlı şekilde uygulanır; etkilenen politika veya yönlendirmelerin yeniden oluşturulmasına gerek kalmaz. Script politikasında çalışma zamanında özel hata mesajı atayan davranış aynen korunur. APNZ-6209

- **OIDC Politikasında Yanıt Çerezi Tutarsızlığı Düzeltildi**

OIDC oturum çerezi yazımındaki `SameSite` ve `Secure` davranışı, OIDC akışının referans uygulaması ile birebir uyumlu hale getirildi. Aynı yanıtta birden fazla `Set-Cookie` üretildiğinde çerezlerin birbirini ezmesine yol açan hata giderildi. `state` doğrulaması etkinken `state` parametresi olmadan gelen callback istekleri CSRF koruması gereği reddedilir; süresi dolmuş geçici oturumlarda nonce doğrulaması artık güvenli biçimde sonuçlanır; oturum sona erdiğinde oturum verisi tutarlı şekilde temizlenir. **Add Token To Cookie** seçeneği etkinken erişim, kimlik ve yenileme token'ları için ayrı `HttpOnly` çerezler yazılması sağlandı. APNZ-6230

- **x-www-form-urlencoded Body ile Liste İki Yönlü Senkronize Tutuluyor**

`application/x-www-form-urlencoded` istekleri için body içeriği ile form parametre listesi artık trafik logu ve script politikası akışlarında çift yönlü senkron tutulur. Önceden bu durumda boş kalan body içeriği artık form parametreleriyle aynı içeriği gösterir. Script politikalarında yalnızca body alanına yazan script'ler ile yalnızca form parametre listesi üzerinden değişiklik yapan script'ler aynı sonucu üretir; her iki taraf da değiştirildiğinde parametre listesi öncelikli olur. APNZ-6229

- **Elasticsearch Index Lifecycle Policy Save & Deploy Sonrası Güncellenmesi**

Elasticsearch bağlantısında Edit Index Lifecycle Policy ekranında phase değerleri değiştirildikten sonra Save & Deploy ile değişikliklerin Elasticsearch'e gönderilmemesi sorunu giderildi. ILM policy adı aynı kalsa bile faz değerlerindeki değişiklikler artık her Save & Deploy işleminde Elasticsearch'e iletilir. APNZ-6185

- **API Trafik Kaydı İndirildiğinde Büyük Body'ler Tam Aktarılıyor**

API Trafik kaydı indirme/dışa aktarma işleminde 5 MB üstü istek/yanıt body içeriklerinin ZIP içerisine "Body size exceeds the display limit..." yer tutucusu olarak yazılması sorunu giderildi. Büyük body içeriklerini ekrana yüklememek için uygulanan kısaltma davranışı arayüzde aynen korunur; indirme işlemi ise artık trafik kaydında tutulan tam body içeriğini ZIP'e yazar. APNZ-6235

- **Upgrade Sonrası Project Owner ve API Developer Rollerine Eksik EXECUTE Yetkileri Eklendi**

Eski sürümlerden yükseltilen ortamlarda Project Owner rolünde Testing kategorisinin Execute yetkisinin ve sonradan eklenen diğer varlık (asset) eylem yetkilerinin; API Developer rolünde ise Testing kategorisinin View ve Execute yetkilerinin eksik kalması nedeniyle Quick Test ve REST API Test çağrılarının HTTP 401 hatası alması sorunu giderildi. Yükseltme sonrasında eksik yetkiler mevcut yetkilere dokunulmadan otomatik olarak tamamlanır. APNZ-6234

- **LDAP Kullanıcıları Proje Listesini Görebiliyor**

LDAP üzerinden Apinizer'a giriş yapan kullanıcıların proje listesini boş görmesi sorunu giderildi. Proje listesi sorgusu artık LDAP kullanıcıları için de standart yetkilendirme kontrolüyle aynı kuralları izler; kullanıcının doğrudan proje üyeliği bulunmasa bile LDAP kullanıcı adı veya LDAP grup üyelikleri üzerinden erişim sağladığı projeler listelenir. APNZ-6233

- **Global Politika View Sayfasında Silme İşlemi Onay Diyaloğu ile Korunuyor**

Global Politikalar altındaki bir politikanın View sayfasında Delete butonuna basıldığında onay diyaloğu gösterilmeden silme işleminin gerçekleşmesi sorunu giderildi. Silme işlemi artık standart onay diyaloğu üzerinden yürütülür; silinmek istenen politika bir veya daha fazla API Proxy tarafından kullanılıyorsa kullanıcı, silmeden önce bağımlılıklar hakkında uyarılır. Aynı korumalı silme akışı tüm global politika türleri için geçerlidir. APNZ-6149

- **Business Rule Add Aksiyonunda Specify Value İçinde Variable Kullanımı**

Business Rule politikasında bir aksiyonun Target tarafında **Specify Value** seçildiğinde, **Add** aksiyonu için girilen değer içindeki `${...}` Environment Variable ve `#{...}` Context Variable yer tutucuları artık runtime'da çözümlenir. Önceden bu yer tutucular ham metin olarak ele alınıyor ve istek/yanıt mesajına olduğu gibi yazılıyordu. APNZ-6226
...
Daha önce Genel Ayarlar altında platform geneli olarak yönetilen "engellenme durumunda devre dışı alanları logla" ve "hata durumunda devre dışı alanları logla" anahtarları kaldırıldı; artık her konnektör için ayrı ayrı yapılandırılır. API Trafik Log Ayarları liste ekranı da yenilenerek konnektör adları okunabilir biçimde gösterilir hale getirildi. Mevcut platform geneli ayarlar her ortamdaki konnektörlere otomatik olarak taşınır. [Bkz.](/tr/develop/api-proxy-konfigurasyonu/api-trafik-log-ayarlari) APNZ-6155

- **Container İmajlarına curl Paketi Eklendi**

API Manager, Gateway Worker, Cache Server ve Integration imajlarında ağ tanılama komutları (curl) artık doğrudan kullanılabilir. APNZ-6170

- **API Proxy ve Proxy Group Düzenlemesinde Çoklu Erişim Yolu Listesinde Proje Öneki Korunuyor**

Proje düzeyinde relative path tanımlı olduğunda, API proxy ve proxy group düzenleme ekranlarındaki çoklu erişim yolu listesine proje öneki ile başlayan bir değer girildiğinde önekin kaydedilirken kaybolması sorunu giderildi. Erişim yolu listesi, tek erişim yolu alanı ile aynı kural ile işlenir hale getirildi; girilen değer her durumda proje öneki ile birleştirilerek kaydedilir. APNZ-6120

<span style={{ display: 'inline-block', backgroundColor: '#e65a2a', color: '#ffffff', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>HATA DÜZELTMELERİ</span>

- **Yanıt Sıkıştırma Akışında Base64 Content-Encoding Tutarsızlığı Düzeltildi**

Müşteride `net::ERR_CONTENT_DECODING_FAILED` hatasına neden olan, sıkıştırılmış yanıt gövdesinin Base64'e çevrildiği durumlarda Content-Encoding başlığının yine `gzip`/`deflate`/`br`/`zstd` olarak yazılması sorunu giderildi. Ayrıca routing düzeyinde yanıt sıkıştırma override için iki yeni seçenek eklendi: `MIRROR_SERVER_RESPONSE` (arka uç yanıtını aynalar) ve `AUTO_BY_CLIENT_ACCEPT_ENCODING` (istemci `Accept-Encoding` başlığına göre otomatik sıkıştırır). Mevcut yapılandırmaların davranışı korunur. APNZ-6160

- **Token Audit Menü Görünürlüğü Düzeltildi**

Sadece sistem yöneticisi yetkisine sahip kullanıcıya Token Audit menüsünün gösterilmesi ve tıklandığında Erişim Reddedildi hatası alınması sorunu giderildi; menü görünürlüğü erişim kontrolü ile tutarlı hale getirildi. APNZ-6171

- **Script Politikasında Özel Hata Yanıt Gövdesi Korunuyor**

Script politikasında özel hata yanıt gövdesi ile akış durdurulduğunda, bu gövdenin hata yönetimi sürecinde proxy şablonu tarafından ezilmesi sorunu giderildi. APNZ-6169

- **Oracle Türkçe Karakter Seti Desteği**

Oracle Türkçe karakter seti (WE8ISO8859P9) ile veritabanı bağlantısı kurulamaması sorunu giderildi. APNZ-6168

- **API Trafik Ekranında Gövde Araması Düzeltildi (Elasticsearch 7.9.2)**

Elasticsearch 7.9.2 sürümünde API Trafik ekranındaki istek ve yanıt gövdesi arama alanlarında, eşleşen kayıt olmasına rağmen sonuç dönmemesi sorunu giderildi. Gelişmiş sekmesindeki anahtar kelime alanı da artık gövde içinde arama yapar. APNZ-6166

- **WSDL Servis Port Adı Endpoint Yeniden Yazımında Korunur**

WSDL endpoint yeniden yazımı sırasında servis port adının yanlışlıkla binding adıyla değiştirilmesi sorunu giderildi. Port adı artık orijinal servis sözleşmesindeki haliyle korunur; SOAP 1.1 ve SOAP 1.2 portları bağımsız olarak ele alınır. APNZ-6164
...
APIOps üzerinden deploy history listeleme, detay görüntüleme, silme ve rollback işlemleri desteklenir hale getirildi. Deployment'lara açıklama ekleme ve kalıcı history tutma parametreleri de eklendi. APNZ-6108

- **Log Tabloları için Şema ve Özel Tablo Adı Yapılandırması**

Veritabanı bağlantısı log hedefi olarak kullanıldığında her log tablosu için şema ön eki ve özel tablo adı ayarlanabilmesi sağlandı. Oracle, PostgreSQL, MSSQL, MySQL ve DB2 üzerinde varsayılan şema tanımı ile tablo bazlı özelleştirme desteklenir. Küçük/büyük harf duyarlı tablo adları için çift tırnak, backtick ve köşeli parantez ile kimlik adı biçimleri kabul edilir. [Bkz.](/tr/integrations/baglanti-yonetimi/database) APNZ-6133

<span style={{ display: 'inline-block', backgroundColor: '#bfdbfe', color: '#1e40af', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>DEĞİŞİKLİKLER VE İYİLEŞTİRMELER</span>

- **Syslog TCP Async Kuyruk ve Watchdog**

Syslog TCP bağlantısı blocking write sorununa karşı async kuyruk ve watchdog mekanizması ile güçlendirildi. TCP protokolü üzerinden gönderilen Syslog mesajları artık asenkron bir kuyrukta işlenerek ağ yavaşlamalarından etkilenmeden iletilir. Kuyruk kapasitesi ve write timeout süresi yapılandırılabilir hale getirildi. APNZ-6111

- **Compress/Decompress Hata Düzeltmeleri ve İyileştirmeler**

gzip ile sıkıştırılmış büyük yanıtlarda veri kaybı yaşanan bir hata düzeltildi. İstek gövdesinin binary modda iletildiği durumlarda oluşan yeniden sıkıştırma hatası giderildi. Routing bazında istek ve yanıt için charset ve içerik kodlama override ayarları yapılandırılabilir hale getirildi. APNZ-6109, APNZ-6114, APNZ-6117

<span style={{ display: 'inline-block', backgroundColor: '#e65a2a', color: '#ffffff', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>HATA DÜZELTMELERİ</span>

- **Cache HTTP/2 Desteği HTTP/1.1'e Geri Döndürüldü**

Java 25 geçişi sonrasında eklenen HTTP/2 desteğinin tüm müşteri load balancer'larıyla uyumlu olmaması nedeniyle cache erişimi HTTP/1.1'e geri döndürüldü. APNZ-6106

- **Oracle ve İlişkisel Veritabanı Log SQL Düzeltmeleri**

Oracle ve diğer ilişkisel veritabanlarında API trafik log tablolarının oluşturulması için kullanılan SQL komutlarındaki hatalar düzeltildi. APNZ-6112

- **API Trafik Log Ayarlarında Eksik Alanlar Düzeltildi**

API trafik loglama bağlantısı olarak veritabanı seçildiğinde bazı yapılandırma alanlarının görünmemesi sorunu giderildi. API proxy bulunamadığında veya tanımsız olduğunda trafik loglarının hedef bağlantıya kaydedilmesi, gönderilemeyen isteklerin yedek bağlantıya aktarılması ve API trafik gizlilik ayarlarının yapılandırılabilmesine yönelik alanlar artık veritabanı bağlantılarında da doğru şekilde görüntülenir ve çalışır. APNZ-6130
...
<Note>Bu özellik yalnızca HTTP context'inde (MessageContextHttp) geçerlidir.</Note>
