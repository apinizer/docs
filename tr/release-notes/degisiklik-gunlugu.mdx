...
- **Proje tanımına çoklu etiket alanı eklendi ve proje seçim penceresinde etikete göre filtreleme yapılabilmesi sağlandı.** Projeleri birim adı, ilgili kurum veya kendi belirlediğiniz başka bir sınıflandırmaya göre etiketleyebilir; çok sayıda projeniz olduğunda üst menüdeki proje seçim penceresinden etiketlerine göre hızlıca filtreleyebilirsiniz. [Bkz.](/tr/admin/project-management/proje-olusturma) APNZ-4954

- **Anahtar deposu, güven deposu ve anahtar tanımlarına dışa/içe aktarma (export/import) desteği eklendi.** Sertifikalarda olduğu gibi; anahtar depolarını, güven depolarını ve anahtarları (özel, açık ve gizli anahtar) kendi dosya formatında veya farklı bir Apinizer ortamına taşınabilen yapılandırma paketi olarak dışa aktarabilir, aynı dosyaları içe aktararak ortamlar arasında taşıyabilirsiniz. [Bkz.](/tr/admin/secrets-management/keystore-yonetimi) APNZ-4959

- **Bir API proxy'sinin ağ geçidi çağrı bilgisini döndüren yeni bir APIops ucu eklendi.** Bir proxy'nin ağ geçidi çağrı URL'ini oluşturmak için daha önce ağ geçidi temel URL'i, platform genelindeki kök bağlam ve proxy'nin göreli yolu üç ayrı yerden birleştiriliyor; bu da otomatik ve uçtan uca testleri hataya açık hale getiriyordu. Tek bir APIops ucu artık her ortam için ağ geçidi temel URL'ini, kök bağlamı, tam yolu, kabul edilen HTTP metotlarını ve desteklenen şemaları döndürüyor; böylece CI/CD ve test çağıranlar ön eki sabit kodlamadan çağrı URL'ini tek adımda oluşturabiliyor. [Bkz.](/api-reference/api-proxies/deployment/get-invoke-info) APNZ-6352

- **İzin Verilen / Engellenen IP Listesi politikalarına artık Management API (APIops) üzerinden IP Grubu eklenebiliyor.** Önceden APIops ile politika oluşturma/güncelleme isteklerinde yalnızca doğrudan IP adresleri ve CIDR aralıkları (`ipList`) verilebiliyor, IP Grubu bağlanamıyordu. Yeni `ipGroupNameList` alanı ile bir veya daha fazla önceden tanımlı IP Grubunu adıyla ekleyebilirsiniz; böylece otomasyon ve CI/CD akışları IP grubu tabanlı izin/engelleme listelerini arayüzdeki gibi yönetebilir. [Bkz.](/api-reference/policies/policies/policy-white-ip) APNZ-6377
...
**APNZ-6373** : APIops tarafında iki sorun düzeltildi. (1) Trafik log ayarlarının metot bazlı geçersiz kılma (override) uçları (güncelleme ve silme), `/` ile başlayan metot adlarında, eğik çizgi `%2F` olarak URL-kodlanmış gönderilse bile çalışmıyordu; kodlanmış eğik çizgi API Manager tarafında çok erken çözüldüğünden metot eşleştirilemiyordu. Bu uçlar artık başında eğik çizgi bulunan metot adlarını doğru şekilde çözüyor. (2) Bir kimlik bilgisi (credential) APIops üzerinden silindiğinde, anlamlı bir durum yerine HTTP 500 hatası dönebiliyordu; uç artık uygun durum kodunu (örneğin kimlik bilgisi bulunmadığında açık bir mesajla 400) döndürüyor.

**APNZ-6301** : Bir Proxy Grubu içindeki proxy'nin Geliştir sekmesinde, görüntüleme modunda bir politika silinmeye çalışıldığında onay verilse de politika listeden kaldırılmıyor, kayıt silinmemiş şekilde listede kalmaya devam ediyordu. Görüntüleme modundaki politika silme işlemi artık doğru şekilde uygulanıyor.

**APNZ-6247** : SOAP servislerinde Metot Ayarları altındaki WSA Ayarları bölümünde tüm alanların doldurulması zorunlu tutulduğundan, yalnızca gereken alanları doldurup kaydetmek mümkün olmuyordu. Artık istenen alanlar doldurulup diğerleri boş bırakılarak kaydedilebiliyor.

**APNZ-5978** : API Portal'da bir API Product detayından tüm ürün listesine dönmek için yeniden "API'ler" sekmesine basmak gerekiyordu ve bu alan buton gibi algılanmıyordu. API Product detay görünümüne net bir "Listeye Dön" navigasyonu eklendi; böylece ürün listesine doğrudan dönülebiliyor.

**APNZ-6295** : Content-Length başlığı Hop-By-Hop Başlık listesinden kaldırılıp bir politika ile ayarlandığında istek arka uca ulaşmıyor ve istemciye hata dönüyordu ("Content-Length header already present"). Ağ geçidi, arka uca giden isteği oluştururken Content-Length gibi çerçeveleme başlıklarını artık kendisi yönetiyor; böylece Content-Length değeri bir politika ile ayarlanan istek arka uca başarıyla iletiliyor.

**APNZ-4929** : Analitik grafiklerinde tarih aralığı olarak "Today" (Bugün) seçildiğinde, grafik zaman dilimleri sunucunun yerel saat dilimi yerine UTC'ye göre gruplandığından zaman ekseni 00:00 yerine 03:00'ten başlıyordu. Zaman dilimleri artık yerel saat dilimine hizalanıyor; böylece "Bugün" görünümü günü 00:00'dan başlatıyor ve seçilen aralığın geri kalanıyla uyumlu hale geliyor.

**APNZ-6342** : Bir projede Testing modülü için "Yönet" yetkisine sahip kullanıcılar, Test Konsolu üzerinde yaptıkları işlemleri kaydedemiyor; kaydetme işlemi yetki hatasıyla başarısız oluyor ve yalnızca Proje Sahibi, Proje Yöneticisi veya Sistem Yöneticisi rolüne sahip kullanıcılar kaydedebiliyordu. Bunun nedeni, rol tanımı ekranında Testing kategorisi için "Yönet" eyleminin seçilebilir eylemler arasında sunulmaması ve bu yetkinin gerçekte hiç atanamamasıydı. Testing kategorisi artık Görüntüle, Yönet ve Çalıştır eylemlerini sunuyor; böylece yetki atanabiliyor ve Test Konsolu kayıt işlemi beklendiği gibi çalışıyor.

**APNZ-6341** : Rol tanımı ekranında (Kullanıcı → Roller), her yetki grubunun yanında parantez içinde gösterilen sayı, o gruptaki işaretli yetki kutucuğu sayısını yansıtmıyordu; örneğin yalnızca iki kutucuğu bulunan bir grup için bu sayı daha yüksek (örneğin "Testing (5)") görünebiliyordu. Parantez içindeki sayı artık ilgili gruptaki gerçek seçili yetki sayısını gösteriyor.

**APNZ-6152** : Global Politikalar bölümünde, kullanımda olan bir İş Kuralı (Business Rule) veya API Bazlı Hız Sınırlama (API Based Throttling) politikası silinmeye çalışıldığında "Silme işlemini onayla" penceresi boş açılıyor; politikanın hangi proxy'lerde kullanıldığına dair uyarı ve etkilenen proxy listesi gösterilmiyordu. Bu iki politika tipinin silme onay penceresi artık diğer politika tiplerinde olduğu gibi kullanım bilgisini ve etkilenen proxy'leri gösteriyor.

**APNZ-6151** : Global Politikalar liste ekranında (Geliştirme → Global Ayarlar → Global Politikalar), silme aksiyonu JOSE Validation, JOSE Implementation ve Client Ban politika tiplerinde "Delete" yerine "Remove" olarak gösteriliyor; diğer politika tiplerinde ise "Delete" görünüyordu. Aksiyon artık tüm politika tiplerinde tutarlı şekilde "Delete" olarak gösteriliyor.

**APNZ-5966** : API İzleme altındaki Uptime Monitor'da, grafikte zaman aralığını daralttığınızda (tarih/saat yakınlaştırma alanında) aralığın başında ve sonunda gösterilen başlangıç ve bitiş tarih/saat bilgisi kayboluyordu. Seçilen aralığın başlangıç ve bitiş tarih/saati artık grafiğin altında gösteriliyor; böylece daraltılan aralığın sınırları görünür kalıyor.
...
**APNZ-6297** : Özelleştirilmiş hata yanıtlarında, URL veya benzeri dış kaynaklardan yansıtılan metin içindeki `#{...}` ifadelerinin hata şablonu çözümlemesinden geçirilmesi nedeniyle istenmeyen ifade değerlendirmesi ve bilgi ifşası riski oluşuyordu. Yansıtılan kullanıcı girdisi artık ifade motorundan geçirilmeden literal olarak işleniyor; güvenlik açığı kapatıldı.
