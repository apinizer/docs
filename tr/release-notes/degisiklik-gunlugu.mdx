...
- **API Proxy Group Üzerinden SOAP İsteklerinde Gövde Kaybı ve Log Eksikliği**

API Proxy Group üzerinden SOAP isteği atıldığında istek gövdesinin proxy seçimi sırasında erken okunup ardından silinmesi nedeniyle backend'e boş POST iletilmesi ve 503 ERR-070 hatası dönmesi sorunu giderildi. Aynı istek doğrudan API Proxy üzerinden başarılı dönerken proxy group üzerinden başarısız oluyordu; proxy ve proxy group log ayarları açık olsa bile ilgili trafik log kayıtlarında gövde alanları boş görünüyordu. SOAP method tespiti artık **SOAPAction** header dolu olduğunda gövdeyi okumadan çalışır; gövde okunması gerektiğinde sıkıştırma açma ve charset doğru uygulanır. İstek pipeline'ında gövde zaten yüklendikten sonra tekrar hazırlama adımının gövdeyi silmesi engellendi. APNZ-6260

- **Proxy Group — API Proxy Log Ayarı Değişikliklerinin Proxy Group'a Yansımaması**

Bir API Proxy Group'a bağlı API Proxy'nin log ayarlarında yapılan değişikliklerin ilgili Proxy Group log ayarlarına yansımaması sorunu giderildi. API Proxy üzerinde güncellenen log yapılandırması artık bağlı olduğu Proxy Group'a da senkronize edilir. APNZ-6266
...
- **Özelleştirilmiş Hata Mesajları Boş Dönüyor**
