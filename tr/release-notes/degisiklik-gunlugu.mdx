...
**APNZ-5802** : API Manager'da aynı API Proxy için birden fazla spec_api kaydı oluşma sorunu giderildi.
...
- **Şablon tabanlı mesajlar:** Serbest metin yazın; istek/hata bilgilerini placeholder'larla otomatik dahil edin (örn: `#{error.errorCode}`, `#{request.uri}`, `#{dateTime.formattedText}`, `#{message.correlationId}`).
...
**Örnek şablon:** `API çağrınız başarısız oldu. Hata Kodu: #{error.errorCode} | Zaman: #{dateTime.formattedText} | İstek ID: #{message.correlationId}`
...
- **Error template placeholder'ları:** `#{error.errorCode}`, `#{error.message}`, `#{error.httpStatusCode}`, `#{error.customizedMessage}` vb.
- **Variable türleri:**
