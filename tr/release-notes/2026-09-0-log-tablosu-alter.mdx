---
title: "2026.09.0 — Log Tablosu ALTER Scriptleri"
description: "2026.09.0 sürümüne yükseltilen SQL/JDBC trafik-log kurulumları için ALTER TABLE scriptleri."
slug: /release-notes/2026-09-0-log-tablosu-alter
unlisted: true
---

Bu sayfa listelenmemiştir (unlisted) — sidebar'da ve arama sonuçlarında görünmez, yalnızca 2026.09.0 sürüm notlarındaki doğrudan bağlantı üzerinden erişilmek üzere tasarlanmıştır.
...
Apinizer 2026.09.0, `log_ApiProxyTraffic` tablosuna AI Gateway, MCP Gateway, A2A Gateway ve routing-diagnostics gözlemlenebilirlik kolonlarını ekler. Bu kolonlar 2026.04.0'da (veya herhangi bir önceki sürümde) mevcut değildi. Yeni kurulumlar bu kolonları CREATE TABLE scripti üzerinden otomatik olarak alır. **2026.04.0'dan (veya bir 2026.04.x patch'inden) yükseltme yapan mevcut kurulumların, 2026.09.0 devreye almadan önce veya sırasında, aşağıdaki ALTER scriptini her ortam veritabanı için bir kez çalıştırması gerekir.**
...
ALTER TABLE log_ApiProxyTraffic ADD (
    ...
    -- AI Guardrail dış servis kararı (SAFE / UNSAFE / TIMEOUT)
    ai_guardrail_verdict VARCHAR2(32),
    -- MCP/A2A giden araç/beceri denetim izi (APNZ-6557)
...
Script'i çalıştırdıktan sonra `log_ApiProxyTraffic` tablosundaki kolon sayısının 61 arttığını ve (oluştuğunda) yeni bir AI/MCP/A2A trafik kaydının sessizce atlanmak yerine ilgili kolonları doldurduğunu doğrulayın. Geçmiş satırlar için veri backfill'i gerekmez ve mümkün değildir — yeni kolonlar, ALTER çalıştırılmadan önce loglanmış tüm trafik için `NULL` olacaktır.
