...
- Api Manager ile yapılabilen işlevselliğin tamamına yakınının Management API'ler ile yapılabilmesi sağlandı ve tüm dökümantasyonu yenilendi. [Bkz.](/api-reference/api-proxies/deployment/deploy-api-proxy)
...
İstemciler, API Proxy veya API Proxy Group adreslerine tek bir "Relative Path" üzerinden erişebilirken artık çoğul "Relative Path"ler ile veya Hostname ile hiç "Relative Path" vermeden erişebilir hale geldi. [Bkz.](/tr/develop/api-proxy-konfigurasyonu/client-route)

- ### Environment Variable özelliği getirildi

Bir varlığa ait değerin farklı ortamlarda ortama özel değer alabilmesi sağlandı. Örneğin veri tabanı bağlantısındaki username ve password alanlarını test ve prod ortamlarına özel değerler girmeden ve ayrı bağlantı tanımlamaya gerek kalmadan ortama özel olarak çalıştırılabilir hale getirildi. [Bkz.](/tr/develop/environment-variables)

- ### OIDC Politikası eklendi

Mevcut sistemde Script Politikası ile yapılabilen OIDC işlemlerinin direk Politika şeklinde yapılabilmesi sağlandı. [Bkz.](/tr/develop/politikalar/oidc)
...
**Timestamp Politikası:** WS-Security politikalarına tek başına Timestamp ekleme desteği eklendi. SOAP mesajlarına zaman damgası eklenerek mesajın geçerlilik süresini kontrol edilmesi sağlandı. [Bkz.](/tr/develop/politikalar/ws-timestamp)
...
**WS-Security Encrypt Politikası:** WS-Security politikalarına tek başına Encrypt yapabilme desteği eklendi. SOAP mesajı içeriğinin şifrelenmesini sağlayarak veri gizliliğinin arttırılması sağlandı. [Bkz.](/tr/develop/politikalar/ws-sifreleme)

**WS-Security Decrypt Politikası:** WS-Security politikalarına tek başına Decrypt yapabilme desteği eklenmiştir. Şifrelenmiş SOAP mesajlarının çözülmesi ve güvenli şekilde işlenmesi desteklendi. [Bkz.](/tr/develop/politikalar/ws-decrypt)

**WS-Security Sign Validation Politikası:** WS-Security politikalarına tek başına Sign Validation yapabilme desteği eklenmiştir. SOAP mesajlarının doğrulanmasını sağlayarak mesaj bütünlüğünü ve kimlik doğrulaması sağlandı. [Bkz.](/tr/develop/politikalar/ws-guvenlik-imza-dogrulama)

- ### WS-Security Politikalarının Grup Düzeyinde Kullanımı

WS-Security politikaları artık Politika Grupları seviyesinde tanımlanabilir hale getirildi. [Bkz.](/tr/develop/global-configurations/policy-groups)
...
Sistem altyapısına FTP Connection desteği eklendi. [Bkz.](/tr/integrations/baglanti-yonetimi/ftp)

- ### FTP Connector'a "Read" ve "List" İşlevleri Eklendi

FTP Connector artık dosya okuma (Read) ve listeleme (List) işlemlerini desteklemektedir. [Bkz. Read](/tr/integrations/konnektorler/ftp-read-konnektor) [Bkz. List](/tr/integrations/konnektorler/ftp-list-konnektor)
...
Upstream Routing'de eklenen adresler "Environment Variable" üzerinden seçilebilir hale getirilerek export/import işlemleri yapıldığında bulunduğu ortam değeri ile çalışabilmesi sağlandı. [Bkz.](/tr/develop/yonlendirme/http-yonlendirme)

- ### Yeni Management API endpointleri eklendi.

Toplam 107 Management API Endpoint ile 163 farklı özellik yapılandırması hizmete açıldı. [Bkz.](/api-reference/api-proxies/deployment/deploy-api-proxy)

- ### Ortamlara asenkron işlemler iş parçacığı havuzu desteği eklendi.

Ortamlarda gerçekleştirilen asenkron işlemler artık ayrı bir thread havuzu tarafından yönetilebiliyor ve parametrelerle ayarlanabiliyor. [Bkz.](/tr/admin/server-management/gateway-runtime-lari)
...
Birden fazla lokasyondaki Kubernetes kümelerinde Active-Active mimari ile yüksek erişilebilirlik ve bölgesel yük dağılımı desteği eklendi. [Bkz.](/tr/admin/server-management/gateway-runtime-lari)

- ### Server Side Streaming Desteği

Sunucudan istemciye sürekli veri akışı sağlayan Server Side Streaming özelliği eklendi. [Bkz.](/tr/develop/yonlendirme/http-yonlendirme)
...
WebSocket ve HTTP protokolleri artık aynı port üzerinden eş zamanlı çalıştırılabilir hale getirildi. [Bkz.](/tr/admin/server-management/gateway-runtime-lari)
:::warning
...
Cache performans ayarları için yeni yapılandırma parametreleri eklendi. [Bkz.](/tr/admin/server-management/dagitik-cache)
...
Routing sekmesine SSL doğrulamasını devre dışı bırakmak için Disable Ssl Validation alanı eklendi. [Bkz.](/tr/develop/yonlendirme/http-yonlendirme)
:::warning
...
İstemci kimliği bazlı erişim kontrolü için Client Ban Politikası oluşturuldu. Routing sekmesinde yer alan eski basit hali kaldırıldı. [Bkz.](/tr/develop/politikalar/istemci-yasaklama)
...
Credential yönetimi Apinizer üzerinden yapılmayan kurumlar için Credential'lara "external" adında yeni bir alan eklendi. [Bkz.](/tr/admin/identity-management/rate-limit-kontrol-listesi)
...
API Proxy oluşturma seçeneklerine iki yeni tip eklendi: gRPC, WebSocket. [Bkz.](/tr/develop/api-proxy-olusturma/grpc-api-proxy-olusturma) [Bkz. WebSocket](/tr/develop/api-proxy-olusturma/websocket-api-proxy-olusturma)
...
Gateway Runtime ortamlarına "İletişim Protokolü Tipi" alanı getirildi. API Proxy'lerin deployment ortamları, proxy türlerine göre otomatik olarak eşleştirilecek şekilde düzenlendi. REST ve SOAP API Proxy'ler HTTP tipi Gateway Runtime ortamlarına, gRPC API Proxy'ler gRPC tipi Gateway Runtime ortamlarına ve WebSocket API Proxy'ler WebSocket tipi Gateway Runtime ortamlarına deploy edilebilir hale getirildi. Bu güncelleme ile proxy türü ve deployment ortamı uyumluluğu garantilenerek yanlış ortam seçimlerinin önüne geçildi. [Bkz.](/tr/admin/server-management/gateway-runtime-lari)
...
IP kontrollerini Geolocation verisine (Ülke,İl) göre kontroller yapılabilmesi sağlandı. [Bkz.](/tr/admin/system-settings/ip-geolocation-ayari)
...
Ayarlar Grubu ekranı ile hem API Proxy'e hem de API Proxy Grubuna Global Ayarlar oluşturulup atanabilmesi sağlandı. [Bkz.](/tr/develop/global-configurations/global-settings-group)

- ### Politika Grubu Yönetimi

Politika Grubu ekranı ile hem API Proxy'e hem de API Proxy Grubuna Politika Grubu oluşturulup atanabilmesi sağlandı. [Bkz.](/tr/develop/global-configurations/policy-groups)
...
Tüm Deploy işlemlerininin detaylı bir şekilde sonucunun gösterilmesi sağlandı. [Bkz.](/tr/develop/yukleme-ve-surum-yonetimi)
:::warning
...
- API Proxy'lere bakım modu özelliği eklendi. [Bkz.](/tr/develop/api-proxy-konfigurasyonu/settings#api-bakim-modu)

- Deployment süresi için timeout değerleri, parametrik olarak yapılandırılabilir hale getirildi. [Bkz.](/tr/admin/server-management/gateway-runtime-lari)

- TLS ayarları JVM parametreleri ile parametrik verilebilir hale getirildi. [Bkz.](/tr/admin/server-management/gateway-runtime-lari#güvenlik-yapılandırması)
