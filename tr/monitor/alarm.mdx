...
- **Infrastructure (Altyapı)**: Kubernetes pod/node sağlığı, K8S Node CPU, Elasticsearch sağlık/CPU/disk, Gateway Thread Sayısı
...
<CardGroup cols={2}>
  ...
  <Accordion title="Gateway Tetiklenme Tipleri">
    **Gateway Thread Sayısı (Gateway Thread Count)**: Ortam üzerinde tanımlı Gateway Management API adres(ler)i (Ortam > Management API ayarları) üzerinden her API Gateway worker pod'unun anlık JVM thread sayısını okur ve her pod'u ayrı ayrı değerlendirir. Herhangi bir pod'un thread sayısı eşik değerini **aştığında** alarm tetiklenir (katı karşılaştırma: tam eşik değerindeki pod *Uyarı* olarak gösterilir, alarm tetiklenmez). Eşik değeri gerektirir (örn: 2000 = 2000 thread). Bağlantı tanımı gerektirmez; ancak ortamda en az bir Gateway Management API adresi tanımlı olmalıdır.

    İsteğe bağlı **Ortam** seçicisi kontrolü tek bir ortamla sınırlar; boş bırakıldığında yayındaki tüm ortamlar kontrol edilir. Alarm geçmişinde her pod ortamı, IP'si ve ölçülen thread sayısıyla listelenir. Okunamayan bir pod ya da adres değer olmadan *Uyarı* olarak listelenir ve tek başına alarmı tetiklemez; hiçbir pod okunamadığında (örneğin tüm adresler erişilemez olduğunda, ortamda management adresi tanımlı olmadığında ya da seçili ortam silindiğinde) koşu *Hata Oluştu* olarak kaydedilir. E-posta ve webhook şablonlarında eşik `<n> threads` biçiminde yazılır.
  </Accordion>
...
- Gateway Thread Sayısı (bağlantı yerine ortamda Gateway Management API adresi gerektirir)
