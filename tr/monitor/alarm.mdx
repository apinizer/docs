...
<CardGroup cols={2}>
  ...
  <Accordion title="Elasticsearch Tetiklenme Tipleri">
    ...
    **Elasticsearch CPU Yüzdesi**: Elasticsearch cluster'ındaki her node'un CPU kullanım yüzdesini ayrı ayrı kontrol eder. Herhangi bir node'un CPU kullanım yüzdesi eşik değerini aştığında alarm tetiklenir; alarm geçmişinde her node ölçülen değeriyle birlikte listelenir, böylece eşiği aşan node görülebilir. Eşik değeri gerektirir (örn: 80 = %80). Bağlantı tanımı gerektirir. Bağlantı tanımlı değilse, kapalıysa ya da okunamıyorsa alarm hata durumuyla kaydedilir ve geçmişte bunun nedeni açıkça yazılır.

    **Elasticsearch Disk Yüzdesi**: Elasticsearch cluster'ındaki her node'un disk kullanım yüzdesini ayrı ayrı kontrol eder. Herhangi bir node'un disk kullanım yüzdesi eşik değerini aştığında alarm tetiklenir; alarm geçmişinde her node ölçülen değeriyle birlikte listelenir. Disk istatistiği bildirmeyen node'lar (yalnız koordinasyon yapan node'lar) değerlendirmeye alınmaz. Eşik değeri gerektirir (örn: 90 = %90). Bağlantı tanımı gerektirir. Bağlantı tanımlı değilse, kapalıysa ya da okunamıyorsa alarm hata durumuyla kaydedilir ve geçmişte bunun nedeni açıkça yazılır.
