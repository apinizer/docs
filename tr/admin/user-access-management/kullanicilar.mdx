...
| **Ayırt Edici Ad** _(Distinguished Name)_ | Hesabın bağlı olduğu LDAP kaydıdır. Kullanıcı kaydedilirken (aktif LDAP bağlantılarında kullanıcı adıyla aranarak) ya da hâlâ boşsa hesabın ilk başarılı LDAP girişinde otomatik doldurulur. Kayıtla birlikte hesap, onu doğrulayan LDAP bağlantısını da hatırlar.<br/><br/>Her girişte yalnız o bağlantıya sorulur ve parolayı kabul eden kayıt bu kayıt olmak zorundadır: başka bir LDAP bağlantısındaki aynı kullanıcı adına sahip hesaba hiç başvurulmaz; aynı dizinin başka bir bölümündeki bir kayıt da bu hesabın oturumunu alamaz — deneme **Giriş Kayıtları** ekranında `LDAP_IDENTITY_MISMATCH` sebebiyle listelenir. Bu bağ kurulmadan önce kaydedilmiş hesaplar, ilk başarılı girişlerinin kaydını ve bağlantısını benimser (uygulama günlüğüne not düşülür); hatırlanan bağlantısı silinip yeniden oluşturulmuş hesaplar da öyle.<br/><br/>Kayıt dizin içinde taşınırsa (örneğin başka bir organizasyon birimine) bu değer güncellenene kadar kullanıcı reddedilir: hesabı açın, alanı boşaltıp kaydedin — değer dizinden yeniden aranır; dizin o anda değeri veremezse alan boş bırakılır ve bir sonraki başarılı giriş yeni kaydı benimser. |
