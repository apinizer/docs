...
| **Ayırt Edici Ad** _(Distinguished Name)_ | Hesabın bağlı olduğu LDAP kaydıdır. Kullanıcı kaydedilirken (aktif LDAP bağlantılarında kullanıcı adıyla aranarak) ya da hâlâ boşsa hesabın ilk başarılı LDAP girişinde otomatik doldurulur.<br/><br/>Her girişte parolayı kabul eden kayıt bu kayıt olmak zorundadır: başka bir LDAP bağlantısında ya da dizinin başka bir bölümünde aynı kullanıcı adına sahip bir hesap bu hesabın oturumunu alamaz; deneme **Giriş Kayıtları** ekranında `LDAP_IDENTITY_MISMATCH` sebebiyle listelenir.<br/><br/>Kayıt dizin içinde taşınırsa (örneğin başka bir organizasyon birimine) bu değer güncellenene kadar kullanıcı reddedilir: hesabı açın, alanı boşaltıp kaydedin; değer dizinden yeniden aranır. |
