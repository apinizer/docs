---
title: "Log Konnektörü Eklenmesi"
description: "Gateway Runtime ortamlarına log konnektörleri ekleyebilir, API trafik loglarının nereye yazılacağını yapılandırabilirsiniz. Gizlilik ayarlarını yönetebilir ve failover konnektörleri tanımlayabilirsiniz."
---
...
<CardGroup cols={4}>
  <Card title="Veritabanı" href="/tr/integrations/konnektorler/database-konnektor" />
  <Card title="Elasticsearch" href="/tr/integrations/konnektorler/elasticsearch-konnektor" />
  <Card title="Kafka" href="/tr/integrations/konnektorler/kafka-konnektor" />
  <Card title="RabbitMQ" href="/tr/integrations/konnektorler/rabbitmq-konnektor" />
  <Card title="ActiveMQ" href="/tr/integrations/konnektorler/activemq-konnektor" />
  <Card title="Syslog" href="/tr/integrations/konnektorler/syslog-konnektor" />
  <Card title="Webhook" href="/tr/integrations/konnektorler/webhook-konnektor" />
  <Card title="Logback" href="/tr/integrations/konnektorler/logback-konnektor" />
</CardGroup>
...
### Veritabanı Alıcısında Operasyon Alanı {#veritabani-alicisinda-operasyon-alani}
...
Aynı diyalog [SIEM ve Log Yönlendirme](/tr/admin/system-settings/siem-log-yonlendirme) ekranında ve bir portalın [Login Log ayarlarında](/tr/api-portal/api-portal-ayarlari#login-log) da paylaşılır; bu davranış oralarda da geçerlidir.
...
### Log Alanlarında Hangi Ayar Geçerli {#log-alanlarinda-hangi-ayar-gecerli}

Detay filtresi açıkken, konnektör düzeyinde **Log alanlarında hangi ayar geçerli** _(Which settings apply to log fields)_ seçicisiyle bu filtrenin ve gövde boyut sınırlarının kime ait olduğuna karar verilir:

| Seçenek | Anlamı |
|---|---|
| **Bu bağlayıcı** _(This connector)_ — varsayılan | Bugünkü davranış: yukarıdaki detay filtresi ve boyut sınırları tüm API Proxy'ler için geçerlidir. **API Proxy geçişine izin ver** anahtarı yalnızca bu modda görünür ve anlamlıdır (bkz. [aşağıda](#bir-api-proxynin-bu-kisitlari-gecmesine-izin-verme)). |
| **API Proxy** | Karar her API Proxy'ye geçer: her proxy kendi [trafik log ayarlarına](/tr/develop/api-proxy-konfigurasyonu/api-trafik-log-ayarlari) göre loglanır, bu konnektörün detay filtresi ve gövde boyut sınırı uygulanmaz. Bu modda **API Proxy geçişine izin ver** anahtarı gizlenir — zaten karar proxy'dedir. |

Hangi mod seçilirse seçilsin, gizlilik (PII) maskeleme/hash'leme/silme/şifreleme kuralları ile [veri saklama profili](/tr/develop/api-proxy-konfigurasyonu/api-trafik-log-ayarlari#veri-saklama-profili) koşulsuz uygulanmaya devam eder; bu seçici bunları hiçbir koşulda atlamaz. Ayar kaydedildiğinde anında geçerli olur, **API Proxy'nin yeniden dağıtımı (redeploy) gerekmez**.

Ayar varsayılan olarak **Bu bağlayıcı**'dır; mevcut konnektörler yükseltme sonrası bu değerle açılır ve davranışları değişmez.

### Bir API Proxy'nin Bu Kısıtları Geçmesine İzin Verme {#bir-api-proxynin-bu-kisitlari-gecmesine-izin-verme}

Burada yapılandırdığınız detay filtresi ortam geneline ait bir kuraldır: trafiği bu konnektöre ulaşan her API Proxy için geçerlidir. Kimi zaman tek bir proxy kuralın izin verdiğinden fazlasına ihtiyaç duyar — örneğin incelemeye alınmış bir ödeme entegrasyonu, ortam genelinde gövde loglaması kapalıyken.

Bu seçenek yalnızca konnektör **Bu bağlayıcı** modundayken görünür ve etkilidir; konnektör **API Proxy** moduna alınmışsa her proxy zaten kendi log ayarlarını kullandığından bu anahtara gerek kalmaz.

**API Proxy geçişine izin ver** seçeneği bu kapıyı, yalnızca bu kapıyı açar. Seçeneği açtığınızda, kendi [trafik log ayarlarında](/tr/develop/api-proxy-konfigurasyonu/api-trafik-log-ayarlari#ortam-kisitlarini-gecme) *Ortam kısıtlarını geç* seçeneğini de açmış olan bir proxy, bu konnektördeki detay filtresinden ve boyut limitlerinden muaf tutulur. Diğer bütün proxy'ler kısıtları aynen uygulamaya devam eder.

Kapının iki taraflı olması bilinçlidir. Hangi konnektörlerde geçişe izin verileceğine ortam yöneticisi, bu proxy'nin bir istisna olup olmadığına proxy sahibi karar verir. Hiçbir taraf tek başına yeterli değildir; böylece bir proxy sahibi kurumsal bir kısıtı tek taraflı kaldıramaz, yönetici de her istisna için ortam ayarını tek tek düzenlemek zorunda kalmaz.

:::warning Geçişin asla dokunmadığı yerler
Gizlilik ayarları bu kapının tamamen dışındadır. Maskeleme, hash'leme, silme ve şifreleme kuralları geçişten bağımsız olarak her kayıtta çalışmaya devam eder; çünkü bunlar bir görüntüleme tercihi değil, veri koruma yükümlülüğüdür. [Veri saklama profili](/tr/admin/system-settings/sistem-ayarlari-ve-limitler#veri-saklama-profili) de etkilenmez: **No Payload** veya **No Persist** yürürlükteyken gövde hiç toplanmaz ve geçişin yeniden açacağı bir şey kalmaz.
:::

Seçenek yalnızca **API Trafik Log Ayarı Detayını Yapılandır** açıkken görünür; detay filtresi yokken geçilecek bir kısıt da yoktur. Varsayılanı kapalıdır, dolayısıyla mevcut ortamlar önceki davranışını aynen sürdürür.
...
| Alan | Açıklama |
|---|---|
| **Engellenme durumunda devre dışı alanları logla** _(Log disabled fields when request is blocked by policy)_ | Bir politika isteği engellediğinde (örneğin yetkilendirme başarısız olduğunda), bu konnektöre gönderilen logda kapalı tutulan alanlar yine de gönderilir. Engelleme nedeninin ve istek sahibinin tespiti için kullanışlıdır. |
| **Hata durumunda devre dışı alanları logla** _(Log disabled fields on policy failure or error)_ | İstek veya yanıt hattında bir hata oluştuğunda, bu konnektöre gönderilen logda kapalı tutulan alanlar yine de gönderilir. Hatanın hangi mesaj içeriğinden kaynaklandığının analizi için kullanışlıdır. |
...
API Proxy bazlı log ayarları için [API Proxy Konfigürasyonu - Ayarlar Sekmesi](/tr/develop/api-proxy-konfigurasyonu/settings) sayfasındaki API Trafik Log Ayarları bölümüne bakabilirsiniz.
