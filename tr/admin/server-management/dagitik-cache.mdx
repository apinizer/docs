...
<Tip>
**GC ve Bellek Profili:** Apinizer, Cache Server'ın container belleğine göre uygun GC algoritmasını otomatik olarak seçer. 1536 MB'ın üzerindeki container'larda ZGC seçilir — bu, büyük heap boyutlarında G1GC ile yaşanabilen Hazelcast cluster partition timeout riskini azaltan sub-millisecond GC duraklamaları sağlar. `JAVA_OPTS` ile otomatik GC seçimini geçersiz kılabilirsiniz. Detaylar için [JVM Garbage Collector Ayarlama](/tr/operations/yonetici-kilavuzlari/jvm-garbage-collector-ayarlama) sayfasına bakın.
</Tip>
