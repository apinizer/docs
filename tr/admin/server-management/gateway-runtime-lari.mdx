...
Gateway Altyapısı modülü, Apinizer platformunun Gateway Runtime altyapı yönetimini sağlar. Bu modül ile:

- Gateway Runtime ortamları oluşturulur ve yönetilir
- Kubernetes kaynakları ve deployment'lar yönetilir
- Gateway Engine yapılandırmaları yapılır
...
<CardGroup cols={3}>
  ...
  <Card title="Gateway Runtime Ortamlarına Konnektör Eklenmesi" icon="plug" href="/tr/admin/server-management/gateway-ortamlarina-konnektor-eklenmesi">
    Gateway Runtime ortamlarındaki API trafiğini diğer ortamlara göndermek için konnektör yapılandırması.
  </Card>
  <Card title="SSL/TLS ile Apinizer Modüllerini Başlatma" icon="lock" href="/tr/operations/yonetici-kilavuzlari/ssl-tls-apinizer-modulleri-baslatma">
...
<Info>
Gateway Runtime'ları ve Cache Server'ları artık ayrı ayrı yönetilmektedir. Cache Server oluşturma ve yönetimi için [Dağıtık Cache](/tr/admin/server-management/dagitik-cache) sayfasına bakabilirsiniz.
</Info>
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Log Server" icon="file-lines">
    Oluşturulan Gateway Runtime ortamındaki tüm API Trafiğinin ve isteklerin loglanacağı Elasticsearch kümesi bağlantı tanımıdır.
  </Accordion>
  ...
  <Accordion title="Deployment (Dağıtım)" icon="server">
    ...
    <Info>
    Cache Server'ları artık ayrı ayrı oluşturulup yönetilmektedir. Detaylı bilgi için [Dağıtık Cache](/tr/admin/server-management/dagitik-cache) sayfasına bakabilirsiniz.
    </Info>
  </Accordion>
...
| **Ad**  | Gateway Runtime ortamının adı. Kubernetes'teki namespace'e karşılık gelmektedir. |
...
<Info>
    Eğer tüm Gateway Runtime ortamlarının metriklerine erişmek isterseniz [Kubernetes İş Yükleri sayfasına](/tr/admin/server-management/kubernetes-is-yukleri) tıklayınız.
</Info>
...
| **Connections** | Connection pool bilgilerini görüntüleme butonu |

### Detaylı Metrik Görüntüleme

Diagnostics tablosundaki **JVM**, **Threads** ve **Connections** butonlarına tıklayarak detaylı JSON formatında metrik bilgilerine erişebilirsiniz:
...
- **Secure Port:** HTTPS servis portu
