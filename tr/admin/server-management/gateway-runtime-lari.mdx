...
<AccordionGroup>
    ...
    <Accordion title="Ek Değişkenler (Additional Variables)" icon="code">
        ...
        **Önemli Değişkenler:**
        ...
        | `tuneStreamingReadTimeout` | HTTP Worker, HTTP+Websocket | Streaming (SSE/LLM) bağlantıları için istemci tarafı okuma timeout'u. SSE gibi uzun ömürlü bağlantılarda istemci veri göndermediği için normal `tuneReadTimeout` bağlantıyı erken kapatabilir. Bu parametre ile streaming bağlantılarına özel timeout değeri atanır (varsayılan: 0 ms / sınırsız) |
        ...
        <Warning>
            **Thread Pool Boyutlandırma:**
            ...
            Detaylı boyutlandırma rehberi için [Gateway Performans Ayarlama](/tr/operations/yonetici-kilavuzlari/gateway-performans-ayarlama) sayfasına bakın.
        </Warning>

        <Tip>
            CPU'ya göre önerilen thread değerleri, tier bazlı yapılandırma önerileri ve tüm tuning parametrelerinin detaylı açıklamaları için [Gateway Performans Ayarlama](/tr/operations/yonetici-kilavuzlari/gateway-performans-ayarlama) sayfasına bakın.
        </Tip>
        ...
        <Warning>
            **Java Options Uyarısı:**
            ...
            Apinizer, **otomatik bellek profili sistemi** kullanarak container belleğine göre uygun GC algoritması ve heap yüzdesini seçer:

            | Container Bellek | GC | Heap Yüzdesi |
            |-----|-----|-----|
            | ≤ 768 MB | Serial GC | %50 |
            | 769 – 1536 MB | G1GC | %60 |
            | > 1536 MB | ZGC | %70 |

            `JAVA_OPTS`'da GC veya heap ayarı belirtirseniz, otomatik değerler geçersiz kılınır. **Önemli:** ZGC, diğer GC'lere göre daha fazla native bellek kullanır. ZGC otomatik seçildiğinde `MaxRAMPercentage` %70'in üzerindeyse, OOM Kill'i önlemek için otomatik olarak %70'e düşürülür.

            UseContainerSupport varsayılan olarak aktif gelmektedir. Eski bayraklar -XX:MinRAMFraction ve -XX:MaxRAMFraction kullanımdan kaldırılmıştır; yerine -XX:MaxRAMPercentage kullanın.

            Detaylı bilgi için [JVM Garbage Collector Ayarlama](/tr/operations/yonetici-kilavuzlari/jvm-garbage-collector-ayarlama) sayfasına bakın.
        </Warning>
