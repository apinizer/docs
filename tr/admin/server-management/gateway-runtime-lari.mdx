...
<CardGroup cols={3}>
    ...
    <Card title="Log Konnektörü Eklenmesi" icon="plug"
...
Ortamın adı sonradan değiştirilirse, o ortama önceden deploy edilmiş API Proxy ve Proxy Group kayıtları yeni adla hemen güncellenir; bu kayıtlar isim değil ortam kimliğine (id) göre eşleştiği için yeniden adlandırma sonucu kaybolmaz ya da yanlışlıkla kaldırılmaz (undeploy edilmez).
...
Gateway Runtime ortamına log konnektörü ekleme hakkında daha fazla bilgi için [bu sayfaya](/tr/admin/server-management/gateway-ortamlarina-konnektor-eklenmesi) bakın.
...
:::warning[Yükseltme yapıyorsanız — DNS resolver ayarının etkinleşmesi için yeniden yayınlayın]
Apinizer'ın oluşturduğu her pod (Gateway, Cache, Integration, Portal, Manager) `ndots=1` DNS resolver seçeneğini ayarlar. Kubernetes'in varsayılan değeri `ndots=5` olduğundan, 5'ten az nokta içeren bir ad — pratikte küme içindeki her kısa ad — önce resolver'ın arama listesine karşı denenir, ardından olduğu gibi denenir. Alpine tabanlı imajlarda resolver ilk boş yanıttan sonra aramayı durdurur; bu nedenle `service.namespace` gibi noktalı ama eksik bir ad çözümlenemezken tam ad çalışmaya devam edebilir. `ndots=1` ile noktalı adlar doğrudan aranır; bu yüzden küme içi servislere her zaman kısa ad yerine **tam ad** (`service.namespace.svc.cluster.local`) ile erişin.

Bu ayar, bir Gateway, Cache, Integration veya Portal deployment'ı oluşturulduğunda veya yeniden uygulandığında devreye girer — yukarıdaki **Yayınla** (Publish) ve **Yeniden Yayınla** (Republish) işlemleri de buna dahildir. Ayar eksikse, mevcut deployment'ın kaynak, güvenlik bağlamı, sağlık probu veya ortam değişkeni düzenlemesi de ayarı ekler; operatörün elle verdiği `ndots` değeri korunur. Salt bir pod **Restart**'ı veya **Yayınla/Yayından Kaldır** (replica ölçekleme) işlemi ayarı EKLEMEZ. Yükseltme sonrasında mevcut her ortamı **Yeniden Yayınla** (ve Cache/Integration/Portal deployment'larını yeniden uygulayın). Yalnızca Restart almış bir ortam kaldıysa, yukarıdaki düzenlemelerden birini uygulayın ya da Yeniden Yayınlayın.
:::
...
:::warning[2026.09.3 öncesi kurulumlar — JWT imzalama anahtarını yenileyin]
Gateway Runtime ortamının token imzalama anahtarı (**JWT Token Doğrulama Anahtarı** sekmesindeki RSA özel anahtar) 2026.09.3 öncesi sürümlerde ortam her kaydedildiğinde denetim kaydına, denetim kayıtlarını alan her SIEM hedefine ve — istek gövdesi denetimi açıksa — yakalanan istek gövdesine **açık metin** olarak yazılıyordu. 2026.09.3 ile bu alan hiçbir denetim çıktısında yer almaz; ancak düzeltme geçmişi temizlemez: eski denetim satırları ve syslog dosyaları anahtarı taşımaya devam eder. Denetim kaydı ya da SIEM yönlendirmesi kullanılmış her ortamda Gateway Runtimes ekranından **Regenerate & Deploy** ile anahtarı yenileyin. Yenileme, o ortamın Apinizer Token Servisi tarafından imzalanmış mevcut token'ları geçersiz kılar; istemciler yeni token almalıdır. Eski denetim satırlarının temizliği için bkz. [Audit Event Collection Temizleme](/tr/operations/database-buyume-yonetimi/audit-event-collection-temizleme).
:::
