...
<Note>
    ...
    <Info>
        Gateway Runtime'ları ve Cache Server'ları artık ayrı ayrı yönetilmektedir. Cache Server oluşturma ve yönetimi
        için [Dağıtık Cache](/tr/admin/server-management/dagitik-cache) sayfasına bakabilirsiniz.
    </Info>
</Note>
...
<AccordionGroup>
    ...
    <Accordion title="Log Server" icon="file-lines">
        Oluşturulan Gateway Runtime ortamındaki tüm API Trafiğinin ve isteklerin loglanacağı Elasticsearch kümesi
        bağlantı tanımıdır.
    </Accordion>
    ...
    <Accordion title="Deployment (Dağıtım)" icon="server">
        ...
        <Info>
            Cache Server'ları artık ayrı ayrı oluşturulup yönetilmektedir. Detaylı bilgi için [Dağıtık
            Cache](/tr/admin/server-management/dagitik-cache) sayfasına bakabilirsiniz.
        </Info>
    </Accordion>
