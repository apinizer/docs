    ...
    <Steps>
      ...
      <Step title="Seçenekler">
        ...
        **Replace Existing** nesnenin kimliğini korur: hedefteki kaydın kimliği, API Proxy ve Proxy Group'un istemci doğrulamasında kullanılan anahtar/secret çifti ve Policy Group'un çalışma zamanı anahtarı olduğu gibi kalır; paketten gelen kaynak kurulumun anahtarları hiçbir zaman hedefe yazılmaz. Deploy durumu da korunur: hedefte deploy edilmiş her ortam **"redeploy gerekli"** olarak işaretlenir, çünkü içerik değişmiştir ve Gateway hâlâ eski tanımı sunar — değişikliğin devreye girmesi için ilgili ortama yeniden deploy edin. **Import as New** ile oluşan nesne ise yeni kimlik ve anahtarlarla, hiçbir ortama deploy edilmemiş olarak başlar; import hiçbir durumda kendiliğinden deploy yapmaz.
        ...
        Kimlik bilgisi (credential) içe aktarılırken kaynak kurulumdaki organizasyon, sertifika/keystore/truststore, kripto anahtarı ve JWK referansları ile kimlik doğrulama servisi (LDAP, veritabanı, API, OIDC sağlayıcı, JWT yenileme API'si), rol, Portal uygulaması/hesabı ve ortama özel ayrıntı (ortam bazlı parola ve IP listesi) referansları hedefte bulunamazsa alan boş bırakılır (liste alanlarında yalnızca bulunamayan öğeler çıkarılır) ve bu durum içe aktarma özetinde bir uyarı olarak listelenir; **Replace Existing** seçildiyse hedefteki mevcut geçerli referans korunur ve bu koruma da aynı özette belirtilir. Portal uygulaması referansında ölçüt organizasyondur: uygulama yalnızca kimlik bilgisi hedef projenin kullanabildiği bir organizasyon taşıyorsa (kimlik bilgisi ekranındaki organizasyon seçicisinin sunduğu gibi o projeye ya da admin projesine kayıtlı bir organizasyon; buna ek olarak, seçicide listelenmeyen ama platformun admin kapsamında saydığı, proje bilgisi hiç olmayan bir organizasyon) ve uygulama da aynı organizasyona aitse korunur; başka bir organizasyona ait bir uygulama, kaydı gerçek olsa da bulunamamış sayılır ve organizasyonu çözülemeyen bir kimlik bilgisi hiçbir uygulama referansı taşımaz (doğrulanacak bir şey yoktur). Uygulamanın projesi bilerek denetlenmez (Portal, bir uygulamaya proje bilgisini yalnızca tek proje sunduğu sırada yazar ve sonradan güncellemez); bu yüzden admin projesine, başka bir projeye ait ya da projesi hiç olmayan uygulamalar, organizasyon eşleştiği sürece kabul edilir. Yönetici tarafından oluşturulan dahili (internal) uygulamaların organizasyonu yoktur; bunlar projesi boşsa, admin projesiyse ya da içe aktarılan projenin kendisiyse korunur. Portal hesabı referansı bir sahiplik değil atıf bilgisidir: korunan bir uygulama varsa hesabın yalnızca aynı portalda var olması yeterlidir; uygulama yoksa hesabın var olması ve kimlik bilgisinin organizasyonuna birincil organizasyon ya da etkin üyelik ile bağlı olması gerekir. Bu denetimlerden geçemeyen referanslar da yukarıdaki gibi boş bırakılıp içe aktarma özetinde uyarı olarak listelenir. **Replace Existing** seçildiğinde hedefteki mevcut uygulama ve hesap da aynı denetimlerden geçer ve yalnızca geçerlerse korunur; organizasyon taşımayan bir paket hedefin organizasyonunu korur. Aynı şekilde uygulama ya da hesap referansı taşımayan bir paket, bu denetimlerden geçiyorlarsa hedefin kendi uygulama ve hesabını korur; bu karar da özette belirtilir. Kaynak kurulumun kimlik numarası hiçbir zaman olduğu gibi kaydedilmez.
