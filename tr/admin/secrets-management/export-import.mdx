...
:::info
MCP sunucusu veya A2A ajanı artık ayrı bir dışa/içe aktarılabilir nesne tipi değildir. Onu gömen politikanın (MCP Araç Çağrısı / MCP Araç Çağırma ya da A2A Ajan Çağrısı / A2A Ajan Çağırma) içinde ve Passthrough modundaki bir MCP/A2A proxy'sinin kendi routing yapılandırması içinde, kendisini içeren **API Proxy** paketinin bir parçası olarak satır içi taşınır. Eski bir sürümden gelen ve hâlâ ayrı bir MCP/A2A bağlantı nesnesi taşıyan bir paket, sessizce atlanmak yerine içe aktarımda açıklayıcı bir hatayla reddedilir.
:::
...
<AccordionGroup>
  ...
  <Accordion title="AI Nesnesi Bağımlılığı Tespiti ve Eşleme">
    ...
    - MCP/A2A gateway proxy'leri **API Proxy** (`type=MCP` / `type=A2A`) olarak export edilir ve çözülecek ayrı bir bağlantı bağımlılığı taşımaz: bir Passthrough proxy'nin arka uç sunucu(ları) ile bir Araç/Ajan Çağrısı politikasının gömülü sunucusu, proxy'nin kendisinin bir parçası olarak satır içi paketlenir.
