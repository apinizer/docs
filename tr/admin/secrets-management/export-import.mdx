...
<AccordionGroup>
  ...
  <Accordion title="AI Nesnesi Bağımlılığı Tespiti ve Eşleme">
    ...
    - MCP veya A2A Outbound Connection, kimlik doğrulamasında kullandığı Credential'ı bağımlılık olarak sunar. MCP/A2A gateway proxy'leri ise **API Proxy** (`type=MCP` / `type=A2A`) olarak export edilir — PASSTHROUGH bir proxy, backend MCP/A2A Outbound Connection'ını bağımlılık olarak sunar (pakette name-based `backendMcpRefName` / `backendA2aRefName` sidecar'ları).
