...
| API Yönetimi | API Tanımı (Spec Designer) | Yok |
...
| Bağlantılar | Bağlantı (Connection) | Var |
...
**Proje Yükle** koşu satırı ve sonuç ekranı, atlanan tüketiciler gibi artık **düşürülen referanslar** ve **üzerine yazılan gizli değerler** uyarılarını da gösterir; önceki sürümde bu iki bilgi yalnızca sunucu günlüğünde kalıyordu (Apinizer 2026.09.4).

Tüm proje paketiyle taşınan bir Bilgi Tabanı'nın Vektör Veritabanı ve embedding sağlayıcı referansı, aynı paketin içindeki AI kayıtlarıyla artık doğru şekilde yeniden bağlanır; aynı pakette taşınan RAG, Semantic Cache ve Topic Guard politikalarının Vektör Veritabanı, embedding sağlayıcısı ve Bilgi Tabanı referansları da aynı düzeltmeden geçer — bir API Proxy'ye gömülü politika dahil. Önceki sürümde bu referanslar hedefte karşılıksız kalıyordu; paket "başarıyla içe aktarıldı" dese bile RAG politikası Vektör Veritabanı'na bağlanamıyor ya da Bilgi Tabanı indekslenemiyordu (Apinizer 2026.09.4).
...
- **"Export as File" (Dosya Olarak Dışa Aktar) düğmesinden farkı**: bu 14 ekranda satır menüsünün ikincil **Export as File** kalemi, bu tip var olmadan önceki ham tek-dosya ZIP yeteneğidir, korunarak buraya indirildi. **LDAP, E-posta, Kafka, Syslog ve Webhook** bağlantı tiplerinde bu buton artık sertifika/key store referansını da adıyla birlikte pakete ekler — böylece bu beş tipte hem detay sayfasının **Dışa Aktar** hem liste sayfasının **Dosya Olarak Dışa Aktar** düğmesi, sertifika kullanan bir bağlantıyı hedefe eksiksiz taşır. Diğer 9 bağlantı tipinde (Veritabanı Havuzu, Elasticsearch, FTP, Graylog, RabbitMQ, ActiveMQ, Logback, SNMP, Linux Script) davranış değişmedi — bu buton yalnızca bağlantının kendi alanlarını dışa aktarır.

:::note
Bu beş tipte önceki sürümde üretilmiş, sertifika içermeyen paketler yine kabul edilir; sertifikası gerçekten çözümlenemeyen ve "sertifika doğrula" ayarı açık bir bağlantı için içe aktarma yine açık bir hatayla reddedilir (sessiz yarım aktarım yoktur). Sertifika ailesi artık bağlantıdan **önce** içe aktarılır; önceki sürümde ikisi aynı, sırasız adımda işlendiği için sıralama dosya sistemine bağlıydı.
:::

  Sertifikayı bağlantıyla **birlikte** taşımanız gerekiyorsa (örn. mTLS sertifikalı bir LDAP havuzu) satır menüsünün birincil **Export** eylemi — yani bu sihirbaz tipi — bunu zaten varsayılan olarak yapar; yukarıdaki beş tipte iki yol artık sertifika/key store açısından aynı sonucu verir. Fark hâlâ vardır: bağlantının host/URL/parola alanındaki `${DEĞİŞKEN}` referansları yalnızca sihirbaz yolunda ortam/global değişken bağımlılığı olarak taşınır; "Export as File" bunları taşımaz.
...
- **Bağımlılığı olmayan tipler** (Ortam Değişkeni, Global Değişken, Sertifika ailesi, IP Grubu, Consumer Rolü, Kayıtlı Organizasyon, Limit Planı, PII Preset, Prompt Guard Preset, DLP Preset, Prompt Template, Prompt Decorator, Topic Category, API Tanımı) için ek bir bağımlılık/eşleştirme adımı yoktur — nesne seçildiği anda doğrudan dışa aktarılır; içe aktarımda ise dosya yüklendiği anda doğrudan seçeneklere (Options) geçilir.

:::info
**API Tanımı** (Spec Designer), bir API Proxy'ye bağlı olmayan **bağımsız** OpenAPI tanımlarını sihirbaza taşır — bir proxy'ye bağlı tanım bu listede görünmez, proxy'nin kendi paketiyle birlikte taşınmaya devam eder. Spec Designer liste ekranındaki satır menüsünün **Dışa Aktar** kalemi ve başlık çubuğundaki **Veriyi Aktar** düğmesi sihirbazı ilgili tanım/tip ön-seçili olarak açar; erişim **API Yönetimi → Export/Import** yetkisine bağlıdır.
:::
  ...
  <TabItem value="ice-aktarma" label="İçe Aktarma">
    <Steps>
      ...
      <Step title="Paketi Yükle">
        ...
        Bu yuvaya bir **Postman veya Testnizer** koleksiyon dosyası (.json) yüklerseniz artık "tip uyuşmuyor" mesajı yerine, dosyanın bir Postman/Testnizer koleksiyonu olduğunu söyleyen ve **Test Koleksiyonları sayfasına git** düğmesi taşıyan bir yönlendirme görürsünüz — bu dosya tipi bu sihirbaz tarafından okunmaz, [Test Konsolu](/tr/develop/test-debug-araclari/test-console) sayfasındaki koleksiyon içe aktarma akışından yüklenmelidir.
      </Step>
      ...
      <Step title="Seçenekler">
        ...
        **Import as New**, kullanımda olan bir ismi reddeder ve hatada üç durumdan hangisi olduğu belirtilir: isim hedef projede zaten vardır (**Replace Existing** seçilir ya da burada yeniden adlandırılır), paketin kendisi aynı türden nesneler için aynı ismi iki kez taşımaktadır (kopyalardan biri yeniden adlandırılır) ya da isim bu kurulumda başka bir yerde kullanılmaktadır. Son durum, adları **tüm** projeler genelinde benzersiz olmak zorunda olan nesne türleri için geçerlidir — kimlik bilgisi kullanıcı adları, ortam değişkenleri, AI hazır ayar katalogları ve model tanımları — bu yüzden hedef projede nesne görünmese bile içe aktarma reddedilebilir; isim başka bir projeye, paylaşılan (admin) kapsama ya da yerleşik (built-in) bir kayda ait olabilir. Diğer türlerde bu durumda **Replace Existing** seçeneğinin bu projede güncelleyeceği bir kayıt yoktur; nesneyi içe aktarmadan önce yeniden adlandırın ya da ismin sahibi olan projeye aktarın. **Kimlik Bilgisi (Consumer)** türü istisnadır: sihirbaz paylaşılan (admin) kapsamdan başlatılırsa, isim başka bir projeye ait olsa bile o kayıt Replace Existing hedefi olarak seçilebilir — aşağıdaki paragrafa bakın.

        **Ortam Değişkeni**'nde **Replace Existing** seçildiğinde, ad hedef projede bulunamazsa içe aktarma artık sessizce yeni bir satır oluşturmaz: ad kurulumun başka bir yerinde (başka bir proje ya da paylaşılan kapsam) zaten kullanılıyorsa içe aktarma, ortam değişkenlerinin kurulum genelinde benzersiz olması gerektiğini söyleyen yukarıdaki aynı mesajla reddedilir. Bu denetim sihirbaz ve APIops üzerinden yapılan içe aktarmalarda geçerlidir; sihirbaz dışındaki eski (legacy) İçe Aktarma penceresinden ya da tüm proje paketiyle yapılan bir içe aktarmada ad çakışması hâlâ adın sonuna `-imported` eklenerek çözülür, reddedilmez (Apinizer 2026.09.4).

        **Bağlantı (Connection)** tipinde bu çakışma artık **Seçenekler** adımında anında gösterilir: paylaşılan (admin) kapsamda aynı ada sahip bir bağlantı varsa ad alanının altında kırmızı bir uyarı çıkar ve **İleri** düğmesi pasif kalır — önceki sürümde bu hata ancak **İçe Aktar**'a basıldığında, "kurulum genelinde benzersiz olmalı" mesajıyla ortaya çıkıyordu. **Replace Existing** seçildiğinde hedef listesinde paylaşılan (admin) kapsamdaki satır görünür ama gri/seçilemezdir ve "paylaşılan (admin) kapsam, burada değiştirilemez" notunu taşır; hedef olarak **Admin (global)** seçildiğinde bu satırlar seçilebilir olur.

        **Kimlik Bilgisi (Credential/Consumer)** tipinde hedef listesi artık **sunucu taraflı arama** ile çalışır: kayıt sayısı 50'yi aştığında listenin üstünde "ilk 50 kayıt gösteriliyor" bilgisi görünür ve arama kutusuna yazdıkça sunucudan eşleşen kayıtlar getirilir. Paketteki kullanıcı adı hedefte zaten varsa o kayıt otomatik olarak en üstte ve önerilmiş biçimde seçili gelir. Sistem yöneticisi hedefi **Admin (global)** seçtiğinde arama tüm projeleri kapsar ve isteğe bağlı bir **Proje** filtresi görünür. Başka bir projeye ait bir tüketici de artık **seçilebilir**; listenin altında bunu açıklayan bir bilgi satırı görünür. Böyle bir satır seçildiğinde hedef seçiminin altında mavi bir bilgi kutusu çıkar: kayıt hangi projeye aitse **orada** güncellenecek ve orada kalacaktır. İçe aktarma tüketiciyi projeler arasında taşımaz; organizasyonu da değişmez. Yetki, içe aktarma sırasında hedefin **kendi projesinde** denetlenir — sistem yöneticisi bu denetimden muaftır. Proje kapsamında başlatılan bir içe aktarmada arama yalnız o projenin tüketicilerini kapsar; başka bir projeye ait bir satır hiç listelenmez. Sihirbaz dışındaki klasik (legacy) İçe Aktarma penceresi ve APIops davranışı değişmedi: kullanıcı adı orada yalnızca hedef projede aranır. Paketteki tüketici bir API İstemcisine taşınmışsa hedef seçiminin altında bilgi kutusu çıkar: kullanıcı adı, parola, anahtar ve organizasyon korunur, yalnızca diğer alanlar güncellenir. Paket ile hedefin organizasyonu farklıysa ayrıca "hedefin organizasyonu korunacak" notu görünür.
        ...
        **Global Politika ve Politika Grubu** için **Replace Existing** ayrıca daha geniş bir kapsamı günceller. Değişen içerik, o politikayı kullanan her API Proxy, Proxy Group ve Politika Grubu'ndaki gömülü kopyaya da yazılır. Bu kayıtlardan dağıtılmış (deployed) olanlar da **"yeniden dağıtım gerekli"** olarak işaretlenir. İçe aktarma kendiliğinden yeniden dağıtım yapmaz; değişikliğin Gateway'de devreye girmesi için ilgili ortamı Deploy/Redeploy edin. **Import as New** ile oluşan kopya ayrı bir kayıttır ve mevcut proxy, proxy grubu ya da politika gruplarına dokunmaz. Sihirbaz dışındaki eski (legacy) İçe Aktarma ekranından yapılan bir değiştirme de bu güncellemeyi tetiklemez; adının sonuna `-imported` eklenmiş yeni bir kayıt oluşturur.

Bu ekrandan ya da APIops üzerinden yapılan bir içe aktarmada, politikanın taşıdığı sertifika, anahtar deposu, IP grubu, kimlik servisi ve AI sağlayıcı/bilgi tabanı gibi referanslar hedefte yeniden yazılamadığı için her zaman boşaltılıyordu; bu karar değişmedi — paketteki kaynak kurulumun kimliğini hedefe yazmak, ya kopuk ya da yanlış bir nesneye bağlanmak anlamına gelirdi. Değişen, görünürlüktür: boşalan alanlar artık içe aktarma sonucunda **düşürülen referanslar** uyarısı olarak listelenir; önceden bu bilgi hiçbir yerde görünmüyordu (Apinizer 2026.09.4).
      ...
      <Step title="Özet ve İçe Aktarma">
        ...
        İçe aktarma, paketin analiz edildiği andaki hedef durumuna karşı uygulanır. Analizden sonra hedef projede
        eşleştirme veya değiştirme seçimlerini etkileyen bir değişiklik olduysa (örneğin başka bir sekmede aynı adda bir
        nesne oluşturulduysa) içe aktarma hiçbir kayıt yazmadan **"Bu paket analiz edildikten sonra hedef değişti"**
        uyarısıyla durur; paketi yeniden yükleyip analizi tekrar çalıştırmanız yeterlidir. Eşleştirme adımındaki
        **Oluştur** düğmesiyle yaptığınız oluşturmalar bu uyarıyı tetiklemez: sihirbaz her oluşturmadan sonra analizi
        arka planda tazeler. Bir bağımlılık satırında Oluştur'a bastığınızda, oluşturulan nesnenin yanında bu
        projede kendiliğinden meydana gelen başka nesneler de olabilir (örneğin bir MCP ağ geçidinin kaynak API
        Proxy'si oluşturulduğunda o proxy'nin kendi politika grubu ve global politikaları). Sihirbaz yalnızca bu
        oluşturma işleminin ürettiği nesneleri, henüz eşlenmemiş satırlara otomatik olarak bağlar; böyle bağlanan
        satırda Eşleme adımında **"Otomatik eşlendi"** rozeti ve üzerine gelindiğinde bunu açıklayan bir ipucu
        görünür. Hedef projede zaten var olan aynı adlı bir nesneye ASLA otomatik bağlanmaz: bilerek boş
        bıraktığınız satır boş kalır ve o referans içe aktarımda düşürülür. Otomatik eşlenen satırlar İnceleme
        tablosunda **"Otomatik eşlendi"** etiketiyle, İçe Aktar onay penceresinde ve içe aktarım sonuç ekranında ayrı
        bir listede gösterilir. İstemiyorsanız listeden farklı bir hedef seçmeniz yeterlidir; seçim yaptığınız anda
        rozet kalkar.
...
APIops üzerinden içe aktarımda aynı onay bir istek parametresidir — yukarıdaki üç durumun hepsini kabul eder. Parametre ve yanıtın geri taşıdığı uyarı için bkz. [Import Objects](/api-reference/export-import/import-objects).
...
Hiçbir ortama atanmamış sertifika, key store ya da kripto anahtarı artık doğru şekilde dışa aktarılır. Önceki sürümde bu nesneler için paket girdisi sessizce boş üretiliyordu; sorun ancak içe aktaran kurulumda okunamayan bir girdi olarak ortaya çıkıyordu. Bir nesne hâlâ pakete yazılamazsa dışa aktarma artık boş bir girdiyle tamamlanmaz: işlem hemen durur ve nesnenin tipini ve adını söyler.
...
LLM Provider, Vector DB ve OpenTelemetry (OTLP) bağlantısı gibi tipler, gizli değerlerini pakete
**okunabilir** olarak yazar; içe aktarma da onları hedef kurulumda yeniden şifreler. Bu davranıştan
önce üretilmiş çok eski paketler ise aynı alanları **şifrelenmiş** hâlde taşır. Böyle bir paket içe
aktarıldığında değer ikinci kez şifrelenir ve **geri döndürülemez biçimde kullanılamaz hâle gelir** —
üstelik işlem başarılı görünür, sorun ancak sağlayıcı ilk kez çağrıldığında ortaya çıkar.

Bu sürümden itibaren bu paketler sessizce bozulmak yerine **reddedilir**: içe aktarma durur ve
paketin güncel bir Apinizer sürümüyle kaynak kurulumdan yeniden dışa aktarılması gerektiğini söyleyen
bir hata alırsınız. Yapmanız gereken, paketi kaynak kurulumda yeniden üretmektir; elinizdeki eski
dosya üzerinde yapılabilecek bir düzeltme yoktur.

Bağlantı tanımları (e-posta, LDAP, veritabanı, SNMP, Elasticsearch, FTP, RabbitMQ, ActiveMQ,
Linux Script) ile
kimlik servisleri bu kuralın **dışındadır**: onların gizli değerleri pakette şifreli taşınır ve
hedefte olduğu gibi saklanır. Bu tiplerde bir paket gizli değeri düz metin taşıyorsa — örneğin elle
yazılmış bir GitOps dosyası, ya da çok eski bir sürümde düz metin kaydedilmiş bir kayıttan üretilmiş
bir paket — içe aktarma artık o değeri hedefte **şifreleyerek** saklar. Yükseltme sırasında çalışan
bir veri düzeltmesi, daha önce düz metin kaydedilmiş bu değerleri de şifreler; zaten şifreli olanlara
dokunmaz.
...
<AccordionGroup>
  ...
  <Accordion title="AI Nesnesi Bağımlılığı Tespiti ve Eşleme">
    - Bir LLM Provider'ın bağımlılık adımı sağlayıcı tipini pakete koymaz. Sağlayıcı tipi kataloğu Apinizer ile gelen salt-okunur bir listedir; içe aktarımda bağlantı, sağlayıcı koduna göre hedefteki katalog kaydına otomatik bağlanır. Özel sağlayıcı (`providerType: OTHER`) wire protokolünü ve yollarını bağlantının kendisinde taşır. Aynı adım, sağlayıcının kullandığı özel (custom) model katalogu kayıtlarını yüzeye çıkarır. Özel model katalogu kayıtları pakete dahil edilir ve içe aktarımda hedefte otomatik olarak oluşturulur. Apinizer'ın platformla birlikte gelen hazır (built-in) katalog modelleri hiçbir zaman pakete dahil edilmez — içe aktarımda sağlayıcının/modelin kendi tanımlayıcı koduna göre otomatik olarak çözülür, dolayısıyla bunlar için eşleştirme yapmanıza gerek kalmaz. LLM Provider'ın kendisi her zaman kendi secret'ını (API anahtarı/sırrı) taşır, bu yüzden her zaman secret bir bağımlılık olarak pakete girer.
  ...
  <Accordion title="MCP Kaynak API Proxy'leri Bağımlılığı ve Eşleme">
  ...
  <Accordion title="Çözülemeyen Referanslar: Düşürme ve Bildirim (Apinizer 2026.09.4)">
    Aşağıdaki referanslar hedefte çözülemediğinde artık **her yüzeyde** (eski İçe Aktarma penceresi, APIops ve tüm proje paketi; sihirbazın kendi eşleştirme adımı zaten çözer) tutarlı biçimde düşürülür ve içe aktarma raporunda adıyla bildirilir — önceki sürümde bu ya sessizce boş bırakılıyor ya da hedefte hiçbir şeye karşılık gelmeyen bir kimlikle kalıyordu.

    - **Kimlik Servisi'nin (LDAP / Veritabanı) bağlantı havuzu**: yalnızca sihirbaz dışındaki eski İçe Aktarma penceresinden, APIops'tan ya da tüm proje paketiyle içe aktarılan bir kayıtta düşürülür; sihirbazın kendi eşleştirme akışında havuz zaten doğru hedefe bağlanır. **Replace Existing**'te hedefin kendi havuzu korunur.
    - **OIDC ve API Kimlik Servisi'nin sertifikası**: hedefte çözülemezse düşürülür; **Replace Existing**'te hedefin sertifikası korunur.
    - **API Proxy ve Proxy Group'un Ayar Grubu referansı**: hedefte çözülemezse düşürülür; **Replace Existing**'te hedefin kendi Ayar Grubu korunur.
    - **DB'den API (API Creator) yönteminin veritabanı bağlantısı**: hedefte çözülemediğinde bu alan zaten boşaltılıyordu; karar değişmedi, artık içe aktarma raporunda da bildiriliyor.
    - **Bilgi Tabanı'nın Vektör Veritabanı ve embedding sağlayıcı referansı**: hedefte çözülemezse düşürülür; **Replace Existing**'te hedefin kendi referansı korunur. Aynı pakette taşınan bir Bilgi Tabanı için bu düşürme artık gerekmiyor — bkz. yukarıdaki [Menüden Erişim](#menüden-erişim) bölümündeki tüm proje paketi notu.
    - **Özel Sorgu, Özel Sorgu Filtresi, Rapor Yapılandırması ve Anomali Dedektörü'nün taşıdığı API Proxy listesi**: hedefte çözülemeyen proxy'ler listeden tek tek çıkarılır ve bildirilir, çözülebilenler listede kalır. Bu alanda **Replace Existing** hedefin eski listesini geri getirmez. **Özel Sorgu**'da listedeki proxy'lerin tamamı çözülemezse kayıt boş bir listeyle içe aktarılır; boş liste "tüm proxy'ler" anlamına gelmez — sorgu çalıştırıldığında açık bir hata verir ya da hiçbir isteği eşleştirmez. İlgili ekrandan proxy'leri yeniden seçin.
    - **Erişilebilirlik İzleme'nin ve Anomali Dedektörü'nün bildirim alıcı bağlantısı ve yedek (failover) bağlantısı**: hedefte çözülemezse temizlenir ve bildirilir.
    - **Erişilebilirlik İzleme'nin çalıştığı ortam ve seçili ortam listesi**: hedefte çözülemezse düşürülür; **Replace Existing**'te hedefin kendi ortamı korunur.
    - **Task Flow'un adımlarında ve hata eyleminde kullanılan bağlantılar**: hedefte çözülemezse temizlenir ve bildirilir.
  </Accordion>
</AccordionGroup>
...
Bir Anomaly Detector kaydının çalıştığı **Ortam** ise kaynak kurulumun kimliğini taşır ve pakete bir bağımlılık olarak eklenmez. Hedefte bu ortam kimliği karşılık bulmuyorsa referans, sertifika ailesindeki aynı kararla tutarlı biçimde **düşürülür** ve içe aktarma raporunda "düşürüldü" satırı olarak görünür; içe aktarılan kayıtta Ortam alanı boş gelir ve siz seçmeden kayıt kaydedilemez ya da etkinleştirilemez. **Replace Existing** yolunda hedef kaydın kendi geçerli ortamı korunur.

Ortam kimliği hedefte geçerli bir ortama karşılık geliyorsa ama o ortamda Elasticsearch bağlantısı tanımlı ya da etkin değilse ortam yine **korunur**; içe aktarma raporunda ortamın korunduğunu ve Elasticsearch bağlantısının tanımlı ya da etkin olmadığını söyleyen bir satır görünür. Dedektörü etkinleştirmeden önce ilgili ortamda Elasticsearch bağlantısını tanımlayıp etkinleştirin (Apinizer 2026.09.4).
...
## API Ürünü Abonelik Planları {#api-urunu-abonelik-planlari}
...
:::note[Plan limit profili referansları doğrulanır]
Bir API Ürünü paketindeki abonelik planının işaret ettiği limit profili artık hedef katalogda **doğrulanır**. Yönetilen (başka bir plana ait), yayınlanmamış, yanlış aileden ya da hedefte hiç bulunmayan bir profil referansı kapatılır — plan paketteki dakika/gün sayılarını **korur** ve kendi yönetilen profilini alır; neden, içe aktarma raporunda "atlanan referanslar" bölümünde listelenir. Yalnız hedefte gerçekten var olan, yayınlanmış ve doğru aileden bir katalog profiline işaret eden referans **korunur**; bu durumda planın sayıları o profilden türetilir. Bir portal klonlamasında aynı bilgi yanıtın **düşürülen limit profili referansları** bölümünde görünür.

Bu doğrulamanın diğer yüzü Limit Profilleri ekranındadır (**Yönetim → Limit Profilleri**): bir abonelik planına ait olan (yönetilen) profiller varsayılan listede **görünmez** — yalnızca "yönetilenleri göster" anahtarıyla, hangi abonelik planı tarafından yönetildiği etiketiyle görünürler — ve doğrudan düzenlenemezler; değişiklik ilgili abonelik planının kendi ekranından yapılır. Yeni üretilen yönetilen profillerin adı artık ürünü de içerir; mevcut profil adları/anahtarları değişmez.
:::
...
- [Bağlantı Yönetimi](/tr/integrations/baglanti-yonetimi/genel-bakis)
