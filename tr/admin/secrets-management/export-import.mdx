...
Sihirbazda dışa/içe aktarılabilecek nesne tipleri beş kategoride toplanır. **Bağımlılık** sütunu, o tipin dışa aktarımında ek bir bağımlılık adımı olup olmadığını gösterir:
...
- **Proje Dışa Aktar** — aktif projenin tamamını tek ZIP olarak dışa aktarır. Aktif proje seçili olmadığında giriş görünmez; menü ve sayfa aynı `PROJECT_MANAGEMENT / EXPORT_IMPORT` yetkisini ister.
...
- Pakette bulunan ve manifestte de adı geçen bir dosyanın **içeriği** dışa aktarımdan sonra değiştiyse — örneğin dosya elle düzenlendiyse — parmak izi kayıtla tutmaz. Bu durumda içe aktarma artık otomatik olarak reddedilmez: ekranda **"İçeriğin farklı olduğunu anladım"** onayını isteyen bir uyarı çıkar. Onaylarsanız içe aktarma pakette geldiği hâliyle sürer; onaylamazsanız işlem burada durur ve hiçbir kayıt yazılmaz. Onayı veren kullanıcı, hangi paketi hangi tarihte onaylayarak içe aktardığı bilgisiyle birlikte [denetim kayıtlarına](/tr/admin/audit-compliance/denetim-kayitlari) yazılır.
...
**"İçeriğin farklı olduğunu anladım"** onayı yalnızca içerik farkını (parmak izi uyuşmazlığını) kapsar; paketin eksik ya da fazladan dosya taşıması veya manifestin kendisinin bozuk olması bu onayla aşılamaz. Onayı kimin verdiği ayrıca kaydedildiği için, bu onayla yapılan bir içe aktarmayı sonradan denetim kayıtlarından geriye dönük olarak görebilirsiniz.
