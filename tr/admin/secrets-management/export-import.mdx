...
Bağlantı Yapılandırması (Kafka, Webhook, E-posta, Syslog, Graylog, RabbitMQ, DB Pool, Elasticsearch, SNMP, FTP, ActiveMQ, Linux Script, OTLP, Logback, LDAP Pool gibi) tiplerinin her birinin kendi liste ekranında, sihirbazın adımlarından bağımsız, tek adımlı bir **İçe Aktar** diyaloğu bulunur — yalnızca dosya seçilir, **Import as New** ya da **Replace Existing** arasında seçim yapılır ve doğrudan içe aktarılır. Bu basit diyalog sihirbazın adım/eşleme akışından bağımsız çalışmaya devam eder; ancak OTLP dışındaki bağlantı tipleri artık AYRICA sihirbazın kendi **Bağlantı (Connection)** ana nesne tipi altında da dışa/içe aktarılabilir — bkz. az aşağıdaki bölüm. OTLP bu yeni kataloğa hiç girmez, yalnızca bu basit diyalogla dışa/içe aktarılabilir. Şifreli bir paket de bu diyalogdan içe aktarılabilir: seçilen dosya parola korumalıysa ilk **Import** denemesinden sonra diyalog parola alanını açar — paketin dışa aktarılırken verilen parolasını girip **Import** düğmesine yeniden basmanız yeterlidir. Yanlış parola satır içinde bildirilir; dosyayı yeniden seçmeden düzeltebilirsiniz.
...
Yukarıdaki basit diyalogdan ayrı olarak **Bağlantı (Connection)**, sihirbazın **Bağlantılar** kategorisi altında kendi başına dışa/içe aktarılabilen bir ana nesne tipidir. Kapsadığı bağlantı tipleri: Veritabanı Havuzu, Kafka, Elasticsearch, E-posta, FTP, Graylog, Syslog, RabbitMQ, ActiveMQ, Webhook, Logback, SNMP, LDAP Havuzu ve Linux Script.
...
- **Bağlantı listesi ekranındaki eski Export düğmesinden farkı**: O düğme (ve yukarıda anlatılan basit **Import** diyaloğu) bağımlılık taşımaz — sertifika ya da key store olmadan yalnızca bağlantının kendi alanlarını dışa aktarır. Sertifikayı bağlantıyla **birlikte** taşımanız gerekiyorsa (örn. mTLS sertifikalı bir LDAP havuzu) o eski düğme yerine sihirbazdaki **Bağlantı** tipini kullanın.
