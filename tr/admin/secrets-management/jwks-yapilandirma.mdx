...
<Tabs>
    ...
    <TabItem value="url-baglantisiyla-al" label="URL Bağlantısıyla Al">
    ...
    <TabItem value="panoya-yapistir" label="Panoya Yapıştır">
    ...
    <TabItem value="keystore-dan-donustur" label="Keystore'dan Dönüştür">
        ...
        | **Takma Ad** _(Alias)_ | Keystore içinde hangi girişin kullanılacağını belirtir; oluşturulacak JWK'nın Key Id değeri buradan alınır. Zorunlu değildir. |
        | **Kullanım Tipi** _(Use Type)_ | JWK'nın imzalamak/imza doğrulamak veya şifrelemek/şifre çözmek için kullanılacağını belirtir. |

        **Takma Ad boş bırakıldığında:** Keystore içinde kullanılabilecek tek bir giriş varsa o giriş kendiliğinden seçilir ve seçilen takma ad, kayıt tamamlandıktan sonra JWK'nın kaynak bilgisinde görünür. Keystore birden fazla giriş içeriyorsa işlem durdurulur ve ekranda seçebileceğiniz takma adları listeleyen bir uyarı gösterilir; uyarı ayrıca seçilemeyen girişleri gerekçesiyle birlikte belirtir (gizli anahtar içermiyor, desteklenmeyen anahtar tipi, adı keystore tarafından üretilmiş). Birden fazla anahtar varken kendiliğinden bir seçim yapılmaz: yanlış anahtarla üretilen bir JWK ancak imza doğrulaması başarısız olduğunda fark edilir.
    </TabItem>

    <TabItem value="acik-anahtardan-donustur" label="Açık Anahtardan Dönüştür">
    ...
    <TabItem value="gizli-anahtardan-donustur" label="Gizli Anahtardan Dönüştür">
    ...
    <TabItem value="sertifikadan-donustur" label="Sertifikadan Dönüştür">
