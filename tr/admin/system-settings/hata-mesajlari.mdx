...
| 3 | Hata satırı bazlı alan özelleştirmesi | Politika içindeki satır bazlı Özelleştirilmiş Mesaj, Özelleştirilmiş Hata Kodu, Özelleştirilmiş HTTP Durum Kodu alanları |
| 4 | Platform geneli hata mesajı özelleştirmesi (Bu sayfa) | Bu sayfadan platform genelinde yapılan özelleştirmeler |
| 5 (En düşük) | Varsayılan hata mesajı | Sistem tarafından tanımlanan varsayılan hata mesajı |
...
| Mesaj Şablonu | Serbest metin şablonu yazılır. `#{error.effectiveErrorCode}`, `#{request.uri}`, `#{message.correlationId}` gibi yer tutucular kullanılabilir. |
...
<Info>
Bu sayfa platform genelindeki hata mesajı özelleştirmesini yönetir (Öncelik sırasında 4. seviye). Politika bazlı satır bazlı özel mesaj şablonu ve alan özelleştirmesi gibi daha yüksek öncelikli yöntemler politika güncelleme ekranında yapılır. Detaylar için [Politika Nedir? -- Hata Mesajı Özelleştirme](/tr/concepts/temel-kavramlar/politika-nedir#hata-mesajı-özelleştirme-error-message-customization) sayfasına bakın.
</Info>
