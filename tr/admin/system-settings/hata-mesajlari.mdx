...
<Info>
Politika ve İş Kuralı STOP aksiyonunda **şablon tabanlı (dinamik) hata mesajları** kullanarak serbest metin şablonu yazabilir ve `#{error.errorCode}`, `#{request.uri}`, `#{message.correlationId}` gibi yer tutucılarla istek/hata bilgilerini otomatik dahil edebilirsiniz. Bu özelliğin detayı için [Politika Nedir? – Şablon Tabanlı (Dinamik) Hata Mesajları](/tr/concepts/temel-kavramlar/politika-nedir#şablon-tabanlı-dinamik-hata-mesajları) bölümüne bakın.
</Info>
