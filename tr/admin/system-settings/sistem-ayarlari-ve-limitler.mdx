...
Başarısız giriş denemeleri iki ayrı sayaçla izlenir: kullanıcı başına sayaç, *kilitleme* eşiğine ulaşıldığında hesabı kilitler (kilit **Kullanıcılar → Kilitli** alanından görülür ve açılır); istemci adresi başına sayaç ise *captcha* eşiğine ulaşıldığında CAPTCHA'yı devreye alır. Giriş ekranı hangi durumun geçerli olduğunu hiçbir zaman söylemez: bilinmeyen kullanıcı adı, yanlış parola, kilitli ya da pasif hesap ve başarısız LDAP kontrolü aynı *Kullanıcı adı veya parola hatalı* yanıtını alır; kalan deneme sayısı gösterilmez. Bu bilgilerden herhangi birinin giriş yapmamış bir çağırana açıklanması, hangi hesapların var olduğunun keşfedilmesine (user enumeration) yol açar. Kesin neden, yöneticiler için **Giriş Kayıtları** ekranında kaydedilmeye devam eder.
