...
| **Oturum (Session)** | Yönetim Konsolu ve API Portal giriş, çıkış, token yenileme, hesap kilitleme ve parola yaşam döngüsü olayları | `persistent_audit_event` | `LOGIN_SUCCESS`, `LOGIN_FAILURE`, `LOGOUT`, `TOKEN_REFRESH_SUCCESS`, `TOKEN_REFRESH_FAILURE`, `ACCOUNT_LOCKED`, `PASSWORD_CHANGED`, `PASSWORD_CHANGE_REQUIRED`, `PASSWORD_RESET_REQUESTED`, `PASSWORD_POLICY_VIOLATION` | Olay tipinden türetilir; `ACCOUNT_LOCKED` için `DENIED`, `PASSWORD_POLICY_VIOLATION` için `FAILURE` |
...
:::info
Portal giriş ve çıkışları **Portal** hattının parçası DEĞİLDİR — bunlar Oturum (Session) olaylarıdır ve `persistent_audit_event` yerine ayrı bir koleksiyonda, `portal_login_logs`'ta fiziksel olarak tutulur. Yukarıdaki **Oturum** satırındaki `PORTAL_LOGIN_SUCCESS`, `PORTAL_LOGIN_FAILURE`, `PORTAL_LOGOUT`, `PORTAL_ACCOUNT_LOCKED`, `PORTAL_PASSWORD_CHANGED`, `PORTAL_PASSWORD_CHANGE_REQUIRED`, `PORTAL_PASSWORD_RESET_REQUESTED`, `PORTAL_PASSWORD_POLICY_VIOLATION` veya `PORTAL_ACCOUNT_DISABLED` olay tipi `source.component: PORTAL` ve `data.portalId` taşır; böylece bir kural, Yönetim Konsolu girişlerine dokunmadan portal girişlerini seçebilir veya dışlayabilir — tam katalog için bkz. [API Portal Giriş Kayıtları](/tr/api-portal/giris-kayitlari#olay-tipleri-ve-sebep-kodlari). `PORTAL_ACCOUNT_DISABLED` şimdilik yalnızca [inaktif-hesap görevi](/tr/api-portal/api-portal-ayarlari#güvenlik) tarafından yazılır.
:::

Ekranda **Planlandı** rozetiyle görünen ve henüz yapılandırılamayan bir hat daha vardır: **API Erişimi (API Access)**. Ağ geçidinde engellenen veya hata veren isteklerin kendisi artık yukarıdaki **API Güvenliği** hattı üzerinden SIEM'e yönlendirilebilir; başarılı isteklerin tam trafik metadata'sı ise (API Erişimi hattı) henüz yapılandırılamaz — trafik logları ağ geçidi ortamının log konnektörleri üzerinden iletilmeye devam eder.
...
| **Oturum** | Çıkış ve başarılı token yenileme 1 · başarılı giriş 2 · parola değiştirildi, parola değişimi zorunlu ve parola sıfırlama talebi 3 · parola politikası ihlali 4 · başarısız giriş ve başarısız token yenileme 5 · hesap kilitlendi 7 — dokuz `PORTAL_` ikizi de aynı değeri taşır, artı `PORTAL_ACCOUNT_DISABLED` 5 |
...
- **API Erişimi hattı** rezervedir ve yapılandırılamaz. Başarılı isteklerin tam trafik metadata'sı SIEM hatlarına henüz dahil değildir; trafik logları ağ geçidi ortamının log konnektörleri üzerinden iletilmeye devam eder.
- **API Güvenliği hedefleri Şema v2, CEF veya LEEF kullanmalıdır — Legacy Raw bu hat için kullanılamaz.** Bu hattın kuralı bağlı olduğu bir hedefin yük profili Legacy Raw ise, API Güvenliği olayı o hedefe hiç gönderilmez.
