...
- **Hat Kuralları** — her hat için bir bölüm; her biri *Bu hattı bu hedefe gönder* anahtarıyla başlar. Denetim ve Portal bölümlerinde ayrıca *Referans nesnesini yüke ekle* (varsayılan kapalı) ve *Değişiklik listesini yüke ekle* (varsayılan açık) içerik anahtarları, Uygulama bölümünde ise *Hata yığınını yüke ekle* (varsayılan açık) anahtarı bulunur.

:::info
Bir **Veritabanı** hedefinin konnektörü, Gateway Runtime konnektörleriyle aynı **Operasyon** alanını kullanır — veritabanı tipinden bağımsız olarak yalnızca **Insert-Update-Delete** sunulur. Detay için bkz. [Log Konnektörü Eklenmesi](/tr/admin/server-management/gateway-ortamlarina-konnektor-eklenmesi#veritabani-alicisinda-operasyon-alani).
:::

Bir hattı taşımadan önce bağlantıyı doğrulamak için kaydedilmiş hedefin **Test olayı gönder** işlemini kullanınız.

<Frame caption="SIEM ve Log Yönlendirme — Hedefler sekmesi">
  <img
    src="/images/2026-09-2/siem-log-forwarding-destinations.png"
    alt="SIEM ve Log Yönlendirme Hedefler sekmesi: hedef kartları ve Teslimat Sağlığı özeti"
    width="1000"
    height="492"
  />
</Frame>
...
<Frame caption="SIEM ve Log Yönlendirme — Hatlar sekmesi">
  <img
    src="/images/2026-09-2/siem-log-forwarding-streams.png"
    alt="SIEM ve Log Yönlendirme Hatlar sekmesi: yerel saklama, hedefler ve yetki sütunları"
    width="1000"
    height="688"
  />
</Frame>
...
<Frame caption="SIEM ve Log Yönlendirme — Şema ve Örnekler sekmesi">
  <img
    src="/images/2026-09-2/siem-log-forwarding-schema.png"
    alt="SIEM ve Log Yönlendirme Şema ve Örnekler sekmesi: hat ve profil seçicisi ile örnek yük"
    width="1000"
    height="519"
  />
</Frame>
...
| Bir değişikliğin türü görülsün, ama alan bazlı eski/yeni değer ayrıntısı yüke eklenmesin | Denetim/Portal hedefi → **Değişiklik listesini yüke ekle**: kapalı (`data.changesStatus` ve `data.changeKinds` yine de gönderilir) |
...
**Birinci katman kapatılamaz.** Token değerleri, parolalar ve parola özetleri, `Authorization`, `Cookie` ve `Set-Cookie` başlıkları, API anahtarı başlıkları, sorgu dizeleri, sertifika ve anahtar materyali ile gizlilik tuzunun kendisi bir v2 yükünün parçası hiçbir zaman olmaz. Bunlar izin verilen alan listesinde yer almaz ve ikinci bir denetim, adı kimlik bilgisine benzeyen kalan alanları da kaldırır. Gateway Runtime ortamının token imzalama anahtarı (JWT Token Doğrulama Anahtarı sekmesindeki RSA özel anahtar) da bu katmanın parçasıdır: ne ortam kaydının denetim satırında ne değişiklik listesinde ne de yakalanan istek gövdesinde yer alır.
...
| **Veriyi kırp** | `data` içindeki büyük alanlar — `changes`, `reference`, `message`, `stackTrace` — yük sığana kadar büyükten küçüğe kısaltılır (`changes` ve `reference` önce kısaltılır), her biri kırpma notuyla işaretlenir ve `apinizer.truncated` değeri `true` olur. Zarf alanlarına dokunulmaz. Kırpma yeterli olmazsa olay düşürülür. |
...
Bir olayı büyüten alanlar uygulama hata yığınları, denetim referans nesneleri ve denetim/Portal değişiklik listeleridir. Üçü de isteğe bağlıdır ya da sınırlanabilir: referans nesnesi denetim hattında varsayılan olarak kapalıdır, değişiklik listesi varsayılan olarak açıktır ve ihtiyaç duymayan bir hedef için kapatılabilir, hata yığını anahtarı ise aynı şekilde ihtiyaç duymayan bir hedef için kapatılabilir.
...
## SESSION hattı — olay tipleri ve legacy yük {#session-hatti-olay-tipleri-ve-legacy-yuk}

Yukarıdaki [Hatlar](#hatlar) tablosu `SESSION` olay tiplerinin adlarını verir; bu bölüm her birinin ne anlama geldiğini ve bir **LEGACY_RAW** hedefinin tam olarak neyi aldığını anlatır. Buradaki içerik Yönetim Konsolu girişleri içindir. Portal girişleri aynı hat üzerinden gider ama kendi olay adlarını ve kendi yük biçimini taşır — onlar için bkz. [API Portal Giriş Kayıtları](/tr/api-portal/giris-kayitlari#konnektor-teslimi-ve-yuk-bicimi).

Hattın yetkisi ne olursa olsun, bir `SESSION` kaydı **her zaman** Apinizer veritabanına yazılır (Yönetim Konsolu için `persistent_audit_event`, portal için `portal_login_logs`) ve bu davranış kapatılamaz. Hedefler, veritabanı yazımına **ek** olarak giden ikinci bir kopyadır; onun yerine geçmez.

### Yönetim Konsolu olay tipleri
...
Her olay ayrıca `userAgent` taşır; başarısız olaylarda makine tarafından okunabilir bir `reasonCode` bulunur. Alan ve neden kodu listesi için bkz. [Giriş Kayıtları](/tr/admin/audit-compliance/giris-kayitlari).

### LEGACY_RAW yük biçimi

Aşağıdaki biçim **LEGACY_RAW** yük profilinin çıktısıdır; alan listesi dondurulmuştur ve sonraki sürümlerde büyümez. Kanonik zarf, ortak aktör/hedef blokları ve alan bazlı gizlilik için hedefi **APINIZER_JSON_V2** profiline geçirin — bkz. [Şema v2](#sema-v2).
...
Başarısız bir girişte `message` ve `errorType` dolar:
...
**Graylog hedefleri** aynı olayı GELF mesajı olarak alır; kısa mesaj (short message) `LogLogin` değeridir. **Mesaja Ekle (Append to Message)** açıkken yukarıdaki JSON gövdesi GELF `full_message` alanına yazılır; **Özniteliklere Ekle (Append to Attributes)** açıkken dolu olan her alan (`source`, `principal`, `email`, `displayName`, `eventDate`, `eventType`, `remoteAddress`, `message`, `errorType`, `portalId`, `accountId`, `userAgent`, `reasonCode`, `targetProjectId`) ayrı bir GELF additional field olarak gönderilir. Değeri `null` olan alanlar gönderilmez.

:::info

**Database** tipindeki bir hedef `SESSION` kayıtlarını `log_LoginLog` tablosuna yazar. Bu tablo **otomatik oluşturulmaz** — hedefi etkinleştirmeden önce oluşturmanız gerekir. Desteklenen her veritabanı (Oracle, MySQL/MariaDB, PostgreSQL, SQL Server) için CREATE TABLE komutları: [Apinizer Log Tabloları Oluşturma Komutları](/tr/operations/yonetici-kilavuzlari/apinizer-log-tablolari-olusturma-komutlari#login-log).
...
`data` nesnesi, platformda tanımlı sabit bir alan kümesi taşır. Genişletilemez; yalnızca daraltılabilir — gizlilik profiliyle ya da kuraldaki üç içerik anahtarıyla.
...
| `target.type`, `target.id`, `target.name`, `target.projectId`, `target.projectName` | `className`, `objectId`, `objectName`, `projectId`, `projectName` |
...
| `data.changesStatus`, `data.changeKinds`, `data.changedPaths`, `data.changes` | Yeni alanlar — varlık değişikliklerinde önceki ve yeni anlık görüntü arasındaki farkı taşır; bkz. aşağıdaki [Değişiklik listesi](#degisiklik-listesi). |
...
Bir **Varlık** olayında Denetim ve Portal hatları, önceki ve yeni anlık görüntü arasındaki farkı da dört alanla taşır.

**`data.changesStatus`** karşılaştırmanın hangi koşulda üretildiğini bildirir; her zaman gönderilir:

| Değer | Anlamı |
|---|---|
| `DIFFED` | Fark başarıyla hesaplandı; `data.changes` ve `data.changedPaths` doludur. |
| `NO_PREVIOUS_SNAPSHOT` | Karşılaştırılacak önceki bir kayıt yok — genellikle ilk oluşturma. |
| `SECRET_FIELDS_MAY_HAVE_CHANGED` | Gizli alanlar karşılaştırma dışı tutulduğu için fark eksik olabilir. |
| `TOO_MANY_CHANGES` | Değişiklik sayısı 200 kaydı aştı; `data.changes` kesildi, `data.changedPaths` yine de eksiksizdir. |

**`data.changeKinds`**, değişikliğin etkilediği alanları sabit bir sözlüğe indirger ve her zaman gönderilir: `ENDPOINT_ADDED`, `ENDPOINT_REMOVED`, `ENDPOINT_METHOD_CHANGED`, `ENDPOINT_PATH_CHANGED`, `RELATIVE_PATH_CHANGED`, `POLICY_ADDED`, `POLICY_REMOVED`, `POLICY_CHANGED`, `ROUTING_CHANGED`, `CREDENTIAL_ACL_CHANGED`, `ROLE_PERMISSION_CHANGED`, `USER_ROLE_CHANGED`, `SETTING_CHANGED`, `LICENSE_CHANGED` — örneğin bir endpoint eklenmesi, bir endpoint'in yolunun veya metodunun değişmesi, bir politikanın eklenip kaldırılması ya da değişmesi, yönlendirmenin değişmesi, bir kimlik bilgisinin erişim izinlerinin (ACL) değişmesi, bir rolün izinlerinin veya bir kullanıcının rolünün değişmesi ya da bir ayarın değişmesi gibi. `RELATIVE_PATH_CHANGED` API Proxy'nin İstemci Rotası (Client Route) göreli yolunu ifade eder. `LICENSE_CHANGED` lisans anahtarının değiştirildiğini bildirir; anahtarın kendisi hiçbir zaman gönderilmez (eski/yeni değeri her zaman `"[REDACTED]"` olur).

**`data.changedPaths`**, değişen her alanın yolunu listeler (örn. `/apiMethodList/getPetById`) ve her zaman gönderilir; liste elemanları kendi kimliği veya adıyla adreslenir, böylece aynı endpoint art arda gelen olaylarda hep aynı yolu taşır.

**`data.changes`**, her biri tek bir alan değişikliğini tanımlayan kayıtların listesidir: `op` (`add`, `remove`, `replace` veya `move`), `path`, eşleşen liste elemanının kimliği/adı olan `key` (varsa), `old`, `new` ve yalnızca `move` işleminde kaynak yolu belirten `from`. **Yalnızca** *Değişiklik listesini yüke ekle* açıkken gönderilir — varsayılan **açık**; kapatıldığında `data.changesStatus` ve `data.changeKinds` yine de gönderilmeye devam eder, yalnızca `data.changes` ve `data.changedPaths` düşer.

:::warning

`old` ve `new` gizli bir alanı hiçbir zaman taşımaz — gizli olduğu bilinen bir alan (örneğin adında parola, token veya secret geçen bir ayar ya da lisans anahtarı) `"[REDACTED]"` değeriyle gönderilir; girişin kendisi, `op` ve `path` yine gider, yani böyle bir alanın eklendiği, silindiği veya değiştiği bilgisi SIEM'e her zaman ulaşır. 512 karakterden uzun değerler kırpılır.

:::

Nesnenin önceki hâlinin ayrı bir kopyası hâlâ hiçbir zaman gönderilmez — bu dört alanın oluşturduğu fark, tam olarak o kopyaya duyulan ihtiyacın yerini almak için vardır.

Aşağıda bir endpoint eklenmesi ve bir Genel Ayarlar değişikliği için üretilen olaylar örneklenmiştir (zarfın geri kalanı yukarıdaki [Zarf](#sema-v2) örneğiyle aynı yapıdadır):

```json
{
  "logType": "AUDIT",
  "schemaVersion": 2,
  "eventId": "68c1a2b380f5de2a3c29d001",
  "timestamp": "2026-09-10T09:12:44.512Z",
  ...
  "correlationId": "b1a2c3d4-5e6f-4a1b-9c2d-3e4f5a6b7c8d",
  "source": { "component": "MANAGER", "host": "manager-0", "environmentId": null },
  "apinizer": { "version": "2026.09.3", "configRevision": 18, "destinationId": "5f2c9a1e-7c4d-4a9b-9f0e-1b2c3d4e5f60", "synthetic": false, "truncated": false },
  "data": {
    "id": "68493bb680f5de2a3c29c609",
    "state": "UPDATED",
    "origin": "MANAGER",
    ...
    "changedPaths": ["/apiMethodList/getPetById"],
    "changes": [
      { "op": "add", "path": "/apiMethodList/getPetById", "key": "getPetById", "new": { "relativePath": "/pet/{id}", "httpMethod": "GET" } }
    ]
  }
}
```
...
```json
{
  "logType": "AUDIT",
  "schemaVersion": 2,
  "eventId": "68c1a2b380f5de2a3c29d044",
  "timestamp": "2026-09-10T09:20:01.884Z",
  "event": { "category": "AUDIT", "action": "ENTITY.UPDATED", "outcome": "SUCCESS", "severity": 3 },
  "actor": { "user": "admin", "ip": "10.0.0.7", "userAgent": "Mozilla/5.0", "sessionId": null },
  "target": {
    "type": "GeneralSettings",
    "id": "5f2c9a1e-0000-4a9b-9f0e-1b2c3d4e5f00",
    "name": "GeneralSettings",
    "projectId": null,
    "projectName": null
  },
  "correlationId": "c2b3d4e5-6f7a-4b1c-8d3e-4f5a6b7c8d9e",
  "source": { "component": "MANAGER", "host": "manager-0", "environmentId": null },
  "apinizer": { "version": "2026.09.3", "configRevision": 19, "destinationId": "5f2c9a1e-7c4d-4a9b-9f0e-1b2c3d4e5f60", "synthetic": false, "truncated": false },
  "data": {
    "id": "5f2c9a1e-0000-4a9b-9f0e-1b2c3d4e5f00",
    "state": "UPDATED",
    "origin": "MANAGER",
    ...
    "changedPaths": ["/enableManagerRequestAudit"],
    "changes": [
      { "op": "replace", "path": "/enableManagerRequestAudit", "old": false, "new": true }
    ]
  }
}
```
...
| `flexString2` + `flexString2Label=projectName` | Proje adı |
...
`sender` sayaçlarından ikisi, "karşı taraf boştaki oturumları kapatıyor" durumunu "karşı taraf gerçekten yetişemiyor" durumundan ayırır: `idleReconnectCount`, bir süre hiçbir şey gönderilmediği için ağ geçidinin yazmadan önce yenilediği soket sayısıdır; `recoveredBatchCount` ise bir kez başarısız olup taze sokette başarılı olan gönderim kümesi sayısıdır. `lossCount` sıfırda kalırken artan bir `recoveredBatchCount`, kayıt kaybı olmadığı ve karşı tarafın oturumları ağ geçidinin kendi eşiğinden daha erken kapattığı anlamına gelir; ayrıntı için Syslog bağlantı sayfasındaki [Boşta Kalan Bağlantılar ve Yeniden Bağlanma](/tr/integrations/baglanti-yonetimi/syslog#bosta-kalan-baglantilar) bölümüne bakın. Artan bir `lossCount` veya `rejectedCount` ise bundan farklı, daha ciddi bir durumdur.

Kalan iki `sender` sayacı ağ yoluna değil doğrudan karşı tarafa işaret eder: `reconnectCount`, bir hedefin soketlerinin pod başladığından bu yana açtığı taze TCP bağlantılarının toplamıdır — her soketin ilk bağlantısı da sayılır, yani hiç kopmamış bir gönderici bile soket başına bir gösterir; bunun üzerindeki her artış bir kopma ve yeniden bağlanmadır. `peerClosedAtAcceptCount` ise bu bağlantılardan kaçının, tek bir bayt bile alışverişi yapılmadan, alıcı tarafından kabul edilir edilmez tekrar kapatıldığını sayar. Sıfırdan büyük bir `peerClosedAtAcceptCount`, arızanın ağ yolunda veya ağ geçidinde değil karşı tarafta olduğu anlamına gelir — bir oturum sınırı, bir kaynak-IP reddi ya da aradaki bir yük dengeleyici. İki sayaç neredeyse her kümede birlikte artıyorsa, alıcı her denemede bir oturum açıp hemen düşürüyordur (bağlantı fırtınası): düşen her bağlantı yine de bir bağlantı hatası sayılır, bu yüzden art arda üç hata o hedefin trafiğini, tıpkı diğer bağlantı hatalarında olduğu gibi yedek (failover) connector'a taşır. Alıcı tarafın işareti ve ayar özellikleri için Syslog bağlantı sayfasındaki [Bağlantıyı Kabul Eder Etmez Kapatan Alıcılar](/tr/integrations/baglanti-yonetimi/syslog#baglanti-ani-canlilik) bölümüne bakın.
