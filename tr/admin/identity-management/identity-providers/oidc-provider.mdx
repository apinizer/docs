...
| Eşleşen Credential Zorunlu Olsun | Açıkken, imzası ve claim'leri geçerli olsa dahi eşleşen bir [Kimlik Bilgisi](/tr/admin/identity-management/kimlik-bilgileri) kaydı bulunamayan bir token anonim kabul edilmek yerine reddedilir. Varsayılan: açık. |
...
| Senkron İstemci ID | Admin API'yi çağırmak için kullanılan istemcidir; genelde OIDC girişinde kullanılan istemciden farklıdır ve service-account yetkilerine sahip olmalıdır. |
| Senkron Sayfa Boyutu | Keycloak Admin API'sinden kullanıcılar sayfalanarak (paged) çekilirken kullanılan sayfa boyutudur; büyük kullanıcı havuzlarında (~4000 kullanıcı) worker'a gönderilen güncelleme sayısını azaltmak için varsayılan değer yükseltilmiştir. |
...
| Deaktivasyon Modu | Son senkronizasyon koşusunda Keycloak'ta artık bulunmayan kullanıcılara karşılık gelen Kimlik Bilgisi kayıtlarına uygulanacak işlemdir. |
...
**Senkron İstemci ID** alanında belirtilen Keycloak istemcisi, **service account'u olan gizli (confidential) bir istemci** olmalıdır. Bu service account'a, ilgili realm'in `realm-management` istemcisi altında en az şu client role'ler atanmalıdır: **`view-users`** ve **`query-groups`**.
