...
<Frame caption="Database sağlayıcı — Identity Synchronization Profile">
  <img
    src="/images/2026-09-2/credential-sync-profil.png"
    alt="Database sağlayıcı Identity Synchronization Profile sekmesi: cron, deaktivasyon modu, senkronizasyon kapsamı ve uzlaştırma emniyet ağı"
    width="1000"
    height="942"
  />
</Frame>
...
Bir kaynak varsayılan olarak okuyabildiği her şeyi senkronize eder. Aynı sekmedeki **Senkronizasyon Kapsamı** kartı bu kümeyi daraltır: **Kullanıcılar**, **Gruplar / Organizasyonlar** (LDAP, OIDC ve Veri Tabanı — API sağlayıcı ekranında henüz yok) ve **Roller** (yalnız OIDC) için ayrı kabul/ret desen listeleri tanımlanır.
...
| Veritabanı | İsteğe bağlı — kullanıcı listesi sorgusuna `external_id` sütunu eklendiğinde. Bir satırda boş bırakılırsa o satır için kullanıcı adıyla eşleştirmeye dönülür. |
| API | İsteğe bağlı — Kararlı Kimlik JSON Path alanı yapılandırıldığında. Bir satırda eşleşmezse o satır için kullanıcı adıyla eşleştirmeye dönülür. |
...
Bu opsiyonel ayarın gelecek bir ana sürümde varsayılan hâline gelmesi planlanmaktadır. Kararlı öznitelik çekmeyen LDAP bağlantılarında ve kararlı kimlik sütunu/alanı yapılandırılmamış Veritabanı ve API kaynaklarında — yani kararlı kimliğin hiç elde edilemediği durumlarda — ayarı şimdiden açmak güvenli taraftır.
...
<Frame caption="Kimlik Senkronizasyonu — sağlayıcı listesi ve geçmiş">
  <img
    src="/images/2026-09-2/credential-sync-izleme.png"
    alt="Kimlik Senkronizasyonu ekranı: LDAP Database API OIDC sekmeleri, sağlayıcı tablosu ve senkronizasyon geçmişi"
    width="1000"
    height="514"
  />
</Frame>
...
Veritabanı ve API kaynakları kullanıcıları ve kurumları her zaman tek bir geçişte birlikte okur; ayrı bir yalnız-kullanıcı ya da yalnız-grup aşamaları yoktur, bu yüzden düğmenin bu iki kaynakta menüsü yoktur — buradaki her koşu **Tam**'dır. Bir koşunun kapsamı [koşu geçmişindeki](#senkronizasyon-koşu-geçmişi) **Kapsam** sütununda görünür; bu sürümden önce kaydedilmiş satırlar **Tam** görünür, çünkü o koşular gerçekten tam koşuydu.
...
| Sonuç | Koşunun o anki durumu: **Kuyrukta**, **Çalışıyor**, **Başarılı**, **Uyarı**, **Başarısız**, **Yarıda Kaldı**, **İptal Edildi** veya **Uzlaştırma Onayı Bekliyor**. Anlamları için [Koşu Yaşam Döngüsü](#koşu-yaşam-döngüsü) bölümüne bakınız. Koşu bu durumlardan biriyle başarıyla sonuçlanamadıysa, durum rozetinin yanında nedeni özetleyen bir **sebep rozeti** de görünür — bkz. [Başarısızlık Sebebi Rozeti](#başarısızlık-sebebi-rozeti). |
| Eklenen/Güncellenen, Değişmeyen, Deaktif Edilen, Atlanan, Hata | Koşuda işlenen kayıt sayıları — **Değişmeyen** için bkz. [Değişmeyeni Atlama](#değişmeyeni-atlama). |
...
| İşlemler | Bekleyen bir uzlaştırma onayı hâlâ onaylanabilir durumdayken **Uzlaştırmayı Onayla** düğmesini, koşu **Kuyrukta** ya da **Çalışıyor** durumundayken **İptal** düğmesini taşır — bkz. [Deaktivasyon Eşiği ve Uzlaştırma Onayı](#deaktivasyon-eşiği-ve-uzlaştırma-onayı) ve [İptal](#i̇ptal). |
...
### LDAP Kaynağı İçin Ek Sütunlar

LDAP kaynaklarına ait satırlarda yukarıdaki sütunlara ek olarak şu sayaçlar görünür:

| Sütun | Açıklama |
|-------|----------|
| Okunan Kullanıcı | Bu koşuda dizinden okunan toplam kullanıcı kaydı sayısı. |
| Üyelik Alanı Olan | Kendi kaydında üyelik alanını (`memberOf` vb.) taşıyan kullanıcılar. |
| Okunan Grup | Bu koşuda okunan toplam grup/OU kaydı sayısı. |
| Üyesi Olan Grup | Üye alanı boş olmayan gruplar. |
| Elenen Grup | Kapsam filtresinin elediği gruplar; bu bir hata değil, sizin kapsam kararınızdır. |
| Bağlanan (S1) | Kendi kaydındaki üyelik alanından kuruma bağlanan kullanıcılar. |
| Bağlanan (S2) | Grubun üye listesinden kuruma bağlanan kullanıcılar. |
| Bağlanan (S3) | DN'inin altında bulunduğu OU'dan kuruma bağlanan kullanıcılar. |
| Bağlanamayan | Hiçbir stratejinin kuruma bağlayamadığı kullanıcılar. |
| Yeniden Etkinleştirilen | Kaynakta yeniden göründüğü için bu koşuda otomatik olarak açılan kayıt sayısı. |

Bu sütunlar yalnızca LDAP kaynağının koşularında dolu görünür. Bu sayaç kümesinden önce kaydedilmiş LDAP
koşularında ve LDAP dışındaki her kaynakta değer `-` görüntülenir. Alan açıklamaları için
[LDAP Active Directory](/tr/admin/identity-management/identity-providers/ldap-active-directory#senkronizasyon)
...
Dizinin okumayı **reddetmesi** bu iki durumdan biri değildir. Dizinde bulunmayan bir Base DN ve o dalda arama yetkisi olmayan bir bağlanma hesabı, koşuyu **Uyarı** ile değil **Başarısız** ile kapatır; rozet **Yapılandırma** ya da **İzin eksik** olur ve gerekçe metni Base DN değerini yazar. İkisi de yöneticinin düzeltebileceği ayarlardır; eksik okuma gibi raporlanmaları onları sayfalamayı desteklemeyen bir dizin sunucusundan ayırt edilemez kılıyordu.

:::

### Veri Tabanı ve API Kaynağı İçin Ek Sütunlar

Veri Tabanı ve API kaynaklarına ait satırlarda yukarıdaki ortak sütunlara ek olarak şu sayaçlar görünür:

| Sütun | Açıklama |
|-------|----------|
| Okunan kurum | Kaynağın kurum listesinden okunan satır sayısı. 0 ise kurum listesi tanımlı değildir. |
| Türetilen kurum | Kurum listesinde karşılığı olmadığı için kullanıcı satırındaki yoldan kurulan kurum sayısı. |
| Kuruma bağlanan | Bir kuruma bağlanan kullanıcı sayısı. |
| Kaynakta kapalı | Kaynakta devre dışı olduğu için bu koşuda kapatılan kayıt sayısı — **Deaktif Edilen** sütunundan ayrıdır (o, kaydın kaynakta artık hiç görülmediğini söyler). |
| Okunan sayfa | Sayfalayan bir kaynaktan gerçekten okunan sayfa sayısı; tek sorguda okuyan kaynaklarda boş kalır. |

Bu beş sütun yalnızca Veri Tabanı ve API kaynaklarının koşularında dolu görünür; LDAP koşularında ve bu sayaçlardan önce kaydedilmiş koşularda değer `-` görüntülenir. **Kuruma bağlanan** sütunu LDAP'ta boştur, çünkü LDAP'ta aynı bilgi yukarıdaki **Bağlanan (S1/S2/S3)** sütunlarına ayrılmıştır.

Aynı [Başarısızlık Sebebi Rozeti](#başarısızlık-sebebi-rozeti) tablosu Veri Tabanı ve API kaynaklarında da aynı şekilde geçerlidir.
...
| İptal Edildi | Koşu, **Kuyrukta** ya da **Çalışıyor** durumundayken bir yönetici tarafından elle durduruldu — bkz. [İptal](#i̇ptal). |
...
Koşu **Başarısız**, **Yarıda Kaldı** ya da **İptal Edildi** durumuyla sonuçlandığında, [koşu geçmişindeki](#senkronizasyon-koşu-geçmişi) durum rozetinin yanında, nedeni özetleyen ikinci bir **sebep rozeti** görünür:
...
| İzin eksik | Kaynak yetkilendirmeyi reddetti (HTTP 403 ya da eşdeğer bir LDAP yetki hatası); rozetin ayrıntısında hangi okumanın — **Kullanıcılar**, **Gruplar** veya **Roller** — engellendiği belirtilir. Dizin kaynağında bu rozet, bağlanabilen ama tanımlı Base DN altında arama yapamayan bağlanma hesabını da kapsar. | Servis hesabına eksik izni/rolü tanımlayın. Dizin kaynağında bağlanma hesabına ilgili dal için okuma ve arama yetkisi verin. |
...
| Yapılandırma | Koşu, kaynağa hiç gidilmeden ön kontrol aşamasında elendi (örn. eksik bağlantı bilgisi, boş bırakılmış zorunlu bir alan). Tanımlı Base DN dizinde bulunmadığı için dizinin okumayı reddettiği durum da bu rozeti taşır; gerekçe metni denenen Base DN değerini yazar. | Sağlayıcının yapılandırmasını tamamlayıp koşuyu yeniden tetikleyin. Dizin kaynağında bağlantının ya da kullanıcı/grup nesne sınıfının Base DN değerini düzeltin. |
...
| İptal edildi | Koşu bir yönetici tarafından elle durduruldu — bkz. [İptal](#i̇ptal). | Gerekirse senkronizasyonu yeniden tetikleyin. |
...
**Kuyrukta** ya da **Çalışıyor** durumundaki bir koşu, [koşu geçmişi](#senkronizasyon-koşu-geçmişi) tablosundaki **İptal** düğmesiyle durdurulabilir; düğme, ilgili kimlik sağlayıcıyı yönetebilen kullanıcılara (kimlik yönetimi yetkisi) açıktır.
...
Koşu **İptal Edildi** durumunda kapanır; kaynağın son senkronizasyon sonucu, tıpkı [uzlaştırma atlandığında](#deaktivasyon-eşiği-ve-uzlaştırma-onayı) olduğu gibi **Uyarı** olarak görünür.
...
:::note Yükseltme sonrası ilk koşu
...
| Yinelenen oturum açma adı | Aynı sayfada okunan başka bir kaynak kaydı aynı oturum açma adını kullanıyor. İlk okunan kayıt tutulur, ikincisi atlanır; bu adla yalnız bir kimlik bilgisi oluşturulur. LDAP'ta hangi iki dizin nesnesinin çakıştığını görmek için her iki kaydın da ayırt edici adı (DN) kaydedilir. |
...
:::note Yalnız LDAP'a özgü üyelik teşhisi

Üç ek gerekçe yalnız LDAP kaynağına özgüdür ve bir kimlik bilgisini atlamaz — kullanıcı yine oluşturulur ya da güncellenir, yalnızca [kurum bağı](#kurum-bağı-nasıl-belirlenir) etkilenir:

- Bir kullanıcı hiçbir üyelik kaynağından kuruma bağlanamadığında, o kullanıcı için (koşu başına en fazla 500 adet) bir kayıt.
- Bağlanamayan kullanıcı sayısı bu 500 sınırını aştığında, kaç kullanıcı daha aynı sonucu verdiğini özetleyen tek bir koşu-geneli kayıt.
- Koşuda okunan **tüm** kullanıcılar bağlanamadığında tek bir koşu-geneli kayıt — bu, tek tek kullanıcılardan çok bir yapılandırma sorununa (üyelik modu, üye özniteliği, grup kapsamı) işaret eder.

Hangi üyelik kaynağının bir kullanıcı için neden başarısız olduğunu görmek için **Açıkla**'yı ya da **Dizin Önizleme**'nin **Sorunlar** sekmesini (I01–I04 kodları) kullanın — bkz. [Sorun Giderme](/tr/admin/identity-management/identity-providers/ldap-active-directory#sorun-giderme).
...
LDAP, Keycloak, Veritabanı veya API senkronizasyonundan doğan her tüketici, artık elle oluşturulan bir tüketici gibi kalıcı bir **tüketici anahtarı** alır ve kaydına **ilk senkron zamanı** damgası işlenir; [envanter ve taşıma](/tr/admin/identity-management/kimlik-bilgileri#eski-kayitlar-envanter-ve-tasima) ekranları ile dışa aktarma bu kayıtları artık anahtarsız bir kayıt olarak görmez.
...
| İstek | Yanıt |
|-------|-------|
| `POST /api/identity-sync-run-histories/{runId}/actions/cancel` | **200** — koşu geçmişi tablosundaki **İptal** düğmesiyle aynı davranış, bkz. [İptal](#i̇ptal). Koşu **Kuyrukta** ya da **Çalışıyor** değilse çevrilmiş bir mesajla **409** döner. |

## Kurum Bağı Nasıl Belirlenir

:::info Tüketici başına tek kurum

Her kaynakta bir tüketici **en fazla bir** kuruma bağlanır. Kullanıcı kaynakta birden fazla aday kuruma uygun görünse bile tek bir kazanan seçilir; diğer aday üyelikler tüketiciye yazılmaz. LDAP'ta bu diğer üyelikler **Açıkla** aracının teşhis çıktısında yine görünür — bkz. [Sorun Giderme](/tr/admin/identity-management/identity-providers/ldap-active-directory#sorun-giderme).

:::

| Kaynak | Adaylar | Kazanan nasıl seçilir |
|---|---|---|
| LDAP | Senkronizasyon kapsamındaki gruplar ve OU'lar | En fazla ayırt edici bileşene (RDN) sahip aday (en derin olan) kazanır. Eşit derinlikte hangi adayın kazanacağı garanti edilmez — dizin tasarımını buna dayandırmayın. **OU ve grup** modunda önce kullanıcının kendi kaydının bulunduğu OU denenir; yalnız o çözümlenmezse açık bir grup üyeliği kullanılır. |
| OIDC (Keycloak) | **Grupları Senkronize Et** açıkken erişilen gruplar | En derin grup yolu (en çok `/` ayracı olan) kazanır. Eşit derinlikte, koşunun önce işlediği grup kazanır — bu, sizin ayarlayabileceğiniz bir kural değil, Keycloak'ın grup ağacını döndürdüğü sıradır. |
| Veri Tabanı | Kullanıcı satırından okunan tek kurum yolu | Seçim yoktur: satır tek bir yol bildirir, kurum da odur. |
| API | Yapılandırılan **Kurum Yolu JSON Path** ifadesinden okunan tek kurum yolu | Veri Tabanı ile aynıdır — tek yol, seçim yok. |

:::note Kurum bağı her tam koşuda yeniden yazılır — elle yapılan atama dahil

Kullanıcının kaynaktaki grubu ya da yolu değiştiğinde, bir sonraki **Tam** koşu tüketicinin kurumunu buna göre günceller. Kullanıcı artık kapsam içi hiçbir kuruma uygun değilse kurum bağı boşalır. Tam koşuda kaynağın cevabı her zaman kazandığı için, senkronize bir tüketicide elle atadığınız bir kurum da bir sonraki tam koşuda aynı şekilde değiştirilir.

Grup/kurum fazını hiç okumayan bir koşu kurumlar hakkında bir şey söylemez, dolayısıyla mevcut bağı olduğu gibi bırakır. "Okumadı" sayılan durum kaynağa göre değişir:

| Kaynak | Kurum bağı ne zaman dokunulmadan kalır |
|---|---|
| LDAP | Koşunun kapsamı **Yalnız kullanıcı** ise — bkz. [Elle Koşunun Kapsamını Seçme](#elle-koşunun-kapsamını-seçme). Kurum ağacı yalnızca kısmen okunduysa (sayfalama uyarısı) ve o kısmi okumada bir kullanıcı için kurum bulunamadıysa mevcut bağ da korunur — "bu koşuda bilinmiyor" ile "kurumu yok" aynı şey değildir. |
| OIDC (Keycloak) | Koşunun kapsamı **Yalnız kullanıcı** ise. **Grupları Senkronize Et** ayarını kapatmak TEK BAŞINA kurum bağını korumaz: normal bir **Tam** koşu yine çalışır, yine hiçbir grup okumaz ve kurumu yine de boş olarak yazar — yani **Grupları Senkronize Et** kapatıldıktan sonraki ilk tam koşu, bu kaynaktan gelen tüm mevcut kurum bağlarını temizler. |
| Veri Tabanı | **Kurum Yolu** sütunu Kullanıcı Listeleme Sorgusu'nda hiç yoksa. Sütun varsa ama bir satırda boş bırakılmışsa, yalnız o satırın bağı kaldırılır. |
| API | **Kurum Yolu JSON Path** alanı boş bırakılmışsa. Alan tanımlıysa ama ifadesi bir satırda hiçbir şeyle eşleşmiyorsa, yalnız o satırın bağı kaldırılır. |

:::

Kurum eşleştirmesi ekranda görünen ada göre değil, kararlı bir kimliğe göre yapılır. LDAP önce kararlı bir öznitelik varsa ona, yoksa ayırt edici ada (DN) göre eşleşir; OIDC grup yoluna göre eşleşir; Veri Tabanı ve API kurum yoluna göre eşleşir. Yolu farklı iki kaynak grubu aynı görünen adı taşısa bile iki ayrı kurum olarak oluşur — bu beklenen bir durumdur ve uyarı üretmez. Çözümlenen yol, elle oluşturulmuş **mevcut** bir kurumla aynıysa senkronizasyon kullanıcıları o mevcut kayda bağlar, kopya bir kurum oluşturmaz — bkz. [Senkronizasyon Davranışları](/tr/admin/identity-management/identity-providers/ldap-active-directory#senkronizasyon-davranışları).
...
- Senkronizasyonun kaynaktan kalktığı için pasifleştirdiği bir tüketici, kaynakta yeniden göründüğünde kendiliğinden yeniden etkinleştirilir — bu yalnız LDAP'ta değil her kaynakta geçerlidir. Elle pasifleştirilen bir tüketiciye bu davranış hiç dokunmaz. Bir kaynak hesabın hâlâ pasif olduğunu açıkça bildiriyorsa bu cevap kazanır ve tüketici pasif kalır; yalnızca hesap durumu hakkında hiçbir şey söylemeyen bir kaynak, önceki otomatik pasifleştirmenin kaldırılmasına izin verir.
