...
Tek fark varsayılandadır: bir uygulamaya ait istemci **Tümünü reddet** modunda başlar. Yetkilendirme alt sekmesinde bir hedef adlandırılana kadar hiçbir şeye erişemez — ve yalnızca uygulamanın **Erişim** sekmesinde zaten sahip olduğu bir hedef adlandırılabilir.

:::warning

Bir API istemcisi artık token alabilir ve bu token sorgulanıp (introspection) iptal edilebilir; ancak token'ı bir API Proxy'ye göndermek **henüz desteklenmez** — gateway böyle bir isteği `POLICY_AUTH_CREDENTIAL_PROXYNOTALLOWED` ile reddeder. Bugün nelerin çalıştığı için [API İstemcisi ile Token Alma](/tr/admin/identity-management/kimlik-bilgileri#api-istemcisi-ile-token-alma) bölümüne bakın. API istemcileri için yetkilendirme sonraki bir sürümde gelene kadar canlı trafik mevcut kimlik bilgisi mekanizması üzerinden doğrulanmaya devam eder.
