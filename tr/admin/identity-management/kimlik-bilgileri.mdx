...
| Kurum _(Organization)_ | Tüketicinin bağlı olduğu kurum/organizasyon bilgisi isteğe bağlı olarak seçilebilir. Raporlamanın ötesinde, bu bağlantı bu tüketici için üretilen token'lara **kurum seviyesi metadata** olarak miras kalır — bkz. [Kurum Seviyesi Metadata](#kurum-seviyesi-metadata). |
...
Bir kimlik sağlayıcıdan [senkronizasyon](/tr/admin/identity-management/identity-providers/credential-sync) ile gelen kayıtlara ve CSV dosyasından toplu olarak içe aktarılan kayıtlara bu sürümde tüketici anahtarı atanmaz; bu kayıtlarda alan boş kalır. Bu ekrandan ve Management API üzerinden oluşturulan tüketicilerde anahtar her zaman bulunur.
...
**Taşıma** bir kaydın yalnızca erişim eksenini taşır. Limitler (kota, hız sınırı, AI token bütçesi) yerinde kalır ve eski erişim listelerine dokunulmaz. Erişim kuralı çözümlenemeyen bir kayıt — örneğin artık var olmayan bir API Proxy'yi gösteren bir satır — yine de taşınır: yalnız o kural atlanır ve kaydın satırında raporlanır; Consumer'a bağlı istemci eski kuralların kendisini uygulamaya devam eder. Deneme (dry-run) koşumu hiçbir şey yazmaz.
...
Taşınmış bir istemcinin [AI Model Erişimi](#ai-model-erisimi) kısıtı ve AI token bütçesi değişmeden çalışmaya devam eder: bunlar istemciye kopyalanmaz, taşımanın koruduğu bağ üzerinden eski tüketici kaydına bağlı kalmaya devam eder. Hiç taşınmamış bir istemcinin ise böyle bir eski-kapsamlı limiti hiç yoktur — bu sürümde yalnızca AI Gateway'in kendi kiracı ve kurulum geneli bütçelerine tabidir.
