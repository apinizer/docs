...
Bu sürümde mod, bir secret üreten ya da döndüren management API isteğindeki bir alan üzerinden seçilir; Client Secrets ekranının kendisi henüz bu seçimi sunmaz, bu yüzden ekrandan üretilen bir secret her zaman kurulumun varsayılanını alır. Bu varsayılan ve nasıl değiştirileceği için bkz. [API İstemcisi Secret Saklama Ayarları](/tr/admin/system-settings/api-istemcisi-secret-saklama-ayarlari).
...
#### Göç Etmiş Kayıtlarda Legacy Alanlar Salt Okunur {#gocmus-kayitlarda-legacy-alanlar-salt-okunur}

Bir tüketicinin istemci kimliği [Secret Taşıma](/tr/admin/identity-management/api-client-tasima#secret-tasima) ile bir API istemcisine — secret materyali dahil olacak şekilde — taşındıktan sonra, o tüketicinin **Kullanıcı Adı** ve **Parola** alanları hem görüntüleme hem düzenleme ekranında **salt okunur** hale gelir. Ekranın üstünde, istemci kimliğinin taşındığı API istemcisini adıyla belirten bir bilgi banner'ı görüntülenir; parola artık yalnızca ilgili API istemcisinin **Client Secrets** alt sekmesinden [döndürülebilir (rotate)](#client-secret-yalnizca-bir-kez-gosterilir).

Kural sunucu tarafında da uygulanır: göç etmiş bir kayıtta kullanıcı adı ya da parolayı fiilen **değiştiren** bir kaydetme isteği `credential.legacyIdentityMigrated` hata anahtarıyla 400 ile reddedilir. Ekrandaki değeri değiştirmeden kaydetmek — ya da parola alanını boş bırakmak — bir değişiklik sayılmaz ve normal şekilde geçer; tüketicinin diğer tüm alanları (roller, IP listesi, erişim denetimi, token ayarları, metadata vb.) bu göç sonrasında da her zamanki gibi düzenlenebilir kalır. Henüz taşınmamış kayıtlarda kullanıcı adı ve parola eskisi gibi düzenlenebilir.

:::note

Bu, verilmiş kimlik doğrulamayı **askıya almaz**. Göç sonrasında da eski kullanıcı adı/parola üzerinden okuma (dual-read) yolu bu sürümde bir emniyet ağı olarak çalışmaya devam eder ve daha önce verilmiş token'lar kendi süreleri dolana kadar geçerliliğini korur — salt okunur kısıt yalnızca yönetim ekranındaki elle değişikliği hedefler.
