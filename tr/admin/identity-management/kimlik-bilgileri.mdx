...
### Sahip ve İstemci Yaşam Döngüsü (Canlı Trafikte) {#sahip-ve-istemci-yasam-dongusu-canli-trafikte}
...
- **Sahip** o an etkin olmalıdır. Bir **kimlik bilgisine** ait istemci, o kimlik bilgisi devre dışıyken reddedilir; bir **uygulamaya** ait istemci, uygulama [askıya alınmışken](/tr/admin/identity-management/portal-applications#uygulama-silme) reddedilir. Bu durumdaki ret `POLICY_AUTH_CREDENTIAL_PROXYNOTALLOWED` ile düşer — sahip canlı değilken istemcinin kendi yetkilendirme modu hiç değerlendirilmez. Bu sahip kontrolü yalnızca canlı API Proxy çağrısı için çalışır; token'ın bizzat alınmasını engellemez.

Sahibi devre dışı bırakmak ya da askıya almak, bu yüzden altındaki tüm istemcileri tek tek dokunmadan aynı anda kesmenin de en hızlı yoludur.
