...
Her kimlik bilgisi (credential) üzerinde arbitrary anahtar/değer metadata girişleri tanımlanabilir. Bu girişler credential üzerinde saklanır, opsiyonel olarak şifrelenir ve şu tüketicilere aktarılabilir:

- **JWT token payload** — *JWT'ye Ekle* seçeneği aktifse, giriş token oluşturma sırasında JWT body'sine custom claim olarak eklenir.
...
- **Script politikaları** (Groovy / JavaScript) — `credentialMap.get(username).getMetadata()` ile erişilebilir. Script context'i tüm metadata'yı görür (include flag'leri filtre uygulamaz).
...
| Hassas (Secret) | Aktif olduğunda değer şifrelenir ve API response'larında maskelenir. Her giriş için bağımsız ayar. |
| JWT'ye Ekle (Include in JWT) | Aktif olduğunda giriş JWT claim'i olarak token'a eklenir. OAuth-opaque grant tiplerinde devre dışıdır. |
...
- **OAuth response (dinamik)**: *Genel Ayarlar → Token Yönetim Ayarları* altında özelleştirilmiş tüm alan isimleri (örn. custom `accessTokenFieldName`) da rezerve sayılır.

### Kurum (Organization) Seviyesi Metadata

`CredentialOrganization` girişleri de aynı metadata yapısını destekler. Token oluşturma sırasında önce kurum metadata'sı uygulanır, ardından credential metadata'sı aynı anahtar üzerinde override eder (child wins). Script policy tarafında `credentialMap.get(username).getMetadata()` merged map'i döner; her iki katmanı ayrı ayrı incelemek için `getCredentialMetadata()` ve `getOrganizationMetadata()` kullanılır.
