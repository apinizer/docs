...
<img
  ...
  alt="CSV ile Toplu Tüketici Aktarımı"
  width="800"
/>
...
Tüketiciler, kullanıcı adının yanı sıra bir **tüketici anahtarı** taşır: kayıt oluşturulurken sunucu tarafından atanan kalıcı ve okunabilir kimliktir. Oluşturma ekranında bir alan olarak yer almaz ve değiştirilemez.
...
Anahtar tek bir tüketiciyi tanımladığı için ikinci bir kayda taşınmaz: **Kopyala (Duplicate)** ile üretilen ya da **Yeni Olarak İçe Aktar (Import as New)** ile getirilen tüketici kendine ait yeni bir anahtar alır. Bu sürümden önce var olan kayıtlara anahtar bir kereye mahsus, sürüm yükseltmesi sırasında atanır.
...
- hedef kurum çözümlenemiyorsa, tüketiciyle aynı projeye ait değilse ya da başka bir kurumla birleştirilmişse;
...
Bugün makine istemcisi olarak kullanılan tüketiciler raporlanabilir ve ardından yeni kayıtlara taşınabilir. Aşağıda envanterin ve taşımanın neyi kapsadığı anlatılır; bu envanter ve taşıma işlemlerini fiilen yürüttüğünüz ekran (karar rozetleri, deneme koşumu ve uygulama akışı, gereken yetkiler) için bkz. [API İstemci Geçişi](/tr/admin/identity-management/api-client-tasima).
...
**Taşıma** bir kaydın yalnızca erişim eksenini taşır. Limitler (kota, hız sınırı, AI token bütçesi — üçü de artık typed Limit Planı atamalarıdır) yerinde kalır ve eski erişim listelerine dokunulmaz. Erişim kuralı çözümlenemeyen bir kayıt — örneğin artık var olmayan bir API Proxy'yi gösteren bir satır — yine de taşınır: yalnız o kural atlanır ve kaydın satırında raporlanır; Consumer'a bağlı istemci eski kuralların kendisini uygulamaya devam eder. Deneme (dry-run) koşumu hiçbir şey yazmaz.
...
Yerel bir parolayla doğrulanan her kayıt taşınır. Dış bir kimlik kaynağı üzerinden doğrulanan, dış bir kimlik sağlayıcının claim'iyle eşleşen ya da saklı bir parolası olmayan bir kayıt ise Consumer olarak kalır — bunun için bir API istemcisi türetilmez — ve değişmeden çalışmaya devam eder.

:::

#### Göç Etmiş Kayıtlarda Kullanıcı Adı Salt Okunur Olur; Parola Değişikliği İstemci Secret'ını Rotate Eder {#gocmus-kayitlarda-legacy-alanlar-salt-okunur}

Bir tüketicinin istemci kimliği [Secret Taşıma](/tr/admin/identity-management/api-client-tasima#secret-tasima) ile bir API istemcisine — secret materyali dahil olacak şekilde — taşındıktan sonra, o tüketicinin **Kullanıcı Adı** alanı hem görüntüleme hem düzenleme ekranında **salt okunur** hale gelir — çünkü bu alan artık her çağıranın zaten elinde tuttuğu client id'nin kendisidir. Ekranın üstünde, istemci kimliğinin taşındığı API istemcisini adıyla belirten bir bilgi banner'ı görüntülenir. **Parola** alanı ise düzenlenebilir kalır ve buraya girilip kaydedilen yeni bir değer iki şeyi birden yapar: eski kimlik bilgisinin kendi parolasını eskisi gibi günceller ve **taşınmış istemcinin secret'ını aynı değere rotate eder** — tıpkı **Client Secrets** alt sekmesinden başlatılan bir rotasyon gibi, önceki secret grace süresi boyunca doğrulamaya devam ederken yeni secret devreye girer. Yeni kuşak, istemcinin mevcut kuşağının saklama modunu korur (Şifreli bir kuşak, kurulum varsayılanı ne olursa olsun Şifreli kalır); `${...}` ya da `#{...}` referansı içeren bir değer ise düz bir secret olarak değil, ağ geçidinin her istekte çözdüğü bir referans olarak taşınır. Böylece iki depo her zaman aynı değeri taşır; iptal edilmiş bir istemci rotasyon almaz, yalnız eski parola değişir.

Tüketicinin diğer tüm alanları düzenlenebilir kalır ve kayıt normal şekilde kaydedilir. Ekrandaki mevcut kullanıcı adını değiştirmeden kaydetmek — ya da parola alanını boş bırakmak — hiçbir şeyi değiştirmez ve normal şekilde geçer. Kural sunucu tarafında da uygulanır: göç etmiş bir kayıtta kullanıcı adını fiilen **değiştiren** bir kaydetme isteği, `credential.legacyIdentityMigrated` hata anahtarıyla 400 ile reddedilir — bu, ekranın kendi göstermediği nadir durumu da yakalar. Henüz taşınmamış kayıtlarda kullanıcı adı ve parola eskisi gibi düzenlenebilir.
...
<img
  ...
  alt="Tüketici Erişim Denetimi Ayarları"
  width="1000"
  height="292"
/>
