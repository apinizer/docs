...
### Bilinen Sınır: Durumun Ortamlara Yayılması {#durum-yayilimi}

Askıya alma, devam ettirme ve iptal önce yönetim kayıtlarına işlenir; ardından yeni durum, gateway ortamlarına eşzamanlı olarak itilir. Bu itme — örneğin bir ortam o an erişilemezse — başarısız olabilir. Böyle bir durumda:

- İşlemin kendisi **geri alınmaz**: durum değişikliği kayıtlıdır ve yanıt başarılıdır; ancak Management API yanıtındaki `runtimeDeployment` alanı yayılımı **DEGRADED** olarak raporlar ve ekran bunu bir uyarıyla gösterir. Hiçbir yüzey bu durumda işlemi ortamlara "tamamen ulaştı" olarak göstermez.
- Değişikliğin etkilenen ortama ulaşması gecikir; durum, o ortam yapılandırmayı yeniden çektiği ilk anda (örneğin worker yeniden başladığında) eşitlenir.
- İptalin en kritik ayağı bu gecikmeden etkilenmez: açık (opaque) token'lar doğrudan yönetim veritabanından silinir ve yayılımı geciken bir ortamda bile silinmiş bir token'la yapılan doğrulama başarılı olmaz.
