...
| Aktif  | Kimlik bilgisinin aktif (kullanılabilir) olup olmadığını belirtir. Seçilmiş ise aktiftir. |
...
| Açıklama  | Kimlik bilgisi hakkında açıklama girilmek istenirse bu alan doldurulabilir. |
...
## Kimlik Bilgisini Görüntüleme (View)

Kimlik bilgileri görüntülemek için iki yöntem bulunmaktadır:

<CardGroup cols={2}>
    <Card title="Yöntem 1: Liste Ekranından" icon="list">
        - Kimlik bilgisi listesinde görüntülemek istediğiniz kimlik bilgisinin **Kullanıcı Adı** alanına tıklayın.
        - View ekranına yönlendirilirsiniz.
    </Card>

    <Card title="Yöntem 2: Menüden" icon="ellipsis-vertical">
        - Kimlik bilgisi listesinde satır sonundaki **⋮** menüsünden **View** seçeneğini tıklayın.
        - View ekranına yönlendirilirsiniz.
    </Card>
</CardGroup>
...
<img
    src="/images/yonetici/kimlik-bilgisi-view.png"
    alt="Kimlik Bilgisi View Ekranı"
    width="1000"
    className="mr-auto"
/>
...
<CardGroup cols={2}>
    <Card title="Edit (Düzenle)" icon="pencil">
        **[Edit]** butonuna tıklayarak kimlik bilgisini düzenleme ekranına yönlendirilirsiniz. Tüm parametreleri güncelleyebilirsiniz.
    </Card>

    <Card title="Back to List (Listeye Dön)" icon="list">
        **[Back to List]** butonuna tıklayarak kimlik bilgileri listesine geri dönebilirsiniz.
    </Card>
</CardGroup>

### View Ekranında Görüntülenen Bilgiler

View ekranı beş farklı sekmeden oluşmaktadır:

<Tabs>
    <Tab title="Bilgi (Info)">

        Genel kimlik bilgilerini görüntüler:

        <CardGroup cols={3}>
            <Card title="Temel Bilgiler" icon="user">
                - **Harici (External):** Harici sistemden gelen kimlik bilgisi olup olmadığı
                - **Kullanıcı Adı/API Anahtarı:** Kimlik bilgisinin adı (username veya API key)
                - **Parola/API Gizli Anahtarı:** Gizlenmiş parola bilgisi
                - **E-posta:** İletişim e-posta adresi
                - **Tam Ad:** Kullanıcının tam adı
                - **Aktif:** Kimlik bilgisinin aktif durumu
                - **Bitiş Tarihi:** Kimlik bilgisinin geçerlilik süresi (varsa)
            </Card>

            <Card title="Proje ve Organizasyon" icon="building">
                - **Proje:** Kimlik bilgisinin bağlı olduğu proje (admin için görünür)
                - **Hesap:** Kimlik bilgisinin bağlı olduğu hesap (varsa)
                - **Kurum:** Kimlik bilgisinin bağlı olduğu kurum/organizasyon
            </Card>

            <Card title="Roller ve Erişim" icon="shield">
                - **Roller:** Kimlik bilgisine atanmış roller
                - **IP Listesi:** İzin verilen IP adresleri
                - **Coğrafi Konum Verisi:** İzin verilen ülkeler ve şehirler
                - **Açıklama:** Kimlik bilgisi hakkında açıklama
                - **Ortam Bazlı Ayarlar:** Ortama özel yapılandırmalar
            </Card>
        </CardGroup>

    </Tab>

    <Tab title="API Proxy ACL">

        API proxy erişim izinlerini görüntüler:

        - **API Proxy Adı:** Erişim verilen API proxy'in adı ve göreceli yolu
        - **Proje Adı:** API proxy'in bağlı olduğu proje (admin için görünür)
        - **İzin Verilmeyen Metodlar:** API proxy'de yasaklanan HTTP metodları
        - **İzin Verilmeyen Ortamlar:** API proxy'de erişimi yasaklanan ortamlar
        - **Bitiş Tarihi:** API proxy erişim izninin geçerlilik süresi

    </Tab>

    <Tab title="API Proxy Grup ACL">

        API proxy grubu erişim izinlerini görüntüler:

        - **API Proxy Grup Adı:** Erişim verilen API proxy grubu adı ve göreceli yolu
        - **Proje Adı:** API proxy grubu'nun bağlı olduğu proje (admin için görünür)
        - **İzin Verilmeyen API Proxy'ler:** Grubun içindeki yasaklanan API proxy'ler
        - **İzin Verilmeyen Ortamlar:** API proxy grubunda erişimi yasaklanan ortamlar
        - **Bitiş Tarihi:** API proxy grubu erişim izninin geçerlilik süresi

    </Tab>

    <Tab title="Token Ayarları">

        Token üretim ve yönetim ayarlarını görüntüler:

        <CardGroup cols={2}>
            <Card title="Genel Ayarlar" icon="cog">
                - **Onay Türü:** Token üretimi için kullanılan onay türü (Password/Client Credentials)
                - **Kimlik/Güven/Grup Servisi:** Password onay türü için kullanılan servis
                - **JWT Yeniden Üretim API Servisi:** JWT token yeniden üretimi için kullanılan API
                - **Önceki Token'ı Sil:** Yeni token üretildiğinde eski token'ın silinmesi
                - **Token Ölümsüz Olsun:** Token'ın süresiz geçerli olması
                - **JWT İmza Algoritması:** Token imzalama algoritması
                - **URL Parametrelerine İzin Ver:** Token isteğinde URL parametrelerinin kullanımı
            </Card>

            <Card title="Token Geçerlilik Süreleri" icon="clock">
                - **Token Geçerlilik Süresi:** Ana token'ın yaşam süresi
                - **Yenileme Token'ı İzin Ver:** Token yenileme özelliği
                - **Yenileme Token Sayısı:** Token'ın kaç kez yenilenebileceği
                - **Yenilenmiş Token Geçerlilik Süresi:** Yenilenen token'ın yaşam süresi
            </Card>
        </CardGroup>

    </Tab>

    <Tab title="Gizli Anahtarlar">

        Gizli anahtar ve sertifika bilgilerini görüntüler:

        <CardGroup cols={2}>
            <Card title="Sertifikalar ve Anahtarlar" icon="key">
                - **Sertifika:** Kimlik bilgisiyle ilişkilendirilmiş sertifika
                - **Genel Anahtar:** Genel anahtar bilgisi
                - **Özel Anahtar:** Özel anahtar bilgisi
                - **Gizli Anahtar:** Gizli anahtar bilgisi
                - **Anahtar Deposu:** Güven deposu bilgisi
                - **Güven Deposu:** Güven deposu bilgisi
            </Card>

            <Card title="JWK Ayarları" icon="certificate">
                - **JWK Aktif:** JWK ayarlarının aktif olup olmadığı
                - **Doğrulama ve İmzalama için JWK:** JOSE işlemlerinde kullanılacak JWK'lar
                - **Şifreleme ve Şifre Çözme için JWK:** Şifreleme işlemlerinde kullanılacak JWK'lar
                - JWK detayları: Ad, açıklama, tip, algoritma bilgileri
            </Card>
        </CardGroup>
...
<Info>
    View ekranından kimlik bilgilerinin tüm detaylarını tablar halinde inceleyebilir ve **[Edit]** butonuna tıklayarak düzenleme işlemlerini başlatabilirsiniz.
</Info>