---
title: 'Takımlar'
description: 'Kullanıcıları bir projeye üye olarak eklerken her proje için tekrar tekrar kullanıcı rollerini tanımlamamak için Takım özelliğinden faydalanılır.'
---

# Takımlar

Kullanıcıları bir projeye üye olarak eklerken her proje için tekrar tekrar kullanıcı rollerini tanımlamamak için Takım özelliğinden faydalanılır.

Kullanıcıların topluca yönetimi için "Takım" oluşturulur ve kullanıcı rolleri bu takım üzerinde tanımlanır.

Bir takımı herhangi bir projeye üye olarak nasıl ekleyebileceğinizi öğrenmek için [tıklayınız](/tr/admin/proje-yonetimi/project-members-roles).

## Yeni Bir Takım Oluşturma

**Takım oluşturma ayarlarını içeren görsele aşağıda yer verilmiştir:**

![Takım Oluşturma](/images/team-creation.png)

Takım oluşturma konfigürasyonu için kullanılan alanlar aşağıdaki tabloda görülmektedir.

| Alan | Açıklama |
|------|----------|
| Ad (Name) | Oluşturulan takım için takım adı bilgisidir. |
| Açıklama (Description) | Oluşturulan takım ile ilgili yönetimi kolaylaştırmak için açıklama yazılabilir. |
| Üyeler (Members) | Üyeler, Apinizer'da tanımlı bulunan kullanıcılardır. Takım üyeleri, kullanıcılar arasında ada göre arama işlemi ile eklenebilir. |
| Rol (Role) | Roller, takımdaki üyelerin yetkisini belirler. Takımdaki üyeler, birbirlerinden farklı rollere de sahip olabilirler; fakat özelliğin önerilen kullanım şekli aynı role sahip üyelerin gruplandırılmasıdır. Rol, ilgili role ait checkbox seçilerek eklenir. Tüm rolleri eklemek için rol arama alanının yanındaki checkbox seçilir. |

:::warning
Silinen takım, etkin olduğu projelerden de sistem tarafından silinir.
:::
