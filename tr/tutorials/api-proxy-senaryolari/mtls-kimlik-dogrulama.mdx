...
mTLS Ayarları hakkında detaylı bilgi için [tıklayınız](/tr/develop/politikalar/mTLS).

:::

## Kimlik Bilgisi'nin mTLS Ayarları

Kimlik bilgisinin kullanıcı adı, politikadaki **Kimlik Kaynağı** seçimine göre belirlenir. Bu iki ayarın birbiriyle uyumlu olması gerekir; uyumsuz olmaları durumunda sertifika geçerli olsa bile istek reddedilir.

| Politikadaki Kimlik Kaynağı | Kimlik bilgisinin kullanıcı adı | Bu senaryodaki karşılığı |
|-----------------------------|--------------------------------|--------------------------|
| **Sertifikayı Veren (Issuer CN)** | Sertifikayı imzalayan otoritenin adı | `MyRootCA` |
| **Sertifika Sahibi (Subject CN)** | İstemci sertifikasının kendi adı | `client` |
| **Önce Sahip, Sonra Veren** (varsayılan) | Her ikisi de kullanılabilir; önce sahip adı, bulunamazsa veren adı aranır | `client` veya `MyRootCA` |

Bu senaryoda her iki adlandırma da varsayılan ayarla çalışır. Otoritenin verdiği **tüm** istemcileri tek bir kimlik altında toplamak istiyorsanız `MyRootCA` adını kullanın; her istemciyi ayrı ayrı tanımak ve gerektiğinde tek bir istemcinin erişimini bağımsız olarak kapatmak istiyorsanız `client` adını kullanıp politikada **Sertifika Sahibi** seçeneğini seçin.

:::note

Kimlik Kaynağı seçeneği ve anlamları hakkında detaylı bilgi için [mTLS Kimlik Doğrulama](/tr/develop/politikalar/mTLS) politika sayfasındaki Kimlik Kaynağı bölümüne bakabilirsiniz.
