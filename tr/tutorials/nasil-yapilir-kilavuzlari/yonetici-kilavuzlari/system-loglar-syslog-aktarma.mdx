---
title: "Sistem Loglarının Syslog ile Aktarılması"
description: "Apinizer'ın uygulama ve sistem loglarını Syslog bağlantısı üzerinden harici bir log toplayıcısına (ELK, Splunk, Graylog vb.) gönderme adımları. System Settings içinden Syslog Connector seçimi ve yapılandırması."
---


Administration > Bağlantı Konfigürasyonlar > Syslog sayfasına gidilir ve bir Syslog bağlantı tanımı oluşturulur. [Detalı bilgi için Syslog Bağlantı Yönetimi](/tr/integrations/baglanti-yonetimi/syslog) sayfasına bakabilirsiniz.

:::info

Uygulama, token, denetim ve giriş loglarının alıcıları **[SIEM ve Log Yönlendirme](/tr/admin/system-settings/siem-log-yonlendirme)** sayfasından yönetilir; her alıcı orada bir **hedef** olarak tanımlanır ve ilgili hat (`APPLICATION`, `TOKEN`, `AUDIT`, `SESSION`) hedefin kurallarında açılır. Olay tipi, sonuç, seviye ve kapsam filtreleri de aynı sayfadadır.

Bu sayfadaki alan listeleri ve syslog örnekleri `LEGACY_RAW` yük profilinin çıktısıdır: biçim **dondurulmuştur**, sonradan eklenen alanlar bu çıktıya girmez ve mevcut ayrıştırıcılarınız değişmeden çalışır. Kanonik zarf, gizlilik profili ve boyut politikası isteniyorsa hedef `APINIZER_JSON_V2` profiline geçirilir; örnek zarf yeni sayfadadır.

:::


## Uygulama Loglarının Syslog ile Aktarılması

Yönetim > SIEM ve Log Yönlendirme sayfasında bir hedef tanımlanır, konnektör olarak Syslog bağlantısı seçilir ve hedefin kurallarında `APPLICATION` hattı açılır.

<img src="/images/tutorials/nasil-yapilir-kilavuzlari/yonetici-kilavuzlari/image-2025-6-13_16-13-19.png" alt="Uygulama Logları Syslog Seçimi" width="1000" />

<img src="/images/tutorials/nasil-yapilir-kilavuzlari/yonetici-kilavuzlari/image-2025-6-13_16-13-57.png" alt="Syslog Connector Seçimi" width="1000" />

Daha önce oluşturulmuş olan Syslog bağlantı tanımı seçilir.

<img src="/images/tutorials/nasil-yapilir-kilavuzlari/yonetici-kilavuzlari/image-2025-6-13_16-14-40.png" alt="Syslog Bağlantı Tanımı Seçimi" width="1000" />
...
Yönetim > SIEM ve Log Yönlendirme sayfasında bir hedef tanımlanır, konnektör olarak Syslog bağlantısı seçilir ve hedefin kurallarında `TOKEN` hattı açılır.

<img src="/images/tutorials/nasil-yapilir-kilavuzlari/yonetici-kilavuzlari/image-2025-6-13_16-30-57.png" alt="Token Logları Syslog Seçimi" width="1000" />
...
- **applicationName**: İsteğin ilgili olduğu uygulamanın adıf (örneğin, "Swagger Petstore Documentation").
...
Yönetim > SIEM ve Log Yönlendirme sayfasında bir hedef tanımlanır, konnektör olarak Syslog bağlantısı seçilir ve hedefin kurallarında `AUDIT` hattı açılır.

<img src="/images/tutorials/nasil-yapilir-kilavuzlari/yonetici-kilavuzlari/image-2025-6-13_16-34-59.png" alt="Audit Logları Syslog Seçimi" width="1000" />
...
Bu hedef üzerinden iletilen her audit log mesajı ayrıca aşağıdaki alanları taşır:

- **eventType**: Her zaman bulunur. Yukarıdaki değişiklik kaydı için `ENTITY`, aşağıda açıklanan iki olay türü için `MANAGER_REQUEST` / `ACCESS_DENIED`.
...
### Yönetim Konsolu İsteği ve Erişim Reddi Olayları

Administration > System Settings > General Settings altındaki **Audit Log — Yönetim Konsolu isteklerinin denetim kaydı** ayarının açılması, Yönetim Konsolu ekranlarına, Yönetim API'lerine veya APIops'a ulaşan her yazma işlemi (HTTP POST) için aynı hedef üzerinden ayrıca bir `MANAGER_REQUEST` denetim olayı gönderir. Bu ayar Genel Ayarlar ekranında kalmaya devam eder; olayın hedefe gidip gitmeyeceği ise SIEM ve Log Yönlendirme sayfasındaki `AUDIT` kuralıyla belirlenir. Reddedilen istekler bu ayardan bağımsız olarak her zaman `ACCESS_DENIED` olayı olarak gönderilir. Bu iki olay türü için `objectName` ve `referenceObjectJson`, bir varlığın verisi yerine işlemin yolunu, HTTP metodunu, sonucunu ve süresini taşır — istek gövdesi, sorgu parametreleri ve kimlik doğrulama başlıkları hiçbir zaman bunlara dahil edilmez. Tam alan referansı ve JSON örnekleri için [Denetim Kayıtları](/tr/admin/audit-compliance/denetim-kayitlari) sayfasına bakabilirsiniz.

:::tip

Denetim olaylarından yalnızca bir bölümünü SIEM'e göndermek, gönderilen olay boyutunu sınırlamak veya kimlik alanlarını maskelemek için [SIEM ve Log Yönlendirme](/tr/admin/system-settings/siem-log-yonlendirme) sayfasındaki hat kurallarını kullanabilirsiniz.
