---
title: "Apinizer Worker Diagnostics İzleme ve Raporlama"
...
- **Bir toplayıcı shell script (`monitor.sh`)** — verilen ortamın Worker Diagnostics API'sini (`/all` endpoint'i) periyodik olarak okur, satır başına bir JSON kaydı olacak şekilde JSONL dosyasına ekler. Aynı script, ortam adı + Worker URL + Env ID argümanları ile birden fazla ortam için paralel olarak çalıştırılabilir.
...
Dosya adı deseni: `<ORTAM_ADI>-worker-diagnostics-YYYYMMDD.jsonl`
...
<img
  ...
  alt="Apinizer Worker Diagnostics Raporu — Genel Bakış Sekmesi"
  style={{ width: "600px" }}
/>
...
**`HATA: <ortam>-worker-diagnostics-YYYYMMDD.jsonl yok. monitor.sh <ortam> ... çalışıyor mu?`**
