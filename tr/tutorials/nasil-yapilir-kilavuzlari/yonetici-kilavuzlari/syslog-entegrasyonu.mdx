...
- **Giriş (Login) Denetimi**: Apinizer management uygulaması arayüzündeki giriş, çıkış, token yenileme ve hesap kilitleme olaylarının log kayıtlarıdır. Genel Ayarlar'da **Giriş Logu** alıcısı olarak seçilen konnektörlere (Syslog dahil) gönderilir.
...
- **Kullanıcı Denetim Logları**: Bu tanımda ayrı bir "kullanıcı denetimi" logu harici olarak bulunmamaktadır. Kullanıcı odaklı denetimler **Giriş (Login) Denetimi** (Giriş Logu alıcılarına gönderilir) ve **Test Konsol Kullanımı Denetimleri** (harici bir uygulamaya gönderilmez) altında ele alınır.
