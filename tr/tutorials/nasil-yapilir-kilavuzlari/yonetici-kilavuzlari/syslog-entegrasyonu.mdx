---
title: "Apinizer Syslog Entegrasyonu"
description: "Apinizer'da syslog entegrasyonu yapabilir, trafik loglarını ve sistem loglarını syslog sunucusuna aktarabilirsiniz."
---
...
- **Giriş (Login) Denetimi**: Apinizer management uygulaması arayüzündeki giriş, çıkış, token yenileme ve hesap kilitleme olaylarının log kayıtlarıdır. SIEM ve Log Yönlendirme sayfasındaki `SESSION` hattı için tanımlanan hedeflere (Syslog dahil) gönderilir.

:::info

Denetim, Giriş, Token ve Uygulama loglarının alıcıları **[SIEM ve Log Yönlendirme](/tr/admin/system-settings/siem-log-yonlendirme)** sayfasından yönetilir; daha önce Genel Ayarlar ekranında bulunan dört alıcı listesi oraya taşınmıştır. Olay tipi, sonuç ve kapsam filtreleri de hat bazında oradan tanımlanır.

Bu sayfada anlatılan çıktı biçimi `LEGACY_RAW` yük profiline karşılık gelir ve alan listesi dondurulmuştur. Kanonik zarf için hedef `APINIZER_JSON_V2` profiline geçirilir; örnek zarf yeni sayfada yer alır.
...
- **Kullanıcı Denetim Logları**: Bu tanımda ayrı bir "kullanıcı denetimi" logu harici olarak bulunmamaktadır. Kullanıcı odaklı denetimler **Giriş (Login) Denetimi** (`SESSION` hattının hedeflerine gönderilir) ve **Test Konsol Kullanımı Denetimleri** (harici bir uygulamaya gönderilmez) altında ele alınır.

## Api Trafik ve Sistem Loglarının Syslog'a Aktarılması ve Log Template'leri

İlk olarak Syslog bağlantı ayarlarının yapılması gerekmektedir bu bağlantı ayarları için [Syslog Bağlantı Yönetimi](/tr/integrations/baglanti-yonetimi/syslog) sayfasına göz atabilirsiniz.

Apinizer trafik loglarının syslog'a aktarım ayarları için [Trafik Loglarını Syslog'a Aktarma](/tr/operations/yonetici-kilavuzlari/traffic-loglar-syslog-aktarma) sayfasını ziyaret edebilirsiniz.

Apinizer sistem loglarının syslog'a aktarım ayarları için [Sistem Loglarını Syslog'a Aktarma](/tr/operations/yonetici-kilavuzlari/system-loglar-syslog-aktarma) sayfasını ziyaret edebilirsiniz.

Hedeflerin tanımlanması, hat başına filtre kuralları, gizlilik profili ve boyut politikası için [SIEM ve Log Yönlendirme](/tr/admin/system-settings/siem-log-yonlendirme) sayfasına göz atabilirsiniz.
