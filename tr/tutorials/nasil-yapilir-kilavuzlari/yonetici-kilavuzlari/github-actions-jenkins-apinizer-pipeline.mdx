...
<Info>
  `GITHUB_TOKEN`, GitHub Actions tarafından otomatik olarak oluşturulan geçici bir token’dır. Bu nedenle Secrets and Variables bölümüne manuel olarak eklenmesine gerek yoktur. Workflow içinde kullanılan `anothrNick/github-tag-action`, repository’ye versiyon tag’lerini push edebilmek için bu token’ı otomatik olarak kullanır.
</Info>
...
1. **Kubernetes Deployment**: Yeni Docker image'ı Kubernetes'e deploy eder
...
|----------|----------|-------------|
| `NAMESPACE` | Kubernetes namespace | `my-app-namespace` |
| `DEPLOYMENT_NAME` | Kubernetes deployment adı | `my-api-deployment` |
| `CONTAINER_NAME` | Kubernetes container adı | `my-api-container` |
| `DOCKER_IMAGE` | Docker image adı (kullanıcı adı/image) | `myusername/my-api` |
| `KUBECTL_PATH` | kubectl binary yolu | `/usr/local/bin/kubectl` |
| `API_BACKEND_URL` | Deploy edilen uygulamanın base URL'i | `https://api.example.com` |
| `API_HEALTH_ENDPOINT` | Uygulamanın health check endpoint'i | `https://api.example.com/health` |
| `API_SPEC_URL` | OpenAPI specification URL'i | `https://api.example.com/api/v1/openapi.json` |
| `APINIZER_BASE_URL` | Apinizer platform URL'i | `https://apinizer.example.com` |
| `APINIZER_GATEWAY_URL` | Apinizer gateway URL'i | `https://gateway.example.com` |
| `APINIZER_PROJECT` | Apinizer proje adı | `my-project` |
| `APINIZER_ENVIRONMENT` | Deploy edilecek environment | `dev` |
| `APINIZER_PROXY_NAME` | API Proxy adı | `my-api-proxy` |
| `APINIZER_PROXY_PATH` | Gateway'deki relative path | `/my-api` |
...
<Steps>
  <Step title="Deployment Mekanizması">
    Kubernetes yerine farklı bir orchestrator veya doğrudan VM deployment kullanıyorsanız, `Kubernetes Deployment` stage'ini kendi deployment metodunuza göre değiştirin.
  </Step>
