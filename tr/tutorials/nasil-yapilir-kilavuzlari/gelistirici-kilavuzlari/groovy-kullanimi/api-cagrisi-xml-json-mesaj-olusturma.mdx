---
title: API Çağrısı ile XML Veriyi Ayrıştırıp JSON Mesaj Oluşturma
---

# API Çağrısı ile XML Veriyi Ayrıştırıp JSON Mesaj Oluşturma

Bu script, XML formatındaki yanıt verisini ayrıştırarak JSON formatına dönüştürür ve başka bir API'ye POST isteği ile gönderir.
...
- XML yanıtındaki `<item>` elemanları sayılır
- Her item için XPath kullanılarak veriler çıkarılır
- Veriler JSON template'ine yerleştirilir
- Oluşturulan JSON, POST isteği ile başka bir API'ye gönderilir
- Hata durumunda hata mesajı oluşturulur, başarılı durumda SOAP yanıtı döner
