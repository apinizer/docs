...
Apinizer, container belleğine göre GC algoritmasını ve heap yüzdesini otomatik seçen bir **otomatik bellek profili sistemi** kullanır:
...
**Örnek (8 GB container, otomatik profil ile):**
* Node RAM: 8 GB
* Container RAM Limit: 8 GB
* Otomatik seçilen GC: ZGC
* Otomatik seçilen MaxRAMPercentage: %70
* JVM Kullanılacak RAM: 8 GB × 0.70 = **5.6 GB**
* İşletim Sistemi, Native Bellek ve Diğer: 8 GB × 0.30 = **2.4 GB**
...
Otomatik profil herhangi bir `JAVA_OPTS` yapılandırması gerektirmez. Geçersiz kılmak istiyorsanız, `JAVA_OPTS`'da belirli GC ve heap değerleri ayarlayabilirsiniz.

<Info>
GC seçimi, güvenlik cap'leri ve manuel override seçenekleri hakkında detaylı bilgi için [JVM Garbage Collector Ayarlama](/tr/operations/yonetici-kilavuzlari/jvm-garbage-collector-ayarlama) sayfasına bakın.
</Info>
...
Worker node'ların performansı; thread, connection pool, timeout ve async executor parametrelerine bağlıdır. Bu ayarlar API Gateway Ortam Ayarlama ekranından yapılandırılır.

Tier bazlı (Tier 1/2/3) önerilen değerler, CPU'ya göre thread hesaplama tablosu, connection pool ve timeout parametrelerinin detaylı açıklamaları için [Gateway Performans Ayarlama](/tr/operations/yonetici-kilavuzlari/gateway-performans-ayarlama) sayfasına bakın.

**JVM Parametreleri:**
* Otomatik profil önerilir (`JAVA_OPTS` gerekmez). Detaylar için [JVM Garbage Collector Ayarlama](/tr/operations/yonetici-kilavuzlari/jvm-garbage-collector-ayarlama) sayfasına bakın.
