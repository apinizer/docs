...
| Sunucu 4 | Ubuntu Server 24.04 LTS / RHEL 9.x | 10 | 12GB | 100GB | Kubernetes Worker Node 1 |
| Sunucu 5 | Ubuntu Server 24.04 LTS / RHEL 9.x | 10 | 12GB | 100GB | Kubernetes Worker Node 2 |
| Sunucu 6 | Ubuntu Server 24.04 LTS / RHEL 9.x | 10 | 12GB | 100GB | Kubernetes Worker Node 3 |
...
Worker node'larındaki sunucu kaynakları, gateway pod limitlerine (W8: 8 core / 8 GB) ek olarak Kubernetes sistem bileşenlerine (kubelet, kube-proxy, container runtime, işletim sistemi) ayrılan **~2 core / ~4 GB** rezervasyonu kapsamaktadır.
...
* **DMZ Zone**: [Worker](/tr/concepts/temel-bilesenler/api-gateway) node'ları (3 adet) bu zone'da bulunur. Load Balancer, trafiği bu üç worker node arasında dağıtarak yük dengeleme ve yüksek erişilebilirlik sağlar.

* **LAN Zone**: [Manager](/tr/concepts/temel-bilesenler/api-manager), [MongoDB](/tr/concepts/temel-kavramlar/baglantilar) Replica Set (3 node) ve [Elasticsearch](/tr/concepts/temel-bilesenler/analytics-engine) Cluster (3 node) bu zone'da bulunur.
...
Bu topoloji, 3 adet W8 worker node ile çalışır. Benchmark'a dayalı planlama değerleri:

| Metrik | Değer |
|--------|-------|
| **Worker Tier** | W8 (8 core / 8 GB RAM) × 3 node |
| **Günlük Planlama Kapasitesi** | ~30M istek/gün (3 × 10M/gün) |
| **Uptime Hedefi** | %99.9+ |
| **Kullanım Amacı** | Kritik production — yüksek erişilebilirlik |

:::note

**Rolling Update ve Kapasite:** Sıfır kesintili (rolling) güncellemeler sırasında Kubernetes bir node'u geçici olarak devre dışı bırakır. Bu topolojide güncelleme sürecinde 2 aktif node (~20M istek/gün) kalır. Tam yük altında kesintisiz güncelleme yapılmak isteniyorsa toplam worker node kapasitesinin ihtiyaç duyulan trafiğin en az 2 katı olması ya da güncelleme penceresi için ek node eklenmesi önerilir.
...
Bu topoloji, kritik production ortamları, yüksek trafikli sistemler (~30M istek/gün ve üzeri, 3×W8), yüksek erişilebilirlik gereksinimleri (%99.9+ uptime), kritik iş süreçleri ve veri güvenliği gereksinimleri olan sistemler için idealdir.
