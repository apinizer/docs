...
<Info>
  Dağıtılmış önbellek mimarisi ve kullanımı hakkında detaylı bilgi için [Önbellek Bileşeni](/tr/concepts/temel-bilesenler/onbellek-bileseni) sayfasına bakabilirsiniz. Cache Server yapılandırması için [Dağıtık Cache](/tr/admin/server-management/dagitik-cache) sayfasına bakın.
</Info>
...
* **[Dağıtılmış Cache (Hazelcast)](/tr/concepts/temel-bilesenler/onbellek-bileseni)**: Quota, throttling, OAuth2 token gibi paylaşılan verilerin tutulduğu cache sistemi. Cache Server'lar Gateway pod'larından bağımsız namespace'lerde çalışabilir ve Kubernetes service discovery ile erişilebilir.
