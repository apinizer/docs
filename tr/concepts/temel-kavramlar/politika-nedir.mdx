...
**İpucu:** Belirli bir saat diliminde veya özel formatta tarih/saat gerektiriyorsa (örn: ISO 8601 offset), `#{dtf.format('+03:00')}` veya `#{dtf.format('+03:00','yyyy-MM-dd\'T\'HH:mm:ssXXX')}` kullanabilirsiniz. Ayrıntılı bilgi için [Dinamik Değişkenler](/tr/concepts/temel-kavramlar/dinamik-degiskenler#tarihsaat-yardımcısı-dtf) sayfasındaki "Tarih/Saat Yardımcısı (dtf)" bölümüne bakabilirsiniz.
