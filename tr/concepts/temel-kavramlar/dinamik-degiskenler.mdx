...
Formatlanmış tarih/saat değerleri şu desenlerde döner:
- **Tarih-saat:** `yyyy-MM-dd'T'HH:mm:ss.SSS'Z'` → `2026-02-04T14:30:45.000Z`
- **Tarih:** `yyyy-MM-dd` → `2026-02-04`
- **Saat:** `HH:mm:ss` → `14:30:45`

Çıktının zaman dilimi, değişken tanımındaki **Zone ID** alanıyla belirlenir.

:::

#### Tarih/Saat Yardımcısı (`dtf`)

`dtf`, mesaj şablonları ve özel hata mesajlarında kullanılabilen bir JEXL helper fonksiyonudur. `#{context.system.dateTime}` gibi sabit UTC format yerine, zaman dilimi seçerek ve özel desenleri kullanarak esnek tarih/saat biçimlendirmesi sağlar.

**API Referansı:**

| İfade | Açıklama | Örnek Sonuç |
|-------|----------|------------|
| `#{dtf.format(zone)}` | Varsayılan format (`yyyy-MM-dd HH:mm:ss`) ile, belirtilen zaman diliminde formatlanmış tarih/saat döner | `#{dtf.format('+03:00')}` → `2026-07-14 15:00:00` |
| `#{dtf.format(zone, pattern)}` | Özel desenle (ISO offset, unix timestamp gibi) formatlanmış tarih/saat döner | `#{dtf.format('+03:00','yyyy-MM-dd\'T\'HH:mm:ssXXX')}` → `2026-07-14T11:57:13+03:00` |
| `#{dtf.epochMillis()}` | Unix timestamp (milisaniye cinsinden) döner | `#{dtf.epochMillis()}` → `1740912645000` |

**Zaman Dilimi (zone):**
- Offset biçimi: `'+03:00'`, `'-05:00'`, `'UTC'`
- Zone ID: `'Europe/Istanbul'`, `'America/New_York'`, `'Asia/Tokyo'`

**Pattern Kuralları:**
- Pattern bir JEXL string argümanı olduğundan, literal karakterler tek-tırnak escape ister: `'T'` → `\'T\'`
- `XXX`: ISO offset biçiminde saat dilimini döner (örn: `+03:00`)
- Diğer yaygın pattern karakterleri: `y` (yıl), `M` (ay), `d` (gün), `H` (saat), `m` (dakika), `s` (saniye)

**Karşılaştırma:**

| İfade | Açıklama | Sonuç |
|-------|----------|-------|
| `#{context.system.dateTime}` | Sabit UTC format, seçim yok | `2026-07-14 07:57:14` |
| `#{dtf.format('+03:00')}` | Zaman dilimi seçilebilir, varsayılan format | `2026-07-14 15:00:00` |
| `#{dtf.format('+03:00','yyyy-MM-dd\'T\'HH:mm:ssXXX')}` | Hem zaman dilimi hem özel pattern | `2026-07-14T11:57:13+03:00` |

**Kullanım Senaryosu:**

İstemci tarafından ISO 8601 offset formatında tarih/saat gerektiren hata mesajı şablonları:
...
Render: `{ "error": "İşlem başarısız", "timestamp": "2026-07-14T11:57:13+03:00" }`
