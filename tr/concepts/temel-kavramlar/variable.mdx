...
### Tarih/Saat Değerlerinin Çıktı Formatı

**Değer** alanında Tarih/Saat grubundan formatlanmış bir değer seçildiğinde, değişken aşağıdaki desenlerde çözümlenir. Çıktının zaman dilimi **Zone ID** alanıyla belirlenir; bu alan boş bırakılırsa sistemin varsayılan zaman dilimi kullanılır.

| Değer | Çıktı Deseni | Örnek |
|-------|--------------|-------|
| Formatlanmış tarih-saat | `yyyy-MM-dd'T'HH:mm:ss.SSS'Z'` | `2026-06-18T14:30:45.123Z` |
| Formatlanmış tarih | `yyyy-MM-dd` | `2026-06-18` |
| Formatlanmış saat | `HH:mm:ss` | `14:30:45` |

:::note

Formatlanmış tarih-saat çıktısı her zaman sonunda **Z** harfiyle biter. Yıl, ay, gün, saat, dakika ve saniye gibi diğer Tarih/Saat değerleri ise tek başına sayısal olarak döner.
...
| **Script Language** | Script Dili seçildiği alandır. Dil değiştirildiğinde script editörünün söz dizimi vurgulaması sayfa yenilenmeden anında güncellenir. |
...
- **JSON Error Response Template ve XML Error Response Template** içinde `#{variableName}` kullanılabilir. Hata bilgisi için: `#{error.customizedErrorCode}`, `#{error.customizedHttpCode}`, `#{error.customizedMessage}`, `#{error.defaultErrorCode}`, `#{error.defaultMessage}`, `#{error.defaultHttpCode}`
...
- Bulunamayan `#{key}` değerleri boş metin "" olarak değiştirilir; exception fırlatılmaz, uygulama loguna uyarı seviyesinde log düşülür.
