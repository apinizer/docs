...
3. **Trafik dağıtımı**:
   - Counter artırılır ve mod 100 hesaplanır
   - Mod değeri traffic percentage'den küçükse: İstek CANARY backend'e yönlendirilir
   - Mod değeri traffic percentage'den büyük veya eşitse: İstek PRIMARY backend'e yönlendirilir
...
Canary release, trafik dağıtımını deterministic hale getirmek için counter-based routing kullanır:

1. **Counter Artırma:** Her istek için counter artırılır (thread-safe atomik sayaç kullanılarak)
2. **Mod 100 Hesaplama:** Counter değeri mod 100'e göre hesaplanır
3. **Karar Verme:** Mod değeri traffic percentage'den küçükse canary'ye yönlendirilir
4. **Overflow Önleme:** Counter 100'e ulaştığında otomatik olarak 0'a reset edilir
...
<Info>
Counter-based routing sayesinde, aynı istek her zaman aynı backend'e yönlendirilir. Bu, test senaryolarında tutarlılık sağlar.
</Info>
...
Counter reset mekanizması şu şekilde çalışır:

1. **Counter 99'dan 100'e artırıldığında**, mod 100 hesaplaması 0 sonucunu verir

2. **Atomik reset işlemi**:
   - Counter değeri 100 ise, atomik işlem ile 0'a reset edilmeye çalışılır
   - Reset işlemi başarılı olursa: Counter 0 olur ve sonraki istek için kullanılır
   - Reset işlemi başarısız olursa (başka bir thread counter'ı değiştirmişse): Yeni counter değeri kullanılır

Bu mekanizma sayesinde counter 100'ü aşmaz ve her zaman 0-99 aralığında kalır.
