...
<img
    src="/images/develop/yonlendirme/routing-ana-new.png"
    alt="Yönlendirme sekmesi — routing durumu, upstream, bağlantı ayarları özeti ve ilgili bölümler"
    width="1000"
    className="mr-auto"
/>
...
<img
    src="/images/develop/yonlendirme/newconnection-settings.png"
    alt="Bağlantı ayarları düzenleme penceresi (Edit Connection Settings)"
    width="1000"
    className="mr-auto"
/>
...
| **Havuzdan Bağlantı Bekleme Zaman Aşımı**  | Bağlantı havuzundan bağlantı istenirken kullanılan zaman aşımı süresidir. Saniye olarak verilir. |
| **Stale Connection Kontrolü** | Belirtilen süre (milisaniye) boyunca kullanılmayan bağlantılar yeniden kullanılmadan önce doğrulanır. Backend load balancer keep-alive timeout'una göre ayarlayın. Varsayılan: 2000 ms. **Not:** Bu ayar yalnızca **Bağlantı Havuzu Yönetim Türü = Özelleştir (Custom)** seçiliyken API Proxy'ye özel (per-proxy) uygulanır. Genel veya Hiçbiri seçiliyse Sistem Ayarları'ndan alınan değer kullanılır. |
| **Idle Connection Temizliği** | Belirtilen süreden (saniye) fazla boşta kalan bağlantılar havuzdan temizlenir. Backend load balancer keep-alive timeout'una göre ayarlayın. Varsayılan: 30 saniye. **Not:** Bu ayar yalnızca **Bağlantı Havuzu Yönetim Türü = Özelleştir (Custom)** seçiliyken API Proxy'ye özel (per-proxy) uygulanır. Genel veya Hiçbiri seçiliyse Sistem Ayarları'ndan alınan değer kullanılır. |
...
| **Yönlendirmeleri Takip Et**  | Backend API'den HTTP redirect (3xx) yanıtları geldiğinde yönlendirme adreslerinin takip edilmesini sağlar. Ayrıca zaman aşımı sürelerinin aşılması veya Backend API'den hata dönmesi durumunda istemciye hemen hata dönülmesi yerine aynı adrese ya da diğer adreslere tekrar istek gönderilerek işlemin başarılı olması sağlanmaya çalışılır. |
...
| **Maks. Yeniden Yönlendirme Sayısı** | Tek bir istek için takip edilecek en fazla backend yönlendirmesi (3xx + Location). HTTP istemcisinin kendisi toplamda en fazla 20 takip isteğine izin verir ve kimlik doğrulama challenge'ları (kimlik bilgisiyle yinelenen 401/407) yönlendirmelerle birlikte aynı üst sınırdan düşer; bu yüzden etkin sınır **min(değer, 20)** olur ve backend ayrıca kimlik doğrulama challenge'ı üretiyorsa daha da düşüktür. 0 veya negatif değer Apinizer tarafındaki sınırı kapatır; yalnız istemcinin 20 üst sınırı uygulanır. |
| **Yönlendirmede HTTP Metodunu ve Gövdeyi Koru** | Varsayılan olarak 301/302/303 yönlendirmelerinde istek GET metoduna düşürülür ve gövde silinir. Bu seçenek etkinleştirildiğinde tüm yönlendirmelerde (301/302/303/307/308) orijinal HTTP metodu ve gövde korunur. Varsayılan olarak kapalıdır; mevcut davranış değişmez. Ayrıntı ve sınırlar için aşağıdaki bölüme bakın. |
...
| **SOAP Action ve Namespace bilgisini düzelt** | SOAP ve Rest2Soap servislerde aktif olan bu seçenek ile SOAP action ve namespacelerinin WSDL'a göre varsayılan olarak düzeltilmesi işlemi gerçekleştirilir. Daha detaylı bilgi için [bu sayfayı](/tr/develop/api-proxy-olusturma/soap-api-proxy-olusturma#soap-versiyonu-belirleme) ziyaret edebilirsiniz. Ayarın devre dışı bırakılması durumunda istemcinin gönderdiği istek üzerinde bir değişiklik yapılmaz. |
| **Silinecek Hop-by-hop başlıklar** | İstemciden backend'e gönderilen istek sırasında gelen başlık bilgileri içerisinden silinecek olan bilgiyi belirler. Bazı başlıklar bu listeden bağımsız olarak, gateway tarafından her zaman silinir; bu başlıklar hakkında detaylı bilgi için aşağıdaki bilgi kutusuna bakabilirsiniz. |
...
**Her Zaman Silinen (Transport-Managed) Başlıklar**

Bazı başlıklar, yukarıdaki **Silinecek Hop-by-hop başlıklar** listesinden bağımsız olarak gateway tarafından backend'e giden istekten her zaman çıkarılır:

- **Content-Length** ve **Transfer-Encoding**: Gövde sınırları, gateway'in backend'e istek gönderirken kullandığı HTTP istemcisi tarafından kendisi hesaplanır.
- **Expect** (100-continue): 100-continue el sıkışması uçtan uca değil, bağlantı (hop) bazlı bir mekanizmadır. Gateway, politika işleme akışı için istek gövdesini zaten tamamen okumuş olduğundan, backend ile yapılacak el sıkışmayı kendi HTTP istemci katmanı üzerinden yönetir. İstemcinin gönderdiği ham **Expect** başlığı backend'e iletilmez — iletilseydi backend'in davranışına göre `417 Expectation Failed` hatası, gereksiz bekleme veya protokol hatası ile sonuçlanabilirdi.

Bu davranış trafik loglarında şu şekilde görünür:

- **1. Bölge (İstemciden Gelen İstek)**: İstemcinin gönderdiği **Expect** başlığı olduğu gibi görünür.
- **2. Bölge (Backend'e Giden İstek)**: **Expect** başlığı listede yer almaz, çünkü gateway tarafından silinmiştir.

**Silinecek Hop-by-hop başlıklar** listesi, yukarıda sayılanlar dışındaki başlıklar için yapılandırılabilir olmaya devam eder. Ancak Content-Length, Transfer-Encoding ve Expect başlıkları bu listeden çıkarılsa bile backend'e iletilmez.

Bu davranışın dosya/gövde yüklemelerine (upload) bir etkisi yoktur: istek gövdesi backend'e her zaman eksiksiz gönderilir. Kaybedilen tek şey, backend'in büyük bir gövdeyi almadan önce reddedebilmesini sağlayan 100-continue optimizasyonudur.

:::

:::info

**Bağlantı Havuzu Yönetim Türü ve Timeout Ayarları İlişkisi**

Stale Connection Kontrolü ve Idle Connection Temizliği ayarları, seçilen bağlantı havuzu yönetim türüne göre farklı davranışlar gösterir:

- **Custom (Özelleştir)**: Bu iki ayar API Proxy'ye özel (per-proxy) uygulanır. Her proxy kendi belirlediği timeout değerleriyle çalışır.
- **General (Genel)** veya **None (Hiçbiri)**: Bu iki ayar için girilen değerler dikkate alınmaz; bunun yerine **Sistem Ayarları > Gateway > Bağlantı Ayarları** bölümünde tanımlanan global değerler kullanılır.

Diğer timeout ayarları (**Bağlantı Zaman Aşımı**, **Okuma Zaman Aşımı**, **Havuzdan Bağlantı Bekleme Zaman Aşımı**) seçilen havuz yönetim türü ne olursa olsun her zaman API Proxy seviyesinde çalışır.

:::

### Yönlendirmede HTTP Metodunu ve Gövdeyi Koru

**Yönlendirmede HTTP Metodunu ve Gövdeyi Koru** ayarı, backend'in isteği başka bir adrese (örneğin HTTP'den HTTPS'e) 301/302/303 ile yönlendirdiği ve gövdesi olan bir POST/PUT isteğinin bu yönlendirme sırasında korunması gereken servislerde etkinleştirilir. Özellikle REST-2-SOAP tipi API Proxy'lerde, yönlendirme sırasında istek gövdesi kaybolduğunda backend'den **"No binding operation info while invoking unknown method with params unknown"** benzeri bir hata alınabilir; bu ayar bu tür sorunları önlemek için kullanılabilir.

:::tip

Backend adresi zaten HTTP'den HTTPS'e (veya benzer bir adrese) yönlendirme yapıyorsa, genellikle en pratik çözüm backend adresini doğrudan nihai adres olarak tanımlamaktır. Bu durumda yönlendirme hiç oluşmaz ve **Yönlendirmede HTTP Metodunu ve Gövdeyi Koru** ayarına gerek kalmaz.

:::

:::warning

Bu ayar **yalnızca** [Server Side Streaming (SSE)](#server-side-streaming-sse) kapalıyken **HTTP/1.1** backend bağlantılarında etkilidir. Aşağıdaki durumlardan biri geçerliyse ayarın **hiçbir etkisi yoktur**: 301/302/303 yönlendirmelerinde istek yine GET metoduna düşer ve gövde silinir.

* Proxy streaming (Server Side Streaming / SSE) etkinse
* [Backend HTTP Versiyonu](#backend-http-versiyonu) HTTP/1.1 dışında bir değere (örneğin HTTP/2) ayarlanmışsa

Bu durumlarda Yönlendirme ekranında ayarın altında aşağıdaki uyarı gösterilir:

> "Bu ayarın burada etkisi yoktur: yalnızca streaming kapalıyken HTTP/1.1 backend bağlantılarında geçerlidir. Yönlendirmede metodu ve gövdeyi korumak için streaming'i kapatıp HTTP sürüm politikasını HTTP/1.1 olarak ayarlayın."

Bu durumu çözmek için streaming'i kapatıp HTTP sürüm politikasını **HTTP/1.1** olarak ayarlayabilirsiniz; çoğu durumda en pratik yöntem ise backend adresini doğrudan nihai HTTPS adresi olarak tanımlamaktır — böylece yönlendirme hiç oluşmaz.

Bu yolda bir yönlendirme HTTP metodunu düşürecekse worker log'una bir uyarı yazılır; sorun sessizce oluşmaz, log kaydından da tespit edilebilir.

:::

Bu ayar APIops ile de yapılandırılabilir: routing bilgisinde ve [bağlantı ayarları güncelleme](/api-reference/api-proxies/settings/update-connection-settings) isteğinin gövdesinde `preserveHttpMethodOnRedirect` (boole) alanı üzerinden yönetilir.
...
<img
    src="/images/develop/yonlendirme/routing-new.png"
    alt="Image 2024 9 9 15 35 35 Pn"
    width="800"
    className="mr-auto"
/>
...
| Alan | Açıklama |
|------|----------|
| **Charset Geçersiz Kılma** | İstemciden gelen istek gövdesini çözmek ve backend'e göndermeden önce kodlamak için kullanılacak karakter seti. `Content-Type` başlığındaki charset değerini geçersiz kılar. Varsayılan için (UTF-8) boş bırakın. |
| **Sıkıştırma Açma Geçersiz Kılma** | İstemciden gelen istek gövdesini açmak için kullanılacak sıkıştırma algoritması. `Content-Encoding` başlığını geçersiz kılar. Sıkıştırma açmayı atlamak için **Uygulanmasın (atla)** seçin. |
| **Sıkıştırma Geçersiz Kılma** | İstek gövdesini backend'e göndermeden önce sıkıştırmak için kullanılacak algoritma. Sıkıştırmayı atlamak için **Uygulanmasın (atla)** seçin. |

### Yanıt Hattı (Backend → İstemci)

| Alan | Açıklama |
|------|----------|
| **Charset Geçersiz Kılma** | Backend'den gelen yanıt gövdesini çözmek ve istemciye göndermeden önce kodlamak için kullanılacak karakter seti. `Content-Type` başlığındaki charset değerini geçersiz kılar. |
| **Sıkıştırma Açma Geçersiz Kılma** | Backend'den gelen yanıt gövdesini açmak için kullanılacak sıkıştırma algoritması. `Content-Encoding` başlığını geçersiz kılar. Sıkıştırma açmayı atlamak için **Uygulanmasın (atla)** seçin. |
| **Sıkıştırma Geçersiz Kılma** | Yanıt gövdesinin istemciye gönderilmeden önce nasıl sıkıştırılacağını belirler. **Varsayılan: Client Accept-Encoding'e göre Otomatik** — istemcinin kabul ettiği ilk yöntem uygulanır. **Backend Yanıtını Mirror'la** seçeneği backend'in döndürdüğü encoding ile aynı şekilde iletir. Belirli bir algoritma seçilirse her zaman o uygulanır. Sıkıştırmayı atlamak için **Uygulanmasın (atla)** seçin. Detaylı davranış için bkz. [İçerik Kodlama ve Sıkıştırma — Yanıt Sıkıştırma Davranış Modları](/tr/concepts/temel-kavramlar/icerik-kodlama-ve-sikistirma#yanıt-sıkıştırma-davranış-modları). |
...
## Query Parametre Kodlama

Query parametre değerlerinin backend'e iletilirken nasıl kodlanacağını seçebilirsiniz. Varsayılan davranışta `/`, `+`, `=` gibi özel karakterler güvenlik için kodlanır; örneğin `/` karakteri adres üzerinde `%2F` olarak iletilir. Bazı backend servisler ise bu karakterleri kodlanmamış olarak bekleyebilir. Bu gibi durumlarda kodlama davranışını ihtiyacınıza göre değiştirebilirsiniz.

| Mod | Davranış |
|-----|----------|
| **Standart (URL Encode)** _(varsayılan)_ | `/`, `+`, `=` dahil tüm özel karakterler kodlanır. Mevcut tüm API Proxy'lerin davranışı budur ve değişmez. |
| **RFC 3986 (Korumalı)** | Adres standardının sorgu bölümünde izin verilen karakterler (`/`, `+`, `=`, `:`, `@`) olduğu gibi korunur; yalnızca gerçekten kaçış gerektiren karakterler kodlanır. Base64 ya da imzalı belirteç gibi değerleri olduğu gibi bekleyen backend'ler için uygundur. |
| **Ham (Encode Etme)** | Değerler kodlanmadan iletilir. Yalnızca adresi bozabilecek karakterler (boşluk, satır sonu gibi) güvenlik için kodlanır. İletilen değerlerin uygunluğu sizin sorumluluğunuzdadır; çoğu durumda **RFC 3986 (Korumalı)** daha güvenli bir tercihtir. |

:::note
Varsayılan mod **Standart**'tır; mevcut tüm API Proxy'ler değişmeden çalışmaya devam eder. Bu ayarı yalnızca backend'iniz query parametrelerini kodlanmamış olarak bekliyorsa değiştirmeniz gerekir.
:::

## Backend HTTP Versiyonu

Backend'e istek gönderilirken hangi HTTP protokol versiyonuyla bağlanılacağını seçebilirsiniz. Bu alan, Bağlantı Ayarları penceresinde **Query Parametre Kodlama** alanının hemen altında yer alır.

| Seçenek | Davranış |
|---------|----------|
| **HTTP/1.1 (Varsayılan)** | Mevcut tüm API Proxy'lerin bugünkü davranışıdır; backend'e HTTP/1.1 ile bağlanılır. |
| **HTTP/2 (ALPN, HTTP/1.1'e düşer)** | https adreslerde TLS ALPN ile HTTP/2 konuşulmaya çalışılır; backend HTTP/2'yi desteklemiyorsa otomatik olarak HTTP/1.1'e düşülür. Düz http adreslerde bu seçenek HTTP/1.1 ile çalışmaya devam eder. |
| **HTTP/2 Prior Knowledge (h2c, cleartext)** | Düz http adreslerde herhangi bir upgrade adımı olmadan doğrudan HTTP/2 konuşulur. Backend'in HTTP/2'yi cleartext (h2c) olarak desteklediğinin bilindiği durumlar içindir. |

:::note
Varsayılan seçenek **HTTP/1.1 (Varsayılan)**'dır ve bugünkü davranışla birebir aynıdır; mevcut tüm API Proxy'ler herhangi bir taşıma veya ek ayar yapılmadan aynen çalışmaya devam eder.
:::

:::warning
Aşağıdaki kombinasyonlar kaydetme sırasında reddedilir ve anlamlı bir hata mesajı döner:

* NTLM kimlik doğrulaması ile HTTP/2 seçeneklerinden (**HTTP/2** veya **HTTP/2 Prior Knowledge**) birinin birlikte kullanılması
* **HTTP/2 Prior Knowledge** seçeneğinin https adresle birlikte kullanılması
* **HTTP/2 Prior Knowledge** seçeneğinin, backend health check path'i tanımlıyken kullanılması

Ayrıca traffic mirror hedeflerine ve health check isteklerine, bu ayardan bağımsız olarak her zaman HTTP/1.1 ile bağlanılır; **Backend HTTP Versiyonu** ayarı yalnızca asıl backend isteklerini etkiler.
:::

:::note
API tanım dosyası istekleri (`?wsdl`, `?wadl`, `?swagger`, `?openapi`, `?xsd=...`) bu ayardan **etkilenmez**. Bu istekler Apinizer'ın kendi sakladığı tanım içeriğinden yanıtlanır ve backend'e hiç gitmez. Tek istisna, API Proxy'nin **Spec Erişim Tipi** ayarının **Backend Proxy** olmasıdır; bu durumda tanım isteği de backend'e yönlendirildiği için **Backend HTTP Versiyonu** ayarına tabidir.
:::
...
| Alan | Açıklama |
|------|----------|
| **SSL Doğrulamayı Atla** | İşaretlendiğinde sunucu sertifikası doğrulanmaz ve tüm sertifikalara güvenilir. Bu seçenek etkinleştirildiğinde TrustStore, KeyStore ve protokol ayarları gizlenir. Self-signed sertifika kullanan ortamlar için kullanışlıdır. |
...
:::note[Truststore ayrı bir kayıt türü değildir]
Bu ekrandaki **Truststore** seçicisi, **Keystore** seçicisiyle aynı kayıtları listeler: ikisi de **Yönetim** → **Gizli Bilgiler** → **Key Stores** altındaki Key Store kayıtlarından seçilir. Ürün içinde ayrıca oluşturulan bir "truststore" kaydı yoktur; JKS ya da PKCS12 truststore dosyanızı bir Key Store kaydı olarak yükleyin, sonra burada seçin. Seçicinin yanındaki artı düğmesi aynı işi bu ekrandan ayrılmadan yapar.

**Gizli Bilgiler** → **Sertifikalar** ekranındaki **Truststore** sekmesi bundan farklıdır: orası seçilen ortamın çalışan truststore içeriğini salt okunur gösterir, oradan seçim yapılmaz.
:::
...
:::warning Yapılandırma hatasında istek gönderilmez

mTLS açıkken istemci sertifikası yüklenemezse — seçili Keystore ortamda bulunamazsa, dosya açılamazsa ya da parolası hatalıysa — istek hedef hizmete **gönderilmez**. Çağrı `ERR-304` hata kodu ve HTTP 502 ile kesilir; API Trafiği ekranındaki **Hata Nedeni** alanında **mTLS Yapılandırma Hatası** görünür ve bu neden **TLS El Sıkışma Hatası**'ndan ayrı raporlanır.

Önceki sürümlerde aynı durumda istek istemci sertifikası olmadan gönderiliyordu: hedef hizmet sertifika zorunlu tutmuyorsa çağrı kimliksiz olarak başarıyla geçiyor, zorunlu tutuyorsa yalnızca karşı tarafın el sıkışma hatası görünüyordu.

İstemciye dönen metni **API Proxy > Hata Mesajları** ekranından özelleştirebilirsiniz. Hangi keystore, hangi ortam ve hangi alias için hata alındığı worker log'una ERROR seviyesinde yazılır.

**SSL Doğrulamayı Atla** işaretliyken bu kontrol çalışmaz; o seçenek zaten istemci sertifikasını devre dışı bırakır ve davranışı değişmemiştir.
...
Poliçelerden hata fırlatıldığında Apinizer, kullanıcıya özelleştirilme yapılmamış ise varsayılan hata mesajını gönderir. Hata mesajlarının özelleştirilmesi hakkında daha fazla bilgi almak için tıklayınız.[Bkz.](/tr/concepts/temel-kavramlar/politika-nedir#şablon-tabanlı-dinamik-hata-mesajları)

:::info

Hata mesajı yapılandırmasının tüm katmanları, iş akışı ve senaryo örnekleri için [Hata Mesajı Yapılandırma Rehberi](/tr/concepts/temel-kavramlar/hata-mesaji-yapilandirma) sayfasına bakın.

:::

## Önemli Notlar

* İstemciden gelen `Content-Encoding` veya `Transfer-Encoding` başlıkları "gzip, deflate, br, zstd, compress" değerlerinden birini içeriyorsa, mesaj gövdesi bu yöntem ile açılır ve istek hattı politikaları uygulandıktan sonra Backend API'ye yine aynı yöntem ile sıkıştırılarak gönderilir.
* Backend API'den dönen yanıtta `Content-Encoding` başlığı varsa, mesaj gövdesi açılır ve yanıt hattı politikaları uygulandıktan sonra **Yanıt Sıkıştırma Geçersiz Kılma** ayarına göre yeniden işlenir. Varsayılan davranış (**Client Accept-Encoding'e göre Otomatik**), istemcinin kabul ettiği ilk yöntem ile sıkıştırma uygular.
* Backend API düz metin dönüyor olsa dahi, **Client Accept-Encoding'e göre Otomatik** modunda istemcinin `Accept-Encoding` başlığında belirttiği yöntem uygulanır. **Backend Yanıtını Mirror'la** modunda ise yanıt düz metin olarak iletilir.
* Politikalar her zaman sıkıştırılmamış (açık) veri üzerinde çalışır.
