...
Bu seçenek aktifleştirildiğinde, backend'den sıkıştırılmış (gzip, deflate, br vb.) olarak dönen indirilebilir içerikler otomatik olarak açılarak (decompress edilerek) istemciye iletilir. Böylece sıkıştırılmış bir dosya bile açık halde indirilir.

<Info>
Bu ayar yalnızca indirilen dosyalar için geçerlidir. Yanıt gövdesi (response body) bu ayardan etkilenmez; yanıt gövdesindeki sıkıştırma işlemi zaten varsayılan olarak doğru şekilde yönetilmektedir.
</Info>
