...
### İndirme İçin Sıkıştırmayı Aç

**İndirme İçin Sıkıştırmayı Aç** ayarı indirme bölümünde yer alır. Bu seçenek aktifleştirildiğinde, indirilen dosyalar sıkıştırılmış (gzip vb.) olsa bile açık halde indirilir; backend'den indirilebilir içerik sıkıştırılmış (gzip, deflate, br vb.) döndüğünde istemciye iletilmeden önce açılır (decompress). **Yanıt gövdesi (response body) bundan etkilenmez**; bu ayar yalnızca indirilen dosyalar için geçerlidir.
