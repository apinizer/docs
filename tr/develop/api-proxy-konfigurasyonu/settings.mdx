...
[API Proxy](/tr/concepts/temel-kavramlar/api-proxy-nedir) için CORS, Cache, XML ve JSON Hata Mesaj Şablonları, İletilen IP Başlık Parametreleri, ProCrypt Entegrasyonu, **API Tanım Dosyası Erişimi** ve Bakım Modu ayarlarının yapıldığı bölümdür. Bu ayarlar [mesaj işleme ve politika uygulama](/tr/concepts/temel-kavramlar/mesaj-isleme-ve-politika-uygulama) sürecinde kullanılır.
...
## API Tanım Dosyası Erişimi

Gateway üzerinden yayınlanan **API tanım dosyasına** (örneğin OpenAPI, Swagger veya WSDL ile ilgili tanım istekleri) dış dünyadan nasıl erişileceğini belirler. Bu ayar, **normal API iş trafiğini** (Backend API'ye yönlendirilen uygulama isteklerini) doğrudan kısıtlamaz; yalnızca **tanım dosyasına erişim** davranışını kontrol eder.

Yönetim konsolunda API Proxy tanımını görüntüleme ve düzenleme bu ayardan **etkilenmez**; ayar, dış erişim adresi üzerinden yapılan tanım istekleri içindir.

Üç erişim modu seçilebilir. Mod değiştirildiğinde, seçimin etkisini onaylamanız istenebilir.
...
| Mod | Açıklama |
|---|---|
| **Herkese Açık** | Tanım dosyasına, ek kimlik doğrulama olmadan erişilebilir. |
| **Kimlik Doğrulama Gerekli** | Tanım dosyası isteğinde **HTTP Basic Authentication** kullanılması gerekir. Kullanıcı adı ve parola, Apinizer üzerinde tanımlı **istemci kimlik bilgisi (credential)** ile eşleşmelidir. |
| **Gizli** | Tanım dosyasına dış erişim adresi üzerinden **erişim tamamen engellenir**. |

### Kimlik doğrulama gerekli modunda ek seçenek

**Kimlik Doğrulama Gerekli** seçildiğinde, aşağıdaki seçenek görüntülenir:

| Alan | Açıklama |
|---|---|
| **API Proxy erişim iznini de kontrol et** | Açıkken, yalnızca kullanıcı adı ve parolası doğru olan değil, bu API Proxy için **yetkili istemci** olarak tanımlanmış kimlik bilgileri kabul edilir. Yani tanım dosyasına erişim, ilgili API Proxy’ye atanmış istemci kısıtına da bağlanır. |

Bu anahtar değiştirildiğinde ayar doğrudan kaydedilebilir (mod değişimindeki onay akışından farklı olarak).

<Info>
Hangi istemci kimlik bilgilerinin bu API Proxy için yetkili sayılacağını [API Proxy ACL](/tr/admin/identity-management/api-proxy-acl) sayfasından yönetirsiniz.
</Info>

<Info>
**Kimlik Doğrulama Gerekli** modunda tanım isteği yapan istemci, geçerli Basic Auth bilgisi göndermezse veya yetki kontrolü başarısız olursa erişim reddedilir; tarayıcı veya istemci uygun şekilde kimlik doğrulama istemine yönlendirilebilir.
</Info>

<Warning>
**Proxy Grup** kullanıyorsanız: Birleşik (grup) tanım dosyasında, **Gizli** olarak işaretlenmiş API Proxy’lerin tanımları **yer almaz**. Gruptaki tüm API Proxy’lerin tanımı gizliyse, dış erişimde tanım alınamaz ve erişim hatası oluşabilir.
</Warning>

<Info>
API Proxy tanım adreslerinin nasıl oluşturulduğuna ilişkin genel bilgi için [Genel Bilgi Sekmesi](/tr/develop/api-proxy-konfigurasyonu/overview) sayfasına bakabilirsiniz.
</Info>
