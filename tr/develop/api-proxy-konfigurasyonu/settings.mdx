...
description: "API Proxy için CORS, Cache, XML Hata Mesaj Şablonları, İletilen IP Başlık Parametreleri, ProCrypt Entegrasyonu, API Tanım Dosyası Erişim Kontrolü ve Bakım Modu ayarlarını yapılandırabilirsiniz."
---
...
API Proxy'nin dış erişim adresi üzerinden API Tanım dosyasına (WSDL, OpenAPI, XSD) erişimi üç farklı seviyede kontrol edebilirsiniz. Bu ayarın Yönetim Konsolu üzerinden gözüken tanım dosyaları için bir etkisi yoktur.
...
API Proxy trafik log ayarları eskiden bu sekmenin en altında yer alıyordu. Bu bölüm artık API Proxy detay sayfasındaki ayrı **Log Ayarları** sekmesinden yönetilir. Ortam bazlı log konfigürasyonu, konnektör etkinleştirme ve metod geçersiz kılmaları bu yeni sekmeden yapılandırılabilir.

<Info>
Log ayarları hakkında detaylı bilgi için [API Proxy Trafik Log Ayarları](/tr/develop/api-proxy-konfigurasyonu/api-trafik-log-ayarlari) sayfasını ziyaret edebilirsiniz.
</Info>
