...
Anahtar tek başına çalışmaz ve yalnızca konnektörün **[Log alanlarında hangi ayar geçerli](/tr/admin/server-management/gateway-ortamlarina-konnektor-eklenmesi#log-alanlarinda-hangi-ayar-gecerli)** seçimi **Bu bağlayıcı** iken anlamlıdır — bu seçim **API Proxy** ise konnektörün detay filtresi zaten uygulanmaz, dolayısıyla geçilecek bir kısıt yoktur. **Bu bağlayıcı** modunda ise anahtar, yöneticinin [konnektörü ortama eklerken](/tr/admin/server-management/gateway-ortamlarina-konnektor-eklenmesi#bir-api-proxynin-bu-kisitlari-gecmesine-izin-verme) **API Proxy geçişine izin ver** seçeneğini de açtığı konnektörlerde etkilidir. Diğer bütün konnektörlerde anahtar yok sayılır ve ortam kuralı uygulanmaya devam eder. İki anahtar, iki sahip: istisnanın nerede mümkün olacağına yönetici, bu istisnanın kullanılıp kullanılmayacağına proxy sahibi karar verir.
