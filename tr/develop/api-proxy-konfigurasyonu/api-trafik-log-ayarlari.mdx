...
**Öğe adı** kural tipiyle birlikte **Tespit Et** işlemi seçili bir kural, yükseltmeden sonra da veriyi
maskelemez. Bu alanın loglarda gizlenmesini istiyorsanız kuralın işlemini **Maskele** olarak değiştirin.
...
**Düzenli ifade ve Hazır PII tipi kuralları artık ayrıştırılmış header ve parametre listelerini de tarar.** Bu iki kural tipi, hem istemciden gelen hem hedefe giden istekteki, hem de yanıt tarafındaki (hedeften gelen, istemciye giden) loglanan header listelerinde ve istek tarafındaki loglanan parametre listelerinde çalışır — yalnızca gövde ve adres alanlarıyla sınırlı değildir. Aynı kapsam, ilgili proxy'nin izleme (trace) kaydındaki öncesi/sonrası header ve parametre alanları için de geçerlidir. Örneğin hazır TCKN kuralı tanımlıysa, geçerli bir TCKN değeri hem adreste hem de ayrıştırılmış parametre listesinde maskelenir. Bu tarama yalnızca ilgili kural tanımlıysa çalışır; hiçbir Düzenli ifade veya Hazır PII tipi kuralı tanımlı değilse ek bir maliyeti yoktur.

:::warning
Hazır PII tipi olarak **IP Adresi** veya **URL** kuralı tanımlarsanız `X-Forwarded-For`, `Referer`, `Location` gibi header değerleri de trafik kaydında maskelenir. Gerçek istek ve yanıt hiçbir zaman değişmez; yalnızca log kaydındaki kopya maskelenir.
:::
...
## Ortam Kısıtlarını Ezme {#ortam-kisitlarini-ezme}
...
Anahtar tek başına çalışır; konnektörden ayrıca bir izin beklenmez. Karar tümüyle proxy sahibine aittir ve ortamdaki [log konnektörünün](/tr/admin/server-management/gateway-ortamlarina-konnektor-eklenmesi) tipinden bağımsız olarak geçerlidir.
...
[Veri saklama profili](#veri-saklama-profili) de etkilenmez. **No Payload** veya **No Persist** yürürlükteyken gövde hiç toplanmaz, dolayısıyla ezmenin yeniden açacağı bir şey yoktur.
:::
