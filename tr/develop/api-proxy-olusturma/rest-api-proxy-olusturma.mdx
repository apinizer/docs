    ...
    <AccordionGroup>
      <Accordion title="Yöntem 1: URL'den Yükleme">
        **URL Gir** bağlantısına tıklanır. OpenAPI/Swagger dosyasının URL'si girilir (örneğin: `https://petstore.swagger.io/v2/swagger.json`) ve **Parse** butonuna tıklanır. Dosya parse edilir ve API bilgileri görüntülenir.
        ...
        <Info>
          **Kimlik Doğrulama ve Zaman Aşımı** Spec URL'si kimlik doğrulama gerektiriyorsa, **Spec Authorization Kullan** seçeneğini etkinleştirerek özel header'lar veya **Basic Auth** (kullanıcı adı/şifre) bilgilerini girebilirsiniz. Ayrıca **SSL/Sertifika Ayarları** bölümünde bağlantı zaman aşımı süresini (5-120 saniye, varsayılan: 30s) ayarlayabilirsiniz. Detaylı bilgi için [API Proxy Oluşturma](/tr/develop/api-proxy-olusturma/api-proxy-olusturma) sayfasına bakınız.
        </Info>
      </Accordion>
