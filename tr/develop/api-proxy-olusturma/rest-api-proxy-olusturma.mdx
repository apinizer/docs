    ...
    <AccordionGroup>
      <Accordion title="Yöntem 1: URL'den Yükleme">
        **URL Gir** bağlantısına tıklanır. OpenAPI/Swagger dosyasının URL'si girilir (örneğin: `https://petstore.swagger.io/v2/swagger.json`) ve **Parse** butonuna tıklanır. Dosya parse edilir ve API bilgileri görüntülenir.
        ...
        <Info>
          **Kimlik Doğrulama ve SSL** Spec URL'si kimlik doğrulama gerektiriyorsa, **Spec Authorization Kullan** seçeneğini etkinleştirerek özel header'lar veya **Basic Auth** (kullanıcı adı/şifre) bilgilerini girebilirsiniz. **SSL / Sertifika Ayarları** panelinde bağlantı zaman aşımı (spec yanıtı için 5-120 saniye, varsayılan 30s), **SSL Doğrulamayı Atla**, **TrustStore (JKS/PKCS12)** ve **Sertifika (PEM)** satırları ile HTTPS erişimini yapılandırabilirsiniz. Detay için [API Proxy Oluşturma](/tr/develop/api-proxy-olusturma/api-proxy-olusturma) sayfasına bakınız.
        </Info>
      </Accordion>
