...
<Info>
  **WSDL URL'den Yükleme** WSDL dosyasını URL üzerinden yüklerken, spec URL'si kimlik doğrulama gerektiriyorsa **Spec Authorization Kullan** seçeneğini etkinleştirerek özel header'lar veya **Basic Auth** (kullanıcı adı/şifre) bilgilerini girebilirsiniz. Ayrıca **SSL/Sertifika Ayarları** bölümünde bağlantı zaman aşımı süresini (5-120 saniye, varsayılan: 30s) ayarlayabilirsiniz. Yetkilendirme bilgileri, WSDL içindeki alt şema (XSD import) dosyalarının indirilmesinde de otomatik olarak kullanılır.
</Info>
