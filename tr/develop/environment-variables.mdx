...
**Kullanım Alanı:** AI Gateway modülündeki giden (outbound) bağlantı kimlik doğrulama ayarları ile LLM/VectorDB bağlantı bilgileri

| Özellik              | Açıklama                                                        |
| -------------------- | ---------------------------------------------------------------- |
| **Konum**            | **MCP Gateway** giden bağlantı formu (Uç Nokta, Bearer Token veya OAuth2 istemci kimlik bilgileri) · **A2A Gateway** giden bağlantı formu (Uç Nokta, Bearer Token veya OAuth2 istemci kimlik bilgileri) · **AI Gateway > LLM Sağlayıcılar** (Uç Nokta URL, API Anahtarı, API Gizli Anahtarı, Organizasyon ID) · **AI Gateway > RAG > Vector Veritabanları** tab'ı (Varsayılan Koleksiyon Adı) |
...
**Kullanım Senaryosu:** Aynı AI Proxy yapılandırmasını farklı ortamlarda farklı LLM sağlayıcı anahtarları veya farklı MCP/A2A giden bağlantı kimlik bilgileriyle çalıştırmak için
