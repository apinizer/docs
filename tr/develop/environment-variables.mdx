...
| Özellik              | Açıklama                                                        |
| -------------------- | ---------------------------------------------------------------- |
| **Konum**            | **AI Gateway > MCP Gateway > Gelen Sunucular** (API Anahtarı, JWKS URL, Issuer, Audience alanları) · **AI Gateway > LLM Sağlayıcılar** (Uç Nokta URL, API Anahtarı, API Gizli Anahtarı, Organizasyon ID) · **AI Gateway > Semantik Önbellek > Vector Veritabanları** (Varsayılan Koleksiyon Adı) |
...
| **Secret Kullanımı**  | API Anahtarı ve API Gizli Anahtarı gibi hassas alanlar secret olarak saklanır; alana bir ortam değişkeni ifadesi girildiğinde (yapıştırma veya elle yazma) şifreleme atlanır ve değer çalışma zamanında ortam değişkeninden çözümlenir |

**Kullanım Senaryosu:** Aynı AI Proxy yapılandırmasını farklı ortamlarda farklı LLM sağlayıcı anahtarları veya farklı MCP API anahtarlarıyla çalıştırmak için
...
| Özellik              | Açıklama                                                        |
| -------------------- | ---------------------------------------------------------------- |
| **Konum**            | **AI Gateway > A2A Gateway > Gelen Ajanlar** (API Anahtarı) · AI RAG Injection politikası (Koleksiyon Adı, Enjeksiyon Şablonu, Embedding Model) |
| **Alanlar**           | Yukarıdaki ekranlardaki ilgili metin girişleri                 |
...
| **Secret Kullanımı**  | API Anahtarı hassas bir alan olarak secret şeklinde saklanır; alana bir ortam değişkeni ifadesi yazıldığında şifreleme atlanır ve değer çalışma zamanında ortam değişkeninden çözümlenir |

**Kullanım Senaryosu:** Aynı AI Proxy yapılandırmasını farklı ortamlarda farklı A2A API anahtarları veya farklı RAG koleksiyon adlarıyla çalıştırmak için
