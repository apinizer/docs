...
**4. AI Gateway — Buton ile Panoya Kopyalama (MCP Gateway, LLM Sağlayıcılar, Vector Veritabanları)**

**Kullanım Alanı:** AI Gateway modülündeki gelen (inbound) kimlik doğrulama ayarları ile LLM/VectorDB bağlantı bilgileri

| Özellik              | Açıklama                                                        |
| -------------------- | ---------------------------------------------------------------- |
| **Konum**            | **AI Gateway > MCP Gateway > Gelen Sunucular** (Sanal API Anahtarı, JWKS URL, Issuer, Audience alanları) · **AI Gateway > LLM Sağlayıcılar** (Uç Nokta URL, API Anahtarı, API Gizli Anahtarı, Organizasyon ID) · **AI Gateway > Semantik Önbellek > Vector Veritabanları** (Varsayılan Koleksiyon Adı) |
...
| **Secret Kullanımı**  | Sanal API Anahtarı, API Anahtarı ve API Gizli Anahtarı gibi hassas alanlar secret olarak saklanır; alana bir ortam değişkeni ifadesi girildiğinde (yapıştırma veya elle yazma) şifreleme atlanır ve değer çalışma zamanında ortam değişkeninden çözümlenir |

**Kullanım Senaryosu:** Aynı AI Proxy yapılandırmasını farklı ortamlarda farklı LLM sağlayıcı anahtarları veya farklı MCP sanal API anahtarlarıyla çalıştırmak için

**5. AI Gateway — Manuel Giriş (A2A Gelen Ajanlar, AI RAG Injection Politikası)**

**Kullanım Alanı:** A2A Gateway'in gelen (inbound) kimlik doğrulama ayarı ve AI RAG Injection politikasının bağlantı/şablon alanları

| Özellik              | Açıklama                                                        |
| -------------------- | ---------------------------------------------------------------- |
| **Konum**            | **AI Gateway > A2A Gateway > Gelen Ajanlar** (Sanal API Anahtarı) · AI RAG Injection politikası (Koleksiyon Adı, Enjeksiyon Şablonu, Embedding Model) |
| **Alanlar**           | Yukarıdaki ekranlardaki ilgili metin girişleri                 |
| **Kullanım**          | Kimlik doğrulama bilgisi ve RAG bağlantı bilgisinde ortam bazlı yönetim |
...
| **Secret Kullanımı**  | Sanal API Anahtarı hassas bir alan olarak secret şeklinde saklanır; alana bir ortam değişkeni ifadesi yazıldığında şifreleme atlanır ve değer çalışma zamanında ortam değişkeninden çözümlenir |

**Kullanım Senaryosu:** Aynı AI Proxy yapılandırmasını farklı ortamlarda farklı A2A sanal API anahtarları veya farklı RAG koleksiyon adlarıyla çalıştırmak için
