...
- Ortak politika kullanma ihtiyacınız varsa [Global Politikalar](/tr/develop/global-configurations/global-policies) veya [Politika Grupları](/tr/develop/global-configurations/policy-groups) kullanabilirsiniz.

:::

Daha fazla bilgi için [API Proxy Group](/tr/concepts/temel-kavramlar/api-proxy-group) kavram sayfasına bakabilirsiniz.
...
| Client Route                                | İstemcilerin API Proxy Group'a erişim için kullanacağı yönlendirme bilgileri. Detaylı bilgi için bakınız: [Client Route Ayarları](/tr/develop/api-proxy-olusturma/client-route-ayarlari)                                                                                                                          |
| Proxy Grubu Anahtarı(_API Proxy Group Key_) | API Proxy Grup üzerinden JWT veya OAuth2 yetkilendirmesi yapılmak istendiğinde ihtiyaç duyulan Genel ve Gizli Anahtarların yönetildiği bölümdür. Detaylı bilgi için bu anahtarların [ API Proxy için nasıl yönetildiğine ](/tr/develop/api-proxy-konfigurasyonu/overview#proxy-anahtarını-göster)bakınız. |
...
Gateway yeniden başlatıldığında bir API Proxy Grubu, yalnızca ilgili ortama deploy edilmişse (deploy durumu açıksa) tekrar yüklenir; deploy edilmemiş ya da kaldırılmış (undeploy edilmiş) bir grup, yeniden başlatma sonrasında da devre dışı kalmaya devam eder. Bu, deploy kaydı silinmiş olduğu hâlde Gateway üzerinde hâlâ yanıt vermeye devam eden bir grubu da kapsar (örneğin önceki bir sürümde üzerine yazarak içe aktarma ve ardından silme yapılmışsa): yeniden başlatmadan sonra bu grup artık sunulmaz; çözüm grubu yeniden deploy etmektir.
...
<CardGroup cols={3}>
  <Card title="API Proxy Group" icon="users" href="/tr/concepts/temel-kavramlar/api-proxy-group">
    API Proxy Group kavramını öğrenin
  </Card>
  <Card title="Client Route Ayarları" icon="route" href="/tr/develop/api-proxy-olusturma/client-route-ayarlari">
    Client Route yapılandırması
  </Card>
  <Card title="API Proxy Oluşturma" icon="plus" href="/tr/develop/api-proxy-olusturma">
    API Proxy oluşturma
  </Card>
  ...
  <Card title="Global Politikalar" icon="globe" href="/tr/develop/global-configurations/global-policies">
    Global politikalar hakkında bilgi edinin
  </Card>
  <Card title="Politika Grupları" icon="layer-group" href="/tr/develop/global-configurations/policy-groups">
