...
Gateway yeniden başlatıldığında bir API Proxy Grubu, yalnızca ilgili ortama deploy edilmişse (deploy durumu açıksa) tekrar yüklenir; deploy edilmemiş ya da kaldırılmış (undeploy edilmiş) bir grup, yeniden başlatma sonrasında da devre dışı kalmaya devam eder.
