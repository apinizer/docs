...
:::warning
Sık yapılan hata, yer tutucuyu doğrudan script gövdesine yazmaktır; örn. `def text = ${benimDegiskenim};`. Bu, bir script hatasıyla sonuçlanır (`ERR-058` hata kodu ve `No signature of method: ... $()` benzeri bir mesaj), çünkü `${...}` ifadesi Apinizer yer tutucusu olarak değil, Groovy söz dizimi olarak ayrıştırılır. Ortam değişkenlerini her zaman `environmentVariableMap.get("benimDegiskenim")` ile okuyun.
:::
