...
<img src="/images/develop/politikalar/script-new.png" alt="Script Politikası" width="1000" />
...
- Detaylı bilgi için [Dinamik Değişkenler](/tr/concepts/temel-kavramlar/dinamik-degiskenler) sayfasını inceleyebilirsiniz.
...
| requestUrlFromClient | İstek | Client → Apinizer | String | Okuma | İstemciden gelen isteğin karşılandığı endpoint yolunu içerir (örn. `/pet/findByStatus`). SOAP tipi API Proxy'lerde bu değişkenin değeri SOAP metodunun adıdır. | `String value= requestUrlFromClient;` |
...
| stopFlowAsSuccess | İstek / Yanıt | Script → Apinizer | boolean | Yazma | Akış kesilerek istemciye mesaj dönüldüğünde, sonucun **başarılı** olarak işaretlenmesini sağlar. Değeri `true` yapılırsa istek trafik loglarında `resultType=SUCCESS` olarak görünür, analitik raporlarda hata/bloke sayılmaz ve durum kodu yazılmadıysa HTTP **200** döner (varsayılan davranışta 401 dönerdi). **Durum kodu istisnası:** yanıt hattında arka uç servis 4xx/5xx döndürmüşse o durum kodu korunur, 200'e çevrilmez — yalnızca sonuç tipi `SUCCESS` olur. Arka ucun hata kodunu da maskelemek istiyorsanız `statusCodeToClient` değişkenine dönmek istediğiniz kodu açıkça yazın; script'in yazdığı durum kodu her zaman önceliklidir. Bu değişken tek başına akışı kesmez — akışın kesilmesi için yine `responseErrorMessageToClient` (yanıt hattı) veya `requestErrorMessageToTargetAPI` (istek hattı) değerinin dolu olması gerekir. Yazılmadığında davranış değişmez, sonuç `BLOCKED` olarak işaretlenir. | `responseErrorMessageToClient= "{\"status\":\"ok\"}";`<br/><br/>`stopFlowAsSuccess= true;` |
...
:::note
**Davranış**:
- Map salt-okunurdur; `put`, `remove`, `clear` çağrıları `UnsupportedOperationException` fırlatır.
- Iteration desteklenir: `keySet()`, `values()`, `entrySet()` çağrılarak aktif ortamdaki tüm değişken isimleri/değerleri keşfedilebilir.
- Aktif ortam üzerinden çalışır; her script çalıştırılışında o anki aktif ortamın değerleri okunur.
- Gizli (Secret) olarak işaretlenmiş değerler çözülmüş plain değer olarak döner.
- Büyük/küçük harf duyarsız: `get("dbHost")`, `get("DBHOST")`, `get("dbhost")` aynı değeri döner.
:::
...
- Temel: `getUsername()`, `getEmail()`, `getFullName()`, `getDescription()`
- Parola: `getResolvedPassword()` (şifresi çözülmüş; parola değerinde `${değişken}` ifadesi varsa yerine geçer), `getEnvironmentPassword("ortam-id")` (ortam-bazlı çözülmüş parola)
- Kimlik metadata: `getOrganizationId()`, `getOrganizationName()`, `getProjectId()`, `getProjectName()`, `getAppId()`, `getAppName()`
- Yetki ve durum: `getRoleNameList()`, `getEnabled()`, `isExternal()`, `getExpireDate()`, `getGrantType()`
- Anahtar materyali: `getSecretKey()`, `getCertificate()`, `getPublicKey()`, `getPrivateKey()`, `getKeyStore()`, `getTrustStore()`, `getJwkForValidationAndSign()`, `getJwkForDecryptionAndEncryption()`
- Anahtar kimlikleri: `getSecretKeyId()`, `getCertificateId()`, `getPublicKeyId()`, `getPrivateKeyId()`, `getKeyStoreId()`, `getTrustStoreId()`
- Özel metadata (credential ve organizasyonu üzerinde tanımlı anahtar/değer çiftleri): `getMetadata()` (organizasyon + credential girdileri birleşik, anahtar çakışmasında credential öncelikli; değerler çözülür ve şifresi açılır), `getCredentialMetadata()` (yalnızca credential seviyesi), `getOrganizationMetadata()` (yalnızca organizasyon seviyesi) — her biri salt-okunur `Map<String, String>` döner
:::
...
| request_httpServlet | Client → Apinizer | jakarta.servlet.http.HttpServletRequest | Okuma | İsteğin alt seviye servlet nesnesine erişim sağlar. Yalnızca standart değişkenlerle çözülemeyen ileri düzey senaryolarda kullanılmalıdır; yanlış kullanımı istek akışını bozabilir. | `def req= request_httpServlet;` |
...
| response_charset | Backend API → Apinizer | String | Okuma, Yazma | Backend API'den dönen yanıtın karakter kümesi (charset) değerine erişmek ve bu değeri değiştirmek için kullanılır. | `String value= response_charset;`<br/><br/>`response_charset= "UTF-8";` |
| response_backendContentEncoding | Backend API → Apinizer | String | Okuma, Yazma | Backend API'den dönen yanıtın orijinal `Content-Encoding` başlık değerine erişmek ve bu değeri değiştirmek için kullanılır. | `String value= response_backendContentEncoding;`<br/><br/>`response_backendContentEncoding= "gzip";` |
...
| response_httpServlet | Backend API → Apinizer | jakarta.servlet.http.HttpServletResponse | Okuma | Yanıtın alt seviye servlet nesnesine erişim sağlar. Yalnızca standart değişkenlerle çözülemeyen ileri düzey senaryolarda kullanılmalıdır; yanlış kullanımı yanıt akışını bozabilir. | `def resp= response_httpServlet;` |

### Hata Hattı Değişkenleri (Error Pipeline)

API Proxy'nin [Error Pipeline](/tr/concepts/temel-kavramlar/mesaj-isleme-ve-politika-uygulama) hattına eklenen bir Script politikası, oluşan hatanın türüne göre dallanma yapabilsin diye hata bilgisine salt-okunur değişkenlerle erişebilir. Bu değişkenler **yalnızca hata hattındaki** Script politikalarında doludur; istek veya yanıt hattında bir hata oluşmadıysa değerleri **boştur (null)**.

| Akış Değişkenleri | Sahip Olduğu Değerin Yeri | Veri Tipi | İzin Verilen İşlem | Açıklama | Örnek Kullanım |
|-------------------|---------------------------|-----------|-------------------|----------|----------------|
| error_category | Hata Hattı → Script | String | Okuma | Hatanın kaba sınıfıdır ve dallanma için temel anahtardır. Olası değerler: `ROUTING_ERROR` (backend yönlendirme hatası), `POLICY_BLOCK` (bir politikanın isteği bloke etmesi), `POLICY_ERROR` (bir politika çalışırken oluşan hata), `GATEWAY_ERROR` (gateway/iç hata). | `if (error_category == "ROUTING_ERROR") { ... }` |
| error_type | Hata Hattı → Script | String | Okuma | Hatanın ayrıntılı türüdür; `error_category`'den daha ince dallanma gerektiğinde kullanılır. | `String value= error_type;` |
| error_parent | Hata Hattı → Script | String | Okuma | Hatanın ait olduğu üst grup bilgisidir. | `String value= error_parent;` |
| error_resultType | Hata Hattı → Script | String | Okuma | Sonuç tipidir: `BLOCKED` (istek bloke edildi) veya `ERROR` (hata oluştu). | `String value= error_resultType;` |
| error_message | Hata Hattı → Script | String | Okuma | Hatanın açıklama/detay mesajıdır. | `String value= error_message;` |
| error_code | Hata Hattı → Script | String | Okuma | Hatanın kodudur. | `String value= error_code;` |
| error_httpCode | Hata Hattı → Script | Integer | Okuma | Hataya karşılık gelen HTTP durum kodudur. | `Integer value= error_httpCode;` |

:::note

Hata hattı değişkenleri **salt-okunurdur**; sistem hatayı yakaladığında atanır ve Script bunları yalnızca okur. Bu değişkenlere değer atamak sistemin hata yanıtını değiştirmez. Hata yanıtının gövdesini, başlığını veya durum kodunu değiştirmek için yanıt hattı değişkenlerini (`responseBodyTextToClient`, `responseHeaderMapToClient`, `statusCodeToClient`) kullanın.

:::

**Örnek — hata türüne göre yanıt dönüştürme:**

```groovy
import groovy.json.JsonOutput

if (error_category == "ROUTING_ERROR") {
    // Backend'e ulaşılamadığında istemciye sade ve tutarlı bir mesaj dön
    ...
    // POLICY_BLOCK / POLICY_ERROR / GATEWAY_ERROR: yanıta dokunulmaz, hata olduğu gibi döner
}
```
...
| message_cacheHit | Çalışma Zamanı | boolean | Okuma, Yazma | İsteğin cache'ten karşılanıp karşılanmadığı bilgisidir. Trafik loglarındaki "Cache Hit" alanına bu değer yazılır. Script içinde `true` ya da `false` atanarak bu değer değiştirilebilir; böylece kendi cache mantığını script ile kuran senaryolarda istek cache'ten karşılanmış olarak raporlanabilir. Yazma yalnızca HTTP hattında geçerlidir. | `boolean value= message_cacheHit;`<br/><br/>`message_cacheHit= true;` |
...
| apiProxy_relativePath | Client → Apinizer | String | Okuma | İstemciden gelen istek API Proxy tarafından karşılanmış ise karşılanan API Proxy'nin yayınlandığı göreli yol (relative path) bilgisine erişmek için kullanılır. | `String value= apiProxy_relativePath;` |
...
Script politikasından dağıtık cache'e yazmak, okumak ve silmek için `cache` değişkeni kullanılır. Bu değişken **Cache Politikası ile aynı cache alanını paylaşır**; depolama dağıtıktır, yani tüm gateway (worker) sunucuları aynı veriyi görür.

| Metot | İmza | Dönüş | Davranış |
|-------|------|-------|----------|
| get | `cache.get(String key)` | String \| null | Cache'teki değeri döner. Kayıt yoksa, anahtar boşsa veya bir hata oluşursa `null` döner. |
| put | `cache.put(String key, Object value, long ttlSeconds)` | boolean | Değeri `ttlSeconds` saniye boyunca saklar. Anahtar boşsa **veya** `ttlSeconds` 0 ya da negatifse hiçbir şey yapmaz ve `false` döner. |
| delete | `cache.delete(String key)` | boolean | Kaydı siler. Silme başarılıysa `true` döner. |
...
if (cached == null) {
    ...
    cache.put(key, fresh, 600)   // 600 saniye boyunca sakla
    cached = fresh
}
...
- **Dönen değer her zaman metindir.** `put` ile bir nesne veya Map verseniz bile `get` size metin karşılığını döner. JSON/XML veriler için `put` öncesi serialize, `get` sonrası parse etmeniz gerekir.
- **TTL zorunludur ve saniye cinsindendir.** Süresiz saklama desteklenmez.
- **Ayrı bir isim alanı (namespace) yoktur.** Cache Politikası ile aynı alan kullanıldığından, anahtar çakışmasını önlemek için ayırt edici bir ön ek kullanın (`profile|...` gibi).
- **Cache hataları script'e yansımaz.** Tüm hatalar yutulur ve loglanır; `cache` çağrıları hiçbir zaman hata fırlatmaz, `null` veya `false` döner.
