...
<Note>
**Davranış**:
- Map salt-okunurdur; `put`, `remove`, `clear` çağrıları hata fırlatır.
- Anahtar listesi çıkarma (`keySet`, `values`) desteklenmez.
- Aktif ortam üzerinden çalışır; ortam değişimi runtime'da yansır.
...
```java
def me = credentialMap.get(credential_username);
...
- Parola: `getResolvedPassword()` (çözülmüş, değişkenleri yerleştirilmiş), `getEnvironmentPassword("ortam-id")` (ortam-bazlı çözülmüş parola)
