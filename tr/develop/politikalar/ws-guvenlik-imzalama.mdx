...
<img src="/images/develop/politikalar/ws-sign-new.png" alt="WS Güvenlik İmzalama Politikası" width="1000" />
...
| **Adım 4: İmza Anahtar Yapılandırması** | - **Signature KeyStore** listesinden kullanılacak sertifika deposunu seçin.<br />- **Key Identifier Type** ile imza tespit yöntemi (X509, Subject Key Identifier, Custom Key Info vb.) belirlenir.<br />- Custom Key Info seçildiğinde `sigCustomKeyIdentifier` ve `sigCustomKeyIdentifierValueType` alanlarını doldurun. |
| **Adım 5: İmza Algoritmaları ve Uyum Seçimleri** | - `sigSigAlgorithm`, `sigC14n`, `sigDigAlgorithm` alanlarından uygun algoritmaları seçin.<br />- `sigUseSingleCert` ve `sigWsiBSPCompliant` bayraklarıyla sertifika kullanım stratejisini ve BSP uyumluluğunu belirleyin.<br />- `mustUnderstand` seçeneği SOAP header düzeyinde zorunluluk oluşturur. |
| **Adım 6: İmzalanacak SOAP Parçalarını Tanımlama** | - `Add Signature Part` butonuyla Body, Header veya özel elementleri listeye ekleyin.<br />- Her parça için ad, namespace ve encodeType (ELEMENT/CONTENT) seçerek XML kapsamını kesinleştirin.<br />- Gerekirse mevcut kayıtları düzenleyin veya silin. |
...
| **Adım 9: Kaydetme** | - Sağ üstteki **[Save]** butonuna tıklayın.<br /><br />**Kontrol Listesi:** Benzersiz isim. Zorunlu alanlar dolu. En az bir imzalanacak parça veya temel imza parametresi mevcut<br /><br />**Sonuç:**<br />- Politika listeye eklenir.<br />- API'lere bağlanabilir.<br />- Global politikaysa otomatik uygulanır. |
