...
| **Kullanım** | Kota aşan müşteri ne zaman tekrar istek yapabilir? | Reset time'a kadar beklemeli. FIXED window'da: Bir sonraki periyot başında (örn: gelecek ayın 1'i). SLIDING window'da: İlk isteğin üzerinden periyot süresi geçince. Yanıt her zaman bir `Retry-After` başlığı (saniye) taşır — pencere kapanana kadar kalan süredir, en az 1'dir ve Rate Limit İstatistikleri ayarından bağımsız olarak eklenir. Rate limit header'ları aktifse response'da ayrıca X-RateLimit-Reset ile timestamp verilir. |
