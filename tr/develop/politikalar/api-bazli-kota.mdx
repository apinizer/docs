...
- **Sayım Modu**: Kota zaten aşıldığı için reddedilen isteklerin de kotayı tüketip tüketmeyeceğini belirler. Varsayılan (Tüm istekler) modda kabul edilen veya reddedilen her istek kullanım sayacını artırır — Apinizer'ın tarihsel davranışıyla aynıdır. Yalnızca kabul edilenler seçilirse reddedilen bir istek kotayı hiç tüketmez.
- **Limit Değişimi Davranışı**: Politika kullanımdayken Kota Sayısı değeri değiştirildiğinde mevcut kullanım sayacına ne olacağını belirler. Varsayılan (Önceki limite kırp) modda sayaç, önceki limit değerine kırpılır; böylece gerçekte izin verilenden fazla görünmez. Sayacın yalnız limit yükseltildiğinde (limit düşürüldüğünde asla) sıfırlanması sağlanabilir ya da sayaç hiç değiştirilmeden bırakılabilir.
...
<img src="/images/develop/politikalar/new-kota.png" alt="API Bazlı Kota Politikası Yapılandırma" width="1000" />
...
| **Kullanım** | Limiti değiştirdiğimde mevcut kullanımın ne olacağını veya reddedilen isteklerin kotaya dahil olup olmayacağını kontrol edebilir miyim? | Evet, Gelişmiş Ayarlar'daki iki bağımsız ayar bunu belirler. **Limit Değişimi Davranışı**, Kota Sayısı değeri değiştirildiğinde kullanım sayacını kontrol eder: varsayılanda (Önceki limite kırp) sayaç önceki limite kırpılır, böylece gerçekte izin verilenden fazla görünmez; sayaç yalnız limit yükseltildiğinde sıfırlanacak şekilde ayarlanabilir ya da hiç değiştirilmeden bırakılabilir. **Sayım Modu**, politika tarafından reddedilen isteklerin kotayı tüketip tüketmeyeceğini kontrol eder: varsayılanda (Tüm istekler) tüketir, tarihsel davranışla aynıdır; Yalnızca kabul edilenler seçilirse reddedilen bir istek kotayı hiç tüketmez. Örnek: Bir müşterinin kotası 5 ve şimdiden 5 kabul + 5 red isteği yapmış, yani Tüm istekler modunda kullanım 10. Kota 8'e yükseltilirse, Önceki limite kırp davranışı kullanımı 5'e (önceki limite) indirir, böylece yeni limite ulaşılana kadar tam olarak 3 istek daha geçer. |
| **Kullanım** | Kota aşan müşteri ne zaman tekrar istek yapabilir? | Reset time'a kadar beklemeli. FIXED window'da: Bir sonraki periyot başında (örn: gelecek ayın 1'i). SLIDING window'da: İlk isteğin üzerinden periyot süresi geçince. Yanıt her zaman bir `Retry-After` başlığı (saniye) taşır — pencere kapanana kadar kalan süredir, en az 1'dir, platform geneli bir tavanla sınırlıdır (varsayılan 1 gün, aşağıdaki nota bakın) ve Rate Limit İstatistikleri ayarından bağımsız olarak eklenir. Rate limit header'ları aktifse response'da ayrıca X-RateLimit-Reset ile timestamp verilir. |

:::note Retry-After tavanı

`Retry-After` değeri platform genelinde bir tavanla sınırlıdır, böylece istemciye makul olmayan bir bekleme süresi bildirilmez. Bu özellikle aylık kotalar için önemlidir: tavan olmasaydı, gelecek ayın 1'inde sıfırlanan bir pencere yaklaşık 31 güne kadar bir bekleme süresi bildirebilirdi. Varsayılan tavan 1 gündür; Gateway (Worker) üzerinde `apinizer.limit.retryAfterClampSeconds` JVM sistem özelliği ile değiştirilebilir — geçersiz ya da pozitif olmayan bir değer varsayılana döner. Tavan sayacın kendisini etkilemez: erken bir tekrar denemesi yine aynı dolu pencereyle karşılaşır ve yine tavanla sınırlı, güncel bir `Retry-After` alır. Aynı tavan [API Bazlı Daraltma](/tr/develop/politikalar/api-bazli-daraltma) politikasının `Retry-After` başlığı için de geçerlidir.
