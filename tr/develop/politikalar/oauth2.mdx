...
- Kimlik ve rol kaynaklarını Secret Manager, API, LDAP, veritabanı veya OIDC sağlayıcılarıyla entegre ederek yetkilendirme süreçlerini uyumlu hale getirir.
...
- **Kimlik Sağlayıcı Entegrasyonu**: Secret Manager, Identity Role Group, LDAP, API, veritabanı veya [OIDC Kimlik Sağlayıcı](/tr/admin/identity-management/identity-providers/oidc-provider) tabanlı kimlik kaynaklarıyla çalışarak kimlik doğrulama esnekliği sunar.
...
<img src="/images/develop/politikalar/Oauth2.png" alt="OAuth 2 Kimlik Doğrulama Politikası" width="1000" />
...
| **Adım 4: OAuth 2 Akış Parametreleri** | - **Managed From This Policy:** Token yönetimini politika mı üstlenecek belirleyin.<br />- **Grant Type:** CLIENT_CREDENTIALS veya PASSWORD seçin.<br />- **Identity Role Group:** PASSWORD akışında kimlik sağlayıcısını Secret Manager, LDAP, Database, API veya OIDC üzerinden seçin. OIDC seçildiğinde aranabilir bir [OIDC Kimlik Sağlayıcı](/tr/admin/identity-management/identity-providers/oidc-provider) listesi açılır (ad, açıklama, sağlayıcı türü); Generic türündeki bir sağlayıcı da burada geçerli bir seçimdir, çünkü yalnızca token doğrulaması kullanılır.<br />- **Check Client IP Address:** Secret Manager akışında token'ın kaynak IP ile eşleşmesini zorunlu kılın.<br />- **Kimlik Kaynağı her grant tipinde görünür:** CLIENT_CREDENTIALS akışında yalnızca **Secret Manager** (varsayılan — token üretimi ve doğrulaması değişmez) veya **OIDC** sunulur.<br />- **CLIENT_CREDENTIALS ile OIDC (sağlayıcının ürettiği token):** politika token üretmez; istemci sağlayıcının erişim token'ını `Authorization: Bearer` ile sunar, Apinizer her istekte doğrular (çevrimdışı JWKS imzası, çevrimiçi introspection/userinfo — opak token'lar introspection ile desteklenir). Kimlik sağlayıcının *Kullanıcı Adı Claim Yolu* alanından (opak token'da introspection yanıtından), roller sağlayıcının rol kaynağından gelir; *Eşleşen Consumer Zorunlu* kimliği kota/ACL atfı için bir Consumer ile eşler. Öncekini Sil, token süresi ve yenileme ayarları gizlenir; proxy'nin `/auth/*` token ucu `400 invalid_request` döner. PASSWORD + OIDC önceki davranışı korur. |
| **Adım 5: Token Yaşam Döngüsü ve Refresh Ayarları (Varsa)** | - **Delete Previous:** Yeni token üretildiğinde eski tokenları iptal edin.<br />- **Token Never Expires:** Süresiz token gerekiyorsa açık bırakın; kapatırsanız süre ve birim tanımlayın.<br />- **Refresh Token Allowed:** Yenileme hakkını aktif edin, sayıyı ve süresini belirleyin.<br />- **Allow URL Parameters / Clear Auth:** Token endpoint'inde URL parametresi kabulü ve eski kimlik bilgilerinin temizlenmesini yapılandırın. |
| **Adım 6: Header ve Yetkilendirme Entegrasyonu (Varsa)** | - **Set Resolved Identity to Context (Çözümlenen Kimliği İstek Bağlamına Yaz):** Bu politikanın çözümlediği kimliğin (kullanıcı/anahtar) isteğin kimliği olarak kaydedilip kaydedilmeyeceğini belirler. Açık (varsayılan) iken bu kimlik trafik loglarında ve sonraki politikalar ile scriptlerde görünür. Kapalı iken kimlik yalnızca gerekli iç kimlik doğrulama/yetkilendirme kontrolleri için kullanılır, isteğin kalıcı kimliği olarak tutulmaz.<br />- **Add User To Header / User Header Name:** Doğrulanan kullanıcı bilgisini özel header'a ekleyin.<br />- **Authorization Configuration:** Method bazlı rol atamaları, yetkilendirme kaynağı (Secret Manager, API, LDAP, DB) ve rol header yönetimini ayarlayın.<br />- **Add Roles To Header / Roles Header Name:** Roller için iletilecek header isimlerini belirleyin. |
...
| **Adım 9: Kaydetme** | - Sağ üstteki **[Save]** butonuna tıklayın.<br /><br />**Kontrol Listesi:** Benzersiz isim. Zorunlu alanlar dolu. En az bir OAuth 2 akış seçimi ve gerekli kimlik sağlayıcısı belirlenmiş durumda.<br /><br />**Sonuç:**<br />- Politika listeye eklenir.<br />- API'lere bağlanabilir.<br />- Global politikaysa otomatik uygulanır. |
...
Bu Client Id / Client Secret görüntüleme, kopyalama ve yeniden oluşturma deneyimi (Otomatik/Manuel seçimi, isim yazarak onaylama, son onay adımı dahil); **API Proxy**, **Politika Grubu (Policy Group)** ve **API Proxy Group** ekranlarındaki kendi anahtar yönetimi bölümleriyle birebir aynıdır. Bu OAuth 2 Kimlik Doğrulama Politikası bir API Proxy'nin, bir API Proxy Group'un veya bir Politika Grubu'nun politika listesine bağlandığında — global (paylaşılan) bir politika olarak seçilmiş olsun ya da o API/Grup'a özel yerel bir politika olarak oluşturulmuş olsun — politika formunda görünen **API Anahtarını Göster** seçeneği aynı deneyimi sunar.
