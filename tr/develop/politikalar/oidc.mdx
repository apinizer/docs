...
<img src="/images/develop/politikalar/oidc-new.png" alt="OIDC Kimlik Doğrulama Politikası" width="1000" />
...
| **Adım 4: OIDC Sağlayıcı ve Uç Nokta Yapılandırması** | - Issuer veya Discovery URL'sini tanımlayın.<br />- Discovery devre dışı ise Authorization, Token, UserInfo, JWKS ve Introspection endpoint adreslerini manuel girin.<br />- Client ID/Secret, Redirect URI, Realm ve scopes alanlarını doldurun.<br />- Token issuer doğrulaması için beklenen değeri belirleyin (opsiyonel).<br />- PKCE ve zorunlu scope ihtiyaçlarını değerlendirerek işaretleyin.<br />- OIDC sağlayıcısına mTLS ile bağlanacaksanız mTLS yapılandırmasını (KeyStore, TrustStore, SSL doğrulama atlama) kullanın. |
| **Adım 5: Akış, Token ve Oturum Parametreleri** | - Flow Type seçerek (Authorization Code, Implicit, Hybrid, OAuth2) yönlendirmeyi belirleyin.<br />- Kimlik doğrulama modunu (EXTERNAL_ONLY, INTERNAL_ONLY, HYBRID) seçin.<br />- Token doğrulama, imza algoritması, issuer ve çoklu audience kontrollerini yapılandırın.<br />- Token kabul yolu (header, cookie, header_cookie) seçin; bearer kullanımı, access token header adı ve cache timeout değerlerini girin.<br />- Session cookie adı, oturum süresi, mutlak oturum süresi ve cookie ayırıcı ayarlarını yapın.<br />- Token cookie ayarları: access token, ID token ve refresh token cookie adları; hangi token'ların cookie'ye yazılacağı.<br />- Logout: logout path, post-logout redirect URI, redirect after logout URI; çıkışta token iptali ve revocation endpoint.<br />- Access token süresi dolduğunda otomatik yenileme.<br />- Politikanın uygulanmayacağı HTTP metodları ve path desenlerini tanımlayın. |
| **Adım 6: Gelişmiş Başlık ve Yetkilendirme Ayarları** | - Custom header ve backend header tablolarına gereksinim duyulan key/value çiftlerini ekleyin.<br />- Token'ları backend isteğine ekleyin; access token ve ID token header isimlerini belirleyin.<br />- Kullanıcı/rol bilgilerini header'a yazmak için toggle'ları aktif edin, header isimlerini belirleyin.<br />- Identity Role Group Service ile rol eşlemeleri yapın, gerekirse Authorization Configuration bileşeni üzerinden ek servisleri bağlayın. |
...
| **Adım 9: Kaydetme** | - Sağ üstteki **[Save]** butonuna tıklayın.<br /><br />**Kontrol Listesi:** Benzersiz isim. Zorunlu alanlar dolu. En az bir OIDC sağlayıcı konfigürasyonu mevcut<br /><br />**Sonuç:**<br />- Politika listeye eklenir.<br />- API'lere bağlanabilir.<br />- Global politikaysa otomatik uygulanır. |
