...
<img src="/images/develop/politikalar/xml-schema-validation-new.png" alt="XML Şema Doğrulama Politikası" width="1000" />
...
| **Adım 4: XPath Yapılandırması** | - `pathForBody` alanına doğrulanacak XML düğümünü hedefleyen XPath ifadesini girin.<br />- Tam gövde doğrulaması için boş bırakın veya `/` kullanın.<br />- XPath doğruluğunu test etmek için Test Data Transformation butonunu kullanabilirsiniz. |
| **Adım 5: WSDL Şeması Kullanımı** *(Varsa)* | - `validateAgainstSpec` seçeneğini işaretleyerek politikayı bağlı WSDL şemasına göre çalıştırın.<br />- İşaretlendiğinde manuel XSD giriş alanları gizlenir ve bakım yükü azalır. |
| **Adım 6: XSD Şema Tanımları** *(Varsa)* | - `validateAgainstSpec` kapalıysa **Add** butonuyla `schemaDefinitionList` öğeleri ekleyin.<br />- Her öğe için XML şema içeriğini düzenleyin ve gerekiyorsa `systemId` alanı ile dış referanslar belirtin.<br />- Sıralama `schemaNo` alanıyla otomatik güncellenir. |
...
| **Adım 9: Kaydetme** | - Sağ üstteki **[Save]** butonuna tıklayın.<br /><br />**Kontrol Listesi:** Benzersiz isim. Zorunlu alanlar dolu. En az bir XSD tanımı veya WSDL seçimi mevcut<br /><br />**Sonuç:**<br />- Politika listeye eklenir.<br />- API'lere bağlanabilir.<br />- Global politikaysa otomatik uygulanır. |
