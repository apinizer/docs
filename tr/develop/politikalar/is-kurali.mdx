...
<img src="/images/develop/politikalar/is-kurali-1.png" alt="İş Kuralı Politikası Yapılandırma" width="1000" />
...
| **Adım 6: Hata Mesajı Özelleştirme (İsteğe Bağlı)** | - **Error Message Customization** sekmesine gidin.<br />- Erişim reddedildiğinde veya STOP aksiyonunda dönecek mesajı özelleştirin.<br />- **Şablon tabanlı (dinamik) hata mesajı** kullanarak `#{error.errorCode}`, `#{request.uri}`, `#{dateTime.formattedText}`, `#{message.correlationId}` gibi yer tutucılarla istek ve hata bilgilerini mesaja dahil edebilirsiniz. Detay için [Politika Nedir? – Şablon Tabanlı Hata Mesajları](/tr/concepts/temel-kavramlar/politika-nedir#şablon-tabanlı-dinamik-hata-mesajları) bölümüne bakın.<br /><br />**Varsayılan:**<br />`{ "statusCode": 403, "message": "[Default hata mesajı]" }`<br /><br />**Özel (şablon örneği):**<br />`Hata Kodu: #{error.errorCode}, Zaman: #{dateTime.formattedText}, İstek ID: #{message.correlationId}` |
