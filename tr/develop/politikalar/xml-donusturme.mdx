...
<img src="/images/develop/politikalar/xml-transformation-new.png" alt="XML Dönüşümü Politikası" width="1000" />
...
| **Adım 4: Dönüşüm Tipini Seçme** | `transformationType` alanından XML2XML veya XML2JSON seçin. Seçim, alt parametrelerin görünürlüğünü belirler. XML2XML seçildiğinde XSLT editörü açılır, XML2JSON seçildiğinde JSON seçenekleri aktif olur. |
| **Adım 5: XSLT Kurallarını Yönetme** *(Varsa)* | XML2XML modunda `xsltValue` editöründe dönüşüm şablonunu girin. Kod editörü syntax highlighting sağlar. `Test Transformation` butonuyla örnek XML üzerinde XSLT'yi doğrulayın. XSLT'nin dışarıdan değer alabilmesi için bu sayfadaki [XSLT Parametre Kullanımı](#xslt-parametre-kullanımı) bölümünü inceleyebilirsiniz. |
| **Adım 6: XML→JSON Ayarlarını Yapılandırma** *(Varsa)* | XML2JSON modunda `xmlToJsonUnwrapElement`, `xmlToJsonIgnoreNull`, `xmlToJsonIgnoreEmpty`, `xmlToJsonNumbersAsStrings`, `xmlToJsonUseNullForNil` bayraklarını ihtiyaca göre işaretleyin. `xmlToJsonArrayPathList` alanına JSON dizisine dönüşmesi gereken XPath ifadelerini ekleyin. |
...
| **Adım 9: Kaydetme** | - Sağ üstteki **[Save]** butonuna tıklayın.<br /><br />**Kontrol Listesi:** Benzersiz isim. Zorunlu alanlar dolu. En az bir XSLT veya XML→JSON yapılandırması mevcut<br /><br />**Sonuç:**<br />- Politika listeye eklenir.<br />- API'lere bağlanabilir.<br />- Global politikaysa otomatik uygulanır. |
...
## XSLT Parametre Kullanımı

XML2XML dönüşümünde XSLT şablonu, `<xsl:param name="..."/>` tanımlarıyla dışarıdan değer alabilir. Bu değerler için ayrı bir alan doldurulmaz; her parametre, **adıyla aynı isimde** tanımlanmış bir değişkenle otomatik olarak eşleştirilir. Böylece aynı XSLT şablonunu farklı ortamlarda farklı değerlerle çalıştırmak veya çalışma anında üretilen verilerle beslemek mümkün olur.

### Değer Nereden Gelir?

Bir parametre adı, sırasıyla aşağıdaki iki kaynakta aranır; bulunan ilk değer XSLT'ye geçirilir:

1. **Özel değişkenler (custom variable):** Aynı istek hattında daha önce çalışan bir politikanın (örneğin Script politikası) ürettiği değerler önce kontrol edilir.
2. **Ortam değişkenleri (environment variable):** Özel değişkenlerde bulunamazsa, aktif ortamda tanımlı ortam değişkenleri kontrol edilir.

Her iki kaynakta da bulunamayan parametreye değer atanmaz; bu durumda XSLT şablonundaki kendi varsayılan değeri (`<xsl:param name="..." select="..."/>`) geçerli olur.

| Değer kaynağı | Kullanım | Eşleşme |
|---------------|----------|---------|
| Özel değişken | Önceki bir politikanın istek bazında ürettiği değer | Parametre adı = özel değişken adı |
| Ortam değişkeni | Ortama göre değişen yapılandırma değeri | Parametre adı = ortam değişkeni adı |

### Örnek 1 — Ortam değişkeniyle parametre

Geliştirme ve üretim ortamında farklı bir bölge değeri kullanılması gerekiyorsa, XSLT parametresiyle aynı adda bir ortam değişkeni tanımlanır.
...
`backendRegion` ortam değişkeni geliştirme ortamında `dev-eu`, üretim ortamında `prod-eu` olarak tanımlıysa; aynı XSLT şablonu her ortamda o ortamın değerini alır. Üretimde sonuç `<region>prod-eu</region>` olur.

### Örnek 2 — Özel değişkenle (custom variable) parametre

Aynı istek hattında daha önce çalışan bir politika `requestType` adıyla bir değer üretmişse, XSLT'de aynı adda bir parametre tanımlandığında bu değer otomatik olarak parametreye geçer.
...
İstek özelinde değişen değer XSLT'ye yansır ve dönüşüm bu değere göre dallandırılabilir.

:::note

- Parametre değeri ayrı bir alana yazılmaz; XSLT'deki parametre **adıyla aynı isimde** bir özel değişken veya ortam değişkeni tanımlamanız yeterlidir.
- Eşleştirme her istek için yapılır; aynı ad önce özel değişkenlerde, ardından ortam değişkenlerinde aranır.
- Hiçbir kaynakta bulunamayan parametreye değer atanmaz; XSLT'deki varsayılan değeri geçerli olur.
- Değerler XSLT'ye metin (string) olarak geçirilir.
