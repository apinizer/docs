...
| **Adım 5: XSLT Kurallarını Yönetme** *(Varsa)* | XML2XML modunda `xsltValue` editöründe dönüşüm şablonunu girin. Kod editörü syntax highlighting sağlar. `Test Transformation` butonuyla örnek XML üzerinde XSLT'yi doğrulayın. |
| **Adım 6: XSLT Parametrelerini Tanımlama** *(Varsa)* | XML2XML modunda XSLT şablonunun beklediği parametre değerleri panelde ad-değer ikilileri halinde tanımlanır. Değer alanı sabit bir metin olabileceği gibi ortam değişkeni veya özel değişken üzerinden de çözümlenebilir. Detaylar için bu sayfadaki [XSLT Parametre Kullanımı](#xslt-parametre-kullanımı) bölümünü inceleyebilirsiniz. |
| **Adım 7: XML→JSON Ayarlarını Yapılandırma** *(Varsa)* | XML2JSON modunda `xmlToJsonUnwrapElement`, `xmlToJsonIgnoreNull`, `xmlToJsonIgnoreEmpty`, `xmlToJsonNumbersAsStrings`, `xmlToJsonUseNullForNil` bayraklarını ihtiyaca göre işaretleyin. `xmlToJsonArrayPathList` alanına JSON dizisine dönüşmesi gereken XPath ifadelerini ekleyin. |
...
| **Adım 9: Hata Mesajı Özelleştirme (İsteğe Bağlı)** | - **Error Message Customization** sekmesine gidin.<br />- Erişim reddedildiğinde dönecek mesajı özelleştirin.<br /><br />**Varsayılan:**<br />`{ "statusCode": 403, "message": "[Default hata mesajı]" }`<br /><br />**Özel:**<br />`{ "statusCode": 403, "errorCode": "[CUSTOM_ERROR_CODE]", "message": "[Özel mesaj]" }` |
| **Adım 10: Kaydetme** | - Sağ üstteki **[Save]** butonuna tıklayın.<br /><br />**Kontrol Listesi:** Benzersiz isim. Zorunlu alanlar dolu. En az bir XSLT veya XML→JSON yapılandırması mevcut<br /><br />**Sonuç:**<br />- Politika listeye eklenir.<br />- API'lere bağlanabilir.<br />- Global politikaysa otomatik uygulanır. |
...
XML2XML dönüşümünde, XSLT şablonu `<xsl:param name="..."/>` etiketleriyle dışarıdan değer kabul edebilir. Bu değerler politika yapılandırmasında ad-değer ikilileri olarak tanımlanır. Aynı XSLT'yi farklı ortamlarda farklı parametrelerle çalıştırmak veya çalışma anında dinamik veriyle beslemek mümkün olur.

### Parametre Değer Türleri

| Değer türü | Kullanım | Örnek değer | Çözümleme |
|------------|----------|-------------|-----------|
| Sabit değer | Tüm ortamlarda aynı değerle çalışır | `customer-portal` | Olduğu gibi XSLT'ye geçer |
| Ortam değişkeni | Aktif ortamda tanımlı değer kullanılır | `${backendRegion}` | Aktif ortamdaki değişken değeriyle değiştirilir; bulunamazsa boş gönderilir |
| Özel değişken | Önceki politikaların `customVariableMap` üzerinden ürettiği değer kullanılır | `#{requestType}` | İstek/yanıt hattında ilgili özel değişken değeriyle değiştirilir |

### Örnek 1 — Sabit parametre
...
Sonuç XML: `<version>v3</version>`

### Örnek 2 — Ortam değişkenli parametre

Geliştirme ve üretim ortamında farklı bir endpoint adıyla işaretleme yapılması gerekiyorsa, ortam değişkeni kullanılır.

| Ad | Değer |
|----|-------|
| `backendRegion` | `${backendRegion}` |

`backendRegion` ortam değişkeni geliştirme ortamında `dev-eu`, üretim ortamında `prod-eu` olarak tanımlıysa XSLT aynı şablonla her iki ortamda da doğru değeri alır.

### Örnek 3 — Özel değişkenle (custom variable) parametre

Önceki bir Script politikası `customVariableMap.put("requestType", "premium")` ile bir değer üretmişse XSLT'ye bu değer parametre olarak geçirilebilir.
...
<Note>
- Değer alanında `${...}` ortam değişkenini, `#{...}` özel değişkeni ifade eder. Aynı değer alanında her iki tip birlikte de kullanılabilir.
- Çözümleme her istek için yapılır; değişken bulunamazsa parametre değeri boş geçer.
- Parametre adları XSLT'deki `<xsl:param name="...">` tanımıyla bire bir aynı olmalıdır.
</Note>
