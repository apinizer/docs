...
description: "HTTP Basic Authentication standardını kullanarak API isteklerini kimlik bilgileriyle doğrulayabilir ve yetkilendirebilirsiniz. Kullanıcı adı ve şifre tabanlı güvenli erişim kontrolü sağlayabilirsiniz"
---
...
:::info Şifre `:` karakteri içerebilir

Base64 çözüldükten sonra kullanıcı adını şifreden yalnızca **ilk** `:` karakteri ayırır; sonrasındaki tüm karakterler şifrenin parçası sayılır (RFC 7617). Örneğin `kullanici:12:34` bilgisi `kullanici` kullanıcı adı ve `12:34` şifresi olarak çözümlenir.
...
<img src="/images/develop/politikalar/basic-new.png" alt="Basic Kimlik Doğrulama Politikası Yapılandırma" width="1000" />
...
| **Adım 4: Identity Role Group Seçimi (Zorunlu)** | - **Identity Role Group** alanında **[Click to Select]** butonuna tıklayın.<br />- Açılan dialogda kimlik kaynağını seçin:<br /><br />**Secret Manager:** Apinizer'da tanımlı kullanıcı/şifre/IP listeleri<br />**LDAP:** Active Directory veya OpenLDAP entegrasyonu<br />**Database:** Kullanıcı bilgilerini veritabanından sorgulama<br />**API:** Harici REST API ile kimlik doğrulama<br /><br />- Seçim yaptıktan sonra **[Select]** butonuna tıklayın.<br />- Yanlış seçim yaparsanız **[X]** butonu ile temizleyebilirsiniz. |
| **Adım 5: Username Variable Tanımlama (Zorunlu)** | - **Variable for Authorization** alanında değişken seçimi yapın.<br />- İki seçenek mevcuttur:<br /><br />**1. Mevcut Variable Seçme:**<br />**[Select Different]** butonuna tıklayın. Variable listesinden username bilgisinin yazılacağı değişkeni seçin.<br /><br />**2. Variable Güncelleme:**<br />Seçili variable'ın **[Update Variable]** butonuna tıklayarak özelliklerini düzenleyebilirsiniz.<br /><br />- Doğrulanan kullanıcı adı bu variable'a yazılır.<br />- Policy chain'de sonraki politikalar veya backend bu değişkeni kullanabilir. |
| **Adım 5b: Gizli Anahtar Doğrulama Modu (İsteğe Bağlı)** | **Gizli Anahtar/Parola Değerini Kontrol Et:**<br />- Varsayılan olarak açıktır. Açıkken, standart Basic Authentication'da olduğu gibi kullanıcı adıyla birlikte bir gizli anahtar/parola değeri de sağlanmalı ve kontrol edilmelidir.<br />- Kapatıldığında kimlik doğrulama yalnızca kullanıcı adı/API anahtarı değerine dayanır (yalnız-anahtar doğrulama) — ayrıca bir gizli değer kontrol edilmez.<br /><br />**API Anahtarını Bearer Token Olarak Kullan (yalnızca Gizli Anahtar/Parola Değerini Kontrol Et kapalıyken görünür):**<br />- Açıkken, username variable üzerinden okunan değer ham bir Basic Authentication kimlik bilgisi olarak değerlendirilmez. `Authorization` header'ı `Bearer` şemasını taşımak zorundadır ve ardından gelen token API anahtarı olarak kullanılır.<br />- `Bearer` şeması yoksa veya token boşsa, istek API anahtarının kendisi eksik olduğunda dönen aynı "kimlik bilgisi eksik" hatasıyla reddedilir.<br />- **Gizli Anahtar/Parola Değerini Kontrol Et** tekrar açılırsa bu seçenek otomatik olarak temizlenir.<br /><br />Kullanım örneği için aşağıdaki [API Anahtarını Bearer Token Olarak Kullanma](#api-anahtarını-bearer-token-olarak-kullanma) bölümüne bakın. |
| **Adım 6: Opsiyonel Güvenlik Ayarları** | **Check Client IP Address (sadece Secret Manager):**<br />- Checkbox aktif edilirse, Secret Manager'da tanımlı IP listesi ile istemci IP'si karşılaştırılır.<br />- IP uyuşmazsa 401 hatası döner.<br /><br />**Clear Auth:**<br />- Checkbox aktif edilirse, Authorization header backend'e iletilmez.<br />- Mikroservis mimarilerinde backend'in şifre bilgisine erişmesini engellemek için kullanılır.<br /><br />**Set Resolved Identity to Context (Çözümlenen Kimliği İstek Bağlamına Yaz):** Bu politikanın çözümlediği kimliğin (kullanıcı/anahtar) isteğin kimliği olarak kaydedilip kaydedilmeyeceğini belirler. Açık (varsayılan) iken bu kimlik trafik loglarında ve sonraki politikalar ile scriptlerde görünür. Kapalı iken kimlik yalnızca gerekli iç kimlik doğrulama/yetkilendirme kontrolleri için kullanılır, isteğin kalıcı kimliği olarak tutulmaz.<br /><br />**Add User to Header:**<br />- Toggle aktif edilirse, doğrulanan kullanıcı bilgisi özel header ile backend'e iletilir.<br />- **User Header Name** alanı açılır (varsayılan: `X-Authenticated-UserId`).<br />- Backend bu header'ı okuyarak kullanıcı bağlamında işlem yapabilir. |
...
| **Adım 9: Kaydetme** | - Sağ üstteki **[Save]** butonuna tıklayın.<br /><br />**Kontrol Listesi:**<br />- Benzersiz isim<br />- Zorunlu alanlar dolu<br />- En az bir Identity Role Group seçili<br /><br />**Sonuç:**<br />- Politika listeye eklenir.<br />- API'lere bağlanabilir.<br />- Global politikaysa otomatik uygulanır. |

:::note

Varsayılan kullanıcı adı/parola modunda **Identity Role Group** seçim listesinde [OIDC Kimlik Sağlayıcı](/tr/admin/identity-management/identity-providers/oidc-provider) yer almaz: Basic Authentication her istekte gönderilen parolayı karşılaştırır, OIDC sağlayıcı ise token doğrular ve hiç parola vermez. **API Anahtarını Bearer Token Olarak Kullan** açıldığında (**Gizli Anahtar/Parola Değerini Kontrol Et** kapalıyken) sunulan değer bir bearer token olduğundan **OIDC seçilebilir hale gelir**: sağlayıcının ürettiği token her istekte sağlayıcının doğrulama moduna göre doğrulanır (çevrimdışı JWKS imzası, çevrimiçi introspection/userinfo), kimlik sağlayıcının *Kullanıcı Adı Claim Yolu* alanından çözülür ve *Eşleşen Consumer Zorunlu* açıksa bir Consumer ile eşlenir. **API Anahtarını Bearer Token Olarak Kullan** tekrar kapatılırsa OIDC kimlik kaynağı Secret Manager'a sıfırlanır (uyarı gösterilir); OIDC kaynağı taşıyan düz Basic politikasını sunucu reddeder. Tarayıcı tabanlı oturum açma akışları için [OIDC Kimlik Doğrulama](/tr/develop/politikalar/oidc) politikasını kullanın.

:::

### API Anahtarını Bearer Token Olarak Kullanma

**API Anahtarını Bearer Token Olarak Kullan** seçeneğini, yalnızca tek bir opak anahtar sunan ve kullanıcı adı/parola doğrulaması uygulamayan istemciler için etkinleştirin — örneğin, kendini tanıtmak için tam bir [OIDC](/tr/develop/politikalar/oidc)/Keycloak token alma akışına ihtiyaç duymaması gereken basit servisler arası veya makine istemcileri.

**Gizli Anahtar/Parola Değerini Kontrol Et** kapalı ve **API Anahtarını Bearer Token Olarak Kullan** açıkken, istek anahtarı standart `Authorization` header'ında `Bearer` şemasıyla taşımalıdır:
...
`Bearer` sonrasındaki token, API anahtarı olarak aranır — seçilen Identity Role Group'a bağlı olarak Consumer'ın kullanıcı adı veya API Client'ın clientId değeri. `Bearer` şeması olmayan veya token'ı boş olan bir istek, anahtar aranmadan önce reddedilir.
