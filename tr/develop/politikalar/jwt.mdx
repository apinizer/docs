...
- **Kimlik doğrulama kaynaklarını yönetmek**: Secret Manager, LDAP, Database, API veya OIDC tabanlı kimlik doğrulama servislerini seçilebilir hale getirir.
...
- **Kimlik Kaynağı Seçimi**: Secret Manager, LDAP, Database, harici API veya [OIDC Kimlik Sağlayıcı](/tr/admin/identity-management/identity-providers/oidc-provider) kaynaklarını JWT üretim sürecine entegre eder.
...
<img src="/images/develop/politikalar/jwt-new.png" alt="JWT Kimlik Doğrulama Politikası" width="1000" />
...
| **Adım 4: JWT Kimlik Doğrulama Parametreleri** | - **Managed From This Policy**: Credentials yönetimini politika mı kontrol edecek? Secret Manager dışı entegrasyonlarda `true` seçilir.<br />- **Grant Type**: CLIENT_CREDENTIALS veya PASSWORD seçin; seçim, kullanılabilir kimlik kaynaklarını belirler.<br />- **Kimlik Kaynağı**: Her grant tipinde görünür. PASSWORD için Identity Role Group modalından LDAP/Database/API/OIDC veya Secret Manager seçimi yapın. CLIENT_CREDENTIALS için yalnızca **Secret Manager** (varsayılan — politika kendi token'ını eskisi gibi üretir ve doğrular) veya **OIDC** sunulur. OIDC seçildiğinde aranabilir bir [OIDC Kimlik Sağlayıcı](/tr/admin/identity-management/identity-providers/oidc-provider) listesi açılır (ad, açıklama, sağlayıcı türü); Generic türündeki bir sağlayıcı da burada geçerli bir seçimdir — yalnızca token doğrulaması kullanılır, senkronizasyon devreye girmez. (JWT Yenileme Servisi seçiminde OIDC bir kimlik sağlayıcı olduğu için bu listede yer almaz — yenileme servisi yalnızca API tipi bir servis olabilir.)<br />- **CLIENT_CREDENTIALS ile OIDC (sağlayıcının ürettiği token)**: politika token **üretmez**. İstemci erişim token'ını OIDC sağlayıcıdan alır ve `Authorization: Bearer` ile gönderir; Apinizer her istekte token'ı sağlayıcının doğrulama moduna göre doğrular (çevrimdışı JWKS imzası veya çevrimiçi introspection/userinfo), kimliği sağlayıcının *Kullanıcı Adı Claim Yolu* alanından çözer ve *Eşleşen Consumer Zorunlu* açıksa kota, ACL ve hız limiti atfı için bir Consumer ile eşler. Token süresi, yenileme ve *Managed From This Policy* ayarları uygulanmadığı için gizlenir; bu proxy'nin `/auth/*` token ucu `400 invalid_request` döner. Keycloak client-credentials token'ında kimlik claim'i `preferred_username = service-account-<clientId>` gelir; Consumer client id ile adlandırıldıysa sağlayıcının Kullanıcı Adı Claim Yolu alanını `azp` yapın.<br />- **PASSWORD ile OIDC** önceki davranışı korur: sağlayıcı sunulan bearer token'ı doğruladıktan sonra politika yine kendi token'ını üretir.<br />- **Check Client IP Address**: Secret Manager tabanlı yapılarda isteğin geldiği IP'yi doğrular. |
| **Adım 5: Token ve Refresh Yapılandırması** | - **Token Never Expires**: Süresiz token üretecekse açın, aksi halde süreyi ve birimini girin.<br />- **Refresh Token Allowed**: Yenileme izni verilecekse açın, hak sayısı ve geçerlilik süresini tanımlayın.<br />- **JWT Signature Algorithm**: RS256/RS384/RS512 arasından hedef sistem gereksinimine göre seçim yapın. |
| **Adım 6: Header ve Yetkilendirme Ayarları** | - **Allow URL Parameters**: Token'ın query parametresiyle gönderimini mümkün kılar; yalnızca güvenilir senaryolarda önerilir.<br />- **Clear Auth**: Gelen Authorization header'ını temizler; eski JWT'nin sıfırlanması için kullanılır.<br />- **Set Resolved Identity to Context (Çözümlenen Kimliği İstek Bağlamına Yaz):** Bu politikanın çözümlediği kimliğin (kullanıcı/anahtar) isteğin kimliği olarak kaydedilip kaydedilmeyeceğini belirler. Açık (varsayılan) iken bu kimlik trafik loglarında ve sonraki politikalar ile scriptlerde görünür. Kapalı iken kimlik yalnızca gerekli iç kimlik doğrulama/yetkilendirme kontrolleri için kullanılır, isteğin kalıcı kimliği olarak tutulmaz.<br />- **Add User/Roles to Header**: Kullanıcı ve rol bilgisini özel header isimleriyle hedef servise gönderir.<br />- **Authorization Configuration**: Rol bazlı gömme, method access ve JWT yenileme servisi gibi ileri seçenekleri açın. |
...
| **Adım 9: Kaydetme** | - Sağ üstteki **[Save]** butonuna tıklayın.<br /><br />**Kontrol Listesi:** Benzersiz isim. Zorunlu alanlar dolu. En az bir kimlik kaynağı seçimi tamamlandı.<br /><br />**Sonuç:**<br />- Politika listeye eklenir.<br />- API'lere bağlanabilir.<br />- Global politikaysa otomatik uygulanır. |

## Client Id / Client Secret Yönetimi

**Managed From This Policy** seçeneği açık olduğunda, token isteklerinde doğrulanacak Client Id ve Client Secret değerleri; bu politikanın bağlı olduğu **API Proxy**, **API Proxy Group** veya **Politika Grubu (Policy Group)**'nun kendi anahtarlarıdır. Politika formunda yer alan **API Anahtarını Göster (Show API Key)** seçeneğiyle açılan pencerede mevcut Client Id ve Client Secret görüntülenebilir, kopyalanabilir ve gerektiğinde yeniden oluşturulabilir (regenerate).

### Anahtar Üretim Tipi (Key Generation Type)

Client Id veya Client Secret'ı yeniden oluştururken iki seçenekten biri belirlenir:

- **Otomatik (Automatic):** Yeni değeri sistem üretir.
- **Manuel (Manual):** Değeri siz belirlersiniz.

<img
  src="/images/develop/politikalar/oauth-jwt-regenerate-client-id.png"
  alt="Client Id veya Client Secret yeniden oluşturma onay diyaloğu"
  width="1000"
  height="486"
/>
...
Yeniden oluşturma (regenerate) geri alınamaz kritik bir işlemdir. Devam edebilmek için ilgili kaydın adını yazarak onaylamanız, ardından son bir uyarı ekranını onaylamanız istenir. Değişiklik kaydedilmeden geçerli olmaz; kayıttan sonra yeni anahtarların etkin olabilmesi için ilgili API Proxy, API Proxy Group veya Politika Grubu'nun yeniden yüklenmesi (redeploy) gerekir. Bu politikayı kullanan mevcut istemcilerin de yeni Client Id/Client Secret değerleriyle yeniden yapılandırılması gerekir.

:::

### Ekranlar Arası Tutarlılık

Bu Client Id / Client Secret görüntüleme, kopyalama ve yeniden oluşturma deneyimi (Otomatik/Manuel seçimi, isim yazarak onaylama, son onay adımı dahil); **API Proxy**, **Politika Grubu (Policy Group)** ve **API Proxy Group** ekranlarındaki kendi anahtar yönetimi bölümleriyle birebir aynıdır. Bu JWT Kimlik Doğrulama Politikası bir API Proxy'nin, bir API Proxy Group'un veya bir Politika Grubu'nun politika listesine bağlandığında — global (paylaşılan) bir politika olarak seçilmiş olsun ya da o API/Grup'a özel yerel bir politika olarak oluşturulmuş olsun — politika formunda görünen **API Anahtarını Göster** seçeneği aynı deneyimi sunar.
