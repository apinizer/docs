...
| **Adım 4: JWT Kimlik Doğrulama Parametreleri** | - **Managed From This Policy**: Credentials yönetimini politika mı kontrol edecek? Secret Manager dışı entegrasyonlarda `true` seçilir.<br />- **Grant Type**: CLIENT_CREDENTIALS veya PASSWORD seçin; seçim, kullanılabilir kimlik kaynaklarını belirler.<br />- **Kimlik Kaynağı**: PASSWORD için Identity Role Group modalından LDAP/Database/API/OIDC veya Secret Manager seçimi yapın. OIDC seçildiğinde aranabilir bir [OIDC Kimlik Sağlayıcı](/tr/admin/identity-management/identity-providers/oidc-provider) listesi açılır (ad, açıklama, sağlayıcı türü); seçilen kaydın token/claim eşlemesi üzerinden kimlik doğrulaması yapılır. Generic türündeki bir sağlayıcı da burada geçerli bir seçimdir — yalnızca token doğrulaması kullanılır, senkronizasyon devreye girmez. (JWT Yenileme Servisi seçiminde OIDC bir kimlik sağlayıcı olduğu için bu listede yer almaz — yenileme servisi yalnızca API tipi bir servis olabilir.)<br />- **Check Client IP Address**: Secret Manager tabanlı yapılarda isteğin geldiği IP'yi doğrular. |
