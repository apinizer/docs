...
| **Adım 4: Request (Before Call) Yapılandırması** | **BODY Sekmesi:**<br /><br />**Clear Body Before Call:**<br />Toggle ile aktif/pasif<br />- Aktif: Gelen istek body'si temizlenir.<br />- Pasif: Gelen body aynen kullanılır.<br /><br />**Use Message Template (Clear Body aktifse):**<br />- Content Type: XML, JSON, RAW, URL_ENCODED<br />- Body Content: Textarea'ya şablon girin. Variable kullanabilirsiniz: `${variable.name}`<br />- URL Encoded seçiliyse: Key-Value tablosu görünür.<br /><br />**Request Data Manipulation:**<br />- [+ Add] butonuna tıklayın.<br />- Operation: ADD, REPLACE, DELETE<br />- Source: Value veya Variable<br />- Target: Value veya Variable (ADD'de targetName zorunlu)<br /><br />**HEADER Sekmesi:**<br /><br />**Remove All Headers Before Call:**<br />Toggle<br />- Aktif: Tüm header'lar silinir.<br />- Pasif: Header'lar korunur.<br /><br />**Deleted Headers (Remove All pasifse):**<br />- Silinecek header adlarını liste halinde ekleyin: `X-Old-Header`<br /><br />**New Headers:**<br />- Name, Description, Value/Variable, Prefix (Variable seçiliyse: BEARER, BASIC vb.)<br />- Örnek: Name: `Authorization`, Value: `Bearer ${token.jwt}`, Prefix: BEARER<br /><br />**PARAMETER Sekmesi:**<br />- Header ile aynı mantık. Query parameter'leri için kullanılır.<br /><br />**CACHE Sekmesi:**<br /><br />**Enable Cache:**<br />Toggle<br />**Apply By (Cache By):**<br />Variable seçin (örn: `user.id`, `product.category`)<br />**Capacity Zorunlu:**<br />Örnek: `1000` (1000 kayıt)<br />**TTL Zorunlu:**<br />Saniye cinsinden. Örnek: `300` (5 dakika)<br />**Cache Storage Type:**<br />DISTRIBUTED (dağıtık) veya LOCAL (yerel)<br />**Cache Null Responses:**<br />Checkbox - Null yanıtlar da cache'lensin mi?<br /><br />**Dinamik TTL (Dynamic TTL):**<br />Toggle ile aktif edin. Aktif edildiğinde cache TTL değeri, API çağrısının yanıtından dinamik olarak hesaplanır. Detaylı açıklama için [Dinamik TTL](#dinamik-ttl-dynamic-ttl) bölümüne bakın.<br /><br />**Yanıt Doğrulaması (Response Assertion):**<br />Toggle ile aktif edin. Sunucu HTTP 200 dönüp gövdede iş hatası bildirdiğinde, hatalı yanıtın cache'lenmesini engeller. Detaylı açıklama için [Yanıt Doğrulaması](#response-assertion) bölümüne bakın.<br /><br />**SETTINGS Sekmesi:**<br /><br />**Timeout:**<br />Saniye cinsinden, zorunlu. İstek süresini bu sekmeden yönetebilirsiniz; üst bölümdeki çağrı timeout değeriyle tutarlı tutmanız önerilir.<br /><br />**Güvenli bağlantı (mTLS) ayarları:**<br />Başlık altında güvenli bağlantıyı **etkinleştir** anahtarı ile açın. Açıkken özet alanında KeyStore durumu, tanımlıysa TrustStore ve sunucu sertifikası (PEM), seçilen TLS protokolleri ve hostname doğrulayıcı türü görüntülenir. Ayrıntılı düzenleme için **Yapılandır** (dişli) düğmesine tıklayın; güvenli bağlantı kapalıyken düğme devre dışıdır.<br /><br />**Yapılandır** penceresinde: **SSL doğrulamayı atla** açıksa sunucu sertifikası doğrulanmaz ve TrustStore, KeyStore ile sertifika panelleri kullanılamaz; yalnızca güvenilir test ortamlarında değerlendirin. Kapalıyken **TrustStore** ve **KeyStore** için havuzdaki kaydı seçebilir veya **+** ile yeni KeyStore oluşturabilirsiniz; **Sertifika** için PEM sertifikayı havuzdan seçebilir veya yeni sertifika tanımlayabilirsiniz. **Protokol ayarları** bölümünde desteklenen TLS sürümlerini ve hostname doğrulayıcıyı seçebilirsiniz. Bu politika ekranında dosya yükleme yerine havuzdan seçim ve yeni kayıt oluşturma kullanılır; pencerenin altında uyarı metni gösterilir. **Kaydet** ile değişiklikleri uygulayıp pencereyi kapatırsınız. |
...
Dinamik TTL, cache'in yaşam süresini (TTL) sabit bir değer yerine API yanıtından gelen bir değere göre otomatik hesaplar. Örneğin, backend servisiniz yanıtta bir `expireAt` alanı dönüyorsa, cache bu tarihe kadar geçerli olur.
...
| **Alan** | **Açıklama** |
|:---------|:-------------|
| **Dinamik TTL Etkinleştir** | Toggle ile aktif/pasif. Aktif edildiğinde sabit TTL alanı gizlenir ve dinamik TTL ayarları görünür. |
| **TTL Kaynak Değişkeni** | TTL bilgisinin okunacağı değişken. Response header, response body (JSONPath/XPath) veya başka bir context variable olabilir. Örnek: Response body'deki `$.expireAt` alanı. |
| **Değer Formatı** | Seçilen değişkenden okunan değerin formatını belirtir. Aşağıdaki formatlar desteklenir: |

**Desteklenen Değer Formatları:**

| **Format** | **Açıklama** | **Örnek Değer** |
|:-----------|:-------------|:----------------|
| **ISO 8601 DateTime** | ISO 8601 standart tarih formatı. UTC, offset veya timezone'suz kabul edilir. | `2026-03-26T15:30:00Z`, `2026-03-26T15:30:00+03:00`, `2026-03-26T15:30:00`, `2026-03-26` |
| **Özel Tarih Formatı** | Java DateTimeFormatter pattern'i ile özel format. Seçildiğinde ek bir pattern alanı görünür. | Pattern: `dd.MM.yyyy HH:mm:ss` → Değer: `26.03.2026 15:30:00` |
| **Unix Epoch (saniye)** | Unix epoch timestamp (saniye cinsinden). | `1743004200` |
| **Unix Epoch (milisaniye)** | Unix epoch timestamp (milisaniye cinsinden). | `1743004200000` |
| **Expires In (saniye)** | Yanıtın kaç saniye geçerli olduğunu belirten göreli süre. | `300` (5 dakika) |
| **Expires In (milisaniye)** | Yanıtın kaç milisaniye geçerli olduğunu belirten göreli süre. | `300000` (5 dakika) |
| **Süre String** | İnsan okunabilir süre string'i. | `1h`, `30m`, `3600s` |

#### Ek Ayarlar

| **Alan** | **Açıklama** |
|:---------|:-------------|
| **Değer JWT Token İçinde** | Checkbox. Seçildiğinde kaynak değişkendeki değer önce JWT token olarak decode edilir, ardından belirtilen claim'den TTL değeri okunur. |
| **JWT Claim Adı** | JWT decode sonrası okunacak claim adı. Basit claim adı (örn: `exp`) veya JSONPath (örn: `$.data.expireAt`) desteklenir. |
| **Offset (saniye)** | Hesaplanan TTL'den düşülecek güvenlik payı (saniye). Örneğin 30 saniye offset ile cache, expire time'dan 30 saniye önce geçersiz olur. Saat farkı veya ağ gecikmesini telafi etmek için kullanılır. |
| **Yedek TTL (saniye)** | Dinamik TTL hesaplanamadığında (parse hatası, değişken bulunamadı vb.) kullanılacak yedek TTL değeri (saniye cinsinden). Tanımlanmazsa politikanın sabit TTL değeri kullanılır. |
...
:::note Süresi geçmiş tarihler
Hesaplanan TTL sıfır veya negatifse (kaynak değişkendeki tarih geçmişte kalmışsa) yanıt **cache'lenmez**. Bazı servisler başarısız yanıtlarda `0001-01-01T00:00:00` gibi "boş tarih" değerleri döner; bu değerler de geçmiş tarih sayılır ve cache dışı bırakılır.
:::

### Yanıt Doğrulaması (Response Assertion) {#response-assertion}

Bazı servisler hata durumunda da **HTTP 200** döner ve başarısızlığı yalnızca yanıt gövdesindeki bir alanla bildirir:

```json
{ "Token": "", "ReturnCode": 330, "ErrorMessage": "Bad Request" }
```

Bu tür yanıtlar HTTP durum koduna bakılarak ayırt edilemez; cache açıksa hatalı yanıt da cache'lenir ve TTL boyunca tüm isteklere aynı hata döner. Yanıt doğrulaması, başarıyı gövdedeki (veya header'daki) bir değerden anlamanızı sağlar.

Doğrulama **varsayılan olarak kapalıdır**. Açmadığınız sürece mevcut yapılandırmalar bugünkü davranışını birebir korur.

#### Yapılandırma Alanları

| **Alan** | **Açıklama** |
|:---------|:-------------|
| **Yanıt Doğrulaması Etkinleştir** | Ana anahtar. Kapalıyken her yanıt (HTTP başarılı olduğu sürece) cache'lenir. |
| **Doğrulama Değişkeni** | Karşılaştırılacak değerin okunacağı değişken. Response header, response body (JSONPath/XPath) veya başka bir context variable olabilir. Örnek: `$.ReturnCode`. |
| **Karşılaştırma** | `Eşittir`, `Eşit Değildir`, `İçerir`, `İçermez`, `Dolu (Değer Var)`, `Boş (Değer Yok)`, `Regex ile Eşleşir`. |
| **Beklenen Değer** | Karşılaştırılacak değer. `Dolu`/`Boş` operatörlerinde istenmez. Ortam değişkeni (`${degisken}`) ve context değişkeni (`#{degisken}`) kullanılabilir; değer her istekte yeniden çözülür. Böylece beklenen değer ortamdan ortama (dev/test/prod) farklı olabilir. |

#### Doğrulama Başarısız Olursa Ne Olur

Yanıt **cache'lenmez**; akış normal şekilde devam eder ve hatalı yanıt çağıran tarafa iletilir. İstek gateway'de durdurulmaz.

Bu davranış, sık karşılaşılan bir sorunu önler: geçici bir backend hatasının cache'e yazılıp TTL boyunca tüm isteklere dönmesi.

#### Örnek Senaryo

Yukarıdaki token servisi için:

1. **Yanıt Doğrulaması Etkinleştir**: Açık
2. **Doğrulama Değişkeni**: Response body → JSONPath: `$.ReturnCode`
3. **Karşılaştırma**: Eşittir
4. **Beklenen Değer**: `0`

`ReturnCode` `0` dönen başarılı yanıtlar cache'lenir; `330` gibi hata kodu dönen yanıtlar cache dışı bırakılır ve bir sonraki istek backend'e gider.

:::tip Dinamik TTL ile birlikte kullanım
İki özellik birbirini tamamlar. Backend'iniz hatalı yanıtlarda geçerli bir sona erme tarihi dönmüyorsa dinamik TTL zaten cache'i engeller; ancak başarı/başarısızlık ayrımı gövdedeki bir koda dayanıyorsa yanıt doğrulamasını da açmanız önerilir.
:::

### Hata Alınırsa Yeniden Deneme (Retry) {#retry}

Hedef servise yapılan çağrı geçici bir hatayla düştüğünde — bağlantı zaman aşımı, DNS veya TLS hatası, ya da karşı tarafın anlık `502`/`503`/`504` yanıtı — isteğin tamamını hataya düşürmek yerine çağrı otomatik olarak yeniden gönderilebilir.

Retry **varsayılan olarak kapalıdır**. Siz açana kadar mevcut yapılandırmalar bugünkü davranışını birebir korur.

#### Yapılandırma Alanları

| **Alan** | **Açıklama** |
|:---------|:-------------|
| **Hata alınırsa yeniden dene** | Ana anahtar. Kapalıyken çağrı eskisi gibi tek kez denenir. |
| **Yeniden Deneme Sayısı** | İlk denemeden sonra yapılacak ek deneme sayısı. Toplam istek sayısı bu değerin bir fazlasıdır. |
| **Bekleme Stratejisi** | `Gecikme Yok`, `Sabit Gecikme` veya `Üstel Bekleme`. |
| **Sabit Bekleme (ms)** | Her yeniden denemeden önce beklenecek süre. Sabit Gecikme seçiliyken görünür. |
| **İlk Bekleme (ms)** | İlk yeniden denemeden önceki bekleme. Üstel Bekleme seçiliyken görünür. |
| **En Fazla Bekleme (ms)** | Artan bekleme süresinin üst sınırı; böylece gecikme sınırsız büyümez. |
| **Artış Çarpanı** | Her adımda uygulanan büyüme katsayısı (1'den büyük olmalıdır). |
| **Yeniden Denenecek Durum Kodları** | Hangi HTTP durum kodlarının yeniden denemeyi tetikleyeceği. Boş bırakılırsa `502`, `503` ve `504` kullanılır. |
| **POST/PATCH çağrılarında sunucu hatasında da yeniden dene** | Varsayılan kapalı. Aşağıdaki uyarıya bakınız. |

#### Hangi Hatalarda Yeniden Denenir

Karar yalnızca durum koduna değil, **isteğin ne kadar ilerlediğine** göre verilir:

| **Durum** | **Anlamı** | **Yeniden denenir mi?** |
|:----------|:-----------|:------------------------|
| Bağlantı hiç kurulamadı (bağlantı reddedildi, connect timeout, DNS hatası, TLS el sıkışma hatası) | İstek **gönderilmedi** | Evet, her HTTP metodunda |
| İstek gönderildi ama yanıt alınamadı (read timeout, bağlantının kapanması) | Servis isteği **işlemiş olabilir** | Yalnızca idempotent metotlarda |
| Servis, listedeki bir hata koduyla yanıt verdi | Servis isteği **işlemiş olabilir** | Yalnızca idempotent metotlarda |
| Diğer durumlar (örneğin `400`, `401`, `404`) | Kalıcı hata, tekrar denemek çözmez | Hayır |

RFC 7231'e göre `GET`, `HEAD`, `OPTIONS`, `PUT` ve `DELETE` idempotenttir; `POST` ve `PATCH` değildir.

:::warning

**POST/PATCH çağrılarında sunucu hatasında da yeniden dene** seçeneğini yalnızca hedef servis tekrarlanan isteklere karşı güvenliyse açın. Servis, isteği *işledikten sonra* hata döndürmüşse yeniden göndermek çift kayıt veya çift ödeme oluşturabilir. Bağlantı hiç kurulamadığında ise istek karşı tarafa ulaşmamıştır; bu durumda ayardan bağımsız olarak yeniden denenir.

:::

#### Cache ile Etkileşimi

Retry ve yanıt cache'i birbirinden ayrı eksenlerde çalışır, birbirini bozmaz:

- Yanıt **cache'ten** karşılandığında hiç HTTP çağrısı yapılmaz; dolayısıyla retry hiç devreye girmez.
- Cache, denemeler başlamadan **bir kez** sorgulanır; yeniden denemeler cache sorgusunu tekrarlamaz.
- Cache'e yalnızca **başarılı** yanıt yazılır. Tüm denemeler başarısız olursa hiçbir şey cache'lenmez ve politika bugünkü gibi hata verir.
- Dinamik TTL kullanılıyorsa TTL, **son ve başarılı** yanıttan hesaplanır; başarısız denemeler TTL'yi etkilemez.
- **Cache servisine** erişilememesi ayrı bir konudur ve retry ile değil, cache hata davranışı ayarıyla yönetilir.

:::caution

Her deneme, tanımlı zaman aşımı kadar sürebilir ve beklemeler bunun üzerine eklenir. 30 saniye zaman aşımı ve 2 yeniden deneme ile çağrı yaklaşık 90 saniye sürebilir. Bu sürenin API proxy'nin genel zaman aşımını aşmamasına dikkat edin — politika ekranı, değerleri değiştirdikçe hesaplanan en kötü durumu gösterir.

:::

#### Trace Üzerinde Görünürlüğü

Trace açıkken her deneme ayrı ayrı kaydedilir. Yürütme detayında toplam deneme sayısı ve her deneme için sonucu, hata sınıfı, süresi ve öncesinde beklenen süre görünür; böylece uzun süren bir istek sonradan açıklanabilir.

### Sıkıştırma (Compression)

Politika varsayılan olarak hedef servisle sıkıştırma pazarlığı yapmaz: `Accept-Encoding` başlığı gönderilmez ve yanıt geldiği biçimde iletilir. Pazarlığı **Ayarlar** sekmesindeki **Sıkıştırmayı Etkinleştir** seçeneğiyle açabilirsiniz.

Etkinleştirildiğinde çağrı, hedef servise kabul ettiği sıkıştırma biçimlerini (gzip, deflate, br, zstd ve platformda bulunan diğer biçimler) bildirir ve sıkıştırılmış bir yanıt geldiğinde politikanın geri kalanı çalışmadan önce yanıtı otomatik olarak açar. Büyük yanıtlarda bu, aktarılan veri miktarını azaltır.

#### Ne Zaman Etkinleştirmelisiniz

| **Durum** | **Öneri** |
|:------------|:------------------------|
| Hedef servis yavaş veya ücretli bir hat üzerinden büyük yanıtlar dönüyor | Etkinleştirin — yanıt sıkıştırılmış olarak taşınır ve otomatik açılır. |
| Hedef servis, çağıranın kabul ettiği biçime bakmaksızın sıkıştırılmış yanıt dönüyor | Etkinleştirin — aksi hâlde yanıt sıkıştırılmış biçimde taşınır ve sonraki adımlar okuyamaz. |
| Hedef servis düz yanıt dönüyor | Kapalı bırakın — varsayılan budur ve gereksiz pazarlığı önler. |

:::note

`Accept-Encoding` başlığı yalnızca bu ayarla yönetilir. **Header** sekmesinde bu başlık için tanımladığınız bir değer hedef servise iletilmez.

:::

:::caution

Sıkıştırma varsayılan olarak kapalıdır; bu nedenle bu ayar eklenmeden önce yapılandırılmış API Çağrısı politikaları yükseltmeden sonra `Accept-Encoding` göndermeyi bırakır. Hedef servis, çağıranın kabul ettiği biçime bakmadan sıkıştırılmış yanıt dönüyorsa, o servisi çağıran politikalarda bu ayarı etkinleştirin.
