...
MongoDB'ye bağlanan modüller (API Manager, Worker, Cache, Integration) bağlantı bilgilerini **base64** kodlu bekler:
...
`base64 -w 0` Linux/Git Bash içindir. PowerShell'de: `$MONGO_URL = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes("mongodb://..."))`
...
| `mongo.secretName` | `mongo-db-credentials` | MongoDB secret adı |
| `mongo.dbUrl` | — | MongoDB bağlantı URL'i (base64) |
| `mongo.dbName` | — | MongoDB veritabanı adı (base64) |
...
Cache, Worker ile **aynı namespace'e** kurulacaksa kurulum komutuna aşağıdaki parametre eklenmelidir:
...
Portal credential'ları base64 kodlu verilir. `apiKey` ve `apiPortalId` değerleri, API Manager arayüzünde portal kaydı yapıldıktan sonra üretilir:
...
| `resources.limits.cpu` | `1` | CPU limiti |
| `resources.limits.memory` | `1Gi` | Bellek limiti |
| `portalSecret.secretName` | `apinizer-portal` | Portal secret adı |
| `portalSecret.apinizerManagementApiBaseUrl` | — | API Manager Management API adresi (base64) |
| `portalSecret.apiKey` | — | Portal API anahtarı (base64) |
| `portalSecret.apiPortalId` | — | Portal kimliği (base64) |
