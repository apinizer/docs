...
Apinizer'ın oluşturduğu pod'lar DNS `ndots` ayarını taşır. Ayar, Publish / Republish sırasında eklenir. Mevcut bir deployment'ta eksikse, kaynak, güvenlik bağlamı, sağlık probu veya ortam değişkeni düzenlemesi de ayarı ekler. Operatörün elle verdiği `ndots` değeri korunur. Salt Restart veya Yayınla/Yayından Kaldır (replica ölçekleme) işlemi ayarı eklemez; bu durumda yukarıdaki düzenlemelerden birini uygulayın ya da Yeniden Yayınlayın. Detay ve cluster-içi adreslerde FQDN kullanım önerisi için → [Kubernetes DNS sorunları](/tr/operations/sorun-giderme/kubernetes-docker-containerd-sorunlari) sayfasına bakın.
...
:::
    ...
    **Ön koşul:** Konnektör seçilebilmesi için önce Sistem Ayarları → **Bağlantı Yönetimi** sayfasından konnektör tanımı yapılmış olmalıdır.
    ...
    Detay için: [Log Konnektörü Eklenmesi](/tr/admin/server-management/gateway-ortamlarina-konnektor-eklenmesi).
...
**Ek Değişkenler**
          ...
          * Eski bayraklar (flag) **-XX: &#123;Min \| Max&#125; RAMFraction** artık kullanımdan kaldırıldı. 0.0 ve 100.0 arasında bir değer alan ve varsayılan olarak 75.0 olan yeni bir **-XX: MaxRAMPercentage** bayrağı (flag) vardır. Bu nedenle, 2 GB bellek sınırı varsa, JVM yığını (heap) varsayılan olarak ~ 1,5 GB ile sınırlıdır.
...
API Manager'ın kurulu olması gerekmektedir. Ayrıca lisans anahtarınızda API Developer Portal'ının bulunması gerekmektedir. Portal'ın Management API anahtarının sahibi olan Manager kullanıcısı **Portal Ayarları → Güvenlik → Management API Anahtarı Kullanıcıları** listesinde olmalıdır (liste boşsa bir portal rolü taşımalıdır); aksi halde Portal'ın çağrıları 403 ile reddedilir.
...
Tüm modüller için bellek limiti en az **2 GB** olmalıdır. CPU limiti 1 core olsa bile 1 GB bellek ile JVM açılmaz. Cache, Integration ve API Portal chart'larının varsayılan `resources.limits.memory` değeri `1Gi` olduğundan bu değeri kurulumda mutlaka `2Gi` veya üzerine çıkarın.
...
| `resources.limits.cpu` | `1` | CPU limiti |
| `resources.limits.memory` | `1Gi` | Bellek limiti. Chart varsayılanı minimumun altındadır; CPU 1 core olsa bile en az `2Gi` verin, JVM 1 GB ile açılmaz |
| `mongo.secretName` | `mongo-db-credentials` | MongoDB Secret adı (Helm dışı, önceden oluşturulur) |
