...
- [Docker Kurulumu](/tr/setup/docker/genel-bakis)
...
### Gelişmiş Undertow Sunucu Seçenekleri

Bu değişkenler alttaki HTTP sunucusunu ince ayarlar. İlk grubun Apinizer tarafından yönetilen varsayılanları vardır; geri kalan seçenekler yalnızca ilgili değişkeni tanımladığınızda uygulanır — tanımlamadığınızda sunucu yerleşik varsayılanını korur ve davranış değişmez. Tanımlanan her değer Worker başlangıç loguna yazılır.

| Değişken | Hedef Ortam | Açıklama |
|----------|-------------|----------|
| `tuneAlwaysSetDateHeader` | HTTP Worker, HTTP+Websocket, Management API | Her yanıta Date header'ı eklenip eklenmeyeceği (varsayılan: true) |
| `tuneMaxEntitySize` | HTTP Worker, HTTP+Websocket, Management API | Tek bir istek gövdesinin maksimum boyutu (byte); -1 sınırsız anlamına gelir (varsayılan: -1) |
| `tuneEnableRfc6265CookieValidation` | HTTP Worker, HTTP+Websocket, Management API | Katı RFC 6265 çerez doğrulamasını etkinleştirir; devre dışıyken esnek/eski çerez biçimleri kabul edilir (varsayılan: false) |
| `tuneAllowUnescapedCharactersInUrl` | HTTP Worker, HTTP+Websocket, Management API | İstek URL'lerinde kaçışsız karakterlere izin verir (varsayılan: true) |
| `tuneAllowEncodedSlash` | HTTP Worker, HTTP+Websocket, Management API | Yol segmentleri içindeki kodlanmış slash karakterlerini korur (varsayılan: true) |
| `tuneWorkerTaskKeepAlive` | HTTP Worker, HTTP+Websocket, Management API | Worker görev thread'lerinin geri alınmadan önce boşta tutulma süresi (milisaniye) (varsayılan: 180000) |
| `tuneAlwaysSetKeepAlive` | HTTP Worker, HTTP+Websocket, Management API | Kesin gerekli olmasa bile keep-alive header'ının her zaman eklenip eklenmeyeceği (sunucu varsayılanı: true) |
| `tuneMaxHeaderSize` | HTTP Worker, HTTP+Websocket, Management API | İstek header'larının toplam maksimum boyutu (byte) (sunucu varsayılanı: 1048576) |
| `tuneMaxHeaders` | HTTP Worker, HTTP+Websocket, Management API | Maksimum istek header sayısı (sunucu varsayılanı: 200) |
| `tuneMaxParameters` | HTTP Worker, HTTP+Websocket, Management API | Maksimum query ve path parametre sayısı (sunucu varsayılanı: 1000) |
| `tuneMaxCookies` | HTTP Worker, HTTP+Websocket, Management API | İstek başına maksimum çerez sayısı (sunucu varsayılanı: 200) |
| `tuneMaxBufferedRequestSize` | HTTP Worker, HTTP+Websocket, Management API | İstek işleme sırasında maksimum buffer'lanan istek boyutu (byte) (sunucu varsayılanı: 16384) |
| `tuneMaxConcurrentRequestsPerConnection` | HTTP Worker, HTTP+Websocket, Management API | Tek bir bağlantı üzerindeki maksimum eşzamanlı istek sayısı; yalnızca tanımlandığında uygulanır |
| `tuneMaxQueuedReadBuffers` | HTTP Worker, HTTP+Websocket, Management API | Bağlantı başına maksimum kuyruğa alınan okuma buffer sayısı; yalnızca tanımlandığında uygulanır |
| `tuneMaxCachedHeaderSize` | HTTP Worker, HTTP+Websocket, Management API | Önbelleğe alınabilecek bir header'ın maksimum boyutu (byte) (sunucu varsayılanı: 150) |
| `tuneHttpHeadersCacheSize` | HTTP Worker, HTTP+Websocket, Management API | Header önbelleğinde tutulan HTTP header sayısı (sunucu varsayılanı: 15) |
| `tuneRequestParseTimeout` | HTTP Worker, HTTP+Websocket, Management API | Bir isteğin ayrıştırılması için izin verilen maksimum süre (milisaniye); yalnızca tanımlandığında uygulanır (aksi halde timeout yok) |
| `tuneIdleTimeout` | HTTP Worker, HTTP+Websocket, Management API | Bağlantı boşta kalma timeout'u (milisaniye); yalnızca tanımlandığında uygulanır (aksi halde timeout yok) |
| `tuneShutdownTimeout` | HTTP Worker, HTTP+Websocket, Management API | Kapanış sırasında devam eden isteklerin beklenmesi için maksimum süre (milisaniye); yalnızca tanımlandığında uygulanır |
| `tuneRecordRequestStartTime` | HTTP Worker, HTTP+Websocket, Management API | Her isteğin başlangıç zamanını kaydeder; belirli gecikme metrikleri ve erişim logları için gereklidir (sunucu varsayılanı: false) |
| `tuneEnableStatistics` | HTTP Worker, HTTP+Websocket, Management API | Sunucu düzeyinde istatistik toplamayı etkinleştirir (sunucu varsayılanı: false) |
| `tuneAllowEqualsInCookieValue` | HTTP Worker, HTTP+Websocket, Management API | Çerez değerleri içinde eşittir karakterine izin verir (sunucu varsayılanı: false) |
| `tuneRequireHostHttp11` | HTTP Worker, HTTP+Websocket, Management API | HTTP/1.1 isteklerinde Host header'ı zorunlu kılar (sunucu varsayılanı: false) |
| `tuneAllowUnknownProtocols` | HTTP Worker, HTTP+Websocket, Management API | Bilinmeyen HTTP protokolleri kullanan isteklere izin verir (sunucu varsayılanı: false) |
| `tuneBufferPipelinedData` | HTTP Worker, HTTP+Websocket, Management API | Pipeline'lanmış istek verisini buffer'lar (sunucu varsayılanı: true) |
| `tuneDecodeUrl` | HTTP Worker, HTTP+Websocket, Management API | İstek URL'ini yapılandırılan karakter setiyle çözer (sunucu varsayılanı: true) |
| `tuneDecodeSlash` | HTTP Worker, HTTP+Websocket, Management API | URL'deki kodlanmış slash karakterlerini çözer (sunucu varsayılanı: false) |
| `tuneSslUserCipherSuitesOrder` | HTTP Worker, HTTP+Websocket, Management API | TLS anlaşması sırasında sunucunun şifre paketi sırasını uygular (sunucu varsayılanı: false) |
| `tuneTrackActiveRequests` | HTTP Worker, HTTP+Websocket, Management API | O an aktif olan istek sayısını izler (sunucu varsayılanı: false) |

:::info

"Yalnızca tanımlandığında uygulanır" veya "sunucu varsayılanı" olarak işaretlenen seçenekler yalnızca değişkeni açıkça tanımladığınızda devreye girer. Tanımlanmadığında sunucu yerleşik varsayılanını korur ve davranış önceki sürümlerle birebir aynıdır.
