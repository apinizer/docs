...
```text
default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data:; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'
```

`'unsafe-inline'` ve `'unsafe-eval'` bilinçli olarak korunmuştur. **Portal Görünüm** önizlemesi, kendi portal HTML'inizin içindeki `<script>` bloklarını çalıştırır; Angular da çalışma anında satır içi `<style>` öğeleri enjekte eder. Bu değerlerin kaldırılması başka bir şeyi sıkılaştırmaz, yalnızca önizlemeyi bozar. Diğer direktifler — `object-src`, `base-uri`, `form-action` ve `frame-ancestors` — eklenti içeriğini, `<base>` kaçırmayı, dış siteye form göndermeyi ve çerçevelenmeyi engeller.

Politika ayarlanabilir; Portal Görünüm önizlemesinde özel script kullanmayan bir kurulum, kod değişikliği yapmadan politikayı sıkılaştırabilir. `conf/application.env` içinde (standalone) veya bir container ortam değişkeni olarak (Docker, Kubernetes):

```bash
APINIZER_APIMANAGER_SECURITY_CONTENT_SECURITY_POLICY="default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data:; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'"
```
...
Yukarıdaki örnek sıkı sürümdür: yalnızca `script-src 'self'`. Bedeli, Portal Görünüm önizlemesine gömülü özel script'lerin artık çalışmamasıdır; Manager konsolunun geri kalanı etkilenmez. Boş bırakılan ya da hiç verilmeyen değer, en üstte gösterilen varsayılan politikaya döner — değişkeni temizlemek Manager'ı politikasız bırakmaz. Değişiklikten sonra API Manager'ı yeniden başlatın.
