...
### HTTP Güvenlik Başlıkları (Content-Security-Policy)

API Manager her yanıtla birlikte bir `Content-Security-Policy` başlığı gönderir; yanında `X-Frame-Options: DENY`, `Referrer-Policy` ve `Permissions-Policy` de bulunur. Varsayılan politika:
...
`script-src` yalnızca `'self'` değerini taşır. Manager konsolu satır içi ya da çalışma anında değerlendirilen (eval) script çalıştırmaz; **Portal Sayfaları** / **Portal Görünüm** önizlemesi portal HTML'inizi içindeki `<script>` bloklarını çalıştırmadan gösterir. Bu script'ler API Portal uygulamasının kendisinde çalışır ve bu başlık onu kapsamaz; dolayısıyla özel portal script'leri etkilenmez.

`style-src` hâlâ `'unsafe-inline'` değerine izin verir: Angular bileşen stillerini çalışma anında satır içi `<style>` öğeleri olarak enjekte eder. Bunun istek başına üretilen bir nonce ile değiştirilmesi ayrı ve daha büyük bir iştir; o gelene kadar güvenlik tarayıcıları stil direktifini gevşek olarak raporlamaya devam eder. Diğer direktifler — `object-src`, `base-uri`, `form-action` ve `frame-ancestors` — eklenti içeriğini, `<base>` kaçırmayı, dış siteye form göndermeyi ve çerçevelenmeyi engeller.

Politika ayarlanabilir. `conf/application.env` içinde (standalone) veya bir container ortam değişkeni olarak (Docker, Kubernetes) — aşağıdaki örnek ek olarak güvenilen bir CDN'den görsel yüklenmesine izin verir:

```bash
APINIZER_APIMANAGER_SECURITY_CONTENT_SECURITY_POLICY="default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://cdn.example.com; font-src 'self' data:; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'"
```

Aynı ayarın `application.yml` karşılığı:

```yaml
apinizer-apimanager:
  security:
    content-security-policy: "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://cdn.example.com; font-src 'self' data:; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'"
```

Boş bırakılan ya da hiç verilmeyen değer, en üstte gösterilen varsayılan politikaya döner — değişkeni temizlemek Manager'ı politikasız bırakmaz. Değişiklikten sonra API Manager'ı yeniden başlatın. Bu değişkeni daha önceki "sıkı" örnekle (`script-src 'self'`) ayarladıysanız kaldırabilirsiniz: varsayılan artık o politikadır.
