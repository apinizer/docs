...
description: "Ubuntu 24.04 LTS (Noble) işletim sistemine sahip sunucularda MongoDB Replicaset 8.0 kurulumunu gerçekleştirebilir. Kurulum sürecinde işletim sistemi konfigürasyonları, MongoDB uygulamasının kurulması, replicaset yapılandırması ve yüksek kullanılabilirlik ayarları yapılabilir."
---
...
**Önemli**
  ...
  **MongoDB:**
  - [https://pgp.mongodb.com/server-8.0.asc](https://pgp.mongodb.com/server-8.0.asc)
...
MongoDB servisini başlatmadan önce `dbPath` dizininin mevcut olduğundan ve sahiplik/izinlerinin doğru ayarlandığından emin olun:
...
`/etc/mongod.conf` içinde `dbPath` farklı bir dizin olarak tanımlandıysa yukarıdaki komutlarda `/var/lib/mongodb` yerine o dizini kullanın.
...
```javascript
rs.initiate({
  _id: "apinizer-replicaset",
  members: [{ _id: 0, host: "<Mongodb_Ip1>:25080" }]
})
rs.status()
```
...
Priority, node'un yeni Primary olarak seçilme önceliğini belirtir ve bu değer 0 ile 1000 arasında bir tam sayıdır. 0 değeri node'un hiçbir zaman Primary olamayacağını belirtir; daha yüksek değerler Primary seçiminde daha yüksek öncelik sağlar. Varsayılan değer 1'dir.
