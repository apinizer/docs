...
description: "Apinizer için MongoDB ReplicaSet 8.0 kurulumu. Ubuntu ve Red Hat Enterprise Linux / Rocky Linux dağıtımları için adım adım kurulum, yapılandırma, ReplicaSet ve yüksek kullanılabilirlik ayarları."
---

Apinizer için MongoDB ReplicaSet 8.0 kurulumu. İşletim sisteminize göre aşağıdaki sekmeden devam edin.

<Info>
Apinizer için MongoDB **özel port `25080`** kullanır. Bu yapılandırma `/etc/mongod.conf` içindeki `net.port` ile sağlanır.
</Info>
  ...
  <Tab title="Ubuntu">
    ...
    <Info>
      **Önemli**
      ...
      İçinde `tr.` olan adreslerin hepsini "Replace All" ile değiştirin. Örnek:
