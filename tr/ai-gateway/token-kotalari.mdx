...
Token kotaları APIops REST API üzerinden görüntülenebilir ve güncellenebilir; bkz. [API Referansı: AI Bütçeleri](/api-reference/ai-budgets). Bu uçlar **tavanları** anlatır. Bu tavanlara karşı gerçekte ne tüketildiği — kurum, tüketici, uygulama, API istemcisi veya API proxy kapsamında token, ABD doları cinsinden maliyet, kalan pay, yüzde ve sıfırlanma zamanı — [Kullanım Raporları API'si](/api-reference/usage-reports) ile okunur; geri faturalama dışa aktarımları ve bütçe alarmı işleri için tasarlanmış beş salt-okunur `GET` ucudur.
