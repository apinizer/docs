...
Token kotaları **APIops REST API** ile de görüntülenip güncellenebilir:

| Metod | Yol | Açıklama |
|-------|-----|----------|
| `GET` | `/apiops/projects/{projectName}/ai-budgets/scope-rules/` | Projede tanımlı kota kurallarını (kimlik bilgisi/kuruluş/uygulama/proje) düz liste olarak döner |
| `GET` | `/apiops/projects/{projectName}/ai-budgets/hierarchy/` | Kapsam hiyerarşisini, her düğümün miras aldığı ve etkin değerleriyle birlikte ağaç yapısında döner |
| `GET` | `/apiops/projects/{projectName}/ai-budgets/hierarchy/{nodeType}/{nodeId}/effective-limit/` | Tek bir düğüm için **fiilen uygulanan** (tüm katmanların en düşüğü) sınırı hesaplayıp döner |
| `PUT` | `/apiops/projects/{projectName}/ai-budgets/` | Bir sahibin (owner) token bütçesini oluşturur/günceller |

`PUT` gövdesi hedef düğümü (`targetType` + `targetId`) ve zaman pencereleri, USD bütçesi, aşım davranışı, model/sağlayıcı bazlı alt-limitleri içeren `budget` nesnesini alır. `targetProjectId`, kimlik doğrulanan path projesine zorlanır — bir çağıran yalnızca `AI_BUDGETS` × MANAGE yetkisi olduğu proje içindeki bütçeleri değiştirebilir, gövdedeki `targetProjectId` başka bir kiracıya yönlendirme yapamaz.

Detaylar için bkz. [API Referansı: AI Budgets](/api-reference/ai-budgets).
