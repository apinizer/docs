...
LLM sağlayıcı bağlantıları, Yönetim Konsolu'na hiç girmeden **APIops REST API** ile de tam CRUD olarak yönetilebilir — proje-kapsamlı, isim tabanlı uç noktalar:

| Metod | Yol | Açıklama |
|-------|-----|----------|
| `GET` | `/apiops/projects/{projectName}/llm-providers/` | Projedeki tüm bağlantıları listeler |
| `GET` | `/apiops/projects/{projectName}/llm-providers/{providerName}/` | Tek bir bağlantıyı adıyla getirir |
| `POST` / `PUT` | `/apiops/projects/{projectName}/llm-providers/{providerName}/` | Bağlantı oluşturur / günceller |
| `DELETE` | `/apiops/projects/{projectName}/llm-providers/{providerName}/` | Bağlantıyı siler |
| `POST` | `/apiops/projects/{projectName}/llm-providers/{providerName}/test-connection/` | Bağlantıyı Yönetim Konsolu üzerinden (manager-içi) test eder |

`test-connection/` gövdesi **isteğe bağlıdır** — boş gönderilirse kayıtlı bağlantı olduğu gibi test edilir. Gövde gönderilip boş bırakılan `apiKey`/`apiSecret`/`serviceAccountJson` alanları, **yalnızca bağlantının hedefi değişmemişse** (uç nokta, kimlik doğrulama şeması, auth header adı, sağlayıcı tipi, dağıtım tipi aynıysa) kayıtlı değerden otomatik doldurulur:

:::warning Kimlik Bilgisi Sızıntısı Koruması
Hedef değiştiyse (örn. `endpoint` farklı bir adrese işaret ediyorsa) kayıtlı sır **asla** yeni hedefe gönderilmez — istek `credentialReentryRequired` hatasıyla reddedilir ve kimlik bilgisinin yeniden girilmesi istenir. Bu, bir çağıranın uç noktayı kendi sunucusuna yönlendirip sırrı boş bırakarak kayıtlı kimlik bilgisini sızdırmasını engelleyen bilinçli bir korumadır.
:::

Detaylar için bkz. [API Referansı: LLM Providers](/api-reference/llm-providers).
