...
- **Tanımlar** — sağlayıcı *tiplerinin* kataloğunu listeler: ad, kod, varsayılan uç nokta bilgileri ve hazır/özel rozeti kolonlarıyla. Kurulumla gelen hazır tanımlar (OpenAI, Anthropic, Azure vb.) da düzenlenebilir: uç nokta ya da varsayılan kimlik doğrulama şemasını değiştirip kaydedebilirsiniz. **Ancak bu ailede ne silme ne de düzenleme kalıcıdır; diğer AI hazır-kayıt katalogları bu noktada farklı davranır.** Olağan bir sürüm yükseltmesi ne sildiklerinize ne de düzenlediklerinize dokunur; fakat Apinizer zaman zaman bir **LLM katalog tazeleme** migration'ı yayınlar — sağlayıcı ekosistemi hızla değiştiği için hazır katalog belirli aralıklarla yeniden yayımlanır (en son APNZ-6550). Böyle bir tazeleme, sildiğiniz her hazır tanımı geri ekler ve düzenlediğiniz her hazır tanımın görünen adını, varsayılan uç noktasını, varsayılan API sürümünü, varsayılan kimlik doğrulama şemasını, varsayılan kimlik doğrulama header adını, ikon URL'ini ve dokümantasyon linkini kurulumla gelen varsayılanlara geri döndürür. Bu yüzden kalıcı özelleştirmenin tek yolu **Kopyala**'dır: hazır tanımın bir kopyasını çıkarıp onu düzenleyin — kopya özel (custom) bir tanım olduğu için tazeleme ona hiç dokunmaz. Özel (custom) bir tanım, bu tab'ın kendi formundan doğrudan oluşturulabildiği gibi, Dışa/İçe Aktarma Sihirbazı ("Provider Definition" tipi) ile içe aktararak ya da APIops REST API ile de eklenebilir. Ekranda özel tanımlar için oluşturma, düzenleme, detay görüntüleme, dışa aktarma ve silme aksiyonları kullanılabilir. Bir Provider Definition paketi içe aktarıldığında, sihirbazın tamamlanma ekranındaki kısayol sizi doğrudan bu tab'a getirir. Kavramın detayı için [Model Kataloğu ve Fiyatlandırma](/tr/ai-gateway/model-katalogu) sayfasındaki **Sağlayıcı Tipi Kataloğu** bölümüne bakın.
