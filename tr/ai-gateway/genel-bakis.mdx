...
:::tip Belirtilmemiş çıkan bir modeli sınıflandırma
Bir model **Belirtilmemiş** olarak listeleniyorsa, **AI Development → LLM Sağlayıcılar** ekranında ilgili sağlayıcıyı açıp **Desteklenen Modeller** tablosundaki **Model Tipi** sütunundan elle seçebilirsiniz. Model, ürünle gelen model kataloğunda da tanımlıysa alan zaten kaydetme sırasında otomatik doldurulur (bkz. [LLM Sağlayıcılar](/tr/ai-gateway/llm-saglayicilari)); katalogda karşılığı olmayan özel modeller ise otomatik olarak bir tipe **atanmaz**, çünkü yanlış sınıflandırma model tipi doğrulamasını ve maliyet hesabını bozar.
:::
