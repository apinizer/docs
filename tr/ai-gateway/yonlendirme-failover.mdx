...
:::warning Varsayılan Hedef Her Zaman Zorunludur
Her AI Gateway'in **varsayılan hedef** olarak seçilmiş bir sağlayıcı ve modeli olmalıdır — tüm trafiği koşullu rotalarla veya yük dağıtımı havuzuyla yönlendirmeyi planlasanız bile. Ağ geçidi varsayılan hedefi, herhangi bir rotayı veya havuzu değerlendirmeden *önce* çözer; hiçbir koşullu rota eşleşmediğinde istek buna düşer ve kendi sağlayıcı/model alanını boş bırakan bir rota veya havuz satırı da bunu devralır. Varsayılan hedefi olmayan bir proxy'nin kaydı *"AI Routing sekmesinde bir LLM sağlayıcısı ve modeli seçin"* hatasıyla reddedilir.

Bu alanı normalde elle doldurmazsınız: ilk yük dağıtımı havuz satırını — ya da hem sağlayıcı hem model içeren ilk koşullu rotayı — eklediğinizde, varsayılan hedef henüz boşsa o satırdan doldurulur. Oluşan hedef, yönlendirme ekranındaki salt-okunur **Varsayılan Hedef** tablosunda görünür.
:::
...
Her havuz satırında bir sağlayıcı seçilmiş olmalıdır. Satırın modeli isteğe bağlıdır; boş bırakılırsa varsayılan hedefin modeli devralınır — aynı kural failover zinciri satırları için de geçerlidir. Sağlayıcısı olmayan bir satır istek anında ağ geçidi tarafından atlanır; bu nedenle hiç çalışmayacak bir yapılandırma olarak ekranda bırakılmak yerine kayıt sırasında reddedilir.
