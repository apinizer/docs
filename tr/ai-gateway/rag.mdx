...
:::warning Politika sırası: RAG, korumalardan ve önbellekten önce
RAG; [istem koruması, veri sızıntısı koruması ve bağlam bütünlüğü](/tr/ai-gateway/gelismis-korumalar) ile [semantik önbellek](/tr/ai-gateway/semantik-onbellek) politikalarından **önce** çalışmalıdır. Getirilen bilgi tabanı içeriğinin de denetimden geçmesi gerekir; aksi halde bir belgeye gömülü zararlı yönerge ya da sır, denetlenmeden modele ulaşır ve önbellek anahtarı bu içeriği kapsamadığı için yanlış önbellek isabeti oluşur. RAG'in kendisi de [Prompt Şablonu](/tr/ai-gateway/prompt-sablonlari) ve [Prompt Süsleyici](/tr/ai-gateway/prompt-susleyici) politikalarından **sonra** gelmelidir.
...
:::warning Sistem Mesajına Ekle (Başa), Sağlayıcı Prompt Cache'ini Etkisizleştirir
...
RAG Injection politikası, her politika tipinin paylaştığı aynı genel politika ucu üzerinden bir API proxy'sine eklenebilir, güncellenebilir ya da kaldırılabilir:
...
`policy.type` alanını `ai-rag-injection` yapın. `vectorDbName`, `embeddingProviderName` ve `knowledgeBaseName` alanları sunucu tarafında **ada göre** çözülür — her APIops kaynağının izlediği ada-dayalı-referans kuralıyla aynıdır, hiçbir zaman iç kimlikle değil — bu sayede aynı istek gövdesi, referans verilen bağlantılar ve bilgi tabanı orada da mevcut olduğu sürece ortamlar arasında taşınabilir.

Tam alan listesi, istek/yanıt örnekleri ve hata kodları için bkz. [API Referansı: RAG Injection Policy](/api-reference/policies/policies/policy-ai-rag-injection).
