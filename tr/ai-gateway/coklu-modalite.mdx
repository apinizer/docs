...
- **Yapılandırılmış prompt özellikleri standart `messages[]` dizisini okur.** [Prompt Süsleyici](/tr/ai-gateway/prompt-susleyici), [Prompt Şablonu](/tr/ai-gateway/prompt-sablonlari), Konu-Dışı Koruması, [RAG](/tr/ai-gateway/rag) bağlam ekleme ve semantik önbelleğin benzerlik eşleşmesi — hepsi standart `messages[]` dizisini okur. Bir Responses isteği bunun yerine `input` taşıdığı için Prompt Süsleyici ve Prompt Şablonu şu an `/v1/responses` isteklerinde etkilenmeden geçer (atlanır); Konu-Dışı Koruması, RAG ve semantik önbellek ise isteği mesaj bazında ayrıştırmak yerine isteğin ham metnini tarayarak çalışmaya devam eder. Kişisel veri maskeleme gibi diğer tarama tabanlı korumalar zaten ham istek metni üzerinden (gerekirse kısaltılarak) çalışır ve bu davranıştan etkilenmez.
