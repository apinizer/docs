...
### JSON Gövdelerinde: Kurallar Yalnız Değerlere Uygulanır {#pii-json-govde}

İstek ya da yanıt gövdesi JSON olduğunda maskeleme, gövdeyi düz metin olarak taramaz; JSON ağacını gezer ve kuralları **yalnızca metinsel değerlere** uygular. Alan adları (anahtarlar) ve JSON sözdizimi — tırnaklar, süslü parantezler, virgüller — hiçbir kural tarafından değiştirilmez.

Bunun pratik sonucu: geniş kapsamlı ya da açgözlü yazılmış bir özel regex, uzun bir istemde bile gövdenin yapısını bozamaz. Maskelenmiş gövde her zaman geçerli JSON olarak sağlayıcıya iletilir; hiçbir kural eşleşmediğinde ise gövde birebir olduğu gibi (bayt bayt) geçirilir. JSON olmayan gövdelerde (düz metin, form verisi) tarama eskisi gibi metnin tamamı üzerinde çalışır.

**Maskelenmiş gövde üretilemezse istek reddedilir.** Maskeleme sonrası yeniden kurulan JSON, özgün gövdenin yerine geçmeden önce doğrulanır. Bu doğrulamanın başarısız olduğu (beklenmeyen) durumda Apinizer özgün gövdeyi olduğu gibi iletmeye geri dönmez — bu, kuralın yakaladığı kişisel veriyi sağlayıcıya maskelenmemiş göndermek olurdu — bozuk gövdeyi de iletmez. İstek **ERR-303** hatasıyla (`AI PII Mask: request rejected — a masked request body could not be produced safely`, HTTP 500) reddedilir. Mesaj ve durum kodu, diğer politika hataları gibi **Hata Mesajı** sekmesinden özelleştirilebilir. Bu durumda istek içeriği uygulama günlüğüne yazılmaz.

:::note
Alan adına göre maskeleme yapmak istiyorsanız regex değil, **alan adı bazlı** kural türünü kullanın — o kural anahtarı eşleştirir ve karşılık gelen **değeri** maskeler.
:::
...
### İki Mevzuatın Ötesinde: Presidio Hizalı Ek Kapsam {#dlp-presidio-kapsami}

Yerleşik paket, yukarıdaki iki mevzuatın dışında kalan ama genel amaçlı PII dedektörlerinin sık işaretlediği birkaç tanımlayıcı için de kalıp içerir — paketin bu kısmı [Microsoft Presidio](https://microsoft.github.io/presidio/)'nun tanımlayıcı kataloğuyla hizalanmıştır (MIT lisanslı; Apinizer'ın düzenli ifadeleri kendi yazımıdır, Presidio'dan birebir kopyalanmamış, atıfla uyarlanmıştır):

| Tanımlayıcı | Kapsama |
|---|---|
| ABD sürücü belgesi numarası | Kalıpla yakalanır, bağlam anahtar kelimesi çapalı |
...
### Dosya İmzası Tespiti {#dosya-imzasi-tespiti}
...
| Dosya tipi | Kapsam | `fileSignatureTypes` değeri |
|---|---|---|
| PDF belgesi | `.pdf` | `PDF` |
| ZIP / Office | `.docx`, `.xlsx`, `.pptx`, `.jar` ve diğer ZIP kapsayıcıları | `ZIP_OOXML` |
| PNG görseli | `.png` | `PNG` |
| JPEG görseli | `.jpg`, `.jpeg` | `JPEG` |
| GIF görseli | `.gif` | `GIF` |
| GZIP arşivi | `.gz` | `GZIP` |
| RAR arşivi | `.rar` | `RAR` |
| 7-Zip arşivi | `.7z` | `SEVEN_ZIP` |
| ELF çalıştırılabilir dosyası | Linux ikilileri ve paylaşılan kitaplıklar | `ELF` |
| PE çalıştırılabilir dosyası | Windows `.exe` / `.dll` | `PE_EXE` |
| OLE2 belgesi | Eski Office `.doc`, `.xls`, `.ppt` | `OLE2` |
| RTF belgesi | `.rtf` | `RTF` |
| Java class dosyası | Derlenmiş `.class` bayt kodu | `JAVA_CLASS` |
...
:::warning Bu ayar, akış yanıtlarında ilk-token süresini doğruluk uğruna feda eder
Dosya İmzası Tespiti açıkken bu politikanın akış (streaming) yanıtları parça parça değil, **bütün olarak biriktirilip taranır** — hangi aksiyonu seçmiş olursanız olun, **İşaretle dahil**. Kodlanmış bir dosya imzası herhangi bir parça sınırına yayılabilir: sihirli baytlar bir parçaya, onları doğrulayan baytlar bir sonrakine düşer; dolayısıyla parça-bazlı bir tarama neredeyse hiçbir şey bulamaz. Bu, [Akış (Streaming) Yanıtlarda: Buffer-and-Scan](#streaming-buffer-and-scan) bölümünde anlatılan aynı buffer-and-scan yaklaşımıdır ve pratik bedeli, bu politikanın akış yanıtlarında ilk-token süresinin (TTFT) artık korunmamasıdır. Belge sızıntısı riski taşımayan, gecikmeye duyarlı proxy'lerde anahtarı kapalı bırakın.
:::

**Eşleşme nerede görünür**

Bir dosya imzası eşleşmesi, bir sır kalıbı eşleşmesinden ayrı olarak kaydedilir; böylece ikisi [Güvenlik ve Guardrail'ler raporunda](/tr/ai-gateway/raporlar) birbirine karışmaz: olay `secret` yerine `file-signature` olarak etiketlenir ve etiketi biçim ile konumu taşır — örneğin `File signature (PDF, at_start)`. Birden fazla imza eşleştiyse etiket kalanların sayısını da belirtir. [İzleme ve Tekrar Oynatma](/tr/ai-gateway/izleme-ve-tekrar-oynatma) kaydında biçim, konum, baytların ham mı yoksa base64/hex kodlu mu geldiği ve kaç eşleşme olduğu tutulur — taranan içeriğin kendisi asla tutulmaz.
...
:::warning Tampon etkinken kademeli akış yoktur — ve bir DLP tampon taramasıyla birlikte hiç çalışmaz
Engelle kurallı bir Konu-Dışı Koruması bir yanıtı tamponlarken istemci, akış bitene kadar hiçbir şey görmez: ilk-token süresi (TTFT) yoktur, yalnızca akışın sonunda tek bir parça gelir — bu parça ya yanıtın tamamını (eşleşme yok / İşaretle) ya da hiçbir şeyi (Engelle) taşır. Karar her iki durumda da [izleme kaydına ve Güvenlik ve Guardrail'ler raporuna](/tr/ai-gateway/raporlar) işlenir. Akış dışı (unary) yanıtlar bu bedelden etkilenmez — oradaki değerlendirme yanıt istemciye ulaşmadan önce zaten tamamlanmıştır.
...
Bu katalogdan alınan bir Yasaklı Konu eşleşmesi, [Güvenlik ve Guardrail'ler raporunda](/tr/ai-gateway/raporlar) **AILuminate kategorisine göre** kırılır — kategori başına isabet sayısı, zaman serisi ve proje/proxy dağılımı — ve her [izleme](/tr/ai-gateway/izleme-ve-tekrar-oynatma) kaydı eşleşen kategoriyi, benzerlik skorunu ve kararı hangi motorun verdiğini (yerleşik embedding karşılaştırması mı, `llm-judge` mı) taşır.
...
### Sayım Modu ve Limit Değişimi Davranışı

Tekrar-Fırtınası (Loop) Koruması, tam olarak **aynı isteğin** (aynı kimlik bilgisi, AI Gateway ve model, bayt-bayt özdeş prompt) tekrarlarını kaydırmalı bir **Pencere (saniye)** içinde sayar ve sayaç **Maks. Tekrar** değerini aştığında yapılandırılan **Aksiyon**'u — İŞARETLE (varsayılan: trafik geçer, olay izleme için kaydedilir) ya da ENGELLE (HTTP 429) — tetikler.

Sayacın kendisinin nasıl davrandığını iki ayar daha belirler:

| Ayar | Seçenekler | Varsayılan | Davranış |
|---|---|---|---|
| **Sayım Modu** | Tüm istekler / Yalnızca kabul edilenler | **Tüm istekler** | Tüm istekler: korumanın kendisinin engellediği bir istek de tekrar eşiğine sayılmaya devam eder — kötüye kullanan bir istemciyi yavaşlatmak için yararlı, bugünkü davranış. Yalnızca kabul edilenler: yalnızca geçmesine izin verilen istekler sayılır; reddedilen bir istek sayacı asla şişirmez. |
| **Limit Değişimi Davranışı** | Önceki limite kırp / Sıfırla / Dokunma | **Dokunma (NONE)** | **Maks. Tekrar** değerini değiştirdiğiniz anda, o anki pencerede zaten birikmiş sayaca ne olacağını belirler — aşağıya bakın. |

**Limit Değişimi Davranışı**, yalnızca limitin kendisi değiştiği anda devreye girer, her istekte değil:

- **Önceki limite kırp** — sayaç, değişiklikten *önceki* limitle sınırlanır (`kullanılan = min(kullanılan, önceki limit)`), yeni limitle değil. Örnek: Maks. Tekrar 100'den 300'e çıkarken sayaç — Tüm istekler sayım modunda reddedilen isteklerle şişmiş halde — 500'deyse, 100'e düşer ve yeni limitin 200'ü kullanılabilir kalır. Limiti düşürmek bu kırpmadan etkilenmez, engelleme bu ayar olmasaydı nasıl çalışacaksa öyle çalışmaya devam eder.
- **Sıfırla** — sayaç sıfırlanır, ama **yalnızca limit yükseltildiğinde**. Apinizer, limit düşürüldüğünde asla sıfırlamaz, çünkü bu, gerçekten tüketilmiş bir kotayı sessizce affetmiş olurdu.
- **Dokunma** (varsayılan) — sayaç olduğu gibi bırakılır; bir limit değişikliği yalnızca o anki pencere kendiliğinden bittiğinde etkili olur.

Bu iki ayar, Apinizer'daki her hız-sınırlama tipi politikayla aynı adı ve anlamı paylaşır — API tabanlı daraltma ve kota politikaları da aynı **Sayım Modu** / **Limit Değişimi Davranışı** seçimini sunar.
...
Semantik önbellek de aynı gerekçeyle aynı üç seçeneği sunar — bkz. [Semantik Önbellek](/tr/ai-gateway/semantik-onbellek).
...
Bir global koruma politikasını bir API Proxy'nin **Geliştir** ekranından açıp kaydetmeye çalıştığınızda, değişiklik merkezi tanıma yazılmaz: politikanın adını yazarak onayladığınızda yalnız o proxy'ye ait bir kopya oluşturulur ve düzenlemeniz bu kopyaya uygulanır. Diğer proxy'ler merkezi tanımı kullanmaya devam eder. Merkezi tanımı gerçekten değiştirmek istiyorsanız politikayı **Global Politikalar** ekranından düzenleyin. Ayrıntı: [Global Politikalar](/tr/develop/global-configurations/global-policies).
