...
PII maskeleme, İstem Koruması ve Veri Sızıntısı Koruması (DLP) preset'lerinin **üçü de** artık **APIops REST API** ile programatik olarak yönetilebiliyor — DLP preset'leri bu sürümde PII ve İstem Koruması'na katıldı. Her preset tipi aynı 5-uçlu CRUD şeklini izler ve **iki farklı kapsamda** sunulur:

| Kapsam | Yol | Kim yönetir |
|--------|-----|-------------|
| Kurulum geneli (admin) | `/apiops/settings/ai-privacy-presets/`, `/apiops/settings/ai-prompt-guard-presets/`, `/apiops/settings/ai-dlp-presets/` | Yalnızca sistem yöneticisi (system-admin); `?projectId=` ile projeye göre filtrelenebilir |
| Proje kapsamlı (yeni) | `/apiops/projects/{projectName}/ai-privacy-presets/`, `/apiops/projects/{projectName}/ai-prompt-guard-presets/`, `/apiops/projects/{projectName}/ai-dlp-presets/` | Projede `AI_DEVELOPMENT` × VIEW/MANAGE yetkisi olan herkes; proje `projectId`'den path üzerinden çözülür, query parametre olarak geçilmez |

:::info Üç Katmanlı Kapsam
Bir preset üç kaynaktan gelebilir: kurulumla gelen **hazır (built-in)** preset'ler, sistem yöneticisinin tanımladığı **kurulum geneli (admin-shared)** preset'ler ve projenin kendi **proje-sahipli** preset'leri.

- **Okuma**: her iki uç de bu üç kaynağın **birleşimini** döner — built-in ∪ admin-shared ∪ proje-sahipli.
- **Yazma** (`POST`/`PUT`/`DELETE`): yalnızca **proje-sahipli** kayıtlar üzerinde yapılabilir. built-in ve admin-shared preset'ler proje kapsamlı uçtan **değiştirilemez** (immutable) — bunları güncellemek için sistem yöneticisi yetkisiyle `/apiops/settings/...` ucu kullanılmalıdır.
:::

Mevcut `/apiops/settings/...` + `?projectId=` yolu geriye dönük uyumlu olarak korunmuştur; yeni proje-kapsamlı uçlar buna bir alternatiftir, yerine geçmez.

Detaylar için bkz. [API Referansı: AI Privacy Presets](/api-reference/ai-privacy-presets), [AI Prompt-Guard Presets](/api-reference/ai-prompt-guard-presets), [AI DLP Presets](/api-reference/ai-dlp-presets).
