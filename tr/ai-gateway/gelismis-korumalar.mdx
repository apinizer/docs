...
İstem Koruması ve Konu-Dışı Koruması, yerleşik (regex/embedding tabanlı) değerlendirmenin yanında **dış bir LLM'i hakem olarak da** (`llm-judge` adaptörü) kullanabilir. Kişisel Veri Maskeleme ve Veri Sızıntısı Koruması da aynı `llm-judge` adaptörünü kullanabilir, ayrıca kurumun kendi vendor-bağımsız HTTP DLP servisine bağlanan **ek bir adaptör türünü** (`http-dlp`) sunar; bu ikisinin dış sağlayıcı davranışı bazı noktalarda farklı kurallar taşır ve aşağıda [Harici DLP Entegrasyonu](#harici-dlp-entegrasyonu) bölümünde ayrıca anlatılır. Tekrar-Fırtınası, Aşırı Boyut ve Bağlam Bütünlüğü Korumaları'nda dış sağlayıcı motoru yoktur.
