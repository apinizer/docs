...
Bu sürümde alan yönetim arayüzünde henüz görünmez; APIops REST API ([API Referansı: AI Gateway](/api-reference/ai-gateway)) ile ya da politika JSON'unda `decodeEncodedContent` alanı `true` yapılarak açılır. Arayüz desteği takip eden bir çalışmadır.
...
Bu üç alan da yönetim arayüzünde henüz görünmez; [Kodlanmış İçeriğin Çözülmesi](#kodlanmis-icerik-cozme) bölümündeki `decodeEncodedContent` ile aynı yolla, APIops REST API ([API Referansı: AI Gateway](/api-reference/ai-gateway)) veya politika JSON'unda `includeSystemPrompts` / `includeAssistantPrompts` / `includeToolPrompts` alanları ayarlanarak açılır.
