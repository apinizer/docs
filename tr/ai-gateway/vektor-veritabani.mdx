...
Vektör veritabanı bağlantıları **APIops REST API** ile de tam CRUD olarak yönetilebilir:

| Metod | Yol | Açıklama |
|-------|-----|----------|
| `GET` | `/apiops/projects/{projectName}/vector-dbs/` | Projedeki tüm bağlantıları listeler |
| `GET` | `/apiops/projects/{projectName}/vector-dbs/{vectorDbName}/` | Tek bir bağlantıyı adıyla getirir |
| `POST` / `PUT` | `/apiops/projects/{projectName}/vector-dbs/{vectorDbName}/` | Bağlantı oluşturur / günceller |
| `DELETE` | `/apiops/projects/{projectName}/vector-dbs/{vectorDbName}/` | Bağlantıyı siler |
| `POST` | `/apiops/projects/{projectName}/vector-dbs/{vectorDbName}/test-connection/` | Bağlantıyı test eder |

Şifre, API anahtarı ve API sırrı gibi alanlar diğer tüm APIops uçlarıyla aynı şekilde şifreli saklanır. Detaylar için bkz. [API Referansı: Vector DBs](/api-reference/vector-dbs).
