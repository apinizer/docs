...
- **Cache ve Yanıt** — cache bağlantı hatasında ne yapılacağını (**Fail**: istek reddedilir, varsayılan; **Continue**: hata loglanır ve istek işlenmeye devam eder), cache bağlantı zaman aşımını (saniye) ve limit bilgilerinin yanıt başlıklarında gösterilip gösterilmeyeceğini belirler. AI Token Bütçesi ataması [Token Kotaları](/tr/ai-gateway/token-kotalari) mekanizmasıyla uygulanır; kendi cache ayarları vardır ve yanıta `RateLimit-*` başlığı eklemez.
...
| Alan | Anlamı |
|------|--------|
| **Kimlik kaynağı** | **Kimlik doğrulamadan gelen kullanıcı** seçildiğinde kimlik, kimlik doğrulama sonucundan alınır (bir kimlik bilgisinin kullanıcı adı ya da bir API istemcisinin client kimliği); **Değişken seçimi** seçildiğinde kimlik, seçilen bir değişkenin istek anındaki değerinden alınır. Bu seçim atamanın kayıtlı *aktör kapsamını* da belirler — **Kimlik doğrulamadan gelen kullanıcı** kimliği doğrulanmış çağıranları, **Değişken seçimi** her çağıranı (değişken anonim trafikte de vardır) sayar; ekranda ayrı bir aktör kapsamı alanı yoktur, `actorScope` göndermeyen bir Yönetim API'si / APIops çağrısı da aynı varsayılanı alır. |
