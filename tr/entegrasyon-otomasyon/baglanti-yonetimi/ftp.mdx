...
description: "FTP/SFTP/FTPS sunucularına güvenli dosya aktarımı için merkezi bağlantı tanımı oluşturmanızı sağlar. Güvenli dosya aktarımı, dizin yönetimi ve otomatik yeniden deneme desteği gerçekleştirebilirsiniz"
---
...
<AccordionGroup>
  <Accordion title="Dışa Aktarma (Export)">
    ...
    <Card title="Dosya Formatı" icon="file-archive">
      Format: Tarih-connection-FTP Bağlantısı-export.zip  
      **Örnek:** `13 Nov 2025-connection-Production_FTP-export.zip`
    </Card>
  ...
  <Accordion title="İçe Aktarma (Import)">
    Ana listede **[Import FTP Bağlantısı]** butonuna tıklayın. İndirilen ZIP dosyasını seçin. Sistem kontrolleri: Format geçerli mi? İsim çakışması var mı? Bağımlılıklar mevcut mu? Ardından **[Import]** butonuna tıklayın
...
<CardGroup cols={2}>
  ...
  <Card title="Integration / Connector Adımlarında Kullanım" icon="plug">
    "Upload File", "Download File", "Read Directory" gibi adımlarda FTP/SFTP seçimi yapılır. Bağlantı seçimi adım yapılandırmasındaki Connection alanından gerçekleştirilir. Aynı bağlantı, hem dosya gönderme hem alma adımlarında yeniden kullanılabilir
  </Card>
  
  <Card title="Scheduled Job Kullanımı" icon="clock">
    Zamanlanmış görevlerde belirli periyotlarda dosya alma/gönderme yapılır. Cron değiştiğinde job, ilgili connection parametrelerini anında kullanır
  </Card>
  
  <Card title="Test Amaçlı Kullanım" icon="flask">
    "Test Connection" sonucu loglarda saklanır, sorun giderme sürecinde referans olur.  
...
<AccordionGroup>
  <Accordion title="Directory Listing Başarısız">
    **Olası Nedenler:** Yanlış workingDir, Kullanıcının listeleme yetkisi yok, Firewall pasif port aralığını engelliyor  
    **Çözüm Adımları:**  
    1. WorkingDir değerini doğrulayın  
    2. FTP kullanıcı izinlerini kontrol edin  
    3. Pasif port aralığını açın
  </Accordion>

  <Accordion title="SSL/TLS Negotiation Hatası">
    **Olası Nedenler:** Yanlış sslProtocol değeri, Sertifika süresi doldu, UseImplicit/UseExplicit uyumsuz  
    **Çözüm Adımları:**  
    1. TLS sürümünü karşı tarafla hizalayın  
    2. Sertifikayı yenileyin  
    3. Doğru FTPS modunu seçin
  </Accordion>
