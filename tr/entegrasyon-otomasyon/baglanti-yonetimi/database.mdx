---
title: "Veritabanı"
description: "Çoklu veritabanı tipleri için havuzlu Connection tanımlarını merkezi olarak yönetir"
---
...
| Alan | Değer |
|------|-------|
| Connection Adı | Veritabanı Connection Havuzu |
| Özet | Çoklu veritabanı tipleri için havuzlu Connection tanımlarını merkezi olarak yönetir |
| Kategori | Database |
| Protocol | JDBC / Mongo URI |
...
**Bağlantı Başlatma:** Integration Flow veya Connector içerisinden Database bağlantısı talep edildiğinde, sistem yapılandırılmış connection parametrelerini okur.

**Connection Pool Yönetimi:** C3P0 uyumlu pool stratejisiyle mevcut Connection havuzdan alınır; limit aşılırsa yeni Connection oluşturulur.

**Kimlik Doğrulama:** Kullanıcı adı/şifre veya environment değişkenine bağlanan credential setiyle Authentication gerçekleştirilir.

**Veri İletişimi:** JDBC veya Mongo URI üzerinden SQL/NoSQL komutları hedef Database sürücüsüne iletilir ve yanıt alınır.

**Bağlantı Yönetimi:** İşlem tamamlandıktan sonra Connection pool'a geri döner, uzun süre kullanılmayan bağlantılar kapatılır.

**Hata Yönetimi:** Bağlantı hatası, timeout veya authentication hatası durumunda deployment sonucu iletişim kutusu üzerinden ayrıntılı hata kaydı ve log yönlendirmesi sağlanır.
