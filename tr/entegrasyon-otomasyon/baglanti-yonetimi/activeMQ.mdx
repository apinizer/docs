---
title: "ActiveMQ "
description: "ActiveMQ broker'larına güvenli ve yönetilebilir bağlantı sağlar. AMQP/TCP üzerinden mesaj gönderme ve alma işlemlerini merkezileştirir."
---
...
| Alan | Değer |
|------|-------|
| Connection Adı | Apache ActiveMQ |
| Özet | AMQP/TCP üzerinden ActiveMQ broker'larına güvenli ve yönetilebilir bağlantı sağlar |
...
**Bağlantı Başlatma:** Integration Flow veya Connector içerisinden Message Queue bağlantısı talep edildiğinde, sistem yapılandırılmış connection parametrelerini okur.

**Connection Pool Yönetimi:** Aktif connection pool stratejisiyle önceden açılmış bağlantılar yeniden kullanılır; pool doluysa yeni connection oluşturulur.

**Kimlik Doğrulama:** Temel Authentication mekanizmasıyla (kullanıcı adı/şifre) veya opsiyonel SSL/TLS sertifika doğrulamasıyla broker'a kimlik doğrulaması yapılır.

**Veri İletişimi:** AMQP/TCP protokolü üzerinden Queue veya Topic hedefine JMS mesajları senkron/asenkron olarak iletilir ya da tüketilir.

**Bağlantı Yönetimi:** İşlem tamamlandığında bağlantı pool'a geri döner; pasif bağlantılar pool politikalarına göre kapatılır.

**Hata Yönetimi:** Bağlantı hatası, timeout veya authentication hatası durumunda hata ayrıntıları loglanır sistem otomatik yeniden deneme ve kullanıcıya uyarı mesajı üretir.
...
|-----------|----------|-------------|--------|
| Name | Connection adı (benzersiz olmalı) | Production_ActiveMQ | Boşlukla başlamaz, özel karakterler kullanılmamalı |
| Broker URL | ActiveMQ broker erişim adresi | tcp://mq.prod.local:61616 | Protokol (amqp/tcp) ve port değerini içerir |
| Username | ActiveMQ erişim kullanıcı adı | mq_service_user | Yetkilendirilmiş hesap kullanılmalı |
| Password | Kullanıcıya ait şifre | ******** | Şifre saklama için secret manager kullanılmalı |
| Destination Type | Queue veya Topic seçimi | QUEUE | Enum değerleri: QUEUE, TOPIC |
| Destination Name | Mesaj kuyruğu veya topic adı | order.events.queue | Namespace kuralına uyulmalı |
| ActiveMQ Connection Protocol Type | Kullanılacak bağlantı protokolü | AMQP | Enum değerleri: AMQP, TCP |
| Session Acknowledgement | JMS oturum onay tipi | AUTO_ACKNOWLEDGE | Enum değerleri: AUTO_ACKNOWLEDGE, CLIENT_ACKNOWLEDGE, SESSION_TRANSACTED, DUPS_OK_ACKNOWLEDGE |
| Environment Id | Ortam seçimi referansı | env-prod-01 | Ortam listesi Environment servisinden gelir |

### İsteğe Bağlı Parametreler

| Parametre | Açıklama | Varsayılan Değer | Önerilen Değer |
|-----------|----------|------------------|----------------|
| Send Timeout | Mesaj gönderimi için bekleme süresi | 60000 | 30000 - 120000 aralığında ihtiyaca göre |
| Request Timeout | Yanıt bekleme süresi | 60000 | Kritik işlemler için 45000 |
| Close Timeout | Bağlantıyı kapatma süresi | 60000 | Uzun oturumlar için 90000 |
| Connect Timeout | Broker'a bağlanma süresi | 60000 | Yüksek gecikmeli ağlarda 90000 ms |
| Content Type | Mesaj içerik tipi | application/json | İş yüküne göre application/xml |
| Content Encoding | Mesaj karakter seti | UTF-8 | Standart UTF-8 kullanımı önerilir |
| Client ID | Kalıcı abonelik kimliği | durable-consumer-1 | Durable topic aboneliklerinde zorunlu |

### Timeout ve Connection Pool Parametreleri

| Parametre | Açıklama | Varsayılan | Min | Max | Birim |
|-----------|----------|-----------|-----|-----|-------|
| Connection Timeout | Bağlantı kurulması için maksimum bekleme süresi | 60000 | 1000 | 180000 | milisaniye |
| Request Timeout | İstek yanıtı için maksimum bekleme süresi | 60000 | 1000 | 180000 | milisaniye |
| Pool Size | Connection pool'daki maksimum bağlantı sayısı | 10 | 1 | 200 | adet |
| Send Timeout | Mesaj gönderiminin en uzun sürmesi beklenen süre | 60000 | 5000 | 180000 | milisaniye |

### Kullanım Senaryoları

| Senaryo | Durum | Çözüm (Connection Yapılandırması) | Beklenen Davranış / Sonuç |
|---------|-------|-----------------------------------|---------------------------|
| Sipariş Kuyruğu Tüketimi | Yeni sipariş mesajları ActiveMQ kuyruğuna düşüyor | Destination Type=QUEUE, Destination Name=order.events.queue, Protocol=AMQP | Integration Flow sipariş mesajlarını sıralı tüketir |
| Canlı Bildirim Yayını | Websocket bildirimleri topic üzerinden dağıtılacak | Destination Type=TOPIC, Destination Name=notifications.topic, ClientID=notif-producer | Tüm aboneler gerçek zamanlı bildirim alır |
| Yüksek Güvenlikli Broker | Production broker sadece SSL kabul ediyor | Protocol=TCP, Broker URL=ssl://mq.prod.local:61617, SSL sertifika ekleri | Şifreli bağlantı başarılı kurulur |
| Uzun Süreli İşlem | Batch job büyük mesaj paketleri gönderiyor | Send Timeout=120000, Request Timeout=120000, Pool Size=30 | Mesaj gönderimi kesintisiz tamamlanır |
| Test Ortamı Simülasyonu | Test broker düşük kaynaklı | Pool Size=5, Request Timeout=45000, Content Type=application/json | Kaynak kullanımı optimize edilir, test stabil çalışır |
| Disaster Recovery | Yedek broker'a geçiş gerekliliği | Broker URL ikincil adrese güncellenir, Enable toggle ile aktif edilir | Trafik kesintisiz şekilde yedek broker'a kayar |
...
| Dosya Formatı | Format: {Date}-connection-{ConnectionName}-export.zip<br/><br/>Örnek: 13 Nov 2025-connection-Production_ActiveMQ-export.zip |
...
| İçe Aktarma (Import) | - Ana listede [Import Apache ActiveMQ] butonuna tıklayın.<br/>- İndirilen ZIP dosyasını seçin.<br/>- Sistem kontrolleri: Format geçerli mi? İsim çakışması var mı? Bağımlılıklar mevcut mu?<br/>- Ardından [Import] butonuna tıklayın. |
...
Bu adımda kullanıcı, oluşturduğu Apache ActiveMQ connection'ını sistemin farklı bileşenlerinde kullanabilir. Connection'lar Integration Flow, Connector adımları veya Scheduled Job'larda seçilerek kullanılır.
