...
Portalda oluşturulan uygulamalar varsayılan olarak **Test only** durumundadır ve test trafiğini uygulamanın kendi typed **API Client**'ı ile doğrular — bu istemci uygulamayla birlikte otomatik oluşturulur, bkz. aşağıda [API Clients](#api-clients). Production anahtarları, onaylı promote sonrasında ayrı prod management bağlantısından okunur; böylece test anahtarları üretim trafiğinde kullanılmaz.
...
:::info Yükseltme notu
Var olan kurulumlarda bu geçiş (backfill + secret taşıma) Manager'ın bu sürümdeki ilk açılışında otomatik olarak çalışır; sonradan taşınabilir hale gelen bir kayıt kaydedildiğinde de aynı adımlardan yeniden geçer — bunun için bir ekran ya da düğme yoktur, bkz. [Eski Kayıtlar: Otomatik Taşıma](/tr/admin/identity-management/kimlik-bilgileri#eski-kayitlar-envanter-ve-tasima). Bu geçiş henüz uygulanmamış uygulamalarda **API Clients** sekmesi boş görünür; kimlik bilgisi bu süre boyunca portalın hiçbir ekranında listelenmez. Kimlik doğrulama için olduğu gibi kullanılmaya devam eder, yalnızca görüntülenemez.
:::
...
:::info Legacy API Keys artık Portalda gösterilmiyor
**API Clients** sekmesi, uygulamanın taşınma durumu ne olursa olsun, artık ayrı bir **Legacy API Keys** bloğu göstermez. Henüz taşınmamış bir anahtarın burada satırı yoktur; Developer Portal bu sayfada legacy API Key / API Secret çiftini hiç göstermez, buradan üretme ya da iptal etme imkânı da vermez. Anahtarın kendisi silinmez, onu kullanan trafiği doğrulamaya devam eder — kaldırılan yalnızca bu ekrandan yönetilebilir olmasıdır. Taşıma zamanlaması için yukarıdaki **Yükseltme notu**'na bakın; bir uygulama için taşıma tamamlandığında istemci, diğer her API Client gibi yukarıda **Legacy'den taşındı** rozetiyle görünür.

Blok kaldırılınca her legacy anahtar satırının altında görünen, salt okunur anahtar/değer ek bilgi (metadata) alt tablosu da (anahtar, değer, secret bayrağı, JWT dahil etme) kalktı. Bu bilgi artık Portalın hiçbir yerinde gösterilmiyor — değerler silinmedi, olduğu gibi okunabilir durumda, ama yalnızca API Manager'dan, tüketicinin kendi **Metadata** sekmesinden (bkz. [Metadata](/tr/admin/identity-management/kimlik-bilgileri#metadata)).

Bu sürümden itibaren oluşturulan bir uygulamanın zaten legacy kimlik bilgisi yoktur: ilk typed istemcisi uygulamanın kendisiyle birlikte var olur (bkz. aşağıdaki [Create Application ve Abonelik](#create-application-ve-abonelik)).
:::

### Secret görünürlüğü

PROD Credentials için geçerlidir; legacy API Key/Secret çifti artık Portalda hiç gösterilmiyor — yukarıdaki nota bakın. API Client secret görünürlüğü yukarıda, [API Clients](#api-clients) altında anlatılmıştır.
...
:::note
Bu sihirbaz da, uygulama listesindeki hızlı **Yeni Uygulama** diyaloğu da artık UUID'li bir legacy API Key (username/password) üretmez; uygulama her iki yoldan da baştan typed bir API Client ile oluşur.
:::
...
Review: test doğrulandı + terms & rate limits kabul → **Send for approval** → Awaiting prod approval → onay sonrası Prod active → credential’lar **API Clients** sekmesinde. Onay sırasında prod ortamında uygulama için bir **API İstemcisi** açılır; sırrı onay sonucunda gösterilmez, sonuç mesajı sırrın **Rotate** ile alınması gerektiğini hatırlatır.
...
* [Eski Kayıtlar: Otomatik Taşıma](/tr/admin/identity-management/kimlik-bilgileri#eski-kayitlar-envanter-ve-tasima)
