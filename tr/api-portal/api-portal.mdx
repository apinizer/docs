...
<Info>
  ...
  * Eski bayraklar (flag) **-XX:MinRAMFraction** ve **-XX:MaxRAMFraction** artık kullanımdan kaldırıldı. 0.0 ve 100.0 arasında bir değer alan ve varsayılan olarak 75.0 olan yeni bir **-XX:MaxRAMPercentage** bayrağı (flag) vardır. Bu nedenle, 1 GB bellek sınırı varsa, JVM yığını (heap) varsayılan olarak yaklaşık 750 MB ile sınırlıdır.
