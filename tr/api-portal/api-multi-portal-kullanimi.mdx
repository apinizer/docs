...
Aynı izolasyon, portalın **MCP (Model Context Protocol) ucu** için de geçerlidir. Her portalın MCP sunucusu yalnızca kendi portalının verisini görür: bir MCP aracıyla listelenen uygulamalar, API ürünleri, abonelikler, kimlik bilgileri ve trafik kayıtları o portala aittir. Başka bir portalın uygulama veya API kimliği verilse bile istek reddedilir; bir portalda üretilmiş kişisel erişim token'ı başka bir portalın MCP ucunda kabul edilmez.

:::info

Tek portallı kurulumlarda bu kural görünür bir değişiklik yaratmaz. Çoklu portal desteğinden önce oluşturulmuş ve henüz bir portala bağlanmamış kayıtlar da eskisi gibi görünmeye devam eder — MCP artık portal arayüzüyle birebir aynı listeyi döndürür.
...
<Tabs>
  <TabItem value="farkli-musteri-segmentleri" label="Farklı Müşteri Segmentleri">
    **Kullanım Senaryosu:**
  ...
  <TabItem value="farkli-departmanlar-birimler" label="Farklı Departmanlar/Birimler">
    **Kullanım Senaryosu:**
  ...
  <TabItem value="farkli-ortamlar" label="Farklı Ortamlar">
    **Kullanım Senaryosu:**
  ...
  <TabItem value="farkli-bolgeler-diller" label="Farklı Bölgeler/Diller">
    **Kullanım Senaryosu:**
...
Kurulum için [Kubernetes'e Kurulum — API Portal Kurulumu](/tr/setup/kubernetes#api-portal-kurulumu) dokümanını inceleyiniz.
...
:::info
Birden fazla portal varsa üst menü **Portal** alanında aktif portalı seçerek context değiştirirsiniz. Destek, Prod Promote, raporlar ve katalog ayarları **aktif portal** kapsamındadır. Kurulum için Kubernetes dokümanındaki API Portal kurulumu bölümüne de bakabilirsiniz.
:::
...
### Portal Paketinde Ne Taşınmaz?

Bir portalı dışa aktarıp aynı ya da başka bir kurulumda yeni portal olarak oluşturabilirsiniz. Portal paketi düz JSON biçimindedir ve kurulumun dışına çıkar; bu nedenle pakete hiçbir gizli değer yazılmaz. Paket biçimi bir **izin listesi (allowlist)** kullanır: yalnızca taşınması güvenli olduğu açıkça belirtilen alanlar pakete yazılır; bu sayede platforma sonradan eklenen bir alan, o listeye eklenene kadar varsayılan olarak dışarı çıkmaz — birinin fark edip gizleyene kadar açıkta kalmaz. Gizli alanlar hem dışa aktarım sırasında hem de içe aktarım sırasında paketten çıkarılır — böylece elle düzenlenmiş ya da daha eski bir paket de bir gizli değeri sızdıramaz. Apinizer'ın önceki sürümlerinde dışa aktarılmış paketler de içe aktarım için kabul edilir ve aynı süzgeçten geçirilir.

:::warning

Portal paketi şu bilgileri **taşımaz**:

- Hesap parolaları
- Hesap doğrulama ve parola sıfırlama token'ları
- Oturum kimliği bilgisi
- Portal API erişim token'ları
- Production Management API anahtarı (Prod Promote ayarları)
- Jira API token'ı
- Bir API Ürünü ya da planın kaynak kurulumdaki üretim ürün/plan bağları ve limit atama referansları
- Askıya alınmış bir organizasyonun askıya alınmış credential listesi (organizasyonu tam olarak eski hâline getirmek için gereklidir — taşınmaz)
- Bir organizasyonun LDAP/OIDC senkronizasyon bağı (kayıt kendisi taşınır ama artık kaynak sağlayıcıya bağlı değildir)
- Bir uygulamanın sürmekte olan yükseltme (promotion) veya silme iş akışı durumu
- Bekleyen bir organizasyon davetinin kabul token'ı — davet hedefte kabul edilemez

:::

:::note

Bunun sonucu olarak, içe aktarılan portaldaki hesaplar kaynak portaldaki parolalarıyla **giriş yapamaz**. Yönetici bu hesaplara yeni parola atamalı ya da kullanıcılar "Şifremi unuttum" akışını kullanmalıdır. Aynı şekilde Jira entegrasyonu ve Prod Promote bağlantısı, hedef portalde kimlik bilgileri yeniden girilene kadar çalışmaz.

:::

:::note

Bir uygulama ya da organizasyon üzerinde **Gizli** olarak işaretlenmiş bir metadata girdisi anahtarıyla birlikte taşınır, ama değeri kaldırılır — paketteki her gizli alanla aynı kurala tabidir. Girdi içe aktarılan kayıtta görünmeye devam eder, böylece yönetici bir değer girilmesi gerektiğini fark eder; ama değerin hedef kurulumda yeniden girilmesi gerekir.
...
Bu ayarları değiştirmek için [Portal Ayarları](/tr/api-portal/api-portal-ayarlari) sayfasına gidin.
...
<Tabs>
  <TabItem value="aktif-portal" label="Aktif Portal">
  ...
  <TabItem value="pasif-portal" label="Pasif Portal">
...
Detaylı bilgi için [Portal Ayarları - Genel](/tr/api-portal/api-portal-ayarlari#genel-ayarlar) bölümünü inceleyin.
...
Detaylı bilgi için [Portal Ayarları - E-posta](/tr/api-portal/api-portal-ayarlari#e-posta-bildirimleri) bölümünü inceleyin.
...
Detaylı bilgi için [Portal Ayarları - Güvenlik](/tr/api-portal/api-portal-ayarlari#güvenlik) bölümünü inceleyin.
...
Detaylı bilgi için [Portal Ayarları - Özellikler](/tr/api-portal/api-portal-ayarlari#özellikler) bölümünü inceleyin.
...
Detaylı bilgi için [Portal Ayarları - Yasal](/tr/api-portal/api-portal-ayarlari#yasal) bölümünü inceleyin.
...
<Tabs>
  <TabItem value="public-portal" label="Public Portal">
    **Herkese Açık Portal:**
  ...
  <TabItem value="internal-portal" label="Internal Portal">
    **İç Kullanım Portalı:**
