---
title: "API Portal Kurulumu"
description: "Apinizer API Portal'ın kurulumunu ve yapılandırmasını sağlar. Kubernetes platformu üzerinde çalışan portal, API Manager'ın sağladığı API'lerle etkileşim kurarak geliştiricilerin API dokümantasyonuna erişmesini, API'leri test etmesini, abone olmasını ve API Key'lerini yönetmesini sağlar."
---
...
- API Manager'ın kurulu olması gerekmektedir. API Manager Kurulumu için [Apinizer Kurulumu ve Ayarları](/tr/kurulum/apinizer/apinizer) dökümanını inceleyiniz.
...
API Manager üzerinden API Developer Portal kurulumu için Genel Ayarlar menüsünde Kubernetes Yönetimi bölümünün aktif olması gerekmektedir.
...
API Manager'da, **Yönetim → Sunucu Yönetimi → Kubernetes Resources** sayfasına gidin. Deployment & Pods sekmesinden API Portal'ı etkinleştirin. Açılan diyalogda, kurumunuza uygun olarak zorunlu alanları tanımlayın.
...
API Developer Portali'nin manuel kurulumu için aşağıdaki adımlar takip edilir.
...
Portal ayarlarını yapılandırmak için [API Portal Ayarları](/tr/api-portal/api-portal-ayarlari) sayfasını ziyaret edebilirsiniz.

## API Developer Portal'i SSL ile Başlatma

İstenilen durumlarda API Developer Portal SSL ile erişime açılabilir.

[Detaylı bilgi için tıklayınız](/tr/operasyon/yonetici-kilavuzlari/api-developer-portal-ssl).

Yukarıdaki adımlar tamamlandıktan ve Kubernetes üzerinde API Portal'e ait olan Pod'ların hazır durumda olduğundan emin olduktan sonra, Portal Arayüzüne aşağıdaki adresten erişim sağlanabilir:
...
<CardGroup cols={2}>
  <Card title="API Portal Genel Bakış" icon="window-maximize" href="/tr/api-portal/genel-bakis">
