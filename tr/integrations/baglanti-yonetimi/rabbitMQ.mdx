  ...
  <Accordion title="Çalışma Prensibi">
    <CardGroup cols={2}>
      ...
      <Card title="Connection Pool Yönetimi" icon="database">
        Tanımlı hostPortList'e göre round-robin stratejisiyle broker seçilir; mevcut açık bağlantı yoksa yeni bir AMQP bağlantısı otomatik olarak oluşturulur. Bu davranış sabittir, kullanıcı tarafından yapılandırılamaz
      </Card>
      ...
      <Card title="Bağlantı Yönetimi" icon="settings">
        İşlem tamamlandıktan sonra bağlantı yeniden kullanılmak üzere tutulur; sabit bir idle süre aşıldığında otomatik olarak kapatılır. Bu süre kullanıcı tarafından yapılandırılamaz
      </Card>
...
**Ayrı bir "Request Timeout" veya "Pool Size" alanı yoktur.** RabbitMQ bağlantısı, kullanıcı tarafından yapılandırılamayan tek bir AMQP `ConnectionFactory` nesnesi üzerinden kurulur; ayarlanabilir bir connection pool boyutu bulunmaz. Aşağıdaki iki alan (Connection Timeout, Requested Heartbeat) gerçek ve yapılandırılabilir alanlardır — UI'da bu alanlar için min/max doğrulaması yoktur, yalnızca zorunluluk kontrolü vardır.
...
<CardGroup cols={2}>
  ...
  <Card title="Düşük Gecikme İşleri" icon="zap">
    ...
    **Çözüm:** connectionFactoryRequestedHeartbeat=30  
...
<Step title="Timeout ve Protokol Ayarları">
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      <Card title="İçe Aktarma Adımları" icon="upload">
        - Ana listedeki **[Import RabbitMQ Bağlantısı]** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
