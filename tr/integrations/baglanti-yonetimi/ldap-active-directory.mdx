...
:::tip[Sertifikayla birlikte taşımanız gerekiyorsa]

Aşağıdaki **Export**/**Import**, connection'ın kendi alanlarını taşır; bir mTLS sertifikası bağladıysanız (Require Certificate Type = REQUIRED_AN_PN) o sertifika bu yoldan **taşınmaz**. LDAP Havuz Bağlantısı'nı sertifikasıyla birlikte taşımak için [Dışa/İçe Aktarma Sihirbazı](/tr/admin/secrets-management/export-import)'ndaki **Bağlantı (Connection)** tipini kullanın — bu tip sertifika/key store bağımlılığını otomatik olarak yüzeye çıkarır ve pakete dahil eder.
  ...
  <Accordion title="Dışa Aktarma (Export)">
    <CardGroup cols={2}>
      ...
      <Card title="ZIP İçeriği" icon="folder-open">
        ...
        - Metadata bilgileri
        - Sertifika veya key store dahil DEĞİLDİR — bunları birlikte taşımak için yukarıdaki Sihirbaz notuna bakın
      </Card>
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      ...
      <Card title="İçe Aktarma Senaryoları" icon="info-circle">
        ...
        **Senaryo 2: Sertifika Gerekiyor** → Bu paket sertifika taşımaz; Require Certificate Type = REQUIRED_AN_PN ise import sonrası Certificate Id'yi hedefte elle seçin, ya da sertifikayla birlikte taşımak için Sihirbaz'daki **Bağlantı (Connection)** tipini kullanın.
      </Card>
    </CardGroup>
  </Accordion>
