...
## Genel Bakış
  ...
  <Accordion title="Çalışma Prensibi">
    <CardGroup cols={2}>
      ...
      <Card title="Bağlantı Yönetimi" icon="database">
        Gateway, seçili ortam için uygun bir bağlantı yoksa yeni bir LDAP bağlantısı açar; bu davranış otomatiktir ve ekrandan yapılandırılamaz. Kullanıcının ayarlayabildiği tek değer bağlanma süresidir (Connection Timeout)
      </Card>
      ...
      <Card title="Bağlantı Sonlandırma" icon="link">
        İşlem tamamlandığında bağlantı sonraki istekler için Gateway tarafından otomatik olarak yönetilir; bu davranış ekrandan yapılandırılamaz
      </Card>
...
## Connection Parametreleri
  ...
  <Accordion title="İsteğe Bağlı Parametreler">
    <CardGroup cols={2}>
      ...
      <Card title="Dizin Türü" icon="sitemap">
        **Açıklama:** Dizin sunucusunun ürün ailesi (Active Directory, OpenLDAP, 389 Directory Server / FreeIPA, Diğer)  
        **Varsayılan Değer:** Bilinmiyor  
        **Notlar:** İsteğe bağlıdır ve bağlantının çalışma biçimini değiştirmez; yalnızca kimlik sağlayıcı ekranlarında doğru şema önerilerinin sunulmasını sağlar. **Dizini Tespit Et** ile otomatik önerilebilir.
      </Card>
    </CardGroup>
  </Accordion>
...
**Ayrı bir "Request Timeout", "Pool Size" veya "Bind Retry Interval" alanı yoktur.** LDAP bağlantısında yalnızca tek bir zaman aşımı alanı vardır: `Connection Timeout`. Bu değer LDAP sunucusuna bağlanma süresini sınırlar; alan boş bırakılırsa veya 0 ya da daha küçük bir değer girilirse sunucu tarafında varsayılan olarak 30000 ms kullanılır.
...
## Dizini Tespit Et ve Gelişmiş Bağlantı Testi

<Accordion title="Dizini Tespit Et">
  **Dizini Tespit Et** butonu, sunucuya dizin türünü, önerilen Base DN değerlerini ve sayfalama desteğini
  sorar. Soru API Manager üzerinden gönderilir; hiçbir şey kaydedilmez, yalnızca öneriler ekranda
  gösterilir ve tek tıkla ilgili alanlara yazılabilir. Sunucu bu bilgiyi paylaşmıyorsa girdiğiniz
  değerlerle devam edebilirsiniz.
</Accordion>

<Accordion title="Bağlantıyı Test Et (Adım Adım)">
  **Bağlantıyı test et (adım adım)** butonu, kaydetmeden önce bağlantıyı 7 ayrı adımda ölçer; eski tek
  sonuçlu **Test Connection** akışı da korunur.

  | # | Adım | Ne kontrol eder |
  |---|------|------------------|
  | 1 | Yapılandırma Kontrolü | Form alanlarının tutarlılığı |
  | 2 | Sunucu Bağlantısı | LDAP sunucusuna ağ üzerinden ulaşılabiliyor mu |
  | 3 | Güvenli Bağlantı (TLS) | LDAPS/StartTLS el sıkışması |
  | 4 | Servis Hesabı Girişi | Bind DN ile oturum açma |
  | 5 | Dizin Bilgisi | Sunucunun bildirdiği dizin türü ve ağaç kökleri |
  | 6 | Şema Kontrolü | Dizin şemasının okunabildiği; nesne sınıfı ve nitelik adları kimlik sağlayıcı ekranında denetlenir |
  | 7 | Başlangıç Noktası (Base DN) | Aramaların başlayacağı kökün okunabilirliği |

  Her adımda durum, açıklama, öneri ve kopyalanabilir bir teknik ayrıntı gösterilir.
</Accordion>

<Accordion title="Teşhis Modu">
  Bağlantı görüntüleme ekranındaki **Teşhis Modu**, seçtiğiniz süre boyunca (15, 30 veya 60 dakika) bu
  bağlantı üzerinden yapılan her LDAP araması ve oturum açma denemesini ayrıntısıyla kaydeder. Parolalar
  hiçbir zaman kaydedilmez. Süre dolduğunda kendiliğinden kapanır, yeniden başlatma gerekmez. Bu ayar
  kaydedildiğinde korunur, ancak dışa aktarma paketine girmez. Teşhis Modu açıkken tekrarlanan bir sorun,
  [Kimlik Bilgisi Senkronizasyonu](/tr/admin/identity-management/identity-providers/credential-sync)
  ekranındaki **Destek paketi**'ne de dahil edilir.
</Accordion>

:::info

Bağlantı listesindeki **Durum** sütunu, her bağlantı için beş dakikada bir çalışan bir sağlık kontrolünün
(giriş denemesi + temel arama) sonucunu, ölçülen gecikmeyle birlikte gösterir. Ortam değişkeni kullanan
bir bağlantı bu şekilde otomatik kontrol edilemez; bu bağlantılar için ekrandaki **Test Connection**
butonunu kullanın.
...
<CardGroup cols={2}>
  ...
  <Card title="Yüksek Trafikli Sorgular" icon="chart-line">
    ...
    **Çözüm:** Connection Timeout değeri gerçek network gecikmesine göre ayarlanır  
    **Beklenen Davranış:** Zaman aşımı hataları azalır, sorgular normal sürede tamamlanır
  </Card>
</CardGroup>
...
:::
    ...
    <Step title="Timeout Ayarı">
        **Connection Timeout:**
        ...
        - Minimum: 1000, Maksimum: 300000 milisaniye
        - Ayrı bir Request Timeout, Pool Size veya Bind Retry Interval alanı yoktur.
    </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Alternatif: Deaktif Etme" icon="toggle-off">
    ...
    - Connection pasif hale gelir ancak silinmez.
    - Gerektiğinde yeniden aktif edilebilir.
  </Card>
</CardGroup>
...
:::tip[Sertifika artık otomatik taşınır]

Liste ekranının satır menüsündeki **Export** ve araç çubuğundaki **Import** artık zaten [Dışa/İçe Aktarma Sihirbazı](/tr/admin/secrets-management/export-import)'ndaki **Bağlantı (Connection)** tipi üzerinden çalışır — bağlı bir mTLS sertifikasını (Require Certificate Type = REQUIRED_AN_PN) ve key store'u bağımlılık adımında otomatik olarak yüzeye çıkarır ve pakete dahil eder, ekstra bir adım gerekmez. Yalnızca satır menüsünün ikincil **Export as File** (Dosya Olarak Dışa Aktar) kalemi (aşağıda) hâlâ sertifikasız, connection'ın kendi alanlarını taşıyan ham bir ZIP üretir.
...
:::
  ...
  <Accordion title="Dışa Aktarma (Export)">
    <CardGroup cols={2}>
      ...
      <Card title="Yöntem 2: Liste Ekranından" icon="list">
        Satır sonundaki **⋮** menüsünde artık iki kalem vardır: birincil **Export**, bu connection ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nı açar — sertifika/key store'u ve varsa `${DEĞİŞKEN}` referansları bağımlılık adımında otomatik yüzeye çıkar (bkz. yukarıdaki not); ikincil **Export as File** (Dosya Olarak Dışa Aktar) ise sihirbaza girmeden aşağıda anlatılan ZIP dosyasını doğrudan indirir.
      </Card>
      ...
      <Card title="ZIP İçeriği (Export as File)" icon="folder-open">
        - Connection JSON dosyası
        - Metadata bilgileri
        - Sertifika veya key store dahil DEĞİLDİR — sihirbazın **Export**'u (yukarıdaki nota bakın) dahil eder
      </Card>
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      <Card title="İçe Aktarma Adımları" icon="upload">
        - Ana listedeki **Import** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
      ...
      <Card title="İçe Aktarma Senaryoları" icon="info-circle">
        ...
        **Senaryo 2: Sertifika Gerekiyor** → Yalnızca satır menüsünün ikincil **Export as File** kalemiyle üretilmiş bir paketin, legacy `/api/export-imports/import` ucundan uygulanmasında geçerlidir — orada sertifika taşınmaz; Require Certificate Type = REQUIRED_AN_PN ise import sonrası Certificate Id'yi hedefte elle seçin. Liste ekranının kendi **Import** butonu (yukarıda) sertifikayı sihirbaz üzerinden zaten taşır, bu boşluk orada oluşmaz.
      </Card>
    </CardGroup>
  </Accordion>
