...
## Genel Bakış
  ...
  <Accordion title="Çalışma Prensibi">
    <CardGroup cols={2}>
      ...
      <Card title="SSH Oturum Yönetimi" icon="terminal">
        Her betik çalıştırmasında yeni bir SSH oturumu açılır; kullanıcı tarafından yapılandırılabilen bir connection pool bulunmaz.
      </Card>
  ...
  <Accordion title="Zorunlu Parametreler">
    <CardGroup cols={1}>
      ...
      <Card title="SSH Port" icon="network">
        ...
        **Örnek Değer:** `22`  
        **Notlar:** Yaygın SSH portu 22'dir; alan zorunludur ve ön tanımlı (varsayılan) bir değeri yoktur. Farklı port kullanılıyorsa Network/Firewall'da açılmalıdır
      </Card>
...
:::
    ...
    <Step title="Linux Sunucu Parametreleri">
        ...
        - **SSH Port:** Yaygın SSH portu 22'dir; alan zorunludur ve ön tanımlı bir değeri yoktur. Farklı bir port kullanılıyorsa portu yazın ve firewall'da açıldığından emin olun.
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      <Card title="İçe Aktarma Adımları" icon="upload">
        - Ana listedeki **[Import Linux Script]** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
  ...
  <Accordion title="Yapılması Gerekenler ve En İyi Uygulamalar">
    <CardGroup cols={2}>
      ...
      <Card title="Kaynak Tüketimi" icon="activity">
        ...
        **En İyi:** Host bazında eşzamanlılığı sınırlı tutup uzun süren scriptleri ayrı zaman dilimlerine planlanmış job'larla dağıtmak
      </Card>
