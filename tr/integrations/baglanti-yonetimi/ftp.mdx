    ...
    <Accordion title="Zorunlu Parametreler">
        <CardGroup cols={2}>
            ...
            <Card title="Host" icon="server">
                ...
                **Notlar:** IPv4/IPv6 veya FQDN kabul edilir (örn. `ftp.partner.local`)
            </Card>
            ...
            <Card title="WorkingDir" icon="folder">
                ...
                **Varsayılan:** `/`\
                **Notlar:** İlgili kullanıcının erişim izni olmalı (örn. `/outbound`)
            </Card>
            ...
            <Card title="Timeout" icon="clock">
                ...
                **Varsayılan:** 30\
                **Notlar:** Saniye cinsindendir; tek değer birden çok zaman aşımına uygulanır (bkz. aşağıdaki bilgi kutusu)
            </Card>
    ...
    <Accordion title="İsteğe Bağlı Parametreler">
        <CardGroup cols={2}>
            ...
            <Card title="VerifyServerCertificate" icon="shield-check">
                **Açıklama:** FTPS sunucu sertifikasının doğrulanıp doğrulanmayacağını belirler\
                **Varsayılan Değer:** false\
                **Önerilen Değer:** true (production ortamında sertifika doğrulaması önerilir)
            </Card>
        </CardGroup>
    </Accordion>
...
**Timeout tek bir değerdir ve birden çok zaman aşımına aynı anda uygulanır.** Ayrı bir "Request Timeout" veya "Data Timeout" alanı **yoktur**. Girilen `Timeout` (saniye) değeri, gateway tarafında `timeout * 1000` ile milisaniyeye çevrilip şu bağlantı türlerinde aynı anda kullanılır:
- FTP/FTPS: varsayılan zaman aşımı, bağlantı kurma (connect) zaman aşımı ve veri (data) zaman aşımı
- SFTP: oturum (session) zaman aşımı
...
:::
    ...
    <Step title="FTP Specific Parametreler - Bölüm 2">
        ...
        * **Timeout:** Ağ gecikmesine göre 30-90 sn arasında bir değer önerilir.
    ...
    <Accordion title="İçe Aktarma (Import)">
        <CardGroup cols={2}>
            <Card title="İçe Aktarma Adımları" icon="upload">
                - Ana listedeki **\[Import FTP Bağlantısı]** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
