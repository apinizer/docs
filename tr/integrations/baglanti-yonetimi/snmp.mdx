  ...
  <Accordion title="Çalışma Prensibi">
    <CardGroup cols={2}>
      ...
      <Card title="Bağlantı Yönetimi" icon="settings">
        İşlem tamamlandıktan sonra bağlantı kapatılır; sürekli trap dinlemede soket açık tutulur
      </Card>
...
### Zaman Aşımı ve Yeniden Deneme Parametreleri

:::warning

SNMP bağlantısında ayrı bir "Request Timeout" veya "Pool Size" alanı yoktur; yalnızca tek bir zaman aşımı (saniye cinsinden) ve bir yeniden deneme sayısı bulunur.

:::

<CardGroup cols={2}>
  <Card title="Timeout (Zaman Aşımı)" icon="clock">
    **Açıklama:** SNMP isteği için yanıt beklenecek tek zaman aşımı değeri  
    **Varsayılan:** Yok (boş bırakılabilir)  
    **Birim:** saniye  
    **Min:** 0
  </Card>
  
  <Card title="Yeniden Deneme Sayısı (Retry Count)" icon="repeat">
    **Açıklama:** Başarısız SNMP isteklerinde yapılacak yeniden deneme sayısı; bir bekleme süresi (interval) değildir  
    **Varsayılan:** Yok (boş bırakılabilir)  
    **Birim:** adet  
    **Min:** 0
  </Card>
...
### Yeni SNMP Oluşturma
<img
    src="/images/yonetici/baglantilar/snmp-variable-new.png"
...
:::
    ...
    <Step title="Timeout ve Retry Ayarları">
        - Gelişmiş bölümde **Timeout** değerini saniye cinsinden ağ gecikmenize uygun şekilde ayarlayın (ayrı bir Request Timeout veya Pool Size alanı yoktur).
        - **Retry Count** ile başarısız iletiler için kaç kez yeniden deneneceğini belirleyin.
    </Step>
...
:::
  ...
  <Accordion title="Dışa Aktarma (Export)">
    <CardGroup cols={2}>
      ...
      <Card title="Yöntem 2: Liste Ekranından" icon="list">
        Satır sonundaki **⋮** menüsünde artık iki kalem vardır: birincil **Export**, bu connection ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nı açar — host/URL/parola alanındaki `${DEĞİŞKEN}` referansları varsa bağımlılık adımında otomatik yüzeye çıkar; ikincil **Export as File** (Dosya Olarak Dışa Aktar) ise sihirbaza girmeden aşağıda anlatılan ZIP dosyasını doğrudan indirir.
      </Card>
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      <Card title="İçe Aktarma Adımları" icon="upload">
        - Ana listedeki **[Import SNMP]** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
  ...
  <Accordion title="Performans İpuçları">
    <CardGroup cols={2}>
      <Card title="Trap Önceliklendirme" icon="arrow-up">
        **Öneri:** Kritik cihazlar için ayrı connection tanımlayın  
