  ...
  <Accordion title="Çalışma Prensibi">
    <CardGroup cols={2}>
      ...
      <Card title="Connection Pool Yönetimi" icon="layer-group">
        HTTP client, host başına sabit boyutlu bir bağlantı havuzu kullanır (host başına 20 bağlantı; toplam limit tanımlı host sayısıyla orantılı büyür). Bu değerler kullanıcı tarafından yapılandırılamaz.
      </Card>
  ...
  <Accordion title="Temel Özellikler">
    <CardGroup cols={2}>
      ...
      <Card title="İdari İşlemler" icon="gear">
        `Administrate` seçeneği açıkken Index Template, ILM Policy oluşturma ve rollover tetikleme butonları etkinleşir. Bağlantı listesinde ILM Policy, Index Template ve Data Stream durumunu gösteren göstergeler ile bu durumu Elasticsearch üzerinden sorgulayan **Live Check** butonu bulunur.
      </Card>
  ...
  <Accordion title="Zorunlu Parametreler">
    <CardGroup cols={2}>
      ...
      <Card title="Connection Timeout (ms)" icon="clock">
        **Açıklama:** Bağlantı kurma, yanıt bekleme ve pool'dan bağlantı alma süreleri için kullanılan tek değer  
        **Örnek Değer:** 5000  
        **Notlar:** Varsayılan 5000; UI'da yalnızca zorunluluk kontrolü vardır (min/max yok). Ayrıntı için aşağıdaki "Timeout Davranışı" bölümüne bakın.
      </Card>
...
### Timeout Davranışı ve Sabit Connection Pool Ayarları

:::warning

**Connection Timeout tek bir değerdir ve üç farklı amaç için kullanılır.** Ayrı bir "Request Timeout" alanı **yoktur**. Girilen `Connection Timeout (ms)` değeri, gateway tarafında aynı anda şu üç süre için kullanılır:
- Bağlantı kurma (connect) süresi
- Elasticsearch yanıtını bekleme (response) süresi
- Havuzdan bağlantı alma (connection request) süresi

1000 ms altındaki değerler, tam sayı bölmesi (ms / 1000) nedeniyle sunucu tarafında 0'a düşer ve otomatik olarak varsayılan 5 saniyeye (5000 ms) geri döner. Bu bir doğrulama kuralı değildir, bölme işleminin bir yan etkisidir — UI'da bu alan için min/max doğrulaması yoktur, yalnızca zorunluluk kontrolü vardır.

:::

<CardGroup cols={2}>
  <Card title="Connection Timeout" icon="clock">
    **Açıklama:** Bağlantı kurma + yanıt bekleme + pool'dan bağlantı alma için kullanılan tek değer  
    **Varsayılan:** 5000  
    **Birim:** milisaniye  
    **Notlar:** UI'da yalnızca zorunluluk kontrolü var; min/max yok. 1000 ms altı değerler 5000 ms'ye döner.
  </Card>
  
  <Card title="Connection Pool Boyutu" icon="layer-group">
    **Açıklama:** Host başına eşzamanlı bağlantı sayısı  
    **Değer:** 20 (sabit)  
    **Toplam Limit:** 20 × tanımlı host sayısı  
    **Notlar:** Kullanıcı tarafından yapılandırılamaz
  </Card>
  
  <Card title="Idle Bağlantı Doğrulama Süresi" icon="timer">
    **Açıklama:** Havuzdaki idle bağlantının tekrar kullanılmadan önce doğrulanma eşiği  
    **Değer:** 10 saniye (sabit)  
    **Notlar:** Kullanıcı tarafından yapılandırılamaz
  </Card>
  
  <Card title="Bağlantı Yaşam Süresi (TTL)" icon="hourglass">
    **Açıklama:** Bir bağlantının havuzda kalabileceği maksimum süre  
    **Değer:** 3 dakika (sabit)  
    **Notlar:** Keep-alive stratejisi 30 saniyedir; her ikisi de kullanıcı tarafından yapılandırılamaz
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="Yüksek Hacimli Log Yazımı" icon="chart-line">
    ...
    **Çözüm:** READ_WRITE, administrate açık, ILM rollover eşikleri sıkı tutulur  
    **Beklenen Davranış:** ILM otomatik rollover yapar; kayıtlar Worker'da arka planda (asenkron) bulk olarak yazılır, istek iş parçacığı Elasticsearch'i beklemez
  </Card>
...
:::
    ...
    <Step title="Timeout Ayarı">
        **Connection Timeout:**

        - **Connection Timeout (ms)** alanına değeri girin.
        - Varsayılan: 5000 milisaniye
        - Bu tek değer; bağlantı kurma, Elasticsearch yanıtını bekleme ve havuzdan bağlantı alma sürelerinin **hepsi** için aynı anda kullanılır. Ayrı bir "Request Timeout" alanı yoktur.
        - UI'da bu alan için yalnızca zorunluluk kontrolü vardır (min/max doğrulaması yok). 1000 ms altındaki değerler, tam sayı bölmesi nedeniyle sunucu tarafında otomatik olarak 5000 ms'ye (5 saniye) döner.

        :::info

Connection pool boyutu (host başına 20 bağlantı) ve idle bağlantı ayarları (validate-after-inactivity, TTL, keep-alive) sabittir; ekrandan yapılandırılamaz.
...
<CardGroup cols={2}>
  <Card title="ILM Policy Göstergesi" icon="calendar">
    `Administrate` aktif ve `Connection Type` `READ_WRITE` olan bağlantılarda görünür. Yeşil onay: policy Elasticsearch'te oluşturulmuş; kırmızı: oluşturulmamış. Göstergeye tıklayarak yeniden oluşturmayı deneyebilirsiniz.
  </Card>

  <Card title="Index Template Göstergesi" icon="file-code">
    `Administrate` aktif ve `Connection Type` `READ_WRITE` olan bağlantılarda görünür. Yeşil onay: template Elasticsearch'te oluşturulmuş; kırmızı: oluşturulmamış. Göstergeye tıklayarak yeniden oluşturmayı deneyebilirsiniz.
  </Card>

  <Card title="Data Stream Göstergesi" icon="database">
    `Administrate` aktif ve `Connection Type` `READ_WRITE` olan bağlantılarda görünür. Yeşil onay: data stream Elasticsearch'te oluşturulmuş; kırmızı: oluşturulmamış. ILM Policy ve Index Template göstergelerinin aksine bu gösterge tıklanarak doğrudan oluşturma tetiklenemez; ILM Policy ve Index Template hazır olduğunda data stream **Save and Deploy** sırasında otomatik olarak oluşturulur.
  </Card>

  <Card title="Live Check (Canlı Durum Kontrolü) Butonu" icon="rotate">
    Bağlantı listesindeki her satırda görünen yenile ikonudur. Tıklandığında ILM Policy, Index Template ve Data Stream'in Elasticsearch cluster'ında gerçekten var olup olmadığı anlık olarak sorgulanır; bağlantı kaydı bu gerçek durumla güncellenir ve son kontrol zamanı butonun altında görüntülenir.
  </Card>
</CardGroup>

:::note Doğrulanamadı durumu (2026.09.3)

Canlı Durum Kontrolü sırasında cluster'a ulaşılamazsa (erişilemez, zaman aşımı ya da kimlik hatası) üç gösterge kırmızıya dönmek yerine turuncu **soru işareti** olur. Kayıtlı "oluşturuldu" değerleri korunur, ipucu başarısız kontrolün zamanını gösterir, bildirim nedeni taşır. Bir göstergeyi kırmızıya çeviren tek yanıt Elasticsearch'ten gelen gerçek "bulunamadı" (404) yanıtıdır; kapalı bir cluster doğrulanmış bir yeşili asla ezmez. Cluster erişilebilir olunca Live Check'i yeniden çalıştırın.

:::

:::info

Live Check çalıştırılmadan önce göstergeler yalnızca Apinizer'ın kendi kaydını yansıtır. Bağlantı farklı bir Elasticsearch cluster'ına yönlendirildiğinde eski durum ekranda görünmeye devam edebilir; güncel durumu görmek için Live Check kullanmanız gerekir.

:::

:::warning

Bağlantı pasif (Disabled) durumdayken Live Check çalıştırılamaz; buton tıklandığında uyarı mesajı gösterilir.

:::

:::info

Kontrol tamamlanamazsa (örneğin cluster'a erişilemiyorsa) hata mesajı gösterilir, ancak kayıtlı göstergeler ve son kontrol zamanı **değiştirilmez**. Erişilemiyor olmak "yok" ile aynı şey değildir — geçici bir kesinti, bilinen doğru durumun üzerine yazmaz.
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      <Card title="İçe Aktarma Adımları" icon="upload">
        - Ana listedeki **[Import Elasticsearch]** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
...
## Best Practices
  ...
  <Accordion title="Kaçınılması Gerekenler">
    <CardGroup cols={2}>
      ...
      <Card title="Az Sayıda Host ile Yüksek Trafik" icon="layer-group">
        **Neden kaçınılmalı:** Connection pool boyutu host başına sabittir (20); tek host'a karşı yüksek eşzamanlı trafik pool'un tükenmesine yol açabilir.  
        **Alternatif:** Yüksek trafikte birden fazla host tanımlayın — toplam pool limiti host sayısıyla orantılı büyür. Pool boyutu ekrandan ayarlanamaz
      </Card>
    </CardGroup>
  </Accordion>
  ...
  <Accordion title="Performans İpuçları">
    <CardGroup cols={2}>
      ...
      <Card title="Thread Pool İzleme" icon="chart-line">
        **Öneri:** Monitor ekranından bulk/search queue uzunluklarını takip edin; eşikler aşarsa host sayısını artırmayı değerlendirin (pool boyutu host başına sabittir, ekrandan değiştirilemez).  
        **Etki:** Kuyrukta bekleyen istek sayısı azalır
      </Card>
      
      <Card title="Connection Pool Farkındalığı" icon="layer-group">
        **Öneri:** Pool boyutunun host başına sabit (20) olduğunu ve ekrandan değiştirilemediğini unutmayın; peak trafik için pool kapasitesini artırmanın tek yolu daha fazla host tanımlamaktır.  
        **Etki:** Bağlantı açma maliyeti azalır, yanıt süreleri düşer
      </Card>
...
Connection pool boyutu host başına sabittir (20 bağlantı); tek host'a karşı çok yüksek eşzamanlı trafik veya uzun süren istekler pool'un tükenmesine yol açabilir. Pool boyutu ve idle timeout gibi değerler ekrandan değiştirilemez.

:::
    
    <Steps>
      <Step title="Host Sayısını Artırma">
        Trafiği birden fazla host'a dağıtmak için ek Elasticsearch host tanımlayın; toplam pool limiti host sayısıyla orantılı büyür.
      </Step>
      
      <Step title="Connection Kontrolü">
        Elasticsearch cluster tarafında uzun süren veya asılı kalan isteklerin olmadığını kontrol edin.
      </Step>
      
      <Step title="Timeout Kontrolü">
        Connection Timeout değerinin gerçekçi olduğunu doğrulayın; 1000 ms altındaki değerler her durumda 5000 ms'ye döner.
      </Step>
...
:::
  ...
  <Accordion title="Connection pool boyutu ayarlanabilir mi?">
    :::info

Hayır. Apinizer, Elasticsearch bağlantıları için connection pool'u her zaman otomatik olarak kullanır (host başına 20 bağlantı; toplam limit host sayısıyla orantılı büyür). Pool boyutu veya idle bağlantı ayarları ekrandan yapılandırılamaz.
...
:::
  ...
  <Accordion title="Live Check butonu ne işe yarar?">
    :::info

ILM Policy, Index Template ve Data Stream'in Elasticsearch cluster'ında gerçekten var olup olmadığını sorgular ve bağlantı kaydını bu gerçek durumla senkronlar. Bağlantı farklı bir Elasticsearch cluster'ına yönlendirildiğinde, göstergelerin eski cluster'a ait durumu göstermeye devam etmesini önler. Kontrol sırasında cluster'a erişilemezse hata mesajı gösterilir ve kayıtlı göstergeler sıfırlanmadan olduğu gibi kalır — geçici bir kesinti "yok" anlamına gelmez.

:::
  </Accordion>

  <Accordion title="Data Stream göstergesine tıklayarak data stream oluşturabilir miyim?">
    :::info

Hayır. ILM Policy ve Index Template göstergelerinin aksine Data Stream göstergesi yalnızca durumu gösterir, tıklanamaz. Data stream, ILM Policy ve Index Template hazır olduğunda Save and Deploy sırasında otomatik oluşturulur; mevcut durumu görmek için Live Check butonunu kullanabilirsiniz.
