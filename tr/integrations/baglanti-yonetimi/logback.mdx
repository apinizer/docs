...
## Genel Bakış
  ...
  <Accordion title="Çalışma Prensibi">
    <CardGroup cols={2}>
      ...
      <Card title="Dosya Tanıtıcı Yönetimi" icon="layer-group">
        Logback RollingFileAppender, worker node üzerinde aynı log dosyası için açık dosya tanıtıcısını (file handle) tek bir kaynak olarak yeniden kullanır; ayarlanabilir bir connection pool kavramı yoktur.
      </Card>
      ...
      <Card title="Hata Yönetimi" icon="triangle-exclamation">
        Bağlantı hatası veya authentication hatası durumunda deployment-result dialog'u ayrıntılı hata mesajıyla devreye girer, kullanıcıya log dizini ve yetki kontrollerini gözden geçirmesi önerilir.
      </Card>
    </CardGroup>
  </Accordion>
...
Logback bağlantısı yerel dosya sistemine yazar; ağ zaman aşımı ve connection pool parametreleri bu connector için geçerli değildir.
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      <Card title="İçe Aktarma Adımları" icon="upload">
        - Ana listedeki **[Import Logback]** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
  ...
  <Accordion title="Performans İpuçları">
    <CardGroup cols={2}>
      ...
      <Card title="Connection Monitoring" icon="chart-bar">
        **Öneri:** Disk kullanım oranını ve log yazma hatalarını izleyin, worker pod health check'lerini alerting sistemine bağlayın  
        **Etki:** Sorunlar proaktif tespit edilir, disk dolma riskleri erken belirlenir, kesinti süresi azalır
      </Card>
    </CardGroup>
  </Accordion>
