  ...
  <Accordion title="Amacı Nedir?">
    <CardGroup cols={2}>
      ...
      <Card title="Performans ve Dayanıklılık" icon="gauge">
        Gelişmiş timeout yönetimi sayesinde yüksek trafikli senaryolarda performans ve dayanıklılık kazandırır.
      </Card>
    </CardGroup>
  </Accordion>
  ...
  <Accordion title="Çalışma Prensibi">
    <CardGroup cols={2}>
      ...
      <Card title="Bağlantı Yönetimi" icon="gear">
        İşlem tamamlandığında bağlantı kapatılır; kapatma işleminde Close Timeout değeri uygulanır.
      </Card>
...
**Oturum Onayı Politikası:** CLIENT_ACKNOWLEDGE varsayılanı ile güvenilir teslim sağlar; AUTO_ACKNOWLEDGE ile hızlı entegrasyon veya SESSION_TRANSACTED ile transactional kontrol seçilebilir.
...
**Connection Monitoring:** Bağlantı sağlığı ve performans metriklerini izleme.
  ...
  <Accordion title="İsteğe Bağlı Parametreler">
    <CardGroup cols={2}>
      ...
      <Card title="Client ID" icon="id-card">
        ...
        **Varsayılan Değer:** apinizer-activemq-clientid  
...
<CardGroup cols={2}>
  ...
  <Card title="Uzun Süreli İşlem" icon="hourglass">
    ...
    **Çözüm:** Send Timeout=120000, Request Timeout=120000  
  ...
  <Card title="Test Ortamı Simülasyonu" icon="flask">
    ...
    **Çözüm:** Request Timeout=45000, Content Type=application/json  
...
<img
  ...
  alt="Apache ActiveMQ Oluşturma Ekranı"
  width="1000"
  className="mr-auto"
/>
...
<Step title="Timeout Ayarları">
  ...
  - Yoğun trafik için timeout sürelerini broker yanıt sürelerine göre optimize edin.
</Step>
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      <Card title="İçe Aktarma Adımları" icon="upload">
        - Ana listedeki **[Import Apache ActiveMQ]** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
...
<CardGroup cols={2}>
  ...
  <Card title="Integration / Connector Adımlarında Kullanım" icon="plug">
    - ActiveMQ kuyruğuna (queue) veya topic'ine mesaj gönderilmesi ya da buradan mesaj tüketilmesi gereken adımlarda connection seçilir.
    - Örnek: "Send Message", "Consume Message" gibi adımlar.
...
| Gözlemlenebilirlik | **Kötü:** Connection loglarını takip etmemek.<br/><br/>**İyi:** Temel hata loglarını izlemek.<br/><br/>**En İyi:** Timeout ve broker metriklerini merkezi izleme sistemine göndermek. |
...
| Message Throughput | **Öneri:** Yüksek hacimli gönderimlerde Send Timeout değerini artırın.<br/><br/>**Etki:** Mesaj kuyrukları birikmeden tüketilir. |
...
| Connection Monitoring | **Öneri:** Timeout oranlarını takip edin, connection health check yapın, alerting kurun.<br/><br/>**Etki:** Sorunlar proaktif tespit edilir, performans darboğazları erken belirlenir, kesinti süresi azalır. |
