...
<CardGroup cols={2}>
  ...
  <Card title="Toplu Kampanya" icon="envelope">
    ...
    **Çözüm:** DeployToWorker=true, Description=Campaign  
...
:::
    ...
    <Step title="Timeout ve Connection Pool Ayarları">
        Bu connector'da ayrı bir timeout veya connection pool alanı yoktur; SMTP tarafı zaman aşımları **Additional Properties** bölümünde `mail.smtp.*` anahtarlarıyla verilebilir (Adım 7'de detaylı).
    </Step>
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      <Card title="İçe Aktarma Adımları" icon="upload">
        - Ana listedeki **[Import E-Posta]** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
