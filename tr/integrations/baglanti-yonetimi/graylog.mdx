...
## Genel Bakış
  ...
  <Accordion title="Çalışma Prensibi">
    <CardGroup cols={2}>
      ...
      <Card title="Kuyruk ve Gönderim Yönetimi" icon="database">
...
<CardGroup cols={2}>
  ...
  <Card title="Gönderim İş Parçacığı Sayısı (Threads)" icon="cog">
...
:::
    ...
    <Step title="Zaman Aşımı ve Gönderim Ayarları">
  ...
  <Accordion title="İçe Aktarma (Import)">
    <CardGroup cols={2}>
      <Card title="İçe Aktarma Adımları" icon="upload">
        - Ana listedeki **[Import Graylog]** butonuna tıklayın — bu buton artık tip **Bağlantı (Connection)** ön-seçili olarak Dışa/İçe Aktarma Sihirbazı'nın içe aktarma adımını açar (Administration'dan açıldığında Admin kapsamında); eski yükleme diyaloğu kaldırıldı.
...
:::
  ...
  <Accordion title="Kuyruk Doldu / Gönderim Geride Kalıyor (Queue Backpressure)">
    :::warning
