...
:::warning 2026.09.2'ye Yükseltme — Kimlik ve Limit Taşıması
Bu sürüme yükselten bir Manager, açılışta **kimlik ve limit verilerinizi kendiliğinden taşır** — operatör müdahalesi gerekmez (bkz. [Eski Kayıtlar: Otomatik Taşıma → Yükseltmede Otomatik Taşıma](/tr/admin/identity-management/kimlik-bilgileri#yukseltmede-otomatik-tasima)). Taşıma, Manager'ın HTTP dinleyicisi açılmadan önce çalışır; yaklaşık 4.000 kayıtlık bir kurulumda birkaç dakika sürer ve bu süre boyunca Manager istek kabul etmez. Sonuçlar `api_client_migration_run` / `api_client_migration_row` kayıt defteri koleksiyonlarına yazılır ve taşıma tamamen yakınsayana kadar her gün 03:45'te çalışan bir arka plan görevi eksik kalan kayıtları yeniden dener.

Bu taşıma için bir **geri alma (rollback) mekanizması yoktur** — yükseltmeden önce aşağıdaki adımda anlatılan MongoDB yedeğini almanız tek geri dönüş yoludur. Aynı sürümle birlikte **[Rate Limit Kontrol Listesi](/tr/admin/identity-management/rate-limit-kontrol-listesi)** emekliye ayrılmıştır ve **[API Proxy ACL](/tr/admin/identity-management/api-proxy-acl)** / **[API Proxy Grup ACL](/tr/admin/identity-management/api-proxy-grup-acl)** satırlarındaki ortam bazlı Daraltma/Kota alanları artık gateway tarafından uygulanmaz. Ortam bazlı **AI token bütçesi** alanı da aynı adımda typed bir AI Token Bütçesi atamasına taşınır; sayaç **sıfırlanmaz**, taşınmış tüketici/istemci aynı sayaç havuzunda kalmaya devam eder — bu, sıfırdan başlayan İstek Hızı sayaçlarının aksinedir. **AI Token Bütçesi** ailesi de artık gölge modunda değildir, typed atamalar gerçekten uygulanır; gölge karşılaştırmasını kapatan `apinizer.limit.shadow.enabled` JVM sistem özelliği bu sürümde kaldırılmıştır.

Taşınmış kimlik bilgilerinin eski (legacy) parola kopyaları hemen silinmez — kimlik adımı tamamlandıktan **7 gün sonra** günlük bir arka plan görevi tarafından temizlenir (bkz. [Eski Kayıtlar: Otomatik Taşıma → Yükseltmede Otomatik Taşıma](/tr/admin/identity-management/kimlik-bilgileri#yukseltmede-otomatik-tasima)). **Bu 7 günlük pencere içinde kurulumunuzdaki tüm gateway'leri (Worker) bu sürüme yükseltin** — eski sürüm bir gateway pod'u bu pencereden sonra yeniden başlarsa taşınmış kimlik bilgilerini doğrulayacak legacy parolayı artık bulamaz.

Bu sürüm ayrıca OAuth2 erişim ve yenileme token'larını yalnızca özet (digest) ile saklanan bir yapıya taşır — ne değiştiğini [Token Servisi](/tr/develop/politikalar/token-service/genel-bakis) sayfasında bulabilirsiniz. Yukarıdaki açılış-anı adımı eksik kalan özetleri tamamlar ve ardından eski düz metin sütunlarını kaldırır; ayrıca bir işlem yapmanız gerekmez. Bu değişikliğe özgü olarak, **gateway (Worker) düğümlerini Manager'dan önce yükseltin**: henüz yükseltilmemiş bir Worker, zaten yükseltilmiş bir Manager'a karşı eski düz metin sütunlarını yazmaya devam ederse, bu sütunlar yalnızca bir sonraki temizlik geçişine kadar kullanılmadan kalır — bu bir güvenlik açığı değil, geçici bir fazlalıktır.

Bu sürüm ayrıca [Yasal Saklama ve Veri Yaşam Döngüsü Temizleme](/tr/admin/identity-management/yasal-saklama-ve-temizleme) özelliğini tanıtır. Yükseltme, yeni **Veri Yaşam Döngüsü** yetki kategorisini ve üç yeni koleksiyonu (yasal saklamalar, temizleme koşuları, temizleme koşusu bulguları) ekler; mevcut verileriniz bunun dışında değişmeden kalır. Yeni yetki otomatik olarak yalnızca **Project Owner** rolüne verilir — önceden tanımlı başka hiçbir role yasal saklama koyma veya temizleme çalıştırma yeteneği verilmez; böylece hiçbir mevcut kullanıcı beklenmedik biçimde veriyi kalıcı olarak silme yeteneği kazanmaz. [Log Saklama](/tr/admin/system-settings/sistem-ayarlari-ve-limitler#log-saklama-log-retention) altındaki diğer kalemlerin yanına, varsayılan olarak kapalı olan özel bir **Lifecycle Purge Run History** (Temizleme Koşusu Geçmişi) saklama ayarı eklenir. Elasticsearch API trafik logları bu özellik tarafından hiçbir yönde yeniden yazılmaz — bunların nasıl zamanla düştüğü için bkz. [Yasal Saklama ve Veri Yaşam Döngüsü Temizleme → Veri Sahibi Silme](/tr/admin/identity-management/yasal-saklama-ve-temizleme#veri-sahibi-silme-ozne-tipine-gore-kapsam).

Token özet (digest) taşıması, çok büyük bir token geçmişine sahip kurulumlarda yükseltme sonrası **ilk açılış süresini biraz uzatabilir** — bu, eksik özetleri tamamlayan açılış-anı adımının kendisinden kaynaklanır ve operatör müdahalesi gerektirmez; ilerlemeyi Manager (Mongock migration) logundan izleyebilirsiniz.

Bu sürümde ayrıca, deploy edilmiş bir kimlik bilgisinin IP listesinin her istekte büyüyüp global ve ortam listelerinin birleşiminin kalıcı hâle gelmesine yol açan bir davranış düzeltildi. **IP kısıtlaması kullanan müşteriler yükseltme sonrasında listelerini gözden geçirsin** — daha önce kabul edilen bir IP artık reddedilebilir.

Hangi dizinden geldiği hesabında kayıtlı olmayan bir LDAP kullanıcısı, yapılandırmada **devre dışı bırakılmış bir LDAP bağlantısı bulunduğu sürece** artık içeri alınmıyor. Bu sürümden önce böyle bir hesap yalnızca açık bağlantılarda aranıyordu; bu, hesabın kalıcı olarak yanlış dizine bağlanmasına ve bilerek kapattığınız bir dizinin kullanıcılarının girmeye devam etmesine yol açabiliyordu. **Yapılandırmanızda kapalı bir LDAP bağlantısı duruyorsa onu açın ya da silin** — kurulumun tamamı için tek işlem — veya etkilenen kullanıcıları bir kez açıp kaydedin; böylece hesap kendi bağlantısını kaydeder. Bağlantısı zaten kayıtlı olan hesaplar etkilenmez, reddedilen girişler ise gerekçesiyle birlikte [Giriş Kayıtları](/tr/admin/audit-compliance/giris-kayitlari)'na yazılır.

:::

:::danger 2026.09.2'ye Yükseltme — Büyük/Küçük Harf Duyarsız Yinelenen Kullanıcı Adı

Bu sürüm, kullanıcı adlarının (login) büyük/küçük harf farkı gözetmeksizin tekil olmasını zorunlu kılan bir dizin kurar. Yükseltmeden **önce**, aşağıdaki §1'de anlatıldığı gibi MongoDB yedeğini aldıktan sonra, MongoDB sunucusunda şu sorguyu çalıştırarak mevcut kullanıcılarınız arasında yalnızca büyük/küçük harfle ayrışan bir çift olup olmadığını kontrol edin:
...
Sonuç boşsa bir işlem gerekmez. Bir veya daha fazla grup dönerse, yükseltmeden önce her grupta yalnızca bir kullanıcı adı kalacak şekilde hesapları **Kullanıcılar** ekranından yeniden adlandırın ya da fazla olan hesabı silin.

Bu adım atlanırsa: Manager yine açılır ve **mevcut kullanıcılarınız normal şekilde giriş yapmaya devam eder** — dizin yalnızca kurulamaz ve günlüğe bir uyarı yazılır. Ancak dizin kurulana kadar **yeni bir LDAP kullanıcısının ilk girişte hesabının otomatik oluşturulması reddedilir** (daha önce hesabı oluşturulmuş LDAP kullanıcıları etkilenmez). Bu nedenle **Manager'ın sorunsuz açılması, dizinin kurulduğunun kanıtı değildir**: yükseltme sonrasında Manager günlüğünde `202609-006722` etiketli satırı arayın — "installed/verified" ile bitiyorsa dizin kuruludur, "NOT installed" ile bitiyorsa yukarıdaki sorguyla yinelenen kayıtları giderip Manager'ı yeniden başlatın; bu adım her açılışta otomatik olarak yeniden denenir.

:::

:::danger 2026.09.2'ye Yükseltme — Denetim Olayı / Login Log tablosu kolonları (yalnızca SQL/JDBC connector)

Kurulumunuz **[Denetim Kayıtları](/tr/admin/audit-compliance/denetim-kayitlari)**'nı veya **[Giriş Kayıtları](/tr/admin/audit-compliance/giris-kayitlari)**'nı SQL/JDBC connector'ı üzerinden ilişkisel bir veritabanına gönderiyorsa, [Denetim ve Login Log Tablosu ALTER Scriptleri](/tr/release-notes/2026-09-2-denetim-login-tablosu-alter)'ni o veritabanına karşı yükseltmeden **önce** çalıştırın — "önce veya sırasında" değil, çünkü bu sürümün bu iki tabloya karşı yaptığı INSERT koşulsuzdur. Script çalışana kadar her Denetim Olayı satırı yazılamaz (yine de yapılandırılmış failover connector'ınıza ulaşır) ve her Login Log satırı **failover olmadan** yazılamaz — tamamen düşer. Bu iki log tipi için Elasticsearch veya MongoDB connector'ı kullanıyorsanız bu adım geçerli değildir.
...
Nexus veya Harbor gibi kurumsal registry'lerde node yapılandırması ve deployment imaj path güncellemesi için [Container Registry](/tr/operations/container-registry/container-imajlari-ve-registry) bölümüne bakın. Aşağıdaki adımlar imajları registry'ye **push** etme senaryosu içindir.
...
:::warning 1.0.0 öncesinden yükseltiyorsanız — Secret silinmesin
...
:::danger 2026.09.2'den Geri Dönüş

2026.09.2'de açılışta çalışan **kimlik ve limit taşıması** uygulama sürümü düşürülerek geri alınamaz. Bu sürümden geri dönmeniz gerekirse tek yol, aşağıdaki §1'de anlatıldığı gibi yükseltmeden önce alınmış **MongoDB yedeğinin geri yüklenmesidir**.
