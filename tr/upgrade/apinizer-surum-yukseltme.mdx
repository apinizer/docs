...
Taşınmış kimlik bilgilerinin eski (legacy) parola kopyaları hemen silinmez — kimlik adımı tamamlandıktan **7 gün sonra** günlük bir arka plan görevi tarafından temizlenir (bkz. [API İstemci Geçişi → Yükseltmede Otomatik Taşıma](/tr/admin/identity-management/api-client-tasima#yukseltmede-otomatik-tasima)). **Bu 7 günlük pencere içinde kurulumunuzdaki tüm gateway'leri (Worker) bu sürüme yükseltin** — eski sürüm bir gateway pod'u bu pencereden sonra yeniden başlarsa taşınmış kimlik bilgilerini doğrulayacak legacy parolayı artık bulamaz.
