...
description: "Log yönetiminin manuel olarak yönetilmesi durumunda Apinizer'ın ihtiyaç duyduğu log varlıklarının manuel oluşturulması için gerekli tanımlamaları sağlar. Elasticsearch cluster'ında ILM politikası ve Template oluşturma işlemlerini adım adım açıklar."
---
...
| `<INDEX_KEY>` | Index'in benzersiz tanımlayıcısıdır. |
| `<INDEX_PATTERN_NAME>` | Index pattern adıdır (örn: apinizer-log-apiproxy-*). |
| `<ILM_POLICY_NAME>` | ILM politikası adıdır (örn: apinizer-log-ilm-policy-default). |
| `<NUMBER_OF_SHARDS>` | Index'in shard sayısıdır. |
| `<NUMBER_OF_REPLICAS>` | Index'in replica sayısıdır. |
| `<REFRESH_INTERVAL>` | Index refresh aralığıdır (örn: 1s). |

## ILM Politikaları Oluşturma
...
```bash
curl -X PUT "http://localhost:9200/_index_template/apinizer-log-apiproxy-template-default" \
...
| `pxt` | Politika Çalışma Süreleri (yalnızca log hedefinde ayar açıkken yazılır) | List&#60;PolicyExecutionTiming&#62; | nested o: integer t: keyword d: integer f: keyword (ignore_above 1) |

:::info

Bu veri yapısında oluşan veri örneğini [API Trafiği Log Kaydı Veri Yapısı](/tr/analytic/api-trafigi-log-kaydi-veri-yapisi) sayfasında inceleyebilirsiniz.

:::

## Indeks Oluşturma ve Sorgulama
