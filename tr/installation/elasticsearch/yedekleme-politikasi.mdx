...
description: "API Trafik loglarının Apinizer yönetimindeki Elasticsearch veri tabanında tutulduğunda kullanılabilecek log yedekleme politikalarını açıklar. Elasticsearch snapshot alma, geri yükleme ve otomatik yedekleme işlemlerini detaylandırır."
---

İlk olarak yedeklemenin nasıl yapılacağı konusu netleştirilmelidir. Sık kullanılan yöntemler aşağıda açıklanmıştır.

### Yöntem 1: path.data Adresinin Yedeklenmesi

Elasticsearch'ün konfigürasyon dosyasında tanımlı olan `path.data` adresi, sistem yöneticileri tarafından incremental şekilde veya belirli aralıklarla yedeklenebilir.

:::info

Geçmiş verilere erişim her zaman aynı kolaylıkta devam eder.

:::

:::warning

Hem aktif hem de yedek disk sürekli büyümeye devam eder. Asıl diskte bir sorun oluştuğunda, yedek disk üzerindeki verilere erişebilmek için sistemin tekrar kurulması gerekir.

:::

### Yöntem 2: Sunucu Seviyesinde Disk Yedekleme (RAID-0)

Elasticsearch'ün bulunduğu sunucu RAID-0 yöntemiyle ya da belirli aralıklarla yedeklenebilir.

:::info

Geçmiş verilere erişim her zaman aynı kolaylıkta devam eder. Asıl diskte sorun oluştuğunda, yedek disk network yönlendirmesiyle anında kullanıma alınabilir.

:::

:::warning

Hem aktif hem de yedek disk sürekli büyümeye devam eder.

:::

### Yöntem 3: Elasticsearch Snapshot API

Elasticsearch verileri Snapshot API'si ile dump alınabilir. Snapshot politikası ayarlanarak belirli bir sistem adresine yedek dosyalarının çıkarılması sağlanabilir; bu yedek dosyaları daha sonra farklı bir sunucuya ayrıca yedeklenmelidir.
...
**Öneri**
  ...
  Yukarıdaki yöntemlerden hangisi kullanılırsa kullanılsın, düzenli yedekleme ayarlandıktan sonra yedeği alınmış loglar Elasticsearch ILM'siyle otomatik silmeye ayarlanabilir ya da istenilen zamanlarda istenilen kadarı Elasticsearch API'si ile manuel olarak silinebilir. Bu şekilde aktif sunucu çok daha düşük bir disk kaynağıyla çalışmaya devam eder. Ancak bu durumda geçmiş verilere erişmek için yedek disk üzerinde bir uygulama kurulması ya da yedeklerin ayrı bir sunucuya aktarılıp orada çalışılması gerekir; bu durumda artık sadece yedek disk büyümeye devam eder.
...
### Repository'i Doğrulama İsteği (Verify Location)
...
Komutun indices kısmında birden fazla değer girilebileceği gibi wildcard olarak * değeri de kullanılabilir.
