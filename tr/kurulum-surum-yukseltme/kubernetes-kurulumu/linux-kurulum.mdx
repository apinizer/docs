...
description: 'Bu doküman, RHEL (Red Hat Enterprise Linux) 8.x / 9.x işletim sistemine sahip bir sunucuya Kubernetes 1.31 sürümünün kurulumunu anlatmaktadır. Örnek kurulum Red Hat 8.6 İşletim sisteminde yapılmıştır. Kurulacak paketlerin sürümleri dışında işletim sistemi sürümleri arasında kurulum yönergelerinde fark bulunmamaktadır. Bu kurulum yönergeleri Rocky Linux dağıtımlarında da kullanılabilir.'
---

Bu doküman, RHEL (Red Hat Enterprise Linux) 8.x / 9.x işletim sistemine sahip bir sunucuya Kubernetes **1.31** sürümünün kurulumunu anlatmaktadır. Örnek kurulum Red Hat 8.6 İşletim sisteminde yapılmıştır. Kurulacak paketlerin sürümleri dışında işletim sistemi sürümleri arasında kurulum yönergelerinde fark bulunmamaktadır. Bu kurulum yönergeleri Rocky Linux dağıtımlarında da kullanılabilir.
...
Bu kılavuzda Flannel ağ eklentisini kullanacağız. Desteklenen diğer ağ eklentilerini seçebilirsiniz. Flannel, Kubernetes için tasarlanmış bir Layer-3 ağ yapısını yapılandırmanın basit ve kolay bir yoludur.
...
Daha fazla bilgi için [lütfen bu linkteki adresi inceleyiniz.](https://kubernetes.io/docs/tasks/administer-cluster/dns-custom-nameservers/#inheriting-dns-from-the-node)
