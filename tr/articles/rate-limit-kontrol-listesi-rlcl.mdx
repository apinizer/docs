...
:::info Tarihsel yazı
Burada anlatılan Rate Limit Kontrol Listesi 2026.09 sürümünde emekliye ayrıldı, ekranı 2026.09.3 sürümünde kaldırıldı. Mevcut listeler yükseltmede otomatik olarak [Limit Planları](/tr/ai-gateway/limit-planlari) ve atamalara dönüştürüldü; aynı hedefli kurallar artık [kural ataması](/tr/ai-gateway/limit-planlari#kural-atamaları) olarak yazılır. Yazı, tasarımın arkasındaki gerekçe için korunmaktadır.
:::
...
1. **İstek gelir.**
2. **İsteğin kimliği belirlenir**: seçilen kimlik kaynağına göre ya bir değişkenin değeri (IP, header, kullanıcı ID vs.) ya da kimlik doğrulamadan gelen kullanıcı kullanılır.
3. **Bu kimliğin hedef kitlede olup olmadığına bakılır** (kimlik listesinde mi yer alıyor, yoksa hedef kitle koşullarından biriyle mi eşleşiyor?).
4. **Eğer hedef kitledeyse**, ona özel tanımlanmış rate limit kuralı devreye girer.
5. **Eğer hedef kitlede değilse**, tanımladığınız hedef-dışı eyleme göre davranılır: istek ya engellenir ya da hedef-dışı istekler için ayrılan genel kota uygulanır.
...
:::info Kimlik Kaynağı Kilitlemesi
...
- Belirli bir değeri **içeren** API anahtarları
...
- **Sayım Modu**: Limite takılıp reddedilen istekler de sayaca dahil olsun mu? Varsayılan (Tüm istekler) modda dahil olur — Apinizer'ın tarihsel davranışıdır. Yalnızca kabul edilenler seçilirse reddedilen bir istek limiti hiç tüketmez.
- **Limit Değişimi Davranışı**: İzin verilen istek sayısı değiştirildiğinde açık penceredeki sayaca ne olacağını belirler. Varsayılan (Önceki limite kırp) sayacı önceki limit değerine kırpar (yeni değere değil); böylece limit yükseltildiğinde kalan hak kadar istek hemen geçebilir. Sayacın yalnız limit yükseltildiğinde sıfırlanması sağlanabilir ya da sayaç hiç değiştirilmeden bırakılabilir.
...
:::tip Şablon ve Değişken Desteği
