...
| --- | --- | --- |
| **OTel Operator** | `monitoring` | Instrumentation CR'ı (Custom Resource) okur, gateway deployment'ına Java agent init container'ı enjekte eder |
...
| **Tempo** | `monitoring` | Trace deposu — WAL (Write-Ahead Log) + bloklar; Grafana Explore'un trace kaynağı |
| **Prometheus Agent** | `monitoring` | Collector `:8889`'u scrape eder, `remote_write` ile merkezi Prometheus'a iletir; yerelde TSDB (Time Series Database) tutmaz |
...
Peki tüm bu mekanizmayı tetikleyen ne? Uygulamanızın deployment'ının pod template'ine eklemeniz gereken tek bir annotation:
...
| --- | --- | --- |
| `memory_limiter` | Collector belleğini korur, OOM'u (Out of Memory) önler | Yük altında gateway saniyede binlerce span üretebilir |
...
<CardGroup cols={2}>
  ...
  <Card title="Depolama" icon="database">
    Tempo için statik PV/PVC (PersistentVolume/PersistentVolumeClaim) veya bir StorageClass.
  </Card>
</CardGroup>
...
Operator pod'u `Running` olana ve CRD'ler (Custom Resource Definition) gelene kadar bekleyelim:
...
:::warning OpenTelemetry Java agent kullanan Worker için minimum bellek: 2 GiB
Apinizer başlangıç betiği, JVM Metaspace üst sınırını container bellek limitine göre kademeli belirler (1 GiB → 128m, 2 GiB → 192m, 4 GiB → 256m). OpenTelemetry Java agent, instrumentation sınıflarıyla Metaspace'e yaklaşık 15-25 MB ek yük getirir; 1 GiB'lik bir Worker'da bu ek yük, gerçek proxy ve politika sınıf yüküyle birleşince 128m tavanının dolmasına ve `java.lang.OutOfMemoryError: Metaspace` kaynaklı deploy/yönetim isteği hatalarına yol açabilir. Heap yüzdesi ayarları Metaspace'i büyütmez.

- Agent enjekte edilecek Worker pod'unda bellek limitini en az **2 GiB** yapın; production için **4 GiB** önerilir.
- 1 GiB limit hiçbir Worker için yeterli değildir; CPU 1 core olsa bile JVM bu limitle açılmaz. Agent olmadan da minimum 2 GiB'dir.
- `JAVA_OPTS` ile yalnızca `-XX:MaxMetaspaceSize` değerini yükseltmek, pod limiti aynı kaldığında toplam native bellek kullanımını container sınırına taşıyabilir (OOMKilled riski); kalıcı çözüm bellek limitini artırmaktır.

Kaynak yapılandırması için [Kubernetes Ortamı](/tr/setup/kubernetes), belirti ve teşhis için [Memory Leaks ve OOM Hataları](/tr/operations/yaygin-sorunlar-cozumleri/memory-leaks-oom-errors) sayfasına bakın.
:::
...
Önce agent için servis hesabı ve RBAC (Role-Based Access Control) tanımları:
