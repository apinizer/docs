...
:::note Yönetilen bir entegrasyonu mu tercih edersiniz?

Bu makalenin yayınlanmasından sonra Apinizer'a Keycloak'ı bir [OIDC Kimlik Sağlayıcısı](/tr/admin/identity-management/identity-providers/oidc-provider) olarak tanımlama imkanı eklendi: kullanıcı/grup/rol senkronizasyonu ve merkezi token doğrulaması artık bu sağlayıcı kaydı üzerinden yönetilebilir. Bu makalenin anlattığı JOSE tabanlı doğrulama yaklaşımı hâlâ desteklenmektedir; DPoP proof doğrulaması gerektiren senaryolar için geçerliliğini korur.
