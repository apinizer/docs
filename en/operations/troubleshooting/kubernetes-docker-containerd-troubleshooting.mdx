...
```
  ...
  <Accordion title="If Nexus proxy is used">
    <p><strong>Reason/Why:</strong> If the organization uses Nexus docker-proxy, registry configuration is required on nodes.</p>
    
    <p><strong>Solution:</strong> For containerd mirror configuration, image pull testing, and deployment image path updates, see the <a href="/en/operations/container-registry/nexus-usage">Nexus Usage</a> page. If using Docker runtime, apply the Docker tab on the same page.</p>
  </Accordion>
...
```
  ...
  <Accordion title="ndots:1 on pods created by Apinizer (musl resolver and search domain effect)">
    <p><strong>Reason/Why:</strong> The <code>options ndots:5</code> seen in the example above is Kubernetes'
    default behavior: any name with fewer than 5 dots is tried first against the search domain list in
    <code>/etc/resolv.conf</code>. Short <em>in-cluster</em> addresses (e.g.
    <code>cache-service.apinizer.svc.cluster.local</code>) require a few extra lookups under this list; in
    addition, the <strong>musl</strong> resolver used in Alpine-based images stops searching at the first
    "NODATA" response in the search list. When an extra search domain is added to the server's
    <code>/etc/resolv.conf</code> (as in the scenario above), this behavior can break in-cluster name
    resolution.</p>

    <p>As of this version, all pod specs created by Apinizer (Worker, Cache, Integration, Portal, Manager)
    carry <code>ndots:1</code> under <code>dnsConfig.options</code>; <code>dnsPolicy</code> is not changed
    (left empty, so the Kubernetes default <strong>ClusterFirst</strong> still applies) and the
    nameserver/search list is not overridden. With <code>ndots:1</code>, names that contain a dot are queried
    directly first, without falling back to the search list.</p>

    <p><strong>Impact:</strong> On Alpine/musl-based components such as Manager, Cache, Integration and Portal,
    dotted but not fully-qualified addresses like <code>service.namespace</code> or
    <code>service.namespace.svc</code> may no longer resolve; only single-label short names (e.g.
    <code>service</code>) continue to resolve via the search list. Worker uses a glibc-based image and is not
    affected by this behavior. For this reason, always using the <strong>full FQDN</strong> (e.g.
    <code>service.namespace.svc.cluster.local</code>) for in-cluster connections is recommended; Apinizer
    already generates the in-cluster addresses it creates as FQDNs, but any manually entered short addresses
    should be reviewed.</p>

    <p><strong>Existing installations:</strong> <code>ndots:1</code> is always added when you
    <strong>Publish</strong> or <strong>Republish</strong> an environment. It is also backfilled on an existing
    deployment when the setting is missing and you edit its resources (CPU/memory), security context, health
    probe, or environment variables. An operator-set <code>ndots</code> value is preserved in that case, and
    <code>dnsPolicy</code>, nameservers, and the search list are left unchanged. A plain <strong>Restart</strong>,
    or <strong>Publish/Unpublish</strong> (replica scaling), does not add <code>ndots</code> to an existing
    deployment. For installations upgraded to this version, if an environment has only been Restarted, apply one
    of the edits above or <strong>Republish</strong> it.</p>
  </Accordion>
