...
```
  ...
  <Accordion title="ndots:1 on pods created by Apinizer (musl resolver and search domain effect)">
    ...
    <p><strong>Existing installations:</strong> <code>ndots:1</code> is only added during operations that
    re-apply the deployment (publishing or republishing an environment, or the corresponding "save and apply"
    flow for Cache/Integration/Portal/Manager — <strong>Publish / Republish</strong>). A plain
    <strong>Restart</strong>, or editing and saving a resource/environment variable without a Republish, does
    not add <code>ndots</code> to the existing deployment. For installations upgraded to this version, existing
    environments are recommended to be <strong>republished (Republish)</strong> to pick up <code>ndots:1</code>.</p>
  </Accordion>
