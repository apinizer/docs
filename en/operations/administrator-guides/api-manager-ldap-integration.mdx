...
Both the LDAP connection page and the LDAP login settings page now offer an **[Apply Preset]** menu (Active Directory, OpenLDAP, 389 Directory Server / FreeIPA, or Active Directory with groups kept as organizational units) that fills in matching field defaults, and a **[Test connection (step by step)]** button that breaks the connection test into individual steps with suggestions. See [LDAP/Active Directory](/en/integrations/connection-management/ldap-active-directory) and [LDAP Login Settings](/en/admin/system-settings/ldap-login-settings) for details.
