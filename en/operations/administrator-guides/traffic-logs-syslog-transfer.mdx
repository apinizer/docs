...
API traffic logs are configured here, on the Gateway environment's log connectors — they are **not** part of the SIEM streams. Audit, session, token and application events are forwarded from the [SIEM & Log Forwarding](/en/admin/system-settings/siem-log-forwarding) page instead. Forwarding API traffic through the SIEM streams is planned for a later release.
...
If you would rather not raise the receiving server's limits at all — or you are on UDP, where there is no framing to lengthen in the first place — the Syslog connection can split an oversized record into several syslog messages instead; see [Splitting Oversized Messages](/en/integrations/connection-management/syslog#splitting-oversized-messages).
