...
The Syslog connection's outbound queue is bounded by a byte ceiling (64 MB by default) in addition to its message-count limit, so a burst of large records cannot grow the queue without bound. When the queue is full, new records are routed to the failover connector described below rather than being dropped. See the [Syslog connection page](/en/integrations/connection-management/syslog#large-records-and-high-volume) for the full parameter list.
...
Because large records make the send queue fill up faster under load, it is worth configuring a failover connector for the Syslog connector on the environment, so that records which cannot be delivered while the primary queue is full are not lost. See [Adding Connectors to Gateway Runtime Environments](/en/admin/server-management/adding-connectors-to-gateway-environments) for how to define a failover connector and connection.
