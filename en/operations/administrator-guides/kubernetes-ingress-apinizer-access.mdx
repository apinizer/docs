...
description: "You can provide access to Apinizer services with Ingress by installing NGINX Ingress Controller, configure LoadBalancer service with MetalLB, pass the real client IP via X-Forwarded-For (XFF), perform DNS-based routing by creating Ingress objects, and provide secure access with TLS certificates."
---
...
## X-Forwarded-For (XFF) Configuration

Starting with Apinizer 2024.05.4, the Management Console requires client IP information to be validated. When accessing through Ingress, the `X-Forwarded-For` (XFF) header must be passed correctly so that Apinizer receives the real client IP.

Add the `use-forwarded-headers` key with the value `true` to the data section of the ConfigMap definition:
...
If the namespace you used during the Helm installation differs (for example `ingress`), edit the `metadata.namespace` and `metadata.name` fields of the ConfigMap according to your environment. You can find the existing Controller ConfigMap with the `kubectl get configmap -n <namespace>` command.
...
The following annotation must also be added to the Ingress object that will be created in the next step:
...
One of the following headers must be present on the load balancer side:
...
The Gateway checks the headers above in order and uses the first valid IP.
