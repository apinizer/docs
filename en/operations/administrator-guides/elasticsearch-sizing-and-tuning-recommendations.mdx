...
On the management traffic list (Analytics > API Traffic), the total record count is shown as **"more than 10,000"** instead of an exact figure once the index is very large. This is deliberate: recounting billions of records on every page change is the most expensive part of the query and contributes nothing to the page being displayed.
...
- The API Portal traffic screens and the Portal MCP traffic query are **not** affected either; they always receive the exact total record count.
