...
The `CREATE TABLE` statement below is for **new installations** and already includes every column, covering AI Gateway, MCP Gateway, A2A Gateway, routing diagnostics, outbound MCP/A2A call audit and policy execution times — no separate `ALTER` is needed.

**If you are upgrading an existing installation**, do not run this statement; your table already exists. Run the `ALTER TABLE` script of every version you pass through, in order:

- **Coming from 2026.04.x:** first the [2026.09.0 — Log Table ALTER Scripts](/en/release-notes/2026-09-0-log-table-alter), then the [2026.09.1 — Log Table ALTER Scripts](/en/release-notes/2026-09-1-log-table-alter). The cumulative 2026.09.0 script already creates the four outbound MCP/A2A audit columns, so on the 2026.09.1 page add only `policy_exec_times`.
- **Coming from 2026.09.0:** only the [2026.09.1 — Log Table ALTER Scripts](/en/release-notes/2026-09-1-log-table-alter). If your table already has the four outbound MCP/A2A audit columns, add only `policy_exec_times` — re-adding an existing column raises an error.
...
:::note SQL Server: row size warning

When creating this table, SQL Server emits: *"the table has been created, but its maximum row size exceeds the allowed maximum of 8060 bytes"*. **This is expected and is not an error.**

The combined declared width of the variable-length `NVARCHAR` columns exceeds the 8060-byte in-row limit per row. Since SQL Server 2005 such columns are automatically moved to ROW_OVERFLOW_DATA pages when needed, leaving a 24-byte pointer in the row. Because the fixed-length columns total only ~287 bytes, `INSERT` statements do not fail. The only effect is extra page I/O for rows where many long text fields are populated at once.

Narrowing the columns enough to silence the warning (every text column down to 64 characters) was rejected because it would truncate fields such as `req_request_uri`, `req_query_string` and `application_name`.
...
| mcp_tool_names_out | Distinct MCP tool names called on outbound requests (CSV, capped at 256 characters) |
| mcp_tool_call_count_out | Total number of outbound MCP tool calls (raw, uncapped) |
| a2a_skill_names_out | Distinct A2A skill/agent names called on outbound requests (CSV, capped at 256 characters) |
| a2a_agent_call_count_out | Total number of outbound A2A agent calls (raw, uncapped) |
| policy_exec_times | Policy execution times — `order:type:duration` CSV; written only when the option is enabled on the log target |
