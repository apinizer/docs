---
title: "Table Creation Commands for Apinizer Logs"
description: "Using the DB Connector, you can create the database tables required to store traffic and audit records, using the appropriate SQL commands for Oracle, MySQL/MariaDB, PostgreSQL and SQL Server databases."
---
...
See the [Database Connection](/tr/integrations/baglanti-yonetimi/database) page for details.
...
- **If you are coming from version 2026.04.x:** first [2026.09.0 — Log Table ALTER Scripts](/tr/release-notes/2026-09-0-log-tablosu-alter), then [2026.09.1 — Log Table ALTER Scripts](/tr/release-notes/2026-09-1-log-tablosu-alter). Since the cumulative 2026.09.0 script already creates the four outbound MCP/A2A audit columns, on the 2026.09.1 page add only the `policy_exec_times` column.
- **If you are coming from version 2026.09.0:** only [2026.09.1 — Log Table ALTER Scripts](/tr/release-notes/2026-09-1-log-tablosu-alter). If your table already has the four outbound MCP/A2A audit columns, add only the `policy_exec_times` column — re-adding an existing column raises an error.
- **Whichever version you are coming from:** also run the [2026.09.2 — Log Table ALTER Scripts](/en/release-notes/2026-09-2-log-table-alter) script once — it adds the client attribution columns, which no earlier script includes.
...
This table grows very quickly under live traffic. Applying **daily partitioning on `log_timestamp`** is strongly recommended. This way the retention policy can be applied with `DROP PARTITION`, and time range queries scan only the relevant partitions. For database-specific examples, see the [Partition Syntax](#partition-söz-dizimi) section.
...
Token volume generally grows in proportion to API traffic volume. **Daily partitioning on `log_timestamp`** is recommended. See [Partition Syntax](#partition-söz-dizimi).

## Log Policy {#log-politikasi}
...
Volume depends on the number of logging policies attached to the flows and how often they are triggered. For policies attached at proxy/method level on high-traffic APIs, **daily partitioning on `log_timestamp`** is recommended. See [Partition Syntax](#partition-söz-dizimi).
...
Application logs accumulate steadily. On live systems, **daily partitioning on `log_timestamp`** is recommended. See [Partition Syntax](#partition-söz-dizimi).
...
| event_type | Which of the [Audit Records event types](/en/admin/audit-compliance/audit-records#event-types) this row is |
...
Audit events are produced by user operations (far fewer than traffic records) and are usually retained longer for compliance reasons. **Monthly partitioning on `audit_event_date`** is generally sufficient. In the [Partition Syntax](#partition-söz-dizimi) section, you can use `MONTH` instead of `DAY`.
...
**If you are upgrading an existing installation**, do not run this command; your table already exists. Run the [2026.09.2 — Audit and Login Table ALTER Scripts](/en/release-notes/2026-09-2-audit-login-table-alter) script once — the insert statement is unconditional, so until the script is run, every Login Log row bound for this table fails to write, and — unlike Audit Event above — that row is not retried through a failover connector; it is simply lost.
...
| user_agent | The `User-Agent` header sent by the client |
| reason_code | The machine-readable [reason](/en/admin/audit-compliance/login-records#user-agent-and-reason) behind a failure, lock, or password event |
...
Login log volume is typically low compared to traffic logs. For the long retention periods often required for security auditing, **monthly partitioning on `log_timestamp`** is sufficient. See the [Partition Syntax](#partition-söz-dizimi) section for examples, and use `MONTH` instead of `DAY` for the interval.
