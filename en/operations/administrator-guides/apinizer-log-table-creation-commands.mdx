...
The `CREATE TABLE` statement below is for **new installations** and already includes every column, covering AI Gateway, MCP Gateway, A2A Gateway and routing diagnostics — no separate `ALTER` is needed.

**If you are upgrading an existing installation to 2026.07.0**, do not run this statement; your table already exists. Use the `ALTER TABLE` script on the [2026.07.0 — Log Table ALTER Scripts](/en/release-notes/2026-07-0-log-table-alter) page instead.
...
### Version Upgrade: Routing Diagnostics Columns (Upgrade ALTER)

The Routing Diagnostics feature adds **24 new columns** to this table, holding failure classification, connect/DNS/TLS/read phase timing, connection pool snapshots, and configured timeouts for requests routed to the backend. **New installations** already include these columns in the `CREATE TABLE` statement above; the `ALTER TABLE` command below is only needed **when upgrading an existing installation**.

:::danger

The Apinizer service account is intentionally not granted DDL (schema alteration) privileges. This command must therefore be run **by a database administrator (DBA)** on the relevant schema.
...
- **When to run it**: This command **should be run before** upgrading Api Manager to the version that includes Routing Diagnostics, so that diagnostic data starts being captured as soon as the new version is active. If run after the upgrade, no data is lost — only requests handled between the upgrade and the time the command is run will be missing diagnostic data.
- **Which environments**: Only required for installations that write API traffic logs to a relational database via a **database connector** (Oracle/MySQL/PostgreSQL/SQL Server).
- **What happens if it is not run**: Apinizer detects that the columns are missing and continues writing the record while simply skipping the Routing Diagnostics fields. **Traffic logging is not interrupted** — only the Routing Diagnostics data for those requests is not recorded. Once the command is run, data collection starts from that point onward.
- **Rollback**: The command only adds new (nullable) columns and does not modify existing data, so it carries no rollback risk. If you want to remove the columns, a standard `ALTER TABLE ... DROP COLUMN` can be used, but this permanently deletes any diagnostic data collected up to that point.
- **Historical data**: These columns are forward-only; records created before the ALTER is applied keep these fields empty (NULL).

:::

:::info

**Elasticsearch connectors do not need this command.** For installations running on Elasticsearch, the corresponding template (mapping) update and index rollover are applied **automatically** as part of the Apinizer version upgrade — no manual step is required.
