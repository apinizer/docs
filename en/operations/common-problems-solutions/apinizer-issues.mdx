...
## Scientific Notation Error in XML→JSON Transformation

#### Problem

In REST API Proxy definitions where the backend returns XML responses, an error occurs during XML→JSON transformation. For example, values such as `1234E567890` in the `ProductCode` field interrupt the conversion.

#### Reason/Why

In Jackson-based XML→JSON transformation, fields containing a mix of letters and digits (product code, serial number, etc.) may be interpreted as numeric expressions. When the letter `E` is read as exponential notation, the value is parsed as BigDecimal and the conversion fails.

#### Solution

<Steps>
  <Step title="Enable Write Numbers as Strings">
    In the API Proxy definition, under **Response Options**, if **Jackson Transformation** is used, the **Write Numbers as Strings** option should be enabled.
  </Step>
  <Step title="Verify the Sample Response">
    Use **Show Sample Message** to verify the transformation result.
  </Step>
</Steps>

:::note

The **Write Numbers as Strings** option applies to the entire transformation, not only the problematic field. Genuinely numeric fields such as `Quantity` and `Amount` are also returned as strings in JSON. The client application consuming this API must read those fields as strings and perform type conversion where needed.
...
**The Gateway checks the above headers in order and uses the first valid IP address found.**
...
**Note:** In your internal systems and development or test environments, this error in MSSQL database connections (including DB2API) can be temporarily resolved by adding the following parameters to the JDBC URL:

- `;trustServerCertificate=true` (recommended temporary workaround; add carefully and consider possible security implications)
- In some environments, `;encrypt=true` may also be required
- `;encrypt=false` (not recommended)

:::

## Timezone Region Error in Oracle Connections

#### Problem

Oracle database connections (including DB2API) cannot be established and the following error is received:
...
The Oracle JDBC driver returns `ORA-01882: timezone region not found` when the timezone region is not properly provided in the connection string.

#### Solution

<Steps>
  <Step title="Add Connection String Parameter">
    Add `?oracle.jdbc.timezoneAsRegion=false` to the Oracle database connection string. If the connection URL already contains query parameters, use `&oracle.jdbc.timezoneAsRegion=false` instead.
  </Step>
</Steps>

:::warning[Warning]

This parameter makes the Oracle JDBC driver interpret timezone information as an offset rather than a region name. After updating your connection definition, retest the DB2API or relevant connection.
...
:::
    ...
    <img 
    src="/images/operations/apinizerolasisorunlar.png" 
    alt="worker deployment yaml. dosyası"
    width="1000"
  />

## Cache Pod Entering Error Loop During Apinizer Upgrade

#### Problem

When updating cache deployments during Apinizer upgrade, if the Cache pod enters an error loop with the following error:
...
<Steps>
  <Step title="Scale Cache Replicas to 0 and Scale Back Up">

    Scale down the cache deployment by setting its replicas to 0 to stop all pods:

    ```bash
    kubectl scale deployment cache --replicas=0 -n apinizer
    ```

    Wait for the pods to completely shut down. Then set the replicas back to the original value to restart the pods:

    ```bash
    kubectl scale deployment cache --replicas=3 -n apinizer
    ```

    The new pods will start with the updated version.

  </Step>

  <Step title="Delete Old ReplicaSet Definitions">

    You can perform a clean restart by deleting old replicaset definitions.
    :::warning
      This operation may delete old pods, so it should be done carefully. The deployment will automatically create new pods.
    :::
  </Step>

  <Step title="Change DeploymentStrategy to Recreate">

    Edit the deployment YAML file and change the `deploymentStrategy` from `RollingUpdate` to `Recreate`:
    ...
    This method shuts down all pods at once instead of gradually, then creates new ones.
  </Step>
</Steps>

## Excessive Application Logs and Threshold Notifications

#### Problem

Application Logs records are growing beyond expectations and the following notification is received:

```
This notification was created because threshold value was exceeded for Apinizer Gateway - Application Logs Monitor! Please control your log levels or clear old logs.
```

#### Reason/Why

This situation may have more than one cause.

  ### Changing the API Proxy Log Level for Debugging
    When the debug level is left enabled on the Application Logs tab, log volume increases. Apinizer does not turn this level off automatically.

  ### High Log Volume at ERROR Level
    If the record count is high even though logs are at the `ERROR` level, there may be a system issue and these logs should be reviewed. Actual error notifications must not be ignored.


:::info

The Application Logs settings screen shows the last saved setting from the page, not the live state.
...
Setting the log level directly to `ERROR` as an administrator may cause development teams to miss the logs they expect during their work. Related teams should therefore be informed first.

:::

If logs are at a level other than `ERROR`, the level can be set to `ERROR` from the Application Logs tab. To make this the default on every pod start, the following parameter can be added to the worker deployment file:

```yaml
logLevel: ERROR
```

For details, see [Gateway Settings — Additional Variables](/en/setup/gateway-settings#variables).

<Steps>
  <Step title="Clean Up Old Logs Regularly">
    It is recommended to clean these logs every 3 days. If the related settings on the Admin [Application Log Cleanup Tasks](/en/admin/audit-compliance/application-log-cleanup-tasks) page are not enabled, they should be enabled and adjusted.
  </Step>
  <Step title="Adjust the Alarm Threshold for the Environment">
    An alert is enabled by default for excessive Application Logs. When the record count reaches the 10,000 limit, notifications appear from the top-right corner of Apinizer. If logging is used heavily in a test environment, the daily log alarm threshold for the worker can be adjusted to a suitable level.
  </Step>
</Steps>

:::note

In future versions, the debug level is planned to open with a short default duration and turn off automatically when that period ends.
