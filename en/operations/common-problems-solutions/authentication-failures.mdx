...
- For mTLS, is the Identity Source selection in the policy consistent with the Credential's username?
- For mTLS, is the authority that signed the certificate present in the key store bound to the Credential?
- For mTLS, is the relevant API Proxy defined in the Credential's access list?
- For mTLS, are HTTPS and mTLS enabled in the environment settings, and does TLS terminate on the gateway?
