...
**Detection:**
- Call the Gateway address of a proxy that appears "undeployed" in the Manager. If it returns `200` instead of `404`, you are facing this situation
- Do not guess the address; read it from the Management API `invoke-info` endpoint — it returns the real invocation address even for records that appear undeployed
