---
title: "Cleaning alert_history Collection"
description: "Enables cleaning old records from the alert_history collection where information such as operation time, message information in the action, and which object triggered it for alerts created using actions under certain conditions on Apinizer is recorded, according to the specified time period."
---
...
The following script is used to delete records older than the specified time from the `alert_history` collection stored in MongoDB. This operation optimizes storage space by cleaning the database.
...
2. Give execute permission to the file: `chmod +x purge_alert_history_collection.sh`
3. Run the file: `./purge_alert_history_collection.sh`
