---
title: "Cleaning user_token_blackList Collection"
description: "Enables cleaning old records from the user_token_blackList collection where records are kept when users log in multiple times, according to the specified time period."
---
...
A record in this collection is what stops a signed-out session from being resumed with the same token. Deleting a record before its token would have expired on its own makes that token valid again.

Only delete records older than the longest token the installation can issue: 24 hours, or 30 days when users sign in with **Remember Me**. The built-in cleanup on the **Application Log Cleanup Tasks** screen already enforces this floor; a manual script does not.

:::

## Record Cleanup Script

The following script is used to delete records older than the specified time from the `user_token_blackList` collection stored in MongoDB. This operation optimizes storage space by cleaning the database.
...
2. Give execute permission to the file: `chmod +x purge_user_token_blacklist_collection.sh`
3. Run the file: `./purge_user_token_blacklist_collection.sh`
