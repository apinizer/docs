---
title: "Cleaning persistent_audit_event Collection"
description: "Enables cleaning old records from the persistent_audit_event collection where API Manager login attempt records are stored."
---

<Warning>
  Login logs are always saved to the database and cannot be automatically deleted via Purge Jobs. Use this manual script only if absolutely necessary.
</Warning>
...
The following script is used to delete records older than the specified time from the `persistent_audit_event` collection stored in MongoDB. This operation optimizes storage space by cleaning the database.
...
2. Give execute permission to the file: `chmod +x purge_persistent_audit_event_collection.sh`
3. Run the file: `./purge_persistent_audit_event_collection.sh`
