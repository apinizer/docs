...
| [Security Events](/en/admin/audit-compliance/security-events) | `log_apisecurityevent` | Bodyless metadata record of every Gateway request blocked by a policy or failed with an error, across HTTP, gRPC and WebSocket traffic |
...
| - | `lifecycle_evidence` | Collection that records, for consumer credentials, credential organizations and portal applications, whether certain lifecycle boundaries have ever been crossed (activated, deployed, exported, and similar). It holds one very small document per record, each entry is written only once and is never updated afterwards |
|
...
The `lifecycle_evidence` collection **must not be purged or cleaned**. It has no cleanup script and no automatic purge, and it is deliberately excluded from time-based cleanup: its records are used to decide whether a record may be deleted, and a missing entry is treated as "no evidence". Deleting these records does not free up meaningful space; it only causes deletion decisions to be made on incomplete information.
