---
title: "Cleaning portal_login_logs Collection"
description: "Enables cleaning old records from the portal_login_logs collection where API Portal login attempt records are stored."
---
...
Automatic cleanup is now available: enable **Portal Login Logs** under [Application Log Cleanup Tasks](/en/admin/audit-compliance/application-log-cleanup-tasks) (off by default) instead of running this script regularly. Use this manual script only for an immediate, one-off cleanup — for example before that setting has been turned on, or for a retention period shorter than you want to configure there.
...
The following script is used to delete records older than the specified time from the `portal_login_logs` collection stored in MongoDB. This operation optimizes storage space by cleaning the database.
...
2. Give execute permission to the file: `chmod +x purge_portal_login_logs_collection.sh`
3. Run the file: `./purge_portal_login_logs_collection.sh`
