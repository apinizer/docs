...
<img
  ...
  alt="Anomaly Detector Overview — KPI cards, activity chart, recent anomalies, and detector health"
  width="1000"
  height="492"
/>
...
### Step 1: Basic Information
    ...
    <Accordion title="Name - Required">
        ...
        - Must be unique within the project — a second detector with the same name cannot be saved, while the same name may still be used in another project
        - Cannot start with a space
        - The system automatically checks the availability of the name

        The name is checked again only when you change it. A detector whose name stays the same can always be
        saved, so if two detectors in the same project already carry the same name — possible on installations
        upgraded from an earlier version, or when a detector is imported with its original name kept — both remain
        ...
        renaming `Response Time Anomaly Detection` to `response time anomaly detection` is accepted and can be saved.
    ...
    <Accordion title="Environment - Required">
        ...
        The environment is required both when you create a new detector and when you edit an existing
        one — the **Save** button stays disabled until an environment is selected, and saving without one
        is also refused server-side. A detector needs its environment because it reads traffic only from
        that environment's Elasticsearch connection; without one, every scheduled run would fail.

        The same rule applies if the environment stored on the record no longer exists in this
        installation — for example, the environment was deleted after the detector was created, or the
        detector was imported from another installation. In that case the field shows **"Select an
        option"**, a message underneath explains that the stored environment can no longer be found, and
        the **Save** button stays disabled until you pick a valid environment. When you create a new
        detector, the environment field only preselects an environment that has Elasticsearch integration
        active; when you edit an existing detector, the environment is never preselected for you.

        The check is not only in the form: **the server also rejects saving or activating a detector
        whose environment has no Elasticsearch connection defined, or has it defined but not enabled** —
        this applies even if a request reaches the server outside the normal form, for example through
        the Management API. Setting a detector to **passive** is never blocked by this check.
    </Accordion>
...
3. Fill in the condition parameters
    ...
    <Accordion title="1. Metric Value Check">
        ...
        **Configuration Fields:**
        ...
        | **for ... times** | Specifies that this condition must be met in ... consecutive analyses, the current analysis included. A single analysis that falls back below the threshold breaks the streak, so a past threshold crossing on its own never produces a new anomaly. |
        ...
        **Example Scenario:**
        ...
        - Repeat Count: 3
        - **Meaning**: Anomaly is detected when average response time exceeds 1000 ms in 3 consecutive analyses, the current analysis included.
    </Accordion>
    ...
    <Accordion title="2. Metric Increase/Decrease Rate Check">
        ...
        **When there is no previous value or the baseline is zero**
        ...
        - **Baseline is zero**: if the previous values are all `0` and the current value is also `0`,
          nothing changed and no anomaly is reported. This is the expected behaviour for a detector
          that watches an API with no traffic in the selected window.

        If the baseline is `0` and the current value is greater than `0`, the change is treated as an
        unbounded increase and the condition is met for the increase operators (`Above`,
        `Above or Equal`) and for `Not Between`.
    ...
    <Accordion title="4. Query/Filter Rate Check">
        ...
        **Configuration Fields:**
        ...
        | **for ... time** | Specifies that this condition must be met in ... consecutive analyses, the current analysis included. |

        **Example Scenario:**
        - Query/Filter Rate: 90%
        - Operator: Above
        - Repeat Count: 3
        - **Meaning**: Anomaly is detected when the ratio of query results to filter results exceeds 90% in 3 consecutive analyses, the current analysis included.
    </Accordion>
...
selector. Opening **Edit** from this page returns you to this same Detail page — on both **Back** and
**Save** — instead of the detector list; **Edit** opened from the list still returns to the list.

Below it are 5 summary cards:
...
The **Expected** and **Deviation** cards read differently depending on the condition type:
- **Metric condition**: Expected is the configured threshold; deviation is the current value against it.
- **EMA condition**: Expected is the current Bollinger band range; deviation is the current value against
  that band.
- **Rate condition**: Expected is the real baseline calculated from the previous results, and deviation is
  the current value against that baseline. Until a baseline exists — the detector's first runs, or a
  result recorded before this behavior was introduced — both cards show **—** instead of a number; a rate
  is a change relative to a baseline, so without one no meaningful percentage can be shown.

On the anomaly detector screens the measured value, the expected value and the threshold are shown with the
unit implied by the condition's metric type: duration metrics (request/routing/response/total time, provider
latency) in **ms**, size metrics (request/response size) in **B**, the rate-percentage metric and
increase/decrease rate thresholds in **%**, while document-count, token and cost metrics stay unitless. The
unit is derived from the condition definition, so past execution records are displayed with the correct unit
as well.
...
All Detections tabs).
    ...
    <Accordion title="Changing Status">
        ...
        Activating a detector is also a save, so the same environment check applies: if the environment
        stored on the record can no longer be resolved in this installation, or its Elasticsearch
        connection is not defined or not enabled, **Activate** is refused with a message explaining why,
        and the detector stays passive. Setting a detector to **passive** is never blocked, even in either
        of these cases — stopping a detector that keeps failing must always be possible.
    </Accordion>
...
## Troubleshooting
    ...
    <Accordion title="Anomalies Reported While the Metric Value Reads 0">
        **Cause:**

        Rate and EMA conditions do not compare the measured metric with a fixed number; they compare it
        with a baseline — the summary of the previous results for a rate condition, the EMA band for an
        EMA condition. When no traffic matches the query and filter in a window, both the measured metric
        and that baseline are 0. The percentage thresholds and the EMA band then collapse onto zero, and
        the *above or equal*, *below or equal* and *between* operators are satisfied on every run. The
        result list fills with anomalies whose metric value is 0.

        **Behavior from version 2026.09.3 on:**

        A run in which the measured metric and its baseline are both zero — and, more generally, an EMA
        run in which the band has no width and the metric sits exactly on it — is recorded as **Not Met**
        instead of **Anomaly**. A rate condition's very first run, where no previous value exists to
        compare against yet, is likewise recorded as **Not Met** rather than as an anomaly; EMA conditions
        already behaved this way. The application log carries an `INFO` line stating why the condition was
        not met, so a quiet detector can be told apart from a broken one.

        Real changes are unaffected. Traffic appearing after a quiet period still raises an anomaly, and
        so does a drop from a non-zero baseline down to zero.

        Results written before the upgrade are not rewritten. The anomaly counters settle as new results
        accumulate; use **Delete All Results** on the detail page if you want the counters to start clean.
