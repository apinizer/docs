...
All Detections tabs).
    ...
    <Accordion title="Changing Status">
        ...
        stored on the record can no longer be resolved in this installation, **Activate** is refused with
        a message asking you to select an environment first, and the detector stays passive. Setting a
        detector to **passive** is never blocked, even if its environment can no longer be resolved —
        stopping a detector that keeps failing must always be possible.
    </Accordion>
