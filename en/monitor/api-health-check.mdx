...
### Step 1: Basic Information
    ...
    <Accordion title="Name - Required">
        ...
        - Must be unique within the project — a second health check with the same name cannot be saved, while the same name may still be used in another project
        - Cannot start with a space
        - System automatically checks the availability of the name

        The name is checked again only when you change it. A health check whose name stays the same can always be
        saved, so if two checks in the same project already carry the same name — possible on installations upgraded
        from an earlier version, or when a health check is imported with its original name kept — both remain
        editable; rename one of them to clear the duplicate.

        Changing only the capitalisation of a name does not count as a duplicate of the record you are editing:
        renaming `Payment API Monitoring` to `payment api monitoring` is accepted and can be saved.
...
planned downtime, plus an optional **time zone** next to the two times — a region id (e.g. `Europe/Istanbul`)
or a UTC offset (e.g. `+03:00`). Leave the time zone empty to use the server's own time zone.

The window recurs **every day** between the two times, from the start minute up to but not including the
end minute. A window that crosses midnight (e.g. `23:00`–`01:00`) is valid and applies overnight. Leaving
the start and end equal, leaving one of the two empty, or leaving both empty means there is no maintenance
window at all — the monitor is never excluded and alerts are never suppressed by this setting.

While a monitor is inside its window, its checks keep running and every result is still recorded, but the
monitor is reported as **Maintenance** instead of Up, Degraded or Down, and **failure notifications
(email/webhook and other recipients) are suppressed** — the first failure after the window closes is
notified normally. Uptime percentage and Service Level figures still include the checks made during the
window; the maintenance window changes what is reported and notified, not what is measured.
...
response-time chart, and a tabbed area at the bottom.
    ...
    <Accordion title="1. Header and Toolbar">
        ...
        Opening **Edit** from this page returns you to this same Detail page — on both **Back** and
        **Save** — instead of the check list; **Edit** opened from the list still returns to the list.
...
- **Maintenance**: How many monitors are currently inside their maintenance window; these monitors are
  excluded from the **Operational** and **Down** counts above and from **Needs attention** below
...
- **Needs attention**: A list of monitors that are currently Degraded or Down, with how long ago the
  issue started; clicking an item opens that monitor's Detail page — a monitor in its maintenance window
  never appears here
...
A table of every monitor matching the current filters:
    ...
    <Accordion title="Search and Filtering">
        ...
        - **Status filter chips**: **All / Up / Degraded / Down / Maintenance / Paused**
    ...
    <Accordion title="Table Columns">
        - **Monitor**: Status dot + name (clickable, opens the Detail page) + HTTP method/URL; a
          **Paused** badge is shown for disabled monitors, and a **Maintenance** badge — with a tooltip
          explaining that checks keep running but failure notifications are held back until the window
          closes — is shown for a monitor currently inside its maintenance window
...
Access to health checks is governed by the **Monitoring** permission of your role in the project. You can
see which actions your role carries under **Administration → Roles**.

- **Monitoring → View**: You can open the health check list, the Detail page and the check results. On the
  Detail page the **Configuration** tab shows the request, assertions, retry policy and notification
  recipients read-only, so you can review a check's settings without being able to change it.
- **Monitoring → Manage**: In addition to the above, you can create, edit, duplicate and delete a check,
  activate or deactivate it, and use **Run now**. Without this permission the **New** and **Edit** screens do
  not open at all, and the corresponding items are hidden from the operations menu.

A health check always belongs to the project it was created in, and that project decides the answer. Opening,
changing, pausing, running or deleting a check is judged against the Monitoring permission you hold **in the
owning project** — holding Monitoring in a different project grants nothing here.

:::note

**Notification recipients and e-mail connections.** Picking a previously defined e-mail connection in the
recipient dialog additionally requires **Connections → View**, and defining a new one from that dialog
requires **Connections → Manage**. These are separate permissions from Monitoring: if your role has
Monitoring but not Connections, opening the recipient dialog shows a permission warning and leaves the
saved e-mail connection list empty. Every other part of the health check form keeps working, and recipients
already saved on the check are shown regardless.

:::

:::warning

**Changed in 2026.09.3.** Two things behave differently:

- A role holding Monitoring → Manage without Connections used to see a **"you do not have permission"**
  warning the moment the Edit screen opened, even though the check could be edited and saved normally. That
  warning was raised by a background list the screen no longer loads, and is gone.
- **New** and **Edit** now require Monitoring → **Manage**. A role with Monitoring → View alone previously
  reached the form through a direct link and only discovered on save that it was not allowed to.
