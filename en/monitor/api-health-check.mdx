...
On the health check list page, you can view and manage all your checks.
    ...
    <Accordion title="Table Columns">
        ...
        - **Status**: Disabled checks are shown with a badge; status is changed via the **Activate/Deactivate** menu item
        - **Project** (Admin mode): Project the check belongs to (the column before Actions)
    ...
    <Accordion title="Operations Menu">
        ...
        2. **Activate / Deactivate**: Enable or disable the check
        3. **Edit**: Update check settings
        4. **Duplicate**: Create a copy of the check
        5. **Delete**: Delete the check
        6. **Move to Global** (if project check): Move check to global project
    ...
    <Accordion title="Changing Status (Activate/Deactivate)">
        ...
        the top bar of the detail screen. Disabled checks do not run, but their historical data is preserved.
    </Accordion>
