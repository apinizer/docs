...
<CardGroup cols={2}>
  ...
  <Accordion title="Elasticsearch Trigger Types">
    ...
    **Elasticsearch CPU Percentage**: Checks the CPU usage percentage of every node in the Elasticsearch cluster separately. Alarm is triggered when the CPU usage percentage of any node exceeds the threshold value; the alarm history lists each node with its measured value, so the offending node can be identified. Requires threshold value (e.g., 80 = 80%). Requires connection definition. If the connection is not defined, is disabled or cannot be read, the alarm is recorded with an error state and the reason is written explicitly into the history.

    **Elasticsearch Disk Percentage**: Checks the disk usage percentage of every node in the Elasticsearch cluster separately. Alarm is triggered when the disk usage percentage of any node exceeds the threshold value; the alarm history lists each node with its measured value. Nodes that report no disk statistics (coordinating-only nodes) are skipped. Requires threshold value (e.g., 90 = 90%). Requires connection definition. If the connection is not defined, is disabled or cannot be read, the alarm is recorded with an error state and the reason is written explicitly into the history.
