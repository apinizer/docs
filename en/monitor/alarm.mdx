...
<CardGroup cols={2}>
  ...
  <Accordion title="Elasticsearch Trigger Types">
    ...
    **Elasticsearch CPU Percentage**: Checks the CPU usage percentage of Elasticsearch cluster. Alarm is triggered when CPU usage percentage exceeds the threshold value. Requires threshold value (e.g., 80 = 80%). Requires connection definition. If the connection is not defined, is disabled or cannot be read, the alarm is recorded with an error state and the reason is written explicitly into the history.

    **Elasticsearch Disk Percentage**: Checks the disk usage percentage of Elasticsearch cluster. Alarm is triggered when disk usage percentage exceeds the threshold value. Requires threshold value (e.g., 90 = 90%). Requires connection definition. If the connection is not defined, is disabled or cannot be read, the alarm is recorded with an error state and the reason is written explicitly into the history.
