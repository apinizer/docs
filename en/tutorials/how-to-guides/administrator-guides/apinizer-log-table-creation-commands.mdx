...
description: "You can create necessary database tables to store traffic and audit records using DB Connector, and use appropriate SQL commands for MySQL/MariaDB and Oracle databases."
---

This document defines tables for storing traffic and audit records using "DB Connector". To provide functionality with DB Connector, it is recommended to use the tables defined below. These tables are designed to ensure that traffic and audit records are stored securely and consistently in the database.

<Warning>
  When creating tables, if you have to choose a column type that does not exist in your database, you should consider alternatives. For example, as indicated in the table below, it is recommended to use 'CLOB' instead of 'LONGTEXT' in Oracle. You should make the right choice considering your application's needs and existing database configuration.
</Warning>
