...
The username of the credential is determined by the **Identity Source** selection in the policy. These two settings must be consistent with each other; otherwise the request is rejected even when the certificate is valid.

| Identity Source in the policy | Username of the credential | Its value in this scenario |
|-------------------------------|---------------------------|----------------------------|
| **Certificate Issuer (Issuer CN)** | Name of the authority that signed the certificate | `MyRootCA` |
| **Certificate Subject (Subject CN)** | Name of the client certificate itself | `client` |
| **Subject, then Issuer** (default) | Either can be used; the subject name is searched first, and the issuer name if not found | `client` or `MyRootCA` |

In this scenario both naming approaches work with the default setting. Use the `MyRootCA` name if you want to gather **all** clients issued by the authority under a single identity; use the `client` name and select the **Certificate Subject** option in the policy if you want to recognize each client individually and be able to revoke a single client's access independently.

:::note

For detailed information about the Identity Source option and its meanings, see the Identity Source section on the [mTLS Authentication](/en/develop/policies/mtls-authentication) policy page.
