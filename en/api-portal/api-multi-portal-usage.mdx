...
The same isolation applies to the portal's **MCP (Model Context Protocol) endpoint**. Each portal's MCP server only ever sees its own portal's data: the applications, API products, subscriptions, credentials and traffic records an MCP tool returns all belong to that portal. Passing another portal's application or API id is refused, and a personal access token issued on one portal is not accepted on another portal's MCP endpoint.

:::info

On a single-portal installation this rule makes no visible difference. Records created before multi-portal support, which are not yet bound to any portal, also stay visible exactly as before — the MCP endpoint now returns the very same list the portal interface shows.
...
<Tabs>
  <TabItem value="different-customer-segments" label="Different Customer Segments">
    **Usage Scenario:**
  ...
  <TabItem value="different-departments-units" label="Different Departments/Units">
    **Usage Scenario:**
  ...
  <TabItem value="different-environments" label="Different Environments">
    **Usage Scenario:**
  ...
  <TabItem value="different-regions-languages" label="Different Regions/Languages">
    **Usage Scenario:**
...
Please review the [Kubernetes Installation — API Portal Setup](/en/setup/kubernetes#api-portal-setup) documentation for installation.
...
:::info
If you have multiple portals, switch the active portal from the top **Portal** menu. Support, Prod Promotion, reports, and catalog settings apply to the **active portal**. See also the API Portal installation section in the Kubernetes docs.
:::
...
This name is used as a technical identifier within the system and cannot be changed later.
...
This name appears on portal home page and in email notifications.
...
### What a Portal Package Does Not Carry

You can export a portal and create it again as a new portal, on the same installation or on another one. A portal package is plain JSON and it leaves the installation, so no secret value is ever written into it. The package format uses an **allowlist**: only fields explicitly named as safe to travel are written into it, so a field added to the platform later is private by default until it is added to that list, rather than public until someone remembers to redact it. Secret fields are removed from the package during export, and again on import — so a hand-edited or an older package cannot smuggle one in either. Packages exported by earlier versions of Apinizer are still accepted for import and pass through the same filter.

:::warning

A portal package does **not** carry:

- Account passwords
- Account confirmation and password reset tokens
- Session identifier
- Portal API access tokens
- The production Management API key (Prod Promote settings)
- The Jira API token
- The production product/plan links an API Product or a plan was bound to on the source installation, and limit-assignment references
- The list of a suspended organization's suspended credentials (needed to resume it precisely — it does not carry over)
- The LDAP/OIDC synchronization binding of an organization (the record itself travels, but no longer linked to its source provider)
- An application's in-progress promotion or deletion workflow state
- A pending organization invitation's acceptance token — the invitation itself cannot be accepted on the target

:::

:::note

As a result, accounts of an imported portal **cannot sign in** with their passwords from the source portal. An administrator has to assign new passwords to those accounts, or the users have to use the "Forgot password" flow. Likewise, the Jira integration and the Prod Promote connection stay inactive until their credentials are entered again on the target portal.

:::

:::note

A metadata entry marked **Secret** on an application or an organization travels with its key, but its value is removed — the same way a secret field is handled everywhere else in the package. The entry still shows up on the imported record so an administrator knows it needs a value, but has to enter it again on the target installation.
...
You can click the copy icon next to the URL to copy the Portal URL.
...
Go to [Portal Settings](/en/api-portal/api-portal-settings) page to change these settings.
...
When multi-language mode is active, portal content can be displayed in both languages.
...
Use Portal Settings → Features tab to activate/deactivate features.
...
Statistics are updated in real-time and help you track portal usage.
...
Changing visibility type may affect existing users' access.
...
When URL is changed, links to the old URL may not work.
...
<Tabs>
  <TabItem value="active-portal" label="Active Portal">
  ...
  <TabItem value="passive-portal" label="Passive Portal">
...
Making portal passive immediately cuts access of existing users. Do this carefully.
...
Review [Portal Settings - General](/en/api-portal/api-portal-settings#general-settings) section for detailed information.
...
Review [Portal Settings - Email](/en/api-portal/api-portal-settings#email-notifications) section for detailed information.
...
Review [Portal Settings - Security](/en/api-portal/api-portal-settings#security) section for detailed information.
...
Review [Portal Settings - Features](/en/api-portal/api-portal-settings#features) section for detailed information.
...
Review [Portal Settings - Legal](/en/api-portal/api-portal-settings#legal) section for detailed information.
...
Portal copying feature may be added in future versions.
