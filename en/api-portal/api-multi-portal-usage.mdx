...
You can export a portal and create it again as a new portal, on the same installation or on another one. A portal package is plain JSON and it leaves the installation, so no secret value is ever written into it. Secret fields are removed from the package during export.
