...
:::info
The verification code (captcha) shown at portal login is validated on the server and renewed on every attempt. On logout, the session identifier is added to a revocation list; requests using the same token receive a 401 response until the token expires. Account details can only be viewed by the account itself.
:::
