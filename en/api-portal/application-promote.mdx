...
:::info Legacy API Keys are no longer shown on the Portal
The **API Clients** tab no longer shows a separate **Legacy API Keys** block, whatever the application's migration state. If an application's original single-key credential has not yet gone through the automatic migration described in [Legacy Records: Automatic Migration](/en/admin/identity-management/credentials#legacy-records-inventory-and-migration), that key simply has no row here yet — the Developer Portal no longer displays the legacy API Key / API Secret pair on this page, and no longer lets you generate or revoke it from here. The key itself is not deleted and keeps authenticating any traffic already using it; it only stops being manageable from this screen. Migration — including carrying the existing secret forward, so authentication keeps working without a forced reset — runs automatically the first time the Manager starts on this version, and again on save for a record that only becomes migratable later; there is no screen or button for it. Once it has run for an application, the migrated client appears above with the **Migrated from legacy** badge, exactly as any other API Client.

Removing the block also removed the read-only key/value metadata sub-table that used to appear underneath each legacy key row (key, value, secret flag, JWT inclusion). It is not shown anywhere on the Portal now — the values are unchanged and still readable, but only from API Manager, on the consumer's own **Metadata** tab (see [Metadata](/en/admin/identity-management/credentials#metadata)).

An application created from this version on never has a legacy credential to begin with: its first typed client already exists from the moment the application itself is created (see [Create Application and Subscription](#create-application-and-subscription) below).
:::
...
:::note[Prod approval issues a typed API Client]
Production access approved through this wizard is now provisioned as a typed API Client, not a legacy username/password credential. The approval result tells you to rotate the client's secret to obtain it — see [The Client Secret Is Shown Once](/en/admin/identity-management/credentials#the-client-secret-is-shown-once) — from **PROD Credentials** on this page or from the client's detail in API Manager.
:::
...
* [Legacy Records: Automatic Migration](/en/admin/identity-management/credentials#legacy-records-inventory-and-migration)
