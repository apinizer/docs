...
Until an application's credential has gone through this migration, its **API Clients** tab is empty and a **Legacy API Keys** block appears instead — the same API Key / API Secret pair as before (masked + show/hide + copy), with **Generate** and **Revoke**. Migration — including carrying the existing secret forward, so authentication keeps working without a forced reset — runs automatically the first time the Manager starts on this version, and again on save for a record that only becomes migratable later; there is no screen or button for it. See [Legacy Records: Automatic Migration](/en/admin/identity-management/credentials#legacy-records-inventory-and-migration). Once it has run for an application, the migrated client appears above with the **Migrated from legacy** badge, and the Legacy API Keys block disappears.
