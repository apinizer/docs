...
| Account Locked, Password Changed, Password Change Required, Password Reset Requested, Password Policy Violation | Reserved event types for the portal password policy and account lockout feature. Not produced by this version. | — |

### Reason codes

Only shown for **Login Failed** and the automatic **Account Disabled** event.

| Reason code | Meaning |
|---|---|
| **Wrong email or password** | The e-mail exists but the password did not match. |
| **Account disabled** | The account's status is Disabled (locked out after repeated failures, or disabled by an administrator). |
| **Account not approved** | The account exists but has not been approved yet, for a reason other than the two below. |
| **Email confirmation needed** | The account was registered but its e-mail address has not been confirmed. |
| **Waiting for approval** | Registration requires administrator approval and it has not happened yet. |
| **Account rejected** | An administrator rejected the registration. |
| **No account with this email** | No account exists with the e-mail address that was submitted. |
| **Account belongs to another portal** | The e-mail and password are valid, but for an account registered on a *different* portal — portal accounts are isolated per portal. |
| **Account inactive for too long** | Written on the automatic **Account Disabled** event by the [inactive-account job](/en/api-portal/api-portal-settings#security), never on a login attempt. |
| Account locked, Changed by the account holder, Changed by an administrator, Changed via a password reset link, Changed after a forced expiry, Password expired, Reset by an administrator | Reserved reason codes for the portal password policy and account lockout feature. Not produced by this version. |
