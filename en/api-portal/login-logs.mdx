...
For per-portal database and external connector settings for login logs, see [Connector delivery and payload](#connector-delivery-and-payload) below (Portal Settings → Login Log tab).
...
## Connector delivery and payload {#connector-delivery-and-payload}

Every row on this page is **always** written to the MongoDB `portal_login_logs` collection, and that cannot be turned off. On top of that, the same events can be forwarded to an external system — Elasticsearch, Kafka, RabbitMQ, ActiveMQ, Webhook, Syslog, Graylog or a JDBC database.

### Where it is configured

`Portal Management → [Select Portal] → Settings → Login Log tab`

<Frame caption="API Portal Settings — Login Log tab">
  <img
    src="/images/2026-09-2/portal-login-log-settings.png"
    alt="API Portal Login Log tab with always-saved warning and connector table"
    width="1000"
    height="397"
  />
</Frame>

On that tab, click `+` in the connector table, pick a connection configuration that already exists for the environment, and save. Login log settings are **independent per portal** — each portal has its own connector list, and each connector can be switched Active or Inactive on its own.

:::info

This per-portal list is only used while the **Session** stream's SIEM authority is **Legacy**. Once an administrator switches Session to **Active** on [SIEM & Log Forwarding](/en/admin/system-settings/siem-log-forwarding), portal sign-in and sign-out events are delivered by the rules and destinations configured there instead — for **every** portal at once — and this list stops being used. See [Portal sign-in delivery](/en/admin/system-settings/siem-log-forwarding#portal-sign-in-delivery) for the cutover.

:::

### Event names sent to a connector

The ten events in [Event types](#event-types) above are what the screen shows. The `eventType` a connector receives matches them, with one exception kept for backward compatibility: **Login Succeeded** and **Login Failed** are sent as `LOGIN_SUCCESS` and `LOGIN_FAILURE` — the same names the Management Console uses — so an existing parser keeps working. Every other type is sent under its own `PORTAL_`-prefixed name: `PORTAL_LOGOUT`, `PORTAL_ACCOUNT_LOCKED`, `PORTAL_ACCOUNT_UNLOCKED`, `PORTAL_ACCOUNT_DISABLED`, `PORTAL_PASSWORD_CHANGED`, `PORTAL_PASSWORD_CHANGE_REQUIRED`, `PORTAL_PASSWORD_RESET_REQUESTED` and `PORTAL_PASSWORD_POLICY_VIOLATION`.

### Payload

A portal event reaches a **Legacy Raw** destination in this shape — `portalId` and `accountId` are what distinguish it from a Management Console sign-in:
...
The field list of this profile is frozen. For the canonical envelope, per-field masking and size limits, move the destination to **Apinizer JSON v2** on the SIEM page — see [Session stream — event types and legacy payload](/en/admin/system-settings/siem-log-forwarding#session-stream-event-types-and-legacy-payload) for the Management Console counterpart of this payload and for the Graylog/GELF and database-table details, which are identical for portal events.
