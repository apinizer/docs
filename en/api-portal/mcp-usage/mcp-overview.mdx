...
<CardGroup cols={2}>
  ...
  <Card title="create_app" icon="plus">
    Create new application. An API Client is issued for it automatically. Used to group API subscriptions
  </Card>
...
**Credentials**: `get_app_credentials` and `get_credential_details` return real API Key/Secret values for **Owner** and **Developer** accounts. For a **Viewer** account these values are blanked out instead. Store this information securely.

:::

:::note

**Authorization**: Application owners can only access their own applications. The account is always identified from the `APINIZER-TOKEN` itself — you cannot look up another account's data by passing a different account reference to a tool.

:::

:::note

**Application and credential tools**: `create_app` and `add_api_key` are refused for a **Viewer** account. Referencing an application or credential id that does not exist, or that does not belong to your account, is refused the same way as an unauthorized one — these tools no longer distinguish "not found" from "not yours" in their response.

:::

:::warning[create_app response shape changed]
`create_app` now issues an **API Client** for the new application instead of a legacy API key. The response carries `initial_api_client` (with `api_client_key`, `client_id`, `name`, `operational_state` and a one-time `client_secret_once`) and an `api_client_note`; the previous `initial_credential_id` and `credential_note` fields are gone. The secret is only ever shown in this response — it cannot be retrieved again with `get_credential_details`. Store `client_secret_once` immediately, or generate a new secret for the application from the Portal if it is lost.
:::

:::note

**API visibility on test tools**: `get_api_spec`, `get_api_access_url` and `test_api` are refused when the API's product is not visible to your account, even if the API id itself is valid.

:::

:::note

**Portal scope (multi portal)**: Each portal's MCP endpoint only ever sees that portal's data. Whichever portal the MCP server you connected to belongs to, the applications, API products, subscriptions, credentials and traffic records the tools return belong to that portal. Passing another portal's application or API id is refused exactly the way a non-existent id is. A personal access token issued on one portal is likewise not accepted on another portal's MCP endpoint. On a single-portal installation the behaviour is unchanged.
