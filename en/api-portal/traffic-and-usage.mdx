...
:::info
Application traffic here is also attributed to an application whose identity is a typed API Client (client ID matching the traffic row's username/key), not only to one still carrying a legacy username/password credential.
:::
...
:::info Which rows are "mine"
The table and the dashboards show the traffic of every application owned by an organization you are an active member of — not only the subscriptions you created yourself. Applications that authenticate with an **API client** (JWT issued for the client id) are matched as well as applications using a legacy credential. Rows of other organizations are never listed and cannot be opened by id.
:::
