...
A proxy identifies the consumer in these cases:

| State on the proxy | Plan limit applied |
|--------------------|--------------------|
| **Active** Basic / Base64 / Digest / mTLS authentication policy | Yes |
| **Active** JWT or OAuth2 policy whose token is issued by a third party | Yes |
...
This is now surfaced in three places:

* **Manager > API Product > Plans** and **Subscriptions** tabs show a warning banner when the connected proxy has no active authentication policy, stating that plan limits will not be applied. When the saved configuration is fine but the proxy has not been redeployed yet, a separate banner asks for the redeploy.
* The proxy table on the **Manager > API Product > Security** tab has a **Consumer identity** column: Identified / Not identified per proxy, with the reason when it is not.
