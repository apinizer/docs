...
### Authentication required for plan limits to apply {#plan-limit-authentication}

Plan limits are counted **per consumer**, so every proxy in the product must be able to tell on whose behalf a request arrives. When a proxy cannot identify the consumer, the request is treated as anonymous, the plan's quota and rate limit are **never applied**, and no 429 is returned on overage.

A proxy identifies the consumer in these cases. An authentication policy kept inside an **active** Policy Group counts exactly like one placed directly on the proxy; one kept inside an **inactive** group counts as no policy at all.

| State on the proxy | Plan limit applied |
|--------------------|--------------------|
| **Active** Basic / Base64 / Digest / mTLS authentication policy | Yes |
| **Active** JWT or OAuth2 policy whose token is issued by a third party (not delegated to an OIDC identity provider — see below) | Yes |
| **Active** JWT or OAuth2 policy issued by Apinizer, **password** grant, provider **Secret Manager (ACL)** | Yes |
| **Active** JWT or OAuth2 policy delegated to an OIDC identity provider (the token is validated at request time against that provider), whose **Require matching Consumer** setting is on | Yes |
| **Active** JWT or OAuth2 policy delegated to an OIDC identity provider whose **Require matching Consumer** is off, or whose provider cannot be resolved | No — the provider validates the token but never confirms which consumer it belongs to |
| **Active** JWT or OAuth2 policy issued by Apinizer, **password** grant, but provider LDAP / Database / API | No — only the signature is verified, no consumer is resolved |
| **Active** JWT or OAuth2 policy issued by Apinizer with the **client_credentials** grant | No — only the signature is verified, no consumer is resolved |
| An authentication policy exists but is **inactive** | No |
| No authentication policy at all | No |
| OIDC session authentication only | No |

See [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) for what **Require matching Consumer** does on the identity provider side.

Activating the policy is not enough on its own: the change does not reach the gateway until the proxy is **redeployed**.

As of Apinizer 2026.09.4, the warning banner's wording matches the actual reason instead of always telling you to activate the policy:

- When every affected proxy's authentication policy is simply **inactive**, the banner tells you to activate it and redeploy.
- When the policy is already active but the scheme itself never resolves a consumer — a client_credentials-issued token, a session-only OIDC policy, or a delegated token whose identity provider has **Require matching Consumer** off — the banner points at the grant type or the identity provider instead. Redeploying alone would not fix it.
- No authentication policy at all, or a mix of the reasons above across several proxies, shows a general wording.
- A separate banner asks for a **redeploy** only once the saved configuration is already capable. It is never shown next to a banner that says the opposite.

This is surfaced in three places:

* **Manager > API Product > Plans** and **Subscriptions** tabs show the warning banner described above, naming the affected proxies.
* The proxy table on the **Manager > API Product > Security** tab has a **Consumer identity** column: Identified / Not identified per proxy, with the reason underneath when it is not, and the redeploy notice next to it when relevant.
* **Developer Portal product page > API Specification** labels the authentication type with **(inactive)** when it comes from an inactive policy, and shows a consumer-facing warning beneath it.
...
4. Ensure Security is ready — every connected proxy needs an **active** consumer-identifying authentication policy, otherwise plan limits are not applied ([details](#plan-limit-authentication))  
5. Set Visibility  
6. Publish  
