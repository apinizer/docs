...
Jira requires Features → **Jira Integration**. Connection fields are on [Jira Integration](/en/api-portal/jira-integration). The Support tab also has project key / issue type / status mapping (Support Ticket API path); when the navbar routes to `/jira-issue`, JiraConfig Project Key / Issue Type apply in practice.
