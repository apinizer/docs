...
<Tabs>
    <TabItem value="syntax-1-querying-data" label="Syntax 1: Querying Data">
        Syntax rules:
    ...
    <TabItem value="syntax-2-using-data-in-loop" label="Syntax 2: Using Data in Loop">
...
<Tabs>
    <TabItem value="manual-triggering" label="Manual Triggering">
    ...
    <TabItem value="automatic-triggering" label="Automatic Triggering">
    ...
    <TabItem value="triggering-via-endpoint" label="Triggering via Endpoint">
...
<Steps>
    ...
    <Step title="Create Token">
        ...
        The token belongs to a user, so it stops working as soon as that user is locked or deactivated, and when the token itself is revoked or reaches its expiry date.

        The token can also be sent in the **APINIZER-TOKEN** header instead. Use that one when the client already fills `Authorization` with something else — a browser session, a saved Basic credential, or a reverse proxy that adds its own header — so the task flow token is not hidden behind it.
    </Step>
</Steps>

:::note

The endpoint answers only while the option above is on. A task flow whose endpoint is not activated cannot be triggered over HTTP even with a valid token — the request is answered as if the task flow did not exist. Turning the option off is therefore enough to withdraw a published task flow.

:::

:::info

The request is authenticated before the task flow is looked up, so a caller without a valid token always gets `401` and learns nothing about which task flows exist. With a valid token, a task flow that does not exist and one whose endpoint is switched off both answer `404` in exactly the same way. The full call — accepted headers, order of checks and every response code — is documented in [Task Flow Trigger Endpoint](/api-reference/integration-task-flow-endpoint).
