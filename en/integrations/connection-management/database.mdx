  ...
  <Accordion title="Timeout and Connection Pool Parameters">
    <CardGroup cols={2}>
      <Card title="Connection Timeout" icon="clock">
        ...
        **Default:** 30000  
        **Notes:** The UI only enforces a required-field check; there is no min/max.  
        **Unit:** milliseconds
      </Card>
      ...
      <Card title="Pool Size" icon="layer-group">
        ...
        **Default:** 5  
        **Notes:** The UI's `min: 1` requirement applies only to the Initial Pool Size and Min Pool Size fields; the Max Pool Size field has no min/max limit.  
        **Unit:** count
      </Card>
...
A connection-level circuit breaker can be enabled to protect log delivery from a failing database server. It only affects log delivery — traffic, token, and audit logs written through this connection; API proxy backend calls and task flow executions that use the same connection type are not affected and keep their existing error handling.
...
When the error threshold is reached, the circuit opens: log delivery through this connection is skipped entirely for the duration of the sleep window — the database server is not contacted at all. Skipped log events are routed to the failover connection defined on this connector in the Gateway Environment when failover is enabled, otherwise they are dropped and counted. Only log delivery is affected; API proxy backend calls and task flow executions keep their normal behavior. After the sleep window elapses, a single attempt is made to reach the database server again — success closes the circuit, failure reopens it for another sleep window. In multi-pod deployments, circuit state is evaluated independently per pod.
  ...
  <Accordion title="Export">
    <CardGroup cols={2}>
      <Card title="Method 1: From the View Screen" icon="eye">
        Go to the connection's view screen and click the **[Export]** button at the top right. ZIP file is automatically downloaded.
      </Card>
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import Database Connection Pool]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
...
## Best Practices
  ...
  <Accordion title="Things to Do and Best Practices">
    <CardGroup cols={2}>
      <Card title="Pool Capacity Planning" icon="layer-group">
        ...
        **Best:** Adjusting Max Pool Size according to traffic and providing elastic scaling with Increment Count.
      </Card>
  ...
  <Accordion title="Things to Avoid">
    <CardGroup cols={2}>
      ...
      <Card title="Mis-sizing the Connection Pool" icon="layer-group">
        **Why to avoid:** The connection pool is always active; setting Max Pool Size too low makes requests queue until the pool frees up, while setting it too high opens unnecessary sessions on the target database and exhausts resources.  
        **Alternative:** Size Initial Pool Size / Min Pool Size / Max Pool Size / Increment Count according to traffic volume, and use Idle Connection Test Period and Max Idle Time to regularly evict idle connections.
      </Card>
    </CardGroup>
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Connection Pool Optimization" icon="layer-group">
    **Recommendation:** Adjust Max Pool Size according to traffic volume, and set Idle Connection Test Period and Max Idle Time based on connection health.  
    **Effect:** Connection opening cost decreases, response times decrease, resource usage is optimized.
  </Card>
...
:::
    ...
    <Steps>
      <Step title="Pool Size">
        Increase the Max Pool Size value.
      </Step>
