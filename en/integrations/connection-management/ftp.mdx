...
## Connection Parameters
  ...
  <Accordion title="Required Parameters">
    <CardGroup cols={2}>
      ...
      <Card title="Host" icon="server">
        ...
        **Default:** `localhost`  
        **Notes:** IPv4/IPv6 or FQDN accepted (e.g., `ftp.partner.local`)
      </Card>
      ...
      <Card title="WorkingDir" icon="folder">
        ...
        **Default:** `/`  
        **Notes:** Relevant user must have access permission (e.g., `/outbound`)
      </Card>
      ...
      <Card title="Timeout" icon="clock">
        ...
        **Default:** 30  
        **Notes:** Value is in seconds; a single value is applied to multiple timeouts (see info box below)
      </Card>
  ...
  <Accordion title="Optional Parameters">
    <CardGroup cols={2}>
      ...
      <Card title="VerifyServerCertificate" icon="shield-check">
        **Description:** Determines whether the FTPS server certificate will be verified  
        **Default Value:** false  
        **Recommended Value:** true (certificate verification recommended in production environment)
      </Card>
    </CardGroup>
  </Accordion>
...
**Timeout is a single value applied to multiple timeouts at the same time.** There is no separate "Request Timeout" or "Data Timeout" field. The entered `Timeout` (seconds) value is converted to milliseconds on the gateway side with `timeout * 1000` and used simultaneously for:
- FTP/FTPS: default timeout, connect timeout, and data timeout
...
:::
    ...
    <Step title="FTP Specific Parameters - Part 2">
        ...
        * **Timeout:** A value between 30-90 seconds is recommended according to network delay.
...
<CardGroup cols={2}>
  <Card title="Method 1: From the View Screen" icon="eye">
    * Go to the connection's view screen.
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import FTP Connection]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
