...
## Overview
  ...
  <Accordion title="Working Principle">
    <CardGroup cols={2}>
      ...
      <Card title="Queue and Send Management" icon="database">
        The asynchronous send queue, sized by maxInflightSends and queueSize, is drained by the configured number of background threads; this is not a connection pool, it is a queuing model over a single persistent connection
      </Card>
      ...
      <Card title="Connection Management" icon="link">
        After the operation completes, the connection remains open (persistent connection); if unused for a long time, tcpKeepAlive setting comes into effect
      </Card>
  ...
  <Accordion title="Advanced Features">
    <CardGroup cols={2}>
      ...
      <Card title="Connection Monitoring" icon="chart-line">
        Monitoring connection health and send performance metrics.
      </Card>
    </CardGroup>
  </Accordion>
...
### Timeout and Delivery Parameters
...
<CardGroup cols={2}>
  ...
  <Card title="Sender Threads" icon="cog">
    **Description:** Number of background threads that deliver GELF messages to Graylog; not a connection pool  
    **Default:** 0 (library default is used)  
    **Notes:** UI only enforces a required-field check; there is no min/max.
  </Card>
...
A connection-level circuit breaker can be enabled to protect log delivery from a failing Graylog server. It only affects log delivery — traffic, token, and audit logs sent through this connection; API proxy backend calls and task flow executions that use the same connection type are not affected and keep their existing error handling.
...
When the error threshold is reached, the circuit opens: log delivery through this connection is skipped entirely for the duration of the sleep window — the Graylog server is not contacted at all. Skipped log events are routed to the failover connection defined on this connector in the Gateway Environment when failover is enabled, otherwise they are dropped and counted. Only log delivery is affected; API proxy backend calls and task flow executions keep their normal behavior. After the sleep window elapses, a single attempt is made to reach the Graylog server again — success closes the circuit, failure reopens it for another sleep window. In multi-pod deployments, circuit state is evaluated independently per pod.
...
<img
    src="/images/yonetici/baglantilar/graylog-variable-new.png"
...
:::
    ...
    <Step title="Timeout and Delivery Settings">
        ...
        - Increase the threads value to prevent the queue (queueSize) from filling up under heavy traffic; this is not a connection pool setting, it is the number of background send threads.
    </Step>
  ...
  <Accordion title="Export">
    <CardGroup cols={2}>
      ...
      <Card title="Method 2: From the List Screen" icon="list">
        The row's **⋮** menu now has two items: the primary **Export** opens the Export/Import Wizard with this connection preselected — any `${VARIABLE}` reference in its host, URL or password field surfaces automatically on the dependency step; the secondary **Export as File** downloads the ZIP described below directly, without going through the wizard.
      </Card>
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import Graylog]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
...
## Best Practices
  ...
  <Card title="Connection Monitoring" icon="chart-line">
    **Recommendation:** Monitor queue (queueSize) saturation and send failure rates, track disconnect/reconnect events, set up alerting  
...
:::
  ...
  <Accordion title="Queue Full / Delivery Falling Behind (Queue Backpressure)">
    :::warning

Graylog has no connection pool; when queueSize fills up or the maxInflightSends concurrent-send limit is reached, new log events may be dropped or delayed, and the threads count may not be enough to keep up with high-volume traffic.

:::
    
    <Steps>
      <Step title="Review Queue Size">
        Increase queueSize according to traffic volume; keep RAM consumption in mind.
      </Step>
      
      <Step title="Check the Concurrent Send Limit">
        Verify that maxInflightSends handles the traffic without creating backpressure on Graylog.
      </Step>
      
      <Step title="Increase the Thread Count">
        Increase the threads value (leaving it at 0 uses the library default).
      </Step>
      
      <Step title="Check the Graylog Side">
        Verify that the Graylog GELF input is not lagging and has enough intake capacity.
      </Step>
    </Steps>
  </Accordion>
