...
**There is no separate "Request Timeout" or "Pool Size" field.** The RabbitMQ connection is established through a single AMQP `ConnectionFactory` object that is not user-configurable; there is no adjustable connection pool size. The two fields below (Connection Timeout, Requested Heartbeat) are real, configurable fields — the UI has no min/max validation for them, only a required-field check.
...
<CardGroup cols={2}>
  <Card title="Connection Timeout" icon="clock">
    **Description:** Time to wait until AMQP connection is established  
    **Default:** 60000 ms  
    **Notes:** UI only checks that the field is required; no min/max.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Low Latency Jobs" icon="zap">
    ...
    **Solution:** connectionFactoryRequestedHeartbeat=30  
...
<img
    src="/images/yonetici/baglantilar/rabbitmq-variable-new.png"
...
<Step title="Timeout and Protocol Settings">
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import RabbitMQ Connection]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
...
<CardGroup cols={2}>
  ...
  <Card title="Usage in Integration / Connector Steps" icon="workflow">
    Connection is selected in steps requiring message sending via the RabbitMQ AMQP protocol. Example: Steps such as "Send Message", "RabbitMQ Publish", "AMQP Message". Connection selection is made from the Connection field in the configuration of these steps
  </Card>
