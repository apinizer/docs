  ...
  <Accordion title="How it Works">
    <CardGroup cols={2}>
      ...
      <Card title="Connection Management" icon="settings">
        After the operation is completed, the connection is closed; the socket is kept open for continuous trap listening
      </Card>
...
There is no separate "Request Timeout" or "Pool Size" field for the SNMP connection; there is only a single timeout (in seconds) and a retry count.

:::

<CardGroup cols={2}>
  <Card title="Timeout" icon="clock">
    **Description:** Single timeout value used while waiting for the SNMP request response  
    **Default:** None (can be left empty)  
    **Unit:** seconds  
    **Min:** 0
  </Card>
  
  <Card title="Retry Count" icon="repeat">
    **Description:** Number of retry attempts for failed SNMP requests; not a wait interval  
    **Default:** None (can be left empty)  
    **Unit:** count  
    **Min:** 0
  </Card>
...
:::
    ...
    <Step title="Timeout and Retry Settings">
        - In the advanced section, set the **Timeout** value in seconds according to your network delay (there is no separate Request Timeout or Pool Size field).
        - Determine how many times failed messages should be retried with **Retry Count**.
    </Step>
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import SNMP]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
...
<CardGroup cols={2}>
  ...
  <Card title="Usage in Integration / Connector Steps" icon="workflow">
    Connection is selected in steps requiring SNMP trap, inform, or get messages. Example: Steps like "SNMP Trap Receiver", "SNMP Inform Handler", "SNMP Get Request". Connection selection is made from the Connection field in the configuration of these steps
  </Card>
  ...
  <Accordion title="Performance Tips">
    <CardGroup cols={2}>
      <Card title="Trap Prioritization" icon="arrow-up">
        **Recommendation:** Define a separate connection for critical devices  
