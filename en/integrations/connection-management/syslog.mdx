...
<CardGroup cols={2}>
  <Card title="Max Message Size (Bytes)" icon="scissors">
    ...
    **Notes:** For UDP, keep this well under your network path's real datagram size; RFC 5426 recommends staying under 2048 bytes for compatibility with older relays and intermediate devices. See [Splitting Oversized Messages](#splitting-oversized-messages) below for what a split message looks like on the wire and how it relates to the size policy on the SIEM & Log Forwarding page.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="Keystore" icon="certificate">
    **Description:** The client certificate presented to the syslog server for mutual TLS (mTLS).

    **Default:** empty

    **Notes:** When left empty, no client certificate is presented and the session is server-only TLS. The list is filled from the key stores defined under **Secrets**; see [Keystore Management](/en/admin/secrets-management/keystore-management).
  </Card>

  <Card title="Truststore" icon="shield">
  ...
  <Card title="Hostname Verification" icon="network-wired">
  ...
  <Card title="Write RFC 5424 Structured Data" icon="file-code">
  ...
  <Card title="Enterprise Number (PEN)" icon="tag">
    ...
    **Notes:** Optional. When it is empty, only the `origin` and `timeQuality` elements are written. A number can be requested free of charge from [IANA](https://www.iana.org/assignments/enterprise-numbers/assignment/apply/).
  </Card>
</CardGroup>
...
:::
  ...
  <Step title="Security and Authentication Settings">
    ...
    - Step-by-step mutual TLS setup, including the syslog server side, is on [Syslog Integration](/en/operations/administrator-guides/syslog-integration#mutual-tls-mtls-setup).
  </Step>
  ...
  <Accordion title="Export">
    <CardGroup cols={2}>
      ...
      <Card title="Method 2: From the List Screen" icon="list">
        The row's **⋮** menu now has two items: the primary **Export** opens the Export/Import Wizard with this connection preselected — its certificate/key store and any `${VARIABLE}` reference surface automatically on the dependency step; the secondary **Export as File** downloads the ZIP described below directly, without going through the wizard.
      </Card>
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import Syslog Connection]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
        - Select the exported package and follow the wizard's steps.
        - The wizard checks format, name conflicts and dependency availability before you confirm the import.
      </Card>
...
### Idle Connections and Reconnects {#idle-connections-and-reconnects}
...
### Receivers That Close the Connection Right After Accepting It {#connect-time-liveness}

The idle-reconnect refresh above only helps when Apinizer is the one leaving a connection unused. Some receivers behave differently: they complete the TCP handshake and then close the session immediately, before a single byte is exchanged — a session-limit ceiling or a source-IP rejection typically does this. Apinizer cannot see that at connect time, so without a check the first write on the fresh socket would fail, the retry on another fresh socket would also land on a receiver that closes it right away, and this could repeat rapidly enough to open a new TCP connection for every batch, with records silently lost in between.

Apinizer now reads from the socket for a short moment right after it connects (after the TLS handshake too, when TLS is used) and before writing anything to it. If the receiver has already closed the session, that read reports it immediately and the connection is treated as a normal connection failure — it counts toward the three-consecutive-failure threshold that moves traffic to the failover connector, and a fresh attempt follows five seconds later, exactly as for any other connection error. If the receiver stays quiet, as an ordinary syslog receiver does, the check simply times out and the socket is used right away; no extra I/O and no measurable delay are added once the connection is up and being reused for further batches.

When a receiver instead accepts the connection cleanly but closes it again after every batch, reconnecting on every failed write could still open connections at a very high rate. Apinizer paces reconnects after a write failure to at most roughly ten per second per socket, and once at least ten batches have gone through in a summary window with more than half of them needing this kind of recovery, a distinct warning is logged once a minute:
...
That line is the signal to check the receiving side's own session limits or connection policy — the sender is retrying correctly, but a receiver that behaves this way can still lose records between the moment a batch is accepted into the kernel send buffer and the moment the receiver's next close is discovered.

:::info

Two JVM system properties tune this behaviour for the whole node; both apply only to Apinizer's TCP syslog sender, not to any other connector.

- `-Dapinizer.syslog.connectLivenessMillis=<milliseconds>` — how long the connect-time liveness read waits before deciding the receiver is silent and healthy. Default **100 ms**, capped at 2000 ms; `0` turns the check off entirely, restoring the pre-APNZ-6839 behaviour where a receiver that closes the session at accept time is only discovered on the next write.
- `-Dapinizer.syslog.minReconnectIntervalMillis=<milliseconds>` — the minimum time between two reconnects that follow a write failure on the same socket. Default **100 ms**, capped at 5000 ms; `0` removes the pacing. This does not affect the first connection, the proactive idle refresh above, or the half-open health probe — those already have their own, longer intervals.

An out-of-range or unparseable value falls back to the default and is logged as a warning rather than failing the connection.

:::

The periodic per-socket summary described above now also reports two more counters: `reconnects`, the number of fresh TCP connections opened on that socket (the first connection counts too, so a socket that has never dropped still shows `1`), and `peerClosedAtAccept`, how many of those connections were found closed by the connect-time liveness check right after being opened. A `peerClosedAtAccept` count above zero means the fault is on the receiving side — a session limit, a source-IP rejection, or a load balancer in between — rather than in the network path or on the Apinizer side.

## Splitting Oversized Messages {#splitting-oversized-messages}
...
See [Delivery health](/en/admin/system-settings/siem-log-forwarding#delivery-health) on the SIEM & Log Forwarding page for the full state reference, the applied configuration revision, and the Gateway-side diagnostics JSON used to compare delivery across nodes.
