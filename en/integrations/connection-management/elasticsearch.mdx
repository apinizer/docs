...
<CardGroup cols={2}>
  <Card title="ILM Policy Indicator" icon="calendar">
  ...
  <Card title="Index Template Indicator" icon="file-code">
  ...
  <Card title="Data Stream Indicator" icon="database">
  ...
  <Card title="Live Check Button" icon="rotate">
...
:::note Unverified state (2026.09.3)

If the cluster cannot be asked during Live Check (unreachable, timeout, or a credential error), the three indicators switch to an orange **question mark** instead of turning red. The saved "created" values are kept, the tooltip shows when the failed check ran, and the toast carries the reason. "Not found" (a real 404 from Elasticsearch) is the only answer that turns an indicator red; a cluster that is down never overwrites a verified green. Run Live Check again once the cluster is reachable.
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import Elasticsearch]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
...
:::
  ...
  <Accordion title="What does the Live Check button do?">
    :::info
...
:::
  ...
  <Accordion title="Can I create a data stream by clicking the Data Stream indicator?">
    :::info
