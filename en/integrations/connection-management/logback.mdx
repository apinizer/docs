...
## Overview
  ...
  <Accordion title="Working Principle">
    <CardGroup cols={2}>
      ...
      <Card title="File Handle Management" icon="layer-group">
        Logback RollingFileAppender reuses a single open file handle for the same log file on the worker node; there is no configurable connection pool concept.
      </Card>
      ...
      <Card title="Error Management" icon="triangle-exclamation">
        In case of connection error or authentication error, deployment-result dialog activates with detailed error message, user is advised to review log directory and permission checks.
      </Card>
    </CardGroup>
  </Accordion>
...
The Logback connection writes to the local file system; network timeout and connection pool parameters do not apply to this connector.
...
<img
    src="/images/yonetici/baglantilar/logback-variable-new.png"
...
:::
  ...
  <Step title="Disk Quota Settings">
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import Logback]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
...
<CardGroup cols={2}>
  ...
  <Card title="Usage in Integration / Connector Steps" icon="plug">
    Connection is selected in steps requiring file-based log writing via the Logback RollingFileAppender. Examples: steps like "Logback Appender", "File Logger", "Rolling File Writer". Connection selection is made from the **Connection** field in these steps' configuration
  </Card>
  ...
  <Accordion title="Performance Tips">
    <CardGroup cols={2}>
      ...
      <Card title="Connection Monitoring" icon="chart-bar">
        **Recommendation:** Monitor disk usage and log write failures, connect worker pod health checks to alerting  
        **Impact:** Problems are proactively detected, disk fullness risks are identified early, downtime decreases
      </Card>
    </CardGroup>
  </Accordion>
