...
:::tip[If you need to carry the certificate with it]

The **Export**/**Import** below carries only the connection's own fields; if you attached an mTLS certificate (Require Certificate Type = REQUIRED_AN_PN), that certificate is **not** carried this way. To move the LDAP Pool Connection together with its certificate, use the **Connection** type in the [Export/Import Wizard](/en/admin/secrets-management/export-import) instead — it surfaces the certificate/key store dependency automatically and packages it along.
  ...
  <Accordion title="Export">
    <CardGroup cols={2}>
      ...
      <Card title="ZIP Contents" icon="folder-open">
        ...
        - Does NOT include the certificate or key store — see the Wizard tip above to carry them together
      </Card>
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      ...
      <Card title="Import Scenarios" icon="info-circle">
        ...
        **Scenario 2: Certificate Needed** → This package carries no certificate; if Require Certificate Type = REQUIRED_AN_PN, select the Certificate Id on the target by hand after import, or use the **Connection** type in the Wizard to carry it together with the certificate.
      </Card>
    </CardGroup>
  </Accordion>
