  ...
  <Accordion title="Working Principle">
    <CardGroup cols={2}>
      ...
      <Card title="Connection Management" icon="database">
        Gateway opens a new LDAP connection if no suitable connection exists for the selected environment; this behavior is automatic and not configurable from the screen. The only value the user can adjust is the connect time (Connection Timeout)
      </Card>
      ...
      <Card title="Connection Termination" icon="link">
        When the operation completes, the connection is automatically managed by Gateway for subsequent requests; this behavior is not configurable from the screen
      </Card>
...
## Connection Parameters
  ...
  <Accordion title="Optional Parameters">
    <CardGroup cols={2}>
      ...
      <Card title="Directory Type" icon="sitemap">
        **Description:** Which directory product family the server belongs to  
        **Default Value:** (Empty — unknown)  
        **Recommended Value:** Detected with the **[Detect Directory]** button, or picked by hand
      </Card>
    </CardGroup>
  </Accordion>
...
**There is no separate "Request Timeout", "Pool Size", or "Bind Retry Interval" field.** The LDAP connection has only one timeout field: `Connection Timeout`. This value limits the time to connect to the LDAP server; if the field is left empty or a value of 0 or less is entered, the server side defaults to 30000 ms.
...
<CardGroup cols={2}>
  ...
  <Card title="High Traffic Queries" icon="chart-line">
    ...
    **Solution:** Connection Timeout value is adjusted to real network latency  
    **Expected Behavior:** Timeout errors decrease, queries complete in normal time
  </Card>
</CardGroup>
...
<img
    src="/images/yonetici/baglantilar/ldap-variable-new.png"
...
:::
    ...
    <Step title="Detecting the Directory Type - Optional">
        **Directory Type:**

        - Pick the directory product from the **Directory Type** dropdown: **Active Directory**, **OpenLDAP**, **389 Directory Server / FreeIPA**, **Other (Standard LDAP)**, or leave it unset (unknown).
        - This choice does not change how the connection works. It only lets identity provider screens suggest matching field defaults.

        **Detect Directory:**

        - Click the **[Detect Directory]** button to ask the server what it is and where its trees start.
        - The question is sent from the API Manager, so the server must be reachable from there.
        - Nothing is saved. The answer is shown as a suggestion for you to review.
        - Click **[Use this directory type]** to fill the Directory Type field with the detected value.
        - Click one of the reported tree roots to fill it into the **Base DN** field.
        - The result also states whether the server can return large result sets page by page.
        - If the server does not publish this information, keep using the values you entered by hand.
    </Step>
    ...
    <Step title="Timeout Setting">
        **Connection Timeout:**
        ...
        - Minimum: 1000, Maximum: 300000 milliseconds
        - There is no separate Request Timeout, Pool Size, or Bind Retry Interval field.
    </Step>
    ...
    <Step title="Testing Connection Step by Step - Optional">
        - Click the **[Test connection (step by step)]** button to run a 7-step check instead of the single pass/fail result above.
        - The steps run in order: Configuration Check, Server Connection, Secure Connection (TLS), Service Account Sign-in, Directory Information, Schema Check, Base DN Read.
        - Each step shows a result, an explanation, a suggestion, and a copyable technical detail.
        - The original single-result **[Test Connection]** button above is still available.
    </Step>
...
Diagnostic Mode writes every LDAP search and sign-in attempt made over this connection to the log in full detail, for a limited time. Passwords are never written.

<CardGroup cols={2}>
  <Card title="Turning It On" icon="stethoscope">
    On the connection detail page, pick a duration of 15, 30, or 60 minutes and click **[Turn On]**.
  </Card>

  <Card title="Automatic Turn-Off" icon="clock">
    Diagnostic Mode turns itself off when the time is up, so no restart is needed.
  </Card>

  <Card title="Kept on Save" icon="save">
    The setting is kept when you save the connection; you do not need to turn it on again after every save.
  </Card>

  <Card title="Not Exported" icon="file-export">
    Diagnostic Mode is never included in an exported connection package.
  </Card>
</CardGroup>

### Connection Health Status

The connection list shows a **Status** badge for each LDAP connection: **Reachable**, **Unreachable**, or **Unknown**. Apinizer checks the connection every 5 minutes with a sign-in and a basic search.

A connection that resolves a critical field from an environment variable can only be checked for one selected environment; use the **[Test Connection]** button on the connection screen for that case.
...
:::tip[Certificate travels automatically now]

The list screen's row-menu **Export** and the toolbar **Import** already go through the **Connection** type in the [Export/Import Wizard](/en/admin/secrets-management/export-import) — it surfaces an attached mTLS certificate (Require Certificate Type = REQUIRED_AN_PN) and key store as dependencies on the dependency step and packages them along automatically, no extra step needed. The row menu's secondary **Export as File** item and the detail page's own **Export** button (below) now bundle the certificate/key store the same way — matched by name on import, so importing that file no longer fails with an unresolved certificate reference.
  ...
  <Accordion title="Export">
    <CardGroup cols={2}>
      ...
      <Card title="Method 2: From the List Screen" icon="list">
        The row's **⋮** menu now has two items: the primary **Export** opens the Export/Import Wizard with this connection preselected — its certificate/key store and any `${VARIABLE}` reference surface automatically on the dependency step (see the tip above); the secondary **Export as File** downloads the ZIP described below directly, without going through the wizard.
      </Card>
      ...
      <Card title="ZIP Contents (Export as File)" icon="folder-open">
        - Connection JSON file
        - Metadata information
        - The attached certificate or key store, if one is set (matched by name on import; created if missing, or updated/duplicated there following the same **Replace Existing** / **Import as New** rule as any other certificate reference)
      </Card>
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click the **Import** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
      ...
      <Card title="Import Scenarios" icon="info-circle">
        ...
        **Scenario 2: Certificate Needed** → Only applies to a package made with the row menu's secondary **Export as File** item, applied through the legacy `/api/export-imports/import` endpoint — the certificate is not included there; select the Certificate Id on the target by hand after import. The list screen's own **Import** button (above) already carries the certificate through the wizard, so this gap does not occur there.
      </Card>
    </CardGroup>
  </Accordion>
