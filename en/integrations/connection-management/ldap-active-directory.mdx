...
The list screen's row-menu **Export** and the toolbar **Import** already go through the **Connection** type in the [Export/Import Wizard](/en/admin/secrets-management/export-import) — it surfaces an attached mTLS certificate (Require Certificate Type = REQUIRED_AN_PN) and key store as dependencies on the dependency step and packages them along automatically, no extra step needed. Only the row menu's secondary **Export as File** item (below) still produces a raw ZIP of the connection's own fields, without the certificate.
  ...
  <Accordion title="Export">
    <CardGroup cols={2}>
      ...
      <Card title="ZIP Contents (Export as File)" icon="folder-open">
        ...
        - Does NOT include the certificate or key store — the wizard's **Export** (see the tip above) does
      </Card>
