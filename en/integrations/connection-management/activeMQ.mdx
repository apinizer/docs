  ...
  <Accordion title="What is its Purpose?">
    <CardGroup cols={2}>
      ...
      <Card title="Performance and Durability" icon="gauge">
        Provides performance and durability in high-traffic scenarios thanks to advanced timeout management.
      </Card>
    </CardGroup>
  </Accordion>
  ...
  <Accordion title="Working Principle">
    <CardGroup cols={2}>
      ...
      <Card title="Connection Management" icon="gear">
        When the operation completes, the connection is closed; the Close Timeout value is applied during closing.
      </Card>
...
**Session Acknowledgement Policy:** Reliable delivery with CLIENT_ACKNOWLEDGE default; AUTO_ACKNOWLEDGE for fast integration or SESSION_TRANSACTED for transactional control.
...
**Connection Monitoring:** Monitoring connection health and performance metrics.
  ...
  <Accordion title="Optional Parameters">
    <CardGroup cols={2}>
      ...
      <Card title="Client ID" icon="id-card">
        ...
        **Default Value:** apinizer-activemq-clientid  
...
<CardGroup cols={2}>
  ...
  <Card title="Long Duration Operation" icon="hourglass">
    ...
    **Solution:** Send Timeout=120000, Request Timeout=120000  
  ...
  <Card title="Test Environment Simulation" icon="flask">
    ...
    **Solution:** Request Timeout=45000, Content Type=application/json  
...
<img
  src="/images/yonetici/baglantilar/active-variable-new.png"
...
<Step title="Timeout Settings">
  ...
  - Optimize timeout durations according to broker response times for heavy traffic.
</Step>
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import Apache ActiveMQ]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
...
<CardGroup cols={2}>
  ...
  <Card title="Usage in Integration / Connector Steps" icon="plug">
    - The connection is selected in steps that need to send messages to, or consume messages from, an ActiveMQ queue or topic.
    - Example: Steps like "Send Message", "Consume Message".
...
| Observability | **Bad:** Not tracking connection logs.<br/><br/>**Good:** Monitoring basic error logs.<br/><br/>**Best:** Sending timeout and broker metrics to centralized monitoring system. |
...
| Message Throughput | **Recommendation:** Increase the Send Timeout value in high volume sends.<br/><br/>**Impact:** Message queues are consumed without accumulation. |
...
| Connection Monitoring | **Recommendation:** Track timeout rates, perform connection health check, set up alerting.<br/><br/>**Impact:** Problems are detected proactively, performance bottlenecks are identified early, downtime decreases. |
