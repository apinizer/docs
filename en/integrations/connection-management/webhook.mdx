...
**Producer Throttle** (default 100 ms, capped at 2000 ms) is how long the sending side waits when the batch buffer is full before routing the event to the failover connector when one is configured, or dropping it otherwise. It is independent from the batch delay above.
