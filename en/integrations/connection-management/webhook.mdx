...
<CardGroup cols={2}>
  <Card title="Connection Timeout" icon="clock">
    **Description:** Wait time for TCP connection establishment (`connectTimeoutSeconds`). Falls back to the single **Timeout** field above when left empty.  
    **Default:** 2000  
    **Min:** 1000 | **Max:** 60000  
    **Unit:** milliseconds
  </Card>
  ...
  <Card title="Request Timeout" icon="hourglass">
    **Description:** Total response wait time of the HTTP request (`responseTimeoutSeconds`). Falls back to the single **Timeout** field above when left empty.  
    **Default:** 2000  
    **Min:** 1000 | **Max:** 60000  
    **Unit:** milliseconds
  </Card>
  ...
  <Card title="Total Pool Connections" icon="database">
    ...
    **Default:** 48  
    **Rule:** must be at least Connections Per Host
  </Card>
...
Four timeout phases can be set independently. Any of them left empty falls back to the single **Timeout** value in Connection Parameters, so existing connections keep working unchanged.
...
<CardGroup cols={2}>
  ...
  <Card title="Response Timeout" icon="hourglass">
    Time allowed to receive the full response.
  </Card>
  ...
  <Card title="Socket Timeout" icon="network-wired">
    Maximum idle time allowed between data packets on an already-open socket.
  </Card>
</CardGroup>

### Write Stall Timeout

**Write Stall Timeout** (default 30 seconds) is a separate guard: if sending the request body takes longer than this duration, the request is cancelled. Setting it to 0 or a negative value disables the guard.
...
**Producer Throttle** (default 100 ms, capped at 2000 ms) is how long the sending side waits when the batch buffer is full before routing the event to the failover connector instead. It is independent from the batch delay above.
...
When the error threshold is reached, the circuit opens: log delivery through this connection is skipped entirely for the duration of the sleep window — the endpoint is not contacted at all. Skipped log events are routed to the connection's failover connector when one is configured, otherwise they are dropped and counted. Only log delivery is affected; API proxy backend calls and task flow executions keep their normal behavior. After the sleep window elapses, a single attempt is made to reach the endpoint again — success closes the circuit, failure reopens it for another sleep window. In multi-pod deployments, circuit state is evaluated independently per pod.
...
<CardGroup cols={2}>
  ...
  <Card title="Max ACK Resend" icon="repeat">
    Number of times an unacknowledged batch is resent before it is routed to the failover connector.  
    **Default:** 2
  </Card>
</CardGroup>
