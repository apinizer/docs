...
## Overview
  ...
  <Accordion title="Working Principle">
    <CardGroup cols={2}>
      ...
      <Card title="SSH Session Management" icon="terminal">
        A new SSH session is opened for every script execution; there is no user-configurable connection pool.
      </Card>
  ...
  <Accordion title="Required Parameters">
    <CardGroup cols={1}>
      ...
      <Card title="SSH Port" icon="network">
        ...
        **Example Value:** `22`  
        **Notes:** Common SSH port is 22; the field is required and has no default value. If a different port is used, it must be opened in Network/Firewall
      </Card>
...
<img
    src="/images/yonetici/baglantilar/linux-variable-new.png"
...
:::
    ...
    <Step title="Linux Server Parameters">
        ...
        - **SSH Port:** Common SSH port is 22; the field is required and has no default value. If a different port is used, write the port and ensure it's opened in firewall.
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import Linux Script]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
  ...
  <Accordion title="Do's and Best Practices">
    <CardGroup cols={2}>
      ...
      <Card title="Resource Consumption" icon="activity">
        ...
        **Best:** Keep per-host concurrency limited and spread long-running scripts across scheduled jobs in separate time windows
      </Card>
