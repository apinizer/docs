...
:::
  ...
  <Accordion title="Kafka Producer Timeout Properties (Properties Map)">
    :::info
...
:::
    <CardGroup cols={2}>
      ...
      <Card title="delivery.timeout.ms" icon="paper-plane">
...
:::
    ...
    <Step title="Timeout and Producer Parameters">
  ...
  <Accordion title="Import">
    <CardGroup cols={2}>
      <Card title="Import Steps" icon="upload">
        - Click **[Import Kafka]** button on the main list — it now opens the Export/Import Wizard's import step with type **Connection** preselected (Admin scope when opened from Administration); the old upload dialog is gone.
