...
| Load Fixed Bitset Filters Eagerly | Whether the parent bitsets of nested fields are kept in memory ahead of time. Turning it off lowers heap usage on the Elasticsearch nodes. | Enabled | UI |
...
<Steps>
    ...
    <Step title="Index Template and ILM Configuration">
        ...
        **Administrate Setting:**
        ...
        - Replica count
        - Refresh interval
        - Preference for loading fixed bitset filters eagerly
...
If Live Check cannot complete (for example, the cluster is unreachable), an error message is shown but the saved indicators and the last check time are **not** changed — an unreachable cluster is not treated as "does not exist".
...
| Technical | Live Check failed with an error, did my status indicators get reset? | No. If the cluster cannot be reached while checking, an error message is shown but the previously saved indicators and last check time are left unchanged; being unreachable is not treated as "does not exist" |
