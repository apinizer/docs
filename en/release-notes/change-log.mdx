...
- **Environment Variable Access in Script Policy**

Environment variable values defined in the active environment can now be read from within a script body. The same dynamic variable mechanism used in other policies is now available inside scripts as well. [See details.](/en/develop/policies/script#accessing-environment-variables) APNZ-6267

- **Access to Other Credentials in Script Policy**

The existing credential variables returned only the authenticated credential of the request; a script can now access another credential by its username or client ID and read identity metadata along with key material (password, certificate, key pairs, key/trust store, JWK). [See details.](/en/develop/policies/script#accessing-credential-information) APNZ-6267 APNZ-6279

- **Dynamic Parameter Support in XSLT Policy**

In the XML Transformation policy, parameters passed to XSLT can now be resolved dynamically from an environment variable or a custom variable in addition to a static value. [See details.](/en/develop/policies/xml-transformation#xslt-parameter-usage) APNZ-6280

- **Credential and Credential Organization Metadata**

Arbitrary key/value metadata entries can now be defined on credentials and credential organizations. Each entry can be flagged independently to be emitted as a JWT payload claim or as an additional field in the OAuth token endpoint response body. Secret-flagged values are encrypted at rest and masked in API responses. Reserved JWT claim names (RFC 7519, Apinizer internal) and reserved OAuth response keys (including dynamically renamed field names from Token Management Settings) are rejected at save time. Organization-level metadata is merged with credential-level metadata at token issuance (credential wins on key collision). The same merged view is exposed to Groovy/JavaScript script policies via `credentialMap.get(username).getMetadata()`, with `getCredentialMetadata()` / `getOrganizationMetadata()` for layer-isolated access. APIops endpoints under `/apiops/projects/{p}/credentials/{u}` and the new `/apiops/projects/{p}/credential-organizations/{code}` expose metadata for programmatic management. [See details.](/en/admin/identity-management/credentials#metadata) APNZ-6285 APNZ-6286
...
**APNZ-6281** : An error that occurred during cache operations from within the Script policy has been fixed.
...
A new standalone policy has been added that enables explicit cache read, write, and delete operations within the API Gateway policy chain. The Cache policy supports three operations: LOOKUP (read from cache with variable interpolation), POPULATE (write a variable's value to cache with static or dynamic TTL), and INVALIDATE (delete a cache entry). Features include cache key hashing for case-insensitive lookups, support for distributed and local cache storage, dynamic TTL from response values, and configurable error handling (CONTINUE or FAULT). The policy reduces backend load, improves response times, and enables flexible caching strategies at any point in the request/response flow. [See.](/en/develop/policies/cache) APNZ-6246
...
A new standard field **Cache Connection Timeout** (in seconds) has been added to all screens that perform cache operations. An additional field **Error Action** (CONTINUE or FAULT) has been added to 10+ cache-using screens: API Call cache, OIDC, JWT cache, JOSE key cache, Backend API Authentication cache, API Based Throttling, API Based Quota, Client Ban list cache, Token Service cache, and others. When a cache operation fails (connection timeout, unavailable service, etc.), the behavior is now consistently configurable across all policy types and screens: CONTINUE causes the flow to proceed without the cached result, while FAULT raises an error and stops the flow. APNZ-6250

- **Trace Screen Policy-Specific Execution Details**

Eight policy types now display an **"Execution Details"** button in the policy node panel header when selected in the trace detail drawer. Clicking the button opens a modal with policy-type-specific domain information in accordion sections. The details include: **Cache Policy** (operation, cache key, hit/miss, TTL); **API Call Policy** (cache status, request/response, timings); **Backend API Authentication** (authentication result, username, roles); **JOSE Key Fetch** (cache hit/miss, key extraction path); **Client Ban** (ban status, duration); **Throttling/Quota** (usage vs limit, allowed/blocked); **Business Rule** (per-action execution details). This view is available on both request-side and response-side policy nodes. APNZ-6259

<span style={{ display: 'inline-block', backgroundColor: '#e65a2a', color: '#ffffff', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>BUG FIXES</span>

- **Variable Usage Inside Specify Value for Business Rule Modify Operators**

In a Business Rule policy, when **Specify Value** is selected on the Target side of an action, the `${...}` Environment Variable and `#{...}` Context Variable placeholders entered inside the value of the **Format**, **Mask**, **Transform**, and **Insert** operators of the **Modify** action are now resolved at runtime. Previously, the value entered for these operators was treated as raw text and the placeholders were used as-is. APNZ-6248

- **Backend Forwarding of Requests with Encoded URL Paths**

Fixed an issue where requests carrying encoded special characters in the URL path (for example, JSON parameters embedded in the path) were rejected with an "Illegal character in path" error when forwarded to the backend service through the API Gateway. All special characters in the path are now consistently re-encoded before the request is forwarded to the backend. APNZ-6249

- **Variable Usage Inside Use Custom Value in Condition Rules and Specify Value in Action Rules**

The `${...}` Environment Variable and `#{...}` Context Variable placeholders entered inside policy condition rules (Query Builder) when **Use Custom Value** is selected for the second value, as well as in action rules under the **Specify Value** branch, are now resolved at runtime. Previously, the value in these fields was compared as a literal string and placeholders were not resolved. The fix applies to the custom-value and target-value fields of every policy that reuses condition and action entities. APNZ-6253

- **Dotted Custom Variable Keys (`#{custom.environment}`) in Routing Addresses and Template Fields**

Fixed an issue where dotted flat custom-variable keys set via a script (e.g. `customVariableMap.put("custom.environment", value)`) returned an empty string when referenced as `#{custom.environment}`. Dotted flat keys are now resolved correctly everywhere `#{...}` is processed — including routing addresses (e.g. `https://petstore.#{custom.environment}.io/v2`), header and parameter templates, and condition/action custom-value fields. APNZ-6254

- **Multi-Word Phrase Search in Body Fields on API Traffic Screen**

Fixed an issue where multi-word expressions (e.g. `date of birth`) entered in the body search fields (From Client Body, To Backend API Body, From Backend API Body, To Client Body) on the API Traffic screen and in the Advanced Filtering tab using CONTAINS / NOT_CONTAINS returned no results. Single-word searches continue to work as before. [See.](/en/analytic/api-traffic#body-field-search) APNZ-6256

- **Variable Usage in Action Definition Conditions and Add Action Injection Targets**

The `${...}` Environment Variable and `#{...}` Context Variable placeholders entered in the **Specify the Value** field under the **Action Definition > Conditions** section of a policy are now resolved at runtime. In addition, the **Injection Target** of a Business Rule **Add** action now writes to the correct region based on the target type: when a body-type Variable Definition is selected as the target, the value is added as a sub-element under the specified path in the message body when **Body Element Name** is provided, or replaces the entire message body when **Body Element Name** is left empty; targets of type header, parameter, custom variable, or context value are written to the region matching the variable type. Previously only the body sub-element append flow worked; the all-body case and other target types could produce missing or incorrect results; when the body content type was undetermined, the operation could also fail unexpectedly. APNZ-6257 APNZ-6258
...
A new **Cluster Diagnostics** tab has been added to the Elasticsearch Monitor screen, presenting raw diagnostic outputs from a single location in the style of Gateway Diagnostics. The tab includes Cluster Health (`_cluster/health`), Cluster Settings (`_cluster/settings`), Shard List (`_cat/shards`), the ILM policy bound to the connection (`_ilm/policy/{name}`), Pending Tasks, Allocation Explanation, Index Templates, and Active Recovery panels. Each panel's content can be copied to the clipboard as formatted JSON with a single click. [See.](/en/integrations/connection-management/elasticsearch-monitor#cluster-diagnostics) APNZ-6177

<span style={{ display: 'inline-block', backgroundColor: '#bfdbfe', color: '#1e40af', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>CHANGES AND IMPROVEMENTS</span>

- **Authorization Basic Header Support for OAuth/JWT Token Endpoints (RFC 6749 §2.3.1)**

Client credentials (`client_id` and `client_secret`) can now be transmitted via the HTTP Basic Authorization header on both proxy and credential token endpoints. The RFC 6749 §2.3.1 `client_secret_basic` standard is fully supported; clients that default to the Basic header (Spring Security, MSAL, `oauth4webapi`, etc.) can now integrate with Apinizer token endpoints. Sending both the header and body parameters in the same request is forbidden (HTTP 400 `invalid_request`); a malformed Basic header is rejected with HTTP 401 (`invalid_client`); non-Basic schemes (Bearer, etc.) are ignored and the request falls back to body parameters. [See.](/en/develop/policies/oauth2) APNZ-6231

- **MKK SSL Certificate Lifetime Management Improvements**

The lifetime tracking and early warning behavior for the short-lived SSL certificates used in the MKK environment have been improved. As the certificate validity period approaches its end, the notification flow is triggered in time, giving users a predictable renewal window during short rotation cycles. APNZ-6183

- **Wildcard and Case-Sensitivity Hints Added to API Traffic Search Fields**

Explanatory placeholders and inline help text have been added to the search fields on the API Traffic screen, covering the supported wildcard characters and case-sensitivity rules. The intent is to let users quickly correct queries that return no results due to syntax mistakes. APNZ-6236

<span style={{ display: 'inline-block', backgroundColor: '#e65a2a', color: '#ffffff', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>BUG FIXES</span>

- **Customized Error Messages No Longer Return Empty**

Fixed an issue where the response body was empty for end users when a customized error message entry — at the policy or API routing level — had only the customized HTTP status code or customized error code set with the message field left blank. When the customized message is blank, the system-defined default error message is now used as a fallback. The fix is applied consistently across policy errors, API routing errors, and general platform error flows, so affected policies and routings do not need to be recreated. The script policy flow that assigns a custom error message at runtime continues to work as before. APNZ-6209

- **OIDC Policy Response Cookie Inconsistencies Fixed**

The `SameSite` and `Secure` behavior used when writing the OIDC session cookie has been aligned with the reference implementation of the OIDC flow. The bug that caused multiple `Set-Cookie` headers in the same response to overwrite each other has been resolved. Callback requests that arrive without a `state` parameter while state validation is enabled are now rejected as CSRF protection; nonce validation on expired temporary sessions now completes safely; session data is consistently cleared when a session ends. When **Add Token To Cookie** is enabled, separate `HttpOnly` cookies for the access, ID, and refresh tokens are written. APNZ-6230

- **x-www-form-urlencoded Body and List Kept in Two-Way Sync**

For `application/x-www-form-urlencoded` requests, the body content and the form parameter list are now kept in two-way sync across traffic logs and script policies. The body field that was previously left empty in this case now mirrors the form parameters. In script policies, scripts that only write to the body field and scripts that only modify the form parameter list produce the same result; if both sides are modified, the parameter list takes precedence. APNZ-6229

- **Elasticsearch Index Lifecycle Policy Updates on Save & Deploy**

Fixed an issue where changes to phase values in the Edit Index Lifecycle Policy screen were not delivered to Elasticsearch when Save & Deploy was clicked. Phase value changes are now delivered to Elasticsearch on every Save & Deploy, even when the ILM policy name remains the same. APNZ-6185

- **Large Bodies Preserved on API Traffic Record Download**

Fixed an issue where request/response bodies larger than 5 MB were written to the downloaded ZIP as the placeholder "Body size exceeds the display limit..." when an API Traffic record was downloaded or exported. The truncation behavior used to avoid rendering large bodies in the UI is preserved; the download operation now writes the full body content stored in the traffic record into the ZIP. APNZ-6235

- **Missing EXECUTE Permissions Backfilled for Project Owner and API Developer After Upgrade**

In environments upgraded from older versions, the Project Owner role was missing the Execute permission of the Testing category along with several other newly added asset action permissions, and the API Developer role was missing the View and Execute permissions of the Testing category, causing Quick Test and REST API Test calls to fail with HTTP 401. After the upgrade, the missing permissions are filled in automatically without altering existing permissions. APNZ-6234

- **LDAP Users Can See Their Project List**

Fixed an issue where users signing in through LDAP saw an empty project list. The project list query now follows the same authorization rules as standard users for LDAP accounts as well; projects the user can access through their LDAP username or LDAP group memberships are listed even when no direct project membership exists. APNZ-6233

- **Global Policy View Page Now Confirms Delete Action**

Fixed an issue where pressing the Delete button on the View page of a policy under Global Policies removed the policy without any confirmation dialog. The delete action is now guarded by the standard confirmation dialog; when the targeted policy is used by one or more API proxies, the user is warned about the dependencies before the deletion proceeds. The same guarded delete flow applies to all global policy types. APNZ-6149

- **Variable Usage Inside Specify Value for Business Rule Add Action**

In a Business Rule policy, when **Specify Value** is selected on the Target side of an action, the `${...}` Environment Variable and `#{...}` Context Variable placeholders entered in the value of the **Add** action are now resolved at runtime. Previously, these placeholders were treated as raw text and written into the request/response message as-is. APNZ-6226
...
The platform-wide "log disabled fields when blocked" and "log disabled fields on failure" toggles previously located under General Settings have been removed; these are now configured independently for each connector. The API Traffic Log Settings list screen has also been redesigned and now displays connector names in a readable format. Existing platform-wide settings are automatically carried over to the connectors of each environment. [See.](/en/develop/api-proxy-configuration/api-traffic-log-settings) APNZ-6155

- **curl Package Added to Container Images**

Network diagnostics commands (curl) can now be used directly inside API Manager, Gateway Worker, Cache Server, and Integration images. APNZ-6170

- **Project Prefix Preserved in API Proxy and Proxy Group Multi-Path List**

Fixed an issue in the API proxy and proxy group edit screens (when a project-level relative path is defined) where the prefix was lost during save if a value starting with the project relative path was entered into the multi-path list. The path list is now processed by the same rule as the single relative path field; the entered value is always combined with the project prefix when saved. APNZ-6120

<span style={{ display: 'inline-block', backgroundColor: '#e65a2a', color: '#ffffff', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>BUG FIXES</span>

- **Base64 Content-Encoding Inconsistency in Response Compression Pipeline Fixed**

Fixed an issue causing `net::ERR_CONTENT_DECODING_FAILED` on the client side, where the Content-Encoding header was still set to `gzip`/`deflate`/`br`/`zstd` even when the compressed response body was Base64-encoded. Two new options were also added for the routing-level response compression override: `MIRROR_SERVER_RESPONSE` (mirrors the backend response) and `AUTO_BY_CLIENT_ACCEPT_ENCODING` (auto-compresses based on the client's `Accept-Encoding` header). The behavior of existing configurations is preserved. APNZ-6160

- **Token Audit Menu Visibility Fix**

Fixed an issue where the Token Audit menu was shown to users with only system administrator privileges and resulted in an Access Denied error when clicked; menu visibility is now consistent with access control. APNZ-6171

- **Script Policy Custom Error Response Body Preserved**

Fixed an issue where the custom error response body set by a script policy was overwritten by the proxy template during error handling. APNZ-6169

- **Oracle Turkish Character Set Support**

Fixed an issue where database connections to Oracle databases using the Turkish character set (WE8ISO8859P9) could not be established. APNZ-6168

- **API Traffic Body Search Fixed (Elasticsearch 7.9.2)**

Fixed an issue on Elasticsearch 7.9.2 where the request and response body search fields on the API Traffic screen returned no results even though matching records existed. The keyword field on the Advanced tab now also searches within the body. APNZ-6166

- **WSDL Service Port Name Preserved on Endpoint Rewrite**

Fixed an issue where the service port name was incorrectly replaced with the binding name during WSDL endpoint rewriting. The port name is now preserved as defined in the original service contract; SOAP 1.1 and SOAP 1.2 ports are handled independently. APNZ-6164
...
APIOps now supports deploy history listing, detail view, deletion, and rollback operations. Parameters for adding descriptions to deployments and keeping persistent history have also been added. APNZ-6108

- **Schema and Custom Table Name Configuration for Log Tables**

When a database connection is used as a log target, schema prefixes and custom table names can now be configured for each log table. A default schema definition with per-table customization is supported on Oracle, PostgreSQL, MSSQL, MySQL, and DB2. Identifier formats with double quotes, backticks, and brackets are accepted for case-sensitive table names. [See.](/en/integrations/connection-management/database) APNZ-6133

<span style={{ display: 'inline-block', backgroundColor: '#bfdbfe', color: '#1e40af', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>CHANGES AND IMPROVEMENTS</span>

- **Syslog TCP Async Queue and Watchdog**

Syslog TCP connection has been strengthened with an async queue and watchdog mechanism against blocking write issues. Syslog messages sent over TCP protocol are now processed in an asynchronous queue, ensuring reliable delivery regardless of network slowdowns. Queue capacity and write timeout duration are now configurable. APNZ-6111

- **Compress/Decompress Bug Fixes and Improvements**

A data loss bug in large gzip-compressed responses has been fixed. The re-compression issue in the encoding pipeline when request body is sent in binary mode has been resolved. Charset and content encoding override settings per routing can now be configured for both request and response. APNZ-6109, APNZ-6114, APNZ-6117

<span style={{ display: 'inline-block', backgroundColor: '#e65a2a', color: '#ffffff', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>BUG FIXES</span>

- **Cache HTTP/2 Support Reverted to HTTP/1.1**

HTTP/2 support added after the Java 25 migration was not compatible with all customer load balancers; cache access has been reverted to HTTP/1.1. APNZ-6106

- **Oracle and Relational Database Log SQL Fixes**

Errors in the SQL commands used to create API traffic log tables in Oracle and other relational databases have been fixed. APNZ-6112

- **Missing Fields Fixed in API Traffic Log Settings**

An issue where some configuration fields were hidden when a database connection was selected as the API traffic log target has been fixed. Fields for saving traffic logs of undefined or missing API proxies to the target connection, forwarding unsent requests to the failover connection, and configuring API traffic privacy settings are now displayed and work correctly for database connections as well. APNZ-6130
...
<span style={{ display: 'inline-block', backgroundColor: '#86efac', color: '#065f46', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>NEW FEATURE</span>
...
<Note>This feature applies only to HTTP context (MessageContextHttp).</Note>
...
<span style={{ display: 'inline-block', backgroundColor: '#6b7280', color: 'white', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>BUG FIX</span>
...
<Note>This feature applies only to HTTP.</Note>
