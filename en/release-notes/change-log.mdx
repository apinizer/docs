...
- **API Proxy Cache Stores Only Successful Responses**

The API Proxy cache could also store unsuccessful responses (e.g. 400 and other 4xx/5xx), so subsequent requests could be served an erroneous response from cache. Only successful (`HTTP < 400`) responses are now cached; error responses (4xx/5xx) are never cached. [See.](/en/develop/api-proxy-configuration/settings) APNZ-6362

- **Form Consistency on Cache Screens**

The usage, sizing and appearance of toggles, checkboxes and option fields on the cache settings screens were aligned with the platform conventions. APNZ-6362

- **Identity Propagation Control in Authentication Policies**

Across all authentication policies (Basic, Plain Text, Digest, JWT, OAuth2, mTLS, OIDC and JOSE Verification), whether the resolved identity (username/key) is recorded as the request identity is now configurable. When enabled, this identity appears in traffic logs and subsequent policies and scripts; when disabled, the identity is used only for the necessary internal checks and is not persisted as the request identity. Enabled by default; existing behavior is unchanged. APNZ-6363
...
**APNZ-6362** : When the cache settings for an API Method/Endpoint were opened for the first time, the default capacity and time-to-live (TTL) values were empty (the API Proxy cache was not affected). These defaults now also appear on the Method/Endpoint cache screen; previously entered values are preserved.
...
**APNZ-6366** : After upgrading from an earlier version, the request and response flow content selections (parameter, header, body) in the API Traffic Log settings could appear reset. This configuration is now preserved during the upgrade, and on installations that were already affected the related content selections are restored to the enabled state.
