...
The `SameSite` and `Secure` behavior used when writing the OIDC session cookie has been aligned with the reference implementation of the OIDC flow. The bug that caused multiple `Set-Cookie` headers in the same response to overwrite each other has been resolved. Callback requests that arrive without a `state` parameter while state validation is enabled are now rejected as CSRF protection; a null-reference error during nonce validation on expired temporary sessions is prevented; the session cache deletion path is now aligned with the same key format used when writing the session. When **Add Token To Cookie** is enabled, separate `HttpOnly` cookies for the access, ID, and refresh tokens are written. APNZ-6230

- **x-www-form-urlencoded Body and List Kept in Two-Way Sync**

For `application/x-www-form-urlencoded` requests, the body content and the form parameter list are now kept in two-way sync across traffic logs and script policies. At request ingress the body field is filled in RFC 3986 form after form parameters are parsed (previously it was left empty in this case). In script policies, both legacy scripts that only write to the body field and new scripts that only modify the form parameter list produce the same result; if both sides are modified, the parameter list takes precedence and the body is automatically re-serialized accordingly. APNZ-6229

- **Elasticsearch Index Lifecycle Policy Updates on Save & Deploy**

Fixed an issue where changes to phase values in the Edit Index Lifecycle Policy screen were not pushed to Elasticsearch when Save & Deploy was clicked. The full ILM policy payload is now pushed to Elasticsearch on every Save & Deploy, even when the policy name has not changed; previously the push was skipped when the policy name remained the same. APNZ-6185
...
Fixed an issue where pressing the Delete button on the View page of a policy under Global Policies removed the policy without any confirmation dialog. The delete action is now guarded by the standard confirmation dialog; when the targeted policy is used by one or more API proxies, the user is warned about the dependencies before the deletion proceeds. The fix was first prepared on `BasePolicy.deletePolicy` as a foundation and then propagated to all policy view templates, including Authentication and SAML. APNZ-6149

- **Policy Drag-and-Drop Preview No Longer Appears Cropped**

Fixed an issue where the preview shown while dragging a request, response, or error policy card on the API Proxy Flow screen displayed only the top half of the card. Policies are now rendered as a full-size card preview attached to the cursor during drag, and a translucent placeholder is shown at the drop position to make the target slot visually distinct. The fix is applied consistently across the request, response, and error policy lists. APNZ-6056
...
Fixed an issue where database connections to Oracle databases using the Turkish character set (WE8ISO8859P9) could not be established (orai18n component added). APNZ-6168
