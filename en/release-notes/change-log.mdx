...
- **A multiple tag field has been added to the project definition, and projects can now be filtered by tag in the project selection popup.** You can tag projects by unit name, related institution, or any other classification you define; when you have a large number of projects, you can quickly filter them by their tags from the project selection popup in the top menu. [See.](/en/admin/project-management/project-creation) APNZ-4954

- **Export/import support has been added for key store, trust store, and key definitions.** As with certificates, you can export key stores, trust stores, and keys (private, public, and secret) either in their own file format or as a configuration package that can be moved to a different Apinizer environment, and import the same files to migrate them between environments. [See.](/en/admin/secrets-management/keystore-management) APNZ-4959

- **A new APIops endpoint returns the gateway invoke information of an API proxy.** Building the gateway call URL of a proxy previously required combining the gateway base URL, the platform-wide root context and the proxy's relative path from three separate places, which made automated and end-to-end tests error-prone. A single APIops endpoint now returns the gateway base URL, root context, full path, accepted HTTP methods and supported schemes for every environment, so CI/CD and test callers can build the call URL in one hop without hard-coding the prefix. [See.](/api-reference/api-proxies/deployment/get-invoke-info) APNZ-6352

- **The Allowed/Blocked IP List policies can now reference IP Groups through the Management API (APIops).** Previously the APIops policy create/update requests only accepted direct IP addresses and CIDR ranges via `ipList`; IP Groups could not be attached. A new `ipGroupNameList` field lets you attach one or more predefined IP Groups by name, so automation and CI/CD flows can manage IP-group-based allow/block lists the same way the UI does. [See.](/api-reference/policies/policies/policy-white-ip) APNZ-6377
...
**APNZ-6373** : Two APIops issues were fixed. (1) The traffic log settings method-level override endpoints (update and delete) failed for method names that begin with `/`, even when the slash was sent URL-encoded as `%2F`, because the encoded slash was resolved too early on the API Manager side and the method could not be matched. These endpoints now correctly resolve method names that contain a leading slash. (2) Deleting a credential through APIops could return an HTTP 500 error instead of a meaningful status; the endpoint now returns the appropriate status code (for example, a 400 with a clear message when the credential does not exist).

**APNZ-6301** : When attempting to delete a policy in view mode on the Develop tab of a proxy within a Proxy Group, the policy was not removed from the list even after confirming — the entry remained in the list as if it had not been deleted. Policy deletion in view mode now works correctly.

**APNZ-6247** : In SOAP services, the WSA Settings section under Method Settings required every field to be filled in, so it was not possible to save after completing only the fields you needed. You can now fill in the fields you need and save with the others left empty.

**APNZ-5978** : In the API Portal, returning to the full product list from an API Product's detail view required clicking the "APIs" tab again, and that area was not clearly recognizable as a button. A clear "Back to list" navigation has been added to the API Product detail view so you can return to the product list directly.

**APNZ-6295** : When Content-Length was removed from the Hop-By-Hop Header list and instead set through a policy, the request did not reach the backend and the client received an error ("Content-Length header already present"). The gateway now manages framing headers such as Content-Length on its own when building the backend request, so a request whose Content-Length is set by a policy is forwarded to the backend successfully.

**APNZ-4929** : In the analytics charts, selecting the "Today" date range made the time axis start at 03:00 instead of 00:00, because the chart's time buckets were grouped by UTC rather than the server's local time zone. The buckets are now aligned to the local time zone, so the "Today" view starts the day at 00:00 and matches the rest of the selected range.

**APNZ-6342** : Users who had the Testing module's "Manage" permission on a project could not save their work in the Test Console — the save failed with a forbidden error, and only Project Owners, Project Admins, or System Admins were able to save. The cause was that the role definition screen did not offer "Manage" as a selectable action for the Testing category, so the permission could never actually be granted. The Testing category now offers the View, Manage, and Execute actions, so the permission can be assigned and saving in the Test Console works as expected.

**APNZ-6341** : On the role definition screen (User → Roles), the number shown in parentheses next to each permission group did not reflect the number of checked permission boxes in that group; for example, a group with only two boxes could show a higher number (such as "Testing (5)"). The number in parentheses now shows the actual count of selected permissions in that group.

**APNZ-6152** : In the Global Policies section, when attempting to delete a Business Rule or API Based Throttling policy that was in use, the "Confirm delete operation" dialog opened empty — the warning about which proxies use the policy and the list of affected proxies were not shown. The delete confirmation dialog for these two policy types now shows the usage information and affected proxies, just like the other policy types.

**APNZ-6151** : In the Global Policies list screen (Development → Global Settings → Global Policies), the delete action was labeled "Remove" instead of "Delete" for the JOSE Validation, JOSE Implementation, and Client Ban policy types, while the other policy types showed "Delete". The action is now consistently labeled "Delete" across all policy types.

**APNZ-5966** : In the API Monitoring Uptime Monitor, when you narrowed the time range on the chart (the date/time zoom area), the start and end date/time shown at each end of the range disappeared. The start and end date/time of the selected range are now displayed below the chart, so the boundaries of the narrowed range stay visible.
