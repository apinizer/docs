...
**APNZ-6244** : Fixed an issue where memory values configured in GB were shown as MB on the Administration > Gateway Infrastructure > Distributed Cache list. The memory unit label is now consistent with the configured value; existing Cache Server configurations do not need to be changed.
...
- **Deleted IP Whitelist Reference Error Fixed on Allowed IP List Policy Edit Screen**

Fixed an issue where the user was shown an error message on the Allowed IP List global policy add or edit screen when the previously selected IP whitelist had been deleted. The error notification is now suppressed for missing references, allowing the screen to open smoothly and letting the user continue with selecting a new IP whitelist. APNZ-6192
...
- **WS Security To Target Policy "Name" Field Fixed**

An issue where the "Name" field on the WS Security To Target policy creation screen was automatically populated with a default value has been fixed. The field is now empty when creating a new policy and the value entered by the user is preserved. APNZ-6148
