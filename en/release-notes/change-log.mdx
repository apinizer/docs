...
- **Clients Not Making Requests Report Now Reflects Requests Blocked by JOSE Verification**

The Blocked column of the Clients Not Making Requests report now correctly includes requests rejected by the JOSE Verification policy. Previously these requests were classified as Error and were not reflected in the Blocked column, so clients whose requests were only blocked by JOSE Verification could still appear in the "no requests" list. With this fix, requests that fail JOSE verification are consistently counted in the Blocked category across the report. [See.](/en/analytic/analytics-reports/clients-not-making-requests) APNZ-6242
