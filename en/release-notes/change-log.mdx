...
The `${...}` Environment Variable and `#{...}` Context Variable placeholders entered in the **Specify the Value** field under the **Action Definition > Conditions** section of a policy are now resolved at runtime. In addition, the **Injection Target** of a Business Rule **Add** action is now written to the correct region based on the target's type: when a body-type Variable Definition is selected as the target, the value is appended as a sub-element under the specified path of the message body when **Body Element Name** is provided, or replaces the entire body when **Body Element Name** is left empty; targets of type header, parameter, custom variable, or context value are written to the region matching the variable's type. Previously only the body sub-element append path worked, and the all-body case along with other target types could produce missing or incorrect results; the flow also failed with a NullPointerException when the body content type was undetermined. APNZ-6257 APNZ-6258
