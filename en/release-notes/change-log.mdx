...
- Validation display for deployment edit operations on the Kubernetes Resources screen has been improved.
- Search capability has been added for project and proxy selection in Product in the API Portal application.
- User experience has been improved for variable type selection and creation screens when adding Business Rule action types. [See](/en/develop/policies/business-rule)
...
**APNZ-5547**: Fixed the issue where the mandatory Trigger field on the Uptime screen could be saved while left empty.

**APNZ-5545**: Various visual and form issues on the API Integration screen have been fixed.

**APNZ-5446**: Fixed environment name validation and state management errors when saving worker and cache settings on the API Gateway Environments screen.

**APNZ-5733**: Fixed the issue where left menu items and team permissions were not loading for users assigned to the project via group or LDAP group on the API Manager screen.

**APNZ-4475**: Fixed the indexing error that occurred in method details after pagination in the API Proxy Group endpoint list.

**APNZ-5628**: Fixed the issue where .pem extension files were not accepted when PEM format was selected in Elasticsearch certificate settings.

**APNZ-5692**: Fixed the issue where supported formats (HTML/Markdown) in the API Portal Products document page content field were not processed correctly.

**APNZ-5675**: Fixed inconsistent logo sizes in API Portal settings and the intermittent non-display of the right logo.

**APNZ-5214**: Fixed the issue where the API Proxy Group list did not load and an error occurred on the API Product creation screen.
...
**APNZ-5609**: Fixed the issue in API Portal where for organization manager users, a valid email address entered in My Accounts showed as "invalid" and the check mark appeared incorrectly.
...
**APNZ-5756**: The issue where disabled database and other connection definitions remained open in workers has been fixed. Disabled connections are now closed in workers.

**APNZ-4919**: Fixed the issue where mail was still triggered from the Task Flow action even when the mail connection in Connections was disabled or deleted.
