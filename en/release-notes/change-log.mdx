...
- **Empty Environment Filter on the API Proxy Application Logs Tab**

Fixed an issue where the **Environment** filter on the **Application Logs** tab of an API proxy detail screen returned **No results found** when opened, which in turn prevented application logs from being displayed for the proxy. The filter now correctly lists the environments that belong to the project of the proxy being viewed, and the logs of the selected environment are loaded as expected. APNZ-6245
