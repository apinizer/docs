...
- **Request Body Loss and Missing Log Bodies for SOAP Requests via API Proxy Group**

Fixed an issue where SOAP requests sent through an API Proxy Group lost the request body during proxy selection, causing an empty POST to be forwarded to the backend, HTTP 503 with ERR-070, and missing body fields in traffic logs even when proxy and proxy group body logging was enabled. The same request succeeded when sent directly through the API Proxy. SOAP method detection now skips body parsing when the **SOAPAction** header is present; when the body must be read, decompression and charset handling are applied correctly. A guard prevents the body from being cleared when the prepare step runs again after the body is already loaded in the request pipeline. APNZ-6260

- **Proxy Group — API Proxy Log Setting Changes Not Reflected in Proxy Group**

Fixed an issue where changes made to the log settings of an API Proxy connected to an API Proxy Group were not reflected in the associated Proxy Group log settings. Log configuration updates on the API Proxy are now synchronized to the linked Proxy Group. APNZ-6266
...
- **Customized Error Messages Now Return Blank**
