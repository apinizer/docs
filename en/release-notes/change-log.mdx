...
Fixed an issue where the response body was empty for end users when a customized error message entry — at the policy or API routing level — had only `customizedHttpStatusCode` or `customizedErrorCode` set with the message field left blank. When the customized message is blank, the default error message is now used as a fallback. The fix is applied consistently in `PolicyException`, `ApiProxyException`, and `ApinizerException`, so affected policies and routings do not need to be recreated. The script policy flow that fills the message at runtime via `errorMessageTT` continues to work as before. APNZ-6209

- **OIDC Policy Response Cookie Inconsistencies Fixed**

The SameSite/Secure behavior used when writing the OIDC session cookie has been aligned with the proven Groovy script reference. A map-read bug that caused multiple `Set-Cookie` headers in the same response to overwrite each other has been resolved. Callback requests that arrive without a `state` parameter while state validation is enabled are now rejected as CSRF protection; a nonce validation NPE on expired temporary sessions is prevented; the cache delete key has been aligned with the writer's key format. When `Add Token To Cookie` is enabled, separate `HttpOnly` cookies for access/id/refresh tokens are written (matching the Groovy `setAllTokenCookies` parity). APNZ-6230

- **x-www-form-urlencoded Body and List Kept in Two-Way Sync**

For `application/x-www-form-urlencoded` requests, the body field and the parameter list (`formUrlEncodedDataParameterList`) are now kept in two-way sync across traffic logs and script policies. At request ingress the body field is filled in RFC 3986 form after form parameters are parsed (previously it was left empty). In script policies, both legacy scripts that only write to `bodyToTarget` and new scripts that only manipulate the `formUrlEncodedTT` list produce the same result; if both sides are modified, the list takes precedence and the body is automatically re-serialized from it. APNZ-6229
...
Fixed an issue where request/response bodies larger than 5 MB were written to the downloaded ZIP as the placeholder "Body size exceeds the display limit..." when an API Traffic record was downloaded or exported. The truncation behavior used to avoid rendering large bodies in the UI is preserved, while the download path (`exportApiTraffic`) now writes the full body fetched from Elasticsearch into the ZIP. APNZ-6235

- **Missing EXECUTE Permissions Backfilled for Project Owner and API Developer After Upgrade**

In environments upgraded from older versions, Project Owner was missing `TESTING:EXECUTE` and several other newly added asset actions, and API Developer was missing `TESTING:VIEW`/`TESTING:EXECUTE`, causing Quick Test and REST API Test calls to fail with HTTP 401. A new `202605-02` migration idempotently backfills all missing actions across the asset category × action matrix for Project Owner, and the REST API Test / Quick Test permissions for API Developer. APNZ-6234

- **LDAP Users Can See Their Project List**

Fixed an issue where users coming through LDAP saw an empty project list because `ProjectService.findByUser()` filtered only on the standard `projectMember` branches. The query now follows the same pattern as `AuthorityControlService.hasPermissionInProject`: the standard user criteria are `OR`-combined with the LDAP `distinguishedName` and group memberships (`projectLdapMember.teamMemberList.fullName`, `projectLdapMember.teamList.name`). APNZ-6233
