...
A template and form-based message building policy has been added. Two build modes are supported: **Form** mode (simple variable copy) and **Template** mode (dynamic content creation with JEXL expression support). Independent condition, default value, required flag, and error management can be configured for each row. Rows can be reordered via drag-and-drop.
...
- **Certificate export options:** Two different export options have been added to the certificate view screen: "Export as Certificate" (in certificate file format) and "Export" (in JSON/ZIP format for cross-environment migration).
- **Context Variable support in IP Group and Credential:** Context Variable (`#{key}`) and Environment Variable (`${key}`) selection dialogs have been added to IP Group and Credential edit screens. Both variable types can now be used in these fields.
- **Message Builder Policy:** New [Message Builder](/en/develop/policies/message-builder) policy has been added.
...
- ### Dynamic Error Message Customization (APNZ-5754)
You can fully customize messages returned in error cases.

- **Template-based messages:** Write free text; request/error information is included automatically via placeholders (e.g. `#{error.effectiveErrorCode}`, `#{request.uri}`, `#{dateTime.formattedText}`, `#{message.correlationId}`).
...
**Example template:** `Your API call failed. Error Code: #{error.effectiveErrorCode} | Time: #{dateTime.formattedText} | Request ID: #{message.correlationId}`
...
- ### Context Variable Substitution (APNZ-5747)
...
- **Error template placeholders:** `#{error.effectiveErrorCode}`, `#{error.effectiveMessage}`, `#{error.effectiveHttpCode}`, `#{error.customizedMessage}`, etc.
- **Variable types:**
...
You can customize error messages returned in error cases in policies and Business Rule STOP action using template-based messages. In the free-text template, placeholders such as `#{error.effectiveErrorCode}`, `#{request.uri}`, `#{dateTime.formattedText}`, `#{message.correlationId}` automatically include error, request, time, API, and message details. HTTP status code and Content-Type (JSON, XML, plain text) can be set. Usage areas: Global policies, API Proxy, Routing, Business Rule STOP. [See](/en/concepts/core-concepts/what-is-policy#template-based-dynamic-error-messages)
...
* The backward compatibility issue that occurred during the URL path handling improvements in the January 22 version 2026.01.0, specifically in the trailing slash value processing enhancements, has been resolved.
...
<Info>
    ...
    For all innovations, improvements, and changes made in this version → [Review Version 2026.11.01 Detailed Release Notes](/en/release-notes/release-notes-2026)
</Info>
