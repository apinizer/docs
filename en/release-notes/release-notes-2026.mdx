...
When a DB-2-API was duplicated, policies appeared on the duplicate screen but could vanish after deploy. The deploy flow now persistently retains policy definitions on the copied DB-2-API proxy. [See.](/en/develop/api-proxy-creation/api-creator-db-api-creation) APNZ-6358, APNZ-3823
...
- **JSON Schema Validation policy now supports automatic retrieval of schema information from the API's OpenAPI definition file.** [See.](/en/develop/policies/json-schema-validation) APNZ-5913, APNZ-4221
