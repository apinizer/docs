...
AI proxies can route across providers by cost or latency, balance traffic across an equal-priority pool of primary connections, and fail over automatically through a configured backup chain when a primary connection errors out or becomes unavailable. A reusable Failover Template lets a chain be defined once and referenced by name from any number of AI proxies — the template is applied into the proxy's own routing configuration when the proxy is deployed. [See.](/en/ai-gateway/routing-and-failover) APNZ-5128
