...
- ### Outbound MCP Tool and A2A Agent Call Audit
Traffic log records for AI proxies now carry which MCP tools and A2A skills/agents the flow called on the way out, and how many calls it made — an audit trail for outbound tool use without turning on request-level tracing. The name fields hold a distinct, capped list while the count fields hold the raw, uncapped total, so a truncated name list still shows the real call volume. No setting is required; the fields are written whenever outbound MCP/A2A traffic occurs. [See.](/en/operations/administrator-guides/apinizer-log-table-creation-commands) APNZ-6557

- ### Quick Test for AI Gateway Endpoints
The AI proxy's **AI Routing** tab now has an **Endpoints** panel listing the address to call for every environment configured in the installation, each with a copy icon. The **Test** button on a row opens the Test Console pre-filled with a minimal chat request against that address, so trying a proxy no longer requires an external client. Because the test call goes to the real LLM provider and consumes cost and quota, the request body deliberately keeps `max_tokens` low and the screen shows a warning. The panel does not filter rows by whether the proxy is deployed to that environment. [See.](/en/ai-gateway/routing-and-failover#endpoints-panel) APNZ-6584

- ### Target Address Transparency on the LLM Provider Screen
The LLM provider screen now explains how the **Endpoint URL** and **API Version** fields are used, right under the fields: the endpoint takes only the base address, and the request path (`/chat/completions`, `/embeddings`, and so on) is appended automatically per request type; API Version is sent as a version header on Anthropic connections and appended to the request address as a query parameter on Azure OpenAI. The Effective Endpoint Paths panel also gained a read-only **Merged Address Preview** showing the address a call actually goes to; on AWS Bedrock and Google Vertex AI, where the address is built entirely in provider-specific code, an explanatory note appears instead of a preview. [See.](/en/ai-gateway/llm-providers#merged-address-preview) APNZ-6582

- ### Routing Address Column in the AI Traffic List
The provider address a request was actually sent to now has its own column in the AI Traffic list; previously it appeared only in the row detail. The cell is shortened for readability (host plus the last path segment) and the full address is shown on hover. The address is deliberately recorded without its query string, so values such as an API key passed as a query parameter never end up in a traffic record. [See.](/en/ai-gateway/reports) APNZ-6583
...
- ### Access Permission Creating a Duplicate Record on Every Save Fixed
After granting a credential access to an API Proxy or API Proxy Group, staying on the screen and saving again created a new record for the same permission every time, so the same grant appeared multiple times in the ACL report and the audit history. The duplicates did not grant any extra access, but when revoking a permission that had more than one row, deleting only one of them did not close access. Access permissions now take back the id of the saved record, so later saves update the existing record instead of creating a new one. Existing duplicates are consolidated automatically during the upgrade. [See.](/en/admin/identity-management/api-proxy-acl) APNZ-6580

- ### Incorrect Request Address on Azure OpenAI Connections Fixed
The `api-version` parameter that Azure OpenAI expects on every call was never appended to the actual request address. Installations that worked around this by embedding the parameter in the **Endpoint URL** field ended up with a broken address instead: the query string stayed in front of the request path, so an invalid address was sent to the provider. A query string embedded in the endpoint is now relocated after the resolved path, and on Azure OpenAI connections a filled-in **API Version** is appended to the end of the address as an `api-version` parameter (skipped when the address already carries one). Providers that use this field as a version header, such as Anthropic, are unchanged. The same fix applies to the chat, embedding, audio and image endpoints alike. [See.](/en/ai-gateway/llm-providers#query-strings-and-api-versioning) APNZ-6581
...
**IMPORTANT — Action required if you use the SQL/JDBC traffic-log connector:** If your Apinizer installation writes API proxy traffic logs to a relational database (Oracle, MySQL, PostgreSQL, or SQL Server) via the SQL/JDBC traffic-log connector, you must run the log-table update script **before or during** the upgrade to 2026.09.0. This adds the new AI Gateway, MCP Gateway, A2A Gateway, and routing-diagnostics information to the relevant log table. **This step is not required if you use the Elasticsearch or MongoDB traffic-log connector** — those already carry the new information automatically. [Log Table Update Scripts](/en/release-notes/2026-09-0-log-table-alter)
...
A full AI Gateway module has been added alongside the existing API proxy model: create an "AI" type API proxy that fronts LLM services behind a single OpenAI-compatible endpoint, backed by providers for OpenAI, Azure OpenAI, Gemini, Bedrock, Vertex, DeepSeek, Moonshot, and Ollama (plus any other OpenAI-compatible engine you host yourself), with end-to-end streamed request/response support. [See.](/en/ai-gateway/overview) APNZ-5128

- ### Multi-Provider Routing and Automatic Failover
AI proxies can route across providers by cost or latency, balance traffic across an equal-priority pool of primary connections, and fail over automatically through a configured backup chain when a primary connection errors out or becomes unavailable. [See.](/en/ai-gateway/routing-and-failover) APNZ-5128
...
AI proxies can cache LLM responses at two layers — an exact-match cache for identical prompts and a meaning-similarity semantic cache for near-duplicate prompts — cutting repeated cost and latency for common questions before a request ever reaches the provider. [See.](/en/ai-gateway/semantic-cache) APNZ-5128

- ### Token and Cost Budgets with Rate Limiting
Token and USD budgets can be enforced at project, application, credential, and organization scope, with a reservation against the estimated request cost before the call and a reconciliation against real usage once the response completes. Soft caps warn while still letting traffic through; hard caps reject further requests once a quota is exhausted. [See.](/en/ai-gateway/token-quotas) APNZ-5128
...
Apinizer now participates on both sides of MCP (Model Context Protocol): an inbound MCP server exposes any AI proxy as a discoverable, callable tool set for MCP-compatible agents, while an outbound MCP Tool Call policy lets an AI flow call tools on an external MCP server as part of handling a request. [See.](/en/ai-gateway/mcp-gateway) APNZ-5128

- ### A2A Gateway — Inbound and Outbound Agent2Agent Protocol
Apinizer AI Gateway also supports A2A (Agent2Agent) bidirectionally: inbound, an AI proxy can be published as an A2A agent with an agent card and message/task lifecycle, and outbound, an A2A Agent Call policy lets an AI flow delegate work to an external A2A agent. [See.](/en/ai-gateway/a2a-gateway) APNZ-5128
...
Portal Applications can now be viewed and managed by admins directly from the Identity Management menu across all projects, with a dedicated read-only view page and Edit/Delete actions, alongside a new encrypted key-value Metadata tab shared with the self-service Portal UI. AI token/cost budgets can now be scoped down to an individual application's credential without breaking existing organization-wide budgets left unset. A follow-up fix also closed a data-sync bug where legacy Portal Applications missing a portal link were invisible from the Portal UI while still showing in Manager. [See.](/en/admin/identity-management/portal-applications) APNZ-6419

- ### Credential Synchronization
Users from LDAP/Active Directory, Database, and API identity providers can now be automatically synchronized into Apinizer credential records on a schedule or manual trigger, with sync profile, deactivation mode, status/history monitoring, and conflict handling managed from each provider screen. [See.](/en/admin/identity-management/identity-providers/credential-sync) APNZ-6445
...
You can now query through APIops whether an API proxy is deployed in a given environment and whether a redeploy is required — for pre- and post-deploy checks in automation workflows. [See.](/api-reference/api-proxies/deployment/get-deploy-status) APNZ-6349

- ### Route Health Check, Invoke Info, and Backend Error Template Setting via APIops
Through APIops you can verify that a route is being served after deploy, retrieve invoke information (address, path, methods), and read or update the Routing setting for the backend error response template. [See.](/api-reference/api-proxies/deployment/get-route-health), [See.](/api-reference/api-proxies/crud/get-invoke-info), [See.](/api-reference/api-proxies/settings/update-connection-settings) APNZ-6350, APNZ-6352, APNZ-6343

- ### Allowed IP List IP Groups and Query Parameter Encoding via APIops
Through the APIops Management API, Allowed IP List policies can now bind existing IP Group records in addition to direct IP/CIDR entries, and connection settings let you choose how query parameter values are encoded toward the backend (Standard, Protected, Passthrough) — the same Query Parameter Encoding options as in Manager. [See.](/api-reference/policies/policies/policy-white-ip), [See.](/api-reference/api-proxies/settings/update-connection-settings), [See.](/en/develop/routing/http-routing#query-parameter-encoding) APNZ-6377, APNZ-6383

- ### Error Information for Scripts on the Error Pipeline
Script policies on an API Proxy error pipeline can now behave differently based on the error type, message, code, and HTTP status — for example transforming the response on routing failures while leaving policy blocks untouched. [See.](/en/develop/policies/script#error-pipeline-variables) APNZ-6381

- ### Promoting Dependencies in API Promotion
API Promotion transfers now automatically detect creator dependencies of the source API Proxy or Proxy Group along with global policy and certificate-family records, which must be mapped or newly created in the destination environment — so not only the proxy definition but also the dependencies it needs to run can move together. [See.](/en/admin/api-promotion/api-mapping-creation) APNZ-6412

- ### Renewed Reparse Flow — Selective Field Overwrite
The API Proxy reparse wizard has been renewed: after re-fetching the source definition (WSDL/OpenAPI, etc.), you can choose which fields — such as routing address, schema, name, and method list — are overwritten, and the import proceeds accordingly. [See.](/en/develop/api-proxy-creation/multiple-schema-port-wsdl) APNZ-6444
...
- ### Keystore Studio
Apinizer Toolbox gained a new "Keystore Studio" tool — a browser-based counterpart to desktop KeyStore Explorer. It opens and inspects common keystore and certificate formats, generates key pairs, imports certificates, and can save the result directly into the Apinizer secret pool. [See.](/en/admin/secrets-management/keystore-management) APNZ-6308
...
The WSA Settings section in SOAP Method Settings no longer requires every WS-Addressing address field to be filled in before saving; these 8 fields are now optional, while the mandatory selects and the top-level SOAP Action remain required. [See.](/en/develop/api-proxy-configuration/soap-method-configuration) APNZ-6247
...
The API Call policy and the dynamic-key calls in JOSE Validation and JOSE Implementation can now apply configurable retries when an error occurs; attempt count, backoff strategy, and limits for connection failures and selected HTTP status codes are managed from the policy screen. [See.](/en/develop/policies/api-call#retry), [See.](/en/develop/policies/jose-verification), [See.](/en/develop/policies/jose-application-policy) APNZ-6492

- ### PDF Upload and Published-File Deletion on API Product Files
The API Product **Files** tab in the API Portal now supports uploading PDF files, and deleting files from published products works correctly. [See.](/en/api-portal/api-product) APNZ-6502

- ### API Product and Portal Setting Export/Import
API Product and Portal Setting definitions can now be exported and imported through the Export/Import wizard, so portal products and portal settings can be included in the same packaging flow as other assets. [See.](/en/admin/secrets-management/export-import), [See.](/en/api-portal/api-product), [See.](/en/api-portal/api-portal-settings) APNZ-6497

- ### HTTP/2 Support in API Routing
HTTP/2 support was added to API routing. [See.](/en/develop/routing/http-routing#backend-http-version) APNZ-6507

- ### RLCL APIops GET and LIST Endpoints
RLCL (Rate Limit Control List) definitions can now be read one by one and listed by project through APIops. [See.](/api-reference/rlcl/crud/get-rlcl), [See.](/api-reference/rlcl/crud/list-rlcls) APNZ-6516

<span style={{ display: 'inline-block', backgroundColor: '#e65a2a', color: '#ffffff', padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500' }}>BUG FIXES</span>

- ### Fixed Rejected Requests Being Counted Against Quota and Throttling Limits
In Quota and Throttling policies, requests that hit the limit and were rejected could still inflate the usage counter, so raising the limit later did not restore the expected remaining allowance. Whether rejected requests are counted and how the counter behaves when the limit changes can now be configured. [See.](/en/develop/policies/api-based-quota), [See.](/en/develop/policies/api-based-throttling) APNZ-2452

- ### Fixed Allowed IP List Policies Disappearing After Upgrade
When upgrading from 2025.11.0 to 2026.04.x, Allowed IP List policies attached to an API Proxy could disappear. Policies added directly on the proxy are now preserved. [See.](/en/develop/policies/allowed-ip-list) APNZ-6494

- ### Fixed Forgot-Password Emails Using the Wrong Portal Settings in Multi-Portal Environments
In environments with multiple API Portals, the Forgot Password flow could use another portal's email and link settings instead of the portal where the request was made, so reset and sign-in links could open the wrong portal. Each portal now uses its own settings. [See.](/en/api-portal/api-portal-settings), [See.](/en/api-portal/api-multi-portal-usage) APNZ-6409

- ### Fixed Worker Pod Restarts Under High Token/Audit Load
Under heavy token-endpoint traffic, token and audit log writing could overload workers and cause pods to restart. Log writing and error logging were improved so they no longer stall under this load. [See.](/en/concepts/deployment/capacity-planning) APNZ-6501

- ### Added Email Filtering on the Credential List
You could not filter the credential list by email when several API keys shared the same address across different applications. Email-based filtering is now available. [See.](/en/admin/identity-management/credentials) APNZ-6500

- ### Fixed Cache Policy Conditions Not Persisting After Save
Conditions defined on the Cache policy were not saved and disappeared when the policy was reopened. Conditions are now persisted. The same issue was fixed on related AI policies that use a Conditions tab. [See.](/en/develop/policies/cache) APNZ-6504

- ### Fixed API Call Policy Sending Accept-Encoding Unintentionally
The API Call policy could send an Accept-Encoding header even when it was not configured on the Header tab. Compression is now off by default and can be enabled from Settings if needed. [See.](/en/develop/policies/api-call) APNZ-6513

- ### Fixed Accidental Undeploy When Update API Proxy Deployment Fails
When updating a proxy via the Management API with an invalid environment, an error was returned but the proxy was still undeployed. When deployment fails, the previous deploy state is now preserved. [See.](/api-reference/api-proxies/crud/update-api-proxy-from-url) APNZ-6515

- ### Fixed Raw Forwarding of Expect: 100-continue to the Backend
Forwarding the client's Expect Continue expectation to the backend could cause an error, a hang, or a protocol issue. This header is no longer forwarded to the backend. [See.](/en/develop/routing/http-routing#connection-settings-definition) APNZ-6509

- ### Fixed False-Positive Application Log Errors for Successful DB-2-API Stored Procedure Calls
A successful DB-2-API stored procedure call invoked through Try It could still be recorded as an Error in the Application Logs even when the call succeeded. Try It and API Proxy invocations of the same procedure now log consistently. [See.](/en/tutorials/api-creator-scenarios/db2api-stored-procedure-calling) APNZ-6086

- ### Fixed Policies Disappearing After Deploy on Duplicated DB-2-API
When a DB-2-API was duplicated, policies appeared on the duplicate screen but could vanish after deploy. The deploy flow now persistently retains policy definitions on the copied DB-2-API proxy. [See.](/en/develop/api-proxy-creation/api-creator-db-api-creation) APNZ-6358
...
Reparsing a WSDL for a REST-to-SOAP-to-REST proxy compared the wrong pair of definitions and could report every endpoint as changed even when nothing had changed. The preview now compares only the client-facing contract and matches SOAP operations reliably across the transformation. [See.](/en/develop/api-proxy-creation/multiple-schema-port-wsdl) APNZ-5956

- ### Fixed Policy Drag-and-Drop Preview Showing Only Its Top Half
When dragging a request/response/error policy button to reorder it in the API Proxy Flow screen, the drag preview box was visually cut off. The full policy box now appears under the cursor while dragging. [See.](/en/develop/policies/message-flow-and-policy-management) APNZ-6056
...
In API Promotion, after selecting a source API while creating a mapping, returning to the list without clicking Save could still show a mapping record even though the Save button was disabled. Mappings are now persisted only when Save is explicitly clicked. [See.](/en/admin/api-promotion/api-mapping-creation) APNZ-6379

- ### Fixed Inconsistent OAuth/JWT Client ID/Secret Edit and Regenerate Flow
On OAuth/JWT policies, client id and client secret could not be edited on policy group, API Proxy Group, global policy, and proxy-group policy screens the same way as on the API Proxy page, and the Automatic/Manual regenerate flow was missing. All of these screens now match the API Proxy edit and regenerate behavior (Automatic / Manual key generation). [See.](/en/develop/policies/oauth2) APNZ-6398

- ### Fixed JSON→XML and XML→JSON Transformations Being Silently Skipped
In JSON and XML transformation policies, conversion could be skipped silently or leave the body in the wrong format. Transformation now runs correctly, and failures are no longer silent. [See.](/en/develop/policies/json-transformation), [See.](/en/develop/policies/xml-transformation) APNZ-6484
