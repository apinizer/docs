...
- A delete confirmation dialog has been wired into the Script policy and Business Rule policy list views. When a policy that is not associated with any API Proxy is deleted, a simple confirmation message is now shown; when the policy is used by one or more API Proxies, the existing dialog listing the affected proxies continues to appear. APNZ-6205
...
- Fixed an issue where the Injection Target and source variable fields selected in the Action Definition add screen were displayed with swapped labels in the view screen of the related policy. APNZ-6174
- Fixed JSON parse error received on first access to the Client Traffic Metrics report page when the date range was not yet initialized. APNZ-6176
