...
- **KeyStore, TrustStore, and Key (private/public/secret) definitions can now be exported and imported via the APIops Management API.** Cryptographic material is decrypted into a portable export and re-encrypted on import; an `override` option controls overwriting existing definitions. [See.](/api-reference/keys-secrets) APNZ-6338
- **The deploy status of an API Proxy in a specific environment can now be queried via the APIops Management API.** Returns whether the proxy is deployed along with the last deploy time, revision, and a redeploy-required flag. Useful for idempotent CI/CD deployment checks and test preflight. [See.](/api-reference/api-proxies/deployment/get-deploy-status) APNZ-6349
...
- **The Secure Connection (mTLS) settings screen was made more consistent and easier to use.** When the secure connection is enabled, all configuration panels are expanded by default, the supported TLS versions (TLS 1.1, TLS 1.2, TLS 1.3) are preselected, and TLS versions are listed with readable labels. The field order in the view and edit screens is now consistent with each other. APNZ-6386
...
- **APNZ-6379** : When creating a variable mapping in the API Promotion module, the mapping could appear as created if you returned to the list before saving it, even though the Save button was disabled. The mapping is now created only when you explicitly save it; no phantom record is created while the Save button is disabled.

- **APNZ-6353** : When an API Proxy that uses a global policy is promoted to another environment (API Promotion), the global policy is now automatically added to the target project and runs with the current global policy definition in the target environment. Previously the promoted proxy could behave like a local policy using the old embedded definition.

- **APNZ-6069** : API search in the API Portal MCP tools now returns correct results when filtered by name and category, and traffic queries now correctly filter by the selected API. Previously these filters could be ignored, returning all results.

- **APNZ-5931** : The automatic deletion that runs when the retention period defined for Alert records expires now runs at the time specified in the configured time zone (for example 00:00). Previously this deletion task could run in the server's default time zone (for example 03:00) instead of the configured one.

- **APNZ-4929** : When the "Today" date range is selected in analytics charts, the time axis now starts from the beginning of the day (00:00). Previously, on some analytics screens (for example API Traffic By Interval) the time axis could start at 03:00 instead of 00:00.

- **APNZ-6093** : Several gaps in the APIops Management API endpoint and routing update operations have been fixed. The client-to-proxy path and HTTP method of an endpoint can now be changed with the new `newName` and `newHttpMethod` fields; previously these values acted only as the identifier used to locate the endpoint and could not be modified. The load balancing algorithm can now be set together with the backend addresses in a single update through the optional `routingAlgorithm` field. WS-Addressing settings are now validated before saving, returning a clear `400 Bad Request` when a required field is missing while WSA is enabled.

- **APNZ-6417** : When the request limit (throttling) value of a plan defined under Plan & Monetization in the API Portal is updated, the change is now also applied to existing applications using that plan. Previously the updated limit took effect only for newly created applications, while existing applications kept running with the old limit.

- **APNZ-6416** : When the rate limit defined for a plan under Plan & Monetization in the API Portal is exceeded, the rejected request that returns 429 Too Many Requests is now written to the API traffic log as a single record. Previously a single client request that exceeded the limit could appear as two separate (duplicate) records in the traffic log.
