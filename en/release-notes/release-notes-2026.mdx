...
- **Cache Policy:** New standalone policy type for cache read/write/delete operations in the policy chain. LOOKUP/POPULATE/INVALIDATE operations, dynamic TTL, hash key, errorAction (CONTINUE/FAULT). Reduces backend load and improves response times by storing computation results in distributed or local cache. [See details.](/en/develop/policies/cache) APNZ-6246
...
- **Unified cache connection standard across all cache-using screens:** `Cache Connection Timeout` and `Error Action` fields added to 10+ screens (API Call cache, OIDC, JOSE Key Cache, Backend Auth, Throttling, Quota, Client Ban, Token Service, etc.); consistent behavior on cache access errors. APNZ-6250
- **Trace screen policy-specific execution details:** 8 policy types (Cache, API Call, Backend Auth, JOSE Key Fetch, Client Ban, Throttling, Quota, Business Rule) now show an "Execution Details" button with policy-type-specific domain information. Visible on both request-side and response-side policies. [See details.](/en/analytic/tracing) APNZ-6259
...
- **Variable interpolation was not working in condition rules' "Use Custom Value" and action rules' "Specify Value" fields.** Placeholders typed into the plain-text input — `${ENV_VAR}` (environment variables) and `#{customVar}` / `#{request.path}` (context/runtime variables) — were not resolved at runtime and were compared as literal strings. Both syntaxes are now resolved in the condition's second-value field, the action's target-value field, and every policy that reuses these entities. APNZ-6253
- **Dotted flat custom-variable keys such as `#{custom.environment}` were not resolved in routing addresses and template fields.** When a script set a value via `customVariableMap.put("custom.environment", value)`, referencing it as `#{custom.environment}` returned an empty string. A fallback was added to `UtilMessageBuilder` so the resolution now works in routing URLs, condition/action custom values, header/parameter templates, and everywhere else `#{...}` is processed. APNZ-6254
- **SOAP requests sent through an API Proxy Group were losing their body.** During proxy selection inside a proxy group, the method-detection step materialized the request body prematurely and consumed the raw bytes; the subsequent decompression step then found no bytes and cleared the body, so the backend received an empty request, `ERR-070 (503)` was returned to the client, and request/response body fields appeared empty in traffic logs. Method detection is now lazy on the SOAPAction header — when SOAPAction matches, the body is never materialized; in fallback scenarios decompression and charset are applied in the correct order. The REST path is unaffected. APNZ-6260
...
- **mTLS Support in Test Console and API Call:** SSL/TLS settings are now presented through a unified component across all screens. [See.](/en/develop/policies/mTLS) APNZ-5778
