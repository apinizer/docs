---
title: "Tracing and Replay"
description: "You can follow an AI request end-to-end through its trace chain, inspect every step in a timeline view, and replay a past request under the same conditions — with metrics exported to OpenTelemetry and Prometheus for APM integration."
---

import {Card, CardGroup} from '@site/src/components/Card';
import {Steps, Step} from '@site/src/components/Steps';

## Overview

In addition to Live Trace, AI Gateway now builds an **end-to-end chain** of requests (trace group / DAG) in real time, enabling two new capabilities:

- **Timeline View** — Visualize every step of a trace group (the main request plus any sub-calls) in a single screen
- **Replay** — Re-execute a past trace with the same request body

## How the Trace Group (DAG) Is Built

When tracing is enabled (in METRIC/FULL_TRACE/DEBUG mode), a unique **trace ID** is generated at the start of each request and carried through the entire lifecycle of that request:

- The main request and any sub-calls (for example, an agent tool calling another AI proxy, or an A2A task chain) are correlated under the same **trace group ID**
- Each sub-step references a **parent** trace ID, establishing a **parent-child** relationship between steps

:::info

This is especially useful for multi-step scenarios such as **[MCP](/en/ai-gateway/mcp-gateway) tool-call sequences** and **[A2A](/en/ai-gateway/a2a-gateway) agent tasks**, where all sub-operations of a single user request need to be traced as a connected chain.

:::

## Access and Retention

Trace data is sensitive — it can contain full request and response bodies. Viewing it requires the AI analytics view permission, scoped to your project, so only people with legitimate access to a project's AI traffic can inspect it. Trace records are also retained for a limited period and purged automatically rather than kept indefinitely.

## Timeline View

From **AI Reports** → **Live Trace**, when you select a trace group:

- All steps in the group are listed in **chronological order**
- Each step's duration, status (success/error), and associated proxy/policy information are displayed
- The parent-child relationship is shown visually (indented list / connecting lines)

{/* TODO: screenshot — trace group timeline, visualization of connections between steps */}

### A Policy's Internal Steps

AI policies that reach the cache (semantic cache, token rate limit) produce several internal steps within a single execution — the request-phase decision, the stream-end commit, consecutive cache calls. These are **not drawn as separate nodes** in the flow; otherwise the policy would look as if it had been applied several times.

Instead the policy is represented by a single node whose duration is the sum of its steps, and the steps are listed one by one in the window opened by the **Execution Details** button on the top bar — the same behaviour as the API Call policy's detail window on the API proxy trace screen.

## Replay

You can re-execute a trace record with **the same request body** — useful for debugging, regression testing, or reproducing a customer-reported issue.

<Steps>
  <Step title="Find the Trace Record">
    In the **Live Trace** screen, locate the request you want to re-execute.
  </Step>

  <Step title="Click Replay">
    Click the **Replay** button on the relevant trace row.
  </Step>

  <Step title="Watch the New Run">
    Apinizer re-executes the original request body with the same configuration (same model, same policy chain); the result is shown as a new trace record.
  </Step>
</Steps>

:::warning

Replay **resends the original request as-is** — if the original request produces a side effect (for example, a tool call that writes to an external system), that side effect may be triggered again. It is recommended for read-only/analysis-oriented requests.

:::

:::note

Replay uses the existing trace-DAG infrastructure as a **prerequisite** — requests made while trace was disabled cannot be replayed.

:::

## Metrics: OpenTelemetry and Prometheus

Beyond the trace and timeline views in the UI, AI Gateway usage metrics (tokens, cost, latency) are exported using OpenTelemetry's GenAI semantic conventions, so they show up consistently alongside your other service metrics in APM tools such as Dynatrace or Instana. The same metrics are also available on the gateway's existing Prometheus endpoint, tagged by provider, model, project, and status — ready to scrape into Grafana or any Prometheus-compatible monitoring stack.

## Next Steps

<CardGroup cols={2}>
  <Card title="MCP Gateway" icon="plug" href="/en/ai-gateway/mcp-gateway">
    See how tool-call sequences show up in the trace chain
  </Card>
  <Card title="A2A Gateway" icon="share-2" href="/en/ai-gateway/a2a-gateway">
    See how agent tasks are traced across the chain
  </Card>
  <Card title="Reports and Analytics" icon="chart-bar" href="/en/ai-gateway/reports">
    Review performance and latency metrics
  </Card>
  <Card title="Advanced Guardrails" icon="shield" href="/en/ai-gateway/advanced-guardrails">
    Correlate guardrail triggers within the trace chain
  </Card>
</CardGroup>
