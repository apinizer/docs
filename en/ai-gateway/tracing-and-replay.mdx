...
:::warning A Data Retention Profile Can Lock Tracing
When the effective [data retention profile](/en/admin/system-settings/system-settings-and-limits#data-retention-profile) is anything other than **Standard** (**No Payload** or **No Persist**), starting a trace is rejected — not even an authorized user can turn on tracing for that proxy/project/installation. Stopping a trace that is already running is not affected by this restriction.
:::
...
From the AI Gateway's **Trace** tab, when you select a trace group:
...
<Steps>
  <Step title="Find the Trace Record">
    In the AI Gateway's **Trace** tab, locate the request you want to re-execute.
  </Step>
