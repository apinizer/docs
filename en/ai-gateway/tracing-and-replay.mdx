...
Both passes are genuine executions, so each gets **its own node** in the flow — the trace shows what really ran, not a simplified summary. The second node carries a **↺ stream-end** badge, and its detail panel names the phase explicitly, so it reads as "second phase" rather than a duplicated policy.
...
So if a node appears earlier in the trace than in Develop, the request was not mishandled — the gateway corrected the order. The Develop screen warns you when you save a policy list that violates one of these rules, so you can fix the configuration and make both screens agree.
