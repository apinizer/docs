...
The gateway always serves the **last deployed** configuration. Saving the A2A Routing tab does not by itself change what callers see; backend agents, the published skill catalog and the routing mode reach clients only after you **redeploy** the proxy to that environment. When saved-but-undeployed changes exist, a warning banner naming the affected environments appears at the top of the A2A Routing tab.
:::
