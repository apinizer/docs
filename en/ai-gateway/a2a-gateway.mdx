...
The **A2A Gateway** menu item takes you to a card-based list of your A2A Gateways, the same layout the AI Gateway uses for AI proxies — open a card to reach that proxy's routing, policies, and trace/traffic/analytics/tasks tabs.
...
- Inside the **A2A Agent Call** / **A2A Agent Invoke** policy, as a single agent a model delegates a sub-task to as part of any AI proxy's own [tool-call loop](/en/ai-gateway/routing-and-failover#tool-call-loop-agentic-requests) — no A2A Gateway required for that path at all.
