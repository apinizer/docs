...
- Inside the **A2A Agent Call** / **A2A Agent Invoke** policy, as a single agent a model delegates a sub-task to as part of any AI Gateway's own [tool-call loop](/en/ai-gateway/routing-and-failover#tool-call-loop-agentic-requests) — no A2A Gateway required for that path at all.
...
When you want to send a task from Apinizer to an external A2A agent, the component that actually delivers it is the **A2A Agent Call** policy (attached to an AI Gateway). As with Passthrough mode, the target agent is defined **inline, on the policy's own screen**:

<Steps>
  <Step title="Add the policy">
    Attach the **A2A Agent Call** policy to an AI Gateway.
  </Step>
