...
The same hub — **AI Gateway** → **LLM Providers** — has a third tab, **Definitions**, listing the provider type catalog: the built-in and custom definitions every connection above is created from (default endpoint, default auth scheme, and so on). Built-in definitions (OpenAI, Anthropic, Azure, etc.) are editable: change an endpoint or auth default and save, exactly as you would on a custom definition. A definition that ships with Apinizer comes back on the next upgrade if you delete it, so removing one is a reset rather than a permanent deletion. To keep the shipped defaults untouched, use **Duplicate** and edit the copy. Once a custom definition exists, you can view its details, export it, or delete it from here.
