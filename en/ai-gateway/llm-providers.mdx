...
The same hub — **AI Gateway** → **LLM Providers** — has a third tab, **Definitions**, listing the provider type catalog: the built-in and custom definitions every connection above is created from (default endpoint, default auth scheme, and so on). Built-in definitions (OpenAI, Anthropic, Azure, etc.) are editable: change an endpoint or auth default and save, exactly as you would on a custom definition. **Neither deleting nor editing a built-in definition is durable here, and this family behaves differently from the other AI preset catalogs.** A routine version upgrade leaves both your deletions and your edits alone, but Apinizer periodically ships an **LLM catalog-refresh** migration — the provider landscape churns, so the shipped catalog is re-published from time to time (most recently APNZ-6550). A refresh re-inserts every built-in definition you deleted, and resets the display name, default endpoint, default API version, default auth scheme, default auth header name, icon URL and documentation link of every built-in definition you edited back to the shipped defaults. **Duplicate** is therefore the only durable way to customize: copy the shipped definition and edit the copy, which is a custom definition and is never touched by a refresh. Once a custom definition exists, you can view its details, export it, or delete it from here.
