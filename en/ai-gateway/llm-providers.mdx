...
LLM provider connections support full CRUD through the **APIops REST API** as well, no UI required — project-scoped, name-based endpoints:

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/apiops/projects/{projectName}/llm-providers/` | List every connection in the project |
| `GET` | `/apiops/projects/{projectName}/llm-providers/{providerName}/` | Get a single connection by name |
| `POST` / `PUT` | `/apiops/projects/{projectName}/llm-providers/{providerName}/` | Create / update a connection |
| `DELETE` | `/apiops/projects/{projectName}/llm-providers/{providerName}/` | Delete a connection |
| `POST` | `/apiops/projects/{projectName}/llm-providers/{providerName}/test-connection/` | Test the connection Manager-side (in-process) |

The `test-connection/` body is **optional** — omit it to test the stored connection as-is. If you do supply a body, any blank `apiKey`/`apiSecret`/`serviceAccountJson` fields are backfilled from the stored value, but **only when the connection's target hasn't changed** (same endpoint, auth scheme, auth header name, provider type, and deployment type):

:::warning Credential Exfiltration Guard
If the target changed (for example, `endpoint` now points somewhere else), the stored secret is **never** reused against the new target — the request is rejected with `credentialReentryRequired` and you must re-supply the credential. This deliberately prevents a caller from repointing the endpoint at their own server, leaving the secret blank, and exfiltrating the stored credential.
:::

See [API Reference: LLM Providers](/api-reference/llm-providers) for full request/response details.
