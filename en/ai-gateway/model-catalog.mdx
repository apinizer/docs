...
- **Sunset At** — a date you record in the catalog to track when the provider itself plans to retire the model; it's shown on the model's **Lifecycle** tab alongside General and Capacity & Pricing.

Don't treat Sunset At as an automatically-enforced cutoff the way pricing is: as the [Price Change Propagation](#price-change-propagation) section below explains, no catalog field copies this date onto connections, so the date by itself neither rejects a request nor drops a model from any list — retiring a model for real means disabling or removing it on the provider connection that serves it. The field's purpose is operational record-keeping: a place to hold the retirement date the provider has announced.
