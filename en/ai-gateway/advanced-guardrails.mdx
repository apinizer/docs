...
Unlike the personal-data, prompt-guard and data-loss protection presets described earlier on this page, this catalog has no "Add from Preset" button and no catalog-bound rule state — there is nothing here for a policy to bind to or fall out of sync with. Each entry carries its own **Copy** action: click it to copy that category's definition text to your clipboard, then paste it into Off-Topic Protection's Denied Topics list yourself. There is no create, edit, or delete action on this catalog, for the same reason Prompt Protection's signature pack is read-only: rewording a standard taxonomy's categories would break the meaning the standard depends on. If you want your own wording for a topic, write it directly into the Denied Topics list instead of starting from this catalog.
