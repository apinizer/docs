...
### In JSON Bodies: Rules Apply to Values Only {#pii-json-body}

When the request or response body is JSON, masking does not scan it as flat text. It walks the JSON tree and applies the rules to **string values only**. Field names (keys) and JSON syntax — quotes, braces, commas — are never rewritten by a rule.

The practical consequence: a broad or greedily written custom regex cannot corrupt the body's structure, not even inside a very long prompt. The masked body always reaches the provider as valid JSON, and when no rule matches, the body is passed through byte for byte. For non-JSON bodies (plain text, form data) the scan still runs over the whole text as before.

**When a masked body cannot be produced, the request is rejected.** After masking, the rebuilt JSON is validated before it replaces the original body. In the unlikely case that validation fails, Apinizer does not fall back to forwarding the original — that would send the very personal data the rule matched to the provider unmasked — and it does not forward a broken body either. The request is rejected with error **ERR-303** (`AI PII Mask: request rejected — a masked request body could not be produced safely`, HTTP 500). The message and status code can be customized on the **Error Message** tab like any other policy error. No request content is written to the application log in this case.

:::note
To mask by field name, use the **element-name** rule type rather than a regex — that rule matches the key and masks the corresponding **value**.
:::
...
### Beyond the Two Mandates: Presidio-Aligned Coverage {#dlp-presidio-coverage}

The built-in package also ships patterns for a handful of identifiers that fall outside both mandates above but are commonly flagged by general-purpose PII detectors — this part of the package is aligned with [Microsoft Presidio](https://microsoft.github.io/presidio/)'s recognizer catalog (MIT-licensed; Apinizer's regular expressions are its own, adapted with attribution rather than copied verbatim from Presidio):
...
Scanning is bounded — on input length, on how many base64/hex candidates are examined, on total decoded bytes, and on nesting depth (base64 wrapped in base64 is decoded, but the chain does not continue indefinitely). When a ceiling is reached the scan returns what it found so far and records that it was truncated, following the same "never truncated silently" contract as the [encoded-content decoding](#decoding-encoded-content) limits. Only base64 and hex are treated as byte carriers: percent-encoding and `\uXXXX` escapes inflate binary data three- to sixfold and are not a realistic way to smuggle a file through a prompt. A base64 blob *wrapped* in those encodings is still reached, because the text produced by encoded-content decoding is scanned as well.
...
:::warning Enabling this trades time-to-first-token for correctness on streamed responses
While File Signature Detection is on, this policy's streamed responses are **accumulated and scanned as a whole** instead of chunk by chunk — whichever action you chose, **Flag included**. An encoded file signature can straddle any chunk boundary: the magic bytes would land in one chunk and the bytes that confirm them in the next, so a per-chunk scan would reliably find nothing at all. This is the same buffer-and-scan approach described under [Streaming Responses: Buffer-and-Scan](#streaming-buffer-and-scan), and the practical cost is that time-to-first-token is no longer preserved for this policy's streamed responses. Leave the switch off on latency-critical proxies that carry no document-leak exposure.
:::

**How a match shows up**

A file-signature hit is recorded distinctly from a secret-pattern hit, so the two never blur together in the [Guardrail Hits report](/en/ai-gateway/reports#security-and-guardrails-tab): the hit is tagged `file-signature` rather than `secret`, and its label carries the format and the position — for example `File signature (PDF, at_start)`. When more than one signature matched, the label counts the rest. In [tracing](/en/ai-gateway/tracing-and-replay) the recorded entry holds the format, the position, whether the bytes arrived raw or base64/hex-encoded, and how many matches there were — never the scanned content itself.

The three fields are also settable through APIops on the data-loss protection policy JSON: `fileSignatureDetectionEnabled`, `fileSignatureAction` (`BLOCK` or `FLAG`) and `fileSignatureTypes` (an array of the values in the table above, empty or omitted for all thirteen). See [API Reference: AI Gateway](/api-reference/ai-gateway).
...
**Resolving a value manually.** An authorized operator calls `POST /api/ai-pii-unmask` with the project id, the request's trace ID (see [Tracing and Replay](/en/ai-gateway/tracing-and-replay)), the rule's identifier, and the exact placeholder or synthetic text as recorded in the traffic. The same resolution is also available from the Manager UI's **PII Unmask** screen (**AI Governance** menu), which calls this endpoint on your behalf. Access requires the AI Development **Manage** permission on that project — denied by default — and every attempt is written to the audit trail, whether it succeeds, is denied, or the entry has already expired. The recovered value itself is never written to that audit trail, only who asked, when, and the outcome. Both the automatic path above and this manual path write to the **same** audit trail. Since 2026.09.2 each attempt is also written as a **PII Unmask** event to [Audit Records](/en/admin/audit-compliance/audit-records) (and forwarded on the SIEM Audit stream) with its outcome — the recovered value is never part of that record either.
...
## Retry-Storm (Loop) Protection: Counting and Limit Changes

Retry-Storm (Loop) Protection counts repeats of the **exact same request** (same credential, AI Gateway and model, byte-identical prompt) inside a sliding **Window (seconds)**, and fires its **Action** — FLAG (default: traffic passes, the hit is recorded for monitoring) or BLOCK (HTTP 429) — once the count exceeds **Max Repeats**.

Two more settings control how that counter itself behaves:

| Setting | Options | Default | Behavior |
|---|---|---|---|
| **Counting Mode** | All requests / Accepted only | **All requests** | All requests: a request the guardrail itself blocks still counts toward the repeat threshold — today's historical behavior, useful for slowing down an abusive client. Accepted only: only requests that were let through count; a rejected request never inflates the counter. |
| **Limit Change Action** | Clamp to previous limit / Reset / Leave unchanged | **Leave unchanged (NONE)** | What happens to the counter already accumulated in the current window at the moment you change **Max Repeats** — see below. |

**Limit Change Action** only fires at the moment the limit itself changes, never on every request:

- **Clamp to previous limit** — the counter is capped to whatever the limit was *before* the change (`used = min(used, previous limit)`), not the new one. Example: Max Repeats goes from 100 to 300 while the counter — inflated by rejected requests under **All requests** counting — sits at 500; it drops to 100, leaving 200 of the new limit available. Lowering the limit isn't affected by this clamp, so blocking continues exactly as it would without it.
- **Reset** — the counter is zeroed, but **only when the limit is raised**. Apinizer never resets on a lowered limit, since that would silently forgive quota that was actually consumed.
- **Leave unchanged** (default) — the counter is left alone; a limit change only takes effect once the current window ends on its own.

These two settings share their names and semantics with every other rate-limit-style policy in Apinizer — API-based throttling and quota policies expose the same **Counting Mode** / **Limit Change Action** choice.
...
The default behavior when a limit is exceeded is to block the request; instead of blocking, you can opt to **automatically truncate the oldest conversation turns** — see [Per-Request Size Limit](/en/ai-gateway/token-quotas#per-request-size-limit).
...
- **Fake role-marker detection** — patterns embedded in a user message's content that try to trick the model into treating it as a prior system/assistant turn (for example, chat-template control markers or fake instruction markers resembling `###Instruction`)
- **Role-order validation** (optional) — blocks a client from adding more system messages than allowed, or from using a role that isn't permitted
...
A hit against a Denied Topic drawn from this catalog is broken down **by AILuminate category** in the [Guardrail Hits report](/en/ai-gateway/reports#security-and-guardrails-tab) — hit counts per category, a time series, and a project/proxy breakdown — and each recorded [trace](/en/ai-gateway/tracing-and-replay) entry carries the matched category, its similarity score, and which engine reached the verdict (the built-in embedding comparison or the `llm-judge`).
...
- **Deleting a shipped category is permanent.** The fourteen AILuminate entries are seeded once; Mongock deduplicates changesets by id and no seed changeset is `runAlways=true`, so a seed that already ran does not run again. A deleted category stays deleted across version upgrades — it comes back only if a future release ships an explicit re-seed changeset. If you only want to stop using a category without losing it, disable it instead of deleting it.
...
### Finding the Setting on Screen {#finding-external-provider-setting}
...
- If the policy already has an external provider configured, a notice appears at the **top** of the
  Definition tab saying so; clicking it scrolls straight to the section.
- While the engine is **Built-in** (the default), everything below the engine selector is hidden, so
  you only see the selector and a short pointer. The provider selection and its fields appear as soon
  as you change the engine.

**Prerequisite.** The **LLM Provider** list shows the LLM *connections* defined in that project, not
provider *definitions*. If the project has no connections the list comes up empty — create one from
[LLM Providers](/en/ai-gateway/llm-providers) first.
...
When you open a global guardrail policy from an API proxy's **Develop** screen and save it, the change is not written back to the central definition. After you confirm by typing the policy name, a copy that belongs to that proxy alone is created and your edit is applied to the copy; every other proxy keeps using the central definition. To change the central definition itself, edit the policy from the **Global Policies** screen. See [Global Policies](/en/develop/global-configurations/global-policies).
