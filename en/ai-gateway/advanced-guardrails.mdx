...
Five of the seven guardrails above — Personal Data Masking, Prompt Protection, Data-Loss Protection (DLP), Off-Topic Protection, and Retry-Storm (Loop) Protection — aren't limited to AI proxies: you can attach them to an [MCP Gateway](/en/ai-gateway/mcp-gateway) or an [A2A Gateway](/en/ai-gateway/a2a-gateway) as well, and they run through the exact same policy pipeline. What they inspect differs by surface, since neither protocol carries an OpenAI-format chat body:
