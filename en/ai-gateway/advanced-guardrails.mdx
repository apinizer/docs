...
In this release the setting is not yet exposed in the management UI. Enable it through the APIops REST API ([API Reference: AI Gateway](/api-reference/ai-gateway)) or by setting `decodeEncodedContent` to `true` in the policy JSON. UI support is follow-up work.
...
None of the three fields is exposed in the management UI yet. Enable them the same way as `decodeEncodedContent` above, through the APIops REST API ([API Reference: AI Gateway](/api-reference/ai-gateway)) or by setting `includeSystemPrompts` / `includeAssistantPrompts` / `includeToolPrompts` in the policy JSON.
