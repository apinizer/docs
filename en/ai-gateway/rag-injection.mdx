...
:::warning Policy order: RAG runs before the guardrails and the cache
RAG Injection must run **before** [prompt protection, data-loss protection and context integrity](/en/ai-gateway/advanced-guardrails) and before [Semantic Cache](/en/ai-gateway/semantic-cache). Retrieved knowledge-base content has to be inspected too — otherwise a malicious instruction or a secret embedded in a retrieved document reaches the model unchecked, and a cache key that excludes that content produces false cache hits. RAG itself must come **after** [Prompt Template](/en/ai-gateway/prompt-templates) and [Prompt Decorator](/en/ai-gateway/prompt-decorator).
...
RAG **reduces** the risk of hallucination, but it does not guarantee the model stays inside the retrieved context. If you want to actively verify that a response is actually grounded in the injected context, using an external judge, see [Groundedness Protection](/en/ai-gateway/advanced-guardrails#groundedness-protection) — it requires Injection Mode to be set to **Inject via Template** (see [Configuration](#configuration) below).
...
| **Injection Mode** | How the retrieved context is added to the request — see [Injection Mode and Prompt Caching](#injection-mode-and-prompt-caching) below. | No | `Prepend to User Message` |
...
:::warning Prepend to System Message breaks provider prompt caching
...
RAG Injection can be added, updated, or removed on an API proxy through the same generic policy endpoint every policy type shares:
...
Set `policy.type` to `ai-rag-injection`. `vectorDbName`, `embeddingProviderName`, and `knowledgeBaseName` are resolved server-side **by name** — the same name-based-reference rule every APIops resource follows, never by internal ID — so the same request body is portable across environments as long as the referenced connections and knowledge base exist there too.

See [API Reference: RAG Injection Policy](/api-reference/policies/policies/policy-ai-rag-injection) for the full field list, request/response examples, and error codes.
