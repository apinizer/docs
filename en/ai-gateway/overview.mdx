...
## AI Proxy vs. Classic API Proxy

An AI proxy is **the same `ApiProxy` entity** in Apinizer — it goes through the same create/deploy/undeploy flow, the same environment management, the same proxy list, and the same endpoints as a classic API proxy. The difference is how the request is routed to the backend:

- **Classic API proxy** → the `routing` object (address list, circuit breaker, mTLS, proxy server, NTLM, etc.)
- **AI proxy** → the `aiRouting` object (LLM provider connection, model, primary pool, conditional routing, failover chain) — see [Routing and Failover](/en/ai-gateway/routing-and-failover)

The AI proxy runtime **never reads** the classic `routing` object, so settings tied to it don't apply to an AI proxy.

:::warning Behavior Change
The following 12 settings endpoints (`PATCH .../apiProxies/{apiProxyName}/settings/<X>/`) now return **HTTP 400** when called on an AI proxy. Previously they returned HTTP 200 but **had no effect at all** (a silent no-op) — that's now an explicit rejection:
...
On an AI proxy, settings like mTLS, proxy server, and circuit breaker live under `aiRouting` instead — the correct endpoint is `PUT .../apiProxies/{apiProxyName}/ai-routing/`. The `metadata` endpoint's 400 check is conditional: it only triggers when the request body sets `fixSoapApiPortType` (a field meaningful only for classic routing) — a plain metadata update keeps working on an AI proxy.
:::

11 type-agnostic settings endpoints — CORS, cache, idempotency, XML/JSON error templates, forwarded-ip-header, spec-access-type, client-route, keys, maintenance mode, trace, and traffic log — continue to work on AI proxies unchanged.
