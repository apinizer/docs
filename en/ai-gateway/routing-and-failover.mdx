...
Below the decision-chain sections, the AI Routing tab lists an **Endpoints** panel: one row per environment the proxy has been deployed to, showing the data-plane address to call in that environment, with a copy icon next to it. The panel is empty until the proxy is deployed to at least one environment.
