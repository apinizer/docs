...
Letting the client move that attribution would let it be billed and counted at a cheaper model's rate than the deployment that actually served the call. Apinizer therefore does **not** apply **Use the client's model** on such a provider: the effective model stays the configured routing model, the client's `model` field is forwarded exactly as sent, and no catalog check is run against it (the client's model is never called, so an unlisted value is not an error either). **Overwrite with the routing model**, **Reject the request**, and every option on the "no model field" axis behave exactly as described above.
:::
