...
The **MCP Gateway** menu item takes you to a card-based list of your MCP Gateways, the same layout the AI Gateway uses for AI proxies — open a card to reach that proxy's routing, policies, and trace/traffic/analytics tabs.
...
- Inside the **MCP Tool Call** / **MCP Tool Invoke** policy, as a single server a model calls as part of any AI proxy's own [tool-call loop](/en/ai-gateway/routing-and-failover#tool-call-loop-agentic-requests) — no MCP Gateway required for that path at all.
