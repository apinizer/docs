...
- Inside the **MCP Tool Call** / **MCP Tool Invoke** policy, as a single server a model calls as part of any AI Gateway's own [tool-call loop](/en/ai-gateway/routing-and-failover#tool-call-loop-agentic-requests) — no MCP Gateway required for that path at all.
...
When an AI Gateway's model wants to call a tool (tool_use / function calling), the component that actually forwards that call to an external MCP server is the **MCP Tool Call** / **MCP Tool Invoke** policy, attached to the proxy's tool-call step. As with Passthrough, the target server is defined **inline, on the policy's own screen**:

<Steps>
  <Step title="Add the policy">
    Attach the **MCP Tool Call** policy to an AI Gateway's tool-call step.
  </Step>
