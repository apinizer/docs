...
The gateway always serves the **last deployed** configuration. Changing a tool name and saving does not by itself change the `tools/list` response; the new name reaches clients only after you **redeploy** the proxy to that environment. When saved-but-undeployed changes exist, a warning banner appears at the top of the MCP Routing tab.
:::
