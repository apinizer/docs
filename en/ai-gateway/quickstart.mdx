...
## Automating Setup with APIops

You can also create your AI proxy through the **APIops REST API** instead of the UI — handy for calling from a CI/CD pipeline or a script:

```bash
curl -X POST "https://your-apinizer-manager/apiops/projects/{projectName}/apiProxies/ai/{apiProxyName}/" \
  -H "Authorization: Bearer {apiToken}" \
  -H "Content-Type: application/json" \
  -d '{
    "relativePath": "/my-chat-api",
...
`relativePath` is required; `llmProviderName` (resolved by name to an existing LLM provider connection in the project), `modelId`, and `streaming` are optional. See [API Reference: Create AI API Proxy](/api-reference/api-proxies/crud/create-ai-api-proxy) for full details.
