...
**Duplicate is the exception to the table above**: it works on shared and built-in records too. The copy is created inside your own project, and because you now own it, it's no longer "built-in" — you can edit or delete it right away. Duplicating is useful when you want the shipped record to stay as it is and work on a copy — the shipped records themselves are editable too, so it is a choice rather than the only route.
