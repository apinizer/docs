...
:::note Target is enforced for both families; Cache Posture and Custom Error Message stay Request Rate-only
The Target, Cache Posture, and Custom Error Message fields appear on every assignment regardless of family. **Target** narrows enforcement for both families — see [Enforcement](#enforcement-status-shadow-mode) below for how an AI Token Budget assignment's target maps onto its counter. **Cache Posture** and **Custom Error Message** are acted on for **Request Rate** only; an AI Token Budget assignment is enforced through the [Token Rate Limit policy](/en/ai-gateway/token-quotas), which has its own cache-behavior settings and applies the plan's own overflow action when a ceiling is exceeded.
:::
...
- **Cache posture and custom message**, **validity window** (`validFrom`/`validUntil`) — the same [fields](#cache-posture-and-custom-error-message) a single assignment carries.
