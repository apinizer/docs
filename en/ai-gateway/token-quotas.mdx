...
:::info Licensing
Token quota and budget management requires the **AI Gateway** license module. Without that module the quota screens are hidden and the matching API calls are rejected with `403 Forbidden` — this applies to the Management API, the APIops endpoints and the global AI Gateway settings write alike.

Losing the module deletes nothing: existing budget configuration and usage counters are preserved, only changes are blocked. Workers that are already deployed keep enforcing the hard limits they were given.
:::
...
<CardGroup cols={2}>
  <Card title="Credential" icon="key">
    The tightest scope — a limit tied to one Consumer or API client's AI traffic on one API proxy and environment. As of 2026.09, this scope is a typed **AI Token Budget** assignment — the same kind of record managed on the [Limits tab](/en/ai-gateway/limit-plans#assigning-a-limit-to-a-subject-the-limits-tab) — so a row created, edited, or removed here is the same row you would see and change there.
  </Card>
...
:::warning The policy is what enforces the quota
...
### Shared Organizations from the Admin Scope {#shared-organizations}
...
The check runs wherever a budget is saved: the App budget of an API Proxy, the Tenant budget of a project, and the global **Default** and **Anonymous Default** budgets in AI Gateway Settings (both from the Manager UI and through APIops). For the global budgets the scope is every AI proxy in the installation. The one exception is the **organization** budget: its scope spans the credentials of the organization and all of its sub-organizations and is not validated at save time — for organization budgets only the runtime warning applies.
:::
...
Consumer-level AI usage shown here is attributed to the typed AI Token Budget assignment now backing the **Credential** scope above, by that assignment's own subject — see [Quota Usage & Alerts](/en/admin/identity-management/quota-usage-and-alerts) for the account-level view of the same counters, which also tracks usage for a native API client and a portal application under their own AI scopes.
...
<CardGroup cols={2}>
  <Card title="Limit Plans" icon="layers" href="/en/ai-gateway/limit-plans">
    Assign a reusable budget plan to an API client or credential, enforced together with this screen's budgets under the stricter ceiling
  </Card>
