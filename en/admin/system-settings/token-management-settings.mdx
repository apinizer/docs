...
For scope to be returned with a value, the relevant user or credential must have a **role (scope) defined**. For the role definition steps, see the [Token Acquisition with Scope](/en/develop/policies/token-service/token-acquisition-methods) page. If no role is defined, scope is returned empty even in Strict mode — unless **Reject When Principal Has No Roles** is enabled, in which case an HTTP 400 `invalid_scope` error is returned.
