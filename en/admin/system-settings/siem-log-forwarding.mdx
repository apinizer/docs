...
Two further streams appear on the screen with a **Planned** badge and cannot be configured yet: **API Access** and **Portal**. API traffic is not part of the SIEM streams — traffic logs continue to be forwarded by the log connectors of the Gateway environment.
...
| **Session** | sign-out and successful token refresh 1 · successful sign-in 2 · password changed, password change required and password reset requested 3 · password policy violation 4 · failed sign-in and failed token refresh 5 · account locked 7 |
...
- **API Access and Portal streams** are reserved and cannot be configured. API traffic itself is not part of the SIEM streams; traffic logs continue to be forwarded by the log connectors of the Gateway environment.
