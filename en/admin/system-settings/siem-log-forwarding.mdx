...
:::warning The API Access stream is the high-volume one
...
Before you turn either on, set a short retention for the collection — **3 to 7 days** is the recommendation — from the **API Access Events** row on [Application Log Cleanup Tasks](/en/admin/audit-compliance/application-log-cleanup-tasks), and consider [sampling](#sampling) the rule that forwards it.

:::

:::info Blocked and failed requests reach two streams
...
:::info
A **Database** destination's connector uses the same **Operation** field as the Gateway Runtime connectors — only **Insert-Update-Delete** is offered, regardless of database type. See [Adding Log Connectors](/en/admin/server-management/adding-connectors-to-gateway-environments#database-recipient-operation-field) for details.
:::

Use **Send test event** on a saved destination to verify connectivity before you switch a stream over.

<Frame caption="SIEM &amp; Log Forwarding — Destinations tab">
  <img
    src="/images/2026-09-2/siem-log-forwarding-destinations.png"
    alt="SIEM and Log Forwarding Destinations tab with destination cards and Delivery Health summary"
    width="1000"
    height="492"
  />
</Frame>
...
| Column | What it does |
|---|---|
| **Stream** | The stream's name. A stream that exists in the platform but is not configurable yet would carry a *Planned* badge; as of this release every stream is configurable. |
...
<Frame caption="SIEM &amp; Log Forwarding — Streams tab">
  <img
    src="/images/2026-09-2/siem-log-forwarding-streams.png"
    alt="SIEM and Log Forwarding Streams tab with Local Retention, Destinations, and Authority columns"
    width="1000"
    height="688"
  />
</Frame>
...
<Frame caption="SIEM &amp; Log Forwarding — Schema &amp; Samples tab">
  <img
    src="/images/2026-09-2/siem-log-forwarding-schema.png"
    alt="SIEM and Log Forwarding Schema and Samples tab with stream profile picker and sample payload"
    width="1000"
    height="519"
  />
</Frame>
...
| 7 | **Sample percent** is set (API Access only) and this event is not in the sampled share | Not sent — sampled out |
| 8 | **Maximum events per second** has been exhausted for this second | Throttled |
| 9 | — | Sent |
...
| Forward successful API traffic without duplicating the security incidents | API Access rule → **Outcomes**: `SUCCESS` (leave the API Security rule to carry the blocked and failed requests) |
| Keep a per-request stream affordable on a metered SIEM | API Access rule → **Sample percent**: `10` — roughly a tenth of the matching events is forwarded, the collection still stores all of them |
...
**The first layer cannot be turned off.** Token values, passwords and password hashes, `Authorization`, `Cookie` and `Set-Cookie` headers, API-key headers, query strings, certificate and key material, and the privacy salt itself are never part of a v2 payload. They are not in the allowlist, and a second check removes any remaining field whose name resembles a credential. The same check runs over the change list described under [Fields per stream](#fields-per-stream): a changed field whose name resembles a credential carries `"[REDACTED]"` in place of both the old and the new value. The token-signing key of a Gateway Runtime (the RSA private key shown on the JWT Token Validation Keys tab) belongs to this layer as well: it appears neither in the environment's audit record, nor in the change list, nor in a captured request body.
...
| **Truncate data** | The bulk fields inside `data` — `reference`, `message`, `stackTrace`, `changes` — are shortened largest-first until the payload fits, each one marked with a truncation note, and `apinizer.truncated` is set to `true`. The envelope fields are never touched. If truncation is not enough, the event is dropped. |
...
Application stack traces, audit reference objects and audit change lists are the fields that make an event large. All three are opt-in or bounded: the reference object is off by default on the Audit stream, the change list is on by default but is already capped before this policy ever runs — at most 200 entries, each value cut at 512 characters, the whole list at 512 KB — and the stack trace switch on the Application stream can be turned off for a destination that does not need it.

:::info

This is a different control from the **Max Message Size (Bytes)** on a Syslog connection itself. This size policy shortens, drops or hashes the event's content so it fits a target size; the Syslog connection's setting instead keeps all of the content and splits an oversized message into several syslog messages. See [Splitting Oversized Messages](/en/integrations/connection-management/syslog#splitting-oversized-messages) for how the two relate when a destination uses both.
...
|---|---|---|
| **Audit** | Always stored — cannot be turned off | Configurable under the **Audit Records** row on [Application Log Cleanup Tasks](/en/admin/audit-compliance/application-log-cleanup-tasks); off by default (365 days once enabled). |
| **Session** | Always stored — cannot be turned off | Configurable under the **Login Records** row on [Application Log Cleanup Tasks](/en/admin/audit-compliance/application-log-cleanup-tasks); off by default (365 days once enabled). |
...
| **API Access** | Switchable — off by default, asks for confirmation before it is turned on | Configurable under the **API Access Events** row on [Application Log Cleanup Tasks](/en/admin/audit-compliance/application-log-cleanup-tasks); off by default (3 to 7 days recommended, since one row is written per request). |
| **Portal** | Always stored — cannot be turned off | Shares the `audit_event` collection with Audit above, so it shares the same **Audit Records** retention row on [Application Log Cleanup Tasks](/en/admin/audit-compliance/application-log-cleanup-tasks); the sixteen domain events are not covered by the **Portal Login Logs** retention switch described below, which only removes sign-in/sign-out records. |
...
## Session stream — event types and legacy payload {#session-stream-event-types-and-legacy-payload}

The [Streams](#streams) table above names the Session event types; this section describes what each one means and exactly what a **Legacy Raw** destination receives for them. It applies to Management Console sign-ins. Portal sign-ins travel on the same stream but carry their own event names and their own payload shape — see [API Portal Login Logs](/en/api-portal/login-logs#connector-delivery-and-payload) for those.

Whichever authority is in force, a Session record is **always** written to the Apinizer database (`persistent_audit_event` for the Management Console, `portal_login_logs` for a portal) and that cannot be turned off. A destination is an **additional** copy sent alongside the database write, never instead of it.

### Management Console event types
...
Every event also carries `userAgent`, and a failed event carries a machine-readable `reasonCode`. See [Login Records](/en/admin/audit-compliance/login-records) for the full field and reason-code reference.

### Legacy Raw payload

This is what the **Legacy Raw** profile produces; its field list is frozen and does not grow in later versions. Switch the destination to **Apinizer JSON v2** for the canonical envelope, the shared actor/target blocks and field-level privacy — see [Schema v2](#schema-v2).
...
A failed sign-in fills `message` and `errorType`:
...
**Graylog destinations** receive the same event as a GELF message whose short message is `LogLogin`. With **Append to Message** on, the JSON above is written to the GELF `full_message` field; with **Append to Attributes** on, each populated field (`source`, `principal`, `email`, `displayName`, `eventDate`, `eventType`, `remoteAddress`, `message`, `errorType`, `portalId`, `accountId`, `userAgent`, `reasonCode`, `targetProjectId`) is sent as a separate GELF additional field. Fields that are `null` are omitted.

:::info

A **Database** destination writes Session records into the `log_LoginLog` table, which is **not created automatically** — create it before enabling the destination. See [Apinizer Log Table Creation Commands](/en/operations/administrator-guides/apinizer-log-table-creation-commands#login-log) for the CREATE TABLE statement for each supported database (Oracle, MySQL/MariaDB, PostgreSQL, SQL Server).
...
The `data` object carries a fixed set of fields defined in the platform. It cannot be extended, and it can only be narrowed — with a privacy profile, or with the content switches on the rule:

| Switch | Stream(s) | Default | When it is off |
|---|---|---|---|
| *Include the reference object in the payload* | Audit, Portal | Off | `data.reference` is omitted |
| *Include the change list in the payload* | Audit, Portal | On | `data.changes` and `data.changedPaths` are omitted; `data.changesStatus` and `data.changeKinds` are still sent on every entity-change event |
| *Include the stack trace in the payload* | Application | On | `data.stackTrace` is omitted |
...
| `target.type`, `target.id`, `target.name`, `target.projectId`, `target.projectName` | `className`, `objectId`, `objectName`, `projectId`, the project's own name |
...
| `data.changesStatus` | Set on every entity-change event, regardless of the content switches above — `DIFFED` when the fields below reflect an actual comparison against the previous version, `NO_PREVIOUS_SNAPSHOT` when there was nothing to compare against (the object's first save), `SECRET_FIELDS_MAY_HAVE_CHANGED` when the only fields that could have changed are ones that are never written to an audit record at all (see [Hidden Fields in Audit Records](/en/admin/audit-compliance/audit-records#hidden-fields-in-audit-records)), and `TOO_MANY_CHANGES` when the comparison produced more entries than the platform keeps — `data.changedPaths` still lists every changed path in that case; only `data.changes` is capped. |
| `data.changeKinds` | A system-defined summary of what kind of change this was, one or more of `ENDPOINT_ADDED`, `ENDPOINT_REMOVED`, `ENDPOINT_METHOD_CHANGED`, `ENDPOINT_PATH_CHANGED`, `RELATIVE_PATH_CHANGED`, `POLICY_ADDED`, `POLICY_REMOVED`, `POLICY_CHANGED`, `ROUTING_CHANGED`, `CREDENTIAL_ACL_CHANGED`, `ROLE_PERMISSION_CHANGED`, `USER_ROLE_CHANGED`, `SETTING_CHANGED` or `LICENSE_CHANGED`. `RELATIVE_PATH_CHANGED` refers to the API Proxy's Client Route relative path. `LICENSE_CHANGED` tells you the licence key was replaced without carrying the key itself (its old and new values are always `"[REDACTED]"`). Set regardless of the content switches above. |
| `data.changedPaths` | Every path that changed, addressed the same way as `data.changes` below. Sent only when *Include the change list in the payload* is on. |
| `data.changes` | One entry per changed value: `op` (`add`, `remove`, `replace` or `move`), `path`, and — for an entry inside a list, such as one endpoint of an API Proxy — `key`, the id or name that was matched between the two versions so that same endpoint keeps the same `path` across events even if its position in the list changes. `old` and `new` carry the previous and current value; a field whose name resembles a secret (for example a setting named after a password, token or secret, or the licence key) carries `"[REDACTED]"` in both instead — the entry itself, its `op` and its `path` are still sent, so the fact that such a field was added, removed or changed always reaches the SIEM. A value longer than 512 characters is cut, and the list stops at 200 entries (`data.changesStatus` then reads `TOO_MANY_CHANGES`). Sent only when *Include the change list in the payload* is on; it is what replaces the need for the previous-state copy of the whole object mentioned above. |

#### Change list examples

An endpoint added to an API Proxy:

```json
{
  "logType": "AUDIT",
  "schemaVersion": 2,
  "eventId": "68c1f2a180f5de2a3c29e001",
  "timestamp": "2026-09-10T09:12:41.000Z",
  ...
  "target": {
    ...
    "name": "Swagger Petstore Documentation",
    "projectId": "68493b6e80f5de2a3c29c5ee",
    "projectName": "Public APIs"
  },
  "correlationId": "3b1f9a2c-6e4d-4a1b-9c0e-2b3c4d5e6f70",
  ...
  "data": {
    "id": "68c1f2a180f5de2a3c29e001",
    "state": "UPDATED",
    "origin": "MANAGER",
    "changesStatus": "DIFFED",
    "changeKinds": ["ENDPOINT_ADDED"],
    "changedPaths": ["/apiMethodList/8f1e2c3a-9b4d-4e77-8a2f-1a7c8b9d0e11"],
    "changes": [
      {
        "op": "add",
        "path": "/apiMethodList/8f1e2c3a-9b4d-4e77-8a2f-1a7c8b9d0e11",
        "key": "/pet/findByStatus",
        "new": { "name": "/pet/findByStatus", "httpMethod": "GET", "active": true }
      }
    ]
  }
}
...
A General Settings change (a platform-wide setting, so it carries no project):

```json
{
  "logType": "AUDIT",
  "schemaVersion": 2,
  "eventId": "68c1f2a180f5de2a3c29e002",
  "timestamp": "2026-09-10T09:15:03.000Z",
  "event": { "category": "AUDIT", "action": "ENTITY.UPDATED", "outcome": "SUCCESS", "severity": 3 },
  "actor": { "user": "admin", "ip": "10.0.0.7", "userAgent": "Mozilla/5.0", "sessionId": null },
  "target": {
    "type": "GeneralSettings",
    "id": "605c1a2b3c4d5e6f70819203",
    "name": "General Settings",
    "projectId": null,
    "projectName": null
  },
  "correlationId": "9d2e8f1a-4c3b-4a2d-8e1f-6b5c4d3e2f10",
  "source": { "component": "MANAGER", "host": "manager-0", "environmentId": null },
  "apinizer": {
    "version": "2026.09.3",
    "configRevision": 22,
  ...
  "data": {
    "id": "68c1f2a180f5de2a3c29e002",
    "state": "UPDATED",
    "origin": "MANAGER",
    "changesStatus": "DIFFED",
    "changeKinds": ["SETTING_CHANGED"],
    "changedPaths": ["/navbarColor"],
    "changes": [
      { "op": "replace", "path": "/navbarColor", "old": "#245ca4", "new": "#1e3a5f" }
    ]
  }
}
```
...
| `flexString2` with `flexString2Label=projectName` | Project name |
    ...
    "destinations": [
      {
        ...
        "sender": {
          ...
          "idleReconnectCount": 0,
          "reconnectCount": 1,
          "peerClosedAtAcceptCount": 0
        }
      }
    ]
  }
...
Two of the `sender` counters separate a receiving side that closes idle sessions from one that genuinely cannot keep up: `idleReconnectCount` counts sockets the Gateway refreshed before writing because nothing had been sent on them for a while, and `recoveredBatchCount` counts batches that failed once and then succeeded on a fresh socket. A `recoveredBatchCount` that keeps rising while `lossCount` stays at zero means no records are being lost and the receiving side is closing sessions sooner than the Gateway's own threshold — see [Idle Connections and Reconnects](/en/integrations/connection-management/syslog#idle-connections-and-reconnects) on the Syslog connection page. A rising `lossCount` or `rejectedCount` is the different, more serious case.

The remaining two `sender` counters point at the receiving side rather than the network: `reconnectCount` is the total number of fresh TCP connections a destination's sockets have opened since the pod started — a socket's first connection counts too, so a sender that has never dropped still shows one per socket, and anything above that is a drop followed by a reconnect. `peerClosedAtAcceptCount` counts how many of those connections the receiver closed again immediately after accepting them, before a single byte went through. A `peerClosedAtAcceptCount` above zero means the fault sits on the receiving side — a session limit, a source-IP rejection, or a load balancer in between — not in the network path or on the Gateway. When both counters climb together on nearly every batch, the receiver is opening and dropping a session per attempt (a connection storm): each dropped connection still counts as a connection failure, so three in a row move that destination's traffic to its failover connector, the same as any other connection error. See [Receivers That Close the Connection Right After Accepting It](/en/integrations/connection-management/syslog#connect-time-liveness) on the Syslog connection page for the receiver-side signal and the tuning properties.
...
- **A blocked or failed request is counted by two streams.** It produces an API Security event and an API Access event. Set **Outcomes** to `SUCCESS` on the API Access rule when both streams go to the same destination — see [Streams](#streams).
...
- **The change list is on by default on every Audit and Portal destination, including ones created before this version.** *Include the change list in the payload* defaults to on, so an existing v2 destination starts sending `data.changes` and `data.changedPaths` after the upgrade without any action on your side; `data.changesStatus` and `data.changeKinds` are sent on every entity-change event regardless of the switch. A **Legacy Raw** destination is not affected at all — its field list is frozen.
