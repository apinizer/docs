...
For a complete guide on all layers, working principles, priority order and scenario examples of the error message configuration system, see the [Error Message Configuration Guide](/en/concepts/core-concepts/error-message-configuration) page.
...
| 3 | Error Response Rules | Condition-based response rules — return different error responses based on request attributes (path, header, method, etc.) |
| 4 | Per-error field customization | Per-row Customized Message, Customized Error Code, Customized HTTP Status Code fields within the policy |
| 5 | Platform-wide error message customization (This page) | Customizations made from this page |
| 6 (Lowest) | Default error message | Default system error message |
...
| Message Template | A free-text template is written. Placeholders such as `#{error.defaultErrorCode}`, `#{request.uri}`, `#{message.correlationId}` can be used. |
...
This page manages platform-wide error message customization (Priority level 5). Higher-priority methods such as per-error custom message template and field customization within policies are configured in the policy update screen. For details, see [What is Policy? -- Error Message Customization](/en/concepts/core-concepts/what-is-policy#error-message-customization).
...
When the **Customize** button is clicked, the Error Response Rules table appears at the top of the dialog:

| Field | Description |
|-------|-------------|
| Order | Evaluation order of the rule (lower number = higher priority) |
| Name | Descriptive name of the rule |
| Condition | The condition under which the rule applies. Rules without conditions are shown as "Catch-all" |
| Content-Type | Content type of the error response (JSON, XML, Plain Text, etc.) |
| HTTP Status Code | HTTP status code to return when the error occurs |
| Active | Active/inactive status of the rule |

Rules can be reordered using the **up/down arrow** buttons. Each rule can be edited or deleted.
...
When adding a new rule or editing an existing one, the dialog contains:

| Field | Description |
|-------|-------------|
| Rule Name | Descriptive name of the rule |
| Active | Active/inactive status of the rule |
| Condition | Matching condition based on request attributes (optional). If left empty, the rule acts as a catch-all |
| Content-Type | Content type of the response |
| HTTP Status Code | HTTP status code of the response |
| Message Template | Template that generates the error response body. Placeholders and variables can be used |
...
The following placeholders and variables can be used in the message template. The **"Available Variables" panel** on the error message customization dialog displays dynamic variables organized by category, including a **"Date/Time Functions"** section that lists timezone-aware date/time helper functions:

- `#{dtf.format('+03:00')}` — Format with default pattern and timezone
- `#{dtf.format('+03:00','yyyy-MM-dd\'T\'HH:mm:ssXXX')}` — ISO 8601 offset format
- `#{dtf.epochMillis()}` — Unix timestamp in milliseconds

For complete documentation on the Date/Time Helper (`dtf`) function, see [Date/Time Helper (`dtf`)](/en/concepts/core-concepts/dynamic-variables#datetime-helper-dtf) in the Dynamic Variables reference.
...
| `#FAULTCODE#` | Error code (e.g., ERR-002) |
...
| `#{message.correlationId}` | Request tracking ID |
...
Environment variables can be used in `${VARIABLE_NAME}` format. For detailed information about environment variables, see [Environment Variables](/en/develop/environment-variables).

</Accordion>

:::warning[Security Note]

Starting with 2026.09.2, the default error response templates (JSON, XML and plain text) no longer contain the `#RESPONSEFROMAPI#` placeholder; a newly created API Proxy returns only the correlation id, fault code, fault message and status code. API Proxies created earlier keep the template stored on them — review them and remove the placeholder if it is present.

The `#RESPONSEFROMAPI#` placeholder passes the target service's error response through to the consumer unchanged. If the target service's error details include internal system information (e.g., a technical error trace, query content, internal address), that information can leak to the consumer and may be reported as a finding in a penetration test. For example, if the message template defined for **ERR-070 (Target service returned error!)** uses `#RESPONSEFROMAPI#`, it is recommended to remove this placeholder from the template and replace it with a fixed, generic message.

If the target service is session-based, a returned cookie header (e.g., `JSESSIONID`) can also leak to the client; you can drop this header on the response side with a **Remove Header** policy. The durable fix is for the target service to run on a stateless architecture.

**To hide this on an existing API Proxy:** open the proxy's own Error Messages settings, edit the row for **ERR-070 (Target service returned error!)**, remove the `#RESPONSEFROMAPI#` placeholder from the response body template, save, and redeploy the proxy — the change only reaches the gateway after a deploy. This is deliberately not applied automatically: some API consumers may already rely on the content of that field.
