...
Failed login attempts are counted twice: per user, which locks the account once the *user lock* threshold is reached (the lock is shown and can be lifted under **Users → Locked**), and per client address, which turns on the CAPTCHA once the *captcha* threshold is reached. The login screen itself never says which case applies: an unknown user name, a wrong password, a locked or inactive account and a failed LDAP check all receive the same *Username or password is incorrect* answer, and the number of remaining attempts is not displayed. Revealing any of that to a caller who has not signed in would let them discover which accounts exist (user enumeration). The exact reason is still recorded for administrators in **Login Logs**.
