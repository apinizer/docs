...
Failed login attempts are counted per user on the server; the account is locked once the *user lock* threshold is reached, and the lock is shown and can be lifted under **Users → Locked**. The *captcha* threshold is applied by the login page itself: after that many consecutive failures in the same browser page a CAPTCHA appears. It is a client-side deterrent against casual retries, not a security control — the lock is the control. The login screen never says which case applies: an unknown user name, a wrong password, a locked or inactive account and a failed LDAP check all receive the same *Username or password is incorrect* answer, and the number of remaining attempts is not displayed. Revealing any of that to a caller who has not signed in would let them discover which accounts exist (user enumeration). The exact reason is still recorded for administrators in **Login Logs**.
