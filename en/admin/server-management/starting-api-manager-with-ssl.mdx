...
description: "You can configure Apinizer API Manager with SSL/HTTPS in Kubernetes, upload certificates, convert PKCS12 format, and configure deployment settings. You can create certificates and manage them as Kubernetes secrets."
---
...
|----------|----------|
| SSL_KEY_STORE | The path of the key store containing the SSL certificate. In our example, we want Spring Boot to search for it in the classpath. |
| SSL_KEY_STORE_PASSWORD | The password used to access the key store. |
| SSL_KEY_STORE_TYPE | The type of the key store (Usage: PKCS12). |
| SSL_KEY_ALIAS | The alias identifying the key in the key store. |
| SSL_ENABLED | Enables the Spring Boot application to use HTTPS protocol. |
| SERVER_PORT | The port the server listens on. 8443 should be used. |
