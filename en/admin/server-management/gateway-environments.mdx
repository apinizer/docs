...
| **Gateway Server Access URL** | The service access address of nodeport or ingress type required for configurations made in Apinizer Management Console to be loaded to Gateway Pods. Example: `http://worker-http-service.prod.svc.cluster.local:8091` If the **HTTPS Enabled** option is selected, attention should also be paid to the address here. Example: `https://worker-https-service.prod.svc.cluster.local:8443` |
| **Cache Server Access URL** | The service access address of nodeport or ingress type required for configurations made in Apinizer Management Console to be loaded to Cache Pods or for Gateway Pods to access cache pods. Example: `http://cache-http-service.prod.svc.cluster.local:8090` |
...
<AccordionGroup>
  ...
  <Accordion title="Additional Variables" icon="code">
    ...
    **Important Variables:**
    ...
    | `tuneBacklog` | HTTP Worker, HTTP+Websocket, Management API | Backlog value |
    | `tuneRoutingConnectionPoolMaxConnectionPerHost` | HTTP Worker, HTTP+Websocket, Management API | Maximum connections per host |
    | `tuneRoutingConnectionPoolMaxConnectionTotal` | HTTP Worker, HTTP+Websocket, Management API | Total maximum connections |
    ...
    **Recommended Thread Values by CPU:**
    ...
    <Warning>
    **Java Options (-Xmx and -Xms) Warning:**

    When configuring the **Java Options** setting in the Additional Variables field, please note the following warning:

    Please remember that **-Xmx** and **-Xms** settings disable automatic heap sizing.
    ...
    **UseContainerSupport** is enabled by default.
    ...
    The old flags **-XX:{Min|Max}RAMFraction** are now deprecated. There is a new **-XX:MaxRAMPercentage** flag that takes a value between 0.0 and 100.0 and defaults to 25.0. Therefore, if there is a 1 GB memory limit, the JVM heap is limited to ~250 MB by default.
    </Warning>
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Cache Server Additional Variables" icon="gear">
    ...
    **Hazelcast Settings:**
    ...
    | `HAZELCAST_OPERATION_RESPONSEQUEUE_IDLESTRATEGY` | Hazelcast operation response queue idle strategy | - |
