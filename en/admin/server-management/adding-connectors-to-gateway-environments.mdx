...
The detail filter you configure here is an environment-wide rule: it applies to every API Proxy whose traffic reaches this connector. Occasionally a single proxy needs more than the rule allows — a payment integration under investigation, for example, while the environment keeps bodies switched off everywhere else.
...
The option is shown only while **Configure API Traffic Log Setting Detail** is on, since without a detail filter there is nothing to override. It is off by default, so existing environments behave exactly as they did before.
