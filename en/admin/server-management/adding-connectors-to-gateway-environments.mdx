...
### Letting a Single Proxy Skip These Restrictions {#log-detail-exceptions-on-proxy}

The detail filter and size limits you configure here are an environment-wide rule that applies to every API Proxy whose traffic reaches this connector. Occasionally a single proxy needs more than the rule allows — a payment integration under investigation, for example, while the environment keeps bodies switched off everywhere else.

That exception is set on the proxy, not here. A proxy owner turns on **Override environment restrictions**, on the top bar of the proxy's own [Log Settings tab](/en/develop/api-proxy-configuration/api-traffic-log-settings#overriding-environment-restrictions), and only that proxy is exempted from this connector's detail filter and body size limits. No connector-side setting or permission is required, and every other proxy keeps the restrictions unchanged.

The connector dialog stays read-only about these exceptions: while the detail filter is on, it shows **API Proxies overriding this filter: N**. When N is greater than zero, the number is clickable and opens a window listing the name and project of each overriding proxy; nothing can be changed from there, since the exception is still managed from the proxy's own Log Settings tab. This line is not shown for an environment that has not been saved yet.

:::warning What the override never touches
Privacy settings (masking, hashing, deletion, encryption) and the [data retention profile](/en/admin/system-settings/system-settings-and-limits#data-retention-profile) apply unconditionally regardless of this override and are never skipped. While **No Payload** or **No Persist** is in effect, no body is captured at all and the override has nothing to reopen.
:::

No redeploy is required after a proxy switches this on; saving is enough, and the change takes effect on the next request.
