---
title: "Adding Log Connectors"
description: "You can add log connectors to Gateway Runtime environments and configure where API traffic logs are written. You can manage privacy settings and define failover connectors."
---
...
**An image showing how to add log connectors is provided below:**

<img
  src="/images/admin/server-management/konnektor-ekleme.png"
  alt="Adding Log Connectors"
  width="1000"
/>
...
:::note
Deactivating the primary log recipient itself on this screen stops all logging for it, even when a failover connector is defined. Failover only takes over once an **active** recipient becomes unreachable — it does not apply to a recipient you have turned off.
:::

### Database Recipient — Operation Field {#database-recipient-operation-field}

When you add or edit a **Database** log recipient, the **Operation** field always offers a single option, **Insert-Update-Delete**, no matter which database type is selected — including MongoDB and Trino. A recipient that was saved earlier with an older value shows Insert-Update-Delete once you reopen it for editing; the stored value only changes when you press Save. **Stored Procedure** is no longer offered here, because it never wrote any log data; the Stored Procedure option used in Jobs, Alerts, and Task Flow Routing is unaffected.

The same dialog is shared by [SIEM & Log Forwarding](/en/admin/system-settings/siem-log-forwarding) and by a portal's [Login Log settings](/en/api-portal/api-portal-settings#login-log), so this behavior applies there as well.

:::info
**Enable Failover Connector** and the detail filter keys under *Configure API Proxy Traffic Log Setting Detail* now open correctly for a Database recipient; they previously failed to open for this recipient type.
:::
...
| Field | Description |
|---|---|
| **Log disabled fields when request is blocked by policy** | When a request is blocked by a policy (for example, when authorization fails), fields marked as disabled in log settings are still sent to this connector. Useful for identifying the reason for the block and the requester. |
| **Log disabled fields on policy failure or error** | When an error occurs in the request or response pipeline, fields marked as disabled in log settings are still sent to this connector. Useful for analyzing which message content caused the error. |
...
<img
    src="/images/yonetici/hata-durumunda-loglama.png"
...
<img
  src="/images/yonetici/gateway-runtime-konnektor-log-ayarlari-ekrani.png"
...
To limit log size for long content, you can enable **Partial Size** per region and set a character-based limit.
...
### Which Settings Apply to Log Fields {#which-settings-apply-to-log-fields}

The detail filter and size limits you configure here are, by default, an environment-wide rule that applies to every API Proxy whose traffic reaches this connector. The **Which settings apply to log fields** selector, shown right below the detail configuration, decides which side owns that decision:

- **This connector** (default) keeps today's behaviour: the detail filter above applies to every API Proxy that logs through this connector.
- **API Proxy** hands the decision to each proxy instead: every proxy logs according to its own [traffic log settings](/en/develop/api-proxy-configuration/api-traffic-log-settings), and this connector's field filter and body size limits are not applied to any of them.

A connector you have never opened this selector on still opens as **This connector**, so existing environments behave exactly as they did before.

:::warning What the selector never affects
Privacy settings (masking, hashing) and the [data retention profile](/en/admin/system-settings/system-settings-and-limits#data-retention-profile) apply unconditionally under both options and are never skipped. Switching to **API Proxy** does not touch either — it only decides whether the field filter and size limits above are enforced.
:::

No redeploy is required after changing this selection; it takes effect on the next request.

### Allowing an API Proxy to Override These Restrictions {#allowing-an-api-proxy-to-override-these-restrictions}

The **Allow API Proxy override** option is only meaningful — and only shown — while **Which settings apply to log fields** is set to **This connector**. Under **API Proxy**, every proxy already uses its own log settings, so there is nothing left to override.

Occasionally, while the connector precedence is **This connector**, a single proxy needs more than the rule allows — a payment integration under investigation, for example, while the environment keeps bodies switched off everywhere else.

**Allow API Proxy override** opens that door, and only that door. Switch it on, and a proxy that has also switched on *Override environment restrictions* in its own [traffic log settings](/en/develop/api-proxy-configuration/api-traffic-log-settings#overriding-environment-restrictions) is exempted from the detail filter and the size limits on this connector. Every other proxy keeps the restrictions unchanged.

The gate has two sides on purpose. The environment administrator decides **which connectors** may be overridden at all; the proxy owner decides **whether this proxy** is an exception. Neither side can act alone, so a proxy owner cannot lift a corporate restriction single-handedly, and the administrator does not have to edit the environment for every exception.

:::warning What the override never touches
Privacy settings are outside this gate entirely. Masking, hashing, deletion and encryption rules keep running on every record regardless of the override, because they are a data-protection obligation rather than a display preference. The [data retention profile](/en/admin/system-settings/system-settings-and-limits#data-retention-profile) is likewise unaffected: while **No Payload** or **No Persist** is in effect, no body is captured at all and the override has nothing to reopen.
:::

The option is shown only while **Configure API Traffic Log Setting Detail** is on and the connector precedence above is **This connector**, since without a detail filter there is nothing to override. It is off by default, and if you switch the connector to **API Proxy** its saved value is kept but simply not used, so switching back to **This connector** later restores it unchanged.
...
At the time of log record creation, scanning is performed on the "header, parameter, body and request URL" fields of request and response messages. A rule finds data in one of three ways, chosen with **Rule type** when the definition is added: by **element name** (the name of a field, header or query parameter), by a **regular expression**, or by a **built-in personal-data type** (Turkish ID number, IBAN, phone number, e-mail address, credit card number and similar). For a match, the operation (masking, deletion, encryption, hashing) defined on the rule is applied.
...
- The logged request URL: the query string and the request path

The query string is covered by both rule kinds: an element-name rule hides `?id=...` exactly as it hides the `id` entry in the parameter list, and pattern rules match the value itself. The request path is covered **only** by a regular expression or a built-in type, because a path segment has no name to match on — this is how the identifier in `/user/12345678901` is hidden. Values that arrive percent-encoded (`user%40example.com`) are decoded before the pattern is applied; when no rule matches, the URL is logged exactly as it arrived. The proxy's own routing address is deliberately left alone, because traffic lists and reports group by it.
...
<img
  src="/images/yonetici/gateway-runtime-konnektor-tanimlar-ekrani.png"
  alt="API Proxy Traffic Privacy Settings"
  width="1000"
/>
For each definition a **Rule type** is chosen first (element name, regular expression or built-in personal-data type), then the matching input for that type is filled in — an element name, a regular expression, or a personal-data type from the list — together with what will be done with the match (operation information). They are added to the definition list with the Add button. Switching the rule type clears the inputs belonging to the other types, so a definition always carries a single way of matching.
