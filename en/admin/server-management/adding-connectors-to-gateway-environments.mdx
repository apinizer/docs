...
When adding a connector, the following two toggles control whether fields marked as disabled in log settings (such as headers, parameters, and bodies) should still be logged in specific situations:

| Field | Description |
|---|---|
| **Enable logging disabled fields on Block** | When a request is blocked by a policy (for example, when authorization fails), fields marked as disabled in log settings are still sent to this connector. Useful for identifying the reason for the block and the requester. |
| **Enable logging disabled fields on Failure** | When an error occurs in the request or response pipeline, fields marked as disabled in log settings are still sent to this connector. Useful for analyzing which message content caused the error. |

These toggles were previously managed centrally for the entire platform via General Settings; they are now configured independently per connector.
