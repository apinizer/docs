...
This setting is applied whenever a Gateway, Cache, Integration or Portal deployment is created or re-applied — including **Publish** and **Republish** above. A plain pod **Restart**, or editing resources/environment variables through the Manager screens without Republishing, does **not** add it to a deployment that predates this setting. After upgrading, **Republish** each existing environment (and re-apply the Cache/Integration/Portal deployments) so the setting takes effect.
:::
