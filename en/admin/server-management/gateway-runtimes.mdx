...
<CardGroup cols={3}>
  ...
  <Card title="Adding Log Connectors" icon="plug" href="/en/admin/server-management/adding-connectors-to-gateway-environments">
    Add log connectors to Gateway Runtime environments and configure where API traffic logs are written.
  </Card>
...
Changing an existing environment's **Name** immediately refreshes the deploy records of every API Proxy and Proxy Group already deployed to it — no republish is needed for the rename itself, since those records are matched to the environment by its internal id, not by name.
...
For more information about adding log connectors to the Gateway Runtime environment, see [this page](/en/admin/server-management/adding-connectors-to-gateway-environments).
...
:::warning[Upgrading? Republish to pick up the DNS resolver setting]
Every pod Apinizer creates (Gateway, Cache, Integration, Portal, Manager) sets a `ndots=1` DNS resolver option. With the Kubernetes default of `ndots=5`, a name containing fewer than 5 dots — in practice any in-cluster short name — is tried against the resolver search list before being tried as-is. On Alpine-based images the resolver stops searching after the first empty answer, so a dotted-but-incomplete name such as `service.namespace` can fail to resolve while the full name still works. With `ndots=1`, dotted names are looked up directly, so always address in-cluster services by their **full name** (`service.namespace.svc.cluster.local`) rather than a short form.

This setting is applied whenever a Gateway, Cache, Integration or Portal deployment is created or re-applied — including **Publish** and **Republish** above. If it is missing, editing the deployment's resources, security context, health probe, or environment variables also adds it; an operator-set `ndots` value is preserved. A plain pod **Restart**, or **Publish/Unpublish** (replica scaling), does **not** add it. After upgrading, **Republish** each existing environment (and re-apply the Cache/Integration/Portal deployments). If an environment has only been Restarted, apply one of the edits above or Republish it.
:::
...
:::warning[Installations upgraded from before 2026.09.3 — rotate the JWT signing key]
Before 2026.09.3 the token-signing key of a Gateway Runtime (the RSA private key shown on the **JWT Token Validation Keys** tab) was written in **clear text** to the audit record every time the environment was saved, to every SIEM destination receiving audit records and — with request-body auditing enabled — to the captured request body. As of 2026.09.3 the field never appears in any audit output; the fix does not clean up history, though: older audit rows and syslog files still carry the key. On every environment where audit records or SIEM forwarding have been in use, rotate the key from the Gateway Runtimes screen with **Regenerate & Deploy**. Rotation invalidates the tokens that environment's Apinizer Token Service has already signed; clients must obtain new tokens. To purge the older audit rows see [Cleaning the Audit Event Collection](/en/operations/database-growth-management/audit-event-collection-cleanup).
:::
