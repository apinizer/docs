...
<AccordionGroup>
  ...
  <Accordion title="Additional Variables" icon="code">
    ...
    **Important Variables:**
    ...
    | `tuneStreamingReadTimeout` | HTTP Worker, HTTP+Websocket | Client-side read timeout for streaming (SSE/LLM) connections. Since SSE clients only receive data and don't send any, the default `tuneReadTimeout` may prematurely close the connection. This parameter sets a separate timeout for streaming connections (default: 0 ms / unlimited) |
    ...
    **Thread Pool Sizing:**
    ...
    For a detailed sizing guide, see [Gateway Performance Tuning](/en/operations/administrator-guides/gateway-performance-tuning).
    </Warning>

    <Tip>
    For recommended thread values by CPU, tier-based configuration recommendations, and detailed explanations of all tuning parameters, see [Gateway Performance Tuning](/en/operations/administrator-guides/gateway-performance-tuning).
    </Tip>
    ...
    **Java Options Warning:**
    ...
    Apinizer uses an **automatic memory profile system** that selects the appropriate GC algorithm and heap percentage based on the container's memory:

    | Container Memory | GC | Heap Percentage |
    |-----|-----|-----|
    | ≤ 768 MB | Serial GC | 50% |
    | 769 – 1536 MB | G1GC | 60% |
    | > 1536 MB | ZGC | 70% |

    If you specify a GC or heap setting in `JAVA_OPTS`, the automatic values are overridden. **Important:** ZGC uses more native memory than other GCs. If ZGC is auto-selected and `MaxRAMPercentage` exceeds 70%, it is automatically capped at 70% to prevent OOM Kill.

    UseContainerSupport is enabled by default. The old flags -XX:MinRAMFraction and -XX:MaxRAMFraction are deprecated; use -XX:MaxRAMPercentage instead.

    For detailed information, see [JVM Garbage Collector Tuning](/en/operations/administrator-guides/jvm-garbage-collector-tuning).
    </Warning>
  </Accordion>
