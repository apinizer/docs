...
Clicking a metric button in the Diagnostics table opens the detailed content of that data in a window.

- When the window opens, the cursor is placed directly in the content area. This way, **Ctrl+A** (**Cmd+A** on macOS) selects only the content in the window and does not affect the page behind it.
- The **Copy to Clipboard** button at the bottom of the window copies the entire content in a single click. An information message is displayed once the copy is complete.
- The content is read-only; it cannot be modified in the window.
