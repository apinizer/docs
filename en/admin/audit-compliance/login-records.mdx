...
| `CAPTCHA_REQUIRED` | A sign-in attempt with a user name that exists in no store (neither in the database nor in a configured LDAP directory), below that address threshold. The name is not written to the record. |
