...
- Certificate, keystore, JWK, cryptographic key, license, export, and package import/export endpoints are always excluded, regardless of this setting.
- Field names that look like a secret are removed from the captured body, and values that look like a token or a key are replaced with a short, non-reversible hash — the same discipline applied everywhere else in audit records.
