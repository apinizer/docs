...
Every grant and revoke recorded here also writes an **ACL Grant** or **ACL Revoke** record — one per credential-API Proxy pair, not one per method — to [Audit Records](/en/admin/audit-compliance/audit-records#operation-events) and, once a destination is configured, to a SIEM (see [SIEM & Log Forwarding](/en/admin/system-settings/siem-log-forwarding)).
...
| Field | Description |
|---|---|
| **Start Date / End Date** | Filters records by a date range on the operation date. The fields also carry a time: picking a day from the calendar sets the start to 00:00:00 and the end to 23:59:59, and you can change either by hand. |
