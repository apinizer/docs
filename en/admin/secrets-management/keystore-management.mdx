...
description: "You can define and manage keystores used in policies. You can perform keystore addition, deletion, and update operations with JKS, PKCS12, and PEM format support. These operations can only be performed by Admin users."
---
...
Two different export options are available in the Key Store row menu. The same options also apply to Trust Store definitions.

<Steps>
  <Step title="Selecting the Export Type">
    Select the export type from the **⋮** menu of the relevant row in the Key Store list:

    - **Export as KeyStore**: Downloads the key store as a key store file in its own format (JKS, PKCS12, or PEM).
    - **Export**: Downloads the entire Key Store configuration as a compressed (ZIP) package. This package is suitable for importing into another Apinizer environment.
  </Step>
  <Step title="Downloading the File">
    The file is downloaded automatically according to the selected export type. The configuration package can be imported into other environments.
  </Step>
</Steps>

:::info

The configuration package (Export) contains the entire Key Store definition and can be used for migration between different Apinizer environments. The "Export as KeyStore" option, on the other hand, downloads the store directly as a key store file.

:::

## Key Store Import

You can import a Key Store configuration package that was exported from another Apinizer environment.

<Steps>
  <Step title="Opening the Import Window">
    Click the **Import** button at the top of the Key Store list screen.
  </Step>
  <Step title="Uploading and Importing the File">
    In the opened import window, select and upload the previously exported configuration package. The imported Key Store definition is displayed in the list.
  </Step>
</Steps>
