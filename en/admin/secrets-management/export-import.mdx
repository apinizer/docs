...
<AccordionGroup>
  ...
  <Accordion title="AI Object Dependency Detection & Mapping">
    ...
    - An MCP or A2A Outbound Connection surfaces the Credential it authenticates with as a dependency. MCP/A2A gateway proxies are exported as **API Proxies** (`type=MCP` / `type=A2A`) — a PASSTHROUGH proxy surfaces its backend MCP/A2A Outbound Connection as a dependency (name-based `backendMcpRefName` / `backendA2aRefName` sidecars in the package).
