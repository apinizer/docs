...
| API Management | API Specification | No |
...
- **How it differs from the list screen's "Export as File" button**: on these 14 screens, the row menu's secondary **Export as File** item (the raw single-file ZIP capability, kept from before this type existed) exports only the connection's own fields. For LDAP Pool, Mail, Kafka, Syslog and Webhook — the five types that can carry a certificate or key store — this legacy item now bundles that certificate/key store into the same file too, resolved by name the same way the wizard's own dependency step does; on import it is matched to a certificate of the same name on the target, created if none exists, or updated/duplicated there following the same **Replace Existing** / **Import as New** rule as any other certificate reference. The other nine connection types (Database, Elasticsearch, FTP, Graylog, RabbitMQ, ActiveMQ, Logback, SNMP, Linux Script) carry no such material and are unaffected. When you need the certificate to travel **together with** the connection, the row menu's primary **Export** action — this wizard type — does the same thing, with the dependency step to review it first.
...
## The API Specification Object Type in the Wizard

**API Specification** is a main object type under the wizard's **API Management** category. It exports a definition created directly in the [Spec Designer](/en/develop/api-design/spec-designer) — reachable both from the wizard's own type list and, on the Spec Designer list screen itself, from the row menu's **Export** action and the header's **Import** button (which opens the wizard's import step with this type preselected). It carries no dependencies of its own, so export and import both skip straight to the review/options step.

Only **standalone** definitions — the ones you create and edit directly in Spec Designer — appear in this list. A specification generated **from an API Proxy** is not a separate object here: it stays part of that proxy's own package and travels with it, exactly as before. An imported specification is never linked to a proxy, whichever installation it came from.

**Import as New** is refused when the target project already has a specification with the same title — rename it in the Options step, or use **Replace Existing** to update the existing one in place instead.
...
- **Types without dependencies** (Environment Variable, Global Variable, the certificate family, IP Group, Consumer Role, Registered Organization, Limit Plan, PII Preset, Prompt Guard Preset, DLP Preset, Prompt Template, Prompt Decorator, Topic Category, API Specification) skip the extra dependency/mapping step — the object is exported directly as soon as it is selected, and on import the wizard moves straight from the file upload to the options step.
  ...
  <TabItem value="import" label="Import">
    <Steps>
      ...
      <Step title="Upload Package">
        ...
        If you upload a Postman or Testnizer collection `.json` file here by mistake, the wizard now recognizes it and tells you so directly, with a button that takes you to the [Test Console → Test Collections](/en/develop/test-debug-tools/test-console) page, where that file format is imported — instead of the generic "type does not match" message this used to show.
      </Step>
      ...
      <Step title="Options">
        ...
        **Replace Existing** does not normally run this check — it assumes the record it will update already exists. As of Apinizer 2026.09.4, **Environment Variable** is the one exception: when the target project holds no variable of that name yet, Replace Existing checks the same installation-wide scope first. It is refused with the same "name taken elsewhere on this installation" reason if the name is already in use there. Every other Replace Existing target is unaffected — a project that already holds a variable of that name still gets that record updated, exactly as before.

        A **Connection** name collision is checked more narrowly, but **immediately** — as soon as you type or accept the name on this step, instead of only surfacing once you click Import. It is flagged when the name is already used in the target project **or** in the shared (admin) scope; a name used only in some other, unrelated project is not flagged here.

        **Replace Existing**'s target list only offers records the import could actually update. For a Connection, a record that belongs to the shared (admin) scope is shown, labeled with that scope, but greyed out and cannot be selected from a project-scoped import — restart the import with **Admin (global)** as the destination to replace it there instead. For a **Consumer**, the target list is a search box rather than a full dropdown: type part of a username to find it. Importing from **Administration** into the shared (admin) scope searches **every project** at once, and a **Project** filter is offered there to narrow that cross-project search; each result is labeled with its own project, and a consumer belonging to a different project than the one you're importing into is fully **selectable**: pick it, and a notice confirms the record is updated in its own project and stays there — it is not moved into the project you're importing into. Because that target belongs to a different project, the permission check for that pick runs against **the target's own project** (Identity → Export/Import there) rather than the scope you launched the import from; a system administrator acting from the admin scope is exempt from this check. Importing from inside a project instead searches **only that project's** consumers, with no project filter to show — a consumer belonging to another project never appears there — restart the import from Administration to reach it. If the package's own username already matches a consumer visible to the search, that consumer is offered pre-selected. When the matched consumer has since been migrated into an API Client, a notice explains that its username, password, key and organization are kept as they are and only its other fields are updated by the replace.
        ...
        **Replace Existing** keeps the object's identity: the target record's id, the key/secret pair an API Proxy or Proxy Group uses for client authentication, and a Policy Group's runtime key all stay as they are; the source installation's keys carried in the package are never written to the target. Deployment state is kept as well: every environment the target is deployed to is marked **"redeploy required"**, because the content has changed while the Gateway still serves the previous definition — redeploy to that environment to make the change live. An object created with **Import as New** starts with a fresh identity and keys and is deployed nowhere; an import never deploys on its own.
        ...
        A Consumer's **Access Control List** — the API Proxies, Proxy Groups and per-proxy quota/throttling overrides granted to it, see [Consumer Access Control Settings](/en/admin/identity-management/credentials#consumer-access-control-settings) — never travels in a credential package, on the wizard, on a single-object (JSON) import, or over APIops. **Import as New** creates a consumer with no access at all; you grant it separately on the target. **Replace Existing** leaves the target consumer's existing Access Control List exactly as it is — nothing from the package is added, removed, or changed. This is deliberate: access control is specific to the installation and environment it runs in. It does not apply to a whole-project export/import (**Administration → Export Project** / **Import Project**), which still carries access control with it.
      ...
      <Step title="Review & Import">
        ...
        The import is applied against the target as it was when the package was analysed. If the target project
        changed after the analysis in a way that affects the mapping or replace choices (for example an object of the
        same name was created in another tab), the import stops without writing anything, with the warning **"This
        package was analysed before the destination changed"**; upload the package and run the analysis again. Objects
        you create with the **Create** button on the Mapping step do not trigger this warning.

        When you click **Create** on a dependency row, the create may also bring other objects into being in this
        project (for example, creating an MCP gateway's source API Proxy also materialises that proxy's own policy
        group and global policies). The wizard automatically links only the objects this create produced to rows that
        are still unmapped; such a row shows an **Auto-mapped** badge on the Mapping step with a tooltip explaining
        why. It never links a row automatically to an object of the same name that already existed in the target
        project: a row you deliberately left empty stays empty and that reference is dropped on import. Auto-mapped
        rows are shown with an **Auto-mapped** tag in the Review table, and listed separately in the Import
        confirmation dialog and on the import result screen. To override one, simply pick a different target from
        the list — the badge disappears as soon as you do.
...
Over APIops, see [Import Objects](/api-reference/export-import/import-objects) for the `acknowledgeChecksumMismatch` parameter — which accepts all three situations above — and the advisory the response carries back when it was used.
...
A certificate, key store or crypto key that had never been assigned to any environment used to fail this way silently: the export produced a blank entry for it, and the problem only surfaced later, on the importing installation, as an unreadable entry. That case is now exported correctly. More generally, when an object still cannot be serialized, export no longer completes with a blank entry in its place — it stops immediately and names the object's type and name, so you can fix it on the source installation before the package ever leaves it.

### Packages That Carry Secret Material

Some object types travel with their secret values readable inside the package — an AI provider's API key, a key store's password, or a certificate's private key, for example. An Uptime Monitor's, Anomaly Detector's, Task Flow Manager's or Report Generator's package carries one more such value: when a notification connector inside it has its own privacy setting turned on, the salt behind that connector's masking/hashing rules travels in the package as plain text too — the same way a SIEM destination's privacy salt does (see [SIEM Settings](#siem-settings) below). When such an object is included, the export wizard now warns you before you download the file and names the objects concerned; the warning never shows the values themselves.

Treat such a file as you would the secrets in it, and where it has to leave a trusted environment, export it with a passphrase — see [Package Encryption](#package-encryption) above.

#### Very Old Packages Can No Longer Be Imported

Types such as an LLM Provider, a Vector DB and an OpenTelemetry (OTLP) connection write their secret
values into the package in **readable** form, and the import encrypts them again on the destination.
Much older packages, produced before that behaviour, carry the same fields **already encrypted**.
Importing one of those encrypts the value a second time and leaves it **irreversibly unusable** — and
the import still reports success, so the problem only surfaces the first time the provider is called.

From this release such a package is **refused** instead of being silently corrupted: the import stops
and you get an error telling you to export the package again from the source installation with a
current Apinizer version. Re-producing the package on the source installation is the fix; there is
nothing that can be repaired in the old file you already hold.

Connection definitions (mail, LDAP, database, SNMP, Elasticsearch, FTP, RabbitMQ, ActiveMQ and
Linux Script) and the
identity services are **outside** this rule: their secret values travel encrypted in the package and
are stored on the destination as they are. If a package of one of those types does carry a secret in
plain text — a hand-written GitOps file, say, or a package produced from a record that a much older
version stored in plain text — the import now stores that value **encrypted** on the destination. A
data repair that runs during the upgrade encrypts values that were already stored in plain text, and
leaves values that are already encrypted untouched.
...
<AccordionGroup>
  ...
  <Accordion title="Global Policy / Environment Variable / Credential Key Mapping">
    - **Replace Existing** on a Global Policy or Policy Group updates more than that object's own record: every API Proxy, Proxy Group, and Policy Group that already carries an embedded copy of it is updated with the new content too, and every environment where one of those is deployed is marked **redeploy required** — the running Gateway keeps serving the previous version until you redeploy it there; the import itself never deploys anything on its own. **Import as New** creates a separate policy or group and leaves every proxy, proxy group, and policy group already using the original one untouched — and so does the classic (non-wizard) Import screen's own "replace" option for a Global Policy or Policy Group: it never replaces the target in place, it always appends an `-imported` suffix and creates a new record, so no proxy, proxy group, or policy group already using the original is ever touched.
  ...
  <Accordion title="Unresolved References Reported on Every Surface">
    - As of Apinizer 2026.09.4, dropping and reporting an unresolved reference is no longer specific to this wizard. A legacy single-object JSON import, an APIops import, and the whole-project **Import Project** screen (see [Project Creation](/en/admin/project-management/project-creation#how-a-project-import-proceeds)) now drop the same kind of reference too — and each names it in its own import result, instead of clearing it silently. The policy Import dialog on an API Proxy/Proxy Group is one such legacy path: it imports an exported policy as a local policy. A reference cleared from that incoming policy is now named in its import result as well.
    - Reference kinds newly covered by this reporting: an Identity Service's own connection pool (LDAP or Database); an OIDC or API Identity Service's own certificate; an API Proxy's or Proxy Group's settings template; a DB-to-API API Creator's database connection; a Knowledge Base's Vector DB and embedding provider; a Custom Query's, Custom Query Filter's, Report Configuration's, or Anomaly Detector's API Proxy references; an Uptime Monitor's notification connections and test environments; and a Task Flow's connections.
    - With **Replace Existing**, a single-valued reference keeps the target's own current value when that value still resolves — an Identity Service's connection pool or certificate, an API Proxy/Proxy Group's settings template, a Knowledge Base's Vector DB or embedding provider, or an Uptime Monitor's test environment. It is never overwritten by an unresolved package value, and keeping it is reported too, the same way a kept certificate-family reference is (see Global Policy / Environment Variable / Credential Key Mapping above). A list-type reference has no such target fallback — a Custom Query's, Custom Query Filter's, Report Configuration's, or Anomaly Detector's API Proxy list, or an Uptime Monitor's or Task Flow's connections. Only the entries that still resolve are kept; the rest are dropped and reported.
  </Accordion>
  ...
  <Accordion title="AI Object Dependency Detection & Mapping">
    - An LLM Provider's dependency step does not package its provider type: the provider type catalog is a read-only list shipped with Apinizer, so on import the connection is re-linked automatically to the target's catalog entry by the provider's own identifying code, and a custom provider (`providerType: OTHER`) carries its wire protocol and paths on the connection itself. The same step surfaces any custom model catalog entries defined under that provider — these travel in the package as dependencies and are created automatically in the target on import; built-in catalog models aren't included, since the target installation already recognizes them out of the box. The LLM Provider itself always carries its own secret (its API key/secret), so it is always packaged as a secret dependency.
  ...
  <Accordion title="MCP Source API Proxy Dependency & Mapping">
...
An Uptime Monitor's or Anomaly Detector's notification connector — the mail or webhook connection its alerts go out on — is packaged as a dependency the same way any other connector-based reference is, and so is its failover connection, when failover is enabled for it, exactly like a SIEM destination's failover connector (see [SIEM Settings](#siem-settings) below). If that connection, or its failover counterpart, cannot be resolved on the target, it is cleared from the monitor or detector. This is reported, rather than left as a raw id that resolves to nothing.

An **Uptime Monitor**'s test can also name one or more environments to run in. Each belongs to the source installation, the same way an Anomaly Detector's does (below). One that doesn't resolve on the target is dropped from the monitor and reported, not carried over as a raw id.

An **Anomaly Detector**'s environment belongs to the source installation, so it is never carried over as a raw reference: if it doesn't resolve to an environment on the target, it is dropped and reported the same way an unresolved certificate-family environment is, and the imported detector arrives with no environment selected — it cannot be saved or activated on the target's [Anomaly Detector](/en/monitor/anomaly-detector) screen until you pick a valid one there. With **Replace Existing**, the target record's own, already-valid environment is kept as it is; only **Import as New** is affected. As of Apinizer 2026.09.4, a target environment that does resolve, but whose Elasticsearch connection is not defined or is disabled there, is kept rather than dropped. A real environment that still needs its Elasticsearch connection enabled is a better starting point than no environment at all. This case is reported as a warning too, naming the detector — complete or enable that connection before activating it.
...
:::warning[Import never makes anything effective]
...
<AccordionGroup>
  ...
  <Accordion title="Limit Assignment">
  ...
  <Accordion title="Access Entitlement">
...
## API Product Subscription Plans {#api-product-subscription-plans}
...
- A plan's packaged reference to its own limit profile is only kept when that profile is a genuine, published, unmanaged entry of the matching family in the target catalog (see [Profiles Owned by a Subscription Plan](/en/ai-gateway/limit-plans#profiles-owned-by-a-subscription-plan)). A reference to a profile that turns out to be owned by another plan, unpublished, of the wrong family, or simply missing on the target is dropped and reported the same way; the plan keeps the rate limit and quota numbers from the package and gets its own newly generated profile from them, exactly as a plan saved directly on the target would.

Cloning a whole portal (**Portal Settings → Clone/Transfer**) applies the same rule with one difference: because a portal package's applications and subscriptions reference its plans, an unusable plan **refuses the whole clone** before the first record is written, rather than being skipped. Nothing is created, and the message names the plan to correct. The clone result also lists every plan whose packaged limit-profile reference could not be kept for the reasons above, alongside the field and the reason, the same way the wizard's import report does.

## SIEM Settings {#siem-settings}
...
- **What travels**: the whole [SIEM & Log Forwarding](/en/admin/system-settings/siem-log-forwarding) configuration exports as a single file — every destination (its connector reference, payload profile, size policy, privacy profile and stream rules) and the two local-storage switches. A destination's connector — and its failover connector, when it has one — is a dependency exactly like any other connector-based type: package it along, or map it to an existing connection configuration by name on import.
- **What never travels**: a stream's authority (Legacy or Active) is never part of the package and is never changed by an import — only the [SIEM & Log Forwarding](/en/admin/system-settings/siem-log-forwarding) page itself, on the target, can cut a stream over. The record's server-managed fields (its revision counter, its last-modified stamp) are likewise never carried; the target keeps its own.
...
This is additive: the existing `/export-import/{type}/export|import` endpoints keep working exactly as before, for pipelines that don't need the drift check. Full request/response shapes, error keys and a worked curl example are in [Transfer API v2](/api-reference/transfer-v2).
...
- [SIEM & Log Forwarding](/en/admin/system-settings/siem-log-forwarding)
- [Connection Management](/en/integrations/connection-management/overview)
...
- [Transfer API v2](/api-reference/transfer-v2) - The versioned, preview-then-apply CI/CD surface built on this same engine
