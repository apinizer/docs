...
**Connection** is a main object type of its own, under the wizard's **Connections** category — separate from the lightweight single-file **Import** dialog every connection type's own list screen carries (see [Import Advisories Outside the Wizard](#import-advisories-outside-the-wizard) below). It covers: Database Pool, Kafka, Elasticsearch, Mail, FTP, Graylog, Syslog, RabbitMQ, ActiveMQ, Webhook, Logback, SNMP, LDAP Pool and Linux Script.
...
- **How it differs from the list screen's legacy Export button**: that button (and the simple **Import** dialog described below) carries no dependencies — it exports only the connection's own fields, without a certificate or key store. When you need the certificate to travel **together with** the connection (for example an LDAP Pool secured with mTLS), use the wizard's **Connection** type instead of that legacy button.
  ...
  <TabItem value="import" label="Import">
    <Steps>
      ...
      <Step title="Review & Import">
        ...
        you create with the **Create** button on the Mapping step do not trigger this warning: the wizard refreshes the
        analysis in the background after every create and automatically binds rows that are still unmapped to the
        same-named objects that arrived with the created object.
...
Every Connection type — Kafka, Webhook, Email, Elasticsearch, database, and the rest listed under [Connection Management](/en/integrations/connection-management/overview) — carries its own lightweight, single-file **Import** dialog directly on its own list screen (see each connection type's own page, for example [Kafka](/en/integrations/connection-management/kafka)). It applies the uploaded package straight away, without this wizard's dependency or mapping steps, and keeps working this way regardless of the wizard's own **Connection** type described above. OTLP is the one connection kind that has no wizard-catalog counterpart at all — it only ever exports and imports through this dialog. An encrypted package can be imported there too: when the uploaded file is passphrase-protected, the dialog asks for the passphrase after the first **Import** attempt — enter the passphrase the package was exported with and press **Import** again. A wrong passphrase is reported inline and can be corrected without re-selecting the file.
