...
:::info
An MCP server or A2A agent is no longer a separate exportable object type. It travels inline — inside the MCP Tool Call / MCP Tool Invoke (or A2A Agent Call / A2A Agent Invoke) policy that embeds it, and inside a Passthrough MCP/A2A Gateway's own routing configuration — as part of the **API Proxy** package that contains it. A package from an older release that still carries a standalone MCP/A2A connection object is rejected on import with an explanatory error rather than silently skipped.
:::
