...
An **Anomaly Detector**'s environment belongs to the source installation, so it is never carried over as a raw reference: if it doesn't resolve to an environment on the target, it is dropped and reported the same way an unresolved certificate-family environment is. The imported detector arrives with no environment selected and stays inactive on the target's [Anomaly Detector](/en/monitor/anomaly-detector) screen until you pick a valid one there — it cannot be saved or activated without one.
