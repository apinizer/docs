...
The check covers JWKs in **all projects** that still reference this key, not only the key's own project. After saving, a notification reports how many JWKs were re-generated; a JWK that could not be re-generated (for example an older JWK without stored parse parameters) is listed by name and must be re-created from the key. JWKs whose link to the key was lost (for example after an import) are not detected.
