...
Two different export options are available in the key row menu for secret, private, and public key definitions.

<Steps>
  <Step title="Selecting the Export Type">
    Select the export type from the **⋮** menu of the relevant row in the key list:

    - **Export as Key**: Downloads the key as a file in its own format.
    - **Export**: Downloads the entire key configuration as a compressed (ZIP) package. This package is suitable for importing into another Apinizer environment.
  </Step>
  <Step title="Downloading the File">
    The file is downloaded automatically according to the selected export type.
  </Step>
</Steps>

:::info

The configuration package (Export) contains the entire key definition and can be used for migration between different Apinizer environments. The "Export as Key" option, on the other hand, downloads the key directly as a file.

:::

## Key Import

You can import a key configuration package that was exported from another Apinizer environment.

<Steps>
  <Step title="Opening the Import Window">
    Click the **Import** button at the top of the key list screen.
  </Step>
  <Step title="Uploading and Importing the File">
    In the opened import window, select and upload the previously exported configuration package. The imported key definition is displayed in the list.
  </Step>
</Steps>

:::note

Export and import operations can only be performed by users with the required permission.
