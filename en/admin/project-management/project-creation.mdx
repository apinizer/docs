...
| Skip conflicting consumers *(Import)* | Only shown on the **Import Project** screen. Off by default. See [How a Project Import Proceeds](#how-a-project-import-proceeds) below for what changes when it is turned on. |
...
As of Apinizer 2026.09.4, two more warnings can appear after the skipped-consumer ones described above, worded exactly like the entity-level Export/Import wizard's own report (see [Export/Import](/en/admin/secrets-management/export-import#mapping--dependencies)): one names a reference the import could not resolve in the new project and had to drop, and the other names a provider secret that was overwritten because an existing target object was replaced. Either one is shown only when it actually applies, and both are also written to the **Application Logs**.

As of the same release, one more warning can appear: when an **environment variable** in the package carries a name that is already in use on this installation, the variable is stored with `-imported` appended to its name. The warning lists both the old and the new name. Read this warning carefully. Environment variable names are unique across the whole installation, and `${VARIABLE_NAME}` references resolve by name. The references in the package therefore do not follow the new name; they resolve to the existing variable that holds that name, which belongs to another project. Check the value of the existing variable after the import. Update it by hand if needed, or edit the objects in the new project to use the new name. This warning is also written to the **Application Logs**. For details about environment variables, see [Environment Variables](/en/develop/environment-variables).

### AI References in a Whole-Project Package

A whole-project package can carry proxies, policies, or knowledge bases that reference an LLM Provider, a Vector DB connection, or a Knowledge Base by id. As of Apinizer 2026.09.4, all three are reconnected correctly during import. The provider, connection, and knowledge base files are imported before anything that references them. This way, a policy's Vector DB, embedding provider, and Knowledge Base references, and a knowledge base's own Vector DB and embedding-provider references, all resolve to the new project's copies instead of the source installation's ids. Earlier releases already imported the objects themselves correctly but left these particular references unresolved — a RAG policy or knowledge base could arrive without a working connection until you reselected it by hand.
