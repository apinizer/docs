...
Fields excluded in the mapping definition are not included in the comparison. You can see which fields were excluded in the execution detail. For each field, the detail also states whether the target value was preserved and, when it was not, the reason — see [API Mapping Creation](/en/admin/api-promotion/api-mapping-creation) for what these reasons mean.
...
Every stage of an execution's lifecycle — creation, approval, rejection, a scheduled or early start, completion, and cancellation — also writes a corresponding record to [Audit Records](/en/admin/audit-compliance/audit-records#operation-events) and, once a destination is configured, to a SIEM (see [SIEM & Log Forwarding](/en/admin/system-settings/siem-log-forwarding)).
