...
If a promoted API Proxy uses a global policy, that policy is automatically created in the target project when missing, or bound to the current global policy definition in the target environment when it already exists. This ensures the promoted proxy runs with the target environment's current global policy definition rather than a stale definition embedded during the transfer.
