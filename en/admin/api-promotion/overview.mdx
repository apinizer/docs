...
API Promotion is available to system administrators, to users granted the **Promotion Manager** role, and — for the mapping list, and for creating or updating a mapping — to a user who holds project-scoped Promotion permission on the mapping's source project: **View** to list and inspect mappings, **Manage** to create or update one. Every other Promotion screen and action (instances, execution, cloning, and the mapping wizard's comparison/dependency/pre-flight steps) requires system administrator authority or the Promotion Manager role.

:::warning

**Security hardening.** In earlier versions, several of the mapping wizard's server-side actions — including some that write to the target instance — did not enforce the permission check above; only the actions listed as protected did. Every Promotion server-side action is now checked. If an integration or a user was relying on one of those unprotected actions without holding the permissions above, it now receives an authorization error and the relevant role or project permission has to be granted.

:::

:::info

The rule is enforced on the server, not only in the interface, on every Promotion endpoint — there is no unauthenticated or under-authorized path through the module.
...
In addition to Creator dependencies, [Global Policies](/en/concepts/core-concepts/what-is-policy) and certificate-family records (Certificate, KeyStore, CryptoKeyInfo, JWK — see [Secrets Management](/en/admin/secrets-management/overview)) used in the source API Proxy (or Proxy Group) definition are also automatically detected. Each of these dependencies must be mapped to an existing record on the target environment or created there; otherwise the mapping cannot be saved. See [API Mapping Creation](/en/admin/api-promotion/api-mapping-creation) for details.
