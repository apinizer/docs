...
On analytics and report screens, the API Proxy and Proxy Group pickers reflect this same project scoping: a user without a system-level role sees only the API Proxies and Proxy Groups from projects where they hold **API Management** View permission — including when no project is explicitly selected. The "all projects" view available from the Admin menu returns an empty list for these users. Users with a system-level role (System Admin, Project Manager, System Analyzer, or a portal role) are unaffected.
