...
| **Distinguished Name** _(Distinguished Name)_ | The LDAP entry the account is bound to. It is filled in automatically when the user is saved (looked up by username in the active LDAP connections) or, when it is still empty, at the account's first successful LDAP sign-in. Together with the entry, the account remembers the LDAP connection that vouched for it.<br/><br/>At every sign-in only that connection is asked, and the entry that accepts the password must be this one: an account with the same username in another configured LDAP connection is never consulted, and an entry elsewhere in the same directory does not obtain this account's session — the attempt is recorded under **Login Records** with the reason `LDAP_IDENTITY_MISMATCH`. Accounts saved before this binding existed adopt the entry and the connection of their first successful sign-in (the application log notes the adoption), as does an account whose remembered connection was removed and re-created.<br/><br/>If the entry moves within the directory (for example to another organizational unit), the user is refused until this value is updated: open the account, clear the field and save — it is looked up again from the directory, or, if the directory cannot supply it at that moment, left empty so that the next successful sign-in adopts the new entry. |
