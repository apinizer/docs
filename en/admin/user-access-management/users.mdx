...
| **Distinguished Name** _(Distinguished Name)_ | The LDAP entry the account is bound to. It is filled in automatically when the user is saved (looked up by username in the active LDAP connections) or, when it is still empty, at the account's first successful LDAP sign-in.<br/><br/>At every sign-in, the entry that accepted the password must be this one: an account with the same username in another configured LDAP connection or in another part of the directory does not obtain this account's session, and the attempt is recorded under **Login Records** with the reason `LDAP_IDENTITY_MISMATCH`.<br/><br/>If the entry moves within the directory (for example to another organizational unit), the user is refused until this value is updated: open the account, clear the field and save, and it is looked up again from the directory. |
