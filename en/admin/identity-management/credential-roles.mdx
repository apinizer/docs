...
{/* TODO: screenshot — Role List screen (including the Project column and the Global tag, with the current breadcrumb); old file: /images/yonetici/role-liste.png; target: /images/2026-09-2/credential-role-list.png */}
...
**Role scope.** A role is either project-scoped or global:

- A role created inside a project belongs to that project and is only visible there.
- A role created from the Administration menu is **global** and is available in every project.

Both the **Consumer Roles** list of a project and the **Role** selector on the consumer edit screen show the same set: the project's own roles plus the global ones. Global roles are marked with a **Global** tag in the list and a **(Global)** suffix in the selector. Editing or deleting a global role requires administration-level identity permission; project users see it read-only.
...
For detailed information about role definitions, you can visit the [Consumer Roles](/en/admin/identity-management/credential-roles)
