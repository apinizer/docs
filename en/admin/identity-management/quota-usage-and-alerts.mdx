...
|------|------------------------|
| **Plan** | The product/portal subscription quota the consumer is linked to |
...
| **AI Token** | An AI Gateway [token budget](/en/ai-gateway/token-quotas), including a typed **AI Token Budget** [assignment](/en/ai-gateway/limit-plans#assigning-a-limit-to-a-subject-the-limits-tab); limit and consumption are shown as token counts. Attribution follows the assignment's own subject: a Consumer or an API client migrated from one is shown as before, while a native API client that never was a Consumer (usage scope `api_client`) and a portal application (usage scope `application`) now appear as their own rows too |
