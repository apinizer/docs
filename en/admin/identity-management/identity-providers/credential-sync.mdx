...
<Frame caption="Database provider — Identity Synchronization Profile">
  <img
    src="/images/2026-09-2/credential-sync-profil.png"
    alt="Database provider Identity Synchronization Profile tab with cron, deactivation mode, synchronization scope, and reconciliation safety net"
    width="1000"
    height="942"
  />
</Frame>
...
LDAP synchronization synchronizes not only users but also the organization tree the users belong to. The OIDC (Keycloak) source can also synchronize the group tree in addition to users when **Synchronize Groups** is enabled. The Database and API sources build an organization tree too — derived from the organization path on each user row and, optionally, completed from a separate organization list query/endpoint — but none of the four sources synchronizes roles except OIDC, and Database/API do not synchronize groups as separate objects the way LDAP and OIDC do.
...
By default a source synchronizes everything it can read. The **Synchronization Scope** card on the same tab narrows that set, with separate include/exclude pattern lists for **Users**, **Groups / Organizations** (LDAP, OIDC, and Database — not yet offered on the API provider's screen), and **Roles** (OIDC only).
...
| Database | Optional — via the `external_id` column, when the user list query includes it. A row that leaves it blank falls back to matching by user name for that row. |
| API | Optional — via the Stable Id JSON Path field, when configured. A row it does not match falls back to matching by user name for that row. |
...
That opt-in is planned to become the default in a future major version. Turning it on early is the safe direction for LDAP connections that do not fetch a stable attribute, and for Database and API sources that leave the stable-identity column or field unconfigured — the sources where a stable identity cannot be obtained at all.
...
<Frame caption="Credential Synchronization — provider list and history">
  <img
    src="/images/2026-09-2/credential-sync-izleme.png"
    alt="Credential Synchronization screen with LDAP Database API OIDC tabs, provider table, and synchronization history"
    width="1000"
    height="514"
  />
</Frame>
...
Database and API sources always fetch users and organizations together in a single pass, so they have no separate users-only or groups-only phase, and their button carries no scope menu — every run on these two sources is **Full**. A run's scope is shown in the **Scope** column of the [run history](#run-history); rows recorded before this version show **Full**, which is what those runs were.
...
| Result | The run's current state: **Queued**, **Running**, **Success**, **Warning**, **Failed**, **Interrupted**, **Cancelled**, or **Awaiting Reconcile Approval** — see [Run Lifecycle](#run-lifecycle). |
| Upserted, Deactivated, Skipped, Unchanged, Errors | The record counts processed by this run — see [Skipping Unchanged Users](#skipping-unchanged-users) for what **Unchanged** means. |
...
| Reason | A translated explanation of the result. A **Failed**, **Interrupted**, or **Cancelled** run also shows a **reason badge** classifying why — see [Failure Reason](#failure-reason). |
| Actions | Carries **Approve Reconciliation** while a staged reconciliation is still approvable — see [Deactivation Threshold and Reconcile Approval](#deactivation-threshold-and-reconcile-approval). Carries **Cancel** while the run is still **Queued** or **Running** — see [Cancelling a Run](#cancelling-a-run). |

#### Additional Columns for LDAP Sources

Rows for LDAP sources carry these extra counters in addition to the columns above:

| Column | Description |
|--------|-------------|
| Users read | Total number of user entries read from the directory in this run. |
| With membership attribute | Users whose own entry carried the membership attribute (`memberOf` and the like). |
| Groups read | Total number of group/OU entries read in this run. |
| Groups with members | Groups whose member attribute was non-empty. |
| Groups filtered out | Groups dropped by the scope filter; this is your own scope decision, not an error. |
| Linked (S1) | Users attached to an organization from the membership attribute on their own entry. |
| Linked (S2) | Users attached from a group member list. |
| Linked (S3) | Users attached from the OU their DN sits under. |
| Unlinked | Users no strategy could attach to an organization. |
| Re-enabled | Records this run automatically re-enabled because they came back in the source. |

These columns are populated only for LDAP source runs. They show `-` for LDAP runs recorded before this
counter set existed, and for every non-LDAP source. For field descriptions, see
[LDAP Active Directory](/en/admin/identity-management/identity-providers/ldap-active-directory#synchronization).

#### Additional Columns for Database and API Sources

Rows for Database and API sources carry these extra counters in addition to the columns above:

| Column | Description |
|--------|-------------|
| Organizations read | Rows read from the source's organization list. `0` means no organization list is configured. |
| Organizations derived | Organizations built from the path on the user row because the organization list has no matching row. |
| Linked | Users attached to an organization. On LDAP sources the same information is split into the three S1/S2/S3 columns, so this column stays empty there. |
| Disabled at source | Records this run switched off because the account is disabled at the source. Distinct from Deactivated, which means the record is no longer seen in the source at all. |
| Pages read | Pages actually read from a paging source. Empty for sources that read everything in one query. |

These columns are populated only for Database and API source runs. They show `-` on LDAP and OIDC runs, and on runs recorded before this counter set existed.
...
| Permission missing | The connection succeeded, but the account used does not have permission to read what this run needed (an HTTP 403, or the equivalent LDAP permission failure). The tooltip names which read failed — Users, Groups, or Roles. For a directory source this also covers a bind account that may connect but may not search under the configured base DN. | Grant the missing read permission for the named area. For a directory source, grant the bind account read and search rights on that branch. For an OIDC (Keycloak) source, see [Keycloak Service Account Roles](/en/admin/identity-management/identity-providers/oidc-provider#keycloak-service-account-roles). |
...
| Configuration | The run could not even attempt to reach the source — a required connection field was missing or invalid — or the directory refused the read because the configured base DN does not exist. The reason text names the base DN that was tried. | Check the source's connection settings. For a directory source, correct the base DN of the connection or of the user/group object class. |
| Interrupted | The run went silent before finishing — see [Run Lifecycle](#run-lifecycle). | Trigger the synchronization again. |
| Cancelled | The run was cancelled before it finished — see [Cancelling a Run](#cancelling-a-run). | No action needed; trigger a new run if the work is still wanted. |
...
A directory that **refuses** the read is not one of these two cases. A base DN that does not exist, and a bind account that may not search under it, end the run as **Failed** with a **Configuration** or **Permission missing** badge and a reason that names the base DN — not as a **Warning** about an incomplete read. Both are settings an administrator can correct, and reporting them as a partial read made them indistinguishable from a directory server that simply refuses paging.
...
| Duplicate login name | Another source record read on the same page uses the same login name. The first one read is kept; the second is skipped, so only one credential is created for that name. On LDAP, both records' distinguished names are recorded so you can tell which two directory objects collided. |
...
:::note LDAP-only membership diagnostics

Three further issue reasons are specific to LDAP and do not skip a credential — the user is still created or updated, only the [organization link](#how-the-organization-link-is-determined) is affected:

- One issue per user (up to 500 per run) when that user could not be linked to any organization by any membership source.
- One run-level issue once the unlinked count passes that 500 cap, summarizing how many more users had the same result.
- One run-level issue when **every** user read in the run ended up unlinked — this points at a configuration problem (membership mode, member attribute, group scope) rather than at individual users.

To see which membership source failed for a given user and why, use **Explain**, or the **Directory Preview**'s **Issues** tab (codes I01–I04) — see [Troubleshooting](/en/admin/identity-management/identity-providers/ldap-active-directory#troubleshooting).
...
This is why, in a healthy repeat run against a source that has not changed, seeing **Updated 0 / Unchanged N** is normal rather than a sign that the run did nothing — see the **Unchanged** counter in the [run history](#run-history).
...
Every credential created by synchronization — from LDAP, Database, API, or OIDC (Keycloak) — is assigned a permanent **consumer key**, the same way a manually created credential is, and carries a **first synchronized** timestamp recording when it was first written. The [inventory and migration](/en/admin/identity-management/credentials#legacy-records-inventory-and-migration) screen, and export, no longer treat these records as keyless.
...
| Request | Response |
|---------|----------|
| `POST /api/identity-sync-run-histories/{runId}/actions/cancel` | **200** — the same behavior as the **Cancel** button in the run history table, described in [Cancelling a Run](#cancelling-a-run). A run that is not **Queued** or **Running** is refused with **409** and a translated message. |

## How the Organization Link Is Determined

:::info One organization per consumer

On every source, a consumer is linked to **at most one** organization. When a user qualifies for more than one candidate organization in the source, a single winner is chosen; the other candidate memberships are not written to the consumer. On LDAP, those other memberships are still visible in the **Explain** tool's diagnostic output — see [Troubleshooting](/en/admin/identity-management/identity-providers/ldap-active-directory#troubleshooting).

:::

| Source | Candidates | How the winner is chosen |
|---|---|---|
| LDAP | Groups and OUs that are inside the synchronization scope | The candidate with the most RDN components (the deepest one) wins. At equal depth, which candidate wins is not guaranteed — do not design a directory layout around it. In **OU and group** mode, the OU the user's own entry lives in is tried first; only when that does not resolve is an explicit group membership used. |
| OIDC (Keycloak) | Groups reached while **Synchronize Groups** is on | The deepest group path (most `/` separators) wins. At equal depth, the group the run happened to process first wins — this follows Keycloak's own group tree order, not a setting you control. |
| Database | The single organization path read from the user's row | There is no selection to make: the row names one path, and that path is the organization. |
| API | The single organization path read from the configured **Organization Path JSON Path** | Same as Database — one path, no selection. |

:::note The link is rewritten on every full run — including a manual reassignment

When a user's group or path changes at the source, the next **Full** run updates the consumer's organization to match. If the user no longer qualifies for any in-scope organization, the link is cleared. Because the source's answer always wins on a full run, an organization you reassigned by hand on a synced consumer is replaced the same way, the next time a full run reads that user.

A run that does not read the group/organization phase makes no statement about organizations, so it leaves the existing link exactly as it is. What counts as "did not read" differs by source:

| Source | When the organization link is left untouched |
|---|---|
| LDAP | The run's scope is **Users only** — see [Choosing What a Manual Run Covers](#choosing-what-a-manual-run-covers). If the organization tree could only be read in part (a paging warning) and no organization was found for a given user in that partial read, the existing link is also kept — "unknown this run" is not the same statement as "no organization". |
| OIDC (Keycloak) | The run's scope is **Users only**. Turning **Synchronize Groups** off does **not**, by itself, leave the link alone: a normal **Full** run still runs, still does not read any group, and still writes the organization as empty — so the next full run after **Synchronize Groups** is turned off clears every existing link from this source. |
| Database | The **Organization Path** column is not part of the User List Query at all. If the column is present but a given row leaves it blank, the link is removed for that row specifically. |
| API | The **Organization Path JSON Path** field is left empty. If the field is configured but its expression matches nothing on a given row, the link is removed for that row specifically. |

:::

Matching is keyed by a stable identity, not by the display name that happens to be shown on screen. LDAP matches first by a stable attribute (when one is fetched) and falls back to distinguished name; OIDC matches by group path; Database and API match by organization path. Two source groups with different paths that happen to carry the same display name resolve to two separate organizations — this is expected and does not raise a warning. When the resolved path is identical to an existing **manually created** organization, synchronization links users to that existing record instead of creating a duplicate — see [Synchronization Behaviors](/en/admin/identity-management/identity-providers/ldap-active-directory#synchronization-behaviors).
...
- A consumer that synchronization disabled because it disappeared from the source is automatically re-enabled once it reappears — this is the same on every source, not only LDAP. A consumer disabled by hand is never touched by this. When a source explicitly reports the account as still inactive, that answer wins and the consumer stays disabled; only a source that says nothing about account state lets the earlier automatic disable be lifted.
