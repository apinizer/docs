...
| Upserted, Deactivated, Skipped, Unchanged, Errors | The record counts processed on the last run — see [Skipping Unchanged Users](#skipping-unchanged-users) for what **Unchanged** means. |
...
| Cancelled | An administrator cancelled the run before it finished — see [Cancelling a Run](#cancelling-a-run). The records already written are kept; only the deactivation/reconciliation step was skipped. |
...
Cancelling does not stop the run instantly. The engine checks for the cancellation request at the next page boundary while it pages through the source, and stops fetching there: the pages already written **stay** — they are not rolled back — but the deactivation/deletion step (reconciliation) is **skipped**, for the same reason a partial read skips it elsewhere: a partial list would make every record on a page that was never fetched look as if it no longer existed in the source.
