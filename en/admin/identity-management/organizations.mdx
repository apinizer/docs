...
The organization detail page lists the consumers that belong to the organization in a **Consumers** card, next to the accounts, subscriptions and applications. The list is read-only and is loaded page by page from the server, so an organization with a large number of consumers does not slow the page down; type part of a consumer name in the search box to narrow it. Clicking a row opens that consumer's own page, where it is managed as usual. Consumers from every project the organization spans are listed; the project badge on a row shows which one a consumer belongs to.
