...
### Legacy Username and Password Become Read-Only After Migration

Once [Secret Carry-Over](/en/admin/identity-management/api-client-migration#secret-carry-over) has given the consumer's derived API client a locally-held secret, the **Username** and **Password** fields on this screen turn read-only, and an information banner names the migrated client by its client id and points to its **Client Secrets** sub-tab, under the [API Clients](#api-clients) tab, for rotating it instead.

Every other field on the consumer stays editable and the record still saves normally. Only an actual change to the username or the password is refused: saving a form that echoes the current values, or that leaves the password blank, goes through untouched. A consumer whose client identity was never carried over keeps editing both fields exactly as before.

:::note
The rejection is a `400` carrying the error key `credential.legacyIdentityMigrated`. This is enforced on the server regardless of what the screen shows, so it also catches the rare case the screen itself cannot detect on its own. The legacy read path is unaffected by the lock: tokens obtained with the still-current username and password keep validating, and nothing here suspends them — this only closes further edits to those two fields.
:::
...
In this version the mode is chosen through a field on the management API request that issues or rotates a secret; the Client Secrets screen itself does not yet offer the choice, so a secret issued from the screen always takes the installation's default. See [API Client Secret Storage Settings](/en/admin/system-settings/api-client-secret-settings) for that default and how to change it.
