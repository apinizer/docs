...
<Frame caption="Consumer list">
<img
    src="/images/2026-09-2/consumer-list.png"
    alt="Consumer list card view with source tag, status, organization, roles, and portal badge"
    width="1000"
    height="492"
/>
</Frame>
...
| Field | Description |
|------|----------|
| Name _(Name)_ | The required display name on the create form. |
| Username _(Username)_ | This is the "username" information that will represent the identity, this value is used to access the user identity for authorization. It must be unique and one-of-a-kind throughout the entire system. It is not on the create form; it is added from [API Clients](#api-clients). |
| Password _(Password)_ | This is the password information of the identity. It can be automatically generated with the button next to it if desired. It is not on the create form; it is added from [API Clients](#api-clients). |
...
:::note Organization on a sync-managed consumer

For a consumer managed by [synchronization](/en/admin/identity-management/identity-providers/credential-sync), the Organization field comes from the source provider. It cannot be changed here, or with the [Transfer](#transferring-a-consumer-to-another-organization) action either. To change it, update the group or OU membership in the source; the next synchronization run writes the new organization.
...
The one relationship that does block it: a consumer that still owns [API clients](#api-clients) not yet revoked cannot be deleted — the request is refused with an HTTP 409 response that names the consumer itself and the real blocker (the count of un-revoked API clients still owned by it). [Revoke](#suspending-resuming-and-revoking-an-api-client) the clients first, then delete the consumer. Already-revoked clients never block the delete.

Deleting a consumer also **ends its limit assignments**. Every Request Rate or AI Token Budget assignment naming this consumer — active ones and unactivated drafts alike — is closed in the same step, with the reason recorded as *The subject was deleted*, and the ceiling is withdrawn from the gateways it was deployed to. The assignments are kept as history, the same way an assignment you end by hand is. If the delete is refused for any of the reasons above, the assignments are left untouched and keep applying. The same applies when an [organization](/en/admin/identity-management/organizations#deleting-an-organization) or a [portal application](/en/admin/identity-management/portal-applications#deleting-an-application) is deleted. See [Limit Plans and Assignments](/en/ai-gateway/limit-plans#lifecycle-assign-edit-activate-end).
...
API clients are managed from the **API Clients** tab of the consumer, on both the view and the edit screen. An API client is never edited on a page of its own: the row in the list is a summary, and the expanded row is the whole editor with its **Overview**, **Client Secrets**, **Authorization**, [Limits](/en/ai-gateway/limit-plans) (for users with permission to view Identity or AI budgets) and **Activity** sub-tabs.

<Frame caption="API Clients tab">
<img
    src="/images/2026-09-2/api-clients-tab.png"
    alt="Consumer detail API Clients tab: client id, status, secret count and validity"
    width="1000"
    height="271"
/>
</Frame>

The consumer's own **Limits** tab, on its own edit screen, assigns a ceiling to that consumer — and to the pooled consumption of every client it owns — only; see [Target](/en/ai-gateway/limit-plans#target-what-the-ceiling-narrows-to).
...
<Frame caption="Client Secrets sub-tab">
<img
    src="/images/2026-09-2/api-client-secrets.png"
    alt="API client detail Client Secrets: version table, storage mode and status; secret value is not shown"
    width="1000"
    height="509"
/>
</Frame>
...
On the **Client Secrets** sub-tab, the issue/rotate dialog offers this choice directly: a **Storage Mode** control lets you pick **Encrypted** or **Hashed** for the generation being created. It is pre-selected to the installation's default — see [API Client Secret Storage Settings](/en/admin/system-settings/global-configuration#api-client-secret-storage) for that default and how to change it — and switching it in the dialog only affects the one generation being issued. Each existing generation's mode is shown in the **Storage** column of the secret versions table.
...
<Frame caption="Authorization sub-tab">
<img
    src="/images/2026-09-2/api-client-authorization.png"
    alt="Edit API client: validity fields and Authorization sub-tab in Allow a subset mode"
    width="1000"
    height="649"
/>
</Frame>
...
- **A consumer's own API client follows the consumer's Token Settings.** The API client that is created automatically for a consumer carries no token settings of its own; the lifetime, never-expires and refresh settings saved on the consumer's **Token Settings** tab apply to the tokens it obtains at `/credential/token` and `/credential/jwt` — exactly as they did before the client existed. Only an API client that was given its own token settings through the Management API uses those instead, and then as a whole: the consumer's values are never mixed into them. Because the token is obtained through the client, the consumer's **Refresh Token Allowed** setting is also read live: turning it off takes effect on that consumer's very next refresh attempt.
- **An API client that belongs to an application never expires unless you give it a lifetime.** A client whose owner is an application rather than a consumer — the clients issued from the API Portal are the usual case — has no consumer **Token Settings** tab behind it, and the portal does not ask for token settings when it issues one. Such a client is treated exactly like a consumer whose token settings were never filled in: its token **never expires** and no refresh token is issued. In the response this shows as `expires_in: 0` with **no** `X-IssuedAt` or `X-ExpiresAt` fields — that combination means "no expiry", and the token stays valid. To give one a real lifetime or refresh capability, set the token settings on that API client through the Management API; a client that carries its own settings uses them as a whole.
...
Revoking a consumer, an API client, or an organization's consumers can never invalidate an already-issued token of either shape before its own expiry arrives — there is nothing to stamp a cutoff on. [Revocation Strictness](/en/admin/identity-management/token-management-settings#revocation-strictness) does not close this gap for that kind of revoke either: turning it on only makes the token's own explicit self-revocation — the caller's own call to the [RFC 7009 revoke endpoint](#obtaining-a-token-with-an-api-client) — actually take effect for a token of this shape, where today that call is accepted and silently does nothing. It does not let an admin-triggered consumer, API client, or organization revoke reach a token it could not reach before.
...
:::note[Access control does not travel with Export/Import]
A consumer's Access Control List is not included when the consumer is exported through the [Export/Import Wizard](/en/admin/secrets-management/export-import) — nor on a single-object (JSON) export, nor over APIops. Importing the package with **Import as New** creates a consumer with no API access at all; grant it here afterward. Importing with **Replace Existing** leaves the target consumer's existing Access Control List exactly as it is — nothing from the package is added, removed, or changed. This is deliberate: access control is specific to the installation and environment it runs in. A whole-project export/import (**Administration → Export Project** / **Import Project**) is unaffected by this and still carries access control with it.
:::

<Frame caption="Access Control tab">
  <img
    src="/images/2026-09-2/access-control-tab.png"
    alt="Edit Consumer Access Control tab with Proxies list and Add Proxy button"
    width="1000"
    height="278"
  />
</Frame>
...
<Frame caption="Add API Proxy — Proxies dialog">
  <img
    src="/images/2026-09-2/add-api-proxy-dialog.png"
    alt="Add API Proxy Proxies dialog with proxy list, filters, and Add button"
    width="1000"
    height="421"
  />
</Frame>
...
<Frame caption="Save and Deploy">
  <img
    src="/images/2026-09-2/access-control-tab.png"
    alt="Edit Consumer screen with Save and Deploy button in the top-right corner"
    width="1000"
    height="278"
  />
</Frame>
...
<Frame caption="API Proxy-based customization panel">
  <img
    src="/images/2026-09-2/api-proxy-custom-settings.png"
    alt="Access Control tab with an API Proxy selected and right-side configuration panel for quota, throttling, and AI Model Access"
    width="1000"
    height="484"
  />
</Frame>
...
<Frame caption="Metadata tab — Consumer Metadata">
  <img
    src="/images/2026-09-2/metadata-tab.png"
    alt="Edit Consumer Metadata tab with key/value table and JWT and token response options"
    width="1000"
    height="289"
  />
</Frame>
...
- **OAuth response (dynamic)**: Any custom names you configure for OAuth response fields under [Token Management Settings](/en/admin/identity-management/token-management-settings) (e.g. the names you set for the access-token or scope fields) are also reserved.
...
<Frame caption="Token Settings tab">
  <img
    src="/images/2026-09-2/token-settings-tab.png"
    alt="Token Settings tab with Grant Type, Token Lifecycle, and Security sections"
    width="1000"
    height="490"
  />
</Frame>
...
<Frame caption="Secrets tab — JOSE JWK">
  <img
    src="/images/2026-09-2/secrets-jwk.png"
    alt="Secrets tab showing JWK for JOSE Sign and Validation and Encryption and Decryption rows"
    width="1000"
    height="488"
  />
</Frame>
...
<Frame caption="Secrets tab — Truststore (mTLS)">
  <img
    src="/images/2026-09-2/secrets-jwk.png"
    alt="Secrets tab showing Truststore row and Secrets and Certificates list"
    width="1000"
    height="488"
  />
</Frame>
