...
<img
  ...
  alt="Bulk Consumer Import via CSV"
  width="800"
/>
...
| Organization _(Organization)_ | The organization the consumer is linked to. Beyond reporting, this link drives **organization-level metadata inheritance** into the tokens issued for this consumer — see [Organization-Level Metadata](#organization-level-metadata). Read-only once the consumer already exists: moving a consumer to a different organization is done from [Transferring a Consumer to Another Organization](#transferring-a-consumer-to-another-organization) below, not by editing this field. |
...
Alongside the username, a consumer carries a **consumer key**: a permanent, readable identity assigned by the server when the consumer is created. It is not a field on the create screen and cannot be edited.
...
Because the key identifies exactly one consumer, it is never carried over to a second record: a consumer produced with **Duplicate**, or brought in with **Import as New**, receives a new key of its own. Consumers that already existed before this version are given a key once, during the upgrade.
...
### Legacy Username Becomes Read-Only After Migration; a Password Change Rotates the Client Secret {#legacy-username-and-password-become-read-only-after-migration}

Once [Secret Carry-Over](/en/admin/identity-management/api-client-migration#secret-carry-over) has given the consumer's derived API client a locally-held secret, the **Username** field on this screen turns read-only — it is the client id every caller already holds — and an information banner names the migrated client by its client id. The **Password** field stays editable, and saving a new value there does two things at once: it updates the legacy credential's own password as before, and it **rotates the migrated client's secret** to the same value, through the same grace-period mechanism as a rotation started from the **Client Secrets** sub-tab, under the [API Clients](#api-clients) tab — the previous secret keeps authenticating through its grace window while the new one takes over. The new generation keeps the storage mode the client's current generation has (an Encrypted generation stays Encrypted, whatever the installation default), and a value containing a `${...}` or `#{...}` reference is carried as a reference the gateway resolves per request, never as a literal secret. Both stores therefore always agree; a client that has been revoked takes no rotation and only the legacy password changes.

Every other field on the consumer stays editable and the record still saves normally. Saving a form that echoes the current username, or that leaves the password blank, goes through untouched and rotates nothing. A consumer whose client identity was never carried over keeps editing both fields exactly as before.

:::note
Only the username change is refused outright — with a `400` carrying the error key `credential.legacyIdentityMigrated`. This is enforced on the server regardless of what the screen shows, so it also catches the rare case the screen itself cannot detect on its own. Neither the username lock nor a password rotation suspends anything already issued: a token obtained before the rotation keeps validating until the previous secret's grace window ends.
:::
...
API clients are managed from the **API Clients** tab of the consumer, on both the view and the edit screen. An API client is never edited on a page of its own: the row in the list is a summary, and the expanded row is the whole editor with its **Overview**, **Client Secrets**, **Authorization** and **Activity** sub-tabs.
...
The **migration** carries the access side of a record and nothing else. The legacy access lists are not modified, and the record's other limits stay where they are — except its AI token budget, which is now carried separately into a typed assignment at upgrade; see [AI Budgets](/en/ai-gateway/limit-plans#ai-budgets). A record with an access rule that cannot be resolved — an entry pointing at an API Proxy that no longer exists — is still migrated: that one rule is skipped and reported on the record's row, while the Consumer-bound client keeps enforcing the legacy rules themselves. A dry run writes nothing at all.

Secret material is not part of this transfer either — a migrated client is created in Draft with no secret at all, exactly like one created by hand. A separate, one-time step, [Secret Carry-Over](/en/admin/identity-management/api-client-migration#secret-carry-over), can additionally give it the exact secret its legacy record already uses, without disturbing the legacy record or anything already issued. From that point on, the legacy record's own username turns read-only on its own edit screen, and a password change there rotates the migrated client's secret instead — see [Legacy Username Becomes Read-Only After Migration; a Password Change Rotates the Client Secret](#legacy-username-and-password-become-read-only-after-migration).
...
<img
  ...
  alt="Consumer Access Control Settings"
  width="1000"
  height="292"
/>
...
- the target organization does not resolve, does not belong to the same project as the consumer, or has been merged into another organization;
