...
A **Keyword** condition searches one part of one message region. First choose the region, then the part; the condition matches only the data of that region.
...
- **Header** and **Parameter** parts require the header/parameter name; the operator is applied to its value. Responses carry no query parameters.
- **Body** parts search the full text of that region's body (case-insensitive; values with punctuation such as `ABC-123` are matched as a phrase, `*`/`?` give explicit wildcard patterns).
...
:::tip
To see which target a request was routed to, simply select the route address field under **Query Result Fields**. This information is kept on all traffic records.
:::
