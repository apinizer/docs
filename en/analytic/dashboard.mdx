...
<img
  ...
  alt="Dashboard — summary cards, Traffic/Response Time charts, Most Active Endpoints/Clients, and Routing Diagnostics panel"
  width="1000"
  height="949"
/>
...
Since an API Proxy can be loaded into multiple [Environments](/en/concepts/core-concepts/what-is-environment), metrics are **queried based on environment**. Environment selection must be made from the top of the page. On **Administration > Analytic > Dashboard**, an additional **Project** filter lets you scope metrics to all projects or selected projects; the dashboard under the Project menu covers only the active project.
...
The bottom section shows a routing diagnostics summary. The first line of the section carries a permanent description of what the panel shows, and hovering the info icon next to each card title explains how to read that card.

- **Failure Reason Distribution** — breakdown of error causes (e.g. Upstream HTTP Error); the chart's vertical axis is labelled in **requests**
- **Phase Latency (p50/p95/p99)** — phase latency for Upstream Selection, DNS, TTFB, Body Read, Pool Wait; the chart's vertical axis is labelled in **milliseconds (ms)**
- **Top Failing Proxies** — API Proxies with the most failures in the selected window; the table is listed under the **API Proxy** and **Failures** columns

Average durations and the most frequently used upstream addresses are shown with their units (ms, requests) as well. When there are no records in the selected range, the card points you to the "How to read?" explanation of when the panel stays empty.

For the same signals at single-request level, see the **Routing Diagnostics** section in the [Gateway Traffic](/en/analytic/api-traffic) detail view.
...
2. Filter failed requests from the [Gateway Traffic](/en/analytic/api-traffic) page
...
3. Examine suspicious requests with [Gateway Traffic](/en/analytic/api-traffic)
...
<CardGroup cols={2}>
  ...
  <Card title="Use Detailed Analysis Tools" icon="magnifying-glass-chart">
    ...
    - Examine in detail with [Gateway Traffic](/en/analytic/api-traffic)
...
<CardGroup cols={2}>
  <Card title="Gateway Traffic" icon="chart-network" href="/en/analytic/api-traffic">
