...
| afr | Authentication Failure Reason | Enum | Response Information (LDAP authentication policy only) |
...
### Why Was an API Request Rejected by LDAP Authentication?

When an LDAP authentication policy rejects a request, the `afr` field records one of these reasons:

- **Sign-in failed**: the credentials were rejected, or the account is locked, disabled, or expired in the directory.
- **User not found**: the search for the user returned no result.
- **Role missing**: the user was verified, but does not carry a role the policy requires.
- **Directory unreachable**: the directory could not be reached at all — a network, TLS, or timeout problem.

The `afr` value is never returned to the client; it is only available in the traffic log for operators. When the directory is unreachable, the request is rejected with a service-unavailable error instead of the usual 401 authentication error, because the request could not be evaluated at all.
...
You can examine the list of values in the **"rt"** field in the "Result Type Values" table on the [Error Messages](/en/admin/system-settings/error-messages) page.

You can examine the list of values in the **"pet"** field in the "Parent Error Type Values" table on the [Error Messages](/en/admin/system-settings/error-messages) page.
...
[2026.09.1 — Log Table ALTER Scripts](/en/release-notes/2026-09-1-log-table-alter) page. New
...
[API Traffic Settings](/en/analytic/api-traffic-log-settings) page.
