...
A Custom Query used as a Trace filter now matches the same requests that the same query returns on the
[Advanced Search](/en/analytic/query-editor/queries) screen. This mainly changes two kinds of query: an
**OR** condition group now traces only the requests that actually match at least one of its conditions
(it no longer traces every request regardless of the group), and a **NOT** condition group with more than
one condition now excludes a request only when none of its conditions match (previously only the last
condition in the group was applied). A query built from a single **AND** group, or a **NOT** group with
only one condition, is unaffected. If a Custom Query used for tracing suddenly captures fewer requests
after upgrading, this is the expected correction, not a regression — compare it against the same query on
Advanced Search to confirm.
...
<Tabs>
  <TabItem value="detailed-viewing" label="Detailed viewing">
  ...
  <TabItem value="view-in-json-format" label="View in JSON format">
...
:::
  ...
  <TabItem value="quick-test" label="Quick Test">
...
<Tabs>
  <TabItem value="successful-flow" label="Successful Flow">
  ...
  <TabItem value="failed-flow" label="Failed Flow">
