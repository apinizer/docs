...
:::note
This report does not show its own date picker; the date range comes from the [shared filter](/en/analytic/analytics-reports/reports#shared-filter-and-refresh) at the top of the Metrics page. When the tab is first opened, the window automatically widens to the last year if the shared range has not been changed.
:::
