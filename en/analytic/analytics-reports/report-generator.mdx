...
| **Break down by** | Gateway Traffic by Interval | Splits the report into one series per dimension value: API Proxy, HTTP Method or Credential. The default, *None (system-wide)*, produces a single series covering all traffic. When set, a **Dimension** column is added to the output. |
