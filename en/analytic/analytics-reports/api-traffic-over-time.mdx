...
The **Project**, **API Proxy**, and **Interval** selectors, as well as the graph/list toggle buttons, that appear in the report's own filter form when it runs standalone (not embedded) are **not shown** when the report is embedded in the Metrics page. In this view the report only uses the shared filter row described above.

:::

### Grouping Interval

The underlying report can group traffic by the following interval values: **Secondly, Minutely, Hourly, Daily, Weekly, Monthly, Quarterly, Annually**. On the Metrics page, grouping runs with the default value, **Hourly**.
