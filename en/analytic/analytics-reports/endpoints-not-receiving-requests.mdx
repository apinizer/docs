...
The **Method/Endpoint** column shows the method and the path together, such as `GET /pet`, so rows for the same path defined with different methods can be told apart. For records without method information (SOAP methods and proxies that carry no method definition) only the path is shown. The Excel output and the Report Generator preview use the same display.
