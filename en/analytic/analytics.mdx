...
- Under the Project menu, Metrics covers only the active project; the **Conflicts** tab is not shown.
- Under Administration you can query all projects with the project filter; the **Conflicts** tab (Route Conflicts, Route Trace) appears only there.
- Traffic, endpoint, client, canary/mirror, and growth sub-reports use the same tab layout in both menus; see the [Metrics overview](/en/analytic/analytics-reports/reports) page for details.
