...
description: "You can list requests and responses to all API Proxies in the project on the Gateway Traffic screen with full detail. You can investigate message issues, apply filters, use detailed and JSON views, export records, and run quick tests."
---
...
<img
  src="/images/2026-09-2/gateway-traffic-overview.png"
  alt="Gateway Traffic — list view, filter bar, and table columns"
  width="1000"
  height="424"
/>
...
Since an API Proxy can be loaded into multiple [Environments](/en/concepts/core-concepts/what-is-environment), metrics are queried per environment. The **Administration** menu adds a **Project** filter; the project-scoped screen covers only the active project.
...
| Column | Description |
|--------|-------------|
| **Status · Time** | HTTP status code (color badge) and request timestamp |
| **Method · Request** | HTTP method and proxy/method name; sub-line shows API type (REST, SOAP, …) and request server |
| **Client** | Client IP address and identity summary (API Client, `anonymous`, or legacy username/key) |
| **Routing** | Routing target; `blocked at gateway` or `direct` badge |
| **Latency [request · routing · response]** | Total duration (ms), phase breakdown, and bar chart |
| **Size** | Request and response body sizes (↑ / ↓) |
| **Actions** | Row menu — detailed view, JSON, download, quick test |
...
<Tabs>
  <TabItem value="basic-filtering" label="Basic Filtering">
    Records can be filtered with predetermined criteria such as a specific time range, endpoint, or HTTP method.
    
      <img
        src="/images/2026-09-2/api-traffic-basic-filter-overview.png"
        alt="Gateway Traffic — Basic Filtering tab with Canary and Mirror Traffic filters"
        width="1000"
        height="358"
      />
    ...
    **Filtering Criteria:**
    ...
    - **API Type**: Filtering by proxy type — SOAP, REST, gRPC, WebSocket, MCP, A2A (see [API Type](#api-type))
  ...
  <TabItem value="advanced-filtering" label="Advanced Filtering">
...
:::
    ...
    **Advanced Filtering Options:**
    ...
    - **API Client**: API Client used for authentication (legacy records may show username/key)
    ...
    The **Clear** button resets the advanced criteria and the hidden drill-down filters that arrive from dashboard charts; it does not switch tabs, so you stay on Advanced Filtering. If the list is refreshed before any criterion is built (auto refresh, pagination, date range), the query runs without a criteria filter.

    :::note

    The **tab that is open on screen** decides which filter set is applied: while Basic Filtering is open the basic fields (HTTP Method, Result Type, Status Code and so on) apply, and while Advanced Filtering is open the criteria apply. The fields of the two tabs are never mixed.
...
<Tabs>
  <TabItem value="mcp-details" label="MCP Details">
    | Field | Description |
    |------|----------|
    | **Tool Name** | The name of the MCP tool the request targets |
    | **JSON-RPC ID** | The `id` value of the JSON-RPC envelope — used to match the request with its response |
  </TabItem>

  <TabItem value="a2a-details" label="A2A Details">
    | Field | Description |
    |------|----------|
    | **Task ID** | The identifier of the A2A task |
    | **Context ID** | The context identifier that groups tasks belonging to the same conversation |
    | **Task State** | The state of the task at the end of the request (e.g. `TASK_STATE_COMPLETED`) |
  </TabItem>
</Tabs>
...
For an aggregate routing diagnostics summary for this API Proxy (failure reason distribution, phase latency p50/p95/p99, etc.), see the [Analytics tab](/en/develop/monitoring-analysis/analytics).
...
:::warning The body may be truncated by log limits
