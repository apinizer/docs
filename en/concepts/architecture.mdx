...
In the Apinizer Platform, an **[Environment](/en/concepts/core-concepts/environment)**, is a virtual server space where API Gateways (Workers) run, has its own access address and settings, is isolated from other Environments, and operates using resources such as CPU and RAM allocated to it.
...
* **Worker Pods**: Pods where [API Proxy](/en/concepts/core-concepts/api-proxy)s run. Each Worker pod is an independent API Gateway instance and has its own Local Cache.

* **Cache Pods**: Distributed cache ([Hazelcast](/en/concepts/core-components/cache-component)) pods. They hold shared cache data among Workers within the Environment.
...
* **Configuration Isolation**: Each Environment has its own configuration settings. [API Proxy](/en/concepts/core-concepts/api-proxy)s can be deployed to one or more Environments and can run with different configurations in each Environment.
...
<Info>
  **Note:** [API Proxy](/en/concepts/core-concepts/api-proxy)s can be deployed to one or more Environments. All Environments where deployment is made are located within a Kubernetes cluster and run on servers where the Apinizer Platform is installed.
</Info>
...
1. **Route Matching**: The incoming request is handled by the Route Matching component. The appropriate [API Proxy](/en/concepts/core-concepts/api-proxy) is found based on Path, Host, Method, and Header information, and MessageContext is created.

2. **Policy Enforcement (Request Pipeline)**: MessageContext passes through the Request Pipeline and 4 categories of [policies](/en/concepts/core-concepts/policy) are applied in order:
   - Security Policies
...
<CardGroup cols={2}>
  ...
  <Card title="Cache API" icon="code">
    Cache API is used for all operations to be performed on the cache.
  </Card>
</CardGroup>

<Info>
  For detailed information about distributed cache architecture and usage, see the [Cache Component](/en/concepts/core-components/cache-component) page.
</Info>
...
* **[Distributed Cache (Hazelcast)](/en/concepts/core-components/cache-component)**: Cache system where shared data such as Quota, throttling, OAuth2 token are stored.
...
<CardGroup cols={3}>
  <Card title="API and Policy Configurations" icon="network-wired">
    * [API Proxy](/en/concepts/core-concepts/api-proxy) definitions and configurations
    * [Policy](/en/concepts/core-concepts/policy) configurations (rate limiting, authentication, transformation, etc.)
    * [Environment](/en/concepts/core-concepts/environment) settings and configurations
