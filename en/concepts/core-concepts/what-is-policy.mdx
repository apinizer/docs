...
| 3 | Error response rules | Condition-based response rules — return different error responses based on request information |
| 4 | Per-error field customization | Per-row Customized Message, Customized Error Code, Customized HTTP Status Code fields |
| 5 | Platform-wide error message customization | Customization made from System Settings > [Error Messages](/en/admin/system-settings/error-messages) page |
| 6 (Lowest) | Default error message | Default system error message |

:::warning

If a higher-priority source is defined, lower-priority ones are not evaluated. For example, if a per-error custom message template is active for an error row, the policy-level template, error response rules, or per-error field customizations are not considered.
...
**Example template:** `Error occurred: #{error.defaultErrorCode} - Request path: #{request.uri}`
...
**Example (Business Rule STOP — UTC timestamp):**
...
To include a timestamp with timezone offset information in your error response, use the Date/Time Helper (`dtf`) function:

```
Your API call failed.
Error Code: #{error.defaultErrorCode}
Time: #{dtf.format('+03:00','yyyy-MM-dd\'T\'HH:mm:ssXXX')}
Request ID: #{message.correlationId}
```

For complete documentation on timezone-aware formatting and custom patterns, see [Date/Time Helper (`dtf`)](/en/concepts/core-concepts/dynamic-variables#date-time-helper-dtf).

:::note

This feature is optional. Your existing error messages continue to work as before.
