...
### Date/Time Value Output Format

When a formatted value from the Date/Time group is selected in the **Value** field, the variable resolves using the following patterns. The output time zone is determined by the **Zone ID** field; if this field is left blank, the system default time zone is used.

| Value | Output Pattern | Example |
|-------|----------------|---------|
| Formatted date-time | `yyyy-MM-dd'T'HH:mm:ss.SSS'Z'` | `2026-06-18T14:30:45.123Z` |
| Formatted date | `yyyy-MM-dd` | `2026-06-18` |
| Formatted time | `HH:mm:ss` | `14:30:45` |

:::note

The formatted date-time output always ends with the letter **Z**. Other Date/Time values such as year, month, day, hour, minute and second are returned as plain numeric values on their own.
...
| **Script Language** | Field where Script Language is selected. Switching the language updates the script editor's syntax highlighting immediately, without reloading the page. |
