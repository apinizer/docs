...
In **Auto by client Accept-Encoding** mode, responses generated by the gateway itself are not automatically compressed. Error responses originating from authentication or policy enforcement, as well as the successful responses of gateway content-producing proxy types such as Database-to-API, Mock API and Fake API, are forwarded plain (uncompressed). If you want these responses to be compressed as well, select a specific method (GZIP / DEFLATE / BR / ZSTD) on the route. Proxies that route to a real Backend API are not affected.
