...
## Value Visibility: Secret and Non-Secret

**All** environment variable values are stored encrypted (AES-GCM) in the database. The `Secret` flag does
**not** control encryption — it controls whether the value can be read back.

<CardGroup cols={2}>
  <Card title="Non-Secret" icon="eye">
    Used for configuration values
    
    Values such as URLs, port numbers and flags. Stored encrypted; masked on screen, revealable with the eye
    icon and copyable. Returned as plain text by the read APIs and carried as plain text in an export package.
  </Card>
  
  <Card title="Secret" icon="lock">
    Used for information that must never be read back
    
    Passwords, API keys, tokens. Once saved they can **never be displayed**: not shown on screen, returned
    as `null` from the API, and never written as plain text into an export package.
    
    :::warning

**Important**: A Secret value cannot be read back. To change it, you must type a new value.
...
:::
  ...
  <Accordion title="What Should Be Marked Secret">
    * **Passwords**: Database passwords, API keys
    * **Certificates**: SSL/TLS certificates
    * **Tokens**: OAuth tokens, JWT secrets
    * **Other Sensitive Information**: Any value nobody should be able to read back
  </Accordion>
  
  <Accordion title="What Does Not Need To Be Secret">
...
<CardGroup cols={2}>
  ...
  <Card title="Secret Manager" icon="lock" href="/en/admin/secrets-management/certificates">
    Secret Manager usage
  </Card>
  <Card title="API Proxy Configuration" icon="network-wired" href="/en/develop/api-proxy-configuration/settings">
