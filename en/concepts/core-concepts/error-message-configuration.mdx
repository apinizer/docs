...
For timezone-aware or custom timestamp formatting, use the **Date/Time Helper (`dtf`)** function. See [Date/Time Helper (`dtf`)](/en/concepts/core-concepts/dynamic-variables#date-time-helper-dtf) in the Dynamic Variables reference for details.
...
**Example template (UTC timestamp):**
```json
{
  "error": {
...
**Example response (UTC timestamp):**
```json
{
  "error": {
    "code": "ERR-035",
...
**Alternative with ISO offset timestamp:**

To include a timestamp with timezone information in ISO 8601 format with offset, use the Date/Time Helper (`dtf`):
...
```json
{
  "error": {
    "code": "#{error.defaultErrorCode}",
    "message": "#{error.defaultMessage}",
    "timestamp": "#{dtf.format('+03:00','yyyy-MM-dd\'T\'HH:mm:ssXXX')}",
    "correlationId": "#{message.correlationId}",
    "path": "#{request.uri}"
  }
}
...
**Example response (ISO offset timestamp):**
```json
{
  "error": {
    "code": "ERR-035",
    "message": "Authentication policy failed! Invalid API Key/Username and Password pair for Authenticator (SecretManager)!",
    "timestamp": "2026-07-14T11:57:13+03:00",
    "correlationId": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
    "path": "/api/v1/users"
  }
}
