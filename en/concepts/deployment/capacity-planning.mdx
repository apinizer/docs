...
Apinizer uses an **automatic memory profile system** that selects the GC algorithm and heap percentage based on the container's memory:
...
**Example (8 GB container with automatic profile):**
* Node RAM: 8 GB
* Container RAM Limit: 8 GB
* Auto-selected GC: ZGC
* Auto-selected MaxRAMPercentage: 70%
* JVM RAM to Use: 8 GB × 0.70 = **5.6 GB**
* Operating System, Native Memory, and Other: 8 GB × 0.30 = **2.4 GB**
...
The automatic profile does not require any `JAVA_OPTS` configuration. If you want to override, you can set `JAVA_OPTS` with specific GC and heap values.

<Info>
For detailed information about GC selection, safety caps, and manual override options, see [JVM Garbage Collector Tuning](/en/operations/administrator-guides/jvm-garbage-collector-tuning).
</Info>
...
Worker node performance depends on thread, connection pool, timeout, and async executor parameters. These settings are configured from the API Gateway Environment Settings screen.

For tier-based (Tier 1/2/3) recommended values, CPU-based thread calculation tables, and detailed explanations of connection pool and timeout parameters, see [Gateway Performance Tuning](/en/operations/administrator-guides/gateway-performance-tuning).

**JVM Parameters:**
* Automatic profile is recommended (no `JAVA_OPTS` needed). See [JVM Garbage Collector Tuning](/en/operations/administrator-guides/jvm-garbage-collector-tuning) for details.
