...
|----|-----------------|-----|-----|------|------------|
| Server 1 | Ubuntu Server 24.04 LTS / RHEL 9.x | 8 | 8GB | 100GB | Kubernetes Control-Plane, Manager |
| Server 2 | Ubuntu Server 24.04 LTS / RHEL 9.x | 6 | 8GB | 80GB | Kubernetes Worker |
| Server 3 | Ubuntu Server 24.04 LTS / RHEL 9.x | 6 | 8GB | 80GB | Kubernetes Worker |
| Server 4 | Ubuntu Server 24.04 LTS / RHEL 9.x | 6 | 8GB | 80GB | Kubernetes Worker |
| Server 5 | Ubuntu Server 24.04 LTS / RHEL 9.x | 6 | 8GB | 80GB | Kubernetes Worker |
| Server 6 | Ubuntu Server 24.04 LTS / RHEL 9.x | 8 | 32GB | 1TB | Elasticsearch (Master+Data) |
| Server 7 | Ubuntu Server 24.04 LTS / RHEL 9.x | 4 | 8GB | 200GB | MongoDB Single Instance (must be configured as Replica Set) |

<Note>
  Worker node server resources include the gateway pod limits (W4: 4 core / 4 GB) plus a **~2 core / ~4 GB** reservation for Kubernetes system components (kubelet, kube-proxy, container runtime, operating system).
</Note>
...
* **Server 1**: Hosts Kubernetes Control-Plane and [Manager](/en/concepts/core-components/api-manager) components.
...
* **Server 6**: [Elasticsearch](/en/concepts/core-components/analytics-engine) — dedicated node for log and analytics data.
* **Server 7**: [MongoDB](/en/concepts/core-concepts/connections) — configuration database (must be configured as Replica Set).
...
This topology runs with 4 W4 worker nodes. Benchmark-based planning values:

| Metric | Value |
|--------|-------|
| **Worker Tier** | W4 (4 core / 4 GB RAM) × 4 nodes |
| **Daily Planning Capacity** | ~20M requests/day (4 × 5M/day) |
| **Usage Purpose** | Production — medium to high traffic environments |

<Note>
  **Rolling Updates and Capacity:** During zero-downtime rolling updates, Kubernetes temporarily takes one node offline. In this topology, 3 active nodes (~15M requests/day) remain during an update. For zero-downtime updates under full load, it is recommended to have total worker node capacity at least 2× your traffic requirements, or to add extra nodes during the update window.
</Note>
...
This topology is ideal for medium to high traffic production environments (~20M requests/day, 4×W4), worker load balancing requirements, and situations requiring budget optimization.
