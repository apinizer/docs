...
<CardGroup cols={2}>
  ...
  <Card title="Test Console Audit Records" icon="flask">
    ...
    * Client IP (the IP of the user who ran the test from the Test Console, honoring the X-Forwarded-For header behind a load balancer or reverse proxy; empty on records written before the field was introduced; also available as a filter)
    * Date range filtering (dates can be picked from the calendar or typed directly)
  </Card>
</CardGroup>
