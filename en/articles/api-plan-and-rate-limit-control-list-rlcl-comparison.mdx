...
The Rate Limit Control List compared here was retired in 2026.09 and its screen was removed in 2026.09.3. Its role is now played by [Limit Plans](/en/ai-gateway/limit-plans) and assignments, and a portal Subscription Plan references a platform-scope Limit Plan directly. The comparison is kept for the reasoning behind the two models.
:::
...
- Flexible target audience targeting with operator-based conditions (equals, contains, starts with, etc.)
- Flexible execution order (can run at six different points in the request processing flow)
- Differentiated handling of out-of-target identities (block or apply a shared or per-identity general quota)
- Custom error message templates with variables (per error or plan-wide)
- Response behavior when limits are exceeded

For detailed information about RLCL configuration and features, see the [Rate Limit Control List admin guide](/en/admin/identity-management/rate-limit-control-list).
...
- Suspicious IP blocks (starting with `123.45.`): Maximum 5 requests per second
- Premium customers: Maximum 50 requests per second, but 30 outside business hours
- Special partners (starting with `API-XYZ-`): Maximum 100 requests per second
