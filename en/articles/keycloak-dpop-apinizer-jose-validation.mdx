...
1. Keycloak: with version 26.1.4 and later no extra configuration is needed; `cnf.jkt` is embedded into the token when the token request carries a DPoP proof.
