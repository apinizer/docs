...
:::note Prefer a managed integration instead?
Apinizer can also connect to Keycloak as a managed [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider): once configured, it synchronizes Keycloak's users, groups, and roles into Credential records and performs centralized token validation for every policy that references it, without the manual JOSE Validation setup described in this article. The JOSE-based DPoP approach below remains fully supported for scenarios that specifically need RFC 9449 proof validation — use whichever fits your integration.
:::
