...
| --- | --- | --- |
| **OTel Operator** | `monitoring` | Reads the Instrumentation CR (Custom Resource) and injects a Java agent init container into the gateway deployment |
...
| **Tempo** | `monitoring` | Trace store — WAL (Write-Ahead Log) + blocks; the trace source for Grafana Explore |
| **Prometheus Agent** | `monitoring` | Scrapes the Collector's `:8889` and forwards via `remote_write` to the central Prometheus; keeps no local TSDB (Time Series Database) |
...
So what triggers this whole mechanism? A single annotation you need to add to your application's deployment pod template:
...
| --- | --- | --- |
| `memory_limiter` | Protects Collector memory, prevents OOM (Out of Memory) | Under load the gateway can produce thousands of spans/second |
...
<CardGroup cols={2}>
  ...
  <Card title="Storage" icon="database">
    A static PV/PVC (PersistentVolume/PersistentVolumeClaim) or a StorageClass for Tempo.
  </Card>
</CardGroup>
...
Then we wait until the operator pod is `Running` and the CRDs (Custom Resource Definitions) show up:
...
:::warning Minimum memory for a Worker running the OpenTelemetry Java agent: 2 GiB
The Apinizer startup script sets the JVM Metaspace ceiling in tiers based on the container memory limit (1 GiB → 128m, 2 GiB → 192m, 4 GiB → 256m). The OpenTelemetry Java agent adds roughly 15-25 MB of instrumentation classes to Metaspace; on a 1 GiB Worker this overhead, combined with the real proxy and policy class load, can fill the 128m ceiling and cause deploy/management request failures with `java.lang.OutOfMemoryError: Metaspace`. Heap percentage settings do not grow Metaspace.

- Set the memory limit of any Worker pod that will receive the agent to at least **2 GiB**; **4 GiB** is recommended for production.
- A 1 GiB limit is not sufficient for any Worker; even with a single CPU core the JVM does not start on it. The minimum is 2 GiB with or without the agent.
- Raising only `-XX:MaxMetaspaceSize` via `JAVA_OPTS` without raising the pod limit can push total native memory over the container limit (OOMKilled risk); the proper fix is increasing the memory limit.

See [Kubernetes Environment](/en/setup/kubernetes) for resource configuration and [Memory Leaks and OOM Errors](/en/operations/common-problems-solutions/memory-leaks-oom-errors) for symptoms and diagnosis.
:::
...
First, the service account and RBAC (Role-Based Access Control) definitions for the agent:
