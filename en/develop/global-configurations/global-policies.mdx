...
The Global Policies interface lists and manages not only classic API proxy policies but also **AI Gateway policies**, centrally. Some policies can be used both on classic API proxies and in the AI flow, while others are specific to AI, MCP, and A2A proxies. Five of the AI guardrail policies — personal-data masking, prompt protection, data-loss protection, off-topic protection, and retry-storm (loop) protection — can be attached to an AI, MCP, or A2A proxy; the rest (for example token quota, retrieval-augmented context) stay AI-proxy only — see [Advanced Guardrails: MCP and A2A Gateways](/en/ai-gateway/advanced-guardrails#mcp-and-a2a-gateways) for the full breakdown. AI policies are not added to classic API Proxy Groups; they are applied to AI, MCP, and A2A proxies.
