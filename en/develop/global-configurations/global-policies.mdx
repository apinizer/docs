...
The Global Policies interface lists and manages not only classic API proxy policies but also **AI Gateway policies**, centrally. Some policies can be used both on classic API proxies and in the AI flow, while others are specific to AI proxies (for example personal-data masking, prompt protection, token quota, retrieval-augmented context). AI policies are not added to classic API Proxy Groups; they are applied to AI proxies.
