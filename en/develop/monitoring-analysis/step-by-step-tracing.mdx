...
A Custom Query used as a tracing filter now matches the same requests the same query returns on the
[Advanced Search](/en/analytic/query-editor/queries) screen — an **OR** condition group traces only
requests matching at least one of its conditions, and a **NOT** group with more than one condition
excludes a request only when none of its conditions match. A query built from a single **AND** group, or
a **NOT** group with only one condition, is unaffected.
