...
<Steps>
  ...
  <Step title="Test the connection">
    Pick an environment in the connector's top action bar, then use **Test Connection** next to it. This is not a TCP ping — Apinizer builds a temporary exporter from the saved settings and sends one real synthetic span to the collector, proving the collector actually accepts Apinizer's OTLP payload (auth, TLS, and path included) before you deploy it.
  </Step>
