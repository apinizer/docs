...
This switch is only meaningful while the connector's [**Which settings apply to log fields**](/en/admin/server-management/adding-connectors-to-gateway-environments#which-settings-apply-to-log-fields) selection is **This connector** — the default and today's behaviour. If the administrator has instead set that connector to **API Proxy**, every proxy already logs according to its own settings and this switch has nothing left to override.

While the connector precedence is **This connector**, the switch still does not work alone: it takes effect only on connectors where the administrator has also switched on **Allow API Proxy override** when [adding the connector to the environment](/en/admin/server-management/adding-connectors-to-gateway-environments#allowing-an-api-proxy-to-override-these-restrictions). On every other connector the switch is simply ignored and the environment rule keeps applying. Two switches, two owners: the administrator decides where an exception is possible at all, the proxy owner decides whether to use it.
