...
Each rule first decides **what to look for**, then **what to do with it**. There are three ways to find data:

| Rule Type | Matches | Use It When |
|-----------|---------|-------------|
| **Element name** | A field, header or query parameter with a given name — whatever its value | The sensitive data always lives in a known field, such as `password` or `cardNumber`. This is the only type that also covers logged headers and parameters |
| **Regular expression** | Any value in the body matching your pattern | The format is specific to your APIs — order codes, internal identifiers |
| **Built-in PII type** | A known personal-data format, wherever it appears | The value can land in any field, or in free text such as an AI prompt |
