...
:::warning Capacity planning — concurrent streams × body limit
...
:::note Streams that stay out of scope
...
When a database connector is selected as a log target, the schema prefix and custom table names for log tables are configured in the **Connection definition**. See [Database Connection](/en/integrations/connection-management/database) for details.
...
- **Privacy**: When enabled, a table defines masking or similar rules by element name and operation; use **+ Add** for environment-specific privacy definitions. If none exist, an empty-state message is shown.
  ...
  <img
    ...
    src="/images/develop/api-proxy-konfigurasyonu/log-ayarlari-sekmesi-elasticsearch.png" alt="Log Settings Tab" style={{ maxWidth: '100%', height: 'auto' }}
...
| Rule Type | Matches | Use It When |
|-----------|---------|-------------|
| **Element name** | A field, header or query parameter with a given name — whatever its value | The sensitive data always lives in a known field, such as `password` or `cardNumber`. This is the only type that matches by name, and it covers logged headers, parameters and the query string of the logged URL |
| **Regular expression** | Any text matching your pattern — in the body, in the request path or in the query string | The format is specific to your APIs — order codes, internal identifiers |
| **Built-in PII type** | A known personal-data format, wherever it appears | The value can land in any field, in a path segment such as `/user/12345678901`, or in free text such as an AI prompt |
...
- **Detect** — notes that the data was present, without changing the logged content in any way

**Detect never masks, deletes, hashes or encrypts — for any rule type** (element name, regular expression, or built-in PII type). It only marks that a match occurred; the value stays in the log exactly as it arrived. There is no separate report screen for these matches — the mark is carried on the traffic log record itself, alongside the value it was found next to.

:::warning[Upgrade note: Element name + Detect does not mask]
A rule combining **Element name** with **Detect** logs the field's value unchanged — it does not mask it. If a field such as `password` or `cardNumber` needs to be hidden from traffic logs, choose the **Mask** (or Delete/Hash/Encrypt) operation for that rule instead of Detect.
:::
...
:::info
How to read an encrypted value back is described on the [Advanced Guardrails](/en/ai-gateway/advanced-guardrails) page — the same encryption and key apply to this screen.
:::
...
**A regular expression or built-in PII rule also covers the parsed header and parameter lists.** Previously only an **Element name** rule reached the header and parameter lists shown per message region (**Request from Client**, **Request to Target**, **Response from Target**, **Response to Client** — parameters apply to the request legs only, since a response has no query parameters). A regular expression or built-in PII rule now scans the value of every logged header and parameter in all four legs as well, the same way it already scanned the URL. The pattern is applied to the value only; the header or parameter name is never changed. The same scan also applies to the header and parameter snapshots recorded for a policy trace (**Policy Capture** and **Policy Execution**), so a value masked in the traffic record is masked the same way there too. One side effect to be aware of: a built-in **IP Address** or **URL** rule now also matches values carried in headers such as `X-Forwarded-For`, `Referer` or `Location` — the log copy of those headers is masked accordingly, while the actual request and response sent over the wire are never touched.
...
For detailed information, you can refer to the [Adding Log Connectors](/en/admin/server-management/adding-connectors-to-gateway-environments) page.
...
## Overriding Environment Restrictions {#overriding-environment-restrictions}
...
The [data retention profile](#data-retention-profile) is not affected either. While **No Payload** or **No Persist** is in effect, no body is captured at all, so there is nothing for the override to reopen.
:::
...
The header/body/parameter settings on this tab operate within the [data retention profile](/en/admin/system-settings/system-settings-and-limits#data-retention-profile) inherited from the project and global levels — you can make the profile stricter at the proxy level, but choosing a looser profile than the one you inherit is rejected on save.
