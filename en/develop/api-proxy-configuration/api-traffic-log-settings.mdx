...
**The URL is covered too, but by different rule types.** The logged query string is masked both by name (an `id` rule hides `?id=...` exactly as it hides the parameter in the parameter list) and by pattern. The request path is masked **only** by regular expression or built-in PII type, because a path segment has no name to match on: to hide the identifier in `/user/12345678901`, define a built-in TCKN rule or a pattern such as `[0-9]{5,}`. The proxy's own routing address is deliberately left alone — it carries no request data and is the key traffic lists and reports group by. Values that reach the URL percent-encoded (`user%40example.com`) are decoded before the pattern is applied, so a built-in e-mail rule catches them there too; if no rule matches, the logged URL is written exactly as it arrived.
...
The switch does not work alone. It takes effect only on connectors where the administrator has also switched on **Allow API Proxy override** when [adding the connector to the environment](/en/admin/server-management/adding-connectors-to-gateway-environments#allowing-an-api-proxy-to-override-these-restrictions). On every other connector the switch is simply ignored and the environment rule keeps applying. Two switches, two owners: the administrator decides where an exception is possible at all, the proxy owner decides whether to use it.
