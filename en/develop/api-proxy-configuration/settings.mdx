...
<img
  ...
  alt="API Proxy General Settings"
  width="1024"
/>
...
| Access-Control-Allow-Methods | Contains the HTTP method or methods allowed when accessing the server. The methods that the API Proxy allows for access are selected by marking them from the list: GET, POST, PUT, HEAD, OPTIONS, DELETE, PATCH, TRACE, ALL, QUERY |
| **Response Headers** | |
...
:::note
Only successful responses are cached: error responses with a status code of `400` and above (4xx/5xx) are never cached and are produced fresh from the Backend API on every request.
:::
...
Idempotency lets a client safely retry a state-changing request without the risk of it being applied twice. The client sends a unique **`Idempotency-Key`** with the request; Apinizer records the key in the distributed cache, forwards the first request, stores its response, and — for any later request carrying the same key — returns the stored response instead of hitting the Backend again. This protects against duplicate submissions from network retries, double-clicks, or client-side retries.

:::note
Idempotency applies only to the configured HTTP methods (by default `POST`, `PUT`, `PATCH`, `DELETE`). Only successful responses (`2xx`–`3xx`) are stored as completed; a `4xx`/`5xx` result releases the key so the request can be retried.
:::
...
```mermaid
flowchart TD
    A[Request arrives] --> B{HTTP method in scope?<br/>POST / PUT / PATCH / DELETE}
    B -- No --> P[Process normally — no idempotency]
    B -- Yes --> C{Idempotency-Key present?}
    C -- No --> D{Key required?}
    D -- Yes --> E[missingKeyHttpStatus — 400]
    D -- No --> P
    C -- Yes --> F[Claim the key in cache]
    F --> G{Claim result}
    G -- New --> H[Lock in-flight → forward to backend]
    H --> I{Response 2xx-3xx?}
    I -- Yes --> J[Store response — COMPLETED<br/>for retentionInDays] --> Z[Return response]
    I -- No --> Y[FAILED — lock released, retryable] --> Z
    G -- Concurrent in-flight --> K[conflictHttpStatus — 409]
    G -- Payload mismatch --> L[mismatchHttpStatus — 422]
    G -- Completed --> M{replayCompletedResponse on?}
    M -- Yes --> N[Return stored response — replay]
    M -- No --> K
    F -. Cache error .-> O{handlingAction}
    O -- STOP --> S[Reject request]
    O -- CONTINUE --> P
```

### Idempotency Settings Fields

| Field | Description |
|---|---|
| **Idempotency Active** | Main switch. When off (default) no idempotency processing runs. |
| **Key Type** | Where the idempotency key comes from: **Header** (default, read from the header below) or **Custom** (built from the selected request variables, joined together). |
| **Header Name** | The request header carrying the key when Key Type = Header. Default `Idempotency-Key`. |
| **Variable List** | The request variables that form the key when Key Type = Custom. |
| **Key Required** | When on, a request without a key is rejected with the *Missing Key HTTP Status*. When off, keyless requests pass through without idempotency. |
| **Missing Key HTTP Status** | Status returned when the key is required but absent. Default `400`. |
| **Applicable HTTP Methods** | The methods idempotency applies to. Default `POST`, `PUT`, `PATCH`, `DELETE`. |
| **Payload Hash** | When on (default), the request body is hashed and stored with the key; a later request that reuses the key with a **different** body is rejected as a mismatch. |
| **Processing Timeout (seconds)** | How long the in-flight lock is held before it is considered abandoned. Default `60`. |
| **Retention (days)** | How long a completed response is kept for replay (1–30, default `7`). |
| **Storage Type** | **Local** (this Gateway node only) or **Distributed** (shared across nodes, default). |
| **Cache Connection Timeout (seconds)** | Connection timeout to the idempotency cache. Default `3`. |
| **Cache Error Action** | When the cache is unreachable: **Continue** lets the request pass without idempotency, **Stop** rejects it. |
| **Conflict HTTP Status** | Returned when the same key is still being processed (concurrent duplicate). Default `409`. |
| **Mismatch HTTP Status** | Returned when the key is reused with a different payload. Default `422`. |
| **Replay Completed Response** | When on (default), a duplicate of a completed request receives the stored response. When off, the duplicate is rejected with the *Conflict HTTP Status*. |
| **Custom Error Bodies** | The response body and content type for the missing-key, conflict, and mismatch cases can be customized; each body can also be disabled to return status-only. |

## Customizing XML Error Response Template

:::info

For information on where error response templates fit within the multi-layered configuration system, priority order and scenario examples, see the [Error Message Configuration Guide](/en/concepts/core-concepts/error-message-configuration) page.
...
| **Backend Proxy** | Access requests to the specification file are forwarded to the Backend API, and the actual specification file is served through the Backend API. |
...
Maintenance Mode is one of the settings a [Global Settings Group](/en/develop/global-configurations/global-settings-group) can carry, the same way CORS or Cache can. When an API Proxy or API Proxy Group is attached to a settings group that defines Maintenance Mode, this tab shows the **global settings are active** warning described above, the toggle and **Configure** button are disabled, and the values shown come from the group. Attaching a proxy that already has its own Maintenance Mode configured to such a group replaces the proxy's own configuration with the group's the next time the group (or the attachment) is saved; detaching the proxy from the group leaves the last-applied values in place on the proxy — Maintenance Mode does not turn itself back off. Attaching or detaching a proxy from a settings group that defines Maintenance Mode also marks every environment the proxy is deployed to as **Redeploy Required**, the same way any other settings-group change does.
