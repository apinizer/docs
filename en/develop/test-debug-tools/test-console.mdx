...
<Frame caption="Settings - Secure Connection Settings">
  <img src="/images/develop/test-debug-araclari/test-console/test-console-settings-secure.png" alt="Secure Connection Settings" width="900" height="500" />
</Frame>

The **Secure Connection Settings** section includes:

* **Enable**: Toggles secure connection settings on or off. When enabled, mTLS-based authentication can be configured with KeyStore and TrustStore.
* **Configure**: Opens the **Edit Secure Connection Settings** dialog when clicked.

The Settings tab displays a summary of the current configuration such as Connection Timeout, KeyStore, TrustStore, Supported Protocols, and Hostname Verifier.

<Frame caption="Edit Secure Connection Settings">
  <img src="/images/develop/test-debug-araclari/test-console/test-console-settings-edit.png" alt="Edit Secure Connection Settings" width="900" height="600" />
</Frame>
...
| Field | Description |
|------|-------------|
| **Timeout** | Duration to wait for a response after the request is sent. |
| **Skip SSL Verification** | When checked, server certificate verification is bypassed. Useful for environments using self-signed certificates. When enabled, TrustStore, Certificate, and KeyStore panels are hidden. |
| **Connection Timeout** | Maximum time in seconds to wait for the SSL/mTLS connection. Overrides general timeout when SSL/mTLS is enabled. |
