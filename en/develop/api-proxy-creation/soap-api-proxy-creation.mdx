...
<Info>
  **Loading from WSDL URL** When loading a WSDL file from URL, if the spec URL requires authentication, you can enable the **Use Spec Authorization** option to enter custom headers or **Basic Auth** (username/password) credentials. You can also configure the connection timeout (5-120 seconds, default: 30s) in the **SSL/Certificate Settings** section. Authorization credentials are also automatically used when downloading sub-schema (XSD import) files within the WSDL.
</Info>
