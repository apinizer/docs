...
| **Truststore** | Truststore is used to verify the server certificate. Truststore content should contain: The root CA certificate of the target server, Intermediate CA certificates (if any) should be present. A truststore is not a separate record type: your JKS/PKCS12 file is uploaded as a Key Store record under **Administration** → **Secret** → **Key Stores**, and that record is what you select here. |
