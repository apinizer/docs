...
| **Max Redirect Count** | Maximum number of backend redirects (3xx + Location) followed for a single request. The HTTP client itself follows at most 20 redirects, so the effective limit is **min(value, 20)**. 0 or a negative value disables the Apinizer-side limit; only the client's hard cap of 20 applies. |
