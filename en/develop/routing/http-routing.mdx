...
| **Preserve HTTP Method and Body on Redirect** | By default, 301/302/303 redirects downgrade the request to GET and drop the body. Enable this to preserve the original HTTP method and body on all redirects (301/302/303/307/308). Applies to HTTP/1.1 backend connections. |
...
**Preserve HTTP Method and Body on Redirect** only applies to **HTTP/1.1** backend connections. If the API Proxy uses the streaming/HTTP2 (OkHttp) backend connection path, this setting has no effect — 301/302/303 redirects still downgrade the request to GET regardless of this option.
