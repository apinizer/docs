...
The **Force Decompress Download** option is in the Download section. When enabled, downloaded files are delivered uncompressed even when the backend returns them compressed (e.g. gzip); downloadable content is decompressed before it is sent to the client. **The response body is not affected** by this setting; it applies only to downloaded files.
