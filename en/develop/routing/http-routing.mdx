...
When this option is enabled, downloadable content returned from the backend in compressed format (gzip, deflate, br, etc.) is automatically decompressed before being sent to the client. This ensures that even compressed files are downloaded in their uncompressed form.

<Info>
This setting only applies to downloaded files. The response body is not affected by this setting; compression handling for the response body is already managed correctly by default.
</Info>
