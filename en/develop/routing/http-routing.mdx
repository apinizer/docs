...
| **Validate After Inactivity** | Connections idle for this period (milliseconds) are validated before reuse. Tune according to backend load balancer keep-alive timeout. Default: 2000 ms. |
| **Idle Connection Timeout** | Connections idle longer than this period (seconds) are evicted from the pool. Tune according to backend load balancer keep-alive timeout. Default: 30 seconds. |
