...
<img
    ...
    alt="Edit Connection Settings dialog"
    width="1000"
    className="mr-auto"
/>
...
| **Connection Pool Wait Timeout** | Timeout duration used when requesting a connection from the connection pool. Given in seconds. |
| **Validate After Inactivity** | Connections idle for this period (milliseconds) are validated before reuse. Tune according to backend load balancer keep-alive timeout. Default: 2000 ms. **Note**: This setting applies per API Proxy only when **Connection Pool Management Type = Custom**. For **General** or **None** pool, the value from global System Settings is used; proxy-level values are ignored. |
| **Idle Connection Timeout** | Connections idle longer than this period (seconds) are evicted from the pool. Tune according to backend load balancer keep-alive timeout. Default: 30 seconds. **Note**: This setting applies per API Proxy only when **Connection Pool Management Type = Custom**. For **General** or **None** pool, the value from global System Settings is used; proxy-level values are ignored. |
...
| **Follow Redirects** | When HTTP redirect responses (3xx) are received from the backend, follows the redirect addresses returned. Additionally, instead of immediately returning an error to the client when timeout durations are exceeded or an error is returned from the Backend API, it is attempted to make the operation successful by sending the request again to the same address or other addresses. |
| **Activate Relative Redirect** | When this feature is enabled, redirect operations are performed to relative addresses. |
| **Max Redirect Count** | Maximum number of backend redirects (3xx + Location) followed for a single request. The HTTP client itself allows at most 20 follow-up requests in total, and authentication challenges (a 401/407 retried with credentials) count against that same cap together with redirects; so the effective limit is **min(value, 20)**, and lower when the backend also issues authentication challenges. 0 or a negative value disables the Apinizer-side limit; only the client's hard cap of 20 applies. |
| **Preserve HTTP Method and Body on Redirect** | By default, 301/302/303 redirects downgrade the request to GET and drop the body. Enable this to preserve the original HTTP method and body on all redirects (301/302/303/307/308). Applies only to HTTP/1.1 backend connections with streaming disabled — see the warning below. |
...
| **Hop-by-hop headers to delete** | Determines the information to be deleted from the header information coming during the request sent from client to backend. Note that some headers are always removed regardless of this list — see the **Always-Removed (Transport-Managed) Headers** info box below. |
...
**Connection Pool Dependency**: The **Validate After Inactivity** and **Idle Connection Timeout** settings are applied per API Proxy only when the **Connection Pool Management Type** is set to **Custom**. When **General** or **None** pool is selected, these values are taken from the global System Settings (System Settings > Gateway > Connection Settings), and any values entered at the API Proxy level are ignored. This is because the General pool is shared across all proxies, while Custom pool is specific to each proxy. Other timeout settings (Connect Timeout, Read Timeout, Connection Pool Wait Timeout) always apply per API Proxy regardless of the pool type.

:::

:::info

**Always-Removed (Transport-Managed) Headers**: Regardless of the **Hop-by-hop headers to delete** configuration, Apinizer Gateway always removes certain headers from the request forwarded to the backend, because these headers are managed by the gateway's own HTTP transport layer rather than by policy configuration:

- **Content-Length** and **Transfer-Encoding**: The gateway's HTTP client calculates the body boundaries itself, so these headers are always recalculated rather than forwarded as received.
- **Expect** (100-continue): The 100-continue handshake is connection-based (hop-by-hop), not end-to-end. The gateway already reads the full request body for policy processing, and the handshake with the backend is managed by the gateway's own HTTP client layer. The raw **Expect** header sent by the client is not forwarded to the backend — forwarding it as-is could otherwise cause a 417 Expectation Failed response, a hang, or a protocol error depending on the backend.

Other headers in the hop-by-hop list remain configurable; the transport-managed headers above are never forwarded to the backend even if they are removed from that list.

**Visibility in traffic logs**: In traffic log **region 1** (the request as received from the client), the **Expect** header sent by the client is shown as-is. In traffic log **region 2** (the request as sent to the backend), the **Expect** header is shown as removed — it does not appear in the header list.

**Impact on uploads**: There is no impact on uploads. The body is always sent to the backend in full; the only thing lost is the backend's optimization of rejecting a large body before receiving it.

:::

:::tip

**When to enable Preserve HTTP Method and Body on Redirect**: enable this setting when the backend responds with a 301/302/303 redirect — for example, redirecting an `http://` address to `https://` — and the original HTTP method and body (such as a POST or PUT payload) must reach the final address unchanged. A typical symptom of the default behavior is a REST-2-SOAP proxy receiving a backend error such as "No binding operation info while invoking unknown method with params unknown" after a redirect silently drops the request body.

In most cases, a simpler fix is to configure the backend address as the final destination address directly (for example, the `https://` address instead of the `http://` one). When no redirect occurs, this setting is not needed at all.

This setting can also be configured through APIops — see [Update Connection Settings](/api-reference/api-proxies/settings/update-connection-settings).

:::

:::warning

**Preserve HTTP Method and Body on Redirect** only applies to **HTTP/1.1** backend connections with streaming disabled. If the API Proxy has streaming enabled (for example, Server-Sent Events) or the [Backend HTTP Version](#backend-http-version) policy is set to a value other than HTTP/1.1 (for example, HTTP/2), this setting has no effect at all — 301/302/303 redirects on that path still downgrade the request to GET and drop the body, regardless of this option.

When this condition applies, the Routing screen shows a warning below the setting:

> This setting has no effect here: it only applies to HTTP/1.1 backend connections with streaming disabled. Disable streaming and set the HTTP version policy to HTTP/1.1 to preserve the method and body on redirect.

To make the setting effective, disable streaming and set the HTTP version policy to **HTTP/1.1 (Default)**. In most cases, an even simpler fix is to configure the backend address as the final destination address directly (for example, the `https://` address instead of the `http://` one) so that no redirect occurs at all.

If a redirect on this path would downgrade the request method, a warning is also written to the worker log, so the drop does not happen silently.
...
The Charset & Encoding Override feature allows explicit control over how request and response bodies are decoded and re-encoded, independent of the `Content-Encoding` and charset header values. When an override is configured, the header value is ignored and the configured value is applied instead.
...
| Field | Description |
|-------|-------------|
| **Charset Override** | Charset used to decode the request body received from the client and encode it before sending to the backend. Overrides the charset in the `Content-Type` header. Leave empty to use the default (UTF-8). |
| **Decompress Override** | Compression algorithm used to decompress the request body received from the client. Overrides the `Content-Encoding` header. Select **Not Apply (skip)** to skip decompression. |
| **Compress Override** | Compression algorithm used to compress the request body before sending to the backend. Select **Not Apply (skip)** to skip compression. |

### Response Line (Backend → Client)

| Field | Description |
|-------|-------------|
| **Charset Override** | Charset used to decode the response body received from the backend and encode it before sending to the client. Overrides the charset in the `Content-Type` header. |
| **Decompress Override** | Compression algorithm used to decompress the response body received from the backend. Overrides the `Content-Encoding` header. Select **Not Apply (skip)** to skip decompression. |
| **Compress Override** | Determines how the response body is compressed before being sent to the client. **Default: Auto by client Accept-Encoding** — the first method the client accepts is applied. **Mirror server response** option forwards the response with the same encoding the backend returned. If a specific algorithm is selected, that algorithm is always applied. Select **Not Apply (skip)** to disable compression. For detailed behavior, see [Content Encoding and Compression — Response Compression Behavior Modes](/en/concepts/core-concepts/content-encoding-and-compression#response-compression-behavior-modes). |

Decompress Override options: **GZIP**, **DEFLATE**, **BR (Brotli)**, **ZSTD**, **Not Apply (skip)**.
...
You can choose how query parameter values are encoded when forwarded to the backend. By default, special characters such as `/`, `+`, and `=` are encoded for safety; for example, `/` is sent as `%2F` in the address. Some backend services, however, expect these characters unencoded. In such cases, you can change the encoding behavior to suit your needs.

| Mode | Behavior |
|------|----------|
| **Standard (URL Encode)** _(default)_ | All special characters, including `/`, `+`, and `=`, are encoded. This is the behavior of all existing API Proxies and does not change. |
| **RFC 3986 (Preserved)** | Characters allowed in the query component of the address standard (`/`, `+`, `=`, `:`, `@`) are preserved as-is; only characters that genuinely require escaping are encoded. Suitable for backends that expect values such as Base64 or signed tokens to arrive unchanged. |
| **Raw (No Encoding)** | Values are forwarded without encoding. Only characters that could break the address (such as spaces or line breaks) are encoded for safety. You are responsible for the suitability of the forwarded values; in most cases, **RFC 3986 (Preserved)** is the safer choice. |

:::note
The default mode is **Standard**, and all existing API Proxies continue to work unchanged. You only need to change this setting if your backend expects query parameters to be unencoded.
:::

## Backend HTTP Version

You can choose which HTTP protocol version Apinizer uses when connecting to the backend. By default, Apinizer speaks HTTP/1.1 with the backend, exactly as before; you can opt in to negotiating HTTP/2 over TLS or, for cleartext backends known to support it, to speaking HTTP/2 directly without an upgrade step.

| Option | Behavior |
|--------|----------|
| **HTTP/1.1 (Default)** | Apinizer connects to the backend using HTTP/1.1. This is the behavior of all existing API Proxies and does not change. |
| **HTTP/2 (ALPN, falls back to HTTP/1.1)** | For `https` addresses, Apinizer negotiates the protocol with the backend during the TLS handshake (ALPN) and uses HTTP/2 if the backend supports it; if not, it automatically falls back to HTTP/1.1. For plain `http` addresses, this option behaves the same as **HTTP/1.1 (Default)**. |
| **HTTP/2 Prior Knowledge (h2c, cleartext)** | For plain `http` addresses, Apinizer speaks HTTP/2 directly, without an upgrade negotiation (h2c). Use this option only when the backend is known to support HTTP/2 over cleartext. |

:::note
The default option is **HTTP/1.1 (Default)**, and all existing API Proxies continue to work unchanged — no migration or additional configuration is required.
:::

:::warning
The following combinations are rejected on save, with a descriptive error message:

* Using **HTTP/2 (ALPN, falls back to HTTP/1.1)** or **HTTP/2 Prior Knowledge (h2c, cleartext)** together with NTLM authentication.
* Using **HTTP/2 Prior Knowledge (h2c, cleartext)** with an `https` address.
* Using **HTTP/2 Prior Knowledge (h2c, cleartext)** while a backend health check path is defined.

Regardless of the selected option, traffic mirror targets and health check requests always connect using HTTP/1.1; this setting only affects requests sent to the actual backend.
:::

:::note
API definition requests (`?wsdl`, `?wadl`, `?swagger`, `?openapi`, `?xsd=...`) are **not affected** by this setting. Apinizer serves them from the definition content it stores itself, so they never reach the backend. The only exception is when the API Proxy's **Spec Access Type** is set to **Backend Proxy**: the definition request is then forwarded to the backend and therefore does follow the **Backend HTTP Version** setting.
:::
...
| Field | Description |
|------|-------------|
| **Skip SSL Verification** | When checked, server certificate verification is bypassed and all certificates are trusted. When this option is enabled, TrustStore, KeyStore, and protocol settings are hidden. Useful for environments using self-signed certificates. |
...
:::note[A truststore is not a separate record type]
The **Truststore** selector on this screen lists exactly the same records as the **Keystore** selector: both are chosen from the Key Store records under **Administration** → **Secret** → **Key Stores**. There is no separately created "truststore" record in the product; upload your JKS or PKCS12 truststore file as a Key Store record, then select it here. The plus button next to the selector does the same thing without leaving this screen.

The **Truststore** tab on **Secret** → **Certificates** is a different thing: it shows the running truststore content of the selected environment as read-only, and nothing is selected from there.
:::
...
:::warning The request is not sent when the configuration fails

When mTLS is enabled and the client certificate cannot be loaded — the selected Keystore is not found in the environment, the file cannot be opened, or its password is wrong — the request is **not sent** to the target service. The call is stopped with error code `ERR-304` and HTTP 502, and the **Failure Reason** field on the API Traffic screen shows **mTLS Configuration Failure**, reported separately from **TLS Handshake Failure**.

In earlier versions the request was sent without a client certificate in the same situation: if the target service did not require a certificate the call succeeded with no client identity at all, and if it did, only the peer's handshake error was visible.

You can customize the text returned to the client on the **API Proxy > Error Messages** screen. Which keystore, which environment and which alias failed is written to the worker log at ERROR level.

This check does not apply when **Skip SSL Verification** is checked; that option already disables the client certificate and its behavior is unchanged.
...
When an error is thrown from policies, Apinizer sends the default error message to the user if it has not been customized. Click to learn more about customizing error messages.[See.](/en/concepts/core-concepts/what-is-policy#template-based-dynamic-error-messages)

:::info

For a complete guide on all layers, workflow and scenario examples of the error message configuration system, see the [Error Message Configuration Guide](/en/concepts/core-concepts/error-message-configuration) page.
