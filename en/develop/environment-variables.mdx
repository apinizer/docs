...
| Feature | Description |
|---------|-------------|
| **Location** | **MCP Gateway** — an MCP Gateway's MCP Routing tab, or the MCP Tool Call / MCP Tool Invoke policy's inline server fields (Endpoint, Bearer Token or OAuth2 client credentials) · **A2A Gateway** — an A2A Gateway's A2A Routing tab, or the A2A Agent Call / A2A Agent Invoke policy's inline agent fields (Endpoint, Bearer Token or OAuth2 client credentials) · **AI Gateway > LLM Providers** (Endpoint URL, API Key, API Secret, Organization ID) · **AI Gateway > RAG > Vector DBs** tab (Default Collection Name) |
...
**Usage Scenario:** To run the same AI Proxy configuration with different LLM provider keys or different MCP/A2A server credentials in different environments
...
**Usage Scenario:** To run the same AI Proxy configuration with different RAG collection names in different environments
