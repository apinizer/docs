...
**4. AI Gateway — Copy to Clipboard via Button (MCP Gateway, LLM Providers, Vector DBs)**

**Usage Area:** Inbound authentication settings and LLM/VectorDB connection information in the AI Gateway module

| Feature | Description |
|---------|-------------|
| **Location** | **AI Gateway > MCP Gateway > Inbound Servers** (Virtual API Key, JWKS URL, Issuer, Audience fields) · **AI Gateway > LLM Providers** (Endpoint URL, API Key, API Secret, Organization ID) · **AI Gateway > Semantic Cache > Vector DBs** (Default Collection Name) |
...
| **Secret Usage** | Sensitive fields such as Virtual API Key, API Key and API Secret are stored as secrets; when a field holds an environment variable expression (typed or pasted), encryption is skipped and the value is resolved from the environment variable at runtime |

**Usage Scenario:** To run the same AI Proxy configuration with different LLM provider keys or different MCP virtual API keys in different environments

**5. AI Gateway — Manual Entry (A2A Inbound Agents, AI RAG Injection Policy)**

**Usage Area:** Inbound authentication setting of the A2A Gateway and the connection/template fields of the AI RAG Injection policy

| Feature | Description |
|---------|-------------|
| **Location** | **AI Gateway > A2A Gateway > Inbound Agents** (Virtual API Key) · AI RAG Injection policy (Collection Name, Injection Template, Embedding Model) |
| **Fields** | The corresponding text inputs on the screens above |
| **Usage** | Environment-based management of authentication and RAG connection information |
...
| **Secret Usage** | Virtual API Key is stored as a secret as a sensitive field; when a field holds an environment variable expression, encryption is skipped and the value is resolved from the environment variable at runtime |

**Usage Scenario:** To run the same AI Proxy configuration with different A2A virtual API keys or different RAG collection names in different environments
