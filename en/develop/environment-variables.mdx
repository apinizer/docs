...
| Feature | Description |
|---------|-------------|
| **Location** | **AI Gateway > MCP Gateway > Inbound Servers** (API Key, JWKS URL, Issuer, Audience fields) · **AI Gateway > LLM Providers** (Endpoint URL, API Key, API Secret, Organization ID) · **AI Gateway > Semantic Cache > Vector DBs** (Default Collection Name) |
...
| **Secret Usage** | Sensitive fields such as API Key and API Secret are stored as secrets; when a field holds an environment variable expression (typed or pasted), encryption is skipped and the value is resolved from the environment variable at runtime |

**Usage Scenario:** To run the same AI Proxy configuration with different LLM provider keys or different MCP API keys in different environments
...
| Feature | Description |
|---------|-------------|
| **Location** | **AI Gateway > A2A Gateway > Inbound Agents** (API Key) · AI RAG Injection policy (Collection Name, Injection Template, Embedding Model) |
| **Fields** | The corresponding text inputs on the screens above |
...
| **Secret Usage** | API Key is stored as a secret as a sensitive field; when a field holds an environment variable expression, encryption is skipped and the value is resolved from the environment variable at runtime |

**Usage Scenario:** To run the same AI Proxy configuration with different A2A API keys or different RAG collection names in different environments
