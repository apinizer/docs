...
**Usage Area:** Outbound connection authentication settings in the AI Gateway module, plus LLM/VectorDB connection information

| Feature | Description |
|---------|-------------|
| **Location** | **MCP Gateway** outbound connection form (Endpoint, Bearer Token or OAuth2 client credentials) · **A2A Gateway** outbound connection form (Endpoint, Bearer Token or OAuth2 client credentials) · **AI Gateway > LLM Providers** (Endpoint URL, API Key, API Secret, Organization ID) · **AI Gateway > RAG > Vector DBs** tab (Default Collection Name) |
...
**Usage Scenario:** To run the same AI Proxy configuration with different LLM provider keys or different MCP/A2A outbound connection credentials in different environments
