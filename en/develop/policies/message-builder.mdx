...
<img
  ...
  alt="Message Builder global policy Definition tab, Rows section and Add menu with Form and Template options"
  width="1024"
/>
...
<img
  ...
  alt="Message Builder row edit Form mode: Source and Target Variable, Default Value, Required, Continue on Error"
  width="1024"
/>
...
<img
  ...
  alt="Message Builder row edit Template mode: Template, Target Variable, Continue on Error"
  width="1024"
/>
...
In JEXL expressions, the `dtf` helper function provides timezone-aware date/time formatting. By default, `#{dateTime.formattedText}` returns UTC time in a fixed format. Use `dtf` to get time in a specific timezone or apply custom formatting:
...
#{dtf.format('+03:00','yyyy-MM-dd\'T\'HH:mm:ssXXX')}           → 2026-07-14T11:57:13+03:00
#{dtf.format('Europe/Istanbul')}                                → 2026-03-02 15:30:45
#{dtf.epochMillis()}                                            → 1740912645000
```

For full documentation including zone parameter formats, pattern syntax, and escape rules, see [Date/Time Helper (`dtf`)](/en/concepts/core-concepts/dynamic-variables#date-time-helper-dtf) in the Dynamic Variables reference.
