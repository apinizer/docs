...
| **Step 8: Customize Error Message (Optional)** | - Go to the **Error Message Customization** tab.<br />- Customize the message returned when the policy throws an error.<br />- Placeholders such as `#{error.defaultErrorCode}`, `#{message.correlationId}` can be used.<br /><br />For details: [Template-Based Error Messages](/en/concepts/core-concepts/what-is-policy#template-based-dynamic-error-messages) |
