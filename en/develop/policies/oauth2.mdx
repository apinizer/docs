...
| **Step 4: OAuth 2 Flow Parameters** | - **Managed From This Policy:** Determine whether the policy will manage token management.<br />- **Grant Type:** Select CLIENT_CREDENTIALS or PASSWORD.<br />- **Identity Role Group:** In PASSWORD flow, select identity provider from Secret Manager, LDAP, Database, API, or OIDC. Selecting OIDC opens a searchable list of [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) records (name, description, vendor type); a Generic-vendor provider is a valid choice here too, since only its token validation is used.<br />- **Check Client IP Address:** Make token's source IP match mandatory in Secret Manager flow. |
