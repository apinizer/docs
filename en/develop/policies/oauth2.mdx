...
- Integrates identity and role sources with Secret Manager, API, LDAP, database, or [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) sources, making authorization processes compatible.
...
- **Identity Provider Integration**: Provides authentication flexibility by working with Secret Manager, Identity Role Group, LDAP, API, database, or OIDC-based identity sources.
...
<img src="/images/develop/politikalar/Oauth2.png" alt="OAuth 2 Authentication Policy" width="1000" />
...
| **Step 4: OAuth 2 Flow Parameters** | - **Managed From This Policy:** Determine whether the policy will manage token management.<br />- **Grant Type:** Select CLIENT_CREDENTIALS or PASSWORD.<br />- **Identity Role Group:** Shown for every grant type. In PASSWORD flow, select identity provider from Secret Manager, LDAP, Database, API, or OIDC; in CLIENT_CREDENTIALS flow only **Secret Manager** (default — unchanged token issuance and validation) or **OIDC** are offered. Selecting OIDC opens a searchable list of [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) records (name, description, vendor type); a Generic-vendor provider is a valid choice here too, since only its token validation is used.<br />- **OIDC with CLIENT_CREDENTIALS (IdP-issued tokens):** the policy issues no tokens; the client presents the provider's access token as `Authorization: Bearer` and Apinizer validates it on every request (JWKS signature offline, introspection/userinfo online — opaque tokens are supported through introspection). Identity comes from the provider's *Username Claim Path* (or the introspection response for opaque tokens), roles from the provider's role source; *Require Matching Consumer* maps the identity to a Consumer for quota/ACL attribution. Delete Previous, token lifetime and refresh settings are hidden, and the proxy's `/auth/*` token endpoint answers `400 invalid_request`. PASSWORD + OIDC keeps the previous behaviour.<br />- **Check Client IP Address:** Make token's source IP match mandatory in Secret Manager flow. |
...
| **Step 6: Header and Authorization Integration (If Any)** | - **Set Resolved Identity to Context:** Determines whether the identity resolved by this policy (username/key) is recorded as the request identity. When on (default), this identity appears in traffic logs and subsequent policies and scripts. When off, the identity is used only for the necessary internal authentication/authorization checks and is not persisted as the request identity.<br />- **Add User To Header / User Header Name:** Add authenticated user information to custom header.<br />- **Authorization Configuration:** Configure method-based role assignments, authorization source (Secret Manager, API, LDAP, DB), and role header management.<br />- **Add Roles To Header / Roles Header Name:** Determine header names to be transmitted for roles. |
