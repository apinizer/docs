...
An [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) is not offered as an Identity Role Group option here. Basic Authentication verifies a username/password pair sent on every request; an OIDC provider validates a token issued once at login, not a password comparison, so the two are not compatible identity sources. Use the [OIDC Authentication](/en/develop/policies/oidc-authentication) policy for token-based verification instead.
