...
:::info The password may contain the `:` character

After the base64 decode, only the **first** `:` separates the username from the password; every character after it is part of the password (RFC 7617). For example `user:12:34` is resolved as the username `user` and the password `12:34`.

Requests from users whose password contained a `:` were previously rejected with a "missing credentials" error. A `:` cannot be used in the username — that is what the standard requires.
...
<img src="/images/develop/politikalar/basic-new.png" alt="Basic Authentication Policy Configuration" width="1000" />
...
| **Step 8: Customize Error Message (Optional)** | - Go to **Error Message Customization** tab.<br />- Customize message to return when access is denied.<br /><br />**Default:**<br />`{ "statusCode": 401, "message": "Unauthorized: Invalid credentials" }`<br /><br />**Custom:**<br />`{ "statusCode": 401, "errorCode": "AUTH_FAILED", "message": "Authentication failed. Username or password is incorrect." }` |
| **Step 9: Save** | - Click **[Save]** button at top right.<br /><br />**Checklist:**<br />- Unique name<br />- Required fields filled<br />- At least one Identity Role Group selected<br /><br />**Result:**<br />- Policy is added to list.<br />- Can be bound to APIs.<br />- If global policy, automatically applied. |

:::info[When is OIDC offered?]

In the default username/password mode an [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) is not offered as an Identity Role Group option: Basic Authentication compares a password sent on every request, while an OIDC provider validates a token and never hands out a password. Once **Use API Key as Bearer Token** is enabled (with **Validate Client Secret/Password** off) the presented value is a bearer token, so **OIDC becomes selectable**: the token issued by the provider is validated on every request through the provider's validation mode (JWKS signature offline, introspection/userinfo online), the identity is resolved from the provider's *Username Claim Path* and, with *Require Matching Consumer* on, matched to a Consumer. Turning **Use API Key as Bearer Token** off again resets an OIDC identity source to Secret Manager (a warning is shown), and the server rejects a plain Basic policy that still carries an OIDC source. For browser-style login flows use the [OIDC Authentication](/en/develop/policies/oidc-authentication) policy instead.

:::

### Authenticating with an API Key as a Bearer Token

Enable **Use API Key as Bearer Token** for clients that only present a single opaque key and do not implement a username/password challenge — for example, simple service-to-service or machine clients that should not need a full [OIDC](/en/develop/policies/oidc-authentication)/Keycloak token issuance flow just to identify themselves.

With **Validate Client Secret/Password** off and **Use API Key as Bearer Token** on, the request must carry the key in a standard `Authorization` header using the `Bearer` scheme:
...
The token after `Bearer` is looked up as the API key — the Consumer's username or the API Client's clientId, depending on the selected Identity Role Group. A request without the `Bearer` scheme, or with an empty token, is rejected before the key is looked up.
