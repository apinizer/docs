...
:::warning
A common mistake is writing the placeholder directly in the script body, e.g. `def text = ${myVariable};`. This fails with a script error (fault code `ERR-058` and a message similar to `No signature of method: ... $()`), because `${...}` is parsed as Groovy syntax rather than an Apinizer placeholder. Always read environment variables with `environmentVariableMap.get("myVariable")`.
:::
