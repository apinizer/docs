...
<Note>
**Behavior**:
- The map is read-only; `put`, `remove`, and `clear` calls raise an error.
- Key enumeration (`keySet`, `values`) is not supported.
- Operates against the active environment; environment switches are reflected at runtime.
...
- Password: `getResolvedPassword()` (resolved with variables substituted), `getEnvironmentPassword("environment-id")` (resolved environment-scoped password)
