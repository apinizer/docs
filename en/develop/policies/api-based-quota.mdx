...
- **Counting Mode**: Determines whether requests rejected because the quota was already exceeded still consume the quota. By default (All requests), every request — accepted or rejected — increases the usage counter, matching Apinizer's historical behavior. It can instead be set to Accepted only, so that a rejected request never consumes quota.
- **Limit Change Action**: Determines what happens to the existing usage counter when the Quota Count value is changed while the policy is already in use. By default (Clamp to previous limit), the counter is capped to the previous limit value so it can never remain above what was actually allowed. It can instead be set to Reset — but only when the limit is raised, never when it is lowered — or to Leave unchanged, keeping the counter untouched.
...
<img src="/images/develop/politikalar/new-kota.png" alt="API Based Quota Policy Configuration" width="1000" />
...
| **Step 8: Error Message Customization (Optional)** | - Go to **Error Message Customization** tab.<br />- Customize the message to be returned when quota is exceeded.<br /><br />**Default:**<br />`{ "statusCode": 429, "message": "Quota Exceeded" }`<br /><br />**Custom:**<br />`{ "statusCode": 429, "errorCode": "MONTHLY_QUOTA_EXCEEDED", "message": "Your monthly quota has been exhausted. Renewal date: {reset_date}. To upgrade your plan: /upgrade" }` |
| **Step 9: Saving** | - Click the **[Save]** button at the top right.<br /><br />**Checklist:** Unique name. Required fields filled. At least one message count and time interval defined<br /><br />**Result:**<br />- Policy is added to the list.<br />- Can be connected to APIs.<br />- If global policy, automatically applied. |
...
| **Usage** | Can I control what happens to current usage when I change the limit, or whether rejected requests count toward the quota? | Yes, with two independent settings in Advanced Settings. **Limit Change Action** controls the usage counter when the Quota Count value is edited: by default (Clamp to previous limit) the counter is capped to the previous limit so it can never stay above what was actually allowed; it can instead be set to Reset only when the limit is raised, or to Leave unchanged. **Counting Mode** controls whether requests rejected by the policy still consume quota: by default (All requests) they do, matching historical behavior; setting it to Accepted only means a rejected request never consumes quota. Example: a customer has a quota of 5 and has already made 5 accepted and 5 rejected requests, so usage is 10 under All requests. If the quota is then raised to 8, Clamp to previous limit brings usage down to 5 (the previous limit), so exactly 3 more requests are allowed before the new limit is reached. |
| **Usage** | When can a customer who exceeded quota make requests again? | Must wait until reset time. In FIXED window: At beginning of next period (e.g., 1st of next month). In SLIDING window: When period duration passes from first request. Timestamp is given with X-RateLimit-Reset in response header. The response also always carries a `Retry-After` header (in seconds) — the time remaining until the window closes, at least 1 and capped at a platform-wide ceiling (1 day by default, see note below) — this header is present regardless of whether rate limit headers are enabled. |

:::note Retry-After ceiling

The `Retry-After` value is capped platform-wide so a client is never told to wait an unreasonably long time. This matters most for a monthly quota: without the cap, a window that resets on the 1st of next month could otherwise advertise close to 31 days. The default ceiling is 1 day; an operator can change it with the `apinizer.limit.retryAfterClampSeconds` JVM system property on the Gateway (Worker) — an invalid or non-positive value falls back to the default. The counter itself is unaffected by the cap: an early retry still meets the same exhausted window and gets a fresh, equally capped `Retry-After`. This same ceiling also applies to the `Retry-After` header of the [API Based Throttling](/en/develop/policies/api-based-throttling) policy.
