...
<img src="/images/develop/politikalar/oidc-new.png" alt="OIDC Authentication Policy" width="1000" />
...
| **Step 4: OIDC Provider and Endpoint Configuration** | - Define Issuer or Discovery URL.<br />- If Discovery is disabled, manually enter Authorization, Token, UserInfo, JWKS, and Introspection endpoint addresses.<br />- Fill Client ID/Secret, Redirect URI, Realm, and scopes fields.<br />- Optionally set expected issuer for token issuer verification.<br />- Evaluate and mark PKCE and mandatory scope needs.<br />- Use mTLS configuration (KeyStore, TrustStore, skip SSL verification) when connecting to OIDC provider via mTLS. |
| **Step 5: Flow, Token, and Session Parameters** | - Select Flow Type (Authorization Code, Implicit, Hybrid, OAuth2).<br />- Set authentication mode (EXTERNAL_ONLY, INTERNAL_ONLY, HYBRID).<br />- Configure token verification, signature algorithm, issuer and multi-audience controls.<br />- Set token acceptance path (header, cookie, header_cookie); bearer usage, access token header name, and cache timeout values.<br />- Configure session cookie name, session timeout, absolute session timeout, and cookie delimiter.<br />- Token cookie settings: access token, ID token and refresh token cookie names; which tokens are written to cookies.<br />- Logout: logout path, post-logout redirect URI, redirect after logout URI; token revocation on logout and revocation endpoint.<br />- Automatic access token refresh when expired.<br />- HTTP methods and path patterns to skip policy. |
| **Step 6: Advanced Header and Authorization Settings** | - Add required key/value pairs to Custom header and backend header tables.<br />- Add tokens to backend request; set access token and ID token header names.<br />- Activate toggles to write user/role information to header, determine header names.<br />- Make role mappings with Identity Role Group Service, connect additional services through Authorization Configuration component if needed. |
...
| **Step 9: Save** | - Click the **[Save]** button at the top right.<br /><br />**Checklist:** Unique name. Required fields filled. At least one OIDC provider configuration exists<br /><br />**Result:**<br />- Policy is added to the list.<br />- Can be connected to APIs.<br />- If global policy, automatically applied. |
