...
<img src="/images/develop/politikalar/connection.png" alt="OIDC Connection: provider, endpoints, client, and flow" width="1000" style={{maxWidth: '1000px', width: '100%', height: 'auto'}} loading="lazy" />
...
| Section | Field | Notes |
|---------|-------|-------|
| **Provider Configuration** | **Shared OIDC Provider** | Optional. Selects a shared [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) to resolve the endpoint, client, and claim-mapping fields below from it instead of entering them inline. See [Connecting to a Shared OIDC Provider](#connecting-to-a-shared-oidc-provider) below. |
...
#### Credential and Role Behavior with a Shared Provider

When this policy references a Shared OIDC Provider, it also follows two of the provider's Claim Mapping settings after the token is validated:

- **Require matching Credential** *(default: on)*: if the username resolved from the claim has no matching [Credential](/en/admin/identity-management/credentials) record, or that Credential is inactive, the request is rejected. The username-resolution chain itself is unchanged.
- **Role Source** set to **Synced Credential** or **Both**: the roles carried by the synchronized Credential are **added on top of** the roles resolved from the token. The policy's own **Role Mappings** and the token-derived roles are preserved — this is additive, never a replacement.

:::tip
If directory synchronization is not yet set up for this environment, turn off **Require matching Credential** on the [OIDC Provider's Claim Mapping tab](/en/admin/identity-management/identity-providers/oidc-provider#claim-mapping-tab). The policy then accepts a validated token even when no matching Credential exists — this is how it behaved before it started following the provider's setting.
:::

Policies that keep their configuration inline (**Shared OIDC Provider** left empty) are unaffected by either setting.
...
<img src="/images/develop/politikalar/oidc-ui-token-validation.png" alt="OIDC Token & Validation tab" width="1000" style={{maxWidth: '1000px', width: '100%', height: 'auto'}} loading="lazy" />
...
<img src="/images/develop/politikalar/oidc-ui-session-logout.png" alt="OIDC Session & Logout tab" width="1000" style={{maxWidth: '1000px', width: '100%', height: 'auto'}} loading="lazy" />

**Session Settings**

| Field | Description |
|-------|-------------|
| **Session Storage Type** | Where the **authenticated** session is stored: **Cache** (default) or **Cookie**. See [Session Storage Options: Cache vs Cookie](#session-storage-options-cache-vs-cookie) below. **This setting covers the authenticated session ONLY** — the login flow's temporary session is controlled by a separate field (see next row). |
...
:::tip Short-session warning on the form
While editing **Session Timeout (min)** or **Absolute Timeout (sec)**, the form shows a non-blocking warning underneath them when either value would produce an unexpectedly short session: **Session Timeout (min)** set below 5 minutes, or **Absolute Timeout (sec)** shorter than **Session Timeout (min)** converted to seconds. The warning is only a reminder — it never blocks saving — and it disappears again once the values no longer trigger it. The same warning appears on both configuration profiles: here, on the OIDC profile's **Session & Logout** tab, and on the OAuth2 profile's **Session & HTML** tab below.
:::
...
<img src="/images/develop/politikalar/oidc-ui-user-mapping.png" alt="OIDC User Mapping tab" width="1000" style={{maxWidth: '1000px', width: '100%', height: 'auto'}} loading="lazy" />
...
<img src="/images/develop/politikalar/security-1.png" alt="OIDC Security & Other tab" width="1000" style={{maxWidth: '1000px', width: '100%', height: 'auto'}} loading="lazy" />
<img src="/images/develop/politikalar/aecurity-2.png" alt="OIDC Security & Other tab" width="1000" style={{maxWidth: '1000px', width: '100%', height: 'auto'}} loading="lazy" />
...
:::info[When to use the OAuth2 profile]
...
<img src="/images/develop/politikalar/oidc-ui-oauth2-parameters.png" alt="OAuth2 Parameters tab: Authorization, Token, Resource, and Backend Request Headers" width="1000" style={{maxWidth: '1000px', width: '100%', height: 'auto'}} loading="lazy" />
...
<img src="/images/develop/politikalar/oidc-ui-session-html.png" alt="Session & HTML tab: session settings and HTML response pages" width="1000" style={{maxWidth: '1000px', width: '100%', height: 'auto'}} loading="lazy" />
...
| Field | Description |
|-------|-------------|
| **Session Storage Type** | Where the **authenticated** session is stored: **Cache** (default) or **Cookie** — see the OIDC section's [Session Storage Options: Cache vs Cookie](#session-storage-options-cache-vs-cookie). **Covers the authenticated session only.** |
...
| **Step 9: Save** | - Click the **[Save]** button at the top right.<br /><br />**Checklist (summary):**<br />- Unique **Name** and required Connection fields: **Authorization** / **Token**; **Client ID**; **Redirect URI**. The **OIDC profile** also requires **OIDC Issuer URL** and **Flow Type**; the **OAuth2 profile** also requires **oauth2ResourceEndpoint** (Issuer is not required on this profile).<br />- **Redirect URI** matches the provider registration.<br />- **OIDC:** **Token & Validation** / **Session & Logout** / **User Mapping** complete as needed; **OAuth2:** **OAuth2 Parameters** and **Session & HTML** complete.<br />- Optional **Conditions** / **Error Message Customization** tested.<br /><br />**Result:** Policy is added to the list; can be connected to APIs; if global, applied automatically. |
...
| **Session Management** | - Choose where session data is stored: **Cache** (server-side, default) or **Cookie** (stateless, entire session encrypted in browser cookie) via **Session Storage Type**.<br />- In Cache mode, session data is kept on the server and the browser cookie carries only the session ID; this is backward-compatible and allows server-side session revocation.<br />- In Cookie mode, the entire session is encrypted and embedded in the cookie; no server state is maintained and the cache is not consulted. Cookie size is limited (~4 KB); not recommended when using both ID token and refresh token together (large payloads).<br />- Session encryption key and IV are automatically generated from policy ID; configure with session cookie name, timeout, absolute timeout, and optional compression.<br />- **Login Flow Session Storage** and **Login Flow Timeout (sec)** manage the login flow's temporary session (state/nonce/PKCE verifier) **independently** of the authenticated session; default 600 seconds, and if left empty, Login Flow Session Storage follows Session Storage Type. |
...
| **Token Cookie Configuration** | - Access token cookie name can be customized; ID and refresh token storage in cookies is controlled with checkboxes.<br />- Use **Renew Access Token on Expiry** to control refresh-on-expiry behavior. |
...
**Cookie Mode Size Limits**
Browser cookies have an approximate 4 KB size limit. If the encrypted session data exceeds this limit, the session cannot be created and an error is returned.
...
Choose Cookie mode for high-scalability deployments with small session payloads and stateless architecture requirements. Optionally, session data can be compressed before encryption to reduce size.
...
| **Session Durations** | **Bad:** Leaving durations at default; confusing Login Flow Timeout with Absolute Timeout.<br />**Good:** Determining timeout according to user profile.<br />**Best:** Implementing Session absolute timeout + refresh token strategy; tuning Login Flow Timeout (sec) separately for the IdP's typical authentication time (including SMS/email OTP) — shortening it does not affect Absolute Timeout and vice versa. |
