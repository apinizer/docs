...
| **Step 4: Selecting Identity Provider** | **Identity Role Group:**<br />- Select Database, LDAP, Secret Manager, or external API provider with the **-Select** button.<br />- Use the cross button to clear the selection.<br />- When Secret Manager is selected, the policy automatically connects to this source. |
| **Step 5: Assigning Credential Variables** | **Username Variable:** Bind username input from project variables. Make changes with update or select different variable options.<br /><br />**Password Variable:** Becomes required if `checkPassword` is checked; the selected variable holds the secret value. |
| **Step 6: Request Processing Options** | **Check Password:** If disabled, only username is verified, password is not checked.<br /><br />**Check Client IP Address:** Forces IP matching in Secret Manager scenarios.<br /><br />**Clear Authorization Header:** Clears the Authorization header after verification without forwarding it to the target service.<br /><br />**Set Resolved Identity to Context:** Determines whether the identity resolved by this policy (username/key) is recorded as the request identity. When on (default), this identity appears in traffic logs and subsequent policies and scripts. When off, the identity is used only for the necessary internal authentication/authorization checks and is not persisted as the request identity.<br /><br />**Add User To Header:** Transmits the username with the `X-Authenticated-UserId` (changeable) header. |
...
| **Step 9: Saving** | - Click the **[Save]** button at the top right.<br /><br />**Checklist:** Unique name. Required fields filled. At least one identity provider selected.<br /><br />**Result:**<br />- Policy is added to the list.<br />- Can be linked to APIs.<br />- If it's a global policy, it is automatically applied. |

:::info[Why not OIDC?]

An [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) is not offered as an Identity Role Group option here. Plain Text Authentication verifies a username/password pair sent on every request; an OIDC provider validates a token issued once at login, not a password comparison, so the two are not compatible identity sources. Use the [OIDC Authentication](/en/develop/policies/oidc-authentication) policy for token-based verification instead.
