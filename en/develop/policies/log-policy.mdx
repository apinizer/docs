...
The Elasticsearch index template and ILM policy for log policy data are not automatically created from the Apinizer UI. For ILM policy, index template, and data stream setup commands, see [Apinizer Log Table Creation Commands — Log Policy](/en/operations/administrator-guides/apinizer-log-table-creation-commands#log-politikasi).
...
For relational databases, the `log_PolicyCapture` table is **not created automatically** — you must create it manually before enabling the connector. See [Apinizer Log Table Creation Commands](/en/operations/administrator-guides/apinizer-log-table-creation-commands#log-politikasi) for the CREATE TABLE statement, recommended indexes, and partitioning guidance for each supported database type.
...
For high-traffic APIs, configure the log policy in **Asynchronous** mode so database write latency does not block the request pipeline.
