...
| **Step 4: JWT Authentication Parameters** | - **Managed From This Policy**: Will the policy control credentials management? `true` is selected in integrations other than Secret Manager.<br />- **Grant Type**: Select CLIENT_CREDENTIALS or PASSWORD; selection determines available identity sources.<br />- **Identity Source**: For PASSWORD, make selection from Identity Role Group modal: LDAP/Database/API/OIDC or Secret Manager. Selecting OIDC opens a searchable list of [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) records (name, description, vendor type); authentication is then performed through the chosen provider's token/claim mapping. A Generic-vendor provider is a valid choice here, since only its token validation is used, not synchronization. (OIDC is not listed in the JWT Refresh Service selector — the refresh service can only be an API-type service.)<br />- **Check Client IP Address**: Validates the IP from which the request came in Secret Manager-based structures. |
