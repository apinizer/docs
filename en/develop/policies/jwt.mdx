...
- **Identity Source Selection**: Integrates Secret Manager, LDAP, Database, external API, or [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) sources into JWT generation process.
...
<img src="/images/develop/politikalar/jwt-new.png" alt="JWT Authentication Policy" width="1000" />
...
| **Step 4: JWT Authentication Parameters** | - **Managed From This Policy**: Will the policy control credentials management? `true` is selected in integrations other than Secret Manager.<br />- **Grant Type**: Select CLIENT_CREDENTIALS or PASSWORD; selection determines available identity sources.<br />- **Identity Source**: Shown for every grant type. For PASSWORD, make selection from Identity Role Group modal: LDAP/Database/API/OIDC or Secret Manager. For CLIENT_CREDENTIALS only **Secret Manager** (default — the policy issues and validates its own tokens exactly as before) or **OIDC** are offered. Selecting OIDC opens a searchable list of [OIDC Provider](/en/admin/identity-management/identity-providers/oidc-provider) records (name, description, vendor type); a Generic-vendor provider is a valid choice here, since only its token validation is used, not synchronization. (OIDC is not listed in the JWT Refresh Service selector — the refresh service can only be an API-type service.)<br />- **OIDC with CLIENT_CREDENTIALS (IdP-issued tokens)**: the policy issues **no** tokens. The client obtains its access token from the OIDC provider and sends it as `Authorization: Bearer`; Apinizer validates it on every request through the provider's validation mode (JWKS signature offline, or introspection/userinfo online), resolves the identity from the provider's *Username Claim Path* and, when *Require Matching Consumer* is on, matches it to a Consumer for quota, ACL and rate-limit attribution. Token lifetime, refresh and *Managed From This Policy* settings are hidden because they do not apply, and this proxy's `/auth/*` token endpoint answers `400 invalid_request`. For a Keycloak client-credentials token the identity claim is `preferred_username = service-account-<clientId>`; set the provider's Username Claim Path to `azp` when the Consumer is named after the client id.<br />- **OIDC with PASSWORD** keeps the previous behaviour: the policy still issues its own token after the provider validates the presented bearer token.<br />- **Check Client IP Address**: Validates the IP from which the request came in Secret Manager-based structures. |
| **Step 5: Token and Refresh Configuration** | - **Token Never Expires**: Open if unlimited token will be generated, otherwise enter duration and unit.<br />- **Refresh Token Allowed**: Open if refresh permission will be given, define right count and validity duration.<br />- **JWT Signature Algorithm**: Select from RS256/RS384/RS512 according to target system requirements. |
| **Step 6: Header and Authorization Settings** | - **Allow URL Parameters**: Enables sending token with query parameter; only recommended in trusted scenarios.<br />- **Clear Auth**: Clears incoming Authorization header; used for resetting old JWT.<br />- **Set Resolved Identity to Context:** Determines whether the identity resolved by this policy (username/key) is recorded as the request identity. When on (default), this identity appears in traffic logs and subsequent policies and scripts. When off, the identity is used only for the necessary internal authentication/authorization checks and is not persisted as the request identity.<br />- **Add User/Roles to Header**: Sends user and role information to target service with custom header names.<br />- **Authorization Configuration**: Open advanced options such as role-based embedding, method access, and JWT refresh service. |
...
| **Step 9: Saving** | - Click the **[Save]** button at the top right.<br /><br />**Checklist:** Unique name. Required fields filled. At least one identity source selection completed.<br /><br />**Result:**<br />- Policy is added to the list.<br />- Can be connected to APIs.<br />- If global policy, automatically applied. |
...
When **Managed From This Policy** is enabled, the Client Id and Client Secret values validated in token requests are the keys of the **API Proxy**, **API Proxy Group**, or **Policy Group** that this policy is attached to. The **Show API Key** option in the policy form opens a dialog where the current Client Id and Client Secret can be viewed, copied, and regenerated when needed.

### Key Generation Type

When regenerating the Client Id or Client Secret, one of two options is selected:

- **Automatic:** The system generates the new value.
- **Manual:** You provide the value yourself.

<img
  src="/images/develop/politikalar/oauth-jwt-regenerate-client-id.png"
  alt="Regenerate Client Id or Client Secret confirmation dialog"
  width="1000"
  height="486"
/>
...
Regenerating is a critical operation that cannot be undone. You must confirm by typing the name of the related record, then confirm a final warning screen. The change is not valid until saved; after saving, the API Proxy, API Proxy Group, or Policy Group must be redeployed for the new keys to become active. Existing clients using this policy must also be reconfigured with the new Client Id/Client Secret values.

:::

### Consistency Across Screens

This Client Id / Client Secret viewing, copying, and regenerating experience (including the Automatic/Manual choice, name-confirmation, and final confirmation step) is identical to the key management sections on the **API Proxy**, **Policy Group**, and **API Proxy Group** screens. Whenever this JWT Authentication policy is attached to the policy list of an API Proxy, an API Proxy Group, or a Policy Group — whether selected as a global (shared) policy or created as a local policy specific to that API/Group — the **Show API Key** option in the policy form provides the same experience.
