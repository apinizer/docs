...
<img src="/images/develop/politikalar/ws-decrypt-new.png" alt="WS Security Decryption Policy" width="1000" />
...
| **Step 4: WS-Security Decryption Parameters** | **Decryption KeyStore (Required):** Select relevant keystore from list. Keystore contains decryption key.<br />**KeyStore Tooltips:** Verifies that selected source is appropriate for project scope. |
| **Step 5: Case-Insensitive ID Control (If Any)** | **Allow Case-Insensitive Identifier:** Ignore case difference of `KeyIdentifier` fields by checking. Enable if SOAP provider uses different case. |
| **Step 6: Decryption Validation Notes (If Any)** | Manually send SOAP message in test environment and check error logs; error code 401 is returned in case of wrong keystore selection. |
...
| **Step 9: Saving** | - Click the **[Save]** button at the top right.<br /><br />**Checklist:** Unique name. Required fields filled. At least one WS-Security key configuration exists<br /><br />**Result:**<br />- Policy is added to the list.<br />- Can be linked to APIs.<br />- If it's a global policy, it is automatically applied. |
