...
The request is sent with the `POST` method and the `application/x-www-form-urlencoded` content type. The caller authenticates the same way as on the token endpoint: in the policy-managed mode with the application key and secret, in the ACL mode with the credential's client identifier and secret — or, as of this version, an [API client](/en/admin/identity-management/credentials#api-clients)'s own client id and client secret.
...
A caller can only inspect tokens that belong to it; a token issued for another application, credential or [API client](/en/admin/identity-management/credentials#obtaining-a-token-with-an-api-client) is always reported as `active: false`, so token details are never leaked across clients.
...
- [Token Management Settings](/en/admin/identity-management/token-management-settings)
