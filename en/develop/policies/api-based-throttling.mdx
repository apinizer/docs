...
4. **Decision Making**:
   ...
   **If Limit Exceeded:**
   * HTTP 429 (Too Many Requests) error is returned, flow is interrupted, goes to error line.
...
* **Counting Mode**: Determines whether requests rejected because the limit was already exceeded still consume the limit. By default (All requests), every request — accepted or rejected — increases the counter, matching Apinizer's historical behavior. It can instead be set to Accepted only, so that a rejected request never consumes the limit.
* **Limit Change Action**: Determines what happens to the existing usage counter when the Message Count value is changed while the policy is already in use. By default (Clamp to previous limit), the counter is capped to the previous limit value so it can never remain above what was actually allowed. It can instead be set to Reset — but only when the limit is raised, never when it is lowered — or to Leave unchanged, keeping the counter untouched.
...
<img src="/images/develop/politikalar/new-throtling.png" alt="API Based Rate Limiting Policy" width="1000" />
...
| **Step 8: Error Message Customization (Optional)** | - Go to **Error Message Customization** tab.<br />- Customize the message to be returned when access is denied.<br /><br />**Default:**<br />`{ "statusCode": 429, "message": "Too Many Requests" }`<br /><br />**Custom:**<br />`{ "statusCode": 429, "errorCode": "THROTTLE_LIMIT_EXCEEDED", "message": "Your per-minute request limit has been exceeded. Please try again after 60 seconds." }` |
| **Step 9: Saving** | - Click the **[Save]** button at the top right.<br /><br />**Checklist:**<br />Unique name. Required fields filled. At least one message count and time interval defined<br /><br />**Result:**<br />- Policy is added to the list.<br />- Can be connected to APIs.<br />- If global policy, automatically applied. |
...
| **Usage** | What happens when user exceeds limit? | HTTP 429 (Too Many Requests) error is returned and request is not processed.<br />The response always carries a `Retry-After` header (in seconds) — the time remaining until the window closes, at least 1 and capped at a platform-wide ceiling (1 day by default, see note below) — regardless of whether rate limit headers are enabled.<br />If rate limit headers are active, X-RateLimit-Reset information in response also indicates when they can try again.<br />If custom error message is defined, that message is shown. |
| **Usage** | Can I control what happens to current usage when I change the limit, or whether rejected requests count toward it? | Yes, with two independent settings in Advanced Settings. **Limit Change Action** controls the usage counter when the Message Count value is edited: by default (Clamp to previous limit) the counter is capped to the previous limit so it can never stay above what was actually allowed; it can instead be set to Reset only when the limit is raised, or to Leave unchanged. **Counting Mode** controls whether requests rejected by the policy still consume the limit: by default (All requests) they do, matching historical behavior; setting it to Accepted only means a rejected request never consumes the limit. Example: a client has a limit of 5 and has already made 5 accepted and 5 rejected requests, so usage is 10 under All requests. If the limit is then raised to 8, Clamp to previous limit brings usage down to 5 (the previous limit), so exactly 3 more requests are allowed before the new limit is reached. |

:::note Retry-After ceiling

The `Retry-After` value is capped platform-wide so a client is never told to wait an unreasonably long time — 1 day by default, regardless of how long the throttling window actually is. An operator can change the ceiling with the `apinizer.limit.retryAfterClampSeconds` JVM system property on the Gateway (Worker); an invalid or non-positive value falls back to the 1-day default. This same ceiling also applies to the `Retry-After` header of the [API Based Quota](/en/develop/policies/api-based-quota) policy.
