...
<img src="/images/develop/politikalar/xml-transformation-new.png" alt="XML Transformation Policy" width="1000" />
...
| **Step 4: Selecting Transformation Type** | Select XML2XML or XML2JSON from `transformationType` field. Selection determines visibility of sub-parameters. When XML2XML is selected, XSLT editor opens, when XML2JSON is selected, JSON options become active. |
| **Step 5: Managing XSLT Rules** *(If applicable)* | Enter transformation template in `xsltValue` editor in XML2XML mode. Code editor provides syntax highlighting. Validate XSLT on sample XML with `Test Transformation` button. To let the XSLT receive values from outside, see the [XSLT Parameter Usage](#xslt-parameter-usage) section on this page. |
| **Step 6: Configuring XML→JSON Settings** *(If applicable)* | In XML2JSON mode, mark `xmlToJsonUnwrapElement`, `xmlToJsonIgnoreNull`, `xmlToJsonIgnoreEmpty`, `xmlToJsonNumbersAsStrings`, `xmlToJsonUseNullForNil` flags as needed. Add XPath expressions that should become JSON arrays to `xmlToJsonArrayPathList` field. |
...
| **Step 8: Error Message Customization (Optional)** | - Go to the **Error Message Customization** tab.<br />- Customize the message to be returned when access is denied.<br /><br />**Default:**<br />`{ "statusCode": 403, "message": "[Default error message]" }`<br /><br />**Custom:**<br />`{ "statusCode": 403, "errorCode": "[CUSTOM_ERROR_CODE]", "message": "[Custom message]" }` |
| **Step 9: Saving** | - Click the **[Save]** button at the top right.<br /><br />**Checklist:** Unique name. Required fields filled. At least one XSLT or XML→JSON configuration exists<br /><br />**Result:**<br />- Policy is added to the list.<br />- Can be connected to APIs.<br />- If global policy, automatically applied. |
...
In XML2XML transformation, an XSLT template can accept external values through `<xsl:param name="..."/>` declarations. No separate value field is filled for these; each parameter is automatically matched with a variable **defined under the same name**. This makes it possible to run the same XSLT template with different values across environments or to feed it with data produced at runtime.

### Where the Value Comes From

A parameter name is looked up in the following two sources in order; the first value found is passed to the XSLT:

1. **Custom variables:** Values produced by a policy that ran earlier in the same request pipeline (for example a Script policy) are checked first.
2. **Environment variables:** If not found among custom variables, the environment variables defined in the active environment are checked.

A parameter that is not found in either source is not assigned a value; in that case the default declared in the XSLT template itself (`<xsl:param name="..." select="..."/>`) applies.

| Value source | Use case | Matching |
|--------------|----------|----------|
| Custom variable | A value produced per request by a preceding policy | Parameter name = custom variable name |
| Environment variable | A configuration value that differs per environment | Parameter name = environment variable name |

### Example 1 — Environment-variable parameter

When a different region value is needed in development and production, an environment variable is defined under the same name as the XSLT parameter.

XSLT template:

```xml
<xsl:param name="backendRegion"/>
<xsl:template match="/">
    <response>
        <region><xsl:value-of select="$backendRegion"/></region>
        ...
    </response>
</xsl:template>
```

If the `backendRegion` environment variable is defined as `dev-eu` in development and `prod-eu` in production, the same XSLT template receives that environment's value in each. In production the result is `<region>prod-eu</region>`.

### Example 2 — Custom-variable parameter

If a policy that ran earlier in the same request pipeline produced a value under the name `requestType`, declaring a parameter with the same name in the XSLT makes that value flow into the parameter automatically.
...
<xsl:template match="/">
    ...
    <xsl:if test="$requestType = 'premium'">
        ...
    </xsl:if>
</xsl:template>
```

The value, which can change per request, flows into XSLT and the transformation can branch on it.

:::note

- No value is written into a separate field; it is enough to define a custom variable or environment variable **under the same name** as the XSLT parameter.
- Matching is performed per request; the same name is searched first among custom variables, then among environment variables.
- A parameter not found in any source is left unassigned; the default declared in the XSLT applies.
- Values are passed to the XSLT as text (string).
