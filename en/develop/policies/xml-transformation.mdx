...
| **Step 5: Managing XSLT Rules** *(If applicable)* | Enter transformation template in `xsltValue` editor in XML2XML mode. Code editor provides syntax highlighting. Validate XSLT on sample XML with `Test Transformation` button. |
| **Step 6: Defining XSLT Parameters** *(If applicable)* | In XML2XML mode, the parameter values expected by the XSLT template are defined as name-value pairs in the panel. The value field can hold a static string or be resolved from an environment variable or a custom variable. See the [XSLT Parameter Usage](#xslt-parameter-usage) section on this page for details. |
| **Step 7: Configuring XML→JSON Settings** *(If applicable)* | In XML2JSON mode, mark `xmlToJsonUnwrapElement`, `xmlToJsonIgnoreNull`, `xmlToJsonIgnoreEmpty`, `xmlToJsonNumbersAsStrings`, `xmlToJsonUseNullForNil` flags as needed. Add XPath expressions that should become JSON arrays to `xmlToJsonArrayPathList` field. |
...
| **Step 9: Error Message Customization (Optional)** | - Go to the **Error Message Customization** tab.<br />- Customize the message to be returned when access is denied.<br /><br />**Default:**<br />`{ "statusCode": 403, "message": "[Default error message]" }`<br /><br />**Custom:**<br />`{ "statusCode": 403, "errorCode": "[CUSTOM_ERROR_CODE]", "message": "[Custom message]" }` |
| **Step 10: Saving** | - Click the **[Save]** button at the top right.<br /><br />**Checklist:** Unique name. Required fields filled. At least one XSLT or XML→JSON configuration exists<br /><br />**Result:**<br />- Policy is added to the list.<br />- Can be connected to APIs.<br />- If global policy, automatically applied. |
...
In XML2XML transformation, an XSLT template can accept external values through `<xsl:param name="..."/>` declarations. These values are defined in the policy configuration as name-value pairs. This makes it possible to run the same XSLT with different parameters across environments or to feed it with dynamic data at runtime.

### Parameter Value Types

| Value type | Use case | Example value | Resolution |
|------------|----------|---------------|------------|
| Static value | Same value in all environments | `customer-portal` | Passed to XSLT as-is |
| Environment variable | Uses the value defined in the active environment | `${backendRegion}` | Replaced with the value of the matching environment variable; empty if not found |
| Custom variable | Uses a value produced by a preceding policy via `customVariableMap` | `#{requestType}` | Replaced with the value of the matching custom variable in the request/response pipeline |

### Example 1 — Static parameter
...
<xsl:template match="/">
    <response>
        <version><xsl:value-of select="$apiVersion"/></version>
        ...
    </response>
</xsl:template>
```
...
Resulting XML: `<version>v3</version>`

### Example 2 — Environment-variable parameter

When a different endpoint marker is needed in development and production, an environment variable is used.

| Name | Value |
|------|-------|
| `backendRegion` | `${backendRegion}` |

If the `backendRegion` environment variable is defined as `dev-eu` in the development environment and `prod-eu` in production, the same XSLT receives the correct value in both environments.

### Example 3 — Custom-variable parameter

If a preceding Script policy produced a value via `customVariableMap.put("requestType", "premium")`, it can be passed to XSLT as a parameter.
...
<Note>
- In the value field, `${...}` refers to an environment variable and `#{...}` refers to a custom variable. Both types can be combined in the same value field.
- Resolution happens per request; if a variable cannot be found, the parameter is passed as empty.
- Parameter names must exactly match the `<xsl:param name="...">` declarations in the XSLT.
</Note>
