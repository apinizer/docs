...
:::
    ...
    The chart and version (`--version 0.1.1`) stay the same for each module; only the new image tag is provided:
...
`--reuse-values` keeps all values of the existing release; `--set image.tag` changes only the image version. Release names and namespaces must match the installation (Portal in the `apinizer-portal` namespace).
