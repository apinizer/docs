...
The migration writes two ledger collections, `api_client_migration_run` and `api_client_migration_row`, and installs a daily reconcile job (03:45) that keeps retrying any record that did not converge on the first pass. Every counter it creates — quota and throttling — **starts at zero**, and **this migration cannot be rolled back**: the MongoDB backup in step 1 below is your only way back if something looks wrong afterward.
