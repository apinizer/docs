...
<CardGroup cols={2}>
  ...
  <Card title="Elasticsearch">
    Recommended for indexing high-volume API traffic logs. Without it, logs are written to MongoDB. [Learn more](/en/installation/elasticsearch/ubuntu-elasticsearch)
  </Card>
...
<Tabs>
  <Tab title="Kubernetes Manifest">
    ...
    **API Manager and Gateway installations are mandatory** — the actual API Gateway of Apinizer is this Gateway module. Follow the steps below in order: API Manager must be up first, then at least one Gateway must be installed and **defined as an environment** through the Manager. Cache, Integration and API Portal modules are **optional** and can be installed from the tabs at the bottom of the page.
    ...
    <Note>
      **Elasticsearch** is important for API traffic log indexing and search; it is optional. Without Elasticsearch, traffic logs are written to MongoDB — Elasticsearch is recommended for high-traffic environments. For details → see [Elasticsearch Installation](/en/installation/elasticsearch/ubuntu-elasticsearch).
    </Note>
    ...
    **Default User Password:** Request help from Apinizer support team.
    ...
    <Tabs>
      <Tab title="Managed Gateway (Apinizer-managed)">
      ...
      <Tab title="Remote Gateway (Manual Deploy)">
    ...
    **NodePort range:** `30080`-`32767` (Kubernetes default range).
    ...
    <Tabs>
      <Tab title="Cache Setup and Configuration">
        ...
        <Tabs>
          <Tab title="Managed Cache (Apinizer-managed)">
          ...
          <Tab title="Remote Cache Server (Manual Deploy)">
      ...
      <Tab title="Integration Setup and Configuration">
      ...
      <Tab title="API Portal Setup">
        ...
        * If Kubernetes management is **not** done through Apinizer, manual installation can be done to Kubernetes and then connection can be established with API Manager.
        ...
        <Tabs>
          ...
          <Tab title="Without API Manager (Manual)">
      ...
      <Tab title="Starting API Manager with SSL">
  ...
  <Tab title="Helm">
    ...
    <Tabs>
      <Tab title="API Manager and Worker">
        <Info>
