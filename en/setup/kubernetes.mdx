...
Pods created by Apinizer carry a DNS `ndots` setting. This setting is only applied to the deployment through **Publish / Republish**; a plain Restart does not add it. For details and the recommendation to use FQDN for in-cluster addresses → see the [Kubernetes DNS issues](/en/operations/troubleshooting/kubernetes-docker-containerd-troubleshooting) page.
