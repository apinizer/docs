...
The Portal chart uses the `apinizer-portal` namespace (hardcoded in the template), so install the Portal into the **`apinizer-portal` namespace**.
...
helm install portal oci://registry-1.docker.io/apinizercloud/apinizer-apiportal \
  --version 0.1.1 \
  -n apinizer-portal --create-namespace \
  --set image.tag="<APINIZER_IMAGE_TAG>" \
