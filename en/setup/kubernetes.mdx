...
kubectl -n apinizer get pods,svc
```
...
```bash
kubectl -n apinizer rollout status deploy/worker
kubectl -n apinizer logs -l app=worker --tail=100
```
...
```bash
kubectl -n apinizer rollout status deploy/cache
kubectl -n apinizer get pods -l app=cache
```
