...
:::
  ...
  <TabItem value="helm" label="Helm">
    Install Apinizer modules with **Helm charts**. Helm parameterizes manifests, provides version management (`helm upgrade`, `helm rollback`) and tracks installed releases on the cluster (`helm list`). For production, **Helm is recommended over raw Manifests** — environment-specific values are managed in a single `values.yaml`, and the flow integrates well with GitOps pipelines. Pick the module you want to install from the tabs below. Modules can use independent charts or a single umbrella chart.

    <Tabs>
      <TabItem value="api-manager-and-worker" label="API Manager and Worker">
        :::info
...
If Helm is not installed, you can review the [Helm Installation Document](/en/setup/kubernetes) page.
...
:::
      ...
      <TabItem value="cache" label="Cache">
        ## Apinizer Cache Helm Installation

        Apinizer Cache is a Hazelcast-based distributed cache cluster. Workers connect to Cache for shared data such as quota, OIDC token cache, circuit breaker states.

        Currently, Cache is deployed together with Manager + Worker as part of the top-level `apinizer-charts/apinizer` chart. A standalone Helm chart (for Cache only) has not been published yet.

        :::note

**TODO:** A standalone Helm chart usage for Apinizer Cache will be added soon. For now, the `apinizer-charts/apinizer` chart in the "API Manager and Worker" tab also covers the Cache deployment.

:::

        For manual YAML-based Cache installation, see the "Kubernetes Manifest → Cache" tab.
      </TabItem>
      <TabItem value="integration" label="Integration">
        ## Apinizer Integration Helm Installation

        Apinizer Integration is a Quartz-based task flow scheduler. It runs workflows by combining multiple endpoints.

        :::note

**TODO:** A standalone Helm chart usage for Apinizer Integration will be added soon.

:::

        For now, the Integration module is installed on Kubernetes using YAML manifest. See the "Kubernetes Manifest → Integration" tab for details.
      </TabItem>
      <TabItem value="api-portal" label="API Portal">
        ## Apinizer Portal Helm Installation

        Apinizer API Developer Portal is the portal where end users and developers can access published APIs and perform documentation, test and credential operations.

        :::note

**TODO:** A standalone Helm chart usage for Apinizer Portal will be added soon.

:::

        For now, the API Portal module is installed on Kubernetes using YAML manifest. See the "Kubernetes Manifest → API Portal" tab for details.
      </TabItem>
    </Tabs>
  </TabItem>
</Tabs>
