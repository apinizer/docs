...
Pods created by Apinizer carry a DNS `ndots` setting. It is added during Publish / Republish. If it is missing on an existing deployment, editing its resources, security context, health probe, or environment variables also adds it. An operator-set `ndots` value is preserved. A plain Restart, or Publish/Unpublish (replica scaling), does not add it — in that case, apply one of the edits above or Republish. For details and the recommendation to use FQDN for in-cluster addresses → see the [Kubernetes DNS issues](/en/operations/troubleshooting/kubernetes-docker-containerd-troubleshooting) page.
...
:::
      ...
      <TabItem value="remote-gateway-manual-deploy" label="Remote Gateway (Manual Deploy)">
        ...
        ### 1. Creating Role and RoleBindings for Worker and Cache
...
:::
    ...
    **Example values (cross-namespace Kubernetes service discovery):**
    ...
    ```
    Gateway Management API URL: http://worker-management-api-http-service.prod.svc.cluster.local:8091
...
:::
    ...
    **Prerequisite:** The connector itself must first be defined under **System Settings → Connection Management** before it can be selected here.
    ...
    → For detailed assignment workflow: [Adding Log Connectors](/en/admin/server-management/adding-connectors-to-gateway-environments)
...
This section is only meaningful for **Managed Gateway** — for **Remote Gateway**, the operator authors the Service YAMLs manually (`apinizer-worker-service.yaml`) and these toggles have no effect.
...
:::
          ...
          <TabItem value="remote-cache-server-manual-deploy" label="Remote Cache Server (Manual Deploy)">
            ...
            The Cache application runs in the same namespace as the Worker, using the same `worker-cache-serviceaccount`. It is assumed that the permission and namespace steps from the **Creating Role and RoleBindings for Worker and Cache** section under the Remote Gateway sub-tab of the "Gateway Setup and Configuration" tab have already been applied.
...
**Additional Variables**
          ...
          * Old flags `-XX:{Min|Max}RAMFraction` are now deprecated. There is a new `-XX:MaxRAMPercentage` flag that takes a value between 0.0 and 100.0 and defaults to 75.0. Therefore, if there is a 2 GB memory limit, the JVM heap is limited to ~ 1.5 GB by default.
...
API Manager must be installed. Also, API Developer Portal must be present in your license key. The Manager user owning the Portal's Management API key must be listed under **Portal Settings → Security → Management API Key Users** (or hold a portal role while that list is empty); otherwise the Portal's calls are rejected with 403.
...
### Common Preparation — Namespace and Secret

:::warning The Secret is not created by Helm

As of version `1.0.0` the charts **do not** create the Secret; they only reference it by **name**. Therefore the target **namespace** and **Secret** must already exist in the cluster before `helm install`.

:::

The modules that connect to MongoDB (API Manager, Worker, Cache, Integration) use a shared Secret holding the `dbUrl` and `dbName` keys. `kubectl create secret` base64-encodes the values automatically — pass plain text (do not encode manually):
...
* The Secret name (`mongo-db-credentials`) must match the module's `mongo.secretName`; the key names `dbUrl` and `dbName` are fixed.
* The Secret must live in the **same namespace** as the module you install. Modules sharing a namespace (API Manager, Worker, Cache, Integration) need the Secret created only once.
* API Portal does not connect to MongoDB; it uses its own Secret (see the **API Portal** tab).
...
Every module needs a memory limit of at least **2 GB**. Even with a CPU limit of 1 core, the JVM does not start on 1 GB of memory. The Cache, Integration and API Portal charts default `resources.limits.memory` to `1Gi`, so always raise it to `2Gi` or more at install time.
...
| `mongo.secretName` | `mongo-db-credentials` | MongoDB Secret name (created outside Helm, beforehand) |
...
Instead of `--set`, you can install with the module's `values.yaml` file. This suits GitOps: keep `values.yaml` under version control (e.g. GitLab); when a parameter changes, apply the update by running `helm upgrade -f ...` with the same file.
...
If Cache is installed in the **same namespace** as Worker, add the following parameter to the installation command to avoid a ServiceAccount conflict:

```bash
  --set serviceAccount.name=cache-serviceaccount
```

Since the Secret is no longer created by Helm in `1.0.0`, Cache and Worker can safely share the same `mongo-db-credentials` Secret; you do not need to use a separate Secret name.
...
| `resources.limits.cpu` | `1` | CPU limit |
| `resources.limits.memory` | `1Gi` | Memory limit. The chart default is below the minimum; set at least `2Gi` even with a single CPU core, the JVM does not start on 1 GB |
| `mongo.secretName` | `mongo-db-credentials` | MongoDB Secret name (created outside Helm, beforehand) |
...
API Portal uses its own Secret (`apinizer-portal`), which is also not created by Helm. Create the namespace and Secret before installation. The `apiKey` and `apiPortalId` values are generated after the portal is registered in the API Manager UI. `kubectl create secret` base64-encodes the values automatically — pass plain text:
...
| `resources.limits.cpu` | `1` | CPU limit |
| `resources.limits.memory` | `1Gi` | Memory limit. The chart default is below the minimum; set at least `2Gi` even with a single CPU core, the JVM does not start on 1 GB |
| `portalSecret.secretName` | `apinizer-portal` | Portal Secret name (created outside Helm, beforehand) |
