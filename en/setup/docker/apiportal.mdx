...
3. The Manager user that owns this API key must be allowed to act for the portal: add the user's login to **Portal Settings → Security → Management API Key Users**, or give the user a portal role (Portal Manager / Business / Developer) when the list is left empty. Otherwise every Portal call is rejected with 403. See [Portal Settings → Security](/en/api-portal/api-portal-settings#security).
