...
### Advanced Undertow Server Options

These variables fine-tune the underlying HTTP server. The first group has Apinizer-managed defaults; the remaining options are applied only when you set the corresponding variable — otherwise the server keeps its built-in default and behavior stays unchanged. Every configured value is written to the Worker startup log.

| Variable | Target Environment | Description |
|----------|-------------|----------|
| `tuneAlwaysSetDateHeader` | HTTP Worker, HTTP+Websocket, Management API | Whether a Date header is added to every response (default: true) |
| `tuneMaxEntitySize` | HTTP Worker, HTTP+Websocket, Management API | Maximum size in bytes of a single request body; -1 means unlimited (default: -1) |
| `tuneEnableRfc6265CookieValidation` | HTTP Worker, HTTP+Websocket, Management API | Enables strict RFC 6265 cookie validation; when disabled, relaxed/legacy cookie formats are accepted (default: false) |
| `tuneAllowUnescapedCharactersInUrl` | HTTP Worker, HTTP+Websocket, Management API | Allows unescaped characters in request URLs (default: true) |
| `tuneAllowEncodedSlash` | HTTP Worker, HTTP+Websocket, Management API | Preserves encoded slash characters within path segments (default: true) |
| `tuneWorkerTaskKeepAlive` | HTTP Worker, HTTP+Websocket, Management API | Idle keep-alive time in milliseconds for worker task threads before they are reclaimed (default: 180000) |
| `tuneAlwaysSetKeepAlive` | HTTP Worker, HTTP+Websocket, Management API | Whether a keep-alive header is always added even when not strictly required (server default: true) |
| `tuneMaxHeaderSize` | HTTP Worker, HTTP+Websocket, Management API | Maximum total size in bytes of request headers (server default: 1048576) |
| `tuneMaxHeaders` | HTTP Worker, HTTP+Websocket, Management API | Maximum number of request headers (server default: 200) |
| `tuneMaxParameters` | HTTP Worker, HTTP+Websocket, Management API | Maximum number of query and path parameters (server default: 1000) |
| `tuneMaxCookies` | HTTP Worker, HTTP+Websocket, Management API | Maximum number of cookies per request (server default: 200) |
| `tuneMaxBufferedRequestSize` | HTTP Worker, HTTP+Websocket, Management API | Maximum buffered request size in bytes during request processing (server default: 16384) |
| `tuneMaxConcurrentRequestsPerConnection` | HTTP Worker, HTTP+Websocket, Management API | Maximum number of concurrent requests on a single connection; applied only when set |
| `tuneMaxQueuedReadBuffers` | HTTP Worker, HTTP+Websocket, Management API | Maximum number of queued read buffers per connection; applied only when set |
| `tuneMaxCachedHeaderSize` | HTTP Worker, HTTP+Websocket, Management API | Maximum size in bytes of a header eligible for caching (server default: 150) |
| `tuneHttpHeadersCacheSize` | HTTP Worker, HTTP+Websocket, Management API | Number of HTTP headers kept in the header cache (server default: 15) |
| `tuneRequestParseTimeout` | HTTP Worker, HTTP+Websocket, Management API | Maximum time in milliseconds allowed to parse a request; applied only when set (otherwise no timeout) |
| `tuneIdleTimeout` | HTTP Worker, HTTP+Websocket, Management API | Connection idle timeout in milliseconds; applied only when set (otherwise no timeout) |
| `tuneShutdownTimeout` | HTTP Worker, HTTP+Websocket, Management API | Maximum time in milliseconds to wait for in-flight requests during shutdown; applied only when set |
| `tuneRecordRequestStartTime` | HTTP Worker, HTTP+Websocket, Management API | Records each request's start time, needed for certain latency metrics and access logs (server default: false) |
| `tuneEnableStatistics` | HTTP Worker, HTTP+Websocket, Management API | Enables server-level statistics collection (server default: false) |
| `tuneAllowEqualsInCookieValue` | HTTP Worker, HTTP+Websocket, Management API | Allows equals characters inside cookie values (server default: false) |
| `tuneRequireHostHttp11` | HTTP Worker, HTTP+Websocket, Management API | Requires a Host header on HTTP/1.1 requests (server default: false) |
| `tuneAllowUnknownProtocols` | HTTP Worker, HTTP+Websocket, Management API | Allows requests using unknown HTTP protocols (server default: false) |
| `tuneBufferPipelinedData` | HTTP Worker, HTTP+Websocket, Management API | Buffers pipelined request data (server default: true) |
| `tuneDecodeUrl` | HTTP Worker, HTTP+Websocket, Management API | Decodes the request URL using the configured character set (server default: true) |
| `tuneDecodeSlash` | HTTP Worker, HTTP+Websocket, Management API | Decodes encoded slash characters in the URL (server default: false) |
| `tuneSslUserCipherSuitesOrder` | HTTP Worker, HTTP+Websocket, Management API | Honors the server's cipher suite order during TLS negotiation (server default: false) |
| `tuneTrackActiveRequests` | HTTP Worker, HTTP+Websocket, Management API | Tracks the number of currently active requests (server default: false) |

:::info

The options marked "applied only when set" or "server default" take effect only when you explicitly define the variable. When left unset, the server keeps its built-in default and behavior is identical to previous versions.
