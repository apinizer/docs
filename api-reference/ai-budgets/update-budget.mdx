...
- **No license gate**: These endpoints do not check the `AI_GATEWAY` license module. The API layer of
  Apinizer applies no module gate at all, so an installation whose license does not name AI Gateway reads
  and writes AI budgets through this resource exactly like a licensed one. Permission is the only gate:
  the caller still needs the `AI_BUDGETS` asset category (`VIEW` for reads, `MANAGE` for writes), and a
  missing permission is reported as a `400 Bad Request` validation error like everywhere else in APIops.
  The AI Gateway **screens** in the Manager UI remain hidden without the module.
