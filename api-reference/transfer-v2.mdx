...
| `permissionDenied` | — | Authenticated, but the token lacks `EXPORT_IMPORT` where this call needs it |
...
A package that cannot be decrypted (wrong passphrase, or not a package this installation can read) still fails with `400`, but on `transfer-import-preflights`/`transfer-import-runs` that particular failure carries its own `errorKey` (`packagePassphraseInvalid` or `packageDecryptFailed`) rather than the envelope above — the same behavior the legacy import endpoint has. Branch on `errorKey` either way; do not assume the outer envelope shape for this one case.
