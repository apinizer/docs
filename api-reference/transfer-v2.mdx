...
| `acknowledgeChecksumMismatch` | boolean | No (default `false`) | Accept a package that changed after export and import it anyway — a file whose *contents* no longer match the integrity manifest, a declared file missing from the package or an undeclared one added to it, and a manifest that cannot be read or disagrees with the package. The name is kept for compatibility. Compatibility refusals are not covered: a manifest format newer than this installation understands, an unrecognised critical feature and an unsupported checksum algorithm are always refused, as is a package file whose JSON cannot be parsed |

See [Export/Import Wizard → Package Integrity](/en/admin/secrets-management/export-import#package-integrity) for what these findings mean and which of them can be acknowledged, and [→ Mapping & Dependencies](/en/admin/secrets-management/export-import#mapping--dependencies) for what `certEnvironmentMergeMethod`/`certEnvironmentUncoveredAction` do to a certificate family object on `REPLACE`.
...
| `advisories` | array | `{ level, key, params }` — the engine's own advisories, e.g. `deployRequired` (`params.names`) or `transfer.checksumMismatchAcknowledged` (`params.entryCount`, `params.acknowledgedBy`) when `acknowledgeChecksumMismatch` accepted a changed package, whichever of the three findings it was |
