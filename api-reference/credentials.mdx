...
| `operationalState` | `DRAFT`, `ACTIVE`, `SUSPENDED`, or `REVOKED` — whether the client is currently allowed to authenticate. A client this surface creates is always born `ACTIVE`; `DRAFT` is only seen on a client the [legacy-record migration](/en/admin/identity-management/credentials#legacy-records-inventory-and-migration) created but has not yet been activated. |
