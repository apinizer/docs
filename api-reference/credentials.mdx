...
A carried-over secret is always stored in [Encrypted mode](/en/admin/identity-management/credentials#secret-storage-mode), so `recoveryState` reads `AVAILABLE` for every `CARRIED` row regardless of `recoverySupported` — a platform-encrypted copy is kept even without a recovery key. `recoverySupported` only reports whether a recovery key is additionally available to seal the envelope Reveal prefers.
