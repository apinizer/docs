---
title: "JOSE Validation Policy"
description: "JOSE Validation policy validates JOSE (JSON Object Signing and Encryption) tokens including JWT (JSON Web Token), JWE (JSON Web Encryption), and JWS (JSON Web Signature)."
---
...
| dpopMaxProofAgeUnit                        | string  | No       | SECONDS | Unit for dpopMaxProofAge. See [EnumTimeUnit](#enumtimeunit) |
...
| dpopCacheErrorHandlingType                 | string  | No       | CONTINUE | Behavior when the replay cache is unreachable. See [EnumCacheErrorHandlingType](#enumcacheerrorhandlingtype) |
...
- `CONTINUE` - When the DPoP replay cache is unreachable, allow the request (replay protection is skipped for that request)
- `FAIL` - When the DPoP replay cache is unreachable, reject the request
...
- [JOSE Implementation Policy](/api-reference/policies/policies/policy-jose-implementation) - Generate JOSE tokens
