...
description: "OAuth2 Authentication policy manages OAuth2 token authentication. It can issue, validate, and refresh OAuth2 access tokens and refresh tokens. OAuth2 is an industry-standard protocol for authorization."
---
...
##### Identity service and OIDC provider

| Field | Type | Description |
|---|---|---|
| `identityServiceType` | string | `SECRET_MANAGER` (default), `DATABASE`, `LDAP`, `API` or `OIDC`. Accepted for any `grantType`. |
| `oidcProviderName` | string | Required when `identityServiceType=OIDC`; resolved by name inside the project. With a non-PASSWORD grant the gateway issues no token — the provider does — and validates the presented bearer token on every request (JWKS / introspection per the provider's validation mode); token-lifetime, refresh and `managedFromThisPolicy` fields are ignored and `/auth/*` token requests answer `400 invalid_request`. Not returned on read. |
