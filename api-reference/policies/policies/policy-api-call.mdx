---
title: "API Call Policy"
description: "API Call policy makes HTTP/HTTPS calls to external APIs during request/response processing. It supports synchronous (two-way) and one-way (fire-and-forget) calls, request/response transformation, caching, mTLS, and comprehensive message manipulation."
---
...
```json
{
  ...
  "policy": {
    ...
    "fallbackTtl": 300,
    "enableResponseAssertion": true,
    "responseAssertionVariable": {
      "type": "BODY",
      "bodyExpression": "$.ReturnCode"
    },
    "responseAssertionOperator": "EQUALS",
    "responseAssertionExpectedValue": "0",
...
| cacheConnectionTimeoutInSeconds    | integer | No       | 3       | Distributed cache connection timeout in seconds |
| cacheErrorAction                   | string  | No       | CONTINUE | What to do when the cache service is unreachable (CONTINUE or STOP) |
| retrySettings                      | object  | No       | -       | Retry settings for the outbound call (see RetrySettings Object) |
| enableCompression                  | boolean | No       | false   | Advertise Accept-Encoding to the target service and automatically decompress its response |
...
| enableResponseAssertion            | boolean | No       | false   | Enable business-level success assertion on the response. When the assertion fails the response is NOT cached and the flow continues |
| responseAssertionVariable          | object  | No*      | -       | Variable to assert on (required if enableResponseAssertion=true) |
| responseAssertionOperator          | string  | No       | EQUALS  | Comparison operator |
| responseAssertionExpectedValue     | string  | No*      | -       | Expected value (required unless operator is EXISTS/NOT_EXISTS). Supports environment variables (`${variable}`) and context variables (`#{variable}`), resolved on every request |
...
Retries the outbound call when it fails with a transient error. Disabled by default; existing configurations are unaffected.

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| enabled | boolean | No | false | Enable retry |
| retryCount | integer | No | 0 | Additional attempts after the first one (total requests = retryCount + 1) |
| retryDelayType | string | No | NO_DELAY | Delay strategy: `NO_DELAY`, `FIXED` or `EXPONENTIAL` |
| retryFixedDelayMs | integer | No | 500 | Fixed wait before each retry (used when retryDelayType=FIXED) |
| retryInitialDelayMs | integer | No | 500 | Wait before the first retry (used when retryDelayType=EXPONENTIAL) |
...
- Connection-phase failures (connection refused, connect timeout, DNS or TLS failure) are always retried — the request was never sent.
- Failures after the request was sent (read timeout, connection reset) and error status codes are retried only for idempotent methods (`GET`, `HEAD`, `OPTIONS`, `PUT`, `DELETE`), unless `retryNonIdempotent` is `true`.
- The cache is looked up once, before the attempts; only a successful response is cached. If every attempt fails, nothing is written to the cache.
...
- `ISO_8601_DATETIME` - Standard ISO 8601 date format (e.g., `2026-03-26T15:30:00Z`, `2026-03-26T15:30:00+03:00`)
- `CUSTOM_DATETIME` - Custom format using Java DateTimeFormatter pattern (requires `dynamicTtlCustomDatePattern`)
- `EPOCH_SECONDS` - Unix epoch timestamp in seconds (e.g., `1743004200`)
- `EPOCH_MILLISECONDS` - Unix epoch timestamp in milliseconds (e.g., `1743004200000`)
- `EXPIRES_IN_SECONDS` - Relative duration in seconds (e.g., `300` for 5 minutes)
- `EXPIRES_IN_MILLISECONDS` - Relative duration in milliseconds (e.g., `300000` for 5 minutes)
- `TTL_STRING` - Human-readable duration string (e.g., `1h`, `30m`, `3600s`)

### EnumResponseAssertionOperator

- `EQUALS` - Resolved value equals the expected value
- `NOT_EQUALS` - Resolved value differs from the expected value
- `CONTAINS` - Resolved value contains the expected value
- `NOT_CONTAINS` - Resolved value does not contain the expected value
- `EXISTS` - Resolved value is present and not blank (expected value is ignored)
- `NOT_EXISTS` - Resolved value is missing or blank (expected value is ignored)
- `MATCHES_REGEX` - Resolved value fully matches the expected value as a regular expression. An invalid pattern is treated as a failed assertion

### Note

- `url` and `httpMethod` are required.
- `callType` is required.
- If `enableCache: true`, `capacity` and `ttl` are required (unless `enableDynamicTtl: true`), and `callType` must be `SYNCHRONOUS`.
- If `enableDynamicTtl: true`, `dynamicTtlVariable` and `dynamicTtlSourceType` are required. When dynamic TTL is enabled, the static `ttl` field is used as fallback only if `fallbackTtl` is not specified.
- If `dynamicTtlSourceType: CUSTOM_DATETIME`, `dynamicTtlCustomDatePattern` is required.
- If `dynamicTtlIsJwt: true`, `dynamicTtlJwtClaimName` is required.
- A dynamic TTL that resolves to zero or a negative value means the response has already expired; the response is not cached and `fallbackTtl` is not applied. Sentinel dates such as `0001-01-01T00:00:00`, returned by some services on failed responses, fall into this case.
- If `enableResponseAssertion: true`, `responseAssertionVariable` is required, and `responseAssertionExpectedValue` is required for every operator except `EXISTS` and `NOT_EXISTS`. A failed assertion only skips caching — the response is still returned to the caller.
- If `certificateEnabled: true`, `certificateName` is required.
- `enableCompression` defaults to `false`: no `Accept-Encoding` header is sent to the target service and the response is returned exactly as received. Set it to `true` when the target service returns a compressed response. This field is the only control for that header — an `Accept-Encoding` entry supplied in `headersToBeAddedBeforeCallList` is not forwarded.
...
- **Caching**:
  ...
  - Requires `capacity` and `ttl` when enabled (static `ttl` is optional if `enableDynamicTtl: true`)
  - Cache key can be based on variable (e.g., Authorization header)
  - **Dynamic TTL**: When `enableDynamicTtl: true`, cache TTL is automatically calculated from a value in the API response (e.g., `expireAt` field, JWT `exp` claim, or `expires_in` value). Requires `dynamicTtlVariable` and `dynamicTtlSourceType`. Use `dynamicTtlOffsetSeconds` as safety margin and `fallbackTtl` as fallback when dynamic calculation fails.

- **Compression**:
  - Disabled by default — no `Accept-Encoding` header is sent and the response is returned exactly as received
  - When `enableCompression: true`, the accepted compression formats (gzip, deflate, br, zstd and others) are advertised to the target service and a compressed response is decompressed automatically
  - Enable it when the target service returns a compressed response regardless of what the caller accepts, otherwise the response is carried on compressed
  - This field is the only control for the `Accept-Encoding` header — an entry for it in `headersToBeAddedBeforeCallList` is not forwarded
