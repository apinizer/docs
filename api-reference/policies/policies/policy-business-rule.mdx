---
title: "Business Rule Policy"
description: "Business Rule policy executes custom business logic actions on request/response messages. It supports ADD, MODIFY, DELETE, and STOP actions with various operators for data manipulation, transformation, and flow control."
---
...
##### Full JSON Body Example - Conditional Stop with Custom Error Message
    ...
    "actionList": [
      {
        ...
        "customMessageHttpStatusCode": 401,
        "customMessageContentType": "application/json;charset=UTF-8",
        "customMessageTemplate": "{ \"error\": \"Unauthorized\", \"message\": \"X-API-Key header is required\" }"
      }
    ]
  }
}
...
| policyCondition                 | object  | No       | -       | Condition for this action. When defined, the action only executes if the condition is met. Uses the same condition structure as policy-level conditions. |
| buildCustomMessage              | boolean | No       | false   | Enable custom error message when action triggers (typically used with STOP action) |
| customMessageHttpStatusCode     | integer | No       | 403     | HTTP status code for the custom error message |
| customMessageContentType        | string  | No       | "application/json;charset=UTF-8" | Content type for the custom error message |
| customMessageTemplate           | string  | No       | ""      | Custom error message body template. Supports variables (e.g., `${variable.name}`) |
...
- **STOP**: No additional fields required. Optionally use `buildCustomMessage` with `customMessageHttpStatusCode` and `customMessageTemplate` to return a custom error response.
- **All actions** optionally support `policyCondition` to execute the action only when the condition is met.
...
- **STOP Action**: When STOP action executes, subsequent actions and policies are not executed. Use `buildCustomMessage: true` with `customMessageHttpStatusCode` and `customMessageTemplate` to return a meaningful error response instead of the default 403.

- **Action-Level Conditions**: Each action can have its own `policyCondition`. This allows conditional execution of individual actions within the same policy (e.g., stop only if a specific header is missing, or add a header only for certain content types).
...
- [Redaction Policy](/api-reference/policies/policies/policy-redaction) - Remove or modify sensitive data
