---
title: "Allowed IP List Policy"
description: "White IP policy allows requests only from specified IP addresses or IP ranges. Requests from IPs not in the whitelist are rejected before reaching the backend. This policy is useful for restricting API access to specific networks, offices, or trusted IPs."
---
...
| ipList             | array   | No*      | -       | List of IP addresses or CIDR ranges to allow (required if ipGroupNameList is not provided) |
| ipGroupNameList    | array   | No*      | -       | List of IP Group names whose IPs/CIDRs are allowed (required if ipList is not provided). Names are resolved to the referenced IP Groups. |
...
- At least one of `ipList` or `ipGroupNameList` must be provided.
- When used, `ipList` must contain at least one IP address or CIDR range.
...
- **IP Groups**: Use `ipGroupNameList` to attach one or more predefined IP Groups by name; the policy then allows all IPs/CIDRs contained in those groups. `ipList` and `ipGroupNameList` can be combined.
...
- [Blocked IP List Policy](/api-reference/policies/policies/policy-black-ip) - Block specific IPs
