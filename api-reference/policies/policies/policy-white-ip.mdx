...
| ipList             | array   | No*      | -       | List of IP addresses or CIDR ranges to allow    |
| ipGroupIdList      | array   | No*      | -       | List of existing [IP Group](/api-reference/ip-groups/crud/create-ip-group) MongoDB ObjectId values to allow |

\* Provide at least one of `ipList` or `ipGroupIdList` (they can be combined).
...
- At least one entry is required across `ipList` and/or `ipGroupIdList`.
- `ipGroupIdList` binds named IP Groups managed under the project; use the group's id (not its name).
- When `targetVariableForIP` is `null`, Apinizer uses default IP detection (from X-Forwarded-For header or direct connection).
- Only IPs in the whitelist (direct entries plus members of referenced IP Groups) are allowed. All other IPs are blocked.
