...
| sessionStorageType         | string (enum) | No | CACHE (`null` resolves to CACHE) | Where the **authenticated** session is stored: `CACHE` (distributed cache; cookie carries only the session id) or `COOKIE` (session encrypted with AES-256-GCM and embedded directly in the cookie, stateless). See [EnumOIDCSessionStorageType](#enumoidcsessionstoragetype). |
| tempSessionStorageType     | string (enum) | No | null (follows `sessionStorageType`) | Storage for the **temporary** login session (state / nonce / PKCE verifier / return URL) held only during the authorize→callback round trip. If `null`, follows `sessionStorageType` above; can be overridden independently (e.g. keep `sessionStorageType=CACHE` for authenticated sessions but set `tempSessionStorageType=COOKIE` for a stateless login round trip). See [EnumOIDCSessionStorageType](#enumoidcsessionstoragetype) and the Note below. |
| loginFlowTimeoutSeconds    | integer | No       | 600     | Login flow completion window in seconds — TTL of the temporary login session (state/nonce/PKCE verifier). `null` or `<= 0` resolves to 600 at runtime. **Independent of `sessionAbsoluteTimeoutSeconds`** (the authenticated session's lifetime); too low a value causes callback failures (`state validation failed (CSRF protection): temporary session not found (expired or already consumed)`). |
...
### EnumOIDCSessionStorageType

Used by both `sessionStorageType` (authenticated session) and `tempSessionStorageType` (temporary login session).

- `CACHE` - Store session data in the distributed cache; cookie holds only the session id (default, backward-compatible; allows server-side revocation)
- `COOKIE` - Embed AES-256-GCM encrypted session data directly in the cookie (stateless, no cache call); bounded by the ~4 KB browser cookie limit and offers no server-side revocation
...
- `sessionStorageType` covers the **authenticated** session only. The temporary login session — state/nonce/PKCE verifier — is governed by `tempSessionStorageType`, which defaults to `null` and then **follows** `sessionStorageType`. So with the default (`null`), setting `sessionStorageType: COOKIE` also moves the login flow off the cache. Set `tempSessionStorageType` explicitly only when you want the two to differ — for example `CACHE` to keep the login flow on the distributed cache while the authenticated session stays in a cookie.
- `loginFlowTimeoutSeconds` is independent of `sessionAbsoluteTimeoutSeconds`. Shortening the authenticated session's absolute timeout does not shorten the login round-trip window, and vice versa.
...
- **Session Management**:
  - Authenticated-session storage is controlled by `sessionStorageType`: `CACHE` (default) keeps session data in the distributed cache with only the session id in the cookie; `COOKIE` embeds the AES-256-GCM encrypted session directly in the cookie (stateless, no cache call, ~4 KB limit)
  - The temporary login session (state/nonce/PKCE verifier) is controlled by `tempSessionStorageType`, which defaults to `null` and then follows `sessionStorageType` — so `COOKIE` on the authenticated session moves the login flow off the cache as well. Set `tempSessionStorageType` explicitly only to make the two differ (e.g. `CACHE` to keep the login flow on the distributed cache)
  - `loginFlowTimeoutSeconds` (default 600) bounds how long the temporary login session stays valid, independently of `sessionAbsoluteTimeoutSeconds`
...
- [OAuth2 Authentication Policy](/api-reference/policies/policies/policy-auth-oauth2) - OAuth 2.0 Authentication
