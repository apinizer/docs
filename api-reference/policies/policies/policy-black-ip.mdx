---
title: "Blocked IP List Policy"
description: "Black IP policy blocks requests from specified IP addresses or IP ranges. Requests from blacklisted IPs are rejected before reaching the backend. This policy is useful for blocking malicious IPs, DDoS attacks, or specific geographic regions."
---
...
| ipList             | array   | No*      | -       | List of IP addresses or CIDR ranges to block (required if ipGroupNameList is not provided) |
| ipGroupNameList    | array   | No*      | -       | List of IP Group names whose IPs/CIDRs are blocked (required if ipList is not provided). Names are resolved to the referenced IP Groups. |
...
- **IP Groups**: Use `ipGroupNameList` to attach one or more predefined IP Groups by name; the policy then blocks all IPs/CIDRs contained in those groups. `ipList` and `ipGroupNameList` can be combined.
...
- [Allowed IP List Policy](/api-reference/policies/policies/policy-white-ip) - Allow only specific IPs
