---
title: "Clear Text Authentication Policy"
description: "Clear Text Authentication policy authenticates requests using username and password extracted from request variables. It supports optional password checking and can add authenticated user information to headers."
---
...
##### Bearer API Key mode and OIDC identity source

| Field | Type | Description |
|---|---|---|
| `bearerApiKey` | boolean | *Use API Key as Bearer Token*: the key is presented as `Authorization: Bearer <key>`, no password is compared. Requires `checkPassword=false`. Default `false`. |
| `oidcProviderName` | string | Name of the OIDC provider used as the identity service. Allowed only with `bearerApiKey=true`; the presented bearer value is then a provider-issued token validated through the provider (JWKS / introspection). Resolved to the provider id at save time; an unknown name is rejected. Not returned on read. |

```json
{
  "type": "policy-auth-clear-text",
  "name": "bearer-oidc-policy",
  "active": true,
  "usernameVar": { "type": "HEADER", "headerName": "Authorization" },
  "checkPassword": false,
  "bearerApiKey": true,
  "oidcProviderName": "keycloak-prod"
}
```

Validation errors: `oidcProviderName is only allowed when bearerApiKey is true`, `bearerApiKey and checkPassword cannot both be true`.
...
- [Digest Authentication Policy](/api-reference/policies/policies/policy-auth-digest) - HTTP Digest Authentication
