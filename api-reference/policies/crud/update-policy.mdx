---
title: "Update Policy"
description: "Updates an existing policy in an API Proxy. The policy can be updated with new configuration, moved to a different position in the pipeline, or moved to a different scope/pipeline."
---
...
The request body structure is identical to [Add Policy](/api-reference/policies/crud/add-policy). See that documentation for complete field descriptions.
...
{
  "status": "FAILURE",
  "resultMessage": "A policy with name (my-policy) does not exist in API Proxy!"
}
```
