---
title: "Add Policy"
description: "Adds a new policy to an API Proxy. The policy can be added to the request, response, or error pipeline, and can apply to all endpoints or a specific endpoint."
---
...
| errorMessageList | array   | No       | -       | Custom error messages (if not provided, standard messages are used). See [Error Messages](#error-messages) |

**Note:** The `name` field is not included in the request body. The policy name is taken from the path parameter `{policyName}`.

###### Error Messages (errorMessageList) {#error-messages}

Optional list of custom error message objects. When omitted, the platform assigns standard policy error messages for the policy type. Structure matches the product error message model (HTTP status, fault code, message template). See individual policy pages for type-specific examples.
...
- [Authentication Basic](/api-reference/policies/policies/policy-auth-basic)
- … (see [Policies Index](/api-reference/policies/crud/add-policy) for complete list)

###### Policy Condition (PolicyConditionDTO) {#policy-condition}
...
{
  "status": "FAILURE",
  "resultMessage": "A policy with same name (my-policy) is already exist in API Proxy!"
}
```
...
- [Policy API Based Throttling](/api-reference/policies/policies/policy-api-based-throttling) - Example policy documentation
