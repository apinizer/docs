...
`reason` names what has to be fixed:

| `reason` | Meaning | What to do |
|---|---|---|
| `MISSING_TOKEN` | Neither header carried a value — or `Authorization` held nothing but the word `Bearer`. | Send the token in `APINIZER-TOKEN`. |
| `UNKNOWN_TOKEN` | No account holds the token that was sent. | Re-copy the token from **Profile menu → My Profile**, and check that it was not regenerated in the meantime. |
| `INACTIVE_ACCOUNT` | The token belongs to a user account that is not activated. | Re-activate the account, or issue the token from an active one. |
| `EXPIRED_TOKEN` | Every stored copy of the token is revoked or past its expiry date. | Issue a new token and update the caller. |

A deactivated account is reported as `INACTIVE_ACCOUNT` even when its token has also expired: re-activating the account is what has to happen first.

The category exists only in the log. The `401` returned to the caller is identical in all four cases, so a client cannot use it to find out whether an account or a token exists.
