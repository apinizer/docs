...
description: "Retrieves all environment variables for a specified project. Environment variables can be global (same value for all environments) or environment-specific (different values per environment)."
---
...
| environmentValueList | array[object]\|null | List of environment-specific values (if global=false). See [Environment Value Object](#environment-value-object-environmentvaluelist) |
...
| visible | boolean | Whether the value may be read back. `false` marks it Secret. Does **not** control encryption: every value is stored encrypted |
