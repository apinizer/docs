...
| serviceAccountJson | string | No | preserved | GCP Vertex service-account JSON (`OAUTH2` credential type). A blank value preserves the stored secret. Encrypted at rest; never returned |
...
`apiKeyLocation` and `apiKeyPrefix` only apply to the API-key family. Full mapping:
[Credential Type by authScheme](/api-reference/llm-providers/create-llm-provider#credential-type-by-authscheme).
...
- **Enum Serialization**:
  - Enum values must be sent as the enum constant name (e.g. `DEEPSEEK`, `BEARER`, `GATEWAY`, `HEADER`, `CLOUD`, `TEXT`), not lowercase
- **Deploy on Save**:
