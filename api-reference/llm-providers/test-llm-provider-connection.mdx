...
- **`CLIENT`-sourced connections need a one-off test credential**: if the stored provider has
  `credentialSource: CLIENT`, no credential is stored on it. The probe requires a plaintext `apiKey`
  in the request body — used only for this single test call and never persisted. Omitting it (or
  testing with no body at all) fails with "This connection stores no credential (provided by the
  client) - enter a test credential to run the probe".
...
- **`CLIENT`-sourced connections have nothing stored to test**: a provider with `credentialSource: CLIENT` stores no `apiKey`. Supply a temporary, real credential in the body's `apiKey` field to run the probe — it is used once and discarded, not saved.
