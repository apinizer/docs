...
| defaultLimitPlans | array[object] | No | - | Default limit plan per family (`family` + `planKey`). **Omitted** = left unchanged; **empty list** = every default cleared; an entry with a blank `planKey` clears that family, other families are untouched. Applies to consumers created afterwards only. |
