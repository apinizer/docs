...
- The credential still owns [API clients](/en/admin/identity-management/credentials#api-clients) not yet revoked. Revoke them from the manager interface first (see [Suspending, Resuming and Revoking an API Client](/en/admin/identity-management/credentials#suspending-resuming-and-revoking-an-api-client)); already-revoked clients never block the delete
...
- **Permanent Deletion**:
  ...
  - Its API clients are the one exception: a credential still owning API clients not yet revoked is refused with `409 Conflict` until they are revoked
