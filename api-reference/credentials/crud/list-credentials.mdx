...
description: "Retrieves all credentials for a specified project. Credentials are used for authentication and authorization to access API Proxies and API Proxy Groups."
---
...
| trafficLogDisabled | boolean\|null | Whether this consumer's traffic is kept out of the traffic log connectors. `null` on a record where it was never set. It silences the traffic log connectors only — trace records, OpenTelemetry spans, metrics and the security/access events are unaffected. |
...
- [Get Granted Access List](/api-reference/credentials/access/get-granted-access-list) - Get access list for credential
