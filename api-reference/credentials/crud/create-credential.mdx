...
description: "Creates a new credential for a specific project. Credentials are used for authentication and authorization to access API Proxies and API Proxy Groups. The credential is automatically deployed to all environments in the project."
---
...
| trafficLogDisabled | boolean\|null | No | null | Do not write this consumer's traffic to the traffic log connectors. **`null` means "not sent" and leaves the stored value untouched** — unlike the other fields on this endpoint, an omitted value is not treated as a reset, so a client written before this field existed cannot silently re-enable logging for a consumer an operator deliberately silenced. Send `false` explicitly to turn logging back on. Narrow scope: trace records, OpenTelemetry spans, metrics and the security/access events are unaffected. |
...
| metadata | array[object] | No | [] | Custom metadata entries. See [Metadata Object](#metadata-object) |
...
- Username already used by another credential — usernames are unique across **all** projects, so the holder may sit in a project other than the one you are posting to
...
- [Update Credential](/api-reference/credentials/crud/update-credential) - Update a credential
- [Change Credential Password](/api-reference/credentials/crud/change-credential-password) - Change credential password
- [Grant Access](/api-reference/credentials/access/grant-access) - Grant access to API Proxy or Group
