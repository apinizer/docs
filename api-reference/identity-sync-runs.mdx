...
| `status` | enum | `QUEUED` \| `RUNNING` \| `SUCCESS` \| `WARNING` \| `FAILED` \| `AWAITING_RECONCILE_APPROVAL` \| `INTERRUPTED` |
...
`WARNING` means the run finished without error but its reconciliation step was deliberately skipped as a safety measure (the source returned no subjects, or its subject list could only be read in part) — nothing was deactivated or deleted.
...
| `lifecycleConflict` | `params.reason` names the cause: `syncDisabled` (the source's synchronization is not enabled), `syncRequiresKeycloakVendor` (an OIDC source without a Keycloak Admin REST API), `syncAlreadyRunning` (a second run was queued while one was already active), `reconcileApprovalConflict` (the run is not awaiting approval, or a newer run replaced the pending approval), or `unknownSourceType` (defensive — the run's own recorded source type could not be resolved to a synchronization job) |
