...
| `reactivated` | integer | Records this run automatically re-enabled because they came back in the source. Synchronization only ever reopens what it deactivated itself — an account an administrator disabled by hand is never touched. Absent on runs recorded before this counter existed |
| `usersRead` | integer | Total number of user entries read from the directory in this run. LDAP sources only; absent for other source types and for runs recorded before this counter existed |
| `usersWithUserAttr` | integer | Of `usersRead`, the users whose own entry carried the membership attribute (`memberOf` and the like). LDAP only |
| `groupsRead` | integer | Total number of group/organizational-unit entries read in this run. LDAP only |
| `groupsWithMembers` | integer | Of `groupsRead`, the groups whose member attribute was non-empty. LDAP only |
| `groupsFiltered` | integer | Groups the group/organization scope filter dropped — a scope decision, not an error; these groups are not created as organizations. Distinct from `filtered` above, which counts excluded subjects, not groups. LDAP only |
| `linkedByS1` | integer | Users attached to an organization from the membership attribute on their own entry (for example `memberOf`). LDAP only |
| `linkedByS2` | integer | Users attached to an organization from a group's member list (for example `member`/`uniqueMember`/`memberUid`). LDAP only |
| `linkedByS3` | integer | Users attached to an organization from the organizational unit their DN sits under. LDAP only |
| `unlinked` | integer | Users no membership source could attach to an organization — the number to act on. LDAP only |
| `orgsRead` | integer | Rows read from the organization list of the source. `0` means no organization list is configured. Database and API sources only |
| `orgsDerived` | integer | Organizations built from the path on the user row because the organization list has no matching row. Database and API only |
| `linked` | integer | Users attached to an organization. On LDAP sources the same information is split into the three `linkedByS1`/`linkedByS2`/`linkedByS3` fields above, so this field stays absent there. Database and API only |
| `disabledAtSource` | integer | Records this run switched off because the account is reported disabled at the source — distinct from `deactivated`, which means the record was no longer seen in the source at all. Database and API only |
| `pagesRead` | integer | Pages actually read from a paging source. Absent for sources that read everything in one query. Database and API only |
...
`WARNING` means the run finished without error but its reconciliation step was deliberately skipped as a safety measure (the source returned no subjects, or its subject list could only be read in part) — nothing was deactivated or deleted; a partial read still applies the rows it did manage to read. `CANCELLED` means a [cancel](#cancel-a-queued-or-running-run) action was accepted and the run stopped before completing its fetch — unlike a partial read, nothing fetched during a cancelled run is written, and reconciliation never ran.

A finished LDAP run, with its membership diagnostics:
...
```json
{
  ...
  "trigger": "SCHEDULED",
  "startedAt": "2026-09-03T10:15:00Z",
  "finishedAt": "2026-09-03T10:15:42Z",
  "durationMs": 41830,
  "phase": "RECONCILE",
  "processedCount": 480,
  "upserted": 12,
  ...
  "reactivated": 1,
  "usersRead": 485,
  "usersWithUserAttr": 480,
  "groupsRead": 40,
  "groupsWithMembers": 37,
  "groupsFiltered": 6,
  "linkedByS1": 470,
  "linkedByS2": 8,
  "linkedByS3": 0,
...
A run recorded before these counters existed reads all ten fields (`reactivated`, `usersRead`, `usersWithUserAttr`, `groupsRead`, `groupsWithMembers`, `groupsFiltered`, `linkedByS1`, `linkedByS2`, `linkedByS3`, `unlinked`) as absent, not zero. The nine LDAP-only fields are also absent on `database`, `api`, and `oidc` runs.

`orgsRead`, `orgsDerived`, `linked`, `disabledAtSource`, and `pagesRead` follow the same rule in the other direction: they are absent on `ldap` and `oidc` runs, and on any run recorded before this counter set existed.
