...
- `409 lifecycleConflict` (`params.reason = alreadyHeld`) — the subject already has an active legal hold.
...
- `409 lifecycleConflict` (`params.reason = notActive`) — the hold has already been released.
...
- `409 lifecycleConflict` (`params.reason = legalHold`) — the subject has an active legal hold.
- `409 lifecycleConflict` (`params.reason = activeRunExists`) — the subject already has a run in progress.
- `409 lifecycleConflict` (`params.reason = modeNotApplicable`) — the mode does not match the subject's current state.
- `409 lifecycleConflict` (`params.reason = correctionNotEligible`) — `CORRECTION_PURGE` was requested on a subject past its pre-operational state.
- `409 lifecycleConflict` (`params.reason = retentionNotReached`) — `RETENTION_PURGE` was requested before `retentionDays` had elapsed since the subject became terminal.
- `400 malformedRequest` — the subject reference does not resolve to any subject of the given type.

Approval is refused with:

- `409 lifecycleConflict` (`params.reason = digestMismatch`) — the submitted digest does not match the run's current manifest (the underlying data changed since preview; run a new preview).
- `409 lifecycleConflict` (`params.reason = sameActor`) — the approving user is the same user who ran the preview (waived for a system administrator).
- `409 lifecycleConflict` (`params.reason = invalidState`) — the run is not in `PREVIEWED` state.

Queuing execution is refused with:

- `409 lifecycleConflict` (`params.reason = invalidState`) — the run is not in `APPROVED` state.

Cancellation is refused with:

- `409 lifecycleConflict` (`params.reason = cannotCancelAfterCommit`) — the run has already passed its point of no return.
- `409 lifecycleConflict` (`params.reason = invalidState`) — the run has already reached a final state.
...
| `lifecycleConflict` | `params.reason` names the cause — see [Legal holds](#legal-holds) and [Purge modes and the four-eyes workflow](#purge-modes-and-the-four-eyes-workflow) above for the full list (`alreadyHeld`, `notActive`, `legalHold`, `activeRunExists`, `modeNotApplicable`, `correctionNotEligible`, `retentionNotReached`, `digestMismatch`, `sameActor`, `invalidState`, `cannotCancelAfterCommit`) |
...
# 409 lifecycleConflict, params.reason = "legalHold"
