---
title: "Update AI Prompt-Decorator Preset"
description: "Updates an existing custom AI prompt-decorator preset in the given scope. Requires an admin API token. The preset must already exist; built-in presets are read-only and reject this call."
---

## Endpoint

```
PUT /apiops/settings/ai-prompt-decorator-presets/{presetName}/
```

## Authentication

Requires a Personal API Access Token with admin privileges.

### Header

```
Authorization: Bearer YOUR_TOKEN
```

## Request

### Headers

| Header | Value | Required |
|--------|-------|----------|
| Authorization | Bearer \{token\} | Yes |
| Content-Type | application/json | Yes |

### Path Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| presetName | string | Yes | Name of the preset to update. Must match the `name` in the body when that field is present. |

### Query Parameters

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| projectId | string | No | `admin` | Scope the preset lives in. Omit for the admin/global scope. |

### Request Body

```json
{
  "name": "support-tone",
  "content": "Answer in a calm, professional tone, always offer a next step, and never promise a delivery date.",
  "role": "system",
  "position": "PREPEND",
  "category": "tone",
  "description": "House style for support assistants",
  "enabled": true
}
```

The body carries the same fields as [Create AI Prompt-Decorator Preset](/api-reference/ai-prompt-decorator-presets/create-ai-prompt-decorator-preset). The whole preset is replaced, so send every field you want to keep — an omitted optional field is cleared, not preserved.

## Response

### Success Response (200 OK)

```json
{
  "status": "SUCCESS"
}
```

### Error Response (400 Bad Request)

Returned when the preset does not exist in the scope, the path and body names disagree, `content` is missing, `role`/`position` is invalid, the target is a built-in preset, or the caller lacks the `ADMIN` role.

```json
{
  "status": "FAILURE",
  "resultMessage": "AI prompt-decorator preset (name: support-tone) was not found!"
}
```

### Error Response (401 Unauthorized)

```json
{
  "status": "FAILURE",
  "resultMessage": "Token is not valid!"
}
```

## cURL Example

```bash
curl -X PUT \
  "https://demo.apinizer.com/apiops/settings/ai-prompt-decorator-presets/support-tone/" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "content": "Answer in a calm, professional tone, always offer a next step, and never promise a delivery date.",
    "role": "system",
    "position": "PREPEND",
    "enabled": true
  }'
```

## Notes and Warnings

- **Must Already Exist**:
  - Unlike `POST`, this endpoint does not create a missing preset — it reports not-found instead
- **Full Replacement**:
  - Optional fields left out of the body are cleared
- **Built-in Presets**:
  - Rows with `builtIn: true` are read-only and reject this call
- **No Redeploy Implied**:
  - Blocks already imported into policies are copies and keep their previous content

## Related Documentation

- [Create AI Prompt-Decorator Preset](/api-reference/ai-prompt-decorator-presets/create-ai-prompt-decorator-preset) - Create or upsert a preset
- [Get AI Prompt-Decorator Preset](/api-reference/ai-prompt-decorator-presets/get-ai-prompt-decorator-preset) - Get a single preset by name
- [AI Prompt-Decorator Presets API](/api-reference/ai-prompt-decorator-presets) - Resource overview
