...
description: "Updates an existing AI privacy (PII) preset by name within the given scope, built-in or custom. The preset must already exist. Requires an admin API token."
---
...
- **Built-in Presets Are Not Read-Only**:
  - Updating a built-in (seeded) preset succeeds the same as a custom one
- **Full Replacement**:
