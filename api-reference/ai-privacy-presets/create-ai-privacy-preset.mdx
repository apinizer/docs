...
description: "Creates a custom AI privacy (PII) preset, or updates it when one with the same name already exists in the scope (upsert by name). Requires an admin API token. A name that resolves to a built-in preset is updated too — built-in presets are not read-only on this endpoint."
---
