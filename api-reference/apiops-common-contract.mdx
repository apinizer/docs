...
Endpoints that existed before this contract keep their original `status` / `resultMessage` envelope, described in [Error Handling](/api-reference/getting-started/error-handling). Both styles are live at the same time. Tell them apart by `correlationId`: an error body that carries it follows the contract on this page, and the older envelope never has it.

A few of the older endpoints answer a validation failure with a third shape, an RFC 7807 problem body, recognisable by its `type` and `path` fields and a numeric `status`. That body carries an `errorKey` too, so the presence of that field on its own does not mean you are reading the contract on this page.
