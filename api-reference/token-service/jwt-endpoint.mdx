...
To refresh an expired JWT access token without re-authenticating, call the main JWT endpoint with `grant_type=refresh_token` (same URL as issuance):
...
## JWT Regenerate Endpoint

When JWT Authentication policy **regenerate** is configured (Authentication API + Regenerator service), you can obtain a new access JWT from a still-valid access token:
...
Send the **current access JWT** as a Bearer token. Authenticate the client with `client_id` and `client_secret` in the form body (or Basic auth):

```bash
curl -X POST \
  "https://yourApinizerAddress/auth/jwt/regenerate" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -H "Authorization: Bearer YOUR_CURRENT_ACCESS_JWT" \
  -d "client_id=your-app-key&client_secret=your-app-secret"
```

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| client_id | string | Yes (if not in Authorization Basic header) | Application key or credential client ID |
| client_secret | string | Yes (if not in Authorization Basic header) | Application secret or credential client secret |

:::note

Regenerate is **not** the same as refresh: refresh uses `grant_type=refresh_token` on `/auth/jwt`; regenerate uses `/auth/jwt/regenerate` with a Bearer access token. Do not send `grant_type=refresh_token` to the regenerate URL.
