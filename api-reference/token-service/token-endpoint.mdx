...
The `scope` field is only included in the response if scope was requested in the request body and the endpoint is configured to return it. This behavior is controlled by [Token Management Settings](/en/admin/identity-management/token-management-settings).
...
- `invalid_client` — Authentication failed (invalid client_id or client_secret). Returned with **HTTP 401 Unauthorized** when credentials are wrong.
...
Other grant/validation errors use **HTTP 400 Bad Request**:
