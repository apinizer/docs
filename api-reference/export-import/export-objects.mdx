...
- **Every package carries an integrity record**: alongside the object files, the package contains `apinizer-transfer-manifest.json` — a list of the files in the package with a fingerprint of each. Import verifies it (see [Import Objects](/api-reference/export-import/import-objects)), so a package must be shipped exactly as this endpoint produced it; a pipeline that unzips, edits and re-zips a package will have it refused on import. That file name is reserved: if an exported object's name resolves to it, the call fails with **400 Bad Request** naming the object, rather than dropping either the object or the record.
- **Objects that could not be written are recorded**: if an object cannot be written into the package, the response is unchanged, but the object's name is listed in the package's integrity record instead of disappearing silently.
