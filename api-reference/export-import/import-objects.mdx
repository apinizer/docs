...
{
  "status": "FAILURE",
  "resultMessage": "This file is a Postman/Testnizer test collection export, not an Apinizer transfer package. Import it from Test Console → Test Collections instead.",
  "errorKey": "importPackageIsTestCollectionInteropJson"
}
```

Returned when a `TEST_COLLECTION` import is called with a Postman- or Testnizer-exported `.json` file — that format is a separate, non-Apinizer file this endpoint does not read; see [Exporting and Importing a Collection](/en/develop/test-debug-tools/test-console#exporting-and-importing-a-collection) for where to import it instead.
