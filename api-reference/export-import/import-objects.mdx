...
| acknowledgeChecksumMismatch | boolean | No | false | When `true`, a transfer-manifest checksum mismatch (a package file whose content was edited or re-saved after export) is accepted instead of rejected, and the import proceeds. The response then carries a `warn` advisory with key `transfer.checksumMismatchAcknowledged`, and the acknowledging user is written to the operational log. Only a checksum mismatch is acknowledgeable this way — a malformed manifest and a missing/undeclared package file (an entry-count mismatch) are rejected with 400 regardless of this flag |
...
When the import ran with `acknowledgeChecksumMismatch=true` and the package did carry a mismatch, the response also carries the advisory that records it:
...
- A package file's content changed since export and `acknowledgeChecksumMismatch` was not sent as `true` — or the package is missing a declared file or carries one the manifest never declared, which stays rejected even with the flag
...
- **A checksum mismatch alone can be overridden**: `acknowledgeChecksumMismatch=true` accepts a file whose content changed after export and lets the import proceed; the response then carries the `transfer.checksumMismatchAcknowledged` warn advisory (in `advisories`, and as a rendered line in `unresolvedReferences`) and the acknowledging user is written to the operational log. This is the only kind of manifest failure the flag widens — a manifest that cannot be read and a file set that does not match the record (a declared file missing, or a file the record never mentions) are rejected with **400 Bad Request** no matter what this parameter is set to.
