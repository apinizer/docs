...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| description | string | No | Description/notes for the deploy history record. Defaults to "Deployed by API Call" if not provided. |
...
```bash
curl -X POST \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxyGroups/PaymentAPIGroup/environments/production/?description=v2.1%20release&persistent=true" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
