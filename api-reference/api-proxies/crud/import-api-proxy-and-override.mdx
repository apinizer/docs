---
title: "Import API Proxy (With Override)"
description: "Imports an API Proxy from a ZIP file. If an API Proxy with the same name already exists, it will be overridden. If it does not exist, a new API Proxy will be created. This endpoint supports optional metadata for deployment and routing configuration."
---
...
|-------|------|----------|-------------|
| algorithm | string | No | Routing algorithm. See [EnumRoutingAlgorithm](#enumroutingalgorithm-routingalgorithm) |
...
| soapType | string | No | SOAP version for SOAP APIs. See [EnumSoapApiPortType](#enumsoapapiporttype-soaptype) |
...
| advisories | array[object] | Machine-readable import report (dropped references, objects that need a deploy, replaced secrets) — stable `key` + `level` + raw `params`, the same shape the generic `/export-import/` endpoints and the POST import return. Returned together with `deploymentResult`; empty when nothing needs attention |
| unresolvedReferences | array[string] | The same report as one human-readable line per advisory (`key: param=value …`) |

:::note Replace keeps identity and marks a redeploy
Importing over an existing API Proxy keeps the target's id, public/secret key, trace settings and deployment state. Every environment the proxy is currently deployed to is flagged as *redeploy required*, because the content changed while the gateway still serves the previous definition. The import itself never deploys — pass `deploy=true` in the metadata (or call the deploy endpoint) to make the change live.
:::
...
or, when the package is an MCP gateway (`withDependencies=true`) that exposes another API Proxy's endpoints as tools — see [Import-Time Errors for MCP Gateway Source Proxies](/api-reference/api-proxies/crud/export-api-proxy#import-time-errors-for-mcp-gateway-source-proxies):
...
- The package is an MCP gateway whose tool's source API Proxy could not be resolved, was ambiguous, or is missing the pinned endpoint (`mcpSourceProxyUnresolved` / `mcpSourceProxyAmbiguous` / `mcpToolEndpointMissing`) — see [Export API Proxy](/api-reference/api-proxies/crud/export-api-proxy#import-time-errors-for-mcp-gateway-source-proxies)
...
- [Export API Proxy](/api-reference/api-proxies/crud/export-api-proxy) - Export API Proxy to ZIP file
- [Import API Proxy](/api-reference/api-proxies/crud/import-api-proxy) - Import API Proxy without override
- [Deploy API Proxy](/api-reference/api-proxies/deployment/deploy-api-proxy) - Deploy API Proxy to environments
- [Update Routing Addresses](/api-reference/api-proxies/settings/update-routing-addresses) - Update routing configuration
