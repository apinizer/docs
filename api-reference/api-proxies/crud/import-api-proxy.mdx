...
description: "Imports an API Proxy from a ZIP file. If an API Proxy with the same name already exists, the imported API Proxy will have a suffix added to its name and relative path to prevent conflicts. This method does not override existing API Proxies."
---
...
```json
{
  ...
  "advisories": [
    {
      "level": "warn",
      "key": "droppedReferences",
      "params": { "names": "my-credential -> organizationId: no matching REGISTERED_ORGANIZATION in this project" }
    }
  ],
  "unresolvedReferences": [
    "droppedReferences: names=my-credential -> organizationId: no matching REGISTERED_ORGANIZATION in this project"
  ]
}
```
...
| advisories | array[object] | Machine-readable import report — everything the import could not carry or had to decide on its own (dropped references, objects that need a deploy, secrets that were replaced). Each entry has a stable `key`, a `level` (`info`/`warn`) and raw `params`. Same shape as the generic `/export-import/` endpoints. Empty when nothing needs attention |
| unresolvedReferences | array[string] | The same report as one human-readable line per advisory (`key: param=value …`). Kept for callers that predate `advisories` |
...
- [List API Proxies](/api-reference/api-proxies/crud/list-api-proxies) - List all API Proxies in project
