...
- **No Override**:
  ...
  - This endpoint never replaces an existing certificate/keystore/key object either, so the `certEnvironmentMergeMethod` / `certEnvironmentUncoveredAction` parameters (accepted by the "With Override" variant of this endpoint) do not apply here
