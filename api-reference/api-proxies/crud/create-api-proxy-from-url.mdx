...
If the project has a default [Global Settings Group](/en/develop/global-configurations/global-settings-group) that itself defines maintenance mode, the values sent here are immediately overwritten by that group's values once the proxy is created — the proxy is attached to the default group at creation time, and a group's values always take precedence over the proxy's own.
