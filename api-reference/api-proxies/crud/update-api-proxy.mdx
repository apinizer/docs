---
title: "Update API Proxy"
description: "Updates an existing API proxy. The update can be done by reparsing from a new specification URL or file. The API proxy can be automatically deployed to specified environments after update."
---
...
The request body structure is identical to [Create API Proxy from URL](/api-reference/api-proxies/crud/create-api-proxy-from-url) or [Create API Proxy from File](/api-reference/api-proxies/crud/create-api-proxy-from-file), with the following differences:
...
5. **Body Read Behavior**: If `requestBodyReadBehavior` or `responseBodyReadBehavior` is not provided, existing values are preserved
...
{
  "status": "FAILURE",
  "resultMessage": "ApiProxy (name: petstore-api) can not be reparsed because it is not exist. First create it!"
}
```
...
- **USD budget price check (400)**: when the saved routing or `aiAppBudget` differs from the stored proxy and a USD budget covering the proxy (its own `aiAppBudget`, the routing's `anonymousBudget`, the project's tenant budget or the global default / anonymous-default budgets) would meter a model with no input price in the model catalog, the request is rejected with `400` and error key `aiQuota.usdBudgetModelPriceMissing`. Nothing is saved. An unchanged re-save is never re-checked.
