---
title: "Update API Proxy from URL"
description: "Updates an existing API proxy by re-parsing an OpenAPI (Swagger), WSDL specification from a URL, or updates a reverse proxy. The API proxy can be automatically deployed to specified environments after update."
---
...
| apiProxyCreationType | string | Yes | - | API creation type. Must match existing API Proxy type. See [EnumApiProxySpecType](#enumapiproxyspectype-apiproxycreationtype) |
| specUrl | string | Yes* | - | Specification URL (required if not reverse proxy) |
| clientRoute | object | Yes | - | Client route configuration. See [ClientRoute](#clientroute-clientroute) |
| routingInfo | object | No | - | Routing configuration. See [RoutingInfo](#routinginfo-routinginfo) |
...
| maintenanceModeSetting | object | No | - | Maintenance mode settings. See [MaintenanceModeSetting](#maintenancemodesetting-maintenancemodesetting) |
| specAuthorizationValueList | array | No | [] | Authorization headers for accessing spec URL. See [SpecAuthorizationValue](#specauthorizationvalue-specauthorizationvaluelist-item) |
...
| hostList | array | No | List of virtual hosts |

### RoutingInfo (routingInfo)

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| routingAddressList | array | No | List of routing addresses. See [RoutingAddress](#routingaddress-routinginforoutingaddresslist-item) |
...
| healthMonitoringEnabled | boolean | No | Enable per-address health monitoring (HTTP type only) |
...
- **Deployment**:
  ...
  - Every environment name is validated **before** the API Proxy is touched. If a name does not exist (or is not accessible to the user), the request fails with `400 Bad Request`, the specification is not re-parsed and the API Proxy is left completely unchanged
  - Environments the API Proxy is already deployed to but that are not listed in `deployTargetEnvironmentNameList` keep their deployment; they are flagged as requiring a redeploy so the updated definition can be rolled out to them later
...
- [Create API Proxy from URL](/api-reference/api-proxies/crud/create-api-proxy-from-url) - Create new API Proxy from URL
- [Update API Proxy](/api-reference/api-proxies/crud/update-api-proxy) - Update API Proxy metadata
