---
title: "Update API Proxy from File"
description: "Updates an existing API proxy by re-parsing an OpenAPI (Swagger), WSDL specification from an uploaded file, or updates a reverse proxy. The API proxy can be automatically deployed to specified environments after update. For WSDLs with many XSDs, use a ZIP file."
---
...
| loadBalanceAlgorithm | string | No | Load balancing algorithm. See [EnumRoutingAlgorithm](#enumroutingalgorithm-routinginfoloadbalancealgorithm) |
...
- [Create API Proxy from File](/api-reference/api-proxies/crud/create-api-proxy-from-file) - Create new API Proxy from file
