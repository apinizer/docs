---
title: "Create API Proxy from File"
description: "Creates an API proxy by parsing an OpenAPI (Swagger), WSDL specification from an uploaded file, or creates a reverse proxy. The API proxy can be automatically deployed to specified environments after creation. For WSDLs with many XSDs, use a ZIP file."
---
...
**Note:** `EMPTY_API` (No-Spec API) is **not** supported by this file-upload endpoint (request is
rejected) — use [Create API Proxy from URL](/api-reference/api-proxies/crud/create-api-proxy-from-url#no-spec-api-empty_api) instead.
...
- [Create API Proxy from URL](/api-reference/api-proxies/crud/create-api-proxy-from-url) - Create from URL instead of file
