---
title: "Delete Method-Level Override"
description: "Deletes a method-level connector override for traffic log settings. After deletion, the method inherits proxy-level settings for the specified connector."
---

## Endpoint

```
DELETE /apiops/projects/{projectName}/apiProxies/{apiProxyName}/trafficLogSettings/{environmentName}/connectors/{connectorType}/methodOverride/?methodName={methodName}
```

## Authentication

Requires a Personal API Access Token.

### Header

```
Authorization: Bearer YOUR_TOKEN
```

## Request

### Headers

| Header | Value | Required |
|--------|-------|----------|
| Authorization | Bearer \{token\} | Yes |

### Path Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| projectName | string | Yes | Project name |
| apiProxyName | string | Yes | API Proxy name |
| environmentName | string | Yes | Environment name |
| connectorType | string | Yes | Connector type (`EnumExecutableType` name). See [Connector Types](#connector-types) below. |

### Query Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| methodName | string | Yes | API method/endpoint name |

### Connector Types

`ELASTICSEARCH_INDEX`, `DB_INSERT_UPDATE_DELETE`, `SYSLOG_SEND`, `WEBHOOK_SEND`, `RABBITMQ_SEND`, `ACTIVEMQ_SEND`, `KAFKA_SEND`, `GRAYLOG_SEND`, `LOGBACK_APPEND`

## Response

### Success Response (200 OK)

```json
{
  "status": "SUCCESS"
}
```

### Error Response (500 Internal Server Error)

```json
{
  "status": "FAILURE",
  "resultMessage": "Traffic log settings not found"
}
```

:::note

If no method-level override exists for the given method and connector type, the endpoint returns **200 OK** with no error (no-op).

:::

## cURL Example

```bash
curl -X DELETE \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/MyAPI/trafficLogSettings/production/connectors/ELASTICSEARCH_INDEX/methodOverride/?methodName=/pets" \
  -H "Authorization: Bearer YOUR_TOKEN"
```

:::tip

`methodName` is a query parameter, so method names that start with or contain `/` (e.g., `/pets`) work correctly.

:::

## Notes and Warnings

- **Automatic Deployment**: Changes are automatically deployed after save
- **Fallback**: After deletion, the method uses proxy-level settings for the connector
- **No-op**: If the override does not exist, the request succeeds with `200 OK` and no changes are made
- **Cleanup**: If no connector overrides remain for a method, the entire method override entry is removed

## Permissions

User must have **`API_MANAGEMENT` + `MANAGE`** permission in the project.

## Related Documentation

- [Update Method Override](/api-reference/api-proxies/traffic-log-settings/update-method-override) - Add/update a method override
- [Update Traffic Log Settings](/api-reference/api-proxies/traffic-log-settings/update) - Update proxy-level settings
