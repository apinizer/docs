...
| identifierHttpMethod | string | Yes | - | HTTP method for the endpoint (used to identify the endpoint). See [EnumHttpRequestMethod](/api-reference/appendix/enum-reference#enumhttprequestmethod) |
...
| wsaMustUnderstand | string | No | NONE | MustUnderstand attribute value. See [EnumSoapWsaMustUnderstand](#enumsoapwsamustunderstand-wsamustunderstand) |
| wsaVersion | string | No | V200508 | WSA version. See [EnumSoapWsaVersion](#enumsoapwsaversion-wsaversion) |
...
When `wsaEnabled` is `true`, the payload is validated before saving. The request is rejected with `400 Bad Request` if any of the following conditions are met:

- `wsaVersion` is missing
- `wsaMustUnderstand` is missing
- `wsaAddDefaultAction` is `false` but `wsaAction` is empty
- `wsaAddDefaultTo` is `false` but `wsaTo` is empty
- `wsaGenerateMessageId` is `false` but `wsaMessageId` is empty

When `wsaEnabled` is `false` (or `wsaSettings` is omitted), these checks are skipped. A text field is not required while its corresponding default toggle is enabled — for example, `wsaAction` may be empty when `wsaAddDefaultAction` is `true`.
...
Returned when WSA is enabled but a required field is missing:

```json
{
  "error": "bad_request",
  "error_description": "wsaVersion can not be null when WSA is enabled!"
}
```

Other validation messages follow the same pattern: `wsaMustUnderstand can not be null when WSA is enabled!`, `wsaAction can not be empty when wsaAddDefaultAction is false!`, `wsaTo can not be empty when wsaAddDefaultTo is false!`, and `wsaMessageId can not be empty when wsaGenerateMessageId is false!`.
