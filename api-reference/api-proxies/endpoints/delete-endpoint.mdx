...
description: "Deletes an endpoint (API method) from an API proxy. This operation is only available for REST API proxies. The endpoint is permanently removed."
---
...
| httpMethod | string | Yes | HTTP method for the endpoint (used to identify the endpoint). See [EnumHttpRequestMethod](/api-reference/appendix/enum-reference#enumhttprequestmethod) |
...
{
  "status": "FAILURE",
  "resultMessage": "Endpoint can only be deleted from REST API Proxies!"
}
```
