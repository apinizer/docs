...
| cacheKeyType | string | No | QUERY_PARAMS | Cache key type. See [EnumCacheKeyType](#enumcachekeytype-cachekeytype) |
| cacheStorageType | string | No | DISTRIBUTED | Cache storage type. See [EnumCacheStorageType](#enumcachestoragetype-cachestoragetype) |
| capacity | integer | No | - | Maximum cache capacity (number of entries) |
| ttl | integer | No | - | Time to live in seconds |
| handlingAction | string | Yes | - | Cache handling action when cache hit occurs. See [EnumCacheHandlingAction](#enumcachehandlingaction-handlingaction) |
