...
description: "Returns the full detail of a specific deploy history record, including the API Proxy snapshot at the time of deployment."
---

## Endpoint

```
GET /apiops/projects/{projectName}/apiProxies/{apiProxyName}/deploy-histories/{deployHistoryId}/
```
...
| deployHistoryId | string | Yes | Deploy history record ID |
...
| resultList[].id | string | Deploy history record ID |
...
{
  "error": "bad_request",
  "error_description": "Deploy history with id (6630a1b2c3d4e5f6a7b8c9d0) is not found!"
}
```
