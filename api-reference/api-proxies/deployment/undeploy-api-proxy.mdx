---
title: "Undeploy API Proxy"
description: "Undeploys an API proxy from a specific environment. The API proxy is removed from the target environment and is no longer accessible."
---
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| description | string | No | Description/notes for the deploy history record. Defaults to "Undeployed by API Call" if not provided. |
...
```bash
curl -X DELETE \
  "https://demo.apinizer.com/apiops/projects/MyProject/apiProxies/petstore-api/environments/production/?description=Scheduled%20maintenance&persistent=true" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
