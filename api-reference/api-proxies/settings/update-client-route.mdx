---
title: "Update Client Route"
description: "Updates the client route configuration for an API Proxy. Client routes define how clients access the API Proxy, including path matching, HTTP methods, host filtering, and header-based routing."
---
...
|-------|------|----------|---------|-------------|
| clientRoute | object | Yes | - | Client route configuration object (see fields below) |
...
| methodList | array[string] | No | null | List of HTTP methods to match. See [EnumHttpRequestMethod](/api-reference/appendix/enum-reference) |
...
{
  "status": "FAILURE",
  "resultMessage": "Client route must have at least one valid route configuration"
}
```
...
- **Deploy**:
  - When `deploy` is set to `true`, the API proxy will be deployed to the specified environments after saving changes
  - `deployTargetEnvironmentNameList` is required when `deploy=true`
  - If deployment fails for some environments, successfully deployed environments are still reported in the response
## Permissions
...
- [Update API Proxy](/api-reference/api-proxies/crud/update-api-proxy) - Update API proxy configuration
