...
| exposedTools | array | **TOOL_EXPOSE only.** Tool definitions — `proxyId` (target ApiProxy id), `toolName`, `toolDescription`, optional `overrideSchema`, `apiMethodHttpMethod`/`apiMethodPath` (pin to a specific method), `requiredScopes`, per-tool `argumentConstraints`. `toolName` is normalized before it is published in `tools/list` (lowercased, any character other than a letter or digit becomes `_`, repeated underscores collapsed, truncated to 64 chars; colliding names get a `_2`/`_3` suffix). A blank `toolName` falls back to the normalized `proxyId`. The published catalog changes only after the proxy is deployed again |
