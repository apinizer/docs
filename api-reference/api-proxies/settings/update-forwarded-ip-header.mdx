...
{
  "status": "FAILURE",
  "resultMessage": "Invalid forwarded IP header settings"
}
```
...
Only `xffOrder` is sent, so `forwardedIpHeaderParamActive` and the header name keep the values stored on the proxy.
