...
The AI proxy runtime does not read the classic `routing` object; the equivalent configuration lives under `aiRouting`. Use [Update AI Routing](/api-reference/api-proxies/settings/update-ai-routing) instead.
:::
...
```json
{
  ...
  "preserveHttpMethodOnRedirect": false,
...
| preserveHttpMethodOnRedirect | boolean | No | false | Preserve the original HTTP method and body on 301/302/303/307/308 redirects. See [Preserve HTTP Method and Body on Redirect](#preserve-http-method-and-body-on-redirect-preservehttpmethodonredirect) below |
| disableContentCompression | boolean | No | - | Disable content compression |
| enableStreaming | boolean | No | - | Enable streaming mode |
| ignoreRoutingError | boolean | No | false | Ignore the API proxy's error response template when the backend returns an error. See [Ignore Error Response Template](#ignore-error-response-template-ignoreroutingerror) below |
| queryEncodingMode | string | No | STANDARD | How query parameter values are encoded before routing to the backend. See [EnumQueryEncodingMode](#enumqueryencodingmode) below |
...
See also the Manager documentation: [HTTP Routing — Query Parameter Encoding](/en/develop/routing/http-routing#query-parameter-encoding).

### Preserve HTTP Method and Body on Redirect (preserveHttpMethodOnRedirect)

`preserveHttpMethodOnRedirect` corresponds to the **"Preserve HTTP Method and Body on Redirect"** option on the API Proxy routing UI.

- `false` (default) — no change from existing behavior: 301/302/303 redirects returned by the backend downgrade the request to GET and drop the body before it is sent to the redirect target.
- `true` — the original HTTP method and body are preserved on all redirects (301, 302, 303, 307, 308) instead of being downgraded/dropped.

This setting only applies to backend connections using HTTP/1.1 **with streaming disabled**. If `enableStreaming=true` in this same request body (for example, for Server-Sent Events proxies), or if the API Proxy's Backend HTTP Version policy (configured separately, not exposed through this endpoint) is set to a value other than HTTP/1.1, `preserveHttpMethodOnRedirect` has no effect regardless of its value — 301/302/303 redirects on that path still downgrade the request to GET and drop the body. In this case the Manager UI shows a warning below the corresponding setting on the routing screen, and the worker log records a warning when such a redirect would drop the method.

See also the Manager documentation: [HTTP Routing — Connection Settings Definition](/en/develop/routing/http-routing#connection-settings-definition).
...
- `false` (default) — when the backend responds with an error, the API proxy's configured error response template (see [Update JSON Error Template](/api-reference/api-proxies/settings/update-json-error-template) / [Update XML Error Template](/api-reference/api-proxies/settings/update-xml-error-template)) is applied and returned to the client instead of the raw backend response.
...
This setting is also available at API proxy create/update time via `routingInfo.ignoreRoutingError` on [Create API Proxy from URL](/api-reference/api-proxies/crud/create-api-proxy-from-url) / [Update API Proxy from URL](/api-reference/api-proxies/crud/update-api-proxy-from-url) (and their file-based equivalents).
...
- **Redirects**: When `preserveHttpMethodOnRedirect=true`, the original HTTP method and body are preserved on all redirects (301/302/303/307/308) instead of being downgraded to GET — see [Preserve HTTP Method and Body on Redirect](#preserve-http-method-and-body-on-redirect-preservehttpmethodonredirect) above. Only applies to HTTP/1.1 backend connections.
...
- **Error Response Template**: When `ignoreRoutingError=true`, backend error responses are passed through to the client as-is instead of being wrapped in the API proxy's error response template — see [Ignore Error Response Template](#ignore-error-response-template-ignoreroutingerror) above
...
- [Update JSON Error Template](/api-reference/api-proxies/settings/update-json-error-template) - Configure the JSON error response template
- [Update XML Error Template](/api-reference/api-proxies/settings/update-xml-error-template) - Configure the XML error response template
- [Get API Proxy](/api-reference/api-proxies/crud/get-api-proxy) - Get API proxy details
- [Get Deploy Status](/api-reference/api-proxies/deployment/get-deploy-status) - Check deploy status after applying connection settings
- [Get Worker Route Health](/api-reference/api-proxies/deployment/get-route-health) - Verify the route is live on worker pods
