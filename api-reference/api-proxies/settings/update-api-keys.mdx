...
description: "Updates the API Key (Public Key) and Secret Key for an API Proxy. These keys are used for API Proxy authentication and identification."
---
...
{
  "status": "FAILURE",
  "resultMessage": "At least one key (publicKey or secretKey) must be provided"
}
```
