...
body. The other four operations rewrite the matched value: `MASK` replaces it with `***`, `DELETE`
removes it, `HASH` stores a one-way hash, `ENCRYPT` stores a reversible encrypted value.
