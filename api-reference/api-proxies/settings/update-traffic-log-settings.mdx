...
Only the fields present in the body are applied. An omitted field (or one sent as `null`) keeps its stored value, so a call that changes one setting never resets the others. The **Default** column below is what a newly created settings document starts with, not what an omitted field falls back to on update. `connectorSettingsMap` and `methodOverrideList` are sent as a whole: what you send replaces what is stored, and an empty object/array clears it.
...
| connectorSettingsMap | object | No | {} | Per-connector settings. **Key = connection name** (as defined in Connections). Replaces the stored object when sent. See below. |
| methodOverrideList | array | No | [] | Method-level connector overrides. Replaces the stored array when sent. See below. |
| retentionProfile | string | No | null | Data retention profile: `STANDARD`, `NO_PAYLOAD`, `NO_PERSIST`. `null` inherits from project and global settings. |
