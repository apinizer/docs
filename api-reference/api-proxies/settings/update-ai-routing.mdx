...
:::note `USE_CLIENT` is not honored on Azure OpenAI
...
| semanticEmbeddingProviderRefName | string | Name-based sidecar for `semanticEmbeddingProviderRef`. Resolved to an id within the project when present (Apinizer 2026.09.4) |
...
- `llmProviderRefName` and `semanticEmbeddingProviderRefName` (and the name sidecars on failover entries, primary pool entries, and conditional routes) are resolved to connection ids within the project before save. A blank sidecar leaves the corresponding id field untouched; an unresolvable name does the same — it does not clear or reject the id by itself. If the id left in place (from the body, or unchanged because the name did not resolve) is missing or belongs to a different project, the save is rejected with error key `aiRouting.providerOutOfScope`.
...
| 400 | A provider id referenced anywhere in `aiRouting` (primary, failover, pool, conditional route, or the semantic embedding provider) is missing or belongs to a different project (error key `aiRouting.providerOutOfScope`). Missing and out-of-scope ids share this key so the response does not reveal whether an id exists in another project. |
