...
- **Global Settings Group override**: if the API proxy is attached to a Global Settings Group that defines its own maintenance mode, that group's values take precedence — a value sent through this endpoint is overwritten by the group's values the next time the group or the attachment is saved. Detach the proxy from the group, or edit the maintenance mode on the group itself, to change the effective values. This also applies when the proxy is created with a project's default Global Settings Group already assigned: the values sent at creation time are immediately overwritten by the default group's values.
