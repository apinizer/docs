...
A plain metadata update that does not set `fixSoapApiPortType` continues to work normally on AI proxies. The AI proxy runtime does not read the classic `routing` object; the equivalent configuration lives under `aiRouting`. Use `PUT /apiops/projects/{projectName}/apiProxies/{apiProxyName}/ai-routing/` instead.
:::
