...
description: "Updates metadata for an API Proxy, including name, description, categories, and sharing type. Metadata is used for API Proxy organization, discovery, and access control."
---
...
:::warning
When the request body includes `fixSoapApiPortType`, this endpoint returns **HTTP 400** for API proxies with `type=AI` — `fixSoapApiPortType` is a routing-shaped field and the guard only triggers when it is present in the body. In earlier releases this returned `200 OK` but had **no effect** (a silent no-op) for AI proxies, which is a behavior change that existing CI/CD pipelines may be affected by.

A plain metadata update that does not set `fixSoapApiPortType` continues to work normally on AI proxies. The AI proxy runtime does not read the classic `routing` object; the equivalent configuration lives under `aiRouting`. Use [Update AI Routing](/api-reference/api-proxies/settings/update-ai-routing) instead.
:::
...
{
  "status": "FAILURE",
  "resultMessage": "ApiProxy (name: Updated API Name) is already exist!"
}
```
