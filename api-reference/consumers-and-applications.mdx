...
| `trafficLogDisabled` | boolean | The consumer's traffic is not written to any traffic log connector. Read-only here; the write lives on the [Credentials API](/api-reference/credentials). Absent on a record where it was never set. |
...
:::note What `trafficLogDisabled` covers
It silences the **traffic log connectors only** — Elasticsearch, Kafka, Syslog, Database and the rest. Trace records, OpenTelemetry spans, Prometheus metrics and the security/access events are unaffected: a privacy preference must not be able to erase a security or audit trail. For zero retention, use the `no-persist` data retention profile instead.

A consumer counts as silenced when **either** its own flag **or** its application's flag is on, so a `false` here does not on its own mean the consumer is being logged — check the owning application too.
:::
...
| `trafficLogDisabled` | boolean | The traffic of every consumer bound to this application is not written to any traffic log connector. Read-only on every Management API surface — it is set from the Manager application screen. Same narrow scope as the consumer field above. Absent on a record where it was never set. |
...
- [Usage Reports API](/api-reference/usage-reports) - What a consumer or an application consumed in a period, and what its live quota budgets say is left
