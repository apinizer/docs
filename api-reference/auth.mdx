...
- **Account State**: An issued token is only as valid as the account behind it. Locking or deactivating the user under **Users** stops every token that user holds from being accepted on the next call — there is no grace period and the tokens do not need to be revoked separately. The refusal is a 401 with the same body as an unknown token, so it never reveals that the login exists.
