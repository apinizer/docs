...
description: "Retrieves all connections for a specific project. Secret fields (passwords, API keys, etc.) are masked and returned as null in list operations."
---
...
- **Secret Fields**: Secret fields (password, apiKey, etc.) are masked and returned as `null` in list operations and in [Get Connection](/api-reference/connections/crud/get-connection) alike; secrets are write-only through the API
- **Get Single Connection**: Use [Get Connection](/api-reference/connections/crud/get-connection) to retrieve the full non-secret configuration of one connection
...
- [Get Connection](/api-reference/connections/crud/get-connection) - Get detailed connection information
