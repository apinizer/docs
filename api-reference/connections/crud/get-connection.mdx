...
description: "Retrieves detailed information about a specific connection. Secret fields (passwords, passphrases, API keys, certificate/keystore files) are never returned; they are write-only through the API."
---
...
- **Secret Fields**: Secret fields are omitted from the response, exactly as in the list endpoint. Earlier releases returned the stored value here (the encrypted form, or plain text for rows created through older releases); that is no longer the case.
- **Round trip**: Because secrets are absent from a GET response, sending that response back with [Update Connection](/api-reference/connections/crud/update-connection) keeps the stored secrets unchanged (an absent secret field means "keep").
...
- [List Connections](/api-reference/connections/crud/list-connections) - List all connections (secrets masked)
