...
- **References**: References to certificates or keystores can be updated by providing new names. A name is resolved within the calling project and then the shared `admin` project. An id supplied directly (`certificateId`, `keyStoreId`, `trustStoreId`) is validated against that same scope: an id belonging to another project is rejected with a "not found" validation error, so a connection cannot be bound to a certificate or key store the project cannot see
