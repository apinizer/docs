...
description: "LDAP (Lightweight Directory Access Protocol) connection for authenticating users and querying directory services. Supports SSL/TLS encryption, certificate validation, and various search scopes. Used by authentication policies and connectors to integrate with LDAP-compatible directory services like Active Directory, OpenLDAP, and others."
---
...
**Note:** `password` is never returned (list or single get). On update, omit the field to keep the stored password, send `""` to clear it, or send a value to replace it — see [Update Connection](/api-reference/connections/crud/update-connection).
...
```json
{
  ...
  "useSsl": false,
  "selectedEnvironmentId": null,
  "directoryVendor": "ACTIVE_DIRECTORY"
}
```
...
| requireCertificateType | string | No | NOT_REQUIRED | Certificate requirement type. See [EnumLdapRequireCertificateType](#enumldaprequirecertificatetype-requirecertificatetype) |
...
| searchScope | string | No | SUBTREE | Search scope. See [EnumSearchScope](#enumsearchscope-searchscope) |
| baseDn | string | Yes | - | Base Distinguished Name for searches |
| certificateName | string | No* | null | Certificate name. Required when the selected certificate-validation mode needs a certificate. |
| useSsl | boolean | No | false | Enable SSL/TLS encryption (use `ldaps://` in serverAddress) |
| useTls | boolean | No | false | Upgrade the plain `ldap://` connection with STARTTLS. Omitting the field stores `false` |
...
| directoryVendor | string | No | null | The directory product family, when known. See [Directory Vendor](#directory-vendor-directoryvendor) below. `null`/absent means unknown and is never resolved to a default; omitting the field on update preserves whatever was stored |

### Directory Vendor (directoryVendor)

- `ACTIVE_DIRECTORY` - Active Directory
- `OPENLDAP` - OpenLDAP
- `DS389_FREEIPA` - 389 Directory Server / FreeIPA
- `GENERIC` - Other (standard LDAP)
...
- `certificateName` is required if certificate validation is enabled. Raw `certificateId` is internal and is rejected on the APIops wire.
- `baseDn` is the root DN for LDAP searches (e.g., `dc=example,dc=com`).
- `directoryVendor` is optional and informational — it does not change how the connection binds or searches. Leave it absent/`null` when the product family is unknown; a round-trip GET → PUT preserves whatever value was last stored.
...
**Note:** Request body structure is the same as Create Connection. An absent or `null` `certificateName` preserves a still-valid stored certificate, `""` clears the optional reference, and a non-empty name resolves a replacement. A dangling or out-of-scope stored certificate must be repaired or cleared before the update can write.
...
- **Certificate**:
  - `certificateName` is required if certificate validation is enabled
  - Upload certificate to Apinizer certificate store first
  - APIops never accepts or returns the internal `certificateId`
