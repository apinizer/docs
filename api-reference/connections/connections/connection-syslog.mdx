...
| syslogMaxMessageBytes | integer | No | 0 | Maximum size, in UTF-8 bytes, of a single syslog message this connection sends, measured on the complete syslog message (header, structured data and body) and excluding only the RFC 6587 framing around it — the same quantity rsyslog measures with `global(maxMessageSize=...)`. `0` disables splitting and keeps every record as one message. A positive value causes any record whose message would exceed it to be sent as several syslog messages instead, each carrying a `[chunk i/N id]` prefix in its message body. See [Syslog](/en/integrations/connection-management/syslog#splitting-oversized-messages) for the receiver-side implications, including the RFC 5426 recommendation to stay under 2048 bytes on UDP. |
