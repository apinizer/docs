...
description: "Kafka connection for sending messages to Kafka topics. Used by policies and other components that need to publish messages to Kafka brokers."
---
...
**Note:** Request body structure is the same as Create Connection. `keyStoreName` and `trustStoreName` are the only public reference fields. On update, absent or `null` keeps a valid stored reference, `""` clears it, and a non-empty name replaces it.
...
- **KeyStore/TrustStore**: Use KeyStore names (not IDs). Storage IDs are internal, are not returned by List/Get or exposed in OpenAPI, and any request that contains an ID field is rejected. A dangling or out-of-scope stored reference must be repaired by name or explicitly cleared before another write.
...
- **Sensitive Entries**: A `propertiesMap` entry is masked when its **key** name contains (case-insensitive) `password`, `secret`, `token`, `authorization`, `credential`, `cookie`, `api-key`/`apikey`, or `jaas`, and additionally for the PEM private-key entries `ssl.keystore.key` and `ssl.key` — for example `sasl.jaas.config` or `ssl.keystore.password`. The two Kafka OAuth endpoint addresses whose names contain `token` — exactly `sasl.oauthbearer.token.endpoint.url` and `sasl.oauthbearer.jwks.endpoint.url` — are the only exemptions; there is no general suffix rule. A bare `key`, such as `key.serializer` or `ssl.keystore.location`, is not sensitive. On List and Get, such an entry comes back with `value: null`; the key itself is still listed. On Create (upsert of an existing connection) and Update, a sensitive entry that is omitted or sent with `value: null` keeps the stored value, `""` removes the entry, and any other value replaces it. Non-sensitive entries are always taken from the request as-is — `propertiesMap` is otherwise replaced wholesale.
