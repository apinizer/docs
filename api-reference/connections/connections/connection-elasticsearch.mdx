...
description: "Elasticsearch connection for indexing and querying data in Elasticsearch clusters. Used by policies and other components that need to store logs, metrics, or other data in Elasticsearch. Supports authentication, encryption, index lifecycle management, and index templates."
---
...
```bash
curl -X GET \
  "https://demo.apinizer.com/apiops/projects/MyProject/connections/?type=elasticsearch" \
  ...
  "resultList": [
    {
      ...
      "bulkBufferCapacity": 50000,
      "bulkFlushIntervalMs": 100,
      "bulkMaxBatchSize": 2500,
      "elasticsearchType": "READ_WRITE",
      "elasticsearchMajorVersion": "AUTO_DETECT"
    }
  ],
  "resultCount": 1
}
...
```json
{
  ...
  "bulkBufferCapacity": null,
  "bulkFlushIntervalMs": null,
...
| lastStatusCheckFailed | boolean | No | false | Read-only in practice: set by the manager's Live Status Check when Elasticsearch could not be asked (unreachable/unauthorized). While `true`, the `*Created` flags are the last verified values, not the current cluster state (2026.09.3, APNZ-6344). |
...
| indexTemplateLoadFixedBitsetFiltersEagerly | boolean | No | null | Value of the static index setting `index.load_fixed_bitset_filters_eagerly` written into the index template. `null` keeps the Elasticsearch default (`true`); `false` lowers heap usage for nested fields. Static setting — applies only to indices created after the template is pushed again (next rollover). |
| connectionTimeoutInMs | integer | No | 5000 | Connection timeout in milliseconds |
| bulkBufferCapacity | integer | No | 50000 | Maximum number of log entries to buffer before dropping |
| bulkFlushIntervalMs | integer | No | 100 | Interval in milliseconds between bulk flush operations |
| bulkMaxBatchSize | integer | No | 2500 | Maximum number of documents per bulk request |
| elasticsearchType | string | No | READ_WRITE | Elasticsearch connection type. See EnumElasticsearchType |
| elasticsearchMajorVersion | string | No | AUTO_DETECT | Elasticsearch major version. See EnumElasticsearchMajorVersion |
...
### EnumElasticsearchMajorVersion

- `AUTO_DETECT` - Automatically detect the Elasticsearch version at runtime
- `ES_7` - Elasticsearch 7.x
- `ES_8` - Elasticsearch 8.x
- `ES_9` - Elasticsearch 9.x
...
- **Connection Settings**:
  - `connectionTimeoutInMs` - Connection timeout (default: 5000ms)
  - `bulkBufferCapacity` - Maximum log entries to buffer (default: 50000)
  - `bulkFlushIntervalMs` - Bulk flush interval (default: 100ms)
  - `bulkMaxBatchSize` - Maximum documents per bulk request (default: 2500)

- **Elasticsearch Version**:
  - `elasticsearchMajorVersion` - Set to `AUTO_DETECT` (default), `ES_7`, `ES_8`, or `ES_9`
  - Auto-detection queries the Elasticsearch cluster at runtime
