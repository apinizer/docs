...
- **Sensitive Entries**: A `headerList` header is masked when its **name** contains (case-insensitive) `password`, `secret`, `token`, `authorization`, `credential`, `cookie`, `api-key`/`apikey`, or `jaas` — for example `Authorization` or `X-Api-Key`. A name ending in `.url`, `.uri`, or `.endpoint` is never treated as sensitive. On List and Get, such a header comes back with `value: null`; the header name itself is still listed. On Create (upsert of an existing connection) and Update, a sensitive header that is omitted or sent with `value: null` keeps the stored value, `""` removes the header, and any other value replaces it. Header matching is by name, case-insensitive, first occurrence; non-sensitive headers are always taken from the request as-is — `headerList` is otherwise replaced wholesale.
