...
description: "Webhook connection for sending HTTP/HTTPS requests to external webhook endpoints. Supports all HTTP methods (GET, POST, PUT, DELETE, PATCH, HEAD, OPTIONS) with custom headers and connection pooling. Used by policies and connectors to send notifications or data to external systems via HTTP webhooks."
---
  ...
  "resultList": [
    {
      ...
      "webhookKeyStoreName": "webhook-client-keystore",
      "webhookTrustStoreName": "corp-ca-truststore",
      "headerList": [
        {
          "name": "Content-Type",
...
`Authorization` comes back as `value: null` — its name looks sensitive, so List and Get mask it the same way a Kafka `propertiesMap` entry is masked. See **Sensitive Entries** under Notes and Warnings below.
...
| webhookKeyStoreName | string | No | - | KeyStore name for the HTTPS client certificate |
| webhookTrustStoreName | string | No | - | KeyStore name used to verify the HTTPS server certificate |
...
**Note:** Request body structure is the same as Create Connection. Webhook key/trust-store references are name-only: absent or `null` preserves a valid stored reference, `""` clears it, and a non-empty name replaces it. Internal `webhookKeyStoreId` and `webhookTrustStoreId` fields are rejected and never returned.
...
- **Sensitive Entries**: A `headerList` header is masked when its **name** contains (case-insensitive) `password`, `secret`, `token`, `authorization`, `credential`, `cookie`, `api-key`/`apikey`, or `jaas` — for example `Authorization` or `X-Api-Key`. There are no exemptions for header names: any name that contains one of those words is masked. On List and Get, such a header comes back with `value: null`; the header name itself is still listed. On Create (upsert of an existing connection) and Update, a sensitive header that is omitted or sent with `value: null` keeps the stored value, `""` removes the header, and any other value replaces it. Header matching is by name, case-insensitive, first occurrence; non-sensitive headers are always taken from the request as-is — `headerList` is otherwise replaced wholesale.
