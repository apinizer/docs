...
description: "Email connection for sending emails via SMTP. Used by policies and other components that need to send email notifications."
---
  ...
  "resultList": [
    {
      ...
      "enableStartTls": true,
      "certificateName": "smtp-ca-certificate",
      "auth": true,
...
**Note:** `password` is never returned by get or list operations (`null`). Omit it on update to keep the stored value; send `""` to clear it.
...
| certificateName | string | No | - | Certificate name used to validate the SMTP server |
...
**Note:** Request body structure is the same as Create Connection. `certificateName` is the only public certificate reference: absent or `null` preserves a valid stored reference, `""` clears it, and a non-empty name replaces it. Internal `certificateId` is rejected and never returned.
...
- **Certificate Reference**: Use `certificateName`; APIops never accepts or returns the internal certificate ID
