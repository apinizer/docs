...
| overridden | boolean | Response-only. `true` when an admin has edited this built-in row through the API or Manager UI — the versioned rule-pack loader then skips it on every future pack refresh instead of overwriting the edit. Always `false`/absent for custom (`builtIn: false`) rows. See [Built-in Presets Are Editable](/api-reference/ai-dlp-presets#built-in-presets-are-editable). |
