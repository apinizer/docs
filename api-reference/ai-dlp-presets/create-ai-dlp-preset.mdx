...
description: "Creates a custom AI DLP (secret/credential) preset, or updates it when one with the same name already exists in the scope (upsert by name). Requires an admin API token. A name that resolves to a built-in preset is updated too — built-in presets are not read-only on this endpoint."
---
...
- **Overriding a Built-in Row**:
  - Upserting onto an existing built-in row flags it `overridden: true` in storage — the next platform upgrade's rule-pack refresh then skips it instead of overwriting your edit. See [Built-in Presets Are Editable](/api-reference/ai-dlp-presets#built-in-presets-are-editable) on the resource overview page.
- **Regex Pattern**:
