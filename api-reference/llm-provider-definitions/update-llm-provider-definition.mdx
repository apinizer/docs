...
| wireProtocol | string | No | `null` | Adapter family this provider's wire format resolves against — one of `OPENAI_COMPATIBLE`, `ANTHROPIC`, `GEMINI`, `BEDROCK`, `VLLM` |
| defaultChatCompletionPath | string | No | `null` | Default chat-completion endpoint path |
...
- This endpoint replaces the full row — a field omitted from the request body is saved as `null`, it is not left at its previous value. When updating a custom definition that already has `wireProtocol` / path fields set, include them again in the body or they will be cleared
