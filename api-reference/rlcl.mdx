...
The Rate Limit Control List is retired as of 2026.09: the gateway no longer evaluates it. Existing lists were migrated automatically to typed Limit Plans and assignments at upgrade — see [Legacy Limits Migrated at Upgrade](/en/ai-gateway/limit-plans#legacy-limits-migrated-at-upgrade). [Create RLCL](/api-reference/rlcl/crud/create-rlcl) and [Update RLCL](/api-reference/rlcl/crud/update-rlcl) now always return `400` with a `resultMessage` naming the retirement (error key `credentialRateLimit.retired`). [List RLCLs](/api-reference/rlcl/crud/list-rlcls), [Get RLCL](/api-reference/rlcl/crud/get-rlcl), and [Delete RLCL](/api-reference/rlcl/crud/delete-rlcl) are unaffected — read and delete stay available for audit and cleanup.
...
|-----------|---------------------|
| List RLCLs / Get RLCL | `IDENTITY` + `VIEW` |
