...
**Shipped categories are editable and deletable.** A deleted shipped category returns on the next version upgrade (`if (!exists) save` seed contract). Rewording one is allowed but breaks its link to the cited MLCommons standard.
