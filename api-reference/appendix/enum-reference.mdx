...
| `EMPTY_API` | No-Spec API — no specification file; endpoints (API methods) are defined manually one by one via the [Create Endpoint](/api-reference/api-proxies/endpoints/create-endpoint) API after proxy creation. Supported only by [Create API Proxy from URL](/api-reference/api-proxies/crud/create-api-proxy-from-url) (`specUrl` omitted) — rejected by the file-upload endpoints and by reparse (`reParse: true`). |
