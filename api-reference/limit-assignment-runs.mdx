...
A plan key that resolves to no plan is `404 resourceNotFound` (`params.plan`). `AI_TOKEN_BUDGET` needs no license module: the API layer applies no module gate, so both families behave identically on every installation.
