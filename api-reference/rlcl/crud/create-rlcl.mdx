...
| executionOrder | string | No | FIRST | Execution order. See [EnumExecutionOrder](#enumexecutionorder-executionorder) |
...
- `AUTH_RESOLVED` - Identity is taken from the username/key resolved by the authentication policy. When selected, `executionOrder` is forced to `LAST` and `targetVariable` is ignored (the API must have an authentication policy running last)
...
- When `identitySource` is `AUTH_RESOLVED`, `executionOrder` is automatically set to `LAST` and `targetVariable` is ignored
