...
| outOfTargetAction | string | No | BLOCK | Behaviour for requests outside the target audience. Omitting it keeps the fail-closed default `BLOCK`. See [EnumRateLimitOutOfTargetAction](#enumratelimitoutoftargetaction-outoftargetaction) |
...
- `BLOCK` - Requests outside the target audience are blocked (default when the field is omitted)
- `GENERAL_QUOTA` - A general quota is applied to requests outside the target audience
- `NONE` - The list does not apply to requests outside the target audience: they are neither blocked nor counted
...
{
  "status": "FAILURE",
  "resultMessage": "An RLCL with same name (PremiumUserRLCL) already exists in project!"
}
```
...
{
  "status": "FAILURE",
  "resultMessage": "generalQuotaPermittedMessageCount must be greater than 0 when outOfTargetAction is GENERAL_QUOTA!"
}
```
