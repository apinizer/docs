---
title: "Create JWK"
description: "Creates a new JWK (JSON Web Key) and deploys it to environments. JWKs can be created from various sources including URLs, JSON strings, certificates, keys, keystores, or generated programmatically."
---
## Endpoint
...
:::note
The referenced key must be a `PUBLIC_KEY` / `PRIVATE_KEY` whose content is PEM-encoded. If it was created via [Create Key](/api-reference/keys-secrets/crud/create-key), supply its content as base64-encoded PEM (or raw PEM) so it is stored in PEM form; otherwise JWK creation fails with `No PEM-encoded keys found`.
:::
...
- For `GENERATE` source type, use the [Generate JWK](/api-reference/keys-secrets/crud/generate-jwk) endpoint
...
{
  "status": "FAILURE",
  "resultMessage": "JWK name can not be empty!"
}
```
...
{
  "status": "FAILURE",
  "resultMessage": "JWK (name: my-jwk) is already exist! Try update operation if want to change its value."
}
```
...
{
  "status": "FAILURE",
  "resultMessage": "Source type is required!"
}
```
...
{
  "status": "FAILURE",
  "resultMessage": "URL is required for URL source type!"
}
```
...
- [Update JWK](/api-reference/keys-secrets/crud/update-jwk) - Update a JWK
