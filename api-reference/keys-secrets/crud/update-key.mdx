...
| algorithm | string | Conditional | Symmetric key algorithm: `AES`, `DES`, `DESede`, or `RSA`. Omit it (or send `null`) to leave the stored algorithm unchanged. **Required** if the key's `keyType` is (or is being changed to) `SECRET_KEY` and it does not already have an algorithm — the update is rejected with 400 otherwise. |
...
- **Key Content format**: for `PRIVATE_KEY` / `PUBLIC_KEY`, send the PEM material base64-encoded (raw PEM is also accepted) — the server decodes it back to PEM before storage; for `SECRET_KEY`, send the base64-encoded key bytes, which are stored as-is. Content that is neither valid base64 nor already PEM is rejected. See [Create Key](/api-reference/keys-secrets/crud/create-key) for details.
- Omit `cryptoKeyInfoEnvironmentList` to leave the stored key content untouched; only supplied environment content is re-processed.
- **Algorithm PATCH semantics**: `algorithm` follows the same partial-update rule as the other fields — `null`/omitted leaves the current value untouched. It only becomes mandatory when the key ends up as `keyType: SECRET_KEY` without a stored algorithm (e.g. changing `keyType` to `SECRET_KEY` in the same request). Same values as [Create Key](/api-reference/keys-secrets/crud/create-key): `AES`, `DES`, `DESede`, `RSA`, case-sensitive.
...
- **Algorithm**:
  - Omitted/`null` leaves the stored algorithm untouched
  - Becomes required (400 if missing) when the key's `keyType` is or becomes `SECRET_KEY` without an existing algorithm — see [Create Key](/api-reference/keys-secrets/crud/create-key) for the exact error message and supported values
