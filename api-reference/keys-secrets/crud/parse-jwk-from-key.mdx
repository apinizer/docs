---
title: "Parse JWK from Key"
description: "Parses a JWK (JSON Web Key) from an existing key (public or private) and creates it. This is a convenience endpoint for extracting JWKs from keys."
---
...
- **Source key must be PEM**: this endpoint only works with a `PUBLIC_KEY` / `PRIVATE_KEY` whose stored content is PEM-encoded. When the referenced key was created via [Create Key](/api-reference/keys-secrets/crud/create-key), make sure its content was supplied as base64-encoded PEM (or raw PEM) so it is stored in PEM form — otherwise JWK parsing fails with `No PEM-encoded keys found`. `SECRET_KEY` keys cannot be converted to a JWK through this endpoint.
...
- [Create JWK](/api-reference/keys-secrets/crud/create-jwk) - Create JWK with source type PUBLIC_KEY or PRIVATE_KEY
