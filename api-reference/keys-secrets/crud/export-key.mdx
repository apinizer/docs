...
- `Content-Disposition: attachment; filename="<keyName>.zip"`
...
- **File Naming**:
  - The download is named after the key, for example `signing-key.zip`
  - The name is sanitized, so a key whose name contains a slash or another reserved character still produces a valid file name
