...
| builtIn | boolean | No | false | Leave `false` for custom entries. See the built-in note below. The pre-2026.04 name `isBuiltIn` is still accepted. |
...
- **Built-in entries (`builtIn=true`)**: Only the price fields
...
- **Built-in price-only edit**: When `builtIn=true`, everything except the price fields is ignored
